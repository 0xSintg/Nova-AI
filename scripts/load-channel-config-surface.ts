...
const jitiFactoryOverrideKey = Symbol.for("nova-ai.channelConfigSurfaceJitiFactoryOverride");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.NOVA_AI_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing NOVA_AI_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        NOVA_AI_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadViaJiti = (candidatePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "nova-ai/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap({
        modulePath: resolvedPath,
        pluginSdkResolution: "src",
      }),
