...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.NOVA_AI_VITEST_MAX_WORKERS || env.NOVA_AI_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      NOVA_AI_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.NOVA_AI_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.NOVA_AI_TEST_PROJECTS_PARALLEL &&
        !baseEnv.NOVA_AI_VITEST_MAX_WORKERS &&
        !baseEnv.NOVA_AI_TEST_WORKERS &&
