...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.NOVA_AI_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "nova-ai.plugin.json");
    ...
    if (packageJson.nova-ai && "extensions" in packageJson.nova-ai) {
      packageJson.nova-ai = {
        ...packageJson.nova-ai,
        extensions: rewritePackageExtensions(packageJson.nova-ai.extensions),
        ...(typeof packageJson.nova-ai.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.nova-ai.setupEntry) }
          : {}),
      };
    }
