...
const logLevel = process.env.NOVA_AI_BUILD_VERBOSE ? "info" : "warn";
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveInteger(env.NOVA_AI_TSDOWN_TIMEOUT_MS);
  const heartbeatMs =
    parseNonNegativeInteger(env.NOVA_AI_TSDOWN_HEARTBEAT_MS) ?? DEFAULT_HEARTBEAT_MS;
  let timedOut = false;
  let settled = false;
