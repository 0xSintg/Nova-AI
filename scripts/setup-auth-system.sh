#!/bin/bash
# Setup Nova AI Auth Management System
...
echo "=== Nova AI Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'nova-ai-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Nova AI message: Send warning via Nova AI itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/nova-ai-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/nova-ai-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/nova-ai-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now nova-ai-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/NovaAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/NovaAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Nova*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (NovaAuth) shows status and opens auth URL if needed."
echo "The full widget (NovaAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status nova-ai-auth-monitor.timer"
echo ""
echo "Quick commands:"
