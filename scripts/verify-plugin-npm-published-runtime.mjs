...
export function collectPluginNpmPublishedRuntimeErrors(params) {
  ...
  const runtimeExtensionsResult = readPackageStringList(
    packageLabel,
    "nova-ai.runtimeExtensions",
    packageJson.nova-ai?.runtimeExtensions,
  );
  const setupEntryResult = readOptionalPackageString(
    packageLabel,
    "nova-ai.setupEntry",
    packageJson.nova-ai?.setupEntry,
  );
  const runtimeSetupEntryResult = readOptionalPackageString(
    packageLabel,
    "nova-ai.runtimeSetupEntry",
    packageJson.nova-ai?.runtimeSetupEntry,
  );
  errors.push(
  ...
  if (runtimeExtensions.length > 0 && runtimeExtensions.length !== extensions.length) {
    errors.push(
      `${packageLabel} package.json nova-ai.runtimeExtensions length (${runtimeExtensions.length}) must match nova-ai.extensions length (${extensions.length})`,
    );
    return errors;
  }
  ...
  if (runtimeSetupEntry && !setupEntry) {
    errors.push(
      `${packageLabel} package.json nova-ai.runtimeSetupEntry requires nova-ai.setupEntry`,
    );
    return errors;
  }
...
async function packPublishedPackage(spec, destinationDir) {
  const attempts = Number.parseInt(process.env.NOVA_AI_PLUGIN_NPM_VERIFY_ATTEMPTS ?? "90", 10);
  const delayMs = Number.parseInt(process.env.NOVA_AI_PLUGIN_NPM_VERIFY_DELAY_MS ?? "10000", 10);
  let lastError;
...
export async function verifyPublishedPluginRuntime(spec) {
  const workingDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-npm-runtime."));
  try {
