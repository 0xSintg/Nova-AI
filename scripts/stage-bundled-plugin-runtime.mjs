...
function ensureNova AIExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "nova-ai");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "nova-ai.plugin.json") ||
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureNova AIExtensionAlias({ repoRoot, distExtensionsRoot });
