...
const PUBLISHED_INSTALLER_BASE_URL = "https://nova-ai.com";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "NOVA_AI_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
const CROSS_OS_AGENT_TURN_OPTIONAL = parseBooleanEnv("NOVA_AI_CROSS_OS_AGENT_TURN_OPTIONAL", true);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `NOVA_AI_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.NOVA_AI_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.nova-ai-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "NOVA_AI_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.NOVA_AI_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.NOVA_AI_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.NOVA_AI_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `nova-ai@${previousVersion}` : "nova-ai@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.NOVA_AI_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.NOVA_AI_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        NOVA_AI_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `nova-ai-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".nova-ai");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    NOVA_AI_HOME: lane.homeDir,
    NOVA_AI_STATE_DIR: lane.stateDir,
    NOVA_AI_CONFIG_PATH: join(lane.stateDir, "nova-ai.json"),
    NOVA_AI_DISABLE_BONJOUR: "1",
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.NOVA_AI_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]nova-ai\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/nova-ai[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "nova-ai@latest") {
    return "";
  }
  if (trimmed.startsWith("nova-ai@")) {
    return trimmed.slice("nova-ai@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "nova-ai@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view nova-ai@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'nova-ai.cmd'),
      (Join-Path $npmPrefix 'nova-ai.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command nova-ai -ErrorAction Stop
...
Write-Output "__NOVA_AI_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__NOVA_AI_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed nova-ai path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v nova-ai >/dev/null 2>&1",
    'printf "__NOVA_AI_PATH__=%s\\n" "$(command -v nova-ai)"',
    "nova-ai --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__NOVA_AI_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed nova-ai path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `nova-ai update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.nova-ai/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.NOVA_AI_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.NOVA_AI_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.NOVA_AI_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.NOVA_AI_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "nova-ai/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.NOVA_AI_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.NOVA_AI_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.NOVA_AI_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing NOVA_AI_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "NOVA_AI_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.NOVA_AI_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.NOVA_AI_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      NOVA_AI_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      NOVA_AI_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Nova AI Control</title>") &&
          html.includes("<nova-ai-app></nova-ai-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Nova AI Control</title>")} app=${html.includes("<nova-ai-app></nova-ai-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runNova AI(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "nova-ai.mjs");
}
