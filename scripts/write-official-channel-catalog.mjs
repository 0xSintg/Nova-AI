...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.nova-ai) ? packageJson.nova-ai : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.nova-ai?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.nova-ai?.channel?.id) || left.name;
    const rightId = trimString(right.nova-ai?.channel?.id) || right.name;
