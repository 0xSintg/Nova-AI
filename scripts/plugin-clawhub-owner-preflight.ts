...
import { collectClawHubNova AIOwnerErrors } from "./lib/plugin-clawhub-release.ts";
...
export async function runClawHubOwnerPreflight(argv: string[]) {
  ...
  const errors = await collectClawHubNova AIOwnerErrors({ plugins: candidates });
  if (errors.length > 0) {
    throw new Error(
      `ClawHub Nova AI package ownership preflight failed:\n${errors.map((error) => `- ${error}`).join("\n")}`,
    );
  }

  console.log(`ClawHub Nova AI owner preflight passed for ${candidates.length} candidate(s).`);
}
