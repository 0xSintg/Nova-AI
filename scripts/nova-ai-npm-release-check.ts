...
const EXPECTED_REPOSITORY_URL = "https://github.com/nova-ai/nova-ai";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@nova-ai/fs-safe";
...
const skipPackValidationEnv = "NOVA_AI_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "nova-ai") {
    errors.push(`package.json name must be "nova-ai"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.nova-ai !== "nova-ai.mjs") {
    errors.push(
      `package.json bin.nova-ai must be "nova-ai.mjs"; found "${pkg.bin?.nova-ai ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`nova-ai-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `nova-ai-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
