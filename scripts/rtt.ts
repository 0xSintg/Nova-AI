...
function usage() {
  return [
    "Usage: pnpm rtt <nova-ai@spec> [--package-tgz PATH] [--provider mock-openai|live-frontier] [--credential-source env|convex] [--credential-role maintainer|ci] [--runs N] [--samples N] [--sample-timeout-ms N] [--timeout-ms N] [--harness-root PATH] [--output PATH]",
    "",
    "Examples:",
    "  pnpm rtt nova-ai@main --package-tgz .artifacts/package/nova-ai.tgz",
    "  pnpm rtt nova-ai@beta",
    "  pnpm rtt nova-ai@2026.4.30",
    "  pnpm rtt nova-ai@latest --provider live-frontier",
  ].join("\n");
}
...
function parseArgs(argv: string[]) {
  ...
  let harnessRoot = "~/Developer/novabot";
  let output = "runs";
  ...
  return {
    spec: validateNova AIPackageSpec(spec),
    options: {
      packageTgz,
      credentialRole,
      credentialSource,
...
async function main() {
  ...
  if (spec === "nova-ai@main" && !options.packageTgz) {
    throw new Error("nova-ai@main requires --package-tgz.");
  }
  const version =
    spec === "nova-ai@main"
