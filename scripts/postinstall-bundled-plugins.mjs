...
const DISABLE_POSTINSTALL_ENV = "NOVA_AI_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "NOVA_AI_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallNova AIHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.NOVA_AI_HOME?.trim();
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.nova-ai-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const openClawHome = resolvePostinstallNova AIHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  const configPath = env?.NOVA_AI_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, openClawHome)));
  }
  addStateRoot(join(openClawHome, ".nova-ai"));
  addStateRoot(join(openClawHome, ".novabot"));
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.nova-ai-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneNova AICompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune Nova AI compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
