...
# Nova AI Auth Widget for Termux
...
SERVER="${NOVA_AI_SERVER:-l36}"

# Check auth status
termux-toast "Checking Nova AI auth..."

STATUS=$(ssh "$SERVER" '$HOME/nova-ai/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/nova-ai/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|NOVA_AI_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/nova-ai/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|NOVA_AI_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Nova AI won't work until re-auth"
                ;;
        esac
        ;;
