...
export function shouldSkipAppLintForMissingSwiftlint(options = {}) {
  ...
  return (
    isTruthyEnvFlag(env.NOVA_AI_TESTBOX_REMOTE_RUN) && platform !== "darwin" && !swiftlintAvailable
  );
}

export function shouldDelegateChangedCheckToCrabbox(argv = [], env = process.env) {
  if (!isTruthyEnvFlag(env.NOVA_AI_TESTBOX)) {
    return false;
  }
  if (isTruthyEnvFlag(env.NOVA_AI_TESTBOX_REMOTE_RUN)) {
    return false;
  }
...
export async function runChangedCheckViaCrabbox(argv = [], env = process.env) {
  console.error(
    "[check:changed] NOVA_AI_TESTBOX=1 set; delegating to Blacksmith Testbox via `pnpm crabbox:run`.",
  );
...
export function createChangedCheckPlan(result, options = {}) {
  ...
  if (lanes.liveDockerTooling) {
    ...
    addCommand("live Docker scheduler dry run", "node", ["scripts/test-docker-all.mjs"], {
      ...baseEnv,
      NOVA_AI_DOCKER_ALL_DRY_RUN: "1",
      NOVA_AI_DOCKER_ALL_LIVE_MODE: "only",
    });
  }
