...
# Nova AI CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
PREFIX="${NOVA_AI_PREFIX:-${HOME}/.nova-ai}"
NOVA_AI_VERSION="${NOVA_AI_VERSION:-latest}"
NODE_VERSION="${NOVA_AI_NODE_VERSION:-22.22.0}"
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${NOVA_AI_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${NOVA_AI_INSTALL_METHOD:-npm}"
GIT_DIR="${NOVA_AI_GIT_DIR:-${HOME}/nova-ai}"
...
Usage: install-cli.sh [options]
  ...
  --version <ver>                     Nova AI version (default: latest)
...
Environment variables:
  ...
  NOVA_AI_VERSION=latest|next|<semver>
  NOVA_AI_GIT_DIR=...
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${NOVA_AI_GIT_DIR:-${HOME}/nova-ai}}"
...
resolve_git_nova-ai_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "nova-ai" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_nova-ai() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"nova-ai\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing Nova AI (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS" "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "nova-ai@latest"; then
      log "npm install nova-ai@latest failed; retrying nova-ai@next"
      emit_json "{\"event\":\"step\",\"name\":\"nova-ai\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS" "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "nova-ai@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "SHARP_IGNORE_GLOBAL_LIBVIPS=$SHARP_IGNORE_GLOBAL_LIBVIPS" "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "nova-ai@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/nova-ai"
  cat > "${PREFIX}/bin/nova-ai" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/nova-ai/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/nova-ai"
  emit_json "{\"event\":\"step\",\"name\":\"nova-ai\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_nova-ai_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"nova-ai\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
...
EOF
  chmod +x "${PREFIX}/bin/nova-ai"
  emit_json "{\"event\":\"step\",\"name\":\"nova-ai\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_nova-ai_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/nova-ai" ]]; then
    version="$("${PREFIX}/bin/nova-ai" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
main() {
  ...
  if [[ "${NOVA_AI_NO_ONBOARD:-0}" == "1" ]]; then
    RUN_ONBOARD=0
  fi
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_nova-ai_from_git "$GIT_DIR"
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "Nova AI installed (${installed_version})."
  else
    emit_json "{\"event\":\"done\",\"ok\":true}"
    log "Nova AI installed."
  fi

  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/nova-ai" onboard
  fi
}

if [[ "${NOVA_AI_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
