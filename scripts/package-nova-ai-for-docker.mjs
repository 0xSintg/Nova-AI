#!/usr/bin/env node
// Builds the Nova AI package artifact used by Docker E2E.
...
async function newestNova AITarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Nova AI tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building Nova AI package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building Nova AI Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing Nova AI package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing Nova AI package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestNova AITarball(outputDir, packOutput);
  ...
  console.error("==> Checking Nova AI package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-nova-ai-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
