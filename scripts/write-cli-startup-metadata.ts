...
import type { Nova AIConfig } from "../src/config/config.js";
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.nova-ai?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.nova-ai?.channel?.order;
      const labelRaw = parsed.nova-ai?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".nova-ai-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "nova-ai-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "nova-ai-build",
...
function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "nova-ai", "browser", "--help"]);`,
  ...
  const result = spawnSync(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", inlineModule],
    {
      ...
      env: {
        ...renderContext.env,
        NOVA_AI_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      },
