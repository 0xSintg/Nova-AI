...
const DEFAULT_OUTPUT_NAME = "nova-ai-current.tgz";
export const NOVA_AI_PACKAGE_SPEC_RE =
  /^nova-ai@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-nova-ai-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateNova AIPackageSpec(spec) {
  if (!NOVA_AI_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be nova-ai@alpha, nova-ai@beta, nova-ai@latest, or an exact Nova AI release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Nova AI branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `nova-ai-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^nova-ai-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no Nova AI tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-nova-ai-for-docker.mjs",
        "--source-dir",
      ...
      validateNova AIPackageSpec(options.packageSpec);
  ...
  console.error(`Checking Nova AI package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-nova-ai-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `Nova AI package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "nova-ai") {
    throw new Error(`package candidate must be named "nova-ai"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
