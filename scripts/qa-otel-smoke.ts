...
function spawnNova AI(args: string[], env: NodeJS.ProcessEnv): ChildProcess {
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "nova-ai-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.NOVA_AI_QA_SUITE_PROGRESS = env.NOVA_AI_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function assertSmoke(params: {
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("nova-ai.content."));
  ...
  const modelSpans = params.spans.filter((span) => span.name === "nova-ai.model.call");
  const modelErrorSpans = modelSpans.filter((span) => {
    const serialized = JSON.stringify(span.attributes);
    return (
      Object.hasOwn(span.attributes, "error.type") ||
      Object.hasOwn(span.attributes, "nova-ai.errorCategory") ||
      serialized.includes("StreamAbandoned")
    );
  });
  if (modelSpans.length === 0) {
    failures.push("no nova-ai.model.call span was exported");
  }
...
async function main() {
  ...
  let childExitCode = 1;
  try {
    const child = spawnNova AI(buildQaArgs(options), buildQaEnv(port));
