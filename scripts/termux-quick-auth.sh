...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/nova-ai/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Nova AI Re-Auth" -c "After getting key, run: ssh $SERVER '~/nova-ai/scripts/mobile-reauth.sh'" --id nova-ai-auth
        ;;
    *)
