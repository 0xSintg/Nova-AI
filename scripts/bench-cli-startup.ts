...
const DEFAULT_ENTRY = "nova-ai.mjs";
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "nova-ai-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".nova-ai");
...
function printUsage(): void {
  console.log(`Nova AI CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: nova-ai.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "nova-ai-cli-bench-"));
