 ...
 * `nova-ai update`. Use it to verify the completion installation flow
...
\`nova-ai update\`, \`nova-ai doctor\`, and \`nova-ai onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Nova AI Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nova-ai completion --install\` later to enable.`));
    return;
  }
