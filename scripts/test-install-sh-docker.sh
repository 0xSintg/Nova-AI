...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${NOVA_AI_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$NOVA_AI_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.NOVA_AI_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `NOVA_AI_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${NOVA_AI_INSTALL_SMOKE_IMAGE:-nova-ai-install-smoke:local}"
NONROOT_IMAGE="${NOVA_AI_INSTALL_NONROOT_IMAGE:-nova-ai-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${NOVA_AI_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${NOVA_AI_INSTALL_URL:-https://nova-ai.bot/install.sh}"
CLI_INSTALL_URL="${NOVA_AI_INSTALL_CLI_URL:-https://nova-ai.bot/install-cli.sh}"
PACKAGE_NAME="${NOVA_AI_INSTALL_PACKAGE:-nova-ai}"
SKIP_NONROOT="${NOVA_AI_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${NOVA_AI_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${NOVA_AI_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${NOVA_AI_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${NOVA_AI_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
SKIP_FRESHNESS="${NOVA_AI_INSTALL_SMOKE_SKIP_FRESHNESS:-0}"
FRESHNESS_INSTALL_URL="${NOVA_AI_INSTALL_SMOKE_FRESHNESS_INSTALL_URL:-file:///tmp/nova-ai-install.sh}"
# npm min-release-age is days; 10000 keeps the control failure independent of normal release cadence.
FRESHNESS_MIN_RELEASE_AGE="${NOVA_AI_INSTALL_FRESHNESS_MIN_RELEASE_AGE:-10000}"
FRESHNESS_NPM_VERSION="${NOVA_AI_INSTALL_FRESHNESS_NPM_VERSION:-11.14.1}"
UPDATE_BASELINE_VERSION="${NOVA_AI_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${NOVA_AI_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${NOVA_AI_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${NOVA_AI_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${NOVA_AI_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${NOVA_AI_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${NOVA_AI_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${NOVA_AI_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
...
' "$pack_json_file"
  ...
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node scripts/check-nova-ai-package-tarball.mjs "${UPDATE_DIR}/${UPDATE_TGZ_FILE}"
  fi
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (NOVA_AI_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e NOVA_AI_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NOVA_AI_INSTALL_METHOD=npm \
    -e NOVA_AI_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e NOVA_AI_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e NOVA_AI_INSTALL_LATEST_OUT="/out/latest" \
    -e NOVA_AI_NO_ONBOARD=1 \
    -e NOVA_AI_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e NOVA_AI_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NOVA_AI_INSTALL_SMOKE_MODE=update \
    -e NOVA_AI_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e NOVA_AI_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e NOVA_AI_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e NOVA_AI_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e NOVA_AI_NO_ONBOARD=1 \
    -e NOVA_AI_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (NOVA_AI_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e NOVA_AI_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e NOVA_AI_INSTALL_SMOKE_MODE=npm-global \
      -e NOVA_AI_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e NOVA_AI_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e NOVA_AI_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e NOVA_AI_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_FRESHNESS" == "1" ]]; then
  echo "==> Skip installer npm freshness smoke (NOVA_AI_INSTALL_SMOKE_SKIP_FRESHNESS=1)"
else
  prepare_npm_cache
  ...
  docker run --rm -t \
    --platform "$SMOKE_PLATFORM" \
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -v "$ROOT_DIR/scripts/install.sh:/tmp/nova-ai-install.sh:ro" \
    -e NOVA_AI_INSTALL_URL="$FRESHNESS_INSTALL_URL" \
    -e NOVA_AI_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NOVA_AI_INSTALL_SMOKE_MODE=freshness \
    -e NOVA_AI_INSTALL_FRESHNESS_VERSION="${NOVA_AI_INSTALL_FRESHNESS_VERSION:-latest}" \
    -e NOVA_AI_INSTALL_FRESHNESS_MIN_RELEASE_AGE="$FRESHNESS_MIN_RELEASE_AGE" \
    -e NOVA_AI_INSTALL_FRESHNESS_NPM_VERSION="$FRESHNESS_NPM_VERSION" \
    -e NOVA_AI_NO_ONBOARD=1 \
    -e NOVA_AI_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (NOVA_AI_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e NOVA_AI_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e NOVA_AI_INSTALL_METHOD=npm \
    -e NOVA_AI_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e NOVA_AI_NO_ONBOARD=1 \
    -e NOVA_AI_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${NOVA_AI_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (NOVA_AI_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e NOVA_AI_INSTALL_URL="$INSTALL_URL" \
  -e NOVA_AI_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e NOVA_AI_NO_ONBOARD=1 \
  -e NOVA_AI_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
