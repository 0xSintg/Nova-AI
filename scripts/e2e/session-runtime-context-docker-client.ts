...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".nova-ai", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "nova-ai.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("NOVA_AI_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-session-runtime-context-"));
  process.env.HOME = root;
  process.env.NOVA_AI_STATE_DIR = path.join(root, ".nova-ai");
  process.env.NOVA_AI_CONFIG_PATH = path.join(process.env.NOVA_AI_STATE_DIR, "nova-ai.json");
  try {
    ...
    if (process.env.NOVA_AI_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
