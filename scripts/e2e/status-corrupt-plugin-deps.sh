...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nova-ai-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.NOVA_AI_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".nova-ai-rename-tmp");
...
run_nova-ai() {
  HOME="$HOME_DIR" \
  NOVA_AI_HOME="$STATE_DIR" \
  NOVA_AI_STATE_DIR="$STATE_DIR" \
  NOVA_AI_CONFIG_PATH="$CONFIG_PATH" \
  NOVA_AI_PLUGIN_STAGE_DIR="$STAGE_DIR" \
  NOVA_AI_DISABLE_BUNDLED_PLUGINS=1 \
  NOVA_AI_NO_ONBOARD=1 \
  NOVA_AI_NO_PROMPT=1 \
  NOVA_AI_SKIP_CHANNELS=1 \
  NOVA_AI_SKIP_PROVIDERS=1 \
...
run_nova-ai status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run nova-ai doctor --fix" "$BEFORE" >/dev/null

run_nova-ai doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_nova-ai status --all --timeout 1 > "$AFTER"
