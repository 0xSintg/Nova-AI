...
import type { Nova AIConfig } from "../../dist/config/types.nova-ai.js";
...
function makeParams(commandBody: string, cfg: Nova AIConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: Nova AIConfig, isGroup = false): Promise<string> {
...
async function main() {
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-crestodian-")));
  const configPath = process.env.NOVA_AI_CONFIG_PATH ?? path.join(stateDir, "nova-ai.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env NOVA_AI_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/nova-ai-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/nova-ai-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as Nova AIConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "NOVA_AI_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/nova-ai-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/nova-ai-work",
    ),
