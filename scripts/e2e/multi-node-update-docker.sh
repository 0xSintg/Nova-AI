...
# Nova AI under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `nova-ai update`. Verifies that:
#
...
#
# Requires: Docker, a built nova-ai-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="nova-ai-multi-node-update-e2e"
DOCKER_RUN_TIMEOUT="${NOVA_AI_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
ARTIFACT_DIR="${NOVA_AI_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"

mkdir -p "$ARTIFACT_DIR"
chmod -R a+rwX "$ARTIFACT_DIR" || true

# Build the bare e2e image and prepare the package tarball.
docker_e2e_build_or_reuse "$IMAGE_NAME" multi-node-update "$ROOT_DIR/scripts/e2e/Dockerfile" "$ROOT_DIR" "bare" "${NOVA_AI_SKIP_DOCKER_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${NOVA_AI_CURRENT_PACKAGE_TGZ:-}")"
...
docker_e2e_run_with_harness \
  ...
  -e NOVA_AI_SKIP_PROVIDERS=1 \
  -e NOVA_AI_SKIP_CHANNELS=1 \
  -e NOVA_AI_DISABLE_BONJOUR=1 \
...
echo ""
echo "── Step 2: Install Nova AI under node-A ──"

# Use node-A to install nova-ai with npm prefix A.
...
echo "Installing Nova AI package under node-A prefix: $NPM_PREFIX_A"
npm install -g /tmp/nova-ai-current.tgz --no-fund --no-audit >"$ARTIFACTS/install-a.log" 2>&1
echo "Installed. Checking nova-ai location..."

NOVA_AI_A="$(command -v nova-ai)"
echo "nova-ai binary: $NOVA_AI_A"
echo "nova-ai version: $(nova-ai --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/nova-ai"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/nova-ai-gateway.service"
...
nova-ai gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err" || true
...
# but nova-ai is NOT installed there.
...
echo "which nova-ai: $(command -v nova-ai)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run nova-ai update (this is the bug) ──"
...
echo "Running nova-ai update --yes --json..."
UPDATE_EXIT=0
nova-ai update --yes --json \
  --tag /tmp/nova-ai-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: nova-ai update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Nova AI package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Nova AI package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/nova-ai/package.json" ]; then
  echo "WARNING: Nova AI was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Nova AI is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Nova AI root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
...
if [ -f "$GATEWAY_UNIT_PATH" ]; then
  ...
  if [ -s "$GATEWAY_PID_FILE" ] && kill -0 "$(cat "$GATEWAY_PID_FILE" 2>/dev/null)" 2>/dev/null; then
    ...
    # Try a health probe.
    if nova-ai gateway status --json >"$ARTIFACTS/status.json" 2>&1; then
