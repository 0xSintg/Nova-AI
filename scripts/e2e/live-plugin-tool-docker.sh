...
IMAGE_NAME="$(docker_e2e_resolve_image "nova-ai-live-plugin-tool-e2e" NOVA_AI_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${NOVA_AI_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${NOVA_AI_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${NOVA_AI_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${NOVA_AI_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${NOVA_AI_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${NOVA_AI_TESTBOX_PROFILE_FILE:-$HOME/.nova-ai-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${NOVA_AI_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "NOVA_AI_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires NOVA_AI_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
NOVA_AI_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e NOVA_AI_LIVE_PLUGIN_TOOL_MODEL="${NOVA_AI_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "NOVA_AI_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${NOVA_AI_LIVE_PLUGIN_TOOL_MODEL:?missing NOVA_AI_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@nova-ai/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="Nova AI E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  nova-ai_e2e_dump_logs \
    ...
    /tmp/nova-ai-plugin-install.log \
    /tmp/nova-ai-plugin-enable.log \
    /tmp/nova-ai-plugins-list.json \
    /tmp/nova-ai-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/nova-ai-live-plugin-tool.XXXXXX)"
...
nova-ai plugins install "npm-pack:$plugin_tgz" --force >/tmp/nova-ai-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
nova-ai plugins enable "$PLUGIN_ID" >/tmp/nova-ai-plugin-enable.log 2>&1
nova-ai plugins list --json >/tmp/nova-ai-plugins-list.json
nova-ai plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/nova-ai-plugin-inspect.json
...
nova-ai agent --local \
  ...
  --thinking off \
  --timeout "${NOVA_AI_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/nova-ai-agent.json 2>/tmp/nova-ai-agent.err
