...
IMAGE_NAME="$(docker_e2e_resolve_image "nova-ai-npm-telegram-rtt-e2e" NOVA_AI_NPM_TELEGRAM_RTT_E2E_IMAGE)"
...
OUTPUT_DIR="${NOVA_AI_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-rtt}"
...
validate_nova-ai_package_spec() {
  local spec="$1"
  if [[ "$spec" =~ ^nova-ai@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$ ]]; then
    return 0
  fi
  echo "NOVA_AI_NPM_TELEGRAM_PACKAGE_SPEC must be nova-ai@main, nova-ai@alpha, nova-ai@beta, nova-ai@latest, or an exact Nova AI release version; got: $spec" >&2
  exit 1
}
...
validate_credential_source() {
  case "$credential_source" in
    "" | env | convex) ;;
    *)
      echo "NOVA_AI_NPM_TELEGRAM_CREDENTIAL_SOURCE must be env or convex; got: $credential_source" >&2
      exit 1
      ;;
  esac
}
...
validate_credential_role() {
  case "$credential_role" in
    "" | maintainer | ci) ;;
    *)
      echo "NOVA_AI_NPM_TELEGRAM_CREDENTIAL_ROLE must be maintainer or ci; got: $credential_role" >&2
      exit 1
      ;;
  esac
}
...
if [ -n "$credential_source" ]; then
  export NOVA_AI_QA_CREDENTIAL_SOURCE="$credential_source"
fi
if [ -n "$credential_role" ]; then
  export NOVA_AI_QA_CREDENTIAL_ROLE="$credential_role"
fi
...
run_log="$(mktemp "${TMPDIR:-/tmp}/nova-ai-npm-telegram-rtt.XXXXXX")"
...
docker_env=(
  ...
  -e NOVA_AI_QA_TELEGRAM_GROUP_ID
  -e NOVA_AI_QA_TELEGRAM_DRIVER_BOT_TOKEN
  -e NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN
  -e NOVA_AI_QA_TELEGRAM_CANARY_TIMEOUT_MS="${NOVA_AI_QA_TELEGRAM_CANARY_TIMEOUT_MS:-180000}"
  -e NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS="${NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS:-180000}"
  -e NOVA_AI_NPM_TELEGRAM_SCENARIOS="${NOVA_AI_NPM_TELEGRAM_SCENARIOS:-telegram-mentioned-message-reply}"
  -e NOVA_AI_NPM_TELEGRAM_PROVIDER_MODE="${NOVA_AI_NPM_TELEGRAM_PROVIDER_MODE:-mock-openai}"
  -e NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES="${NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES:-20}"
  -e NOVA_AI_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS="${NOVA_AI_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS:-30000}"
  -e NOVA_AI_NPM_TELEGRAM_MAX_FAILURES="${NOVA_AI_NPM_TELEGRAM_MAX_FAILURES:-${NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES:-20}}"
)
...
if [ -n "${NOVA_AI_QA_CREDENTIAL_SOURCE:-}" ]; then
  docker_env+=(-e NOVA_AI_QA_CREDENTIAL_SOURCE="$NOVA_AI_QA_CREDENTIAL_SOURCE")
fi
if [ -n "${NOVA_AI_QA_CREDENTIAL_ROLE:-}" ]; then
  docker_env+=(-e NOVA_AI_QA_CREDENTIAL_ROLE="$NOVA_AI_QA_CREDENTIAL_ROLE")
fi
...
for key in \
  ...
  NOVA_AI_QA_ALLOW_INSECURE_HTTP; do
...
command -v nova-ai
nova-ai --version
node -p "require('/npm-global/lib/node_modules/nova-ai/package.json').version"
EOF
...
export HOME="$(mktemp -d "/tmp/nova-ai-npm-telegram-rtt.XXXXXX")"
export NPM_CONFIG_PREFIX="/npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export OPENAI_API_KEY="sk-nova-ai-rtt"
export GATEWAY_AUTH_TOKEN_REF="nova-ai-rtt"
export NOVA_AI_DISABLE_BONJOUR="1"

install_source="${NOVA_AI_NPM_TELEGRAM_INSTALL_SOURCE:?missing NOVA_AI_NPM_TELEGRAM_INSTALL_SOURCE}"
package_label="${NOVA_AI_NPM_TELEGRAM_PACKAGE_LABEL:-$install_source}"
mock_port="${NOVA_AI_NPM_TELEGRAM_MOCK_PORT:-44080}"
config_path="$HOME/.nova-ai/nova-ai.json"
gateway_log="/tmp/nova-ai-npm-telegram-rtt-gateway.log"
mock_log="/tmp/nova-ai-npm-telegram-rtt-mock.log"
...
if [ "${NOVA_AI_QA_CREDENTIAL_SOURCE:-}" = "convex" ]; then
  credential_env_file="$(mktemp "/tmp/nova-ai-npm-telegram-rtt-credential-env.XXXXXX")"
  credential_lease_file="$(mktemp "/tmp/nova-ai-npm-telegram-rtt-credential-lease.XXXXXX")"
  rm -f "$credential_env_file" "$credential_lease_file"
...
export TELEGRAM_BOT_TOKEN="${NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN:?missing NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN}"

command -v nova-ai
nova-ai --version
installed_version="$(node -p "require('/npm-global/lib/node_modules/nova-ai/package.json').version")"
...
mkdir -p "$(dirname "$config_path")" "$HOME/.nova-ai/workspace" "$HOME/.nova-ai/agents/main/sessions" "$HOME/workspace"
...
nova-ai gateway run --verbose >"$gateway_log" 2>&1 &
gateway_pid="$!"
