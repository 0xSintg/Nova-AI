...
async function main() {
  ...
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-crestodian-first-run-")));
  ...
  assert(
    shouldStartCrestodianForBareRoot(["node", "nova-ai"]),
    "bare nova-ai invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "nova-ai", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
  ...
  const config = JSON.parse(await fs.readFile(configPath, "utf8")) as Nova AIConfig;
  assert(
