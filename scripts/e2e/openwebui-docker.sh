#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Nova AI Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "nova-ai-openwebui-e2e" NOVA_AI_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${NOVA_AI_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${NOVA_AI_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${NOVA_AI_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${NOVA_AI_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${NOVA_AI_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${NOVA_AI_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="nova-ai-openwebui-e2e-$$"
GW_NAME="nova-ai-openwebui-gateway-$$"
OW_NAME="nova-ai-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${NOVA_AI_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${NOVA_AI_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${NOVA_AI_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${NOVA_AI_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${NOVA_AI_TESTBOX_PROFILE_FILE:-$HOME/.nova-ai-testbox-live.profile}"
...
docker_e2e_docker_cmd run -d \
  ...
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "NOVA_AI_GATEWAY_TOKEN=$TOKEN" \
  -e "NOVA_AI_OPENWEBUI_MODEL=$MODEL" \
  -e "NOVA_AI_SKIP_CHANNELS=1" \
  -e "NOVA_AI_SKIP_GMAIL_WATCHER=1" \
  -e "NOVA_AI_SKIP_CRON=1" \
  -e "NOVA_AI_SKIP_CANVAS_HOST=1" \
  -e "NOVA_AI_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    entry="$(nova-ai_e2e_resolve_entrypoint)"

    openai_api_key="${OPENAI_API_KEY:?OPENAI_API_KEY required}"
    batch_file="$(mktemp /tmp/nova-ai-openwebui-config.XXXXXX.json)"
    NOVA_AI_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    nova-ai_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "NOVA_AI_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Nova AI E2E" \
  -e WEBUI_SECRET_KEY="nova-ai-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Nova AI E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="nova-ai/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "NOVA_AI_HTTP_PROBE_BEARER='$TOKEN' NOVA_AI_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Nova AI smoke..."
