...
function nova-aiPath(...parts) {
  return path.join(home, ".nova-ai", ...parts);
}
...
function records() {
  const index = readJson(nova-aiPath("plugins", "installs.json"));
...
function config() {
  return readJson(process.env.NOVA_AI_CONFIG_PATH ?? nova-aiPath("nova-ai.json"));
}
