...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/nova-ai-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/nova-ai-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${NOVA_AI_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${NOVA_AI_SECRET_ENV_NAME:-}" ] && [ -n "${NOVA_AI_SECRET_FILE:-}" ] && [ -f "$NOVA_AI_SECRET_FILE" ]; then
  secret_value="$(cat "$NOVA_AI_SECRET_FILE")"
  rm -f "$NOVA_AI_SECRET_FILE"
  export "${NOVA_AI_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "NOVA_AI_SECRET_ENV_NAME=$api_key_env" \
      "NOVA_AI_SECRET_FILE=$secret_path" \
      "NOVA_AI_WRAPPER_FILE=$wrapper_path" \
