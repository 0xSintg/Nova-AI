...
const inspect = readJson("/tmp/nova-ai-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@nova-ai/codex")) {
  throw new Error(`expected @nova-ai/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @nova-ai/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@nova-ai/codex") {
...
const list = readJson("/tmp/nova-ai-plugins-list.json");
...
if (authRaw.includes("sk-nova-ai-codex-on-demand-e2e")) {
