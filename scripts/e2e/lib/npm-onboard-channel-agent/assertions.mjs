...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write nova-ai.json");
  }
  ...
  if (authRaw.includes("sk-nova-ai-npm-onboard-e2e")) {
...
function assertAgentTurn() {
  ...
  const output = fs.readFileSync("/tmp/nova-ai-agent.combined", "utf8");
