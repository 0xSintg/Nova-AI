...
function writeConfig(kind) {
  const configPath = requireArg(process.env.NOVA_AI_CONFIG_PATH, "NOVA_AI_CONFIG_PATH");
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.NOVA_AI_GATEWAY_TOKEN, "NOVA_AI_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.NOVA_AI_STATE_DIR, "nova-ai.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.NOVA_AI_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(
    process.env.NOVA_AI_CONFIG_BATCH_PATH,
    "NOVA_AI_CONFIG_BATCH_PATH",
  );
  writeJson(batchPath, [
    ...
    { path: "models.providers.openai.models", value: [] },
    {
      ...
      value: Number.parseInt(process.env.NOVA_AI_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS ?? "900", 10),
    },
    ...
    { path: "gateway.auth.token", value: process.env.NOVA_AI_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.NOVA_AI_OPENWEBUI_MODEL },
  ]);
}
