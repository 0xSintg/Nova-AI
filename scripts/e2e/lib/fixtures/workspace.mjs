...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.NOVA_AI_WORKSPACE_DIR || path.join(process.env.HOME, ".nova-ai", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.NOVA_AI_STATE_DIR, "NOVA_AI_STATE_DIR");
  ...
  writeJson(path.join(stateDir, "nova-ai.json"), {
    agents: {
      list: [
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.NOVA_AI_STATE_DIR, "nova-ai.json"))?.agents?.list ?? [];
