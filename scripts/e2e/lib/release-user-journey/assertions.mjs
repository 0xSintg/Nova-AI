...
function configPath() {
  return (
    ...
    path.join(process.env.HOME ?? "", ".nova-ai", "nova-ai.json")
  );
}
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write nova-ai.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-nova-ai-release-user-journey"),
...
function assertPluginUninstalled() {
  ...
  const cfg = readJson(configPath());
  const recordsPath = path.join(process.env.HOME ?? "", ".nova-ai", "plugins", "installs.json");
