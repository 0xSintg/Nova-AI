...
function scanLogs() {
  const roots = ["/tmp", path.join(process.env.HOME, ".nova-ai")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /nova-ai-kitchen-sink-/u.test(path.basename(entry))) {
...
function assertClawHubExternalInstallContract(installPath) {
  const nova-aiPeerPath = path.join(installPath, "node_modules", "nova-ai");
  if (!fs.existsSync(nova-aiPeerPath)) {
    throw new Error(`missing kitchen-sink nova-ai peer symlink: ${nova-aiPeerPath}`);
  }
  if (!fs.lstatSync(nova-aiPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink nova-ai peer is not a symlink: ${nova-aiPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(nova-aiPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink nova-ai peer ${linkedHostRoot} to target ${hostRoot}`);
  }
