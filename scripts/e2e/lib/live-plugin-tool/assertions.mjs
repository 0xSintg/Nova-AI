...
const agentTurnTimeoutSeconds = Number.parseInt(
  process.env.NOVA_AI_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS ?? "300",
  10,
);
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/nova-ai-plugin-inspect.json")
    ? readJson("/tmp/nova-ai-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "nova-ai.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const inspect = readJson("/tmp/nova-ai-plugin-inspect.json");
...
function assertAgentTurn() {
  ...
  const toolName = requireEnv("TOOL_NAME");
  const stdout = fs.readFileSync("/tmp/nova-ai-agent.json", "utf8");
  const stderr = fs.existsSync("/tmp/nova-ai-agent.err")
    ? fs.readFileSync("/tmp/nova-ai-agent.err", "utf8")
    : "";
