...
export PATH="/tmp/nova-ai-bin:$PATH"
mkdir -p /tmp/nova-ai-bin
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/nova-ai-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/nova-ai-bin/loginctl
chmod +x /tmp/nova-ai-bin/systemctl /tmp/nova-ai-bin/loginctl

package_tgz="${NOVA_AI_CURRENT_PACKAGE_TGZ:?missing NOVA_AI_CURRENT_PACKAGE_TGZ}"
git_root="/tmp/nova-ai-git"
mkdir -p "$git_root"
...
(
  cd "$git_root"
  npm install --omit=optional --no-fund --no-audit >/tmp/nova-ai-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@nova-ai.local"
  git config user.name "Nova AI Docker E2E"
  git add -A
  git commit -qm "test fixture"
)
npm_log="/tmp/nova-ai-doctor-switch-npm-install.log"
if ! npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  cat "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/nova-ai"
npm_root="/tmp/npm-prefix/lib/node_modules/nova-ai"
...
run_flow() {
  ...
  local install_log="/tmp/nova-ai-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/nova-ai-doctor-switch-${name}-doctor.log"
  local command_timeout="${NOVA_AI_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"

  echo "== Flow: $name =="
  nova-ai_test_state_create "switch-${name}" empty
  ...
  unit_path="$HOME/.config/systemd/user/nova-ai-gateway.service"
  if [ ! -f "$unit_path" ]; then
...
run_flow \
  ...
  "$npm_entry" \
  "NOVA_AI_UPDATE_IN_PROGRESS=1 node $git_cli doctor --repair --force --yes --non-interactive" \
  "$git_entry"
...
run_flow \
  ...
  "$git_entry" \
  "NOVA_AI_UPDATE_IN_PROGRESS=1 $npm_bin doctor --repair --force --yes --non-interactive" \
  "$npm_entry"
...
run_proxy_env_flow() {
  ...
  if ! timeout "$command_timeout" env NOVA_AI_UPDATE_IN_PROGRESS=1 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/nova-ai-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/nova-ai-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/nova-ai-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/nova-ai-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/nova-ai-doctor-switch-${name}-clear.log"
  local command_timeout="${NOVA_AI_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/nova-ai-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/nova-ai-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/nova-ai-gateway.service"
  ...
  assert_env_value "$unit_path" "NOVA_AI_WRAPPER" "$wrapper"
  ...
  sed -i "/^Environment=NOVA_AI_WRAPPER=/d" "$unit_path"
  ...
  sed -i "s#^Environment=NOVA_AI_WRAPPER=.*#Environment=NOVA_AI_WRAPPER=/tmp/stale-nova-ai-wrapper#" "$unit_path"
  if ! timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    cat "$env_repair_log"
    exit 1
  fi
  ...
  if ! grep -Fq "Gateway service invokes NOVA_AI_WRAPPER:" "$doctor_log"; then
  ...
  if ! timeout "$command_timeout" env NOVA_AI_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    cat "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "NOVA_AI_WRAPPER"
