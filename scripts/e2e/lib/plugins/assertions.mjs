...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.NOVA_AI_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function assertPluginRemoved(params) {
  ...
  const config = readNova AIConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".nova-ai", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Nova AI plugin install/update/uninstall commands. Use `nova-ai plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".nova-ai", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(nova-aiPeerPath)) {
    throw new Error(`missing ClawHub nova-ai peer symlink: ${nova-aiPeerPath}`);
  }
  if (!fs.lstatSync(nova-aiPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub nova-ai peer is not a symlink: ${nova-aiPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(nova-aiPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub nova-ai peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@nova-ai/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@nova-ai/demo-plugin-npm") {
...
function assertInvalidNova AIExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  const output = fs.readFileSync("/tmp/plugins-invalid-nova-ai-extensions.log", "utf8");
  for (const expected of ["nova-ai.extensions[1]", "non-empty string"]) {
  ...
  const list = readJson("/tmp/plugins-invalid-nova-ai-extensions-list.json");
  ...
  const managedInstallPath = path.join(process.env.HOME, ".nova-ai", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.NOVA_AI_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.NOVA_AI_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".nova-ai", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".nova-ai", "nova-ai.json");
...
const commands = {
  ...
  "invalid-nova-ai-extensions": assertInvalidNova AIExtensionsRejected,
