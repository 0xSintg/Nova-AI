...
nova-ai_e2e_install_package /tmp/nova-ai-install.log "mounted Nova AI package" /tmp/npm-prefix
...
NOVA_AI_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/nova-ai-e2e-registry.log 2>&1 &
registry_pid=$!
...
if ! node "$probe" wait-registry; then
  ...
  cat /tmp/nova-ai-e2e-registry.log || true
  exit 1
fi

before_config_hash=""
if [ "$NOVA_AI_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$NOVA_AI_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="${NOVA_AI_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$NOVA_AI_CONFIG_PATH" | awk '{print $1}')"
