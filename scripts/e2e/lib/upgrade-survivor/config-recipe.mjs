    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/nova-ai-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.NOVA_AI_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
function runNova AI(step) {
  const result = spawnSync("nova-ai", step.argv, {
    encoding: "utf8",
    env: process.env,
  });
  return {
    id: step.id,
    intent: step.intent,
    command: ["nova-ai", ...step.argv].join(" "),
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runNova AI(adaptedStep);
