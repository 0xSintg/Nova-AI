...
function getScenario() {
  const scenario = process.env.NOVA_AI_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("NOVA_AI_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.NOVA_AI_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  const stateDir = requireEnv("NOVA_AI_STATE_DIR");
  const workspace = requireEnv("NOVA_AI_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".nova-ai", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".nova-ai-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
    write(
      path.join(
        runtimeRoot,
        plugin,
        ".nova-ai-runtime-deps-copy-stale",
        "node_modules",
        "stale-sentinel",
        "package.json",
      ),
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `nova-ai-${version}-${plugin}`,
          ".nova-ai-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/nova-ai-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const stage = process.env.NOVA_AI_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-nova-ai");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`nova-ai-${version}-`))
      : [];
    assert(
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  if (installedFromNpm) {
    const npmRoot = path.join(requireEnv("NOVA_AI_STATE_DIR"), "npm", "node_modules");
    assert(
  ...
  const extensionsRoot = path.join(requireEnv("NOVA_AI_STATE_DIR"), "extensions");
  assert(
...
function assertConfiguredPluginInstalls() {
  ...
  assertOptionalConfiguredPluginIndex(records, index.plugins ?? [], {
    packageName: "@nova-ai/brave-plugin",
    pluginId: "brave",
  });
