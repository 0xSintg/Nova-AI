...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >"$NOVA_AI_STATE_DIR/gateway.systemd.env"
}
...
prepare_update_restart_probe_current_install() {
  ...
  env -u NOVA_AI_GATEWAY_TOKEN -u NOVA_AI_GATEWAY_PASSWORD nova-ai gateway --port "$port" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
  printf '%s\n' "$gateway_pid" >"$NOVA_AI_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE"
  nova-ai_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360
  ready_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  start_seconds=$(((ready_epoch - start_epoch + 999) / 1000))
  write_update_restart_service_auth_env
  if ! env -u NOVA_AI_GATEWAY_TOKEN -u NOVA_AI_GATEWAY_PASSWORD nova-ai gateway install --force --json >"$NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" 2>"$NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR"; then
    echo "gateway service install failed" >&2
    cat "$NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_ERR" >&2 || true
    cat "$NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SERVICE_INSTALL_JSON" >&2 || true
    return 1
  fi
}
