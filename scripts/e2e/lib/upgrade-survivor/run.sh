...
ARTIFACT_ROOT="$(dirname "${NOVA_AI_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/nova-ai-upgrade-survivor-artifacts/summary.json}")"
...
SUMMARY_JSON="${NOVA_AI_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${NOVA_AI_UPGRADE_SURVIVOR_BASELINE:?missing NOVA_AI_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${NOVA_AI_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${NOVA_AI_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${NOVA_AI_CURRENT_PACKAGE_TGZ:-}}"
...
export NOVA_AI_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "NOVA_AI_UPGRADE_SURVIVOR_BASELINE must be nova-ai@latest, nova-ai@beta, nova-ai@alpha, an exact Nova AI release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "NOVA_AI_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    nova-ai@*)
      baseline_spec="$raw"
      baseline_version="${raw#nova-ai@}"
      ;;
    *@*)
      echo "NOVA_AI_UPGRADE_SURVIVOR_BASELINE must be nova-ai@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="nova-ai@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "NOVA_AI_UPGRADE_SURVIVOR_BASELINE must be nova-ai@latest, nova-ai@beta, nova-ai@alpha, nova-ai@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "NOVA_AI_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.NOVA_AI_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.NOVA_AI_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.NOVA_AI_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      nova-ai_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/nova-ai\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${NOVA_AI_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "NOVA_AI_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@nova-ai-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${NOVA_AI_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$NOVA_AI_STATE_DIR/extensions/opik-nova-ai"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-nova-ai",
...
JSON
  cat >"$shadow_root/nova-ai.plugin.json" <<'JSON'
{
  "id": "opik-nova-ai",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.nova-ai-runtime-deps.json" \
      "$plugin_dir/.nova-ai-runtime-deps-stamp.json" \
      "$plugin_dir/.nova-ai-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.nova-ai-install-stage-upgrade-survivor" \
      "$plugin_dir/.nova-ai-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$NOVA_AI_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$NOVA_AI_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/nova-ai-upgrade-survivor-dep" \
      "$plugin_dir/.nova-ai-runtime-deps-copy-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep" \
      "$plugin_dir/.nova-ai-install-stage-upgrade-survivor" \
      "$plugin_dir/.nova-ai-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep" \
      "$NOVA_AI_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep" \
      "$NOVA_AI_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep"
    printf '{"name":"nova-ai-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/nova-ai-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.nova-ai-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.nova-ai-runtime-deps-stamp.json"
    printf '{"name":"nova-ai-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.nova-ai-runtime-deps-copy-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep/package.json"
    printf '{"name":"nova-ai-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep/package.json"
    printf '{"name":"nova-ai-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$NOVA_AI_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep/package.json"
    printf '{"name":"nova-ai-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$NOVA_AI_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nova-ai-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"nova-ai-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
install_baseline() {
  ...
  if ! command -v nova-ai >/dev/null; then
    echo "baseline install did not expose nova-ai on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(nova-ai --version 2>&1)"; then
    echo "baseline nova-ai --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline nova-ai --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  nova-ai_e2e_eval_test_state_from_b64 "${NOVA_AI_TEST_STATE_FUNCTION_B64:?missing NOVA_AI_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.nova-ai /root/workspace
    nova-ai_test_state_create /root minimal
  else
    nova-ai_test_state_create "$ARTIFACT_ROOT/state-home" minimal
  fi
  export NOVA_AI_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! nova-ai config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
...
log_file="${NOVA_AI_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/nova-ai-systemctl-shim.log}"
pid_file="${NOVA_AI_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/nova-ai-systemctl-shim.pid}"
daemon_log="${NOVA_AI_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/nova-ai-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/nova-ai-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! env -u NOVA_AI_GATEWAY_TOKEN -u NOVA_AI_GATEWAY_PASSWORD nova-ai gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
...
const stateDir = process.env.NOVA_AI_STATE_DIR;
if (!stateDir) {
  throw new Error("missing NOVA_AI_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$NOVA_AI_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$NOVA_AI_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  NOVA_AI_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing NOVA_AI_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  local update_env=(
    env
    -u NOVA_AI_GATEWAY_TOKEN
    -u NOVA_AI_GATEWAY_PASSWORD
    -u NOVA_AI_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(NOVA_AI_ALLOW_ROOT=1)
  fi
  if ! "${update_env[@]}" nova-ai "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
...
assert_root_managed_vps_cli_usable() {
  ...
  "${root_cli_env[@]}" nova-ai config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  "${root_cli_env[@]}" nova-ai plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! nova-ai doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
...
validate_post_doctor_config() {
  if ! nova-ai config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${NOVA_AI_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$NOVA_AI_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  args+=(--out "$out_file")
...
start_gateway() {
  local port=18789
  local budget="${NOVA_AI_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u NOVA_AI_GATEWAY_TOKEN -u NOVA_AI_GATEWAY_PASSWORD nova-ai gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_probes() {
  ...
  export NOVA_AI_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING="discord,telegram,whatsapp,feishu,matrix"
  readyz_seconds="$(probe_gateway_endpoint /readyz ready "$READYZ_JSON")"
  unset NOVA_AI_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING
}

check_gateway_status() {
  local port=18789
  local budget="${NOVA_AI_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! nova-ai gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
