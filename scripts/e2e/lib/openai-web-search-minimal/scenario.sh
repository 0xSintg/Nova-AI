...
SUCCESS_MARKER="NOVA_AI_SCHEMA_E2E_OK"
RAW_SCHEMA_ERROR="400 The following tools cannot be used with reasoning.effort 'minimal': web_search."
MOCK_REQUEST_LOG="/tmp/nova-ai-openai-web-search-minimal-requests.jsonl"
GATEWAY_LOG="/tmp/nova-ai-openai-web-search-minimal-gateway.log"
mock_pid=""
gateway_pid=""
...
dump_debug_logs() {
  ...
  for file in \
    "$GATEWAY_LOG" \
    /tmp/nova-ai-openai-web-search-minimal-mock.log \
    /tmp/nova-ai-openai-web-search-minimal-client-success.log \
    /tmp/nova-ai-openai-web-search-minimal-client-reject.log \
    "$MOCK_REQUEST_LOG" \
    "$NOVA_AI_STATE_DIR/nova-ai.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
MOCK_PORT="$MOCK_PORT" \
  ...
  node scripts/e2e/lib/openai-web-search-minimal/mock-server.mjs >/tmp/nova-ai-openai-web-search-minimal-mock.log 2>&1 &
mock_pid="$!"
...
PORT="$PORT" NOVA_AI_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs success >/tmp/nova-ai-openai-web-search-minimal-client-success.log 2>&1

node scripts/e2e/lib/openai-web-search-minimal/assertions.mjs assert-success-request "$MOCK_REQUEST_LOG"

PORT="$PORT" NOVA_AI_GATEWAY_TOKEN="$TOKEN" node scripts/e2e/lib/openai-web-search-minimal/client.mjs reject >/tmp/nova-ai-openai-web-search-minimal-client-reject.log 2>&1
