...
const guestNova AI = "/opt/homebrew/bin/nova-ai";
const guestNova AIEntry = "/opt/homebrew/lib/node_modules/nova-ai/nova-ai.mjs";
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://nova-ai.com/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://nova-ai.com/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("nova-ai-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("nova-ai-parallels-macos-tgz.");
    try {
      ...
      if (await this.needsHostTgz()) {
        this.artifact = await packNova AI({
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("nova-ai-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.NOVA_AI_PARALLELS_MACOS_AGENT_TIMEOUT_S || 2700),
      () => this.verifyTurn(),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("nova-ai-main-upgrade.tgz"),
      );
      ...
      await this.phase(
        "upgrade.update-dev",
        Number(process.env.NOVA_AI_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S || 1800),
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'nova-ai.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nova-ai-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nova-ai.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g nova-ai >/dev/null 2>&1 || true
rm -rf "$HOME/.nova-ai"
...
curl -fsSL ${shellQuote(this.options.installUrl)} -o /tmp/nova-ai-install.sh
bash /tmp/nova-ai-install.sh --version ${shellQuote(this.installVersion)}
${guestNova AI} --version`,
    );
  }
...
${guestNova AI} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestNova AI, "--version"]);
...
check_path "$root/nova-ai"
check_path "$root/nova-ai/extensions"
if [ -d "$root/nova-ai/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/nova-ai/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/nova-ai-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/nova-ai`)}
export PATH=${shellQuote(`/tmp/nova-ai-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".nova-ai", "nova-ai.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 NOVA_AI_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 NOVA_AI_DISABLE_BUNDLED_PLUGINS=1 ${guestNode} ${guestNova AIEntry} update --channel dev --yes --json
${guestNode} ${guestNova AIEntry} --version
${guestNode} ${guestNova AIEntry} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestExec([guestNode, guestNova AIEntry, "update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'nova-ai.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nova-ai-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nova-ai.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} NOVA_AI_HOME=${shellQuote(home)} NOVA_AI_STATE_DIR=${shellQuote(`${home}/.nova-ai`)} NOVA_AI_CONFIG_PATH=${shellQuote(
        `${home}/.nova-ai/nova-ai.json`,
      )} ${guestNode} ${guestNova AIEntry} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/nova-ai-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestNova AI(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestNova AI, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestNova AI(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestNova AI(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`nova-ai ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/nova-ai-dashboard-smoke.html 2>/dev/null; then
    grep -F '<title>Nova AI Control</title>' /tmp/nova-ai-dashboard-smoke.html >/dev/null &&
      grep -F '<nova-ai-app></nova-ai-app>' /tmp/nova-ai-dashboard-smoke.html >/dev/null &&
      exit 0
  fi
  sleep 1
done
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestExec([guestNode, guestNova AIEntry, "models", "set", this.auth.modelId]);
...
rm -f "$provider_config_batch"`);
    ...
    this.guestExec([guestNode, guestNova AIEntry, "config", "set", "tools.profile", "minimal"]);
    this.guestSh(
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestNode} ${guestNova AIEntry} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    ...
    const matches = [...log.matchAll(/Nova AI\s+([0-9][^\s]*)/gi)];
