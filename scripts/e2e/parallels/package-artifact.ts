...
export function resolveNova AIRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("nova-ai@") ? rawValue.slice("nova-ai@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`nova-ai@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "nova-ai", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no nova-ai registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packNova AI(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "nova-ai-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `nova-ai-main-${shortHead}.tgz`);
...
async function acquirePackageLock(lockDir: string, ownerToken: string): Promise<void> {
  const timeoutMs = Number(process.env.NOVA_AI_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS || 30 * 60_000);
  const staleMs = Number(process.env.NOVA_AI_PARALLELS_PACKAGE_LOCK_STALE_MS || 2 * 60 * 60_000);
