...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'nova-ai-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-Nova AI config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:NOVA_AI_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.nova-ai\\nova-ai.json' }
$agentTurnVersionText = Invoke-Nova AI --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'Nova AI\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:NOVA_AI_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:NOVA_AI_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:NOVA_AI_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'nova-ai-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.NOVA_AI_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.NOVA_AI_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.NOVA_AI_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
Remove-Item Env:NOVA_AI_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:NOVA_AI_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:NOVA_AI_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsNova AIResolver = String.raw`function Resolve-Nova AICommand {
  if ($script:Nova AIResolvedCommand) { return $script:Nova AIResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\nova-ai.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\nova-ai.ps1'
  }
  foreach ($name in @('nova-ai.cmd', 'nova-ai.ps1', 'nova-ai')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'nova-ai.cmd'
    $shimCandidates += Join-Path $npmPrefix 'nova-ai.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:Nova AIResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:Nova AIResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:Nova AIResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:Nova AIResolvedCommand
    }
  }
  throw 'nova-ai command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Nova AI {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $Nova AIArgs)
  $command = Resolve-Nova AICommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @Nova AIArgs
    } else {
      & $command.Path @Nova AIArgs
    }
  } finally {
