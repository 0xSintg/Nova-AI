...
const updateTimeoutSeconds = Number(process.env.NOVA_AI_PARALLELS_NPM_UPDATE_TIMEOUT_S || 1200);

function usage(): string {
  return `Usage: bash scripts/e2e/parallels-npm-update-smoke.sh [options]

Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: nova-ai@latest
  --update-target <target>    Target passed to guest 'nova-ai update --tag'.
...
class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("nova-ai-parallels-npm-update.");
    this.tgzDir = await makeTempDir("nova-ai-parallels-npm-update-tgz.");
    try {
      this.latestVersion = resolveLatestVersion();
      this.packageSpec = this.options.packageSpec || `nova-ai@${this.latestVersion}`;
      ...
      say(`Run same-guest nova-ai update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          NOVA_AI_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveNova AIRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("nova-ai@") ? target : `nova-ai@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      macosVm,
      script,
      "nova-ai-parallels-npm-update-macos",
    );
  ...
  private async guestLinux(
    ...
    const scriptPath = this.writeGuestScript(
      this.linuxVm,
      script,
      "nova-ai-parallels-npm-update-linux",
    );
    try {
      const status = await this.runStreamingToJobLog(
        "prlctl",
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveNova AIRegistryVersion(this.packageSpec);
    const target = resolveNova AIRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to nova-ai@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private configurePublishedTargets(): void {
    if (this.options.betaValidation) {
      const version = resolveNova AIRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `nova-ai@${version}`;
      say(`Beta validation target: nova-ai@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveNova AIRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveNova AIRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `nova-ai@${version}` : this.options.freshTargetSpec;
    }
  }
