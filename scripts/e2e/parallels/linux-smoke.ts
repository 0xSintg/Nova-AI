...
function parseNova AIPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareNova AIPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseNova AIPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke {
  ...
  private disableBonjour = parseBoolEnv(process.env.NOVA_AI_PARALLELS_LINUX_DISABLE_BONJOUR);
  private hostIp = "";
  private hostPort = 0;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("nova-ai-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("nova-ai-parallels-linux-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("nova-ai-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.NOVA_AI_PARALLELS_LINUX_AGENT_TIMEOUT_S || 1500),
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("nova-ai-main-upgrade.tgz"),
    );
...
printf 'preflight.npmRoot=%s\n' "$(npm root -g 2>/dev/null || true)"`);
  ...
  private installLatestRelease(): void {
    this.guestExec(["curl", "-fsSL", this.options.installUrl, "-o", "/tmp/nova-ai-install.sh"]);
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "NOVA_AI_NO_ONBOARD=1",
        "bash",
        "/tmp/nova-ai-install.sh",
        "--version",
    ...
    this.guestExec(["nova-ai", "--version"]);
  }
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["nova-ai", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.nova-ai/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@nova-ai/test-bad-plugin","version":"1.0.0","nova-ai":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/nova-ai.plugin.json" <<'JSON'
...
config_path = Path("/root/.nova-ai/nova-ai.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseNova AIPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareNova AIPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " NOVA_AI_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "nova-ai gateway run" >/dev/null 2>&1 || true
rm -f /tmp/nova-ai-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env NOVA_AI_HOME=/root NOVA_AI_STATE_DIR=/root/.nova-ai NOVA_AI_CONFIG_PATH=/root/.nova-ai/nova-ai.json NOVA_AI_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["nova-ai", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["nova-ai", "gateway", "status", "--deep", "--require-rpc"]
      : ["nova-ai", "gateway", "status", "--deep"];
    const result = run(
      "prlctl",
      ["exec", this.options.vmName, "/usr/bin/env", "HOME=/root", "NOVA_AI_ALLOW_ROOT=1", ...args],
      {
        check: false,
        quiet: true,
        timeoutMs: this.remainingPhaseTimeoutMs(),
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.nova-ai/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.nova-ai/test-bad-plugin`);
  }

  private verifyLocalTurn(): void {
    this.guestExec(["nova-ai", "models", "set", this.auth.modelId]);
...
JSON
nova-ai config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["nova-ai", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env NOVA_AI_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} nova-ai agent --local --agent main --session-id "$session_id" --message ${shellQuote(
