...
const REMOTE_ROOT = "/tmp/nova-ai-telegram-user-crabbox";
...
function usageText() {
  return [
    "Usage:",
    "  node --import tsx scripts/e2e/telegram-user-crabbox-proof.ts [probe] [--text /status] [--expect Nova AI]",
    ...
    "  --repo <owner/name>           GitHub repo for publish. Default: nova-ai/nova-ai.",
...
function parseArgs(argv: string[]): Options {
  ...
  const opts: Options = {
    ...
    command,
    crabboxBin: trimToValue(process.env.NOVA_AI_TELEGRAM_USER_CRABBOX_BIN) ?? "crabbox",
    desktopChatTitle:
      trimToValue(process.env.NOVA_AI_TELEGRAM_USER_DESKTOP_CHAT_TITLE) ?? "Nova AI Testing",
    dryRun: false,
    expect: ["Nova AI"],
    gatewayPort: 19_879,
    idleTimeout: "60m",
    keepBox: false,
    mockResponseText: "NOVA_AI_E2E_OK",
    mockPort: 19_882,
    outputDir: path.join(DEFAULT_OUTPUT_ROOT, stamp),
    previewCropWidth: TELEGRAM_PROOF_CROP.cropWidth,
    previewFps: 24,
    previewWidth: 1920,
    provider: process.env.NOVA_AI_TELEGRAM_USER_CRABBOX_PROVIDER?.trim() || "aws",
    publishFullArtifacts: false,
    publishRepo: "nova-ai/nova-ai",
    recordFps: 24,
    recordSeconds: 35,
    remoteCommand: [],
    target: "linux",
    ...
    ttl: "120m",
    userDriverScript:
      trimToValue(process.env.NOVA_AI_TELEGRAM_USER_DRIVER_SCRIPT) ?? DEFAULT_USER_DRIVER,
  };
...
function repoRoot() {
  ...
  if (
    ...
    throw new Error("Run from the Nova AI repo root.");
  }
  return cwd;
}
...
function childProcessBaseEnv() {
  const keys = [
    ...
    "LC_ALL",
    "NODE_OPTIONS",
    "NOVA_AI_BUILD_PRIVATE_QA",
    "NOVA_AI_ENABLE_PRIVATE_QA_CLI",
    "PATH",
    "PNPM_HOME",
    "SHELL",
    "TEMP",
...
function gatewayEnv(params: { configPath: string; stateDir: string; sutToken: string }) {
  return {
    ...childProcessBaseEnv(),
    OPENAI_API_KEY: "sk-nova-ai-e2e-mock",
...
function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-tg-crabbox-sut-"));
  const stateDir = path.join(tempRoot, "state");
  const workspace = path.join(tempRoot, "workspace");
  fs.mkdirSync(stateDir, { recursive: true });
  fs.mkdirSync(workspace, { recursive: true });
  const configPath = path.join(tempRoot, "nova-ai.json");
  const config = {
    agents: {
      defaults: {
...
async function startLocalSut(params: {
  ...
  const gateway = spawnLogged(
    "pnpm",
    ["nova-ai", "gateway", "--port", String(params.gatewayPort)],
    {
      cwd: params.repoRoot,
...
async function startLocalSutDaemon(params: {
  ...
  try {
    ...
    gatewayPid = spawnDaemon({
      command: "pnpm",
      args: ["nova-ai", "gateway", "--port", String(params.gatewayPort)],
      cwd: params.repoRoot,
      env: gatewayEnv({ ...config, sutToken: params.sutToken }),
      logPath: gatewayLog,
    });
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  const hasConvexEnv =
    trimToValue(process.env.NOVA_AI_QA_CONVEX_SITE_URL) &&
    trimToValue(process.env.NOVA_AI_QA_CONVEX_SECRET_CI);
  ...
  try {
    ...
    return {
      ...
      commands: {
        send: `nova-ai-telegram-user-crabbox-proof send --session ${path.relative(root, pathname)} --text '/status'`,
        view: `nova-ai-telegram-user-crabbox-proof view --session ${path.relative(root, pathname)} --message-id <message-id>`,
        run: `nova-ai-telegram-user-crabbox-proof run --session ${path.relative(root, pathname)} -- bash -lc 'source ${REMOTE_ROOT}/env.sh && python3 ${REMOTE_ROOT}/user-driver.py transcript --limit 20 --json'`,
        finish: `nova-ai-telegram-user-crabbox-proof finish --session ${path.relative(root, pathname)} --preview-crop telegram-window`,
      },
    };
  } catch (error) {
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-telegram-crabbox-"));
  const summary: JsonObject = {
    artifacts: {},
