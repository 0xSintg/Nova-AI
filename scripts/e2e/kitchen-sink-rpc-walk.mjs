...
const PLUGIN_SPEC =
  process.env.NOVA_AI_KITCHEN_SINK_NPM_SPEC || "npm:@nova-ai/kitchen-sink@latest";
const PLUGIN_ID = process.env.NOVA_AI_KITCHEN_SINK_PLUGIN_ID || "nova-ai-kitchen-sink-fixture";
...
const READY_TIMEOUT_MS = readPositiveInt(process.env.NOVA_AI_KITCHEN_SINK_RPC_READY_MS, 240000);
const COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.NOVA_AI_KITCHEN_SINK_RPC_COMMAND_MS,
  180000,
);
const RPC_TIMEOUT_MS = readPositiveInt(process.env.NOVA_AI_KITCHEN_SINK_RPC_CALL_MS, 60000);
const MAX_RSS_MIB = readPositiveInt(process.env.NOVA_AI_KITCHEN_SINK_MAX_RSS_MIB, 2048);
...
function resolveNova AIRunner() {
  if (process.env.NOVA_AI_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.NOVA_AI_ENTRY],
      label: process.env.NOVA_AI_ENTRY,
    };
  }
  ...
  return { command: "pnpm", baseArgs: ["nova-ai"], label: "pnpm nova-ai" };
}

function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      NOVA_AI_SKIP_PROVIDERS: "0",
      NOVA_AI_KITCHEN_SINK_PERSONALITY:
        process.env.NOVA_AI_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runNova AI(runner, args, env, options = {}) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = await callGateway({
    config: readJson(options.env.NOVA_AI_CONFIG_PATH),
    configPath: options.env.NOVA_AI_CONFIG_PATH,
...
function configureKitchenSink(env, port) {
  const configPath = env.NOVA_AI_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.NOVA_AI_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function main() {
  const runner = resolveNova AIRunner();
  const port = readPositiveInt(process.env.NOVA_AI_KITCHEN_SINK_RPC_PORT, 19173);
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");

  console.log(`Kitchen Sink RPC walk using ${PLUGIN_SPEC} via ${runner.label}`);
  await runNova AI(runner, ["plugins", "install", PLUGIN_SPEC], env, { timeoutMs: 240000 });
  configureKitchenSink(env, port);
  await runNova AI(runner, ["plugins", "enable", PLUGIN_ID], env, { timeoutMs: 60000 });
  const inspect = parseJsonOutput(
    (await runNova AI(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env))
      .stdout,
  );
