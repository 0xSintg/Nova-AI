...
// against the Nova AI npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/nova-ai-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      NOVA_AI_STATE_DIR: "/tmp/nova-ai-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[nova-ai mcp] ${String(chunk)}`);
  });
