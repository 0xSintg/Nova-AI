...
import { applyDockerOpenAiProviderConfig, type Nova AIConfig } from "./docker-openai-seed.ts";
...
process.title = "nova-ai-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.NOVA_AI_STATE_DIR?.trim() || path.join(os.homedir(), ".nova-ai");
  const configPath =
    process.env.NOVA_AI_CONFIG_PATH?.trim() || path.join(stateDir, "nova-ai.json");
