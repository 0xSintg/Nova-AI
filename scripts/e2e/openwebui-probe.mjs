...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.NOVA_AI_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsRes?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "nova-ai/default") ?? modelIds.find((id) => id === "nova-ai");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing nova-ai model: ${JSON.stringify(modelIds)}`;
  } else if (modelsRes) {
...
if (!targetModel) {
  throw new Error(
    `nova-ai model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
