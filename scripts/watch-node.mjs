...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.NOVA_AI_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[nova-ai] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[nova-ai] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[nova-ai] This usually means a file in node_modules is empty or truncated.");
  console.error("[nova-ai] Recommended recovery:");
  console.error("[nova-ai]   rm -rf node_modules");
  console.error("[nova-ai]   pnpm store prune");
  console.error("[nova-ai]   pnpm install");
  console.error("");
  console.error("[nova-ai] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.NOVA_AI_WATCH_MODE = "1";
  childEnv.NOVA_AI_WATCH_SESSION = watchSession;
  ...
  childEnv.NOVA_AI_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.NOVA_AI_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `nova-ai doctor --fix --non-interactive` once.",
        deps,
      );
