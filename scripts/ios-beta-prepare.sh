...
- regenerates apps/ios/Nova AI.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${NOVA_AI_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid NOVA_AI_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid NOVA_AI_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid NOVA_AI_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid NOVA_AI_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing NOVA_AI_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(NOVA_AI_URL_SLASH)$(NOVA_AI_URL_SLASH)#g'
)"
...
NOVA_AI_CODE_SIGN_STYLE = Automatic
NOVA_AI_DEVELOPMENT_TEAM = ${TEAM_ID}
NOVA_AI_IOS_SELECTED_TEAM = ${TEAM_ID}
NOVA_AI_APP_BUNDLE_ID = ai.nova-ai.client
NOVA_AI_SHARE_BUNDLE_ID = ai.nova-ai.client.share
NOVA_AI_ACTIVITY_WIDGET_BUNDLE_ID = ai.nova-ai.client.activitywidget
NOVA_AI_WATCH_APP_BUNDLE_ID = ai.nova-ai.client.watchkitapp
NOVA_AI_WATCH_EXTENSION_BUNDLE_ID = ai.nova-ai.client.watchkitapp.extension
NOVA_AI_APP_PROFILE =
NOVA_AI_SHARE_PROFILE =
NOVA_AI_PUSH_TRANSPORT = relay
NOVA_AI_PUSH_DISTRIBUTION = official
NOVA_AI_URL_SLASH = /
NOVA_AI_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
NOVA_AI_PUSH_APNS_ENVIRONMENT = production
EOF
