#!/usr/bin/env bash
# Rootless Nova AI in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-nova-ai-podman.sh launch        # Start gateway
#   ./scripts/run-nova-ai-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   nova-ai --container nova-ai dashboard --no-open
#   nova-ai --container nova-ai channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      NOVA_AI_GATEWAY_TOKEN|NOVA_AI_PODMAN_CONTAINER|NOVA_AI_PODMAN_IMAGE|NOVA_AI_IMAGE|NOVA_AI_PODMAN_PULL|NOVA_AI_PODMAN_GATEWAY_HOST_PORT|NOVA_AI_GATEWAY_PORT|NOVA_AI_PODMAN_BRIDGE_HOST_PORT|NOVA_AI_BRIDGE_PORT|NOVA_AI_GATEWAY_BIND|NOVA_AI_PODMAN_USERNS|NOVA_AI_BIND_MOUNT_OPTIONS|NOVA_AI_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-nova-ai-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${NOVA_AI_CONFIG_DIR:-$EFFECTIVE_HOME/.nova-ai}"
ENV_FILE="${NOVA_AI_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${NOVA_AI_CONFIG_DIR:-$EFFECTIVE_HOME/.nova-ai}"
ENV_FILE="${NOVA_AI_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${NOVA_AI_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${NOVA_AI_PODMAN_CONTAINER:-nova-ai}"
NOVA_AI_IMAGE="${NOVA_AI_PODMAN_IMAGE:-${NOVA_AI_IMAGE:-nova-ai:local}}"
PODMAN_PULL="${NOVA_AI_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${NOVA_AI_PODMAN_GATEWAY_HOST_PORT:-${NOVA_AI_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${NOVA_AI_PODMAN_BRIDGE_HOST_PORT:-${NOVA_AI_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${NOVA_AI_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$NOVA_AI_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v nova-ai >/dev/null 2>&1; then
    return 0
  fi
  NOVA_AI_CONTAINER="" NOVA_AI_CONFIG_DIR="$config_dir" \
    nova-ai config get gateway.bind 2>/dev/null || true
}
...
# NOVA_AI_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${NOVA_AI_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'NOVA_AI_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v nova-ai >/dev/null 2>&1; then
    echo "Warning: nova-ai not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    NOVA_AI_CONTAINER="" NOVA_AI_CONFIG_DIR="$config_dir" \
      nova-ai config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    NOVA_AI_CONTAINER="" NOVA_AI_CONFIG_DIR="$config_dir" \
      nova-ai config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  NOVA_AI_CONTAINER="" NOVA_AI_CONFIG_DIR="$config_dir" \
    nova-ai config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${NOVA_AI_GATEWAY_TOKEN:-}" ]]; then
  export NOVA_AI_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "NOVA_AI_GATEWAY_TOKEN" "$NOVA_AI_GATEWAY_TOKEN"
  echo "Generated NOVA_AI_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/nova-ai.json"
...
PODMAN_USERNS="${NOVA_AI_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported NOVA_AI_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (NOVA_AI_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${NOVA_AI_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${NOVA_AI_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$NOVA_AI_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.nova-ai/.npm \
    -e NOVA_AI_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.nova-ai:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.nova-ai/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$NOVA_AI_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME nova-ai dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME nova-ai devices approve --latest  # if pairing required"
