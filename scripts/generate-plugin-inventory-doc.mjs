...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving Nova AI messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to Nova AI.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an Nova AI plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.nova-ai?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in Nova AI";
  }
  const install = packageJson.nova-ai?.install;
  const release = packageJson.nova-ai?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.nova-ai?.install?.clawhubSpec === "string" ||
    typeof packageJson.nova-ai?.install?.npmSpec === "string";
...
function renderReferenceIndex(records) {
  return `---
summary: "Generated index of Nova AI plugin reference pages"
read_when:
  - You need a reference page for a specific Nova AI plugin
...
\`nova-ai.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "nova-ai.plugin.json");
