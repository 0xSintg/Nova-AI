...
type DriverMode = "token" | "webhook" | "nova-ai";
...
function resolveStateDir(): string {
  const override = process.env.NOVA_AI_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.NOVA_AI_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".nova-ai");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "nova-ai") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or nova-ai.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver nova-ai] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Nova AI spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|nova-ai> Driver transport mode (default: token)\n" +
    ...
    "  --nova-ai-bin <path>        Nova AI CLI binary for driver=nova-ai (default: nova-ai)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  NOVA_AI_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  NOVA_AI_DISCORD_SMOKE_DRIVER\n" +
    "  NOVA_AI_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  NOVA_AI_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  NOVA_AI_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  NOVA_AI_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  NOVA_AI_DISCORD_SMOKE_AGENT\n" +
    "  NOVA_AI_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  NOVA_AI_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  NOVA_AI_DISCORD_SMOKE_POLL_MS\n" +
    "  NOVA_AI_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  NOVA_AI_DISCORD_SMOKE_NOVA_AI_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.NOVA_AI_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.NOVA_AI_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token") || process.env.NOVA_AI_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.NOVA_AI_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.NOVA_AI_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.NOVA_AI_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.NOVA_AI_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.NOVA_AI_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.NOVA_AI_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.NOVA_AI_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.NOVA_AI_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  ...
  const threadBindingsPath =
    ...
    process.env.NOVA_AI_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const nova-aiBin =
    resolveArg("--nova-ai-bin") || process.env.NOVA_AI_DISCORD_SMOKE_NOVA_AI_BIN || "nova-ai";
...
async function nova-aiCliJson<T>(params: { nova-aiBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.nova-aiBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`nova-ai ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await nova-aiCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    nova-aiBin: params.nova-aiBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "nova-ai") {
    return await readMessagesWithOpenclaw({
      nova-aiBin: params.args.nova-aiBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `nova-ai-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await nova-aiCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("nova-ai message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Nova AI.`,
        diagnostics: {
          bindingCandidates: [
            {
