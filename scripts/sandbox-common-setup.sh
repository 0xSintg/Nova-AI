...
BASE_IMAGE="${BASE_IMAGE:-nova-ai-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-nova-ai-sandbox-common:bookworm-slim}"
...
NOVA_AI_DOCKER_BUILD_USE_BUILDX="${NOVA_AI_DOCKER_BUILD_USE_BUILDX:-0}"
NOVA_AI_DOCKER_BUILD_CACHE_FROM="${NOVA_AI_DOCKER_BUILD_CACHE_FROM:-}"
NOVA_AI_DOCKER_BUILD_CACHE_TO="${NOVA_AI_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=nova-ai.sandbox=1)
NOTE
