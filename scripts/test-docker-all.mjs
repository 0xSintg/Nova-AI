...
// Builds shared Docker images, prepares one Nova AI npm tarball, assigns lanes
...
const ROOT_DIR = path.resolve(process.env.NOVA_AI_DOCKER_E2E_REPO_ROOT || SCRIPT_ROOT_DIR);
...
const DEFAULT_GITHUB_WORKFLOW = "nova-ai-live-and-e2e-checks-reusable.yml";
...
function resourceLimitEnvName(resource) {
  return `NOVA_AI_DOCKER_ALL_${resource.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_LIMIT`;
}
...
function parseSchedulerOptions(env, parallelism) {
  const weightLimit = parsePositiveInt(
    env.NOVA_AI_DOCKER_ALL_WEIGHT_LIMIT,
    parallelism,
    "NOVA_AI_DOCKER_ALL_WEIGHT_LIMIT",
  );
...
function appendExtension(env, extension) {
  const current = env.NOVA_AI_DOCKER_BUILD_EXTENSIONS ?? env.NOVA_AI_EXTENSIONS ?? "";
  const tokens = current.split(/\s+/).filter(Boolean);
  if (!tokens.includes(extension)) {
    tokens.push(extension);
  }
  env.NOVA_AI_DOCKER_BUILD_EXTENSIONS = tokens.join(" ");
}
...
function githubWorkflowRef() {
  const explicit = process.env.NOVA_AI_DOCKER_E2E_WORKFLOW_REF;
  if (explicit) {
    return explicit;
  }
...
function githubWorkflowRerunCommand(laneNames, ref) {
  ...
  const releasePath = process.env.NOVA_AI_DOCKER_ALL_PROFILE === RELEASE_PATH_PROFILE;
  const fields = [
    "gh workflow run",
    shellQuote(process.env.NOVA_AI_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
  ...
  if (process.env.GITHUB_RUN_ID) {
    ...
    fields.push(
      ...
      `package_artifact_name=${shellQuote(
        process.env.NOVA_AI_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || "docker-e2e-package",
      )}`,
    );
  }
  if (process.env.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPEC) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baseline=${shellQuote(process.env.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPEC)}`,
    );
  }
  if (process.env.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPECS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baselines=${shellQuote(process.env.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPECS)}`,
    );
  }
  if (process.env.NOVA_AI_UPGRADE_SURVIVOR_SCENARIOS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_scenarios=${shellQuote(process.env.NOVA_AI_UPGRADE_SURVIVOR_SCENARIOS)}`,
    );
  }
  if (process.env.NOVA_AI_DOCKER_E2E_BARE_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_bare_image=${shellQuote(process.env.NOVA_AI_DOCKER_E2E_BARE_IMAGE)}`,
    );
  }
  if (process.env.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_functional_image=${shellQuote(process.env.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE)}`,
    );
  }
  return fields.join(" ");
}
...
function buildLaneRerunCommand(name, baseEnv) {
  ...
  const image = poolLane ? e2eImageForLane(poolLane, baseEnv) : baseEnv.NOVA_AI_DOCKER_E2E_IMAGE;
  const env = [
    ["NOVA_AI_DOCKER_ALL_LANES", name],
    ["NOVA_AI_DOCKER_ALL_BUILD", build],
    ["NOVA_AI_DOCKER_ALL_PREFLIGHT", "0"],
    ["NOVA_AI_SKIP_DOCKER_BUILD", "1"],
    ["NOVA_AI_DOCKER_E2E_IMAGE", image || DEFAULT_E2E_IMAGE],
    ["NOVA_AI_DOCKER_E2E_BARE_IMAGE", baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE],
    ["NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE", baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE],
    ["NOVA_AI_CURRENT_PACKAGE_TGZ", baseEnv.NOVA_AI_CURRENT_PACKAGE_TGZ],
    ["NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPEC],
    ["NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPECS", baseEnv.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPECS],
    ["NOVA_AI_UPGRADE_SURVIVOR_SCENARIOS", baseEnv.NOVA_AI_UPGRADE_SURVIVOR_SCENARIOS],
  ];
  if (baseEnv.NOVA_AI_DOCKER_ALL_PNPM_COMMAND) {
    env.push(["NOVA_AI_DOCKER_ALL_PNPM_COMMAND", baseEnv.NOVA_AI_DOCKER_ALL_PNPM_COMMAND]);
  }
  return `${env
...
function withResolvedPnpmCommand(command, env) {
  const pnpmCommand = env.NOVA_AI_DOCKER_ALL_PNPM_COMMAND?.trim();
  if (!pnpmCommand) {
    return command;
  }
...
function liveDockerHarnessScriptCommand(script) {
  return `bash -c 'harness="\${NOVA_AI_DOCKER_E2E_TRUSTED_HARNESS_DIR:-}"; if [ -z "$harness" ]; then if [ -d .release-harness/scripts ]; then harness=.release-harness; else harness=.; fi; fi; NOVA_AI_LIVE_DOCKER_REPO_ROOT="\${NOVA_AI_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
async function writeRunSummary(logDir, summary) {
  ...
  const payload = {
    ...summary,
    packageArtifactName: process.env.NOVA_AI_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        process.env.GITHUB_SERVER_URL && process.env.GITHUB_REPOSITORY && process.env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: process.env.NOVA_AI_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary) {
  const ref =
    ...
    process.env.NOVA_AI_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: process.env.NOVA_AI_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:nova-ai-(?:gateway-e2e|openwebui|openwebui-gateway|config-reload-e2e)-)/.test(name),
    );
}
...
async function prepareNova AIPackage(baseEnv, logDir) {
  const existing = baseEnv.NOVA_AI_CURRENT_PACKAGE_TGZ;
  if (existing) {
    const packageTgz = path.resolve(existing);
    baseEnv.NOVA_AI_CURRENT_PACKAGE_TGZ = packageTgz;
    baseEnv.NOVA_AI_BUNDLED_CHANNEL_HOST_BUILD = "0";
    baseEnv.NOVA_AI_NPM_ONBOARD_HOST_BUILD = "0";
    console.log(`==> Nova AI package: ${packageTgz}`);
    return;
  }

  const packDir = path.join(logDir, "nova-ai-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "nova-ai-current.tgz");
  await runForeground(
    "Prepare Nova AI package once",
    `node scripts/package-nova-ai-for-docker.mjs --output-dir ${shellQuote(packDir)} --output-name nova-ai-current.tgz`,
    baseEnv,
  );
  ...
  baseEnv.NOVA_AI_BUNDLED_CHANNEL_HOST_BUILD = "0";
  baseEnv.NOVA_AI_NPM_ONBOARD_HOST_BUILD = "0";
  console.log(`==> Nova AI package: ${baseEnv.NOVA_AI_CURRENT_PACKAGE_TGZ}`);
}

function e2eImageForLane(poolLane, baseEnv) {
  if (poolLane.e2eImageKind === "bare") {
    return baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE;
  }
  if (poolLane.e2eImageKind === "functional") {
    return baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE;
  }
  return undefined;
}
...
function laneEnv(poolLane, baseEnv, logDir, cacheKey) {
  ...
  env.NOVA_AI_DOCKER_ALL_LANE_NAME = name;
  const image = e2eImageForLane(poolLane, baseEnv);
  if (image) {
    env.NOVA_AI_DOCKER_E2E_IMAGE = image;
  }
  if (poolLane.e2eImageKind) {
    env.NOVA_AI_DOCKER_E2E_IMAGE_KIND = poolLane.e2eImageKind;
  }
  const cacheName = cacheKey || name;
  if (!process.env.NOVA_AI_DOCKER_CLI_TOOLS_DIR) {
    env.NOVA_AI_DOCKER_CLI_TOOLS_DIR = path.join(logDir, `${cacheName}-cli-tools`);
  }
  if (!process.env.NOVA_AI_DOCKER_CACHE_HOME_DIR) {
    env.NOVA_AI_DOCKER_CACHE_HOME_DIR = path.join(logDir, `${cacheName}-cache`);
  }
  return env;
}
...
async function runLane(lane, baseEnv, logDir, fallbackTimeoutMs) {
  ...
  await mkdir(env.NOVA_AI_DOCKER_CLI_TOOLS_DIR, { recursive: true });
  await mkdir(env.NOVA_AI_DOCKER_CACHE_HOME_DIR, { recursive: true });
  await fs.promises.writeFile(
    logFile,
    [
      `==> [${name}] cli tools dir: ${env.NOVA_AI_DOCKER_CLI_TOOLS_DIR}`,
      `==> [${name}] cache dir: ${env.NOVA_AI_DOCKER_CACHE_HOME_DIR}`,
      ...
      `==> [${name}] e2e image: ${env.NOVA_AI_DOCKER_E2E_IMAGE ?? ""}`,
      "",
    ].join("\n"),
  );
  ...
  return {
    ...
    attempts,
    finishedAt: new Date().toISOString(),
    image: env.NOVA_AI_DOCKER_E2E_IMAGE,
      ...
      if (running.size === 0) {
        ...
        throw new Error(
          `No Docker lanes fit scheduler limits (${describeDockerSchedulerLimits(
            parallelism,
            options,
          )}): ${blocked}. Tune NOVA_AI_DOCKER_ALL_PARALLELISM, NOVA_AI_DOCKER_ALL_WEIGHT_LIMIT, or NOVA_AI_DOCKER_ALL_<RESOURCE>_LIMIT.`,
        );
      }
...
async function main() {
  ...
  const parallelism = parsePositiveInt(
    process.env.NOVA_AI_DOCKER_ALL_PARALLELISM,
    DEFAULT_PARALLELISM,
    "NOVA_AI_DOCKER_ALL_PARALLELISM",
  );
  const tailParallelism = parsePositiveInt(
    process.env.NOVA_AI_DOCKER_ALL_TAIL_PARALLELISM,
    Math.min(parallelism, DEFAULT_TAIL_PARALLELISM),
    "NOVA_AI_DOCKER_ALL_TAIL_PARALLELISM",
  );
  const tailLines = parsePositiveInt(
    process.env.NOVA_AI_DOCKER_ALL_FAILURE_TAIL_LINES,
    DEFAULT_FAILURE_TAIL_LINES,
    "NOVA_AI_DOCKER_ALL_FAILURE_TAIL_LINES",
  );
  const laneTimeoutMs = parsePositiveInt(
    process.env.NOVA_AI_DOCKER_ALL_LANE_TIMEOUT_MS,
    DEFAULT_LANE_TIMEOUT_MS,
    "NOVA_AI_DOCKER_ALL_LANE_TIMEOUT_MS",
  );
  const laneStartStaggerMs = parseNonNegativeInt(
    process.env.NOVA_AI_DOCKER_ALL_START_STAGGER_MS,
    DEFAULT_LANE_START_STAGGER_MS,
    "NOVA_AI_DOCKER_ALL_START_STAGGER_MS",
  );
  const statusIntervalMs = parseNonNegativeInt(
    process.env.NOVA_AI_DOCKER_ALL_STATUS_INTERVAL_MS,
    DEFAULT_STATUS_INTERVAL_MS,
    "NOVA_AI_DOCKER_ALL_STATUS_INTERVAL_MS",
  );
  const preflightRunTimeoutMs = parsePositiveInt(
    process.env.NOVA_AI_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS,
    DEFAULT_PREFLIGHT_RUN_TIMEOUT_MS,
    "NOVA_AI_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS",
  );
  const failFast = parseBool(process.env.NOVA_AI_DOCKER_ALL_FAIL_FAST, true);
  const dryRun = parseBool(process.env.NOVA_AI_DOCKER_ALL_DRY_RUN, false);
  const preflightEnabled = parseBool(process.env.NOVA_AI_DOCKER_ALL_PREFLIGHT, true);
  const preflightCleanup = parseBool(process.env.NOVA_AI_DOCKER_ALL_PREFLIGHT_CLEANUP, true);
  const timingsEnabled = parseBool(process.env.NOVA_AI_DOCKER_ALL_TIMINGS, true);
  const buildEnabled = parseBool(process.env.NOVA_AI_DOCKER_ALL_BUILD, true);
  const planJson =
    cliArgs.has("--plan-json") || parseBool(process.env.NOVA_AI_DOCKER_ALL_PLAN_JSON, false);
  const planReleaseAll = parseBool(process.env.NOVA_AI_DOCKER_ALL_PLAN_RELEASE_ALL, false);
  const profile = parseProfile(process.env.NOVA_AI_DOCKER_ALL_PROFILE);
  const releaseProfile = normalizeReleaseProfile(
    process.env.NOVA_AI_DOCKER_ALL_RELEASE_PROFILE || process.env.NOVA_AI_RELEASE_PROFILE,
  );
  const releaseChunk = process.env.NOVA_AI_DOCKER_ALL_CHUNK || process.env.DOCKER_E2E_CHUNK || "";
  const includeOpenWebUI = parseBool(
    process.env.NOVA_AI_DOCKER_ALL_INCLUDE_OPENWEBUI ?? process.env.INCLUDE_OPENWEBUI,
    true,
  );
  const selectedLaneNamesRaw =
    process.env.NOVA_AI_DOCKER_ALL_LANES || process.env.DOCKER_E2E_LANES || "";
  const selectedLaneNames = parseLaneSelection(selectedLaneNamesRaw);
  if (selectedLaneNamesRaw && selectedLaneNames.length === 0) {
    throw new Error("NOVA_AI_DOCKER_ALL_LANES must include at least one lane name");
  }
  const liveMode = parseLiveMode(process.env.NOVA_AI_DOCKER_ALL_LIVE_MODE);
  const liveRetries = parseNonNegativeInt(
    process.env.NOVA_AI_DOCKER_ALL_LIVE_RETRIES,
    DEFAULT_LIVE_RETRIES,
    "NOVA_AI_DOCKER_ALL_LIVE_RETRIES",
  );
  const timingsFile = path.resolve(
    process.env.NOVA_AI_DOCKER_ALL_TIMINGS_FILE || DEFAULT_TIMINGS_FILE,
  );
  const runId = process.env.NOVA_AI_DOCKER_ALL_RUN_ID || utcStampForPath();
  const logDir = path.resolve(
    process.env.NOVA_AI_DOCKER_ALL_LOG_DIR ||
      path.join(ROOT_DIR, ".artifacts/docker-tests", runId),
  );

  const baseEnv = commandEnv({
    NOVA_AI_DOCKER_E2E_BARE_IMAGE:
      process.env.NOVA_AI_DOCKER_E2E_BARE_IMAGE ||
      process.env.NOVA_AI_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_BARE_IMAGE,
    NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE:
      process.env.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE ||
      process.env.NOVA_AI_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_FUNCTIONAL_IMAGE,
  });
  baseEnv.NOVA_AI_DOCKER_E2E_IMAGE =
    process.env.NOVA_AI_DOCKER_E2E_IMAGE || baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE;
  ...
  const { orderedLanes, orderedTailLanes, plan, scheduledLanes } = resolveDockerE2ePlan({
    ...
    selectedLaneNames,
    timingStore,
    upgradeSurvivorBaselines: process.env.NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPECS,
    upgradeSurvivorScenarios: process.env.NOVA_AI_UPGRADE_SURVIVOR_SCENARIOS,
  });
  ...
  console.log(`==> Docker E2E bare image: ${baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE}`);
  console.log(`==> Docker E2E functional image: ${baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE}`);
  if (profile === RELEASE_PATH_PROFILE) {
  ...
  console.log(`==> Live-test bundled plugins: ${baseEnv.NOVA_AI_DOCKER_BUILD_EXTENSIONS}`);
  ...
  if (lanesNeedNova AIPackage(scheduledLanes)) {
    await runPhase(phases, "prepare-nova-ai-package", {}, async () => {
      await prepareNova AIPackage(baseEnv, logDir);
    });
  } else {
    console.log("==> Nova AI package: not needed for selected lanes");
  }
  ...
  if (buildEnabled) {
    ...
    if (lanesNeedE2eImageKind(scheduledLanes, "bare")) {
      buildEntries.push({
        ...
        env: {
          NOVA_AI_DOCKER_E2E_IMAGE: baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE,
          NOVA_AI_DOCKER_E2E_TARGET: "bare",
        },
        label: `shared bare Docker E2E image once: ${baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE}`,
        phaseDetails: { image: baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE, imageKind: "bare" },
        phases,
      });
    }
    if (lanesNeedE2eImageKind(scheduledLanes, "functional")) {
      buildEntries.push({
        command: "pnpm test:docker:e2e-build",
        env: {
          NOVA_AI_DOCKER_E2E_IMAGE: baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE,
          NOVA_AI_DOCKER_E2E_TARGET: "functional",
        },
        label: `shared functional Docker E2E image once: ${baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
        phaseDetails: {
          image: baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE,
  ...
  if (failFast && failures.length > 0) {
    await writeRunSummary(logDir, {
      ...
      failures,
      image: baseEnv.NOVA_AI_DOCKER_E2E_IMAGE,
      images: {
        bare: baseEnv.NOVA_AI_DOCKER_E2E_BARE_IMAGE,
        functional: baseEnv.NOVA_AI_DOCKER_E2E_FUNCTIONAL_IMAGE,
      },
      lanes: allResults,
      phases,
      profile,
