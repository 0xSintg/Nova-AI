...
function printUsage(): void {
  console.log(`Nova AI Gateway restart benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "nova-ai.json");
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "nova-ai-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "nova-ai-bench",
    npm_config_update_notifier: "false",
    NOVA_AI_CONFIG: configPath,
    NOVA_AI_CONFIG_PATH: configPath,
    NOVA_AI_GATEWAY_RESTART_TRACE: "1",
    NOVA_AI_GATEWAY_STARTUP_TRACE: "1",
    NOVA_AI_HOME: root,
    NOVA_AI_NO_RESPAWN: "1",
    NOVA_AI_STATE_DIR: path.join(root, "state"),
    NOVA_AI_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "nova-ai-gateway-restart-bench-"));
