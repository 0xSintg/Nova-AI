...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "nova-ai.json");
  ...
  const sessionId = process.env.NOVA_AI_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["nova-ai", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["nova-ai", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
