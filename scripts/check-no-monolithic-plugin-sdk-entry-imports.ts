...
import { discoverNova AIPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']nova-ai\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']nova-ai\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']nova-ai\/plugin-sdk\/channel-runtime["']/,
    label: "nova-ai/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']nova-ai\/plugin-sdk\/config-runtime["']/,
    label: "nova-ai/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']nova-ai\/plugin-sdk\/infra-runtime["']/,
    label: "nova-ai/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic nova-ai/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy nova-ai/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused nova-ai/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
