...
# Build and bundle Nova AI into a minimal .app we can open.
# Outputs to dist/Nova AI.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Nova AI.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Nova AI"
MLX_TTS_HELPER_PRODUCT="nova-ai-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.nova-ai.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/nova-ai/nova-ai/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Nova AI/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Nova AIBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Nova AIGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Nova AI"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Nova AI"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Nova AI"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Nova AI" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Nova AI/Resources/Nova AI.icns" "$APP_ROOT/Contents/Resources/Nova AI.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Nova AI/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying Nova AIKit resources"
NOVA_AIKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/Nova AIKit_Nova AIKit.bundle"
if [ -d "$NOVA_AIKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/Nova AIKit_Nova AIKit.bundle"
  cp -R "$NOVA_AIKIT_BUNDLE" "$APP_ROOT/Contents/Resources/Nova AIKit_Nova AIKit.bundle"
else
  echo "WARN: Nova AIKit resource bundle not found at $NOVA_AIKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Nova AI"
killall -q Nova AI 2>/dev/null || true
