...
function printUsage(): void {
  process.stdout.write(`Nova AI issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): Nova AIConfig {
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureNova AIModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "nova-ai-issue-78851-"));
