...
function parseArgs(argv) {
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("NOVA_AI_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("NOVA_AI_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
  ...
  const envIds = normalizeCsv(process.env.NOVA_AI_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function nova-aiCommand(repoRoot, args) {
  return {
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    NOVA_AI_LOG_DIR: path.join(runRoot, "logs"),
    NOVA_AI_QA_SUITE_PROGRESS: process.env.NOVA_AI_QA_SUITE_PROGRESS ?? "1",
    ...
    for (const { phase, args } of commands) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...nova-aiCommand(params.repoRoot, ["plugins", ...args]),
    ...
    for (const alias of plugin.cliCommandAliases) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...nova-aiCommand(params.repoRoot, [command, "--help"]),
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-gauntlet-"));
