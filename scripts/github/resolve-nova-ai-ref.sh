...
REMOTE_URL="${NOVA_AI_REF_REMOTE:-https://github.com/nova-ai/nova-ai.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-nova-ai-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Nova AI branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Nova AI ref: ${REF}" >&2
exit 1
