...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  NOVA_AI_SKIP_ACPX_RUNTIME: "1",
  NOVA_AI_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$NOVA_AI_WATCH_PID_FILE"',
    'mkdir -p "$NOVA_AI_HOME/.nova-ai"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$NOVA_AI_HOME/.nova-ai/nova-ai.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    NOVA_AI_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    NOVA_AI_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-gateway-watch-"));
