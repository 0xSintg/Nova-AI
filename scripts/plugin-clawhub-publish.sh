...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-nova-ai/nova-ai}}"
source_commit="${SOURCE_COMMIT:-$(git rev-parse HEAD)}"
source_ref="${SOURCE_REF:-$(git symbolic-ref -q HEAD || true)}"
clawhub_workdir="${NOVAHUB_WORKDIR:-${CLAWHUB_WORKDIR:-$(pwd)}}"
...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/nova-ai-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${NOVA_AI_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${NOVA_AI_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
for attempt in $(seq 1 "${NOVA_AI_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${NOVA_AI_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${NOVA_AI_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${NOVA_AI_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
