...
DOCKER_BUILD_EXTENSIONS="${NOVA_AI_DOCKER_BUILD_EXTENSIONS:-${NOVA_AI_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "NOVA_AI_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${NOVA_AI_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because NOVA_AI_DOCKER_BUILD_ON_MISSING/NOVA_AI_TESTBOX allows fallback."
fi
