...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "nova-ai.plugin.json"));
    }
...
const listRequiredNova AIExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "nova-ai");
  return [
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredNova AIExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.NOVA_AI_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.NOVA_AI_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.NOVA_AI_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by NOVA_AI_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by NOVA_AI_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "NOVA_AI_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "NOVA_AI_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "NOVA_AI_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "NOVA_AI_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "NOVA_AI_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "NOVA_AI_RUN_NODE_BUILD_LOCK_STALE_MS";
...
const logRunner = (message, deps) => {
  if (deps.env.NOVA_AI_RUNNER_LOG === "0") {
    return;
  }
  const line = `[nova-ai] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.NOVA_AI_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[nova-ai] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `nova-ai-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.NOVA_AI_TRACE_SYNC_IO === "1") {
...
const runNova AI = async (deps) => {
  const diagnosticArgs = resolveRunNodeDiagnosticArgs(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "nova-ai.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[nova-ai] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldUseExistingDistForGatewayClient = (deps, buildRequirement) =>
  ...
  deps.env.NOVA_AI_FORCE_BUILD !== "1" &&
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.NOVA_AI_BUILD_PRIVATE_QA = "1";
    deps.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
  ...
  try {
    ...
    if (!buildRequirement.shouldBuild) {
      ...
      exitCode = await runNova AI(deps);
      return await closeRunNodeOutputTee(deps, exitCode);
    }
