...
# Nova AI Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Nova AI."
...
GUM_VERSION="${NOVA_AI_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Nova AI Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://nova-ai.com/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$NOVA_AI_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.nova-ai.com/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_nova-ai_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.nova-ai-* "$npm_root"/nova-ai 2>/dev/null || true
}

extract_nova-ai_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_nova-ai_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/nova-ai" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/nova-ai"|"/usr/local/bin/nova-ai")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/nova-ai/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale nova-ai symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing nova-ai binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Nova AI package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_nova-ai_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Nova AI npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*nova-ai" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_nova-ai_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_nova-ai_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Nova AI npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an nova-ai binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${NOVA_AI_TAGLINE_INDEX:-}" ]]; then
        if [[ "${NOVA_AI_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((NOVA_AI_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${NOVA_AI_NO_ONBOARD:-0}
NO_PROMPT=${NOVA_AI_NO_PROMPT:-0}
DRY_RUN=${NOVA_AI_DRY_RUN:-0}
INSTALL_METHOD=${NOVA_AI_INSTALL_METHOD:-}
NOVA_AI_VERSION=${NOVA_AI_VERSION:-latest}
USE_BETA=${NOVA_AI_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/nova-ai"
GIT_DIR=${NOVA_AI_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${NOVA_AI_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${NOVA_AI_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${NOVA_AI_VERBOSE:-0}"
VERIFY_INSTALL="${NOVA_AI_VERIFY_INSTALL:-0}"
NOVA_AI_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Nova AI installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/nova-ai)
...
Environment variables:
  NOVA_AI_INSTALL_METHOD=git|npm
  NOVA_AI_VERSION=latest|next|main|<semver>|<spec>
  NOVA_AI_BETA=0|1
  NOVA_AI_GIT_DIR=...
  NOVA_AI_GIT_UPDATE=0|1
  NOVA_AI_NO_PROMPT=1
  NOVA_AI_VERIFY_INSTALL=1
  NOVA_AI_DRY_RUN=1
  NOVA_AI_NO_ONBOARD=1
  NOVA_AI_VERBOSE=1
  NOVA_AI_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Nova AI checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Nova AI source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_nova-ai_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"nova-ai"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://nova-ai.com/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Nova AI updates; use npm i -g nova-ai@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_nova-ai_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/nova-ai" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/nova-ai" ]]; then
        ln -sf "$npm_root/nova-ai/dist/entry.js" "${npm_bin}/nova-ai"
        ui_info "Created nova-ai bin link at ${npm_bin}/nova-ai"
    fi
    return 0
}

# Check for existing Nova AI installation
check_existing_nova-ai() {
    if [[ -n "$(type -P nova-ai 2>/dev/null || true)" ]]; then
        ui_info "Existing Nova AI installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_nova-ai_ref() {
    local requested="${NOVA_AI_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "nova-ai" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_nova-ai_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${NOVA_AI_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_nova-ai_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/nova-ai"
        ...
        local version=""
        version="$(nova-ai_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_nova-ai_npm_root_candidates)
}

warn_duplicate_nova-ai_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_nova-ai_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Nova AI global installs detected"
    echo "  Different Node/npm environments can run different Nova AI versions."

    local active_node active_npm active_nova-ai
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_nova-ai="${NOVA_AI_BIN:-}"
    if [[ -z "$active_nova-ai" ]]; then
        active_nova-ai="$(type -P nova-ai 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active nova-ai: ${INFO}${active_nova-ai:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make nova-ai show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

nova-ai_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "nova-ai"
        return 0
...
warn_nova-ai_not_found() {
    ui_warn "Installed, but nova-ai is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t nova-ai 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named nova-ai; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_nova-ai_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P nova-ai 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/nova-ai" ]]; then
        echo "${npm_bin}/nova-ai"
        return 0
    fi
...
install_nova-ai_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/nova-ai/nova-ai.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Nova AI from git checkout: ${repo_dir}"
    else
        ui_info "Installing Nova AI from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Nova AI" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_nova-ai_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_nova-ai_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building Nova AI" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/nova-ai" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/nova-ai"
    ui_success "Nova AI wrapper installed to \$HOME/.local/bin/nova-ai"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view nova-ai dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:nova-ai/nova-ai#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_nova-ai() {
    local package_name="nova-ai"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            NOVA_AI_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="nova-ai"
        else
            NOVA_AI_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${NOVA_AI_VERSION}" ]]; then
        NOVA_AI_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${NOVA_AI_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${NOVA_AI_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Nova AI v${resolved_version}"
    else
        ui_info "Installing Nova AI (${NOVA_AI_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${NOVA_AI_VERSION}")"

    if ! install_nova-ai_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_nova-ai_paths
        install_nova-ai_npm "${install_spec}"
    fi

    if [[ "${NOVA_AI_VERSION}" == "latest" && "${package_name}" == "nova-ai" ]]; then
        if ! resolve_nova-ai_bin &> /dev/null; then
            ui_warn "npm install nova-ai@latest failed; retrying nova-ai@next"
            cleanup_npm_nova-ai_paths
            install_nova-ai_npm "nova-ai@next"
        fi
    fi

    ensure_nova-ai_bin_link || true

    ui_success "Nova AI installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${NOVA_AI_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_nova-ai_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (nova-ai not on PATH yet)"
        warn_nova-ai_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${NOVA_AI_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.nova-ai/workspace-${profile}"
    else
        echo "${HOME}/.nova-ai/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${NOVA_AI_CONFIG_PATH:-$HOME/.nova-ai/nova-ai.json}"
    if [[ -f "${config_path}" || -f "$HOME/.novabot/novabot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        local user_claw
        user_claw="$(nova-ai_command_for_user "${NOVA_AI_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but nova-ai not on PATH; skipping onboarding"
        warn_nova-ai_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(nova-ai_command_for_user "$claw")"
...
resolve_nova-ai_version() {
    local version=""
    local raw_version_output=""
    local claw="${NOVA_AI_BIN:-}"
    if [[ -z "$claw" ]] && command -v nova-ai &> /dev/null; then
        claw="$(command -v nova-ai)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_nova-ai_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/nova-ai/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/nova-ai/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: nova-ai not on PATH yet"
        warn_nova-ai_not_found
        return 1
    fi

    run_quiet_step "Checking Nova AI version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: nova-ai gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_nova-ai_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Nova AI checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set NOVA_AI_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_nova-ai; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Nova AI"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g nova-ai &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g nova-ai 2>/dev/null || true
        ...
        install_nova-ai_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/nova-ai" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/nova-ai"
    ...
    NOVA_AI_BIN="$(resolve_nova-ai_bin || true)"
    warn_duplicate_nova-ai_global_installs || true
    ...
    installed_version=$(resolve_nova-ai_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Nova AI installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Nova AI installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/nova-ai"
        ui_kv "Update command" "nova-ai update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running nova-ai doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                NOVA_AI_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                NOVA_AI_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                NOVA_AI_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (nova-ai not on PATH yet)"
                    warn_nova-ai_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v nova-ai &> /dev/null; then
        local claw="${NOVA_AI_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_nova-ai_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (nova-ai daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if NOVA_AI_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: nova-ai daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${NOVA_AI_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
