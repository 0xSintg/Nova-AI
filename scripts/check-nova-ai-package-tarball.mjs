...
function usage() {
  return "Usage: node scripts/check-nova-ai-package-tarball.mjs <nova-ai.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`Nova AI package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.NOVA_AI_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-nova-ai-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Nova AI package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Nova AI package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Nova AI package tarball integrity passed.");
