...
const env = {
  ...
  pnpm_config_verify_deps_before_run: process.env.pnpm_config_verify_deps_before_run || "false",
  NOVA_AI_LIVE_TEST: process.env.NOVA_AI_LIVE_TEST || "1",
  NOVA_AI_LIVE_TEST_QUIET: quietOverride ?? process.env.NOVA_AI_LIVE_TEST_QUIET ?? "1",
  ...(forceCodexHarness ? { NOVA_AI_LIVE_CODEX_HARNESS: "1" } : {}),
};
...
const heartbeatMs = parsePositiveInt(process.env.NOVA_AI_LIVE_WRAPPER_HEARTBEAT_MS, 20_000);
