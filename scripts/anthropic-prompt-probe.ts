...
// Usage:
// NOVA_AI_PROMPT_TRANSPORT=direct|gateway
// NOVA_AI_PROMPT_MODE=extra|override
// NOVA_AI_PROMPT_TEXT='...'
// NOVA_AI_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.NOVA_AI_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.NOVA_AI_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.NOVA_AI_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.NOVA_AI_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.NOVA_AI_USER_PROMPT?.trim() || "is nova here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "NOVA_AI_PROMPT_CAPTURE",
  raw: process.env.NOVA_AI_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "NOVA_AI_PROMPT_INCLUDE_RAW",
  raw: process.env.NOVA_AI_PROMPT_INCLUDE_RAW,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.NOVA_AI_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "NOVA_AI_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.NOVA_AI_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "NOVA_AI_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.NOVA_AI_PROMPT_GATEWAY_TIMEOUT_MS,
});
const SETUP_TOKEN_RAW = process.env.NOVA_AI_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.NOVA_AI_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.NOVA_AI_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set NOVA_AI_LIVE_SETUP_TOKEN_VALUE or NOVA_AI_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["nova-ai.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        NOVA_AI_CONFIG_PATH: params.configPath,
        NOVA_AI_STATE_DIR: params.stateDir,
        NOVA_AI_AGENT_DIR: params.agentDir,
        NOVA_AI_GATEWAY_TOKEN: params.gatewayToken,
        NOVA_AI_SKIP_CHANNELS: "1",
        NOVA_AI_SKIP_GMAIL_WATCHER: "1",
        NOVA_AI_SKIP_CANVAS_HOST: "1",
        NOVA_AI_SKIP_BROWSER_CONTROL_SERVER: "1",
        NOVA_AI_DISABLE_BONJOUR: "1",
        NOVA_AI_SKIP_CRON: "1",
        NOVA_AI_TEST_MINIMAL_GATEWAY: "1",
        NOVA_AI_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing NOVA_AI_PROMPT_TEXT or NOVA_AI_PROMPT_LIST_JSON");
  }
