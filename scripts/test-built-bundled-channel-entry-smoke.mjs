...
process.env.NOVA_AI_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "NOVA_AI_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "NOVA_AI_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/nova-ai");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "nova-ai");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.nova-ai?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.nova-ai.extensions) && packageJson.nova-ai.extensions.length > 0
    ...
    const setupEntry = packageJson.nova-ai.setupEntry;
