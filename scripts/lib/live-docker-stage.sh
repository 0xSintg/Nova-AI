...
nova-ai_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${NOVA_AI_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "NOVA_AI_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
nova-ai_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
nova-ai_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
nova-ai_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.nova-ai"
  ...
  export NOVA_AI_STATE_DIR="$dest_dir"
  export NOVA_AI_CONFIG_PATH="$dest_dir/nova-ai.json"
}
