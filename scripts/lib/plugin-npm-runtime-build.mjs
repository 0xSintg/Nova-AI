...
export function isPublishablePluginPackage(packageJson) {
  return packageJson.nova-ai?.release?.publishToNpm === true;
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "nova-ai" || id.startsWith("nova-ai/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "nova-ai.plugin.json")) {
    merged.add("nova-ai.plugin.json");
  }
...
function normalizeNova AIPeerRange(value) {
...
function resolveNova AIPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeNova AIPeerRange(packageJson.nova-ai?.compat?.pluginApi) ||
    normalizeNova AIPeerRange(packageJson.peerDependencies?.nova-ai) ||
    normalizeNova AIPeerRange(packageJson.nova-ai?.build?.nova-aiVersion) ||
    normalizeNova AIPeerRange(rootPackageJson?.version) ||
    normalizeNova AIPeerRange(packageJson.version)
  );
}

export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const nova-aiPeerRange = resolveNova AIPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!nova-aiPeerRange) {
    throw new Error(
      `cannot infer nova-ai peerDependency range for ${plan.pluginDir}; set nova-ai.compat.pluginApi or package version`,
    );
  }
  ...
  const existingNova AIMeta = getRecord(existingPeerDependenciesMeta.nova-ai);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      nova-ai: nova-aiPeerRange,
    },
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.nova-ai?.extensions)
      ? packageJson.nova-ai.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.nova-ai?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.nova-ai.setupEntry)
      : undefined,
  };
  return {
    ...plan,
