...
export async function resolveCodexAppServerProtocolSource(repoRoot: string): Promise<{
  ...
  throw new Error(
    [
      ...
      "Set NOVA_AI_CODEX_REPO to a checkout of openai/codex, or keep a sibling `codex` checkout next to the primary Nova AI checkout.",
...
async function collectCodexRepoCandidates(repoRoot: string): Promise<string[]> {
  const candidates = [
    process.env.NOVA_AI_CODEX_REPO,
