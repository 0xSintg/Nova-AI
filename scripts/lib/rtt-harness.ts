...
const NOVA_AI_PACKAGE_SPEC_RE =
  /^nova-ai@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;
...
function resolveRttCredentialSource(
  ...
  const rawSource =
    env.NOVA_AI_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.NOVA_AI_QA_CREDENTIAL_SOURCE;
  if (rawSource?.trim()) {
    return parseRttCredentialSource(rawSource);
  }
  if (
    env.CI &&
    env.NOVA_AI_QA_CONVEX_SITE_URL?.trim() &&
    (env.NOVA_AI_QA_CONVEX_SECRET_CI?.trim() || env.NOVA_AI_QA_CONVEX_SECRET_MAINTAINER?.trim())
  ) {
    return "convex";
  }
  return "env";
...
function resolveRttCredentialRole(
  ...
  const rawRole = env.NOVA_AI_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.NOVA_AI_QA_CREDENTIAL_ROLE;
  if (rawRole?.trim()) {
...
export function validateNova AIPackageSpec(spec: string) {
  if (!NOVA_AI_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `Package spec must be nova-ai@main, nova-ai@alpha, nova-ai@beta, nova-ai@latest, or an exact Nova AI release version; got: ${spec}`,
    );
  }
  return spec;
}
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    NOVA_AI_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...(params.packageTgz ? { NOVA_AI_NPM_TELEGRAM_PACKAGE_TGZ: params.packageTgz } : {}),
    NOVA_AI_NPM_TELEGRAM_PACKAGE_LABEL: `${params.spec} (${params.version})`,
    NOVA_AI_NPM_TELEGRAM_PROVIDER_MODE: params.providerMode,
    ...(params.credentialSource
      ? { NOVA_AI_NPM_TELEGRAM_CREDENTIAL_SOURCE: params.credentialSource }
      : {}),
    ...(params.credentialRole
      ? { NOVA_AI_NPM_TELEGRAM_CREDENTIAL_ROLE: params.credentialRole }
      : {}),
    NOVA_AI_NPM_TELEGRAM_SCENARIOS: params.scenarios.join(","),
    NOVA_AI_NPM_TELEGRAM_OUTPUT_DIR: params.rawOutputDir,
    NOVA_AI_NPM_TELEGRAM_FAST: params.baseEnv.NOVA_AI_NPM_TELEGRAM_FAST ?? "1",
    NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    NOVA_AI_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    NOVA_AI_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
...
export function assertRequiredEnv(
  ...
  if (credentialSource === "convex") {
    ...
    if (!env.NOVA_AI_QA_CONVEX_SITE_URL?.trim()) {
      missing.push("NOVA_AI_QA_CONVEX_SITE_URL");
    }
    if (credentialRole === "ci" && !env.NOVA_AI_QA_CONVEX_SECRET_CI?.trim()) {
      missing.push("NOVA_AI_QA_CONVEX_SECRET_CI");
    }
    if (credentialRole === "maintainer" && !env.NOVA_AI_QA_CONVEX_SECRET_MAINTAINER?.trim()) {
      missing.push("NOVA_AI_QA_CONVEX_SECRET_MAINTAINER");
    }
    if (missing.length > 0) {
...
export async function assertHarnessRoot(harnessRoot: string) {
  ...
  try {
    ...
    throw new Error(`Missing Nova AI Telegram npm harness: ${scriptPath}`);
  }
}
...
export async function resolveMainVersion(harnessRoot: string) {
  ...
  if (typeof packageJson.version !== "string" || packageJson.version.trim().length === 0) {
    throw new Error("Nova AI package.json must contain a non-empty version.");
  }
