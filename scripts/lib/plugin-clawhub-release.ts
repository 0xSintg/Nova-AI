...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/nova-ai-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.nova-ai?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.nova-ai?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
export async function collectClawHubNova AIOwnerErrors(params: {
  ...
  const requiredOwnerHandle = params.requiredOwnerHandle ?? "nova-ai";
  const errors: string[] = [];

  await Promise.all(
    params.plugins.map(async (plugin) => {
      if (!plugin.packageName.startsWith("@nova-ai/")) {
        return;
      }
      ...
      if (response.status === 404) {
        errors.push(
          `${plugin.packageName}: ClawHub package row must already exist under @${requiredOwnerHandle} before Nova AI release publish.`,
        );
        return;
      }
      if (!response.ok) {
