...
function isSystemThrottleDisabled(env) {
  const normalized = env.NOVA_AI_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.NOVA_AI_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.NOVA_AI_VITEST_MAX_WORKERS ?? env.NOVA_AI_TEST_WORKERS);
  if (override !== null) {
