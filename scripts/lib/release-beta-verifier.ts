...
const DEFAULT_REPO = "nova-ai/nova-ai";
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--full-release-validation-run ID] [--nova-ai-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--npm-telegram-run ID] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--nova-ai-npm-run":
        parsed.workflowRuns.nova-aiNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const nova-aiNpm = verifyNpmPackage("nova-ai", args.version, args.distTag);
  lines.push(`nova-ai npm OK: ${args.version} (${args.distTag})`);

  if (!args.skipPostpublish) {
    runCommandInherited("node", [
      "--import",
      "tsx",
      "scripts/nova-ai-npm-postpublish-verify.ts",
      args.version,
    ]);
    lines.push("nova-ai postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.nova-aiNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.nova-aiNpm,
        label: "Nova AI NPM Release",
        repo: args.repo,
        expectedWorkflowName: "Nova AI NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          nova-aiNpmIntegrity: nova-aiNpm.integrity,
