...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@nova-ai/${params.dirName}`) {
    return false;
  }
...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.nova-ai?.build?.bundledDist !== false;
}

export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.nova-ai?.extensions)
    ? packageJson.nova-ai.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.nova-ai?.setupEntry === "string" &&
    packageJson.nova-ai.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of collectBundledPluginCandidates(cwd, extensionsRoot)) {
    ...
    const hasManifest = relativeFiles?.includes("nova-ai.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "nova-ai.plugin.json"));
    }
    if (hasPackageJson) {
