...
const RELEASE_OPENWEBUI_COMMAND =
  "NOVA_AI_OPENWEBUI_MODEL=openai/gpt-5.4-mini OPENWEBUI_SMOKE_MODE=models NOVA_AI_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=300 NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui";
export const BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS = 24;
const upgradeSurvivorCommand = "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor";
const rootManagedVpsUpgradeCommand =
  "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:root-managed-vps-upgrade";
const updateRestartAuthCommand =
  "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth";
...
function liveDockerScriptCommand(script, envPrefix = "", options = {}) {
  ...
  const skipBuild = options.skipBuild === false ? "" : "NOVA_AI_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c 'harness="\${NOVA_AI_DOCKER_E2E_TRUSTED_HARNESS_DIR:-}"; if [ -z "$harness" ]; then if [ -d .release-harness/scripts ]; then harness=.release-harness; else harness=.; fi; fi; NOVA_AI_LIVE_DOCKER_REPO_ROOT="\${NOVA_AI_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `NOVA_AI_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} NOVA_AI_BUNDLED_PLUGIN_SWEEP_INDEX=${index} NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
function livePluginToolLane() {
  return liveLane(
    "live-plugin-tool",
    "NOVA_AI_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=300 NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-plugin-tool",
    {
...
function liveOpenAiChatToolsLane() {
  return liveLane(
    "openai-chat-tools",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-chat-tools",
    {
...
export const mainLanes = [
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "NOVA_AI_IMAGE=nova-ai:local-live-gateway NOVA_AI_DOCKER_BUILD_EXTENSIONS=matrix NOVA_AI_LIVE_GATEWAY_PROVIDERS=claude-cli,google-gemini-cli",
      { skipBuild: false },
    ),
    {
      providers: ["claude-cli", "google-gemini-cli"],
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "NOVA_AI_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    needsLiveImage: false,
    provider: "openai",
    resources: ["service"],
  ...
  serviceLane("onboard", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane("codex-on-demand", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand", {
    resources: ["service"],
    stateScenario: "empty",
    weight: 3,
  }),
  serviceLane(
    "codex-media-path",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-media-path",
    {
      resources: ["npm"],
      stateScenario: "empty",
      weight: 3,
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-discord-channel-agent",
    "NOVA_AI_NPM_ONBOARD_CHANNEL=discord NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-slack-channel-agent",
    "NOVA_AI_NPM_ONBOARD_CHANNEL=slack NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "release-user-journey",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-user-journey",
    {
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-typed-onboarding",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-media-memory",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-upgrade-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-plugin-marketplace",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  serviceLane("gateway-network", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  serviceLane(
    "agents-delete-shared-workspace",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  serviceLane("mcp-channels", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  lane("pi-bundle-mcp-tools", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:pi-bundle-mcp-tools", {
    stateScenario: "empty",
  }),
  lane("crestodian-rescue", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-rescue", {
    stateScenario: "empty",
  }),
  lane("crestodian-planner", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-planner", {
    stateScenario: "empty",
  }),
  serviceLane(
    "cron-mcp-cleanup",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
    { resources: ["npm"], stateScenario: "empty", weight: 3 },
  ),
  npmLane("doctor-switch", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch", {
    stateScenario: "empty",
    weight: 3,
  }),
  npmLane(
    "update-channel-switch",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
    {
  ...
  npmLane("skill-install", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install", {
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
    {
      stateScenario: "upgrade-survivor",
  ...
  npmLane("update-migration", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration", {
    stateScenario: "upgrade-survivor",
    timeoutMs: 30 * 60 * 1000,
    weight: 3,
  }),
  lane("plugins", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    "plugins-offline",
    "NOVA_AI_PLUGINS_E2E_CLAWHUB=0 NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      weight: 6,
  ...
  npmLane("plugin-update", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
  lane("openai-image-auth", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-image-auth", {
    stateScenario: "empty",
  }),
  lane(
    "crestodian-first-run",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:crestodian-first-run",
    { stateScenario: "empty" },
  ),
  lane(
    "session-runtime-context",
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:session-runtime-context",
  ),
  lane("commitments-safety", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:commitments-safety", {
    stateScenario: "empty",
  }),
...
export const tailLanes = [
  serviceLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "NOVA_AI_LIVE_CODEX_HARNESS_AUTH=api-key NOVA_AI_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE=1 NOVA_AI_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 NOVA_AI_LIVE_CODEX_HARNESS_MCP_PROBE=0 NOVA_AI_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0 NOVA_AI_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0",
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-codex-harness-docker.sh",
      "NOVA_AI_LIVE_CODEX_BIND=1 NOVA_AI_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts",
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-codex-npm-plugin",
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "NOVA_AI_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "NOVA_AI_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "NOVA_AI_LIVE_ACP_BIND_AGENT=droid NOVA_AI_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "NOVA_AI_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "NOVA_AI_LIVE_ACP_BIND_AGENT=opencode NOVA_AI_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const releasePathPackageInstallOpenAiLanes = [
  npmLane(
    "install-e2e-openai",
    "NOVA_AI_INSTALL_TAG=beta NOVA_AI_E2E_MODELS=openai NOVA_AI_INSTALL_E2E_IMAGE=nova-ai-install-e2e-openai:local NOVA_AI_INSTALL_E2E_AGENT_TOOL_SMOKE=0 NOVA_AI_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini NOVA_AI_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 NOVA_AI_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120 pnpm test:install:e2e",
    {
      resources: ["service"],
...
const releasePathPackageInstallAnthropicLanes = [
  npmLane(
    ...
    "NOVA_AI_INSTALL_TAG=beta NOVA_AI_E2E_MODELS=anthropic NOVA_AI_INSTALL_E2E_IMAGE=nova-ai-install-e2e-anthropic:local pnpm test:install:e2e",
    {
      resources: ["service"],
      weight: 3,
    },
...
const primaryReleasePathChunks = {
  core: [
    lane("qr", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr"),
    serviceLane("onboard", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
      stateScenario: "empty",
      weight: 2,
    }),
    ...
    lane(
      "pi-bundle-mcp-tools",
      "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:pi-bundle-mcp-tools",
      { stateScenario: "empty" },
    ),
    serviceLane("mcp-channels", "NOVA_AI_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
      resources: ["npm"],
...
export function releasePathChunkLanes(chunk, options = {}) {
  ...
  if (!base) {
    throw new Error(
      `NOVA_AI_DOCKER_ALL_CHUNK must be one of: ${[
