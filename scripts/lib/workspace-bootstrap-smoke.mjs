...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "nova-ai-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "nova-ai.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".nova-ai", "workspace");
