...
function createTempHome(): string {
  return makeTempDir(tempDirs, "nova-ai-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-nova-ai.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.nova-ai/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.nova-ai/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".nova-ai", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".nova-ai", "external-plugins", "glueclaw", "nova-ai.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".nova-ai", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.NOVA_AI_LIVE_TEST_QUIET = "1";
    process.env.NOVA_AI_CONFIG_PATH = "~/custom-nova-ai.json5";
    process.env.NOVA_AI_TEST_HOME = priorIsolatedHome;
    process.env.NOVA_AI_STATE_DIR = path.join(priorIsolatedHome, ".nova-ai");
    ...
    expect(process.env.NOVA_AI_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".nova-ai", "nova-ai.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".nova-ai", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".nova-ai", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.NOVA_AI_LIVE_TEST = "1";
    process.env.NOVA_AI_LIVE_USE_REAL_HOME = "1";
    process.env.NOVA_AI_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.NOVA_AI_LIVE_TEST;
    delete process.env.NOVA_AI_LIVE_GATEWAY;
    delete process.env.NOVA_AI_LIVE_USE_REAL_HOME;
    delete process.env.NOVA_AI_LIVE_TEST_QUIET;
