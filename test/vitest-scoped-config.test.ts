...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "nova-ai/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
  it("aliases private QA plugin SDK subpaths for source tests only", () => {
    for (const subpath of PRIVATE_PLUGIN_SDK_SUBPATHS) {
      expect(findAlias(sharedVitestConfig.resolve.alias, `nova-ai/plugin-sdk/${subpath}`)).toEqual(
        {
          find: `nova-ai/plugin-sdk/${subpath}`,
          replacement: path.join(process.cwd(), "src", "plugin-sdk", `${subpath}.ts`),
        },
      );
      expect(() =>
        findAlias(sharedVitestConfig.resolve.alias, `@nova-ai/plugin-sdk/${subpath}`),
      ).toThrow(`missing alias @nova-ai/plugin-sdk/${subpath}`);
    }
  });
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ NOVA_AI_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ NOVA_AI_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ NOVA_AI_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from NOVA_AI_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the nova-ai runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the nova-ai runtime setup", () => {
  ...
  it("keeps the ui lane off both the nova-ai runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from NOVA_AI_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "nova-ai-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
