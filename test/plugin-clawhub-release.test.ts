...
import {
  ...
  collectClawHubNova AIOwnerErrors,
...
import {
  ...
  NOVA_AI_PLUGIN_NPM_REPOSITORY_URL,
...
describe("collectClawHubPublishablePluginPackages", () => {
  it("requires the ClawHub external plugin contract", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "nova-ai.compat.pluginApi is required for external code plugin packages.",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@nova-ai/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@nova-ai/demo-plugin"]);
  });
});

describe("Nova AI dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "diagnostics-otel",
      packageName: "@nova-ai/diagnostics-otel",
    },
    {
      extensionId: "diagnostics-prometheus",
      packageName: "@nova-ai/diagnostics-prometheus",
    },
  ] as const;
  ...
  it("keeps diagnostics plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.nova-ai?.install).toEqual({
        clawhubSpec: `clawhub:${plugin.packageName}`,
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: plugin.packageName,
      });
      expect(packageJson.nova-ai?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@nova-ai/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
      ...
      JSON.stringify(
        {
          ...
          repository: {
            type: "git",
            url: NOVA_AI_PLUGIN_NPM_REPOSITORY_URL,
          },
          nova-ai: {
            extensions: ["./index.ts"],
            compat: {
            ...
            install: {
              npmSpec: "@nova-ai/demo-plugin",
            },
            build: {
              nova-aiVersion: "2026.4.1",
            },
            release: {
              publishToClawHub: true,
            },
...
describe("collectPluginClawHubReleasePlan", () => {
  it("skips versions that already exist on ClawHub", async () => {
    ...
    expect(plan.skippedPublished[0]).toEqual({
      ...
      packageName: "@nova-ai/demo-plugin",
      publishTag: "latest",
      version: "2026.4.1",
    });
  });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@nova-ai/demo-plugin"]);
  });
});

describe("collectClawHubNova AIOwnerErrors", () => {
  it("requires Nova AI-scoped release candidates to already belong to the Nova AI publisher", async () => {
    const errors = await collectClawHubNova AIOwnerErrors({
      plugins: [
        { packageName: "@nova-ai/demo-plugin" },
        { packageName: "@nova-ai/missing-plugin" },
        { packageName: "@other/safe-plugin" },
      ],
      registryBaseUrl: "https://clawhub.ai",
      fetchImpl: async (url) => {
        const pathname = new URL(url instanceof Request ? url.url : url).pathname;
        if (pathname.includes("%40nova-ai%2Fmissing-plugin")) {
    ...
    expect(errors).toEqual([
      "@nova-ai/demo-plugin: ClawHub package owner must be @nova-ai; got @steipete.",
      "@nova-ai/missing-plugin: ClawHub package row must already exist under @nova-ai before Nova AI release publish.",
    ]);
  });

  it("passes when Nova AI-scoped release candidates belong to the Nova AI publisher", async () => {
    const errors = await collectClawHubNova AIOwnerErrors({
      plugins: [{ packageName: "@nova-ai/demo-plugin" }],
      registryBaseUrl: "https://clawhub.ai",
      fetchImpl: async () =>
        new Response(JSON.stringify({ owner: { handle: "nova-ai" } }), {
          status: 200,
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/nova-ai-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@nova-ai/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          NOVA_AI_PLUGIN_NPM_RUNTIME_BUILD: "0",
...
function createTempPluginRepo(
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@nova-ai/${currentExtensionId}`,
          version: "2026.4.1",
          repository: {
            type: "git",
            url: NOVA_AI_PLUGIN_NPM_REPOSITORY_URL,
          ...
          nova-ai: {
            ...
            install: {
              npmSpec: `@nova-ai/${currentExtensionId}`,
            },
            release: {
