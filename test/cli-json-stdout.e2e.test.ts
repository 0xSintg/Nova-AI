...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".novabot");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "novabot.json"), "{}", "utf8");
        ...
        delete env.NOVA_AI_HOME;
        delete env.NOVA_AI_STATE_DIR;
        delete env.NOVA_AI_CONFIG_PATH;
        delete env.VITEST;
