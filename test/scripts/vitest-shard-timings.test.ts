...
describe("scripts/lib/vitest-shard-timings.mjs", () => {
  ...
  it("uses the CI shard name for include-pattern timing keys", () => {
    expect(
      resolveShardTimingKey({
        ...
        env: { NOVA_AI_VITEST_SHARD_NAME: "auto-reply/reply agent dispatch" },
  ...
  it("persists include-pattern timing metadata", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-shard-timings-"));
    tempDirs.push(tempDir);
    const env = {
      NOVA_AI_TEST_PROJECTS_TIMINGS_PATH: path.join(tempDir, "timings.json"),
      NOVA_AI_VITEST_SHARD_NAME: "auto-reply-reply-agent-runner",
    };
    ...
    const persistedTiming = JSON.parse(
      fs.readFileSync(env.NOVA_AI_TEST_PROJECTS_TIMINGS_PATH, "utf8"),
