...
function pr(params: {
  ...
  return {
    ...
    url: `https://github.com/nova-ai/nova-ai/pull/${params.number}`,
  };
}
    ...
    expect(plan).toStrictEqual([
      {
        action: "close",
        candidate,
        comment: `Thanks for the fix. This is now covered by the landed #70532 / commit https://github.com/nova-ai/nova-ai/commit/6415e35.
...
Closing #70592 as a duplicate.`,
  ...
  it("dry-runs through gh reads without mutating", () => {
    ...
    const responses = new Map<string, string>([
      [
        "pr view 70532 --repo nova-ai/nova-ai --json number,title,body,state,mergedAt,mergeCommit,closingIssuesReferences,files,url",
        JSON.stringify(
          pr({
            body: "Fixes #70491",
            mergeCommit: "6415e35",
      ...
      [
        "pr view 70592 --repo nova-ai/nova-ai --json number,title,body,state,mergedAt,mergeCommit,closingIssuesReferences,files,url",
        JSON.stringify(pr({ body: "Closes #70491", number: 70592 })),
      ],
      [
        "pr diff 70532 --repo nova-ai/nova-ai --color=never",
        "diff --git a/ui/src/ui/chat/grouped-render.ts b/ui/src/ui/chat/grouped-render.ts\n@@ -402,8 +402,11 @@",
      ],
      [
        "pr diff 70592 --repo nova-ai/nova-ai --color=never",
        "diff --git a/ui/src/ui/chat/grouped-render.ts b/ui/src/ui/chat/grouped-render.ts\n@@ -286,8 +286,11 @@",
      ],
    ]);
    ...
    const args = parseArgs(["--landed-pr", "70532", "--duplicates", "70592"], {
      GITHUB_REPOSITORY: "nova-ai/nova-ai",
    });
  ...
  it("applies labels, comment, and close commands for close actions", () => {
    ...
    expect(calls).toEqual([
      ...
      ["pr", "comment", "70592", "--repo", "nova-ai/nova-ai", "--body", "closing"],
      ["pr", "close", "70592", "--repo", "nova-ai/nova-ai"],
    ]);
  });
});
