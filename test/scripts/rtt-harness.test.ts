...
describe("RTT harness", () => {
  it("validates Nova AI package specs", () => {
    expect(validateNova AIPackageSpec("nova-ai@main")).toBe("nova-ai@main");
    expect(validateNova AIPackageSpec("nova-ai@alpha")).toBe("nova-ai@alpha");
    expect(validateNova AIPackageSpec("nova-ai@beta")).toBe("nova-ai@beta");
    expect(validateNova AIPackageSpec("nova-ai@latest")).toBe("nova-ai@latest");
    expect(validateNova AIPackageSpec("nova-ai@2026.4.30")).toBe("nova-ai@2026.4.30");
    expect(validateNova AIPackageSpec("nova-ai@2026.4.30-beta.2")).toBe(
      "nova-ai@2026.4.30-beta.2",
    );
    expect(validateNova AIPackageSpec("nova-ai@2026.4.30-alpha.2")).toBe(
      "nova-ai@2026.4.30-alpha.2",
    );

    expect(() => validateNova AIPackageSpec("@nova-ai/nova-ai@beta")).toThrow(
      /Package spec must be/,
    );
    expect(() => validateNova AIPackageSpec("nova-ai@next")).toThrow(/Package spec must be/);
  });

  it("builds stable run labels", () => {
    expect(safeRunLabel("nova-ai@beta")).toBe("nova-ai_beta");
    expect(
      buildRunId({
    ...
    ).toBe("2026-05-01T030405678Z-nova-ai_beta-2");
  });

  it("constructs harness env without dropping caller env", () => {
    const env = createHarnessEnv({
      baseEnv: {
        NOVA_AI_QA_TELEGRAM_GROUP_ID: "-100123",
        NOVA_AI_NPM_TELEGRAM_FAST: "0",
      },
    ...
    expect(env.NOVA_AI_QA_TELEGRAM_GROUP_ID).toBe("-100123");
    expect(env.NOVA_AI_NPM_TELEGRAM_PACKAGE_SPEC).toBe("nova-ai@beta");
    expect(env.NOVA_AI_NPM_TELEGRAM_PACKAGE_LABEL).toBe("nova-ai@beta (2026.4.30-beta.1)");
    expect(env.NOVA_AI_NPM_TELEGRAM_PROVIDER_MODE).toBe("mock-openai");
    expect(env.NOVA_AI_NPM_TELEGRAM_SCENARIOS).toBe("telegram-mentioned-message-reply");
    expect(env.NOVA_AI_NPM_TELEGRAM_OUTPUT_DIR).toBe(".artifacts/rtt/run/raw");
    expect(env.NOVA_AI_NPM_TELEGRAM_FAST).toBe("0");
    expect(env.NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES).toBe("20");
    expect(env.NOVA_AI_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS).toBe("30000");
    expect(env.NOVA_AI_QA_TELEGRAM_CANARY_TIMEOUT_MS).toBe("180000");
    expect(env.NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS).toBe("180000");
  });

  it("forwards Convex credential controls without dropping RTT sample controls", () => {
    const env = createHarnessEnv({
      baseEnv: {
        NOVA_AI_QA_CONVEX_SITE_URL: "https://qa-credentials.example.convex.site",
        NOVA_AI_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
      },
    ...
    expect(env.NOVA_AI_NPM_TELEGRAM_CREDENTIAL_SOURCE).toBe("convex");
    expect(env.NOVA_AI_NPM_TELEGRAM_CREDENTIAL_ROLE).toBe("maintainer");
    expect(env.NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES).toBe("7");
    expect(env.NOVA_AI_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS).toBe("45000");
    expect(() =>
  ...
  it("exports the Telegram bot token after Convex credentials are sourced", async () => {
    ...
    expect(script).not.toContain('export TELEGRAM_BOT_TOKEN="$NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN"');
  });
  ...
  it("generates final-only Telegram RTT delivery config for release packages", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-rtt-config-test-"));
  ...
  it("builds normalized result JSON", async () => {
    ...
    expect(result).toStrictEqual({
      ...
      package: { spec: "nova-ai@beta", version: "2026.4.30-beta.1" },
      run: {
        durationMs: 12_000,
  ...
  it("appends JSONL rows", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-rtt-test-"));
    tempDirs.push(tempDir);
  ...
  it("parses CLI options", () => {
    ...
    expect(parsed.spec).toBe("nova-ai@latest");
    expect(parsed.options).toStrictEqual({
      packageTgz: "/tmp/nova-ai.tgz",
      credentialRole: "ci",
      credentialSource: "convex",
