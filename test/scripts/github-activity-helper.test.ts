...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/nova-ai-pr-maintainer/scripts/github-activity.sh",
);
...
describe("nova-ai-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("nova-ai/nova-ai last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/nova-ai\/nova-ai\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/nova-ai\/nova-ai\/commits/g)).toHaveLength(1);
