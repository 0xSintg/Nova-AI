  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "nova-ai/plugin-sdk/foo";',
          'export type * from "nova-ai/plugin-sdk/bar";',
          'export { named } from "nova-ai/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "nova-ai/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "nova-ai/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "nova-ai/plugin-sdk/foo";',
          'export type { Named } from "nova-ai/plugin-sdk/foo";',
