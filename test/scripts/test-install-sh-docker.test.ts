...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/nova-ai-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/nova-ai-live-and-e2e-checks-reusable.yml";
...
describe("test-install-sh-docker", () => {
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(runner).toContain(
      'UPDATE_BASELINE_VERSION="${NOVA_AI_INSTALL_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(workflow).toContain(
      "NOVA_AI_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${NOVA_AI_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
  ...
  it("passes the baked browser build arg through Docker setup", () => {
    ...
    expect(script).toContain('export NOVA_AI_INSTALL_BROWSER="${NOVA_AI_INSTALL_BROWSER:-}"');
    expect(script).toContain("NOVA_AI_INSTALL_BROWSER \\");
    expect(script).toContain('--build-arg "NOVA_AI_INSTALL_BROWSER=${NOVA_AI_INSTALL_BROWSER}"');
  });
  ...
  it("passes image-scoped pip packages through Docker and Podman setup", () => {
    ...
    expect(dockerfile).toContain("ARG NOVA_AI_IMAGE_PIP_PACKAGES");
    expect(dockerfile).toContain(
      "python3 -m pip install --no-cache-dir --break-system-packages $NOVA_AI_IMAGE_PIP_PACKAGES",
    );
    expect(dockerSetup).toContain(
      'export NOVA_AI_IMAGE_PIP_PACKAGES="${NOVA_AI_IMAGE_PIP_PACKAGES:-}"',
    );
    expect(dockerSetup).toContain("NOVA_AI_IMAGE_PIP_PACKAGES \\");
    expect(dockerSetup).toContain(
      '--build-arg "NOVA_AI_IMAGE_PIP_PACKAGES=${NOVA_AI_IMAGE_PIP_PACKAGES}"',
    );
    expect(dockerSetup).not.toContain("NOVA_AI_DOCKER_PIP_PACKAGES");
    expect(podmanSetup).toContain('NOVA_AI_IMAGE_PIP_PACKAGES="${NOVA_AI_IMAGE_PIP_PACKAGES:-}"');
    expect(podmanSetup).toContain(
      'BUILD_ARGS+=(--build-arg "NOVA_AI_IMAGE_PIP_PACKAGES=${NOVA_AI_IMAGE_PIP_PACKAGES}")',
    );
    expect(podmanSetup).not.toContain("NOVA_AI_DOCKER_PIP_PACKAGES");
  });
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an Nova AI branch or release tag");
  });
  ...
  it("writes the package dist inventory before packing ignore-scripts tarballs", () => {
    ...
    expect(script).toContain("node scripts/check-nova-ai-package-tarball.mjs");
  });
});
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain(
      'HEARTBEAT_INTERVAL="${NOVA_AI_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"',
    );
    expect(script).toContain(
      'INSTALL_COMMAND_TIMEOUT="${NOVA_AI_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"',
    );
    ...
    expect(script).toContain("nova-ai update --tag");
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${NOVA_AI_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${NOVA_AI_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("NOVA_AI_INSTALL_SMOKE_MODE=npm-global");
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("NOVA_AI_BUN_GLOBAL_SMOKE_DIST_IMAGE");
  });
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "NOVA_AI_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("NOVA_AI_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain('NOVA_AI_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
