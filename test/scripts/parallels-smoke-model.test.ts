...
describe("Parallels smoke model selection", () => {
  ...
  it("keeps provider auth and model defaults in the shared TypeScript helper", () => {
    ...
    expect(providerAuth).toContain("NOVA_AI_PARALLELS_OPENAI_MODEL");
    expect(providerAuth).toContain("NOVA_AI_PARALLELS_WINDOWS_OPENAI_MODEL");
...
`;
  ...
  it("keeps snapshot, host, package, and quote helpers shared", () => {
    ...
    expect(packageArtifact).toContain("export async function packNova AI");
  ...
  it("quotes shell args and resolves fuzzy snapshot hints through the shared TypeScript helper", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "nova-ai-parallels-helper-"));
...
`,
  ...
  it("uses one Ubuntu VM fallback resolver for Linux lanes", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "nova-ai-parallels-vm-helper-"));
    const prlctlPath = join(tempDir, "prlctl");
    writeFileSync(
      prlctlPath,
      `#!/usr/bin/env bash
...
`,
  ...
  it("keeps Linux bad-plugin diagnostics gated for historical update baselines", () => {
    ...
    expect(script).toContain("parseNova AIPackageVersion");
...
`;
    ...
    expect(
      JSON.parse(
        runTsEval(source, {
          ...
          NOVA_AI_PARALLELS_WINDOWS_OPENAI_MODEL: "openai/custom-windows",
        }),
      ),
    ).toEqual({
      apiKeyEnv: "OPENAI_API_KEY",
  ...
  it("preseeds dev update channel before stable-to-dev update lanes", () => {
    ...
    expect(macos).toContain("NOVA_AI_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1");
    expect(windows).toContain("NOVA_AI_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS");
  });
  ...
  it("passes aggregate model overrides into each OS fresh lane", () => {
    ...
    expect(script).toContain("NOVA_AI_PARALLELS_LINUX_DISABLE_BONJOUR");
  });
  ...
  it("keeps aggregate update guest scripts isolated from the npm-update orchestrator", () => {
    ...
    expect(updateScripts).toContain("Invoke-Nova AI update");
  ...
  it("keeps Windows gateway reachability on a real deadline with start recovery", () => {
    ...
    expect(script).toContain("NOVA_AI_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S");
  ...
  it("runs Windows ref onboarding through a detached done-file runner", () => {
    ...
    expect(transports).toContain("__NOVA_AI_LOG_OFFSET__");
...
`),
  ...
  it("runs the Windows agent turn through the detached done-file runner", () => {
    ...
    expect(script).toContain('guestPowerShellBackground(\n      "agent-turn"');
    expect(script).toContain("NOVA_AI_PARALLELS_WINDOWS_AGENT_TIMEOUT_S");
    expect(script).toContain("NOVA_AI_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700");
...
`;
    ...
    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      "NOVA_AI_PARALLELS_MACOS_AGENT_TIMEOUT_S || 2700",
    );
    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
  ...
  it("keeps Windows update-only env flags scoped before verification", () => {
    ...
    expect(windows).toContain(
      "Invoke-WithScopedEnv @{ NOVA_AI_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS",
    );
    expect(windows).toContain("$script:Nova AIUpdateExit = $LASTEXITCODE");
    expect(windows).not.toContain("$env:NOVA_AI_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("resolves Windows Nova AI commands without assuming the npm shim path", () => {
    const powershell = readFileSync(TS_PATHS.powershell, "utf8");
    const windows = readFileSync(TS_PATHS.windows, "utf8");

    expect(powershell).toContain("windowsNova AIResolver");
    ...
    expect(powershell).toContain("NOVA_AI_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED");
    ...
    expect(powershell).toContain("Resolve-Nova AICommand");
    ...
    expect(windows).toContain("windowsNova AIResolver");
    expect(windows).toContain("Invoke-Nova AI gateway");
