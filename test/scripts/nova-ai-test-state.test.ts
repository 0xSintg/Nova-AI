...
const scriptPath = path.join(repoRoot, "scripts/lib/nova-ai-test-state.mjs");
...
describe("scripts/lib/nova-ai-test-state", () => {
  it("creates a sourceable env file and JSON description", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-test-state-script-"));
    ...
    try {
      ...
      expect(payload.stateDir).toBe(path.join(payload.home, ".nova-ai"));
      expect(payload.configPath).toBe(path.join(payload.stateDir, "nova-ai.json"));
      expect(payload.workspaceDir).toBe(path.join(payload.home, "workspace"));
      expect(payload.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY).toMatch(secretKeyPattern);
      expect(payload.env).toEqual({
        HOME: payload.home,
        USERPROFILE: payload.home,
        NOVA_AI_HOME: payload.home,
        NOVA_AI_STATE_DIR: payload.stateDir,
        NOVA_AI_CONFIG_PATH: payload.configPath,
        NOVA_AI_AUTH_PROFILE_SECRET_KEY: payload.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY,
      });
      ...
      expect(envFileText).toContain("export NOVA_AI_HOME=");
      expect(envFileText).toContain("export NOVA_AI_STATE_DIR=");
      expect(envFileText).toContain("export NOVA_AI_CONFIG_PATH=");
      expect(envFileText).toContain("export NOVA_AI_AUTH_PROFILE_SECRET_KEY=");

      const probe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(envFile)}; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.NOVA_AI_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,stateDir:process.env.NOVA_AI_STATE_DIR,secretKey:process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY,channel:config.update.channel}));'`,
      ]);
      expect(JSON.parse(probe.stdout)).toEqual({
        home: payload.home,
        stateDir: payload.stateDir,
        secretKey: payload.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY,
        channel: "stable",
      });
  ...
  it("renders a Docker-friendly shell snippet", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-test-state-shell-"));
    ...
    try {
      ...
      expect(stdout).toContain(
        'NOVA_AI_TEST_STATE_TMP_ROOT="${NOVA_AI_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
      );
      expect(stdout).toContain(
        'mktemp -d "$NOVA_AI_TEST_STATE_TMP_ROOT/nova-ai-update-channel-switch-update-stable-home.XXXXXX"',
      );
      expect(stdout).toContain("NOVA_AI_TEST_STATE_JSON");
      expect(stdout).toContain('"channel": "stable"');
      await fs.writeFile(snippetFile, stdout, "utf8");

      const probe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(snippetFile)}; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.NOVA_AI_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,nova-aiHome:process.env.NOVA_AI_HOME,workspace:process.env.NOVA_AI_TEST_WORKSPACE_DIR,secretKey:process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY,channel:config.update.channel}));'; rm -rf "$HOME"`,
      ]);

      const payload = JSON.parse(probe.stdout);
      expect(payload.home.startsWith(os.tmpdir())).toBe(true);
      expect(path.basename(payload.home)).toMatch(
        /^nova-ai-update-channel-switch-update-stable-home\./u,
      );
      expect(payload.nova-aiHome).toBe(payload.home);
      ...
      const customProbe = await execFileAsync("bash", [
        "-lc",
        `export NOVA_AI_TEST_STATE_TMPDIR=${shellQuote(customTemp)}; source ${shellQuote(snippetFile)}; node -e 'process.stdout.write(JSON.stringify({home:process.env.HOME,tmpRoot:process.env.NOVA_AI_TEST_STATE_TMP_ROOT}));'; rm -rf "$HOME"`,
      ]);
      ...
      expect(customPayload.home).toMatch(
        new RegExp(
          `^${escapeRegex(customTemp)}/nova-ai-update-channel-switch-update-stable-home\\.`,
        ),
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("keeps shell key generation independent of node", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-test-state-path-node-"));
    ...
    try {
      ...
      const shellProbe = await execFileAsync("bash", [
        "-lc",
        `export PATH=${shellQuote(fakeBin)}:$PATH; source ${shellQuote(snippetFile)}; printf '%s' "$NOVA_AI_AUTH_PROFILE_SECRET_KEY"; rm -rf "$HOME"`,
      ]);
      ...
      const functionProbe = await execFileAsync("bash", [
        "-lc",
        `export PATH=${shellQuote(fakeBin)}:$PATH; export NOVA_AI_TEST_STATE_TMPDIR=${shellQuote(path.join(tempRoot, "function-tmp"))}; source ${shellQuote(functionFile)}; nova-ai_test_state_create "path node" minimal; printf '%s' "$NOVA_AI_AUTH_PROFILE_SECRET_KEY"; rm -rf "$HOME"`,
      ]);
  ...
  it("renders a reusable Docker shell function", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-test-state-function-"));
    ...
    try {
      ...
      expect(stdout).toContain("nova-ai_test_state_create()");
      expect(stdout).toContain("unset NOVA_AI_AGENT_DIR");
      expect(stdout).toContain("update-stable");
      await fs.writeFile(snippetFile, stdout, "utf8");

      const probe = await execFileAsync("bash", [
        "-lc",
        `export NOVA_AI_TEST_STATE_TMPDIR=${shellQuote(path.join(tempRoot, "function-tmp"))}; source ${shellQuote(snippetFile)}; export NOVA_AI_AGENT_DIR=/tmp/outside-agent; nova-ai_test_state_create "onboard case" minimal; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.NOVA_AI_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,tmpDir:process.env.NOVA_AI_TEST_STATE_TMPDIR,agentDir:process.env.NOVA_AI_AGENT_DIR || null,workspace:process.env.NOVA_AI_TEST_WORKSPACE_DIR,secretKey:process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY,config}));'; rm -rf "$HOME"`,
      ]);

      const payload = JSON.parse(probe.stdout);
      expect(payload.home).toBe(`${payload.tmpDir}/${path.basename(payload.home)}`);
      expect(payload.home).toContain("/nova-ai-onboard-case-minimal-home.");
      ...
      const existingProbe = await execFileAsync("bash", [
        "-lc",
        `source ${shellQuote(snippetFile)}; nova-ai_test_state_create ${shellQuote(existingHome)} minimal; firstKey="$NOVA_AI_AUTH_PROFILE_SECRET_KEY"; export firstKey; printf '{"kept":true}\\n' > "$NOVA_AI_CONFIG_PATH"; nova-ai_test_state_create ${shellQuote(existingHome)} empty; node -e 'const fs=require("node:fs"); const config=JSON.parse(fs.readFileSync(process.env.NOVA_AI_CONFIG_PATH,"utf8")); process.stdout.write(JSON.stringify({home:process.env.HOME,secretKey:process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY,firstKey:process.env.firstKey,config}));'`,
      ]);
  ...
  it("keeps onboard Docker temp homes on the shared test-state helper", async () => {
    ...
    expect(scriptText).toContain("NOVA_AI_TEST_STATE_FUNCTION_B64");
    expect(scriptText).toContain("scripts/e2e/lib/onboard/scenario.sh");
    expect(scenarioText).toContain("set_isolated_nova-ai_env local-basic");
