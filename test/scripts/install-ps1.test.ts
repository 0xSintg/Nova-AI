...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "Nova AI installation failed with exit code/);
  });

  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-Nova AI");
  ...
  it("cleans legacy git submodules only from the selected git checkout", () => {
    const gitInstallBody = extractFunctionBody(source, "Install-Nova AIFromGit");
    const mainBody = extractFunctionBody(source, "Main");
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveNova AICommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveNova AICommand onboard");
  });

  runIfPowerShell("exits non-zero when run as a script file", () => {
    const tempDir = harness.createTempDir("nova-ai-install-ps1-");
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=Nova AI installation failed with exit code 1.");
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingNova AI { return $false }",
        "function Add-ToPath { param([string]$Path) }",
        "function Install-Nova AI { Write-Output 'npm stdout'; return $true }",
        "function Ensure-Nova AIOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-Nova AICommand { return 'Nova AI test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
