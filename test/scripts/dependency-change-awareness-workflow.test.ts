...
describe("dependency change awareness workflow", () => {
  ...
  it("uses a pinned GitHub Script action and bounded sticky comments", () => {
    ...
    expect(step.with?.script).toContain("<!-- nova-ai:dependency-change-awareness -->");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-change-awareness.yml @nova-ai/nova-ai-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-change-awareness-workflow.test.ts @nova-ai/nova-ai-secops",
    );
  });
});
