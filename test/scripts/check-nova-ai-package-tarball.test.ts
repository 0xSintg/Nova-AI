...
const CHECK_SCRIPT = "scripts/check-nova-ai-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "nova-ai-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "nova-ai", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><nova-ai-app></nova-ai-app>",
    ...
    const tarball = join(root, "nova-ai.tgz");
...
describe("check-nova-ai-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Nova AI package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
