...
describe("scripts/test-live-codex-harness-docker.sh", () => {
  it("mounts cache and npm tool dirs outside the bind-mounted Docker home", () => {
    ...
    expect(script).toContain('DOCKER_CACHE_CONTAINER_DIR="/tmp/nova-ai-cache"');
    expect(script).toContain('DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/nova-ai-npm-global"');
    expect(script).toContain("nova-ai_live_codex_harness_is_ci()");
  ...
  it("fails before Docker build when codex-auth has no host auth file", () => {
    ...
    expect(script).toContain(
      "NOVA_AI_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image",
    );
    expect(script).toContain(
      "If this is a Testbox/API-key run, set NOVA_AI_LIVE_CODEX_HARNESS_AUTH=api-key and run through nova-ai-testbox-env.",
    );
    expect(script.indexOf("requires ~/.codex/auth.json before building")).toBeLessThan(
      script.indexOf('NOVA_AI_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR"'),
    );
  });
  ...
  it("keeps API-key runs on the ephemeral Docker home", () => {
    ...
    expect(script.indexOf('PROFILE_STATUS="api-key-env"')).toBeLessThan(
      script.indexOf("nova-ai_live_append_array DOCKER_RUN_ARGS PROFILE_MOUNT"),
    );
