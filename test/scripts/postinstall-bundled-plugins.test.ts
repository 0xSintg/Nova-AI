...
async function createExtensionsDir() {
  const root = await createTempDirAsync("nova-ai-postinstall-");
...
describe("bundled plugin postinstall", () => {
  ...
  it("recognizes direct invocation through symlinked temp prefixes", () => {
    ...
    expect(
      isDirectPostinstallInvocation({
        entryPath: "/var/folders/tmp/nova-ai/scripts/postinstall-bundled-plugins.mjs",
        modulePath: "/private/var/folders/tmp/nova-ai/scripts/postinstall-bundled-plugins.mjs",
        realpathSync,
      }),
    ).toBe(true);
  });
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    const configuredBase = path.join("/tmp", "nova-ai-cache");
    ...
    expect(removed).not.toContain(path.join(configuredBase, "nova-ai"));
  ...
  it("keeps pruning sibling compile cache dirs after one removal fails", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] could not prune Nova AI compile cache: Error: locked",
    );
  });

  it("does not warn when compile-cache pruning hits EACCES or EPERM (shared caches)", () => {
    const base = path.join("/tmp", "nova-ai-shared-compile-cache");
  ...
  it("does not warn when the compile-cache base directory cannot be listed (EACCES)", () => {
    const base = path.join("/tmp", "nova-ai-compile-cache-no-list");
    const warn = vi.fn();
    const rmSync = vi.fn();
  ...
  it("patches the Baileys upload helper dispatcher guard", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-baileys-postinstall-");
  ...
  it("prunes source-checkout bundled plugin node_modules", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-source-checkout-");
  ...
  it("keeps source-checkout prune non-fatal", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-source-checkout-prune-error-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("does not prune user-state legacy runtime deps during source-checkout postinstall", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-source-checkout-state-skip-");
    const home = await createTempDirAsync("nova-ai-source-checkout-home-");
    const legacyRuntimeRoot = path.join(home, ".nova-ai", "plugin-runtime-deps");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
  ...
  it("honors disable env before source-checkout pruning", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-source-checkout-disabled-");
    const extensionsDir = path.join(packageRoot, "extensions");
    await fs.mkdir(path.join(packageRoot, ".git"), { recursive: true });
    await fs.mkdir(path.join(packageRoot, "src"), { recursive: true });
    ...
    runBundledPluginPostinstall({
      env: { NOVA_AI_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1" },
      packageRoot,
      log: { log: vi.fn(), warn: vi.fn() },
    });
  ...
  it("migrates the plugin registry during postinstall from built dist contracts", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-postinstall-registry-");
  ...
  it("surfaces deprecated plugin registry migration break-glass warnings", async () => {
    ...
    const migratePluginRegistryForInstall = vi.fn(async () => ({
      ...
      migrated: false,
      preflight: {
        deprecationWarnings: ["NOVA_AI_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated"],
      },
    }));
    ...
    expect(warn).toHaveBeenCalledWith(
      "[postinstall] NOVA_AI_FORCE_PLUGIN_REGISTRY_MIGRATION is deprecated",
    );
  });
  ...
  it("honors plugin registry postinstall migration disable env", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { NOVA_AI_DISABLE_PLUGIN_REGISTRY_MIGRATION: "1" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("does not disable plugin registry migration for falsey env flag strings", async () => {
    ...
    await expect(
      runPluginRegistryPostinstallMigration({
        packageRoot: "/pkg",
        env: { NOVA_AI_DISABLE_PLUGIN_REGISTRY_MIGRATION: "0" },
        existsSync: vi.fn(() => true),
        importModule,
        log: { log: vi.fn(), warn: vi.fn() },
      }),
  ...
  it("prunes stale dist files from packaged installs", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-");
  ...
  it("prunes legacy plugin runtime deps state during packaged postinstall", async () => {
    const prefix = await createTempDirAsync("nova-ai-packaged-prefix-");
    const packageRoot = path.join(prefix, "lib", "node_modules", "nova-ai");
    const nodeModulesRoot = path.dirname(packageRoot);
    const home = await createTempDirAsync("nova-ai-packaged-home-");
    const stateOverride = path.join(home, "custom-state");
    const systemState = path.join(home, "system-state");
    const defaultLegacyRoot = path.join(home, ".nova-ai", "plugin-runtime-deps");
    const oldBrandLegacyRoot = path.join(home, ".novabot", "plugin-runtime-deps");
    ...
    runBundledPluginPostinstall({
      env: {
        HOME: home,
        NOVA_AI_STATE_DIR: stateOverride,
  ...
  it("keeps legacy plugin runtime deps cleanup non-fatal", () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(
      1,
      "[postinstall] could not prune legacy plugin runtime deps /home/alice/.novabot/plugin-runtime-deps: Error: locked",
    );
    expect(warn).toHaveBeenNthCalledWith(
      2,
      "[postinstall] could not prune legacy plugin runtime deps /home/alice/.nova-ai/plugin-runtime-deps: Error: locked",
    );
  });

  it("resolves legacy plugin runtime deps roots from Nova AI state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          NOVA_AI_HOME: "/srv/nova-ai-home",
          NOVA_AI_CONFIG_PATH: "~/profile/nova-ai.json",
          NOVA_AI_STATE_DIR: "~/state",
          STATE_DIRECTORY: "/var/lib/nova-ai",
        },
        homedir: () => "/users/alice",
      }),
    ).toEqual([
      "/srv/nova-ai-home/.novabot/plugin-runtime-deps",
      "/srv/nova-ai-home/.nova-ai/plugin-runtime-deps",
      "/srv/nova-ai-home/profile/plugin-runtime-deps",
      "/srv/nova-ai-home/state/plugin-runtime-deps",
      "/var/lib/nova-ai/plugin-runtime-deps",
    ]);
  });

  it("keeps imported dist chunks even when inventory is stale", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-import-");
  ...
  it("does not abort dist pruning when a listed chunk disappears before import expansion", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-missing-chunk-");
  ...
  it("prunes stale private QA files without restoring compat sidecars", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-qa-compat-");
    const currentFile = path.join(packageRoot, "dist", "entry.js");
    ...
    await expectPathMissing(
      path.join(packageRoot, "dist", "extensions", "qa-channel", "nova-ai.plugin.json"),
    );
    await expectPathMissing(
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is missing", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-missing-inventory-");
    const staleFile = path.join(packageRoot, "dist", "channel-CJUAgRQR.js");
  ...
  it("keeps packaged postinstall non-fatal when the dist inventory is invalid", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-invalid-inventory-");
    const currentFile = path.join(packageRoot, "dist", "channel-BOa4MfoC.js");
  ...
  it("prunes stale bundled plugin dependency debris from packaged dist", async () => {
    const packageRoot = await createTempDirAsync("nova-ai-packaged-install-dist-prune-");
