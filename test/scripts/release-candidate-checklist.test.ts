...
describe("release candidate checklist", () => {
  ...
  it("extracts a workflow run id from gh dispatch output", () => {
    expect(
      parseRunIdFromDispatchOutput(
        "https://github.com/nova-ai/nova-ai/actions/runs/25922042055\n",
      ),
    ).toBe("25922042055");
  });
  ...
  it("falls back to a single compatible artifact from the same run", () => {
    expect(
      resolveArtifactName(
        [{ name: "nova-ai-npm-preflight-dba00", expired: false }],
        "nova-ai-npm-preflight-v2026.5.16-beta.2",
        "nova-ai-npm-preflight-",
      ),
    ).toBe("nova-ai-npm-preflight-dba00");
  });
});
