...
function realBehaviorProofBody(evidence: string, overrides: Record<string, string> = {}) {
  const fields = {
    ...
    environment: "macOS 15.4, Node 24, local Nova AI gateway, redacted Discord token.",
    steps: "pnpm nova-ai gateway restart and pnpm nova-ai gateway status",
    evidence,
...
describe("barnacle-auto-response", () => {
  it("keeps Barnacle-owned labels documented and ClawHub spelled correctly", () => {
    ...
    expect(managedLabelSpecs["r: skill"].description).not.toContain("Novahub");
  ...
  it("does not classify a linked core plugin auto-enable fix as an external plugin candidate", () => {
    const labels = classifyPullRequestCandidateLabels(
      pr(
        ...
        [
          "- Problem: nova-ai doctor --fix adds duplicate installed plugin entries",
  ...
  it("does not close automation PRs for the active PR limit", async () => {
    for (const automationPullRequest of [
      { head: { ref: "clawsweeper/nova-ai-nova-ai-73880" }, login: "app/nova-ai-clawsweeper" },
      { headRefName: "clawsweeper/nova-ai-nova-ai-73880", login: "app/nova-ai-clawsweeper" },
      {
        head: { ref: "clownfish/ghcrawl-156993-autonomous-smoke" },
        login: "app/nova-ai-clownfish",
      },
      { headRefName: "clownfish/ghcrawl-156993-autonomous-smoke", login: "app/nova-ai-clownfish" },
    ]) {
      const { calls, github } = barnacleGithub([]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [PROOF_SUPPLIED_LABEL, PROOF_SUFFICIENT_LABEL],
        {
          ...
          sender: { login: "nova-ai-clawsweeper[bot]", type: "Bot" },
        },
      ),
      core: {
        info: () => undefined,
  ...
  it("actions manually applied candidate labels", async () => {
    const { calls, github } = barnacleGithub([file("extensions/example/nova-ai.plugin.json")]);
  ...
  it("keeps bot-applied candidate labels passive", async () => {
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext({}, [candidateLabels.externalPluginCandidate], {
        action: "labeled",
        label: { name: candidateLabels.externalPluginCandidate },
        sender: { login: "nova-ai-bot[bot]", type: "Bot" },
      }),
      core: {
        info: () => undefined,
      },
