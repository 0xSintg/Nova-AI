...
describe("scripts/test-extension.mjs", () => {
  ...
  it("can fail safe to all extensions when the base revision is unavailable", () => {
    const extensionIds = listChangedExtensionIds({
      base: "refs/heads/nova-ai-test-missing-base",
  ...
  it("runs extension batch config groups concurrently when requested", async () => {
    ...
    const runPromise = runExtensionBatchPlan(
      ...
      {
        env: { NOVA_AI_EXTENSION_BATCH_PARALLEL: "2" },
        runGroup,
    ...
    expect(firstRunGroupParams).toEqual({
      ...
      config: "heavy",
      env: {
        NOVA_AI_EXTENSION_BATCH_PARALLEL: "2",
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { NOVA_AI_EXTENSION_BATCH_PARALLEL: "2" })).toBe(2);
    expect(resolveExtensionBatchParallelism(1, { NOVA_AI_EXTENSION_BATCH_PARALLEL: "4" })).toBe(1);
    expect(resolveExtensionBatchParallelism(3, { NOVA_AI_EXTENSION_BATCH_PARALLEL: "nope" })).toBe(
      1,
    );
  });
