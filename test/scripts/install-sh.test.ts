...
describe("install.sh", () => {
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "nova-ai-install-nvm-"));
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "nova-ai-install-node-promote-"));
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "nova-ai-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "nova-ai-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g nova-ai@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "nova-ai-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute nova-ai path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "nova-ai-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const nova-aiBin = join(npmBin, "nova-ai");
    writeFileSync(nova-aiBin, "#!/bin/sh\nexit 0\n");
    chmodSync(nova-aiBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(nova-ai_command_for_user "${nova-aiBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${nova-aiBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=nova-ai");
  });
  ...
  it("resolves requested git install versions to checkout refs", () => {
    const result = runInstallShell(`
      ...
      source "${SCRIPT_PATH}"
      npm() {
        if [[ "$1" == "view" && "$2" == "nova-ai" && "$3" == "dist-tags.beta" ]]; then
          printf '2026.5.12-beta.3\\n'
          return 0
        fi
        return 1
      }
      NOVA_AI_VERSION=v2026.5.12-beta.3
      printf 'tag=%s\\n' "$(resolve_git_nova-ai_ref)"
      NOVA_AI_VERSION=2026.5.12-beta.3
      printf 'semver=%s\\n' "$(resolve_git_nova-ai_ref)"
      NOVA_AI_VERSION=beta
      printf 'beta=%s\\n' "$(resolve_git_nova-ai_ref)"
      NOVA_AI_VERSION=main
      printf 'main=%s\\n' "$(resolve_git_nova-ai_ref)"
    `);
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "nova-ai-install-sh-gum-"));
    try {
...
describe("install.sh duplicate Nova AI install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/nova-ai" "$root/fnm/nova-ai"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/nova-ai/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/nova-ai/package.json"
      collect_nova-ai_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      NOVA_AI_BIN="$root/fnm/.bin/nova-ai"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_nova-ai_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Nova AI global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/nova-ai");
    expect(result.stdout).toContain("/fnm/nova-ai");
    expect(result.stdout).toContain("Active nova-ai:");
    expect(result.stdout).toContain("npm uninstall -g nova-ai");
  });

  it("stays quiet when only one Nova AI npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/nova-ai"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/nova-ai/package.json"
      collect_nova-ai_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_nova-ai_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Nova AI global installs detected");
  });
});
