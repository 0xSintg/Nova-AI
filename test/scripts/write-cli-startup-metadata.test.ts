...
describe("write-cli-startup-metadata", () => {
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("nova-ai-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: nova-ai\n",
      renderSourceBrowserHelpText: () => "Usage: nova-ai browser\n",
    });
    ...
    expect(written.browserHelpText).toContain("nova-ai browser");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("nova-ai");
  });
});
