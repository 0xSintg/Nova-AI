...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "nova-ai-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "clawhub",
          spec: "clawhub:@nova-ai/matrix",
          installPath: matrixInstallDir,
          clawhubPackage: "@nova-ai/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        NOVA_AI_TEST_WORKSPACE_DIR: workspace,
        NOVA_AI_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        NOVA_AI_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  ...
  it("rejects ClawHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "nova-ai-upgrade-survivor-outside-"));
    try {
      expect(() =>
