...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "nova-ai-changelog-credit-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$NOVA_AI_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$NOVA_AI_TEST_CONTRIB"',
    ],
    {
      NOVA_AI_PR_CHANGELOG_SH: changelogScriptPath,
      NOVA_AI_TEST_CONTRIB: contrib,
    },
  );
}
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @nova-ai.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @Nova AI.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "nova-ai", text: "- Org-owned fix. Thanks @nova-ai." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "nova-ai", text: "- Mixed credit. Thanks @contributor and @Nova AI." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @nova-ai and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "nova-ai",
        text: "- Mixed credit (#123). Thanks @nova-ai and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("nova-ai")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("nova-ai-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("nova-ai-clawsweeper[bot]")).toBe(true);
