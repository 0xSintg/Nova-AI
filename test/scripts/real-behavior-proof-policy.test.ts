...
function proofBody(evidence: string, overrides: Record<string, string> = {}) {
  const fields = {
    ...
    environment: "macOS 15.4, Node 24, local Nova AI gateway with a redacted Discord token.",
    steps: "pnpm nova-ai gateway restart, then pnpm nova-ai gateway status",
    evidence,
...
describe("real-behavior-proof-policy", () => {
  it.each([
    ...
    "Linked artifact: https://github.com/nova-ai/nova-ai/actions/runs/123456789/artifacts/987654321",
    "Redacted runtime log: gateway connected Discord channel and delivered the reply.",
    ["Terminal transcript:", "```text", "$ nova-ai gateway status", "discord ready", "```"].join(
      "\n",
    ),
...
describe("isMaintainerTeamMember", () => {
  ...
  it("returns true for active members", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.github.com/orgs/nova-ai/teams/maintainer/memberships/private-maint",
