...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.nova-ai).toBe(
        plan.packageJson.nova-ai.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.nova-ai.optional).toBe(true);
    }
  });
