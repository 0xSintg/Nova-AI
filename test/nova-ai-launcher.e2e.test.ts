...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "nova-ai-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "nova-ai.mjs"),
    path.join(fixtureRoot, "nova-ai.mjs"),
  );
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/nova-ai\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.NOVA_AI_SOURCE_COMPILE_CACHE_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
describe("nova-ai launcher", () => {
  ...
  it("keeps the bootstrap Node floor aligned with package and runtime guards", async () => {
    const [launcher, runtimeGuard, packageJsonRaw] = await Promise.all([
      fs.readFile(path.resolve(process.cwd(), "nova-ai.mjs"), "utf8"),
    ...
    if (!launcherMatch) {
      throw new Error("nova-ai.mjs MIN_NODE_* constants were not found");
    }
    if (!runtimeMatch) {
    ...
    expect(
      ...
      "nova-ai.mjs MIN_NODE_* must match package.json engines.node",
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", mockNodeVersionPath, path.join(fixtureRoot, "nova-ai.mjs"), "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stderr).toContain(
      `nova-ai: Node.js v${launcherMinimumLabel}+ is required (current: v${mockedNodeVersion}).`,
    );
  });
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-nova-ai-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "nova-ai.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-nova-ai-launcher-dep");
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const configPath = path.join(fixtureRoot, "nova-ai.json");
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "nova-ai.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ NOVA_AI_CONFIG_PATH: configPath }),
      encoding: "utf8",
    });
  ...
  it("checks the NOVA_AI_HOME default config path before using precomputed root help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const nova-aiHome = path.join(fixtureRoot, "home");
    const configDir = path.join(nova-aiHome, ".nova-ai");
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "nova-ai.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ NOVA_AI_HOME: nova-aiHome }),
      encoding: "utf8",
    });
  ...
  it("checks legacy config candidates before using precomputed root help", async () => {
    ...
    const legacyConfigDir = path.join(home, ".novabot");
    ...
    await fs.writeFile(
      path.join(legacyConfigDir, "novabot.json"),
      JSON.stringify({ plugins: { slots: { memory: "memory-lancedb" } } }),
      "utf8",
    );
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "nova-ai.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, NOVA_AI_HOME: undefined }),
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:nova-ai/nova-ai#<ref>");
  });
  ...
  it("keeps compile cache off for source-checkout launchers", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "nova-ai.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "nova-ai-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "nova-ai.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          'process.title = "nova-ai-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          "setInterval(() => {}, 1000);",
          "",
        ].join("\n"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "nova-ai-launcher-link-");
      const linkedRoot = path.join(linkParent, "nova-ai-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "nova-ai.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "nova-ai", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "nova-ai-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "nova-ai-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "nova-ai", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "nova-ai"));
  });
