...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.NOVA_AI_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through nova-ai infer", () => {
    const outputBase = path.join(os.tmpdir(), `nova-ai-infer-image-${process.pid}.png`);
