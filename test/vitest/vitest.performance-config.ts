...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.NOVA_AI_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.NOVA_AI_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.NOVA_AI_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.NOVA_AI_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.NOVA_AI_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.NOVA_AI_VITEST_PRINT_IMPORT_BREAKDOWN)) {
