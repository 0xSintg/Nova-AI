...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.NOVA_AI_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "NOVA_AI_TEST_FAST", value: process.env.NOVA_AI_TEST_FAST },
    {
      key: "NOVA_AI_STRICT_FAST_REPLY_CONFIG",
      value: process.env.NOVA_AI_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "NOVA_AI_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.NOVA_AI_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "NOVA_AI_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.NOVA_AI_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "NOVA_AI_STATE_DIR", value: process.env.NOVA_AI_STATE_DIR },
    { key: "NOVA_AI_CONFIG_PATH", value: process.env.NOVA_AI_CONFIG_PATH },
    { key: "NOVA_AI_GATEWAY_PORT", value: process.env.NOVA_AI_GATEWAY_PORT },
    { key: "NOVA_AI_BRIDGE_ENABLED", value: process.env.NOVA_AI_BRIDGE_ENABLED },
    { key: "NOVA_AI_BRIDGE_HOST", value: process.env.NOVA_AI_BRIDGE_HOST },
    { key: "NOVA_AI_BRIDGE_PORT", value: process.env.NOVA_AI_BRIDGE_PORT },
    { key: "NOVA_AI_CANVAS_HOST_PORT", value: process.env.NOVA_AI_CANVAS_HOST_PORT },
    { key: "NOVA_AI_TEST_HOME", value: process.env.NOVA_AI_TEST_HOME },
    { key: "NOVA_AI_AGENT_DIR", value: process.env.NOVA_AI_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NOVA_AI_TEST_HOME = tempHome;
  process.env.NOVA_AI_TEST_FAST = "1";
  process.env.NOVA_AI_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.NOVA_AI_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.NOVA_AI_BRIDGE_ENABLED;
  delete process.env.NOVA_AI_BRIDGE_HOST;
  delete process.env.NOVA_AI_BRIDGE_PORT;
  delete process.env.NOVA_AI_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.NOVA_AI_STATE_DIR = path.join(tempHome, ".nova-ai");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.NOVA_AI_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.NOVA_AI_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".nova-ai");
  const priorIsolatedHome = params.env.NOVA_AI_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".nova-ai")
  ) {
    realStateDir = path.join(params.realHome, ".nova-ai");
  }
  const tempStateDir = path.join(params.tempHome, ".nova-ai");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.NOVA_AI_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.NOVA_AI_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "nova-ai.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "nova-ai.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.NOVA_AI_LIVE_TEST === "1" ||
    process.env.NOVA_AI_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.NOVA_AI_LIVE_USE_REAL_HOME);
