...
import {
  ...
  type Nova AITestStateOptions,
} from "../../src/test-utils/nova-ai-test-state.js";
import { sleep } from "../../src/utils.js";

export type Nova AITestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<Nova AITestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type Nova AITestInstanceCommandResult = {
  code: number | null;
...
export type Nova AITestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  ) => Promise<Nova AITestInstanceCommandResult>;
...
export async function createNova AITestInstance(
  options: Nova AITestInstanceOptions,
): Promise<Nova AITestInstance> {
  ...
  const instance: Nova AITestInstance = {
    name: options.name,
    port,
