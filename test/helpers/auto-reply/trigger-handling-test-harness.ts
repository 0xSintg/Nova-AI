...
const piEmbeddedMocks = getSharedMocks("nova-ai.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("nova-ai.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("nova-ai.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("nova-ai.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    nova-aiHome: process.env.NOVA_AI_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("NOVA_AI_HOME", snapshot.nova-aiHome);
  restoreKey("NOVA_AI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.NOVA_AI_STATE_DIR = join(home, ".nova-ai");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "nova-ai-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".nova-ai", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): Nova AIConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-7" },
        workspace: join(home, "nova-ai"),
