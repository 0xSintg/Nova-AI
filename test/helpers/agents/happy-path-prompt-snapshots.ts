...
import type { Nova AIConfig } from "../../../src/config/types.nova-ai.js";
import type {
  AnyAgentTool,
...
import { createNova AICodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/nova-ai-happy-path/workspace";
const AGENT_DIR = "/tmp/nova-ai-happy-path/agent";
const SESSION_FILE = "/tmp/nova-ai-happy-path/session.jsonl";
...
const baseConfig: Nova AIConfig = {
  messages: {
    groupChat: {
...
const dynamicToolsConfig: Nova AIConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
function createScenarios(): PromptScenario[] {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "Nova AI maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "Nova AI",
    ConversationLabel: "Nova AI/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@Nova AI please verify the Codex happy path too.",
      },
    ],
    Body: "@Nova AI can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to Nova AI/Codex.",
      ],
      trigger: "user",
...
function renderModelBoundPromptLayers(params: {
  ...
  return [
    ...
    "",
    "This is the deterministic model-bound layer stack Nova AI can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, Codex thread config instructions when present, Nova AI developer instructions, turn-scoped collaboration-mode instructions when Nova AI provides them, turn input with Nova AI runtime context, and the Nova AI dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.",
    "",
    "### Layer Metadata",
    "",
    markdownFence(
      ...
      stableJson({
        ...
        openClawRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input Nova AI runtime context",
    ...
    "",
    "### Developer: Nova AI Runtime Instructions",
    "",
...
function buildCodexNova AIRuntimeContext(): string {
  return [
    ...
    "",
    "## Nova AI Workspace Context",
    "",
    CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT,
  ].join("\n");
}

function prependCodexNova AIRuntimeContext(prompt: string): string {
  return [buildCodexNova AIRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(scenario: PromptScenario): string {
  ...
  const codexTurnPromptText = prependCodexNova AIRuntimeContext(scenario.prompt);
  ...
  return [
    ...
    "- This captures the Nova AI-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, Nova AI developer instructions, turn input with simulated Nova AI workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how Nova AI routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path Nova AI tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after Nova AI sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
