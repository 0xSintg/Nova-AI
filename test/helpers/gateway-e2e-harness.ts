...
import { createNova AITestInstance, type Nova AITestInstance } from "./nova-ai-test-instance.js";
...
export type GatewayInstance = Nova AITestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createNova AITestInstance({ name });
  try {
