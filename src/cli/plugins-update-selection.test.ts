      ...
      resolvePluginUpdateSelection({
        installs: {
          "voice-call": createNpmInstall({
            ...
            resolvedName: "@nova-ai/voice-call",
          }),
        },
        rawId: "@nova-ai/voice-call@beta",
      }),
    ).toEqual({
      pluginIds: ["voice-call"],
      specOverrides: {
        "voice-call": "@nova-ai/voice-call@beta",
      },
    });
  });
  ...
  it("maps an explicit npm version update to the tracked plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "nova-ai-codex-app-server@0.2.0-beta.4",
      }),
    ).toEqual({
      pluginIds: ["nova-ai-codex-app-server"],
      specOverrides: {
  ...
  it("keeps recorded npm tags when update is invoked by plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "nova-ai-codex-app-server",
      }),
    ).toEqual({
      pluginIds: ["nova-ai-codex-app-server"],
    });
