...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "nova-ai", "terminal"] },
    { argv: ["node", "nova-ai", "chat"] },
    { argv: ["node", "nova-ai", "gateway"] },
    { argv: ["node", "nova-ai", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "nova-ai", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "nova-ai", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "nova-ai", "gateway", "status"] },
    { argv: ["node", "nova-ai", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "nova-ai", "--help"] },
    { argv: ["node", "nova-ai", "gateway"] },
    { argv: ["node", "nova-ai", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "nova-ai.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("nova-ai.internal");
    } finally {
      log.mockRestore();
    }
  });
