...
const serveNova AIChannelMcp = mocks.serveNova AIChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveNova AIChannelMcp: mocks.serveNova AIChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("nova-ai-cli-mcp-home-", async (home) => {
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("nova-ai-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `No MCP server named "missing" in ${configPath}. Run nova-ai mcp list to see configured servers.`,
      );
    });
  });

  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("nova-ai-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      ...
      expect(serveNova AIChannelMcp).toHaveBeenCalledWith({
