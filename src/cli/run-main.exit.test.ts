...
vi.mock("../infra/path-env.js", () => ({
  ensureNova AICliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.NOVA_AI_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.NOVA_AI_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "nova-ai", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "nova-ai", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "nova-ai", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "nova-ai", "config"]);

    expect(createCliProgressMock).toHaveBeenCalledWith({
      label: "Loading Nova AI CLI…",
      indeterminate: true,
      delayMs: 0,
    });
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "nova-ai", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "nova-ai", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "nova-ai", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "nova-ai", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.NOVA_AI_HIDE_BANNER = "1";

    await runCli(["node", "nova-ai", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "nova-ai", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "nova-ai", "browser", "--help"]);
  ...
  it("keeps root help on the precomputed path without proxy bootstrap", async () => {
    ...
    await runCli(["node", "nova-ai", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "nova-ai", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "nova-ai", "gateway", "run"]],
    ["bare gateway runtime", ["node", "nova-ai", "gateway"]],
    ["node runtime", ["node", "nova-ai", "node", "run"]],
    ["local agent runtime", ["node", "nova-ai", "agent", "--local"]],
    ["provider inference", ["node", "nova-ai", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "nova-ai", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "nova-ai", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "nova-ai", "skills", "search", "browser"]],
    ["update command", ["node", "nova-ai", "update", "check"]],
    ["channel probe", ["node", "nova-ai", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "nova-ai", "channels", "capabilities"]],
    ["directory plugin command", ["node", "nova-ai", "directory", "peers", "list"]],
    ["message plugin command", ["node", "nova-ai", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "nova-ai", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "nova-ai", "--help"]],
    ["root version", ["node", "nova-ai", "--version"]],
    ["gateway help", ["node", "nova-ai", "gateway", "--help"]],
    ["gateway run help", ["node", "nova-ai", "gateway", "run", "--help"]],
    ["status", ["node", "nova-ai", "status"]],
    ["health", ["node", "nova-ai", "health"]],
    ["gateway status", ["node", "nova-ai", "gateway", "status"]],
    ["gateway health", ["node", "nova-ai", "gateway", "health"]],
    ["remote agent control-plane", ["node", "nova-ai", "agent", "run"]],
    ["chat control-plane", ["node", "nova-ai", "chat"]],
    ["terminal control-plane", ["node", "nova-ai", "terminal"]],
    ["config", ["node", "nova-ai", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "nova-ai", "channels"]],
    ["completion", ["node", "nova-ai", "completion", "zsh"]],
    ["debug proxy cli", ["node", "nova-ai", "proxy", "start"]],
    ["agents list", ["node", "nova-ai", "agents", "list"]],
    ["models list", ["node", "nova-ai", "models", "list"]],
    ["models status without live probe", ["node", "nova-ai", "models", "status"]],
    ["skills check", ["node", "nova-ai", "skills", "check"]],
    ["skills info", ["node", "nova-ai", "skills", "info", "weather"]],
    ["skills list", ["node", "nova-ai", "skills", "list"]],
    ["tasks list", ["node", "nova-ai", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "nova-ai", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "nova-ai", "tools", "effective"]],
    ["migrate", ["node", "nova-ai", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "nova-ai", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "nova-ai", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "nova-ai", "foo"])).rejects.toThrow(
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "nova-ai", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "nova-ai", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "nova-ai", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "nova-ai", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "nova-ai", "auth", "--help"]],
    ["tool", ["node", "nova-ai", "tool", "image_generate"]],
    ["tools", ["node", "nova-ai", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "nova-ai", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "nova-ai", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "nova-ai", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "nova-ai", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "nova-ai", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "nova-ai", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "nova-ai", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "nova-ai", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "nova-ai", "status"]],
    ]);
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    await runCli(["node", "nova-ai", "doctor", "--help"]);

    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "nova-ai", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "nova-ai", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[nova-ai] Nova AI hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[nova-ai] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[nova-ai] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
