...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration Nova AI backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai migrate list", "Show available migration providers."],
          ["nova-ai migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["nova-ai migrate hermes --dry-run", "Preview Hermes migration only."],
          [
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing Nova AI state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
