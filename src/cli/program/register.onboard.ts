...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.nova-ai.com/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.nova-ai/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. NOVA_AI_GATEWAY_TOKEN)",
    )
