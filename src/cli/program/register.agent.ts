...
${formatHelpExamples([
  ['nova-ai agent --to +15555550123 --message "status update"', "Start a new session."],
  ['nova-ai agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'nova-ai agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'nova-ai agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['nova-ai agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'nova-ai agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.nova-ai.com/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.nova-ai.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['nova-ai agents set-identity --agent main --name "Nova AI" --emoji "🦞"', "Set name + emoji."],
  ["nova-ai agents set-identity --agent main --avatar avatars/nova-ai.png", "Set avatar path."],
  [
    "nova-ai agents set-identity --workspace ~/.nova-ai/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "nova-ai agents set-identity --identity-file ~/.nova-ai/workspace/IDENTITY.md --agent main",
