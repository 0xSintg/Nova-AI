...
export async function ensureConfigReady(params: {
  ...
  if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
    ...
    if (!params.suppressDoctorStdout) {
      ...
      const originalSuppressNotes = process.env.NOVA_AI_SUPPRESS_NOTES;
      process.stdout.write = (() => true) as unknown as typeof process.stdout.write;
      process.env.NOVA_AI_SUPPRESS_NOTES = "1";
      try {
        preflightSnapshot = (await runDoctorConfigPreflight()).snapshot;
      } finally {
        process.stdout.write = originalStdoutWrite;
        if (originalSuppressNotes === undefined) {
          delete process.env.NOVA_AI_SUPPRESS_NOTES;
        } else {
          process.env.NOVA_AI_SUPPRESS_NOTES = originalSuppressNotes;
        }
      }
    }
  }
  ...
  params.runtime.error(heading("Nova AI config is invalid"));
  ...
  params.runtime.error(
    `${muted("Fix:")} ${commandText(formatCliCommand("nova-ai doctor --fix"))}`,
  );
  params.runtime.error(
    `${muted("Inspect:")} ${commandText(formatCliCommand("nova-ai config validate"))}`,
  );
  params.runtime.error(
