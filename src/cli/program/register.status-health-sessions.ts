...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai status", "Show channel health + session summary."],
          ["nova-ai status --all", "Full diagnosis (read-only)."],
          ["nova-ai status --json", "Machine-readable output."],
          ["nova-ai status --usage", "Show model provider usage/quota snapshots."],
          [
            "nova-ai status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["nova-ai status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.nova-ai.com/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.nova-ai.com/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai sessions", "List all sessions."],
          ["nova-ai sessions --agent work", "List sessions for one agent."],
          ["nova-ai sessions --all-agents", "Aggregate sessions across agents."],
          ["nova-ai sessions --active 120", "Only last 2 hours."],
          ["nova-ai sessions --limit 25", "Show the newest 25 sessions."],
          ["nova-ai sessions --json", "Machine-readable output."],
          ["nova-ai sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.nova-ai.com/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "nova-ai sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          [
            "nova-ai sessions cleanup --dry-run --fix-dm-scope",
            "Preview stale direct-DM rows after returning dmScope to main.",
          ],
          ["nova-ai sessions cleanup --enforce", "Apply maintenance now."],
          ["nova-ai sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["nova-ai sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "nova-ai sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .nova-ai/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai commitments", "List pending inferred follow-ups."],
          ["nova-ai commitments --all", "List all inferred follow-ups."],
          ["nova-ai commitments --agent work", "List one agent's inferred follow-ups."],
          ["nova-ai commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
