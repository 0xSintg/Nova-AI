  ...
  program
    ...
    .addHelpText(
      ...
      () =>
        ...
        `  ${theme.command("nova-ai setup")}\n` +
        `    ${theme.muted("Create config, workspace, and session folders.")}\n` +
        `  ${theme.command("nova-ai setup --wizard")}\n` +
        `    ${theme.muted("Run full onboarding for auth, models, Gateway, and channels.")}\n\n` +
        `${theme.muted("Docs:")} ${formatDocsLink("/cli/setup", "docs.nova-ai.com/cli/setup")}\n`,
    )
    .option(
      "--workspace <dir>",
      "Agent workspace directory (default: ~/.nova-ai/workspace; stored as agents.defaults.workspace)",
    )
