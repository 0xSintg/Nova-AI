...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.NOVA_AI_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.NOVA_AI_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.NOVA_AI_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "nova-ai", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "nova-ai"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "nova-ai", "nodes", "list"], "nova-ai");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "nova-ai", "infer"], "nova-ai");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "nova-ai", "acp", "--help"], "nova-ai");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "nova-ai", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "nova-ai", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("nova-ai");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "nova-ai", "completion", "--write-state"];
    const program = new Command().name("nova-ai");
  ...
  it.each([
    ...
    process.argv = ["node", "nova-ai", ...args];
    const program = new Command().name("nova-ai");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "nova-ai", "plugins"];
    const program = new Command().name("nova-ai");
