...
describe("proxy cli runtime", () => {
  const envKeys = [
    "NOVA_AI_DEBUG_PROXY_DB_PATH",
    "NOVA_AI_DEBUG_PROXY_BLOB_DIR",
    "NOVA_AI_DEBUG_PROXY_CERT_DIR",
    "NOVA_AI_DEBUG_PROXY_SESSION_ID",
    "NOVA_AI_DEBUG_PROXY_ENABLED",
    "FORCE_COLOR",
    "NO_COLOR",
  ] as const;
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "nova-ai-proxy-cli-runtime-"));
    process.env.NOVA_AI_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.NOVA_AI_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.NOVA_AI_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.NOVA_AI_DEBUG_PROXY_ENABLED;
    delete process.env.NOVA_AI_DEBUG_PROXY_SESSION_ID;
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout.write).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or NOVA_AI_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or NOVA_AI_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout.write).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, NOVA_AI_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or NOVA_AI_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or NOVA_AI_PROXY_URL",
            ],
          },
          checks: [],
        },
  ...
  it("stops the proxy server and ends the session when child spawn fails", async () => {
    ...
    const store = getDebugProxyCaptureStore(
      process.env.NOVA_AI_DEBUG_PROXY_DB_PATH!,
      process.env.NOVA_AI_DEBUG_PROXY_BLOB_DIR!,
    );
