...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/nova-ai-root")).toEqual([
      path.join("/tmp/nova-ai-root", "dist", "index.js"),
      path.join("/tmp/nova-ai-root", "dist", "index.mjs"),
      path.join("/tmp/nova-ai-root", "dist", "entry.js"),
      path.join("/tmp/nova-ai-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { NOVA_AI_GATEWAY_PORT: "19001" },
        serviceEnv: { NOVA_AI_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/nova-ai",
      baseEnv: {
        PATH: "/bin",
        NOVA_AI_STATE_DIR: "/wrong/state",
        NOVA_AI_CONFIG_PATH: "/wrong/nova-ai.json",
        NOVA_AI_PROFILE: "wrong",
      },
      serviceEnv: {
        NOVA_AI_STATE_DIR: "daemon-state",
        NOVA_AI_CONFIG_PATH: "daemon-state/nova-ai.json",
    ...
    expect(env.NOVA_AI_STATE_DIR).toBe(path.join("/srv/nova-ai", "daemon-state"));
    expect(env.NOVA_AI_CONFIG_PATH).toBe(
      path.join("/srv/nova-ai", "daemon-state", "nova-ai.json"),
    );
    expect(env.NOVA_AI_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        NOVA_AI_STATE_DIR: "/caller/state",
        NOVA_AI_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.NOVA_AI_STATE_DIR).toBe("/caller/state");
    expect(env.NOVA_AI_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@nova-ai", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@nova-ai", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@nova-ai/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@nova-ai/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@nova-ai/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@nova-ai/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@nova-ai/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@nova-ai", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("nova-ai gateway restart");
    expect(line).toContain("nova-ai gateway install --force");
    expect(line).toContain("nova-ai gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { NOVA_AI_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, NOVA_AI_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { NOVA_AI_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { NOVA_AI_PROFILE: "stomme", NOVA_AI_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `nova-ai update` completes (#78445).
