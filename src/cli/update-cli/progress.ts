...
const STEP_LABELS: Record<string, string> = {
  ...
  "nova-ai doctor entry": "Checking doctor entrypoint",
  "nova-ai doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g nova-ai@latest");
    hints.push(
      "System install outline: nova-ai gateway stop -> sudo <system-npm> i -g nova-ai@latest -> nova-ai gateway install --force -> nova-ai gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g nova-ai@latest --omit=optional");
  }
