...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-Nova AIScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-Nova AISchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "nova-ai restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-Nova AIListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-Nova AISchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        NOVA_AI_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'nova-ai-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `nova-ai-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("nova-ai-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^nova-ai-restart-/);
    ...
    it("uses NOVA_AI_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        NOVA_AI_PROFILE: "default",
        NOVA_AI_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("nova-ai-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        NOVA_AI_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        NOVA_AI_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped nova-ai gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart nova-ai-gateway.service");
      expect(calls).toContain("--user is-active --quiet nova-ai-gateway.service");
      expect(calls).toContain("is-active --quiet nova-ai-gateway.service");
      expect(calls).not.toContain("--user restart nova-ai-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.nova-ai.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.nova-ai.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.nova-ai/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.nova-ai/logs/gateway-restart.log' 2>&1");
    ...
    it("uses NOVA_AI_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/nova-ai-state/logs' 2>/dev/null && : >>'/tmp/nova-ai-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/nova-ai-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("nova-ai restart attempt source=update target=ai.nova-ai.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.nova-ai.gateway");
      expect(log).toContain("nova-ai restart failed source=update status=42");
      expect(log).not.toContain("nova-ai restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        NOVA_AI_LAUNCHD_LABEL: "ai.nova-ai.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        NOVA_AI_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.nova-ai.$(echo injected)");
      expect(log).not.toContain("target=ai.nova-ai.injected");
    });

    it("uses NOVA_AI_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        NOVA_AI_PROFILE: "default",
        NOVA_AI_LAUNCHD_LABEL: "com.custom.nova-ai",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.nova-ai'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Nova AI Gateway'");
      expect(content).toContain("function Invoke-Nova AISchtasksWithTimeout");
      expect(content).toContain("function Get-Nova AIScheduledTaskState");
      expect(content).toContain("function Invoke-Nova AIStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("nova-ai restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".nova-ai\\gateway.cmd"',
      );
      expect(content).toContain("nova-ai restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%NOVA_AI_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses NOVA_AI_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        NOVA_AI_PROFILE: "default",
        NOVA_AI_WINDOWS_TASK_NAME: "Nova AI Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'Nova AI Gateway (custom)'");
      expect(content).toContain("Get-Nova AIScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-Nova AISchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-Nova AIStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        NOVA_AI_PROFILE: "production",
      });
      expect(content).toContain("nova-ai-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        NOVA_AI_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.nova-ai.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'Nova AI Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        NOVA_AI_LAUNCHD_LABEL: "ai.nova-ai.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.nova-ai.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
