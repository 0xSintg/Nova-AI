...
import { disableCurrentNova AIUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const POST_CORE_UPDATE_ENV = "NOVA_AI_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "NOVA_AI_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "NOVA_AI_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "NOVA_AI_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "NOVA_AI_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_SOURCE_CONFIG_PATH_ENV = "NOVA_AI_UPDATE_POST_CORE_SOURCE_CONFIG_PATH";
const POST_CORE_UPDATE_STARTED_AT_ENV = "NOVA_AI_UPDATE_POST_CORE_STARTED_AT_MS";
...
const POST_UPDATE_PLUGIN_REPAIR_GUIDANCE = "Run nova-ai doctor --fix to attempt automatic repair.";
...
function restoreDroppedPreUpdateChannels(
  ...
  const authoredChannels = resolveRestoredAuthoredChannels({
    ...
    currentAuthoredChannels: isRecord(snapshot.parsed)
      ? (snapshot.parsed as Nova AIConfig).channels
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run nova-ai plugins inspect ${pluginId} --runtime --json for details.`;
}
...
export function buildInvalidConfigPostCoreUpdateResult(): {
  ...
  const guidance = [
    "Run `nova-ai doctor` to inspect the config validation errors.",
    "Once the config parses, rerun `nova-ai update`.",
  ];
  const message =
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("nova-ai gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("nova-ai gateway install --force"),
    CLI_NAME,
  );
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall Nova AI ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("nova-ai gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `nova-ai update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("nova-ai update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
function isRunningInsideGatewayService(
  ...
  if (env.NOVA_AI_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.NOVA_AI_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `${runtimeLabel} is too old for nova-ai@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    runtime.nodeRunner
      ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `nova-ai update`."
      : "Upgrade Node to 22.19+ or Node 24, then rerun `nova-ai update`.",
    "Bare `npm i -g nova-ai` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g nova-ai@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.NOVA_AI_SERVICE_MARKER;
  delete resolvedEnv.NOVA_AI_SERVICE_KIND;
  return resolvedEnv;
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("nova-ai completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
        ...
        return await runUpdateStep({
          ...
          env: {
            ...
            ...(candidateHostVersion === null
              ? {}
              : { NOVA_AI_COMPATIBILITY_HOST_VERSION: candidateHostVersion }),
          },
          timeoutMs: params.timeoutMs,
          progress: params.progress,
        });
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as Nova AIConfig["channels"],
      };
    }
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (expectedGatewayVersion: string | undefined) => {
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("nova-ai gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        await createUpdateConfigSnapshot();
        process.env.NOVA_AI_UPDATE_IN_PROGRESS = "1";
        ...
        try {
          ...
          delete process.env.NOVA_AI_UPDATE_IN_PROGRESS;
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("nova-ai gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (isPackageManagerUpdateMode(params.result.mode)) {
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("nova-ai doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("nova-ai gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("nova-ai gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
function withUpdateFinalizationEnv<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.NOVA_AI_UPDATE_IN_PROGRESS;
  ...
  return run().finally(() => {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.NOVA_AI_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
  ...
  const preFinalizeConfig = configSnapshot.valid
    ? {
        ...
        authoredConfig: isRecord(configSnapshot.parsed)
          ? (configSnapshot.parsed as Nova AIConfig)
...
function normalizePreUpdateConfigRestoreInput(
  ...
  if (isRecord(sourceConfig) && isRecord(authoredConfig)) {
    return {
      sourceConfig: sourceConfig as Nova AIConfig,
      authoredConfig: authoredConfig as Nova AIConfig,
    };
  }
  const authored = parsed as Nova AIConfig;
  return {
...
function resolvePreUpdateSourceConfigFromAuthored(
  ...
  try {
    ...
    return isRecord(resolved) ? (resolved as Nova AIConfig) : authoredConfig;
  } catch {
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-update-post-core-"));
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun !== true) {
    await disableCurrentNova AIUpdateLaunchdJob().catch(() => undefined);
    assertConfigWriteAllowedInCurrentMode();
  }
  ...
  const devTargetRef =
    channel === "dev" ? process.env.NOVA_AI_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (updateInstallKind === "package") {
    ...
    if (managedServiceRootRedirect) {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.warn(
            `Shell Nova AI root differs from the managed gateway service root: ${managedServiceRootRedirect.previousRoot}`,
          ),
        );
        defaultRuntime.log(
          theme.muted(
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Nova AI..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `nova-ai update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Nova AI install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("nova-ai doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("nova-ai gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g nova-ai@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g nova-ai@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
