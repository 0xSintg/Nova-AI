...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "nova-ai", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "nova-ai", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "nova-ai", "--profile", "work", "gateway", "status"],
