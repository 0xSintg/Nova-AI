...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai security audit", "Run a local security audit."],
          [
            "nova-ai security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["nova-ai security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "nova-ai security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["nova-ai security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["nova-ai security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.nova-ai.com/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Nova AI security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("nova-ai security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("nova-ai security audit --fix")}`));
        if (!fixResult) {
