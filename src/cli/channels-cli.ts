...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai channels list", "List configured channels."],
          ["nova-ai channels list --all", "Show configured, bundled, and installable channels."],
          ["nova-ai channels add", "Open guided channel setup."],
          ["nova-ai channels status --probe", "Run channel status checks and probes."],
          [
            "nova-ai channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["nova-ai channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.nova-ai.com/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai channels add", "Open guided setup for available chat channels."],
          [
            "nova-ai channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["nova-ai channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
