...
function normalizeModelProviderRefsForConfigMutation(
  providers: NonNullable<Nova AIConfig["models"]>["providers"] | undefined,
): unknown {
...
function normalizeConfigMutationModelRefs(cfg: Nova AIConfig): Nova AIConfig {
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "nova-ai config set secrets.providers.vault --provider-source file --provider-path /etc/nova-ai/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("nova-ai config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("nova-ai doctor --fix")}\` ${message}`;
}
...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`Nova AI config is invalid: ${shortenHomePath(snapshot.path)}`);
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("nova-ai plugins install <spec>")}`,
    `  ${formatCliCommand("nova-ai plugins update <plugin-id>")}`,
    `  ${formatCliCommand("nova-ai plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
function formatAutoManagedMetaError(paths: readonly PathSegment[][]): string {
  ...
  return [
    `${subject} is auto-managed by Nova AI and cannot be edited; the value would be overwritten on the next config write.`,
    "",
    "These fields are stamped on every config write to record the Nova AI version and timestamp that produced the file.",
  ].join("\n");
}

function collectDryRunSchemaErrors(params: { config: Nova AIConfig }): ConfigSetDryRunError[] {
...
async function runConfigOperations(params: {
  ...
  const nextConfig = normalizeConfigMutationModelRefs(next as Nova AIConfig);
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("nova-ai config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("nova-ai config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "nova-ai.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("nova-ai onboard")} or run ${formatCliCommand("nova-ai doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`Nova AI config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("nova-ai config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.nova-ai.com/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing nova-ai.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing nova-ai.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for nova-ai.json")
    .action(async () => {
