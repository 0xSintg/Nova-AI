...
export function hasPluginHelpAffectingConfig(config: Nova AIConfig | null | undefined): boolean {
...
export function hasPluginHelpAffectingEnv(env: NodeJS.ProcessEnv): boolean {
  return Boolean(
    env.NOVA_AI_BUNDLED_PLUGINS_DIR?.trim() || env.NOVA_AI_DISABLE_BUNDLED_PLUGINS?.trim(),
  );
}
