...
export function buildToolPluginPackageManifest(params: {
  ...
  const nova-ai =
    params.packageManifest.nova-ai &&
    typeof params.packageManifest.nova-ai === "object" &&
    !Array.isArray(params.packageManifest.nova-ai)
      ? { ...(params.packageManifest.nova-ai as JsonObject) }
      : {};
  return {
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("nova-ai.plugin.json generated metadata is stale. Run nova-ai plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `nova-ai.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("nova-ai.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`nova-ai.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `nova-ai.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include nova-ai.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json nova-ai.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run nova-ai plugins build.");
...
export async function runPluginsInitCommand(id: string, opts: PluginsInitOptions): Promise<void> {
  ...
  const packageManifest = {
    name: `nova-ai-plugin-${id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && nova-ai plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && nova-ai plugins validate --entry ./dist/index.js",
      test: "vitest run",
    },
  ...
  const descriptionLiteral = jsStringLiteral(`Add ${name} tools to Nova AI.`);
...
import { getToolPluginMetadata } from "nova-ai/plugin-sdk/tool-plugin";
...
`;
  ...
  writeJsonFile(path.join(rootDir, PLUGIN_MANIFEST_FILENAME), {
    id,
    name,
    description: `Add ${name} tools to Nova AI.`,
