    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai nodes status", "List known nodes with live status."],
          ["nova-ai nodes pairing pending", "Show pending node pairing requests."],
          ["nova-ai nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'nova-ai nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["nova-ai nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.nova-ai.com/cli/nodes")}\n`,
    );
