...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      NOVA_AI_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { NOVA_AI_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { NOVA_AI_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-daemon-cli-"));
    ...
    process.env.NOVA_AI_STATE_DIR = tmpDir;
    process.env.NOVA_AI_CONFIG_PATH = path.join(tmpDir, "nova-ai.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        NOVA_AI_PROFILE: "dev",
        NOVA_AI_STATE_DIR: "/tmp/nova-ai-daemon-state",
        NOVA_AI_CONFIG_PATH: "/tmp/nova-ai-daemon-state/nova-ai.json",
        NOVA_AI_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.nova-ai.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).NOVA_AI_WRAPPER).toBe(
      "/usr/local/bin/nova-ai-doppler",
    );
  });
