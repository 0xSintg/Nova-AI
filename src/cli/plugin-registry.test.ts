...
function expectLoadNova AIPluginsCall(
  callIndex: number,
  expected: {
    config?: unknown;
  ...
  const params = mocks.loadNova AIPlugins.mock.calls[callIndex]?.[0] as
    | {
        config?: unknown;
        activationSourceConfig?: unknown;
        autoEnabledReasons?: unknown;
...
const mocks = vi.hoisted(() => ({
  loadNova AIPlugins: vi.fn<typeof import("../plugins/loader.js").loadNova AIPlugins>(),
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("uses the resolved runtime load context for configured channel scope", () => {
    ...
    expectLoadNova AIPluginsCall(0, {
      config: autoEnabledConfig,
  ...
  it("reloads when escalating from configured-channels to channels", () => {
    ...
    expectLoadNova AIPluginsCall(1, {
