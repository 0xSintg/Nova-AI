...
import { installedPluginRoot } from "nova-ai/plugin-sdk/test-fixtures";
...
const CLI_STATE_ROOT = "/tmp/nova-ai-state";
const ORIGINAL_NOVA_AI_STATE_DIR = process.env.NOVA_AI_STATE_DIR;
const ORIGINAL_NOVA_AI_NIX_MODE = process.env.NOVA_AI_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/nova-ai-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.NOVA_AI_STATE_DIR = PROFILE_STATE_ROOT;
  return path.resolve(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): Nova AIConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): Nova AIConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@nova-ai/${pluginId}`,
    npmTarballName: `nova-ai-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@nova-ai/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@nova-ai/${pluginId}@1.2.3`,
...
function createPathHookPackInstalledConfig(tmpRoot: string): Nova AIConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): Nova AIConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing nova-ai.plugin.json",
  });
...
function primeBlockedNpmPluginInstall(params: {
  ...
  loadConfig.mockReturnValue({} as Nova AIConfig);
...
function replaceConfigCall(callIndex = 0): { baseHash?: string; nextConfig?: Nova AIConfig } {
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_NOVA_AI_STATE_DIR === undefined) {
      delete process.env.NOVA_AI_STATE_DIR;
    } else {
      process.env.NOVA_AI_STATE_DIR = ORIGINAL_NOVA_AI_STATE_DIR;
    }
    if (ORIGINAL_NOVA_AI_NIX_MODE === undefined) {
      delete process.env.NOVA_AI_NIX_MODE;
    } else {
      process.env.NOVA_AI_NIX_MODE = ORIGINAL_NOVA_AI_NIX_MODE;
    }
  });
  ...
  it("refuses plugin installs in Nix mode before installer side effects", async () => {
    process.env.NOVA_AI_NIX_MODE = "1";
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `nova-ai doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(npmInstallCall().spec).toBe("@nova-ai/brave-plugin");
    ...
    expect(record.spec).toBe("@nova-ai/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-nova-ai-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-nova-ai-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-nova-ai-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-nova-ai-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing nova-ai.extensions",
      code: "missing_nova-ai_extensions",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-nova-ai-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-nova-ai-plugin@2026.5.7");
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/nova-ai-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@nova-ai/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("nova-ai-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@nova-ai/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@nova-ai/discord"]);
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-nova-ai-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-nova-ai-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-nova-ai-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("reports npm install failures without trying ClawHub when npm: prefix is used", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing nova-ai.hooks",
    });
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@nova-ai/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as Nova AIConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("nova-ai plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the active profile extensions dir to local path installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-local-plugin-"));
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/nova-ai/.nova-ai/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing nova-ai.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `nova-ai plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-local-hook-pack-"));
    const cfg = {} as Nova AIConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("does not fall back to npm when explicit ClawHub rejects a real package", async () => {
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "nova-ai skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "nova-ai skills install demo" instead.');
  });
