...
export function shouldUseRootHelpFastPath(
  ...
  const invocation = resolveCliArgvInvocation(argv);
  return (
    env.NOVA_AI_DISABLE_CLI_STARTUP_HELP_FAST_PATH !== "1" &&
...
export function shouldUseBrowserHelpFastPath(
  ...
  if (env.NOVA_AI_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
  const invocation = resolveCliArgvInvocation(argv);
  return (
  ...
  if (parentPluginId) {
    if (allow.length > 0 && !allow.includes(parentPluginId)) {
      if (parentPluginId === normalizedPluginId) {
        return (
          `The \`nova-ai ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (
      ...
      return (
        `The \`nova-ai ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`nova-ai plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`nova-ai ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
    ...
    if (ownerEnabled) {
      ...
      if (toolOwner.availability === "manifest-only") {
        return (
          ...
          "Run `nova-ai --help` to see available CLI subcommands."
        );
      }
      return (
