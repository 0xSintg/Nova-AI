...
function createGatewayCliMainStartupTrace(argv: string[]) {
  const enabled =
    isTruthyEnvValue(process.env.NOVA_AI_GATEWAY_STARTUP_TRACE) &&
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.NOVA_AI_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.NOVA_AI_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: Nova AIConfig): Nova AIConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return (
    ...
    `Unknown command: nova-ai ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`
  );
}
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.NOVA_AI_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<Nova AIConfig> | null = null;
  const readBestEffortCliConfig = async (): Promise<Nova AIConfig> => {
  ...
  try {
    ...
    if (shouldRunBareRootCrestodian) {
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `nova-ai crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[nova-ai] Non-fatal uncaught exception (continuing):",
            formatUncaughtError(error),
          );
          return;
        }
        for (const line of formatCliFailureLines({
          title: "Nova AI hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
