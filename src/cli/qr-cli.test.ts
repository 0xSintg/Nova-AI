...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("nova-ai devices approve <requestId>");
  });
  ...
  it("uses NOVA_AI_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("NOVA_AI_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
