...
describe("plugin authoring commands", () => {
  ...
  it("preserves manifest-owned metadata while updating generated fields", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        entry: "./src/index.ts",
        manifest,
        packageManifest: { version: "1.2.3", nova-ai: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([]);
  });
  ...
  it("drops stale manifest-owned tool metadata when no generated metadata remains", () => {
    const metadata = createDemoMetadata();
    const packageManifest = { version: "1.2.3", nova-ai: { extensions: ["./src/index.ts"] } };
    const manifest = buildToolPluginManifest({
      metadata,
      packageManifest,
      existingManifest: {
  ...
  it("aligns package metadata with the selected runtime extension entry", () => {
    expect(
      buildToolPluginPackageManifest({
        packageManifest: {
          name: "demo",
          nova-ai: { setupEntry: "./setup.ts", extensions: ["./src/other.ts"] },
        },
        entry: "./src/index.ts",
      }),
    ).toEqual({
  ...
  it("reports stale manifest contracts", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        packageManifest: { nova-ai: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([
      "nova-ai.plugin.json generated metadata is stale. Run nova-ai plugins build.",
      "nova-ai.plugin.json contracts.tools is missing: demo_echo",
      "nova-ai.plugin.json contracts.tools has no matching defineToolPlugin tool: other_tool",
    ]);
  });

  it("reports missing entries with an author-facing path", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-missing-"));
  ...
  it("loads source entries that import the Nova AI plugin SDK package subpath", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-source-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "nova-ai-plugin-source-demo",
          type: "module",
...
`,
  ...
  it("scaffolds a dist-entry tool plugin project", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-init-"));
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(projectDir, "nova-ai.plugin.json"), "utf8")),
    ).toMatchObject({
      id: "stock-quotes",
      name: 'Stock "Quotes"',
      configSchema: {
