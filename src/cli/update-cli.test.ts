...
import type { Nova AIConfig, ConfigFileSnapshot } from "../config/types.nova-ai.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: Nova AIConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentNova AIUpdateLaunchdJob: vi.fn(async () => false),
}));
...
vi.mock("../infra/nova-ai-root.js", () => ({
  resolveNova AIPackageRoot: vi.fn(),
  resolveNova AIPackageRootSync: vi.fn(() => process.cwd()),
}));

vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.NOVA_AI_NIX_MODE === "1") {
      throw new Error(
        [
          "Config is managed by Nix (`NOVA_AI_NIX_MODE=1`), so Nova AI treats nova-ai.json as immutable.",
          "Do not run setup, onboarding, nova-ai update, plugin install/update/uninstall/enable, doctor repair/token-generation, or config set against this file.",
          "Agent-first Nix setup: https://github.com/nova-ai/nix-nova-ai#quick-start",
          "Nova AI Nix overview: https://docs.nova-ai.com/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/nova-ai-config",
  };
});
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentNova AIUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob,
}));
...
const { resolveNova AIPackageRoot } = await import("../infra/nova-ai-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/nova-ai-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as Nova AIConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/nova-ai-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveNova AIPackageRoot).mockResolvedValue(root);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [{ channel?: string; config?: Nova AIConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [{ config?: Nova AIConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as Nova AIConfig;
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("nova-ai-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("nova-ai-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveNova AIPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("nova-ai");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["nova-ai", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/nova-ai-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "nova-ai-gateway" }],
      hints: [],
    });
    ...
    launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("nova-ai-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "nova-ai.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.NOVA_AI_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ NOVA_AI_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("NOVA_AI_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob).toHaveBeenCalledOnce();
    expect(runGatewayUpdate).not.toHaveBeenCalled();
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("nova-ai-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("nova-ai completion --write-state");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.NOVA_AI_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.NOVA_AI_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.NOVA_AI_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("nova-ai-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run nova-ai doctor --fix to attempt automatic repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    await withEnvAsync(
      {
        NOVA_AI_SERVICE_MARKER: "nova-ai",
        NOVA_AI_SERVICE_KIND: "gateway",
      },
      async () => {
        await updateCommand({ yes: true });
      },
    ...
    expect(spawnEnv?.NOVA_AI_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.NOVA_AI_SERVICE_KIND).toBeUndefined();
  });
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@nova-ai/demo@1.0.0",
        installPath: "/tmp/nova-ai-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.NOVA_AI_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.NOVA_AI_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.NOVA_AI_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.NOVA_AI_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.NOVA_AI_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("nova-ai-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_POST_CORE: "1",
        NOVA_AI_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("nova-ai-post-core-result");
    ...
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_POST_CORE: "1",
        NOVA_AI_UPDATE_POST_CORE_CHANNEL: "stable",
        NOVA_AI_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("nova-ai-post-core-records");
    ...
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_POST_CORE: "1",
        NOVA_AI_UPDATE_POST_CORE_CHANNEL: "stable",
        NOVA_AI_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("nova-ai-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as Nova AIConfig,
      sourceConfig: { update: { channel: "stable" } } as Nova AIConfig,
      runtimeConfig: { update: { channel: "stable" } } as Nova AIConfig,
      config: { update: { channel: "stable" } } as Nova AIConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_POST_CORE: "1",
        NOVA_AI_UPDATE_POST_CORE_CHANNEL: "dev",
        NOVA_AI_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@nova-ai/demo@1.0.0",
        resolvedSpec: "@nova-ai/demo@1.0.0",
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @nova-ai/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      "Run nova-ai doctor --fix to attempt automatic repair.",
      "Run nova-ai plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("nova-ai-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run nova-ai doctor --fix to attempt automatic repair.");
    expect(logs).toContain("Run nova-ai plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; Nova AI will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.NOVA_AI_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
              ...
              warnings: [
                {
                  ...
                  message:
                    'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run nova-ai doctor --fix to attempt automatic repair. Run nova-ai plugins inspect demo --runtime --json for details.',
                  guidance: [
                    "Run nova-ai doctor --fix to attempt automatic repair.",
                    "Run nova-ai plugins inspect demo --runtime --json for details.",
                  ],
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("Nova AI update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "nova-ai", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as Nova AIConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("nova-ai@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("nova-ai-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "That path replaces the active Nova AI dist tree while the live gateway may still lazy-load old chunks.",
        "Run `nova-ai update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("nova-ai@latest");
  });
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["nova-ai", "gateway", "run"],
      environment: {
        NOVA_AI_SERVICE_MARKER: "nova-ai",
        NOVA_AI_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors.join("\n")).toContain(
      "nova-ai update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g nova-ai` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:nova-ai/nova-ai#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("nova-ai-update"));
        await updateCommand({ yes: true, tag: "github:nova-ai/nova-ai#main" });
      },
      expectedSpec: "github:nova-ai/nova-ai#main",
    },
    {
      name: "NOVA_AI_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("nova-ai-update"));
        await withEnvAsync(
          { NOVA_AI_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/nova-ai-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/nova-ai-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "nova-ai");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "nova-ai", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "nova-ai", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "nova-ai");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "nova-ai", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "nova-ai");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "nova-ai", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.NOVA_AI_UPDATE_IN_PROGRESS,
    ).toBe("1");
  });

  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "nova-ai", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.NOVA_AI_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "nova-ai");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.NOVA_AI_SERVICE_MARKER).toBe("nova-ai");
    expect(serviceStopCall?.env?.NOVA_AI_SERVICE_KIND).toBe("gateway");
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "nova-ai");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob.mockResolvedValue(true);
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
        return true;
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "nova-ai");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "nova-ai", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.NOVA_AI_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.NOVA_AI_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("nova-ai-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "nova-ai");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "nova-ai", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "nova-ai");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("nova-ai-update");
    const localAppData = createCaseDir("nova-ai-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("nova-ai-shell-root");
    const serviceRoot = await createTrackedTempDir("nova-ai-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "nova-ai", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell Nova AI root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `nova-ai` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("nova-ai-shell-root");
    const servicePrefix = await createTrackedTempDir("nova-ai-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "nova-ai");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("nova-ai-same-root");
  ...
  it("uses the managed service Node for follow-up commands when roots match but nodes differ", async () => {
    const servicePrefix = await createTrackedTempDir("nova-ai-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "nova-ai");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("nova-ai-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "nova-ai");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("does not repair legacy config during a dry run", async () => {
    ...
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).not.toHaveBeenCalled();
    expect(launchdUpdateCleanupMocks.disableCurrentNova AIUpdateLaunchdJob).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("nova-ai-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as Nova AIConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/nova-ai-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (Nova AIConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: Nova AIConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
  ...
  it("restores pre-update channels when post-core resume sees post-doctor config without them", async () => {
    const tempDir = createCaseDir("nova-ai-update");
    const configPath = path.join(tempDir, "nova-ai.json");
    const preUpdateConfig = {
      update: { channel: "stable" },
      channels: {
        whatsapp: {
    ...
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_POST_CORE: "1",
        NOVA_AI_UPDATE_POST_CORE_CHANNEL: "stable",
        NOVA_AI_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_POST_CORE: "1",
        NOVA_AI_UPDATE_POST_CORE_CHANNEL: "stable",
        NOVA_AI_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    const syncConfig = syncPluginCall()?.config as
      | (Nova AIConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: Nova AIConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("nova-ai-update");
    const gitRoot = path.join(tempDir, "..", "nova-ai");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: Nova AIConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `nova-ai update`.",
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  it.each([
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("nova-ai-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("nova-ai-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("nova-ai-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            NOVA_AI_CONFIG_PATH: "./config/nova-ai.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        NOVA_AI_STATE_DIR: path.resolve("./state"),
        NOVA_AI_CONFIG_PATH: path.resolve("./config/nova-ai.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        NOVA_AI_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ NOVA_AI_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.NOVA_AI_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        NOVA_AI_UPDATE_IN_PROGRESS: undefined,
        NOVA_AI_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        NOVA_AI_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.NOVA_AI_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.NOVA_AI_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.NOVA_AI_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(process.env.NOVA_AI_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.NOVA_AI_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.NOVA_AI_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config?: Nova AIConfig }) => ({
        changed: true,
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as Nova AIConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as Nova AIConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `nova-ai update --channel <stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("nova-ai-update-wizard");
    await withEnvAsync({ NOVA_AI_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/nova-ai as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ NOVA_AI_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "nova-ai"));
    });
