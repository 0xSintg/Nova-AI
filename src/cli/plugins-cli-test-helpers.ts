...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as Nova AIConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: Nova AIConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/nova-ai-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: Nova AIConfig) =>
    invokeMock<
      [Nova AIConfig],
...
export function resetPluginsCliTestState() {
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: Nova AIConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
