...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "nova-ai", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "nova-ai", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "nova-ai", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "nova-ai", "health"],
    });
  });

  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "nova-ai", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "nova-ai", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "nova-ai", "status"])).toEqual({
      ok: true,
      container: null,
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "nova-ai", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "nova-ai", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to NOVA_AI_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "nova-ai", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "nova-ai", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "nova-ai", "status"], {
        NOVA_AI_CONTAINER: "demo",
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "nova-ai", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "nova-ai", "status"],
    });
  });

  it("uses NOVA_AI_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "nova-ai", "status"], {
        env: { NOVA_AI_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      [
        ...
        "--env",
        "NOVA_AI_CLI_CONTAINER_BYPASS=1",
        "demo",
        "nova-ai",
        "status",
      ],
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "nova-ai", "status"], {
      env: {
        ...
        NOVA_AI_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("passes the proxy URL env fallback into the child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "nova-ai", "status"], {
      env: {
        NOVA_AI_CONTAINER: "demo",
        NOVA_AI_PROXY_URL: " http://proxy.internal:3128 ",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("redacts proxy URL credentials and URL suffixes before rejecting loopback container proxy forwarding", () => {
    ...
    expect(message).toContain("NOVA_AI_PROXY_URL=http://redacted:redacted@127.0.0.1:3128/");
  ...
  it("allows explicitly overridden loopback proxy URL forwarding into a child container CLI", () => {
    ...
    maybeRunCliInContainer(["node", "nova-ai", "status"], {
      env: {
        ...
        NOVA_AI_CONTAINER_ALLOW_LOOPBACK_PROXY_URL: "1",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
    ...
    expect(podmanCall[1]).toContain("NOVA_AI_PROXY_URL=http://127.0.0.1:3128");
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "nova-ai", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "nova-ai", "--container", "demo", "health"], {
        env: { USER: "nova-ai" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "nova-ai", NOVA_AI_CONTAINER: "" },
      },
    );
  });
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", NOVA_AI_CONTAINER: "" },
      },
    );
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "nova-ai", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over NOVA_AI_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "nova-ai", "--container", "flag-demo", "health"], {
        env: { NOVA_AI_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "nova-ai", "--container", "demo", "status"], {
        env: { NOVA_AI_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
      argv: ["node", "nova-ai", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "nova-ai", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
      "nova-ai update is not supported with --container; rebuild or restart the container image instead.",
    );
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "nova-ai", "--container", "demo", "--no-color", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
