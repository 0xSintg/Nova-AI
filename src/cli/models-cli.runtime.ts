...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `nova-ai models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("nova-ai agents list")} and set the per-agent model in agent config.`,
  );
}
