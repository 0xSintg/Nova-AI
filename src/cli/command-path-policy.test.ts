...
describe("command-path-policy", () => {
  ...
  it("applies exact overrides after broader channel plugin rules", () => {
    ...
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "nova-ai", "channels", "status"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("bypass");
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "nova-ai", "channels", "status", "--probe"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("default");
  ...
  it("defaults unknown command paths to network proxy routing", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "googlemeet", "login"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "tool", "image_generate"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "tools", "effective"])).toBe("bypass");
  });

  it("resolves static network proxy bypass policies from the catalog", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "nova-ai", "config", "get", "proxy.enabled"]),
    ).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "proxy", "start"])).toBe("bypass");
  });

  it("resolves mixed network proxy policies from argv-sensitive catalog entries", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "gateway"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "gateway", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "gateway", "health"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "node", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "node", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "agent", "--local"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "agent", "run"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "channels", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "nova-ai", "channels", "status", "--probe"]),
    ).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "models", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "models", "status", "--probe"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "skills", "info", "browser"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "skills", "check"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "skills", "list"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "skills", "search", "browser"])).toBe(
      "default",
    );
  });
  ...
  it("uses the longest catalog command path for deep network proxy overrides", async () => {
    ...
    expect(resolveCliCatalogCommandPath(["node", "nova-ai", "nodes", "camera", "snap"])).toEqual([
      "nodes",
      "camera",
      "snap",
    ]);
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "nodes", "camera", "snap"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "nodes", "camera", "list"])).toBe(
      "bypass",
    );
  });
  ...
  it("stops catalog command path resolution before positional arguments", () => {
    expect(
      resolveCliCatalogCommandPath(["node", "nova-ai", "config", "get", "proxy.enabled"]),
    ).toEqual(["config", "get"]);
    expect(
      resolveCliCatalogCommandPath(["node", "nova-ai", "message", "send", "--to", "demo"]),
    ).toEqual(["message"]);
  });

  it("treats bare gateway invocations with options as the gateway runtime", () => {
    const argv = ["node", "nova-ai", "gateway", "--port", "1234"];
  ...
  it("does not let gateway run option values spoof bypass subcommands", () => {
    for (const argv of [
      ["node", "nova-ai", "gateway", "--token", "status"],
      ["node", "nova-ai", "gateway", "--token=status"],
      ["node", "nova-ai", "gateway", "--password", "health"],
      ["node", "nova-ai", "gateway", "--password-file", "status"],
      ["node", "nova-ai", "gateway", "--ws-log", "compact"],
    ]) {
  ...
  it("still resolves real gateway bypass subcommands after their command token", () => {
    expect(resolveCliCatalogCommandPath(["node", "nova-ai", "gateway", "status"])).toEqual([
      "gateway",
      "status",
    ]);
    expect(
      resolveCliCatalogCommandPath(["node", "nova-ai", "gateway", "status", "--token", "secret"]),
    ).toEqual(["gateway", "status"]);
    expect(resolveCliNetworkProxyPolicy(["node", "nova-ai", "gateway", "status"])).toBe("bypass");
  });
});
