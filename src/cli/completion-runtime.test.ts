...
describe("completion-runtime", () => {
  ...
  it("formats PowerShell source and reload commands with single-quoted paths", () => {
    expect(
      formatCompletionSourceLine("powershell", "nova-ai", "C:\\Users\\Ada\\open'claw.ps1"),
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-completion-state-"));

    process.env.HOME = homeDir;
    process.env.NOVA_AI_STATE_DIR = stateDir;

    try {
      const cachePath = resolveCompletionCachePath("powershell", "nova-ai");
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

      await installCompletion("powershell", true, "nova-ai");

      const profilePath = resolveCompletionProfilePath("powershell");
      const profile = await fs.readFile(profilePath, "utf-8");
      expect(profile).toBe(`# Nova AI Completion\n. '${cachePath}'\n`);
    } finally {
