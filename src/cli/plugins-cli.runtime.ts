...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, params.env, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "nova-ai doctor --fix" to install ${installSpec}, or install it manually with "nova-ai plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsEnableCommand(id: string): Promise<void> {
  ...
  const cfg = (snapshot.sourceConfig ?? snapshot.config) as Nova AIConfig;
  ...
  let next: Nova AIConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("nova-ai plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    nova-ai plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    nova-ai plugins registry --refresh");
      lines.push("    nova-ai gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.nova-ai.com/plugin");
  lines.push("");
