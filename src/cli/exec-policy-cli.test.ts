...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: Nova AIConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: Nova AIConfig }>
    >(async () => ({
      path: "/tmp/nova-ai.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: Nova AIConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/nova-ai.json");
    ...
    expect(output).not.toContain("/tmp/nova-ai.json\nforged");
