...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.nova-ai${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.NOVA_AI_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.NOVA_AI_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.NOVA_AI_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.NOVA_AI_CONFIG_PATH)) {
    env.NOVA_AI_CONFIG_PATH = path.join(stateDir, "nova-ai.json");
  }

  if (profile === "dev" && !env.NOVA_AI_GATEWAY_PORT?.trim()) {
    env.NOVA_AI_GATEWAY_PORT = "19001";
  }
}
