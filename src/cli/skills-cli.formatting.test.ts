...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-bundled-skills-test-"));
    process.env.NOVA_AI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
