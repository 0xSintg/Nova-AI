...
import type { StaleNova AIUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleNova AIUpdateLaunchdJobs = vi.fn<() => Promise<StaleNova AIUpdateLaunchdJob[]>>(
  async () => [],
);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    NOVA_AI_STATE_DIR: "/tmp/nova-ai-daemon",
    NOVA_AI_CONFIG_PATH: "/tmp/nova-ai-daemon/nova-ai.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.NOVA_AI_STATE_DIR ?? "/tmp/nova-ai-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.NOVA_AI_CONFIG_PATH ?? `${stateDir}/nova-ai.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/nova-ai-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleNova AIUpdateLaunchdJobs: () => findStaleNova AIUpdateLaunchdJobs(),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.NOVA_AI_STATE_DIR = "/tmp/nova-ai-cli";
    process.env.NOVA_AI_CONFIG_PATH = "/tmp/nova-ai-cli/nova-ai.json";
    ...
    findStaleNova AIUpdateLaunchdJobs.mockReset();
    findStaleNova AIUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/nova-ai-daemon/nova-ai.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/nova-ai-cli/nova-ai.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.NOVA_AI_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.NOVA_AI_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.NOVA_AI_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.NOVA_AI_STATE_DIR).toBe("/tmp/nova-ai-daemon");
    expect(handoffInput.NOVA_AI_CONFIG_PATH).toBe("/tmp/nova-ai-daemon/nova-ai.json");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      findStaleNova AIUpdateLaunchdJobs.mockResolvedValueOnce([
        {
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleNova AIUpdateLaunchdJobs).not.toHaveBeenCalled();
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-nova-ai/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-status-config-"));
    const configPath = path.join(tmp, "nova-ai.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.NOVA_AI_STATE_DIR = tmp;
    process.env.NOVA_AI_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        NOVA_AI_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.NOVA_AI_GATEWAY_TOKEN = "env-token";
    process.env.NOVA_AI_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "nova-ai-gateway" }],
        hints: [],
      },
      healthy: false,
