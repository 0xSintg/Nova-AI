...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("nova-ai gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.missingSupervision) {
    hints.push(
      `LaunchAgent installed but not loaded. Run: ${formatCliCommand("nova-ai gateway restart", env)}`,
    );
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.NOVA_AI_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.NOVA_AI_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const hints = buildPlatformServiceStartHints({
    installCommand: formatCliCommand("nova-ai gateway install", env),
    startCommand: formatCliCommand("nova-ai gateway", env),
