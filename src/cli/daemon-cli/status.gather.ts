...
type DaemonConfigContext = {
  ...
  cliCfg: Nova AIConfig;
  daemonCfg: Nova AIConfig;
...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): Nova AIConfig | null {
...
function coerceStatusConfig(value: unknown): Nova AIConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleNova AIUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleNova AIUpdateLaunchdJobs }) => findStaleNova AIUpdateLaunchdJobs())
          .catch(() => [])
      : [];
