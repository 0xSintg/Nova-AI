...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("nova-ai-daemon-install-int-");
    configPath = path.join(tempHome, "nova-ai.json");
    process.env.HOME = tempHome;
    process.env.NOVA_AI_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer Nova AI", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.NOVA_AI_GATEWAY_TOKEN).toBeUndefined();
  });
});
