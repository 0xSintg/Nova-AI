...
describe("isGatewayRunFastPathArgv", () => {
  it("matches only plain gateway foreground starts without root options or help", () => {
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "--force"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "--port", "18789"])).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "--auth=none"])).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "nova-ai", "--no-color", "gateway", "--bind", "loopback"]),
    ).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "run"])).toBe(true);
    expect(
      isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "run", "--raw-stream-path", "x"]),
    ).toBe(true);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "call", "health"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "--help"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "--port"])).toBe(false);
    expect(isGatewayRunFastPathArgv(["node", "nova-ai", "gateway", "--unknown"])).toBe(false);
  });
});
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "nova-ai", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "nova-ai"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "--profile", "work"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "approvals"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "channels"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "cron"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "devices"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "plugins"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "mcp"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "models", "status", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "nova-ai", "tools", "effective"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "nova-ai", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "nova-ai", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "nova-ai", "acp", "-v"])).toBe(true);
  });
});

describe("shouldStartCrestodianForBareRoot", () => {
  it("starts Crestodian for bare root invocations", () => {
    expect(shouldStartCrestodianForBareRoot(["node", "nova-ai"])).toBe(true);
    expect(shouldStartCrestodianForBareRoot(["node", "nova-ai", "--profile", "work"])).toBe(true);
    expect(shouldStartCrestodianForBareRoot(["node", "nova-ai", "--dev"])).toBe(true);
  });

  it("does not start Crestodian for help, version, or commands", () => {
    expect(shouldStartCrestodianForBareRoot(["node", "nova-ai", "--help"])).toBe(false);
    expect(shouldStartCrestodianForBareRoot(["node", "nova-ai", "-V"])).toBe(false);
    expect(shouldStartCrestodianForBareRoot(["node", "nova-ai", "status"])).toBe(false);
  });
});
...
describe("shouldStartCrestodianForModernOnboard", () => {
  ...
  it("keeps classic onboard and help on the normal command path", () => {
    expect(shouldStartCrestodianForModernOnboard(["node", "nova-ai", "onboard"])).toBe(false);
    expect(
      shouldStartCrestodianForModernOnboard(["node", "nova-ai", "onboard", "--modern", "--help"]),
    ).toBe(false);
  });
});
...
describe("shouldStartProxyForCli", () => {
  it("starts managed proxy routing for the --update shorthand", () => {
    expect(shouldStartProxyForCli(["node", "nova-ai", "--update"])).toBe(true);
    expect(shouldStartProxyForCli(["node", "nova-ai", "--profile", "p", "--update"])).toBe(true);
  });

  it("skips managed proxy routing for bare parent default help", () => {
    expect(shouldStartProxyForCli(["node", "nova-ai", "plugins"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "nova-ai", "channels"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "nova-ai", "cron"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "nova-ai", "devices"])).toBe(false);
    expect(shouldStartProxyForCli(["node", "nova-ai", "mcp"])).toBe(false);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "help", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "tools", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "--help", "status"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "nova-ai", "help", "gateway"])).toBe(false);
  });
});

describe("shouldUseBrowserHelpFastPath", () => {
  it("uses the fast path for browser command help only", () => {
    expect(shouldUseBrowserHelpFastPath(["node", "nova-ai", "browser", "--help"])).toBe(true);
    expect(shouldUseBrowserHelpFastPath(["node", "nova-ai", "browser", "-h"])).toBe(true);
    expect(
      shouldUseBrowserHelpFastPath(["node", "nova-ai", "--profile", "work", "browser", "-h"]),
    ).toBe(true);
    expect(shouldUseBrowserHelpFastPath(["node", "nova-ai", "browser", "status", "--help"])).toBe(
      false,
    );
    expect(shouldUseBrowserHelpFastPath(["node", "nova-ai", "status", "--help"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("nova-ai memory");
  });
  ...
  it("explains disabled-by-default parent plugins for CLI command aliases", () => {
    ...
    expect(message).toContain("nova-ai plugins enable voice-call");
  });
