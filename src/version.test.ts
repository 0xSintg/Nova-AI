...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "nova-ai-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __NOVA_AI_VERSION__");
    expect(source).toContain("? __NOVA_AI_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "nova-ai", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "nova-ai", version: "2.3.4" });
  ...
  it("ignores non-nova-ai package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers NOVA_AI_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        NOVA_AI_VERSION: "9.9.9",
        NOVA_AI_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale NOVA_AI_VERSION for compatibility checks", () => {
    const previous = process.env.NOVA_AI_VERSION;
    const previousService = process.env.NOVA_AI_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.NOVA_AI_VERSION = "2026.3.25";
      process.env.NOVA_AI_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("NOVA_AI_VERSION", previous);
      restoreEnvValue("NOVA_AI_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        NOVA_AI_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        NOVA_AI_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        NOVA_AI_VERSION: "2026.3.99",
        NOVA_AI_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
