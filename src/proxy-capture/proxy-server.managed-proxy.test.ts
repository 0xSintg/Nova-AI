...
async function makeSettings() {
  testRoot = await mkdtemp(join(tmpdir(), "nova-ai-debug-proxy-managed-proxy-"));
...
describe("debug proxy managed-proxy direct upstream policy", () => {
  const originalProxyActive = process.env["NOVA_AI_PROXY_ACTIVE"];
  const originalAllowDirect =
    process.env["NOVA_AI_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];

  beforeEach(async () => {
    await cleanupTestDirs();
    delete process.env["NOVA_AI_PROXY_ACTIVE"];
    delete process.env["NOVA_AI_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];
  });
  ...
  afterEach(async () => {
    if (originalProxyActive === undefined) {
      ...
      process.env["NOVA_AI_PROXY_ACTIVE"] = originalProxyActive;
    }
  ...
  it("uses shared truthy parsing for managed proxy mode", () => {
    process.env["NOVA_AI_PROXY_ACTIVE"] = "true";
  ...
  it("allows direct upstreams with explicit diagnostic override", () => {
    process.env["NOVA_AI_PROXY_ACTIVE"] = "1";
    process.env["NOVA_AI_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"] = "1";
