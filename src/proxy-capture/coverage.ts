...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[nova-ai proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[nova-ai proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`nova-ai proxy coverage\` for details.`,
  );
}
