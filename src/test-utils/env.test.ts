...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "NOVA_AI_ENV_TEST_A";
    const keyB = "NOVA_AI_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "NOVA_AI_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "NOVA_AI_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "NOVA_AI_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "NOVA_AI_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "NOVA_AI_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "NOVA_AI_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "NOVA_AI_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "nova-ai-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousNova AIHome = process.env.NOVA_AI_HOME;
    const previousStateDir = process.env.NOVA_AI_STATE_DIR;
    const previousBundledDir = process.env.NOVA_AI_BUNDLED_PLUGINS_DIR;
    process.env.NOVA_AI_HOME = "/srv/nova-ai-home";
    process.env.NOVA_AI_STATE_DIR = "/srv/nova-ai-state";
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = "/srv/nova-ai-bundled";
    ...
    try {
      ...
      expect(env.NOVA_AI_HOME).toBeUndefined();
      expect(env.NOVA_AI_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.NOVA_AI_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("NOVA_AI_HOME", previousNova AIHome);
      restoreEnvKey("NOVA_AI_STATE_DIR", previousStateDir);
      restoreEnvKey("NOVA_AI_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processNova AIHome: process.env.NOVA_AI_HOME,
          processBundledDir: process.env.NOVA_AI_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.NOVA_AI_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processNova AIHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.NOVA_AI_HOME).toBe("/srv/nova-ai-home");
    } finally {
      restoreEnvKey("NOVA_AI_HOME", previousNova AIHome);
    }
  });
