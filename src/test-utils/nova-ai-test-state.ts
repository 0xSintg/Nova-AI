...
type Nova AITestStateLayout = "home" | "state-only" | "split";

type Nova AITestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type Nova AITestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: Nova AITestStateLayout;
  scenario?: Nova AITestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "nova-ai-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "nova-ai.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "nova-ai.json"),
...
function scenarioConfig(options: Nova AITestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "nova-ai-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: Nova AITestStateOptions): Record<string, string | undefined> {
...
export async function createNova AITestState(
  options: Nova AITestStateOptions = {},
): Promise<Nova AITestState> {
...
export async function withNova AITestState<T>(
  options: Nova AITestStateOptions,
  fn: (state: Nova AITestState) => Promise<T>,
): Promise<T> {
  const state = await createNova AITestState(options);
  try {
