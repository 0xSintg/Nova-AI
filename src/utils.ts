...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".nova-ai");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.NOVA_AI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$NOVA_AI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via NOVA_AI_STATE_DIR.
