...
homepage: https://docs.nova-ai.com/automation/hooks#command-logger
metadata:
  {
    "nova-ai":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.nova-ai/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.nova-ai/logs/commands.log ~/.nova-ai/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/nova-ai`:
   ```
   /home/username/.nova-ai/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
