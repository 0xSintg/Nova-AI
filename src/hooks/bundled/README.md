...
This directory contains hooks that ship with Nova AI. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.nova-ai/workspace`)
...
homepage: https://docs.nova-ai.com/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.nova-ai/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.nova-ai/nova-ai.json`:
...
2. Restart gateway: `pkill -9 -f 'nova-ai.*gateway' && pnpm nova-ai gateway`
3. Enable the hook: `nova-ai hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.nova-ai.com/automation/hooks
