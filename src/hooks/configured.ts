...
export function hasConfiguredInternalHooks(config: Nova AIConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: Nova AIConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
