...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "nova-ai-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "nova-ai-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "nova-ai-bundled");
      const plugin = makeHookEntry("shared", "nova-ai-plugin");
      const managed = makeHookEntry("shared", "nova-ai-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("nova-ai-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "nova-ai-managed");
      const workspace = makeHookEntry("shared", "nova-ai-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "nova-ai-workspace");
      const second = makeHookEntry("shared", "nova-ai-workspace");
      second.hook.handlerPath = "/tmp/nova-ai-workspace/shared/handler-2.js";
