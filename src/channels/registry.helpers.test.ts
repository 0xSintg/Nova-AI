...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://nova-ai.com");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("nova-ai-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["nova-ai-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("nova-ai-weixin");
    expect(getRegisteredChannelPluginMeta("NOVA_AI-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
