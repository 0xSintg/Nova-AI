...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: Nova AIConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: Nova AIConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: Nova AIConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: Nova AIConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: Nova AIConfig): string {
