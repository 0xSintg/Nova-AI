...
import type { Nova AIPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["NOVA_AI_PLUGIN_CATALOG_PATHS", "NOVA_AI_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveNova AIPackageRootSync({ cwd: process.cwd() }),
    resolveNova AIPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<Nova AIPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
