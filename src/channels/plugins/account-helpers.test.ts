...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): Nova AIConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as Nova AIConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: Nova AIConfig) => string[];
  input: Nova AIConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: Nova AIConfig, expected: string) {
