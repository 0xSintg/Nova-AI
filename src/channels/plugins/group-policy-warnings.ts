...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: Nova AIConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: Nova AIConfig }>(
  collector: WarningCollector<{ cfg: Nova AIConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: Nova AIConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: Nova AIConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: Nova AIConfig },
>(
  projectCfg: (cfg: Nova AIConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: Nova AIConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: Nova AIConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: Nova AIConfig },
>(params: {
  providerConfigPresent: (cfg: Nova AIConfig) => boolean;
