...
// resolveNova AIPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/nova-ai-root.js", () => ({
  resolveNova AIPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveNova AIPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
const originalTrustBundledPluginsDir = process.env.NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalTrustBundledPluginsDir === undefined) {
    ...
    process.env.NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPluginsDir;
  }
...
function useBundledPluginsDir(extensionsRoot: string | undefined): void {
  if (extensionsRoot) {
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = extensionsRoot;
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "nova-ai" });
...
function seedChannelPkg(
  ...
  writeJsonFile(path.join(pluginDir, "nova-ai.plugin.json"), {
    id: opts.id,
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // NOVA_AI_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an NOVA_AI_BUNDLED_PLUGINS_DIR override pointing at
