...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronStorePath: () => "/tmp/nova-ai-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/nova-ai-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "nova-ai-task-registry-" }, async (root) => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through Nova AI subagent sessions", async () => {
    await withTaskRegistryTempDir(async (root) => {
      process.env.NOVA_AI_STATE_DIR = root;
      resetTaskRegistryForTests();
      const task = createTaskRecord({
