...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "nova-ai.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "nova-ai.taskRegistry.controlRuntimeOverride",
);
  ...
  try {
    if (task.runtime !== "cli") {
      ...
      if (!childSessionKey) {
        ...
        // not have Nova AI child sessions to terminate. Cancellation clears
