...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        NOVA_AI_BASE_ENV: "base",
        NOVA_AI_TO_REMOVE: undefined,
      },
      env: {
        NOVA_AI_TEST_ENV: "ok",
      },
    ...
    expect(resolved.NOVA_AI_BASE_ENV).toBe("base");
    expect(resolved.NOVA_AI_TEST_ENV).toBe("ok");
    expect(resolved.NOVA_AI_TO_REMOVE).toBeUndefined();
    expect(resolved.NOVA_AI_CLI).toBe(NOVA_AI_CLI_ENV_VALUE);
  });
