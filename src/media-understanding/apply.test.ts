...
const TEMP_MEDIA_PREFIX = "nova-ai-media-";
...
function createGroqAudioConfig(): Nova AIConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): Nova AIConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): Nova AIConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: Nova AIConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredNova AITmpDir();
