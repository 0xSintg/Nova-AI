...
type HomeEnvSnapshot = {
  ...
  NOVA_AI_STATE_DIR: string | undefined;
  NOVA_AI_AGENT_DIR: string | undefined;
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".nova-ai", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.NOVA_AI_STATE_DIR = path.join(home, ".nova-ai");
    process.env.NOVA_AI_AGENT_DIR = path.join(home, ".nova-ai", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".nova-ai", "agent");
