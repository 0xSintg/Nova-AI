...
import { buildCurrentNova AICliCommand } from "./commands-nova-ai-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.nova-ai.com/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "nova-ai gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentNova AICliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Nova AI session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Nova AI session file.",
    ) === true
  );
}
