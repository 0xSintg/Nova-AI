...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("nova-ai.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("nova-ai.reply.full-runtime");

type ReplyConfigWithMarker = Nova AIConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.NOVA_AI_ALLOW_SLOW_REPLY_TESTS === "1" || env.NOVA_AI_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends Nova AIConfig>(
  config: T,
...
export function withFastReplyConfig<T extends Nova AIConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends Nova AIConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is Nova AIConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  getRuntimeConfig: () => Nova AIConfig;
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set NOVA_AI_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as Nova AIConfig;
}
