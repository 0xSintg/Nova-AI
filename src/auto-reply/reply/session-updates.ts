...
// Skill frontmatter `requires.config` reads the full Nova AI config, so cache
// reuse must follow the same boundary without putting raw secrets in Map keys.
function fingerprintSkillSnapshotConfig(config: Nova AIConfig): string {
  return crypto
    .createHash("sha256")
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.NOVA_AI_TEST_FAST === "1") {
