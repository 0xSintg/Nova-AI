...
const workspaceHarness = createCommandWorkspaceHarness("nova-ai-command-plugins-install-");
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs a plugin from a local path", async () => {
    ...
    await withTempHome("nova-ai-command-plugins-home-", async () => {
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@nova-ai/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@nova-ai/clawhub-demo",
    ...
    await withTempHome("nova-ai-command-plugins-home-", async () => {
      ...
      const params = buildPluginsParams(
        "/plugins install clawhub:@nova-ai/clawhub-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@nova-ai/clawhub-demo@1.2.3",
      );
      expectPersistedInstall("clawhub-demo", {
        source: "clawhub",
        spec: "clawhub:@nova-ai/clawhub-demo@1.2.3",
  ...
  it("refuses plugin installs in Nix mode before package installer side effects", async () => {
    const previousNixMode = process.env.NOVA_AI_NIX_MODE;
    ...
    try {
      await withTempHome("nova-ai-command-plugins-home-", async () => {
        ...
        expect(result.reply?.text).toContain("NOVA_AI_NIX_MODE=1");
        expect(result.reply?.text).toContain("nix-nova-ai#quick-start");
      ...
      if (previousNixMode === undefined) {
        ...
        process.env.NOVA_AI_NIX_MODE = previousNixMode;
      }
    }
  });
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@nova-ai/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@nova-ai/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("nova-ai-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugin add clawhub:@nova-ai/alias-demo@1.0.0",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@nova-ai/alias-demo@1.0.0",
      );
    });
  });
  ...
  it("trusts catalog npm package installs with alternate selectors", async () => {
    ...
    await withTempHome("nova-ai-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install @wecom/wecom-nova-ai-plugin@latest",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
