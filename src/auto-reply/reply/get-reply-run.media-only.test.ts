...
describe("runPreparedReply media-only handling", () => {
  ...
  it("hydrates current MediaPaths into queued followup images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "nova-ai-followup-image-"));
  ...
  it("runs room events as contextual events instead of direct user prompts", async () => {
    ...
    const call = requireLastRunReplyAgentCall();
    expect(call?.commandBody).toBe("[Nova AI room event]");
    expect(call?.transcriptCommandBody).toBe("");
    expect(call?.followupRun.prompt).toBe("[Nova AI room event]");
    expect(call?.followupRun.transcriptPrompt).toBe("");
    ...
    expect(call?.followupRun.currentInboundContext?.text).toContain("[Nova AI room event]");
    expect(call?.followupRun.currentInboundContext?.text).toContain(
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[Nova AI heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[Nova AI heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[Nova AI session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[Nova AI session ${startupAction}]`);
