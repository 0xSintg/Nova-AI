...
type TransformConfigFileWithRetryMockParams<T = unknown> = {
  afterWrite?: unknown;
  transform: (
    currentConfig: Nova AIConfig,
    context: { snapshot: ConfigSnapshotMock; previousHash: string | null; attempt: number },
  ) =>
    | Promise<{ nextConfig: Nova AIConfig; result?: T }>
    | { nextConfig: Nova AIConfig; result?: T };
};

function configFromSnapshot(snapshot: ConfigSnapshotMock): Nova AIConfig {
...
vi.mock("../../plugins/toggle-config.js", () => ({
  setPluginEnabledInConfig: vi.fn((config: Nova AIConfig, id: string, enabled: boolean) => ({
    ...config,
    plugins: {
      ...config.plugins,
      entries: {
...
function buildCfg(): Nova AIConfig {
  return {
