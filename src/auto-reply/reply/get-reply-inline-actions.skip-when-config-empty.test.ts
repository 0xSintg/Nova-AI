...
const { buildStatusReplyMock, createNova AIToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createNova AIToolsMock: vi.fn(),
...
vi.mock("../../agents/nova-ai-tools.runtime.js", () => ({
  createNova AITools: (...args: unknown[]) => createNova AIToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(
      mockObjectArg(createNova AIToolsMock, "createNova AITools").requesterAgentIdOverride,
    ).toBe("named-worker");
  ...
  it("passes senderIsOwner into inline tool runtimes before owner-only filtering", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(mockObjectArg(createNova AIToolsMock, "createNova AITools").senderIsOwner).toBe(true);
  ...
  it("honors construction-time before-tool-call blocks for inline tool dispatch", async () => {
    ...
    const toolsArgs = mockObjectArg(createNova AIToolsMock, "createNova AITools");
  ...
  it("applies subagent policy to ACP envelope inline dispatch sessions", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-inline-acp-policy-"));
    try {
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledWith(
