...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: nova-ai doctor --non-interactive"),
  ...
  triggerNova AIRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerNova AIRestart: mocks.triggerNova AIRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerNova AIRestart.mockReset();
    mocks.triggerNova AIRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(sentinelPayload?.doctorHint).toBe("Run: nova-ai doctor --non-interactive");
    expect(sentinelPayload?.stats).toEqual({
      mode: "gateway.restart",
      reason: "/restart",
    });
    expect(mocks.triggerNova AIRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerNova AIRestart).not.toHaveBeenCalled();
      ...
      expect(sentinelPayload?.continuation).toEqual({
        kind: "agentTurn",
        message:
          "The gateway restart completed successfully. Tell the user Nova AI restarted successfully and continue any pending work.",
      });
    } finally {
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerNova AIRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
