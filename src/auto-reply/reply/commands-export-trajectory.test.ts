...
const hoisted = await vi.hoisted(async () => {
  ...
  return {
    ...
    exportTrajectoryBundleMock: vi.fn(() => ({
      outputDir: "/tmp/workspace/.nova-ai/trajectory-exports/nova-ai-trajectory-session",
      manifest: {
        eventCount: 7,
        runtimeEventCount: 3,
    ...
    resolveDefaultTrajectoryExportDirMock: vi.fn(
      () => "/tmp/workspace/.nova-ai/trajectory-exports/nova-ai-trajectory-session",
    ),
    accessMock: vi.fn(
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-export-command-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "nova-ai sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryReply", () => {
  ...
  it("builds a trajectory bundle from the target session", async () => {
    ...
    expect(String(exportParams.workspaceDir)).toContain("nova-ai-export-command-");
  });
  ...
  it("keeps user-named output paths inside the workspace trajectory export directory", async () => {
    ...
    expect(exportBundleParams().outputDir).toBe(
      path.join(params.workspaceDir, ".nova-ai", "trajectory-exports", "my-bundle"),
    );
  });
  ...
  it("rejects output paths redirected by a symlinked exports directory", async () => {
    ...
    fs.mkdirSync(path.join(workspaceDir, ".nova-ai"), { recursive: true });
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".nova-ai", "trajectory-exports"));
  ...
  it("rejects symlinked state directories before creating export folders", async () => {
    const workspaceDir = makeTempDir();
    const outsideDir = makeTempDir();
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".nova-ai"));
    const params = makeParams(workspaceDir);
    params.command.commandBodyNormalized = "/export-trajectory my-bundle";
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.nova-ai.com/tools/trajectory");
    ...
    expect(command).not.toContain("nova-ai sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("nova-ai-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("nova-ai sessions export-trajectory");
