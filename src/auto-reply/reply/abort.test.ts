...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop nova-ai!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@nova-ai_bot", { botUsername: "nova-ai_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@nova-ai_bot", { botUsername: "nova-ai_bot" })).toBe(true);
