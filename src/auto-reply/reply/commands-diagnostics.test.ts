...
function buildDiagnosticsParams(
  ...
  return {
    cfg: { commands: { text: true } } as Nova AIConfig,
    ctx: {
      Provider: "whatsapp",
      Surface: "whatsapp",
      CommandSource: "text",
...
describe("diagnostics command", () => {
  it("requests Gateway diagnostics approval without a duplicate pending chat reply", async () => {
    ...
    expect(execCall.defaults.approvalWarningText).toContain(
      "https://docs.nova-ai.com/gateway/diagnostics",
    );
    ...
    expect(command).not.toBe("nova-ai gateway diagnostics export --json");
  });
  ...
  it("falls back to a visible reply when approval cannot be queued", async () => {
    ...
    expect(result?.reply?.text).toContain("https://docs.nova-ai.com/gateway/diagnostics");
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, Nova AI session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
