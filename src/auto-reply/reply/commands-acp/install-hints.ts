...
export function resolveAcpInstallCommandHint(cfg: Nova AIConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `nova-ai plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^nova-ai plugins install /u, "");
    ...
    return "nova-ai plugins install acpx";
  }
