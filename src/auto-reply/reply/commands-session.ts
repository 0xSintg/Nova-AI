...
import { scheduleGatewaySigusr1Restart, triggerNova AIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Nova AI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  let sentinelPath: string | null = null;
  ...
  const restartMethod = triggerNova AIRestart();
  if (!restartMethod.ok) {
    await removeRestartSentinelFile(sentinelPath);
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Nova AI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
