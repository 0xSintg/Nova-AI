...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_nova-ai_decorated() { printf '%s\\n' 'Nova AI 2026.3.10 (abcdef0)'; }
fake_nova-ai_raw() { printf '%s\\n' "Nova AI dev's build"; }
NOVA_AI_BIN=fake_nova-ai_decorated resolve_nova-ai_version
NOVA_AI_BIN=fake_nova-ai_raw resolve_nova-ai_version
(
  cd "$1"
  source /dev/stdin <<'NOVA_AI_STDIN_INSTALLER'
${versionHelperSource}
fake_nova-ai_stdin() { printf '%s\\n' 'Nova AI 2026.3.10 (abcdef0)'; }
NOVA_AI_BIN=fake_nova-ai_stdin
resolve_nova-ai_version
NOVA_AI_STDIN_INSTALLER
)`,
      "nova-ai-version-test",
      params.stdinCwd,
    ],
    {
      cwd: process.cwd(),
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "nova-ai-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "Nova AI dev's build", "2026.3.10"]);
    },
  );
});
