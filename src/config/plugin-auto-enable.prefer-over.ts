...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.nova-ai) || !isRecord(entry.nova-ai.channel)) {
      continue;
    }
    const channel = entry.nova-ai.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: Nova AIConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: Nova AIConfig, pluginId: string) => boolean;
