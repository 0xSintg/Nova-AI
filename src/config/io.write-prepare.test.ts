...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "nova-ai-web-search": {
                source: "npm",
                spec: "@ollama/nova-ai-web-search",
                installPath: "/tmp/nova-ai-web-search",
                resolvedName: "@ollama/nova-ai-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "nova-ai-web-search": {
                source: "npm",
                spec: "@ollama/nova-ai-web-search@0.2.2",
                installPath: "/tmp/nova-ai-web-search",
                resolvedName: "@ollama/nova-ai-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("nova-ai config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('nova-ai config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/nova-ai.json",
        expectedConfigPath: "/tmp/nova-ai.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: Nova AIConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://nova-ai.com/config-from-include.json",
      gateway: { mode: "local" },
    };
