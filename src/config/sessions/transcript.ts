...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlyNova AIAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlyNova AIAssistant &&
    isTranscriptOnlyNova AIAssistantMessage(message)
  ) {
    return undefined;
  }
...
function isTranscriptOnlyNova AIAssistantMessage(message: {
  provider?: unknown;
  model?: unknown;
}): boolean {
  return (
    message.provider === "nova-ai" &&
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlyNova AIAssistant: true,
      });
      if (assistantText) {
...
export async function readTailAssistantTextFromSessionTranscript(
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      ...
      // Skip non-message entries (e.g. `nova-ai.cache-ttl` custom events) so
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return message.provider === "nova-ai" && message.model === "delivery-mirror";
}
