...
export function listConfiguredSessionStoreAgentIds(cfg: Nova AIConfig): string[] {
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "nova-ai agents list" to see configured agents.`,
      );
    }
    return [
      {
