...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("emits transcript update events for delivery mirrors", async () => {
    ...
    expect(message?.provider).toBe("nova-ai");
  ...
  it("skips transcript-only Nova AI assistant entries when reading latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("does not report transcript-only Nova AI assistant entries as latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("keeps transcript-only Nova AI assistant entries available to the tail reader", async () => {
    writeTranscriptStore();
  ...
  it("scans past trailing non-assistant entries (e.g. nova-ai.cache-ttl) to find the latest assistant text", async () => {
    // Regression for nova-ai/nova-ai#83427: the cache-ttl custom entry was
  ...
  it("does not reuse an older matching assistant message across turns", async () => {
    ...
    if (olderResult.ok && latestResult.ok && mirrorResult.ok) {
      ...
      expect(messageLine.message.provider).toBe("nova-ai");
