...
import { buildWebSearchProviderConfig, withTempHome, writeNova AIConfig } from "./test-helpers.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = Nova AISchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://nova-ai.com/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = Nova AISchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://nova-ai.com/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@nova-ai"]);
    }
  });
...
describe("config strict validation", () => {
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeNova AIConfig(home, {
        gateway: { bind: "${NOVA_AI_BIND}" },
      });

      const prev = process.env.NOVA_AI_BIND;
      process.env.NOVA_AI_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.NOVA_AI_BIND;
        } else {
          process.env.NOVA_AI_BIND = prev;
        }
      }
    });
  });
