...
import { withTempHome as withTempHomeBase } from "nova-ai/plugin-sdk/test-env";
import { resetPluginLoaderTestStateForTest } from "../plugins/loader.test-fixtures.js";
import { clearPluginSetupRegistryCache } from "../plugins/setup-registry.js";
import { resetConfigRuntimeState, type Nova AIConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        NOVA_AI_MPM_CATALOG_PATHS: undefined,
        NOVA_AI_LOAD_SHELL_ENV: undefined,
        NOVA_AI_DEFER_SHELL_ENV_FALLBACK: undefined,
        NOVA_AI_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeNova AIConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.NOVA_AI_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected NOVA_AI_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeNova AIConfig(home, config);
