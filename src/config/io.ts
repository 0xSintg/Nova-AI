...
import type { Nova AIConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: Nova AIConfig };
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: Nova AIConfig;
  /**
...
function coerceConfig(value: unknown): Nova AIConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as Nova AIConfig;
}
...
function stampConfigVersion(cfg: Nova AIConfig, version?: string): Nova AIConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: Nova AIConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your Nova AI config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `nova-ai --version`, `which nova-ai`, and `nova-ai gateway status --deep`.",
        "If unexpected, update PATH so `nova-ai` points to the version you want, or reinstall the Gateway service from that same Nova AI install.",
      ].join("\n"),
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as Nova AIConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: Nova AIConfig): Nova AIConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run nova-ai plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function loadConfig(): Nova AIConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as Nova AIConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
      ...
      const loadValidationPluginMetadataSnapshot = (config: Nova AIConfig) => {
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    let fallbackSourceConfig: Nova AIConfig = {};
  ...
  async function readBestEffortConfig(): Promise<Nova AIConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<Nova AIConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
  ...
  ): Promise<{ persistedHash: string; persistedConfig: Nova AIConfig }> {
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as Nova AIConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as Nova AIConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as Nova AIConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.NOVA_AI_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.NOVA_AI_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.NOVA_AI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `NOVA_AI_CONFIG_PATH` (and friends) are expected to work even
...
export function projectConfigOntoRuntimeSourceSnapshot(config: Nova AIConfig): Nova AIConfig {
...
export function loadConfig(): Nova AIConfig {
  ...
  // reparsing nova-ai.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): Nova AIConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<Nova AIConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<Nova AIConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: Nova AIConfig = nextCfg;
  try {
