...
import type { ConfigFileSnapshot, Nova AIConfig } from "./types.nova-ai.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "nova-ai-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { NOVA_AI_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("logs health-state write failures through public config reads", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".nova-ai", "nova-ai.json");
      const healthPath = path.join(home, ".nova-ai", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".nova-ai", "nova-ai.json");
      const pluginDir = path.join(home, ".nova-ai", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "nova-ai.plugin.json");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".nova-ai"),
          }),
        ).resolves.toBeNull();
        await expect(fs.readFile(configPath, "utf-8")).resolves.toBe(initialRaw);
      } finally {
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".nova-ai", "nova-ai.json");
      const pluginDir = path.join(home, ".nova-ai", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".nova-ai", "nova-ai.json");
      const unwritableStatePath = path.join(home, ".nova-ai");
  ...
  it("keeps writes inside an NOVA_AI_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".nova-ai", "nova-ai.json");
      ...
      const env = { NOVA_AI_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const io = createConfigIO({
        env,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "nova-ai.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "nova-ai.json"), "utf-8"),
      ) as {
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          NOVA_AI_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            $schema: "https://nova-ai.com/config.json",
            gateway: { mode: "local" },
          },
          null,
          2,
      ...
      expect(persisted.$schema).toBe("https://nova-ai.com/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/nova-ai-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as Nova AIConfig;
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".nova-ai", "nova-ai.json");
      const includePath = path.join(home, ".nova-ai", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://nova-ai.com/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/nova-ai-test-required-plugin",
          source: "/tmp/nova-ai-test-required-plugin/index.ts",
          manifestPath: "/tmp/nova-ai-test-required-plugin/nova-ai.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      process.env.NOVA_AI_CONFIG_PATH = configPath;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.NOVA_AI_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
  ...
  it("notifies in-process reloaders with resolved source config when persisted env refs are restored", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".nova-ai", "nova-ai.json");
      const previousConfigPath = process.env.NOVA_AI_CONFIG_PATH;
      const previousGatewayToken = process.env.NOVA_AI_GATEWAY_TOKEN;
      process.env.NOVA_AI_CONFIG_PATH = configPath;
      process.env.NOVA_AI_GATEWAY_TOKEN = "gateway-token-runtime";
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${NOVA_AI_GATEWAY_TOKEN}" },
            },
      ...
      try {
        ...
        expect(persisted.gateway?.auth?.token).toBe("${NOVA_AI_GATEWAY_TOKEN}");
        ...
        if (previousGatewayToken === undefined) {
          ...
          process.env.NOVA_AI_GATEWAY_TOKEN = previousGatewayToken;
        }
      }
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/nova-ai-test-demo",
          source: "/tmp/nova-ai-test-demo/index.ts",
          manifestPath: "/tmp/nova-ai-test-demo/nova-ai.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        const postWriteSnapshot = await createConfigIO({
          env: { NOVA_AI_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
          homedir: () => home,
          logger: silentLogger,
  ...
  it("skipPluginValidation bypasses plugin schema rejection on writeConfigFile (#76800)", async () => {
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/nova-ai-test-strict-plugin",
            source: "/tmp/nova-ai-test-strict-plugin/index.ts",
            manifestPath: "/tmp/nova-ai-test-strict-plugin/nova-ai.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await expect(
          writeConfigFile({ agents: { list: "not-array" } } as unknown as Nova AIConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/nova-ai-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "nova-ai-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as Nova AIConfig;
      expect(persisted.logging?.file).toBe("~/nova-ai-upgrade-survivor/gateway.jsonl");
