...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ NOVA_AI_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-nova-ai",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-nova-ai#general");
  });
  ...
  it("derives session transcripts dir from NOVA_AI_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NOVA_AI_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when NOVA_AI_STATE_DIR differs from stored paths", () => {
