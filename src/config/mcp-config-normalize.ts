...
type Nova AIMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_NOVA_AI_TRANSPORT: Record<string, Nova AIMcpHttpTransport | "stdio"> = {
...
export function resolveNova AIMcpTransportAlias(
  value: unknown,
): Nova AIMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_NOVA_AI_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_NOVA_AI_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveNova AIMcpTransportAlias(next.type);
