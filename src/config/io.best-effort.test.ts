...
import { withTempHome, writeNova AIConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.nova-ai/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeNova AIConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.nova-ai`);
      expect(entries.some((entry) => entry.startsWith("nova-ai.json.clobbered."))).toBe(false);
    });
  });
