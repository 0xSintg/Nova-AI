...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.NOVA_AI_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveNova AIPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    NOVA_AI_STATE_DIR: path.join(os.tmpdir(), "nova-ai-config-doc-baseline-state"),
