...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: Nova AIConfig): boolean {
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: Nova AIConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: Nova AIConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: Nova AIConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: Nova AIConfig, plugin: PluginManifestRecord): boolean {
...
function collectConfiguredChannelIds(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): string[] {
...
function hasConfiguredWebSearchPluginEntry(cfg: Nova AIConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: Nova AIConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: Nova AIConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: Nova AIConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: Nova AIConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: Nova AIConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: Nova AIConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: Nova AIConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: Nova AIConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: Nova AIConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: Nova AIConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: Nova AIConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: Nova AIConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: Nova AIConfig): boolean {
  return (
...
function hasPluginEntries(cfg: Nova AIConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: Nova AIConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: Nova AIConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: Nova AIConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: Nova AIConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: Nova AIConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: Nova AIConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
