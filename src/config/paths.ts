...
/**
 * Nix mode detection: When NOVA_AI_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.NOVA_AI_NIX_MODE === "1";
}

export const isNixMode = resolveIsNixMode();

// Support the remaining legacy pre-rebrand state dir.
const LEGACY_STATE_DIRNAMES = [".novabot"] as const;
const NEW_STATE_DIRNAME = ".nova-ai";
const CONFIG_FILENAME = "nova-ai.json";
const LEGACY_CONFIG_FILENAMES = ["novabot.json"] as const;

function resolveDefaultHomeDir(): string {
  return resolveRequiredHomeDir(process.env, os.homedir);
}

/** Build a homedir thunk that respects NOVA_AI_HOME for the given env. */
 ...
 * Can be overridden via NOVA_AI_STATE_DIR.
 * Default: ~/.nova-ai
 */
export function resolveStateDir(
  ...
  const override = env.NOVA_AI_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.NOVA_AI_TEST_FAST === "1") {
    return newDir;
  }
 ...
 * outside the config directory. Set via `NOVA_AI_INCLUDE_ROOTS` as a
 ...
 * directory containing `nova-ai.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.NOVA_AI_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via NOVA_AI_CONFIG_PATH.
 * Default: ~/.nova-ai/nova-ai.json (or $NOVA_AI_STATE_DIR/nova-ai.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.NOVA_AI_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.NOVA_AI_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.NOVA_AI_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const nova-aiStateDir = env.NOVA_AI_STATE_DIR?.trim();
  if (nova-aiStateDir) {
    const resolved = resolveUserPath(nova-aiStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/nova-ai-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `nova-ai-${uid}` : "nova-ai";
 ...
 *
 * Precedence:
 * - `NOVA_AI_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.NOVA_AI_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.NOVA_AI_GATEWAY_PORT?.trim();
