...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.NOVA_AI_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.NOVA_AI_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.NOVA_AI_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `nova-ai doctor --fix` once the
