...
import type { ConfigFileSnapshot, Nova AIConfig } from "./types.js";

type MockValidationIssue = { path: string; message: string };
type MockValidationResult =
  | { ok: true; config: Nova AIConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: Nova AIConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/nova-ai.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "nova-ai-config-mutate-" });
  const originalNixMode = process.env.NOVA_AI_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.NOVA_AI_NIX_MODE = originalNixMode;
    }
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("Nova AI Nix overview: https://docs.nova-ai.com/install/nix");
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".nova-ai", "config", "plugins.json5");
  ...
  it("keeps single-file top-level plugins include writes when plugin validation is skipped", async () => {
    ...
    const nextConfig: Nova AIConfig = {
      plugins: {
        entries: {
          "strict-plugin": { enabled: true },
        },
