...
import {
  compareNova AIVersions,
  isSameNova AIStableFamily,
  parseNova AIVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseNova AIVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseNova AIVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseNova AIVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseNova AIVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseNova AIVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseNova AIVersion("2026.3")).toBeNull();
    expect(parseNova AIVersion("latest")).toBeNull();
  });
});

describe("compareNova AIVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareNova AIVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareNova AIVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareNova AIVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareNova AIVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareNova AIVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareNova AIVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameNova AIStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameNova AIStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameNova AIStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameNova AIStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameNova AIStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
