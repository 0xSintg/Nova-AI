...
async function tryWriteSingleTopLevelIncludeMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: Nova AIConfig } | null> {
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: Nova AIConfig,
): { persistedHash: string | null; persistedConfig: Nova AIConfig } {
  if (result) {
    return {
...
export async function mutateConfigFile<T = void>(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
  mutate: (draft: Nova AIConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
}): Promise<ConfigMutationResult<T>> {
