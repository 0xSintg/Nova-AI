...
function collectConfigEnvVarsByTarget(cfg?: Nova AIConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: Nova AIConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: Nova AIConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.NOVA_AI_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
