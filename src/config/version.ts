...
export function parseNova AIVersion(raw: string | null | undefined): Nova AIVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeNova AIVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseNova AIVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameNova AIStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseNova AIVersion(a);
  const parsedB = parseNova AIVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareNova AIVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseNova AIVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseNova AIVersion(current);
  const parsedTouched = parseNova AIVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameNova AIStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareNova AIVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: Nova AIVersion): number {
