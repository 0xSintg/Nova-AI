    ...
    const res = validateConfigObject({
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: ["D:/data/nova-ai/src:/src:ro", "D:\\data\\nova-ai\\output:/output:rw"],
            },
          },
        },
      },
...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("nova-ai-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "nova-ai-sandbox-browser-global" },
      agentBrowser: { network: "nova-ai-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("nova-ai-sandbox-browser-agent");
  });
