...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses NOVA_AI_HOME for tilde expansion", () => {
    vi.stubEnv("NOVA_AI_HOME", "/srv/nova-ai-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/nova-ai-home", "cron", "jobs.json"));
  });
});
