...
type DispatchCronDeliveryParams = {
  cfg: Nova AIConfig;
  cfgWithAgentDefaults: Nova AIConfig;
  deps: CliDeps;
  job: CronJob;
  agentId: string;
...
function pruneCompletedDirectCronDeliveries(now: number) {
  const ttlMs = process.env.NOVA_AI_TEST_FAST === "1" ? 60_000 : 24 * 60 * 60 * 1000;
...
function resolveDirectCronRetryDelaysMs(): readonly number[] {
  return process.env.NODE_ENV === "test" && process.env.NOVA_AI_TEST_FAST === "1"
    ? [0, 0, 0]
    : [5_000, 10_000, 20_000];
}
...
export async function dispatchCronDelivery(
  ...
  const formatDeliveryTargetError = (error: string) =>
    params.sourceDeliveryOutcome.unverifiedMessageToolDelivery
      ? `${error}; the agent used the message tool, but Nova AI could not verify that message matched the cron delivery target`
      : error;
    ...
    try {
      ...
      const runDelivery = async () => {
        ...
        const send = await sendDurableMessageBatch({
          ...
          // See: https://github.com/nova-ai/nova-ai/issues/40545
          skipQueue: true,
        });
        if (
          send.status === "failed" ||
