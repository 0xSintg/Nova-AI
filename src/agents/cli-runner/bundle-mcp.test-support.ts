...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    envSnapshot = captureEnv(["NOVA_AI_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("nova-ai-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("nova-ai-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("nova-ai-cli-bundle-mcp-bundled-");
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
function createEnabledBundleProbeConfig(): Nova AIConfig {
  return {
    plugins: {
      entries: {
