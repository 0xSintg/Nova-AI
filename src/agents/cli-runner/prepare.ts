...
const prepareDeps = {
  ...
  resolveNova AIReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveNova AIReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveNova AIReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ...
          NOVA_AI_MCP_AGENT_ID: sessionAgentId ?? "",
          NOVA_AI_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          NOVA_AI_MCP_SESSION_KEY: params.sessionKey ?? "",
          NOVA_AI_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          NOVA_AI_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const loadNova AIHistoryMessages = async () => {
  ...
  const openClawReferences = await prepareDeps.resolveNova AIReferencePaths({
    workspaceDir,
  ...
  try {
    const hookResult = await resolvePromptBuildHookResult({
      ...
      messages: await loadNova AIHistoryMessages(),
      hookCtx: {
        runId: params.runId,
        agentId: sessionAgentId,
        sessionKey: params.sessionKey,
  ...
  const shouldPrepareNova AIHistoryPrompt =
    !reusableCliSession.sessionId || allowRawTranscriptReseed;
  const openClawHistoryPrompt = shouldPrepareNova AIHistoryPrompt
    ? buildCliSessionHistoryPrompt({
