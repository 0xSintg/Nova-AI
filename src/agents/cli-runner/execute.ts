...
const CLI_BACKEND_PRESERVE_ENV = "NOVA_AI_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.NOVA_AI_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.NOVA_AI_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.NOVA_AI_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.NOVA_AI_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.NOVA_AI_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.NOVA_AI_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.NOVA_AI_MCP_TOKEN ||
            env.NOVA_AI_MCP_SESSION_KEY ||
            env.NOVA_AI_MCP_SENDER_IS_OWNER
          ) {
