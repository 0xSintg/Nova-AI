...
const NOVA_AI_CLAUDE_PLUGIN_NAME = "nova-ai-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredNova AITmpDir(), "nova-ai-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, NOVA_AI_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: NOVA_AI_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Nova AI skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
