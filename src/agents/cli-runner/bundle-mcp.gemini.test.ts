...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/nova-ai-bundle-mcp-gemini",
    ...
    expect(prepared.env?.NOVA_AI_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["nova-ai"]);
    expect(raw.mcpServers?.nova-ai?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.nova-ai?.headers?.Authorization).toBe("Bearer loopback-token-123");
