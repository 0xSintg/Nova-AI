...
const LIVE = isLiveTestEnabled(["NOVA_AI_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "nova-ai-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("nova-ai_live_probe", "Nova AI Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Nova AI-configured streamable-http server", async () => {
    const geminiCommand = process.env.NOVA_AI_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("nova-aiLiveProbe");
