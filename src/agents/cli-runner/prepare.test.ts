...
function createSessionFile() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-cli-prepare-"));
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  beforeEach(() => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveNova AIReferencePaths: vi.fn(async () => ({ docsPath: null, sourcePath: null })),
    });
  ...
  it("does not allocate a non-legacy context engine before fallible CLI preparation finishes", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      resolveNova AIReferencePaths: vi.fn(async () => {
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      expect(context.preparedBackend.env).toMatchObject({
        NOVA_AI_MCP_MESSAGE_CHANNEL: "telegram",
        NOVA_AI_MCP_INBOUND_EVENT_KIND: "room_event",
      });
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
