...
import {
  buildNova AIToolFallbackText,
  shouldRenderNova AIToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__nova-ai__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__nova-ai__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__nova-ai__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Nova AI handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Do not use `message(action=send)` to deliver the current source-channel reply; reply normally so Nova AI can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.nova-ai.com",
    docsPath ? "Mirror: https://docs.nova-ai.com" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/nova-ai/nova-ai",
    docsPath
      ? "Nova AI behavior/config/architecture: read local docs first."
      : "Nova AI behavior/config/architecture: read docs mirror first.",
    ...
    "Diagnosing issues: run `nova-ai status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Nova AI process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Nova AI agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Nova AI agent ids allowed for sessions_spawn",
  ...
  const renderNova AIToolWorkflowHints = shouldRenderNova AIToolWorkflowHints({
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside Nova AI.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefixCacheKey = hashStablePromptInput({
    ...
    promptSurface,
    toolLines,
    renderNova AIToolWorkflowHints,
    hasGateway,
    readToolName,
    execToolName,
    processToolName,
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside Nova AI.",
      "",
      "## Tooling",
      ...
      "CLI lifecycle only on explicit user request: `nova-ai gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## Nova AI Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, Nova AI pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by Nova AI and included below in Project Context.",
      "",
