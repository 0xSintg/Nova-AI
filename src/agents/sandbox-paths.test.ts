...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-managed-media-"));
...
async function withOutsideHardlinkInNova AITmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside nova-ai tmp root",
      media: path.join(os.tmpdir(), "outside-nova-ai", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Nova AI tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Nova AI tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInNova AITmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Nova AI tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInNova AITmp(
