...
function buildCoreToolGroupMap() {
  ...
  const nova-aiTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInNova AIGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:nova-ai": nova-aiTools,
