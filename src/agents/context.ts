...
function isLikelyNova AICliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "nova-ai" ||
    entryBasename === "nova-ai.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Nova AI CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureNova AIModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyNova AICliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): Nova AIConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureNova AIModelsJson(cfg);
    } catch {
