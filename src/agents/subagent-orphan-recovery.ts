 ...
 *
 * @see https://github.com/nova-ai/nova-ai/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /nova-ai\.json|nova-ai gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify nova-ai.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
