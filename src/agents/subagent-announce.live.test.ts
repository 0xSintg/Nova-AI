...
import {
  createNova AITestState,
  type Nova AITestState,
} from "../test-utils/nova-ai-test-state.js";
...
const LIVE = isLiveTestEnabled() && isTruthyEnvValue(process.env.NOVA_AI_LIVE_SUBAGENT_E2E);
...
type LiveSubagentModelProviders = NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]>;

function resolveLiveSubagentModelConfig(): LiveSubagentModelConfig {
  const modelKey = process.env.NOVA_AI_LIVE_SUBAGENT_E2E_MODEL?.trim() || "openai/gpt-5.5";
...
function liveSubagentConfig(
  ...
  token: string,
  options?: {
    queue?: NonNullable<Nova AIConfig["messages"]>["queue"];
    toolAllow?: string[];
  },
): Nova AIConfig {
...
describeLive("subagent announce live", () => {
  let state: Nova AITestState | undefined;
  ...
  it(
    ...
    async () => {
      if (!isTruthyEnvValue(process.env.NOVA_AI_SUBAGENT_ISSUE_82913_REPRO)) {
        console.warn(
          "[issue-82913] skip: set NOVA_AI_SUBAGENT_ISSUE_82913_REPRO=1 to run this focused repro",
        );
        return;
      }
      ...
      state = await createNova AITestState({
        ...
        env: {
          ...
          NOVA_AI_DISABLE_PERSISTED_PLUGIN_REGISTRY: "1",
          NOVA_AI_BUNDLED_PLUGINS_DIR: path.resolve("extensions"),
          NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
          NOVA_AI_PLUGIN_CATALOG_PATHS: undefined,
          NOVA_AI_PLUGINS_PATHS: undefined,
        },
      });
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 240,
          message: [
            "Run this exact Nova AI busy-parent subagent scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
    ...
    async () => {
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 180,
          message: [
            "Run this exact Nova AI subagent steering scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
            `Step 1: call sessions_spawn with exactly this JSON input: ${JSON.stringify({
              task: childTask,
  ...
  it(
    ...
    async () => {
      ...
      if (!modelConfig.modelKey.startsWith("google/")) {
        console.warn(
          "[subagent-stress] skip: set NOVA_AI_LIVE_SUBAGENT_E2E_MODEL=google/gemini-3.1-pro-preview",
        );
        return;
      }
      requireLiveSubagentAuth(modelConfig);
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({ name: "nova-ai-gemini-stress-live", private: true }, null, 2)}\n`,
        "utf8",
      );
      await fs.writeFile(
        path.join(state.workspaceDir, "AGENTS.md"),
        "Nova AI live stress test workspace. Keep responses concise.\n",
        "utf8",
      );
      await state.writeConfig(
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 420,
          message: [
            "Run this exact Nova AI Gemini subagent stress scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
              ...
              return `Call sessions_spawn for child ${childNumber} with exactly this JSON input: ${JSON.stringify(
                {
                  task: [
                    ...
                    "Then run a tiny shell command if the bash tool is available: printf nova-ai.",
