...
const CLEANUP_STATE_KEY = Symbol.for("nova-ai.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("nova-ai.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("nova-ai.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "NOVA_AI_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "NOVA_AI_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "NOVA_AI_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.NOVA_AI_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isNova AISessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "nova-ai" || exe.endsWith("/nova-ai") || exe.endsWith("/nova-ai-gateway")) {
    return true;
  }
  if (
    normalized.some(
      (arg) =>
        arg === "nova-ai" ||
        arg.endsWith("/nova-ai") ||
        arg === "nova-ai.mjs" ||
        arg.endsWith("/nova-ai.mjs"),
    )
  ) {
    return true;
  }
  ...
  const hasNova AICommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasNova AICommandToken,
  );
}
...
function shouldTreatAsNonNova AIOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isNova AISessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-nova-ai-owner"],
    };
  }
