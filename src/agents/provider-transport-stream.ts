...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "nova-ai-openai-responses-transport",
  "openai-codex-responses": "nova-ai-openai-responses-transport",
  "openai-completions": "nova-ai-openai-completions-transport",
  "azure-openai-responses": "nova-ai-azure-openai-responses-transport",
  "anthropic-messages": "nova-ai-anthropic-messages-transport",
  "google-generative-ai": "nova-ai-google-generative-ai-transport",
};
...
function hasNova AITransportRequirement(model: Model<Api>): boolean {
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasNova AITransportRequirement(model)) {
    return undefined;
  }
...
export function createNova AITransportStreamFnForModel(
  model: Model<Api>,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Nova AI's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // native HTTP streams preserve the same Nova AI request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
