...
type CatalogSource = "nova-ai" | "mcp" | "client";
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    nova-ai: { value: controller.nova-ai, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("nova-ai.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: Nova AIConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: Nova AIConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): { source: CatalogSource; sourceName?: string } {
  ...
  if (pluginId) {
    return { source: "nova-ai", sourceName: pluginId };
  }
  return { source: "nova-ai", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with nova-ai.tools.search, nova-ai.tools.describe, and nova-ai.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The nova-ai.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through Nova AI.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
