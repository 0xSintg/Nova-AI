...
let configOverride: Nova AIConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: Nova AIConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/nova-ai/nova-ai/issues/31298
    previousFastTestEnv = process.env.NOVA_AI_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.NOVA_AI_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // NOVA_AI_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Nova AI runtime context (internal):");
