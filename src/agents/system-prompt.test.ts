...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes an Nova AI control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/nova-ai",
    });

    expect(prompt).toContain("## Nova AI Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("nova-ai gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__nova-ai__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List Nova AI agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/nova-ai/docs");
    expect(prompt).toContain("Nova AI behavior/config/architecture: read local docs first.");
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/nova-ai",
      docsPath: "/tmp/nova-ai/docs",
      sourcePath: "/tmp/nova-ai",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("Docs: /tmp/nova-ai/docs");
    expect(prompt).toContain("Source: /tmp/nova-ai");
    expect(prompt).toContain("Nova AI behavior/config/architecture: read local docs first.");
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.nova-ai.com");
    expect(prompt).toContain("Source: https://github.com/nova-ai/nova-ai");
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/nova-ai",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/nova-ai/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/nova-ai/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Nova AI Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/nova-ai. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSupplement", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/nova-ai/BOOTSTRAP.md");
  ...
  it("adds pending bootstrap guidance and BOOTSTRAP.md contents for override prompts", () => {
    ...
    expect(supplement).toContain("## /tmp/nova-ai/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`nova-ai ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Nova AI subagents");
