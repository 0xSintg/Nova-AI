...
function resolveHeartbeatConfigForAgent(params: {
  cfg: Nova AIConfig;
  agentId: string;
}): NonNullable<NonNullable<Nova AIConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Nova AI config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Nova AI config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: Nova AIConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: Nova AIConfig): string[] {
