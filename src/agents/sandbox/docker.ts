...
import { markNova AIExecEnv } from "../../infra/nova-ai-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Nova AI will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "nova-ai.sandbox=1");
  args.push("--label", `nova-ai.sessionKey=${params.scopeKey}`);
  args.push("--label", `nova-ai.createdAtMs=${createdAtMs}`);
  args.push("--label", `nova-ai.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `nova-ai.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markNova AIExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "nova-ai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`nova-ai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`nova-ai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("nova-ai sandbox recreate --all");
}
