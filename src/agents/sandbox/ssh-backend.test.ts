...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "nova-ai-test-ssh-config"),
    host: "nova-ai-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "nova-ai-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "nova-ai-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/nova-ai");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/nova-ai/nova-ai-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "nova-ai-browser",
          containerPrefix: "nova-ai-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/nova-ai/nova-ai-ssh-agent-worker");
