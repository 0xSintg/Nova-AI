...
import { resolveNova AIMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: Nova AIConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "nova-ai-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "nova-ai-bundled") {
    return {
...
function resolveAllowedSymlinkTargetRealPaths(config?: Nova AIConfig): string[] {
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    // skill directories (e.g. ~/.nova-ai/skills/coze/koze-retrieval/SKILL.md).
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values())
    ...
    .map((record) => {
      ...
      return {
        skill,
        frontmatter,
        metadata: resolveNova AIMetadata(frontmatter),
        invocation,
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`nova-ai skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`nova-ai skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
