...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseNova AIManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyNova AIManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): Nova AISkillMetadata | undefined {
