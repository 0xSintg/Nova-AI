...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NOVA_AI_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns NOVA_AI_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-bundled-override-"));
    process.env.NOVA_AI_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.NOVA_AI_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "nova-ai" }));
