    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "nova-ai.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "nova-ai.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "nova-ai.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("nova-ai-");
  const acpxRoot = await tempDirs.make("nova-ai-acpx-plugin-");
  const helperRoot = await tempDirs.make("nova-ai-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("nova-ai-");
  const pluginRoot = await tempDirs.make("nova-ai-plugin-");
  const outsideDir = await tempDirs.make("nova-ai-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("nova-ai-");
    const pluginRoot = await tempDirs.make("nova-ai-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("nova-ai-");
    const pluginRoot = await tempDirs.make("nova-ai-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
