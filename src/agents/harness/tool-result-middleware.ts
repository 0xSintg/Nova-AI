...
type MiddlewareContentBlock = Nova AIAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is Nova AIAgentToolResult {
...
function coerceMiddlewareToolResult(
...
): Nova AIAgentToolResult | undefined {
  ...
  const content: Nova AIAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: Nova AIAgentToolResult): Nova AIAgentToolResult {
...
function buildMiddlewareFailureResult(): Nova AIAgentToolResult {
  return {
    content: [
      {
        type: "text",
...
export function createAgentToolResultMiddlewareRunner(
  ...
  return {
    ...
    ): Promise<Nova AIAgentToolResult> {
