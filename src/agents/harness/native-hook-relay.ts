        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by Nova AI",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveNova AICliExecutable();
  const argv =
    executable === "nova-ai"
      ? ["nova-ai"]
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `nova-ai-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("nova-ai:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.nova-ai_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `nova-ai-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveNova AICliExecutable(): string {
  const envPath = process.env.NOVA_AI_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "nova-ai.mjs"),
  ...
  throw new Error("Cannot resolve Nova AI CLI executable path for native hook relay");
}
