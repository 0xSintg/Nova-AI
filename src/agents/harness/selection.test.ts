...
const originalRuntime = process.env.NOVA_AI_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.NOVA_AI_AGENT_RUNTIME;
  } else {
    process.env.NOVA_AI_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: Nova AIConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
function groupSenderDenyAllConfig(): Nova AIConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function groupDenyAllConfig(): Nova AIConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function providerRuntimeConfig(provider: string, runtime: string): Nova AIConfig {
  return {
    models: {
      providers: {
        [provider]: {
...
describe("selectAgentHarness", () => {
  ...
  it("ignores env-forced PI for OpenAI default runtime selection", () => {
    process.env.NOVA_AI_AGENT_RUNTIME = "pi";
    registerFailingCodexHarness();
