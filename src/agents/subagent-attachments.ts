...
function resolveAttachmentLimits(config: Nova AIConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".nova-ai", "attachments");
  const relDir = path.posix.join(".nova-ai", "attachments", attachmentId);
