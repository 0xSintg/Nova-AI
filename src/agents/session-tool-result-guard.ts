...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000NOVA_AI_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[Nova AI persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[Nova AI persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[Nova AI persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[Nova AI persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[Nova AI persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyNova AIAssistantMessage(message: AgentMessage): boolean {
  ...
  return provider === "nova-ai" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyNova AIAssistantMessage(nextMessage);
    if (
