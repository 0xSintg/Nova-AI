...
describe("provider transport stream contracts", () => {
    ...
    const cases = [
      {
        ...
        alias: "nova-ai-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "nova-ai-openai-completions-transport",
      },
      {
        ...
        alias: "nova-ai-azure-openai-responses-transport",
      },
      {
        ...
        alias: "nova-ai-anthropic-messages-transport",
      },
      {
        ...
        alias: "nova-ai-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Nova AI transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createNova AITransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Nova AI simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("nova-ai-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Nova AI transport until PI preserves attribution", () => {
