...
import { resolveOptionalMediaToolFactoryPlan } from "./nova-ai-tools.media-factory-plan.js";
...
type CreateNova AIToolsOptions = Parameters<
  typeof import("./nova-ai-tools.js").createNova AITools
>[0];

async function createNova AIToolsForTest(options?: CreateNova AIToolsOptions) {
  const { createNova AITools } = await import("./nova-ai-tools.js");
  return createNova AITools(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/nova-ai.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): Nova AIConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("preserves implicit allow-all from alsoAllow-only policies for built-in media factories", async () => {
    ...
    const toolNames = (
      await createNova AIToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: Nova AIConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createNova AIToolsForTest({
      config,
      agentDir: "/tmp/nova-ai-agent-main",
