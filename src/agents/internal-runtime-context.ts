export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_NOVA_AI_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_NOVA_AI_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[NOVA_AI_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[NOVA_AI_INTERNAL_CONTEXT_END]]";

export const NOVA_AI_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
export const NOVA_AI_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "Nova AI runtime context for the immediately preceding user message.";
export const NOVA_AI_RUNTIME_EVENT_HEADER = "Nova AI runtime event.";
export const NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE = "nova-ai.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["Nova AI runtime context (internal):", NOVA_AI_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === NOVA_AI_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === NOVA_AI_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === NOVA_AI_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${NOVA_AI_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${NOVA_AI_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${NOVA_AI_RUNTIME_EVENT_HEADER}\n${NOVA_AI_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isNova AIRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isNova AIRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isNova AIRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  return messages.filter(
    (message, index) => !isNova AIRuntimeContextCustomMessage(message) || index > lastUserIndex,
  );
}
