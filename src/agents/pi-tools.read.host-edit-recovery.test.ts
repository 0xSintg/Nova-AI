...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-edit-recovery-"));
  ...
  it("recovers post-write failures when edit calls use file_path", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-edit-recovery-"));
    const workspaceDir = path.join(tmpDir, ".nova-ai", "workspace");
  ...
  it("recovers tilde paths against the OS home even when NOVA_AI_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const nova-aiHome = path.join(tmpDir, "nova-ai-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(nova-aiHome, { recursive: true });

    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousOpenclawHome = process.env.NOVA_AI_HOME;
    process.env.HOME = osHome;
    process.env.USERPROFILE = osHome;
    process.env.NOVA_AI_HOME = nova-aiHome;
    ...
    try {
      ...
      await expectPathMissing(path.join(nova-aiHome, "demo.txt"));
    } finally {
      ...
      if (previousOpenclawHome === undefined) {
        ...
        process.env.NOVA_AI_HOME = previousOpenclawHome;
      }
    }
  });
