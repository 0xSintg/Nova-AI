...
function resolveExecConfig(params: { cfg?: Nova AIConfig; agentId?: string }) {
...
export type Nova AICodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeNova AITools: boolean;
  includePluginTools: boolean;
};

export function createNova AICodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep Nova AI web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: Nova AICodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeNova AITools: includeCoreTools,
  ...
  const includeNova AITools = includeCoreTools && toolConstructionPlan.includeNova AITools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createNova AIReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includeNova AITools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("nova-ai-tools");
