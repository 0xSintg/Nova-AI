...
import "./test-helpers/fast-nova-ai-tools.js";
import * as openClawPluginTools from "./nova-ai-plugin-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createNova AICodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createNova AICodingTools>) {
...
type Nova AICodingTool = ReturnType<typeof createNova AICodingTools>[number];
type Nova AIToolsOptions = NonNullable<Parameters<typeof createNova AITools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: Nova AICodingTool[], name: string): Nova AICodingTool {
...
function requireToolExecute(tool: Nova AICodingTool): NonNullable<Nova AICodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateNova AIToolsOptions(): Nova AIToolsOptions {
  const calls = vi.mocked(createNova AITools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createNova AITools call");
  }
  return options;
}
...
describe("createNova AICodingTools", () => {
  const testConfig: Nova AIConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createNova AICodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-hook-channel-"));
  ...
  it("keeps PI Tool Search controls when core Nova AI tools are not materialized", () => {
    const createNova AIToolsMock = vi.mocked(createNova AITools);
    createNova AIToolsMock.mockClear();
    ...
    expect(createNova AIToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateNova AIToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:nova-ai"], ["*"]]) {
      const tools = createNova AICodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to Nova AI tool construction", () => {
    const createNova AIToolsMock = vi.mocked(createNova AITools);
    createNova AIToolsMock.mockClear();
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNova AIToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into Nova AI tool construction plans", () => {
    const createNova AIToolsMock = vi.mocked(createNova AITools);
    createNova AIToolsMock.mockClear();
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNova AIToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the Nova AI core factory", () => {
    const createNova AIToolsMock = vi.mocked(createNova AITools);
    createNova AIToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(openClawPluginTools, "resolveNova AIPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNova AIToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to Nova AI tool factory planning", () => {
    const createNova AIToolsMock = vi.mocked(createNova AITools);
    createNova AIToolsMock.mockClear();
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNova AIToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to Nova AI plugin discovery", async () => {
    const createNova AIToolsMock = vi.mocked(createNova AITools);
    createNova AIToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `nova-ai-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNova AIToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createNova AIToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateNova AIToolsOptions().inheritedToolAllowlist;
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("nova-ai-tools:test-helper")).toBeLessThan(
      stages.indexOf("nova-ai-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createNova AICodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createNova AICodingTools({
      config: testConfig,
    ...
    const codexTools = createNova AICodingTools({
      config: testConfig,
    ...
    const disabledConfig: Nova AIConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createNova AICodingTools({
    ...
    const anthropicTools = createNova AICodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: Nova AIConfig = {
      tools: {
        exec: {
    ...
    const allowed = createNova AICodingTools({
    ...
    const denied = createNova AICodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createNova AICodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createNova AICodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNova AICodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createNova AICodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createNova AICodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createNova AICodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createNova AICodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createNova AICodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createNova AICodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createNova AICodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createNova AICodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createNova AICodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-read-"));
    try {
      ...
      const contents = "Hello from nova-ai read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "nova-ai-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "nova-ai-workspace"),
    ...
    const tools = createNova AICodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-canonical-"));
    try {
      const tools = createNova AICodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-legacy-alias-"));
    try {
      const tools = createNova AICodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-structured-write-"));
    try {
      const tools = createNova AICodingTools({ workspaceDir: tmpDir });
      ...
      await expect(
        writeExecute("tool-structured-write", {
          ...
          content: [
            ...
            { type: "input_text", text: "const root = path.join(process.env.HOME, 'nova');\n" },
          ],
        }),
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-structured-edit-"));
    try {
