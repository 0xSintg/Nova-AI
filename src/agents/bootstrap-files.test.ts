...
async function createHeartbeatAgentsWorkspace() {
  const workspaceDir = await makeTempWorkspace("nova-ai-bootstrap-");
...
describe("resolveBootstrapFilesForRun", () => {
  ...
  it("keeps BOOTSTRAP.md when setup state cannot be read", async () => {
    const workspaceDir = await makeTempWorkspace("nova-ai-bootstrap-");
    await fs.mkdir(path.join(workspaceDir, ".nova-ai", "workspace-state.json"), {
      recursive: true,
    });
    await fs.writeFile(path.join(workspaceDir, "AGENTS.md"), "rules", "utf8");
  ...
  it("ignores stale root BOOTSTRAP.md for home-relative workspace paths", async () => {
    registerBootstrapFileHook();
    const parentDir = await makeTempWorkspace("nova-ai-bootstrap-home-");
    ...
    process.env.NOVA_AI_HOME = parentDir;
    try {
...
describe("hasCompletedBootstrapTurn", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(await fs.realpath("/tmp"), "nova-ai-bootstrap-turn-"));
  });
