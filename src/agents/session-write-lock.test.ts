...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            NOVA_AI_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            NOVA_AI_SESSION_WRITE_LOCK_STALE_MS: "60000",
            NOVA_AI_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/nova-ai/nova-ai.mjs", "doctor"],
      });
      ...
      const envOverride = await cleanStaleLockFiles({
        sessionsDir,
        config: { session: { writeLock: { staleMs: 30_000 } } },
        env: { NOVA_AI_SESSION_WRITE_LOCK_STALE_MS: "60000" },
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/nova-ai/nova-ai.mjs", "doctor"],
      });
  ...
  it("cleans stale .jsonl lock files in sessions directories", async () => {
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/nova-ai/nova-ai.mjs", "agent"],
      });
  ...
  it("cleans fresh live .jsonl lock files owned by a non-Nova AI process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-nova-ai-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-Nova AI entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with Nova AI or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const nova-aiLock = path.join(sessionsDir, "nova-ai-live.jsonl.lock");
    ...
    try {
      ...
      const nova-aiResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(nova-aiResult.cleaned).toEqual([]);
      await expect(fs.access(nova-aiLock)).resolves.toBeUndefined();

      await fs.rm(nova-aiLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
