...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "nova-ai" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "nova-ai") {
    return false;
  }
  offset += 1;
...
function formatDiagnosticsExportSuccess(aggregated: string): string {
  ...
  try {
    ...
    if (typeof manifest.nova-aiVersion === "string") {
      lines.push(`Nova AI version: ${manifest.nova-aiVersion}`);
    }
