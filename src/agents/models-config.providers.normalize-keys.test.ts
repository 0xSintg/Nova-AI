...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-agent-"));
    try {
      const providers: NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("normalizes retired Google Gemini model ids before emitting provider config", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]> = {
        ...
        "google-gemini-cli": {
          baseUrl: "nova-ai://google-gemini-cli",
          models: [
            createModel({
              id: "gemini-3-pro-preview",
  ...
  it("replaces resolved env var value with env var name to prevent plaintext persistence", async () => {
    ...
    const env = {
      ...
      NOVA_AI_SKIP_PROVIDERS: undefined,
      NOVA_AI_TEST_MINIMAL_GATEWAY: undefined,
    };
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
