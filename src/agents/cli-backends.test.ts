...
function createClaudeCliOverrideConfig(config: CliBackendConfig): Nova AIConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Nova AI's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__nova-ai__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__nova-ai__*");
  ...
  it("keeps Claude permission mode unset when Nova AI exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
