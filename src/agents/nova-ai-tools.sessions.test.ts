...
import "./test-helpers/fast-nova-ai-tools-sessions.js";
...
function createNova AITools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createNova AITools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createNova AITools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.nova-aiReasoningReplay).toBeUndefined();
  });
