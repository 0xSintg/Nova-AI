...
export async function markRestartAbortedMainSessions(params: {
  cfg?: Nova AIConfig;
  additionalCfgs?: Iterable<Nova AIConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, NOVA_AI_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is Nova AIConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "Nova AI was waiting on tool/model work. Continue from the existing " +
...
async function resolveRestartRecoveryStorePaths(params: {
  ...
  if (params.cfg) {
    const env = { ...process.env, NOVA_AI_STATE_DIR: stateDir };
