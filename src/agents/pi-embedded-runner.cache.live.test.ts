...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.NOVA_AI_CACHE_TRACE,
      file: process.env.NOVA_AI_CACHE_TRACE_FILE,
      messages: process.env.NOVA_AI_CACHE_TRACE_MESSAGES,
      prompt: process.env.NOVA_AI_CACHE_TRACE_PROMPT,
      system: process.env.NOVA_AI_CACHE_TRACE_SYSTEM,
    };
    process.env.NOVA_AI_CACHE_TRACE = "1";
    process.env.NOVA_AI_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.NOVA_AI_CACHE_TRACE_MESSAGES = "0";
    process.env.NOVA_AI_CACHE_TRACE_PROMPT = "0";
    process.env.NOVA_AI_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("NOVA_AI_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("NOVA_AI_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("NOVA_AI_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("NOVA_AI_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("NOVA_AI_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "NOVA_AI_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "NOVA_AI_LIVE_ANTHROPIC_CACHE_MODEL",
