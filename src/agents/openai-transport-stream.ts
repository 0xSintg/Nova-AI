...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__nova-ai_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "nova-aiReasoningReplay";
...
type BaseStreamOptions = {
  ...
  nova-aiCodeModeToolSurface?: boolean;
...
function createResponsesFirstEventTimeoutError(model: Model<Api>, timeoutMs: number): Error {
  return new Error(
    `Azure OpenAI Responses stream did not deliver a first event within ${timeoutMs}ms after HTTP streaming headers. ` +
      ...
      "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable NOVA_AI_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  );
}
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { nova-aiCodeModeToolSurface?: unknown } | undefined)
            ?.nova-aiCodeModeToolSurface === true
        ) {
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as Nova AI thinking blocks so channel/UI
