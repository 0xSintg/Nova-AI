...
describe("CLI attempt execution", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-cli-attempt-"));
  ...
  function makeClaudeCliSessionEntry(
    nova-aiSessionId: string,
    cliSessionId: string,
  ): SessionEntry {
    return {
      sessionId: nova-aiSessionId,
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
  ...
  it("passes session-bound OpenAI Codex auth profile to codex-cli aliases", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-codex",
      updatedAt: Date.now(),
  ...
  it("embedded assistant gap-fill skips trailing nova-ai.cache-ttl custom entries (regression for #83427)", async () => {
  ...
  it("embedded assistant gap-fill appends repeated replies after a user tail", async () => {
    ...
    expect(
      sessionFile.endsWith(
        path.join(".nova-ai", "agents", "main", "sessions", `${sessionEntry.sessionId}.jsonl`),
      ),
    ).toBe(true);
  ...
  it("forwards separate user trigger, channel, and provider context to CLI runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-channel",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards runtime toolsAllow into CLI attempts so the CLI harness can fail closed", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-cli-tools-allow",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes canonical Anthropic models through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-canonical-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes provider-qualified Anthropic shorthand through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-shorthand-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes canonical OpenAI models through the configured embedded Codex runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-canonical-codex-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards user-pinned OpenAI API-key backup profiles to Codex harness runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-openai-codex-api-key",
      updatedAt: Date.now(),
  ...
  it("keeps one-shot model runs on the raw embedded provider path", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-model-run-raw",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards trusted elevated defaults to embedded agent runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-elevated-followup",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    const bashElevated = {
  ...
  it("forwards one-shot CLI cleanup to CLI providers", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nova-ai-session-cleanup-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
...
describe("embedded attempt harness pinning", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-embedded-attempt-"));
    runCliAgentMock.mockReset();
    runEmbeddedPiAgentMock.mockReset();
  });
