...
describe("provider attribution", () => {
  it("resolves the canonical Nova AI product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      NOVA_AI_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Nova AI runtime wrapper.",
      product: "Nova AI",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a documented NVIDIA attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "X-BILLING-INVOKE-ORIGIN": "Nova AI",
      },
    });
    expect(resolveProviderAttributionHeaders("NVIDIA", { NOVA_AI_VERSION: "2026.3.22" })).toEqual({
      "X-BILLING-INVOKE-ORIGIN": "Nova AI",
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { NOVA_AI_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { NOVA_AI_VERSION: "2026.3.22" })).toEqual({
      originator: "nova-ai",
      version: "2026.3.22",
      "User-Agent": "nova-ai/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { NOVA_AI_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { NOVA_AI_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
      reviewNote:
        "xAI api.x.ai accepts a standard nova-ai User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
      product: "Nova AI",
      version: "2026.3.22",
      headers: {
        originator: "nova-ai",
    ...
    expect(resolveProviderAttributionHeaders("xai", { NOVA_AI_VERSION: "2026.3.22" })).toEqual({
      originator: "nova-ai",
      version: "2026.3.22",
      "User-Agent": "nova-ai/2026.3.22",
    });
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ NOVA_AI_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
