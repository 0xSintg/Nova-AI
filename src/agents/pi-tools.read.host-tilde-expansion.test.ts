...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("nova-ai-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("nova-ai-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("nova-ai-tilde-test-mkdir-");
  ...
  it("ignores NOVA_AI_HOME for write operations", async () => {
    const nova-aiHome = await createTempDir("nova-ai-home-override-", os.tmpdir());
    const dir = await createTempDir("nova-ai-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("NOVA_AI_HOME", nova-aiHome);

    createHostWorkspaceWriteTool(nova-aiHome, { workspaceOnly: false });
    ...
    await expectMissingPath(fs.access(path.join(nova-aiHome, path.basename(testFile))));
  });

  it("ignores NOVA_AI_HOME for mkdir operations", async () => {
    const nova-aiHome = await createTempDir("nova-ai-home-override-", os.tmpdir());
    const dir = await createTempDir("nova-ai-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expectMissingPath(fs.access(path.join(nova-aiHome, path.basename(newDir))));
  });

  it("ignores NOVA_AI_HOME for readFile operations", async () => {
    const nova-aiHome = await createTempDir("nova-ai-home-override-", os.tmpdir());
    const dir = await createTempDir("nova-ai-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(nova-aiHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores NOVA_AI_HOME for access operations", async () => {
    const nova-aiHome = await createTempDir("nova-ai-home-override-", os.tmpdir());
    const dir = await createTempDir("nova-ai-tilde-test-edit-");
