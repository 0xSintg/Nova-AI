...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    nova-ai: process.env.NOVA_AI_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.nova-ai === undefined) {
      delete process.env.NOVA_AI_GATEWAY_TOKEN;
    } else {
      process.env.NOVA_AI_GATEWAY_TOKEN = envSnapshot.nova-ai;
    }
  });
  ...
  it("uses NOVA_AI_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.NOVA_AI_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
