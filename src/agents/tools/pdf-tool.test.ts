...
function withPdfModel(primary: string): Nova AIConfig {
  return {
    agents: { defaults: { pdfModel: { primary } } },
  } as Nova AIConfig;
}

function withDefaultModel(primary: string): Nova AIConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureNova AIModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
        const tool = requirePdfTool(
  ...
  it("uses native PDF path without eager extraction", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      ...
      const ensureModelsJsonMock = vi.mocked(modelsConfig.ensureNova AIModelsJson);
