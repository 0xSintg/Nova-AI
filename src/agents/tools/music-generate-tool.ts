...
function hasExplicitMusicGenerationModelConfig(cfg?: Nova AIConfig): boolean {
...
export function createMusicGenerateTool(options?: {
  ...
  const cfg: Nova AIConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
