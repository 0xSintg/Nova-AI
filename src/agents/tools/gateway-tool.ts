...
import { isNova AIOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `nova-ai security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as Nova AIConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as Nova AIConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isNova AIOwnerOnlyCoreToolName("gateway"),
    description:
