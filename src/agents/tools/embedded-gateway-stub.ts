...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: Nova AIConfig }) => string;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: Nova AIConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: Nova AIConfig) => {
    storePath: string;
    store: unknown;
  };
