...
export function describeNova AIGenerationToolRegistration(params: {
  suiteName: string;
  toolName: string;
  toolLabel: string;
}) {
  describe(params.suiteName, () => {
    it(`registers ${params.toolName} when ${params.toolLabel} is present`, () => {
      ...
      expect(collectPresentNova AITools([tool])).toEqual([tool]);
    });

    it(`omits ${params.toolName} when ${params.toolLabel} is absent`, () => {
      expect(collectPresentNova AITools([null]).map((tool) => tool.name)).not.toContain(
        params.toolName,
      );
    });
  });
