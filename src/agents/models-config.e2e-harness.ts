...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousNova AIAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousNova AIAgentDir = process.env.NOVA_AI_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousNova AIAgentDir === undefined) {
      delete process.env.NOVA_AI_AGENT_DIR;
    } else {
      process.env.NOVA_AI_AGENT_DIR = previousNova AIAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "NOVA_AI_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: Nova AIConfig = {
  models: {
    providers: {
      "custom-proxy": {
