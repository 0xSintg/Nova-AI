...
describe("processGatewayAllowlist", () => {
  ...
  it("requires approval for security audit suppression edits unless yolo mode is active", async () => {
    ...
    const result = await runGatewayAllowlist({
      command: "nova-ai config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for read-only suppression inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      allowlistSatisfied: true,
      segments: [
        { resolution: null, argv: ["nova-ai", "config", "get", "security.audit.suppressions"] },
      ],
      segmentAllowlistEntries: [],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    await runGatewayAllowlist({
      command: "nova-ai config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for profile-scoped read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["nova-ai", "--profile", "rescue", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    await runGatewayAllowlist({
      command: "nova-ai --profile rescue config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a mutating segment follows read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        ...
        {
          resolution: null,
          argv: ["nova-ai", "config", "set", "security.audit.suppressions", "[]"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command:
        "nova-ai config get security.audit.suppressions; nova-ai config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a heredoc patch follows read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["nova-ai", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    analyzeShellCommandMock.mockReturnValueOnce({
      ...
      segments: [
        {
          raw: "nova-ai config get security.audit.suppressions",
          resolution: null,
          argv: ["nova-ai", "config", "get", "security.audit.suppressions"],
        },
        {
          raw: "nova-ai config patch --stdin <<'EOF'",
          resolution: null,
          argv: ["nova-ai", "config", "patch", "--stdin"],
        },
      ],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    const result = await runGatewayAllowlist({
      command: `nova-ai config get security.audit.suppressions; nova-ai config patch --stdin <<'EOF'
  ...
  it("formats diagnostics approvals as direct pasteable followups", async () => {
    ...
    const outcome = {
      ...
      aggregated: JSON.stringify({
        path: "/tmp/nova-ai-diagnostics.zip",
        bytes: 1234,
        manifest: {
          generatedAt: "2026-04-28T20:58:29.311Z",
          nova-aiVersion: "2026.4.27",
          contents: [
    ...
    const approvalFollowup = vi.fn<ExecApprovalFollowupFactory>(async () =>
      [
        ...
        "Session 1",
        "Channel: telegram",
        "Nova AI session id: `session-1`",
        "Codex thread id: `thread-1`",
      ].join("\n"),
    );

    await runGatewayAllowlist({
      command: "nova-ai gateway diagnostics export --json",
      trigger: "diagnostics",
    ...
    expect(followupText).toContain("Path: /tmp/nova-ai-diagnostics.zip");
