...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    ensureNova AIModelsJson: vi.fn(async () => {}),
...
vi.mock("./models-config.runtime.js", () => ({
  ensureNova AIModelsJson: contextTestState.ensureNova AIModelsJson,
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureNova AIModelsJson.mockClear();
  return { ensureNova AIModelsJson: contextTestState.ensureNova AIModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real nova-ai startup commands that need model metadata", async () => {
    const { shouldEagerWarmContextWindowCache } = await importContextModule();

    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "chat"])).toBe(true);
    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "chat", "--help"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "nova-ai", "matrix", "encryption", "help"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "help", "matrix"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "nova-ai", "browser", "status", "--help"]),
    ).toBe(false);
    expect(
    ...
    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "logs", "--limit", "5"])).toBe(
      false,
    );
    expect(
      shouldEagerWarmContextWindowCache(["node", "nova-ai", "memory", "search", "--json"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "message", "read"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "status", "--json"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "nova-ai", "sessions", "--json"])).toBe(
      false,
    );
    expect(
  ...
  it("skips model normalization during warmup but preserves provider-owned context metadata", async () => {
    ...
    expect(
      path.normalize(discoverAgentDir).endsWith(path.join(".nova-ai", "agents", "main", "agent")),
    ).toBe(true);
