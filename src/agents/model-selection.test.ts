...
function resolveAnthropicOpusThinking(cfg: Nova AIConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: Nova AIConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<Nova AIConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as Nova AIConfig,
...
describe("model-selection", () => {
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<Nova AIConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("nova-ai-model-selection-test");
      try {
        const cfg: Partial<Nova AIConfig> = {
          agents: {
            defaults: {
