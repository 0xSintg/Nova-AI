...
describe("createCodexNativeWebSearchWrapper", () => {
  ...
  it("does not enable code-mode transport enforcement when config is on but controls are inactive", () => {
    ...
    expect(observedOptions[0]?.nova-aiCodeModeToolSurface).toBeUndefined();
  ...
  it("enforces the code-mode transport surface when the run enables it at agent scope", () => {
    ...
    expect(observedOptions[0]?.nova-aiCodeModeToolSurface).toBe(true);
    expect(payloads[0]?.tools).toEqual([
      { type: "function", name: "exec" },
      { type: "function", name: "wait" },
    ]);
...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the Nova AI transport so attribution survives PI defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("nova-ai");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^nova-ai\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("nova-ai");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^nova-ai\//);
  });
});
