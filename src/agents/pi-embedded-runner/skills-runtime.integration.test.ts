...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("nova-ai-bundled-");
  const workspaceDir = await createTempDir("nova-ai-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: Nova AIConfig) {
