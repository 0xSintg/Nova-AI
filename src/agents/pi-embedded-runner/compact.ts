...
import { createNova AICodingTools, resolveProcessToolScopeKey } from "../pi-tools.js";
...
async function compactEmbeddedPiSessionDirectOnce(
  ...
  await ensureNova AIModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const toolsRaw = createNova AICodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createNova AICodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Nova AI-managed registrations so custom tools survive startup.
