...
import { resolveNova AIReferencePaths } from "../../docs-path.js";
  ...
  try {
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-nova-ai-coding-tools");
    ...
    const openClawReferences = await resolveNova AIReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Nova AI
      // compaction overrides applied in createPreparedEmbeddedPiSettingsManager — same
      // rehydration also restores Pi's auto-compaction (nova-ai#75799), so re-apply
      // both guards.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Nova AI-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Nova AI agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        await sessionLockController.withSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/nova-ai/nova-ai/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("nova-ai:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/nova-ai/nova-ai/issues/8643
