...
import type { Nova AICodingToolConstructionPlan } from "../../pi-tools.js";
...
// Names here must be emitted directly by createNova AITools(). Catalog entries
...
const NOVA_AI_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "canvas",
  "cron",
  "gateway",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: Nova AICodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: Nova AICodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: Nova AICodingToolConstructionPlan,
): Nova AICodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  return (
    ...
    NOVA_AI_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeNova AITools = normalized.some((name) => NOVA_AI_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: Nova AICodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeNova AITools;
