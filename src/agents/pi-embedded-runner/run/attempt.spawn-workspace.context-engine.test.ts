...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  it("enables Tool Search controls for embedded PI runs when configured", async () => {
    ...
    expect(hoisted.createNova AICodingToolsMock).toHaveBeenCalledTimes(1);
    const options = mockParams(
      hoisted.createNova AICodingToolsMock,
      0,
      "createNova AICodingTools options",
    );
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.nova-aiCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and queues runtime context as hidden custom context", async () => {
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "nova-ai.runtime-context",
    ...
    expect(JSON.stringify(seen.messages)).not.toContain(
      "Nova AI runtime context for the immediately preceding user message.",
    );
    ...
    expect(seen.systemPrompt).not.toContain("NOVA_AI_INTERNAL_CONTEXT");
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("NOVA_AI_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.nova-ai/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.nova-ai/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.nova-ai/skills/smaug",
              source: "nova-ai-workspace",
              sourceInfo: {
                path: "/Users/alice/.nova-ai/skills/smaug/SKILL.md",
                source: "nova-ai-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.nova-ai/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/nova-ai-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/nova-ai-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context when system prompt override is configured", async () => {
    ...
    hoisted.createNova AICodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/nova-ai-override-workspace/BOOTSTRAP.md",
    ...
    expect(systemPrompt).toContain("## /tmp/nova-ai-override-workspace/BOOTSTRAP.md");
    expect(systemPrompt).toContain("Ask who I am.");
  });

  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/nova-ai-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("NOVA_AI_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the Nova AI runtime event.");
    expect(result.finalPromptText).toBe("Continue the Nova AI runtime event.");
    expect(
    ...
    const trajectoryEvents = (
      ...
      .map((line) => JSON.parse(line) as TrajectoryEvent);
    const contextCompiled = trajectoryEvents.find((event) => event.type === "context.compiled");
    expect(contextCompiled?.data?.prompt).toBe("Continue the Nova AI runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the Nova AI runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    expect(seenPrompt).toContain("[Nova AI room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[Nova AI room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the Nova AI runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
    ...
    expect(contextCompiled?.data?.prompt).toContain("[Nova AI room event]");
  });
