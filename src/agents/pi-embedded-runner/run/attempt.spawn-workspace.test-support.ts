...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createNova AICodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createNova AICodingToolsMock = vi.fn(() => []);
...
vi.mock("../../docs-path.js", () => ({
  resolveNova AIReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../pi-tools.js", () => ({
  createNova AICodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createNova AICodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("nova-ai.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "nova-ai.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createNova AICodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.NOVA_AI_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.NOVA_AI_TRAJECTORY = "0";
  }
  try {
    return await (
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.NOVA_AI_TRAJECTORY;
    } else {
      process.env.NOVA_AI_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
