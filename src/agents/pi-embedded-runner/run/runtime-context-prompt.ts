import {
  ...
  NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE,
  NOVA_AI_RUNTIME_CONTEXT_NOTICE,
  NOVA_AI_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentInboundPromptContext } from "./params.js";
export { NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE };

const NOVA_AI_RUNTIME_EVENT_USER_PROMPT = "Continue the Nova AI runtime event.";
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt) {
    return runtimeContext
      ? {
          prompt: NOVA_AI_RUNTIME_EVENT_USER_PROMPT,
          runtimeContext,
          runtimeOnly: true,
...
export async function queueRuntimeContextForNextTurn(params: {
  ...
  await params.session.sendCustomMessage(
    {
      customType: NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE,
      content: runtimeContext,
      display: false,
      details: { source: "nova-ai-runtime-context" },
    },
    { deliverAs: "nextTurn" },
  );
}
