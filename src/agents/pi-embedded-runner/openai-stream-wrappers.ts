...
type Nova AISimpleStreamOptions = SimpleStreamOptions & {
...
function isCodeModeEnabled(config?: Nova AIConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  baseStreamFn: StreamFn | undefined,
  params: { config?: Nova AIConfig; agentDir?: string; codeModeToolSurfaceEnabled?: boolean },
): StreamFn {
  const underlying = baseStreamFn ?? streamSimple;
  return (model, context, options) => {
    if (
      ...
      const originalOnPayload = options?.onPayload;
      const codeModeOptions: Nova AISimpleStreamOptions = {
        ...options,
