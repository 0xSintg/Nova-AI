 ...
 * See: https://github.com/nova-ai/nova-ai/issues/60552
 */
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      const pluginHarnessNeedsNova AIAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_CODEX_PROVIDER_ID &&
        effectiveModel.api === "openai-codex-responses";
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedsNova AIAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedsNova AIAuthBootstrap
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedsNova AIAuthBootstrap) {
      ...
      try {
        ...
        while (true) {
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // Codex builds Nova AI tools inside its harness. Keep transport
