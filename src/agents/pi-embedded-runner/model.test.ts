...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/nova-ai.plugin.json and src/plugins/manifest-model-suppression.ts.
...
describe("resolveModel", () => {
  ...
  it("invalidates PI discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-model-cache-"));
  ...
  it("invalidates PI discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-model-cache-state-"));
    vi.stubEnv("NOVA_AI_STATE_DIR", rootDir);
