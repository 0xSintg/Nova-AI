...
// `provider:"nova-ai"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_NOVA_AI_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyOpenclawAssistant(message: AgentMessage): boolean {
  ...
  return (
    provider === "nova-ai" &&
    typeof model === "string" &&
    TRANSCRIPT_ONLY_NOVA_AI_MODELS.has(model)
  );
}
