...
import { resolveNova AIPluginToolsForOptions } from "./nova-ai-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./nova-ai-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./nova-ai-tools.nodes-workspace-guard.js";
import {
  collectPresentNova AITools,
  isUpdatePlanToolEnabledForNova AITools,
} from "./nova-ai-tools.registration.js";
import {
  type HookContext,
...
type Nova AIToolsDeps = {
  callGateway: typeof callGateway;
  config?: Nova AIConfig;
};

const defaultNova AIToolsDeps: Nova AIToolsDeps = {
  callGateway,
};

let openClawToolsDeps: Nova AIToolsDeps = defaultNova AIToolsDeps;

export function createNova AITools(
  options?: {
  ...
  options?.recordToolPrepStage?.("nova-ai-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("nova-ai-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("nova-ai-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("nova-ai-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("nova-ai-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("nova-ai-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("nova-ai-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("nova-ai-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("nova-ai-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("nova-ai-tools:nodes-tool");
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentNova AITools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentNova AITools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...collectPresentNova AITools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("nova-ai-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveNova AIPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("nova-ai-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("nova-ai-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<Nova AIToolsDeps>) {
    openClawToolsDeps = overrides
      ? {
          ...defaultNova AIToolsDeps,
          ...overrides,
        }
      : defaultNova AIToolsDeps;
  },
