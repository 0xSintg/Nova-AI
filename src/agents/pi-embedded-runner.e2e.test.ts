...
const ensureNova AIModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureNova AIModelsJson: (...args: Parameters<typeof ensureNova AIModelsJsonMock>) =>
        ensureNova AIModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("nova-ai-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureNova AIModelsJsonMock.mockReset();
  ensureNova AIModelsJsonMock.mockResolvedValue({ wrote: false });
