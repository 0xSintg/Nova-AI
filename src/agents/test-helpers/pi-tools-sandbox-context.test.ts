...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("nova-ai-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("nova-ai-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("nova-ai-sbx-");
