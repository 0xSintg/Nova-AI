...
const GOOGLE_LIVE_SINGLE_KEY = "NOVA_AI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "NOVA_AI_LIVE_ANTHROPIC_KEY",
    listVar: "NOVA_AI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "NOVA_AI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `NOVA_AI_LIVE_${base}_KEY`;
