...
function pushConfiguredModelRuntimeIds(config: Nova AIConfig, runtimes: Set<string>): void {
...
function pushLegacyAgentRuntimeIds(config: Nova AIConfig, runtimes: Set<string>): void {
...
export function collectConfiguredAgentHarnessRuntimes(
  ...
  if (includeEnvRuntime) {
    const envRuntime = normalizeRuntimeId(env.NOVA_AI_AGENT_RUNTIME);
