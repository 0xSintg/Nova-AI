import { listNova AIPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
import {
...
const NOVA_AI_ATTRIBUTION_PRODUCT = "Nova AI";
const NOVA_AI_ATTRIBUTION_ORIGINATOR = "nova-ai";
...
function formatNova AIUserAgent(version: string): string {
  return `${NOVA_AI_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listNova AIPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: NOVA_AI_ATTRIBUTION_PRODUCT,
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": NOVA_AI_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: NOVA_AI_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatNova AIUserAgent(identity.version),
    },
  };
}
