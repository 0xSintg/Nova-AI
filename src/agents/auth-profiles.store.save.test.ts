...
describe("saveAuthProfileStore", () => {
  it("strips plaintext when keyRef/tokenRef are present", async () => {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-"));
    try {
  ...
  it("preserves legacy oauthRef only as doctor migration metadata during saves", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-oauth-ref-"));
    const authPath = resolveAuthStorePath(agentDir);
    const oauthRef = {
      source: "nova-ai-credentials",
  ...
  it("keeps rehydrated legacy oauthRef sidecar tokens runtime-only during ordinary saves", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-oauth-ref-"));
    const authPath = resolveAuthStorePath(agentDir);
    const previousOAuthDir = process.env.NOVA_AI_OAUTH_DIR;
    const previousSecretKey = process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY;
    process.env.NOVA_AI_OAUTH_DIR = path.join(agentDir, "credentials");
    process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY = "legacy-seed";
    const oauthRef = {
      source: "nova-ai-credentials" as const,
    ...
    try {
      ...
      delete process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY;
      ...
      if (previousOAuthDir === undefined) {
        ...
        process.env.NOVA_AI_OAUTH_DIR = previousOAuthDir;
      }
      if (previousSecretKey === undefined) {
        delete process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY;
      } else {
        process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY = previousSecretKey;
      }
      clearRuntimeAuthProfileStoreSnapshots();
      await fs.rm(agentDir, { recursive: true, force: true });
    }
  ...
  it("refreshes the runtime snapshot when a saved store rotates oauth tokens", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-runtime-"));
    try {
      replaceRuntimeAuthProfileStoreSnapshots([
        {
          agentDir,
  ...
  it("writes runtime scheduling state to auth-state.json only", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-state-"));
    try {
      const store: AuthProfileStore = {
        version: 1,
        profiles: {
  ...
  it("does not persist unchanged inherited main OAuth when saving secondary local updates", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-inherited-"));
  ...
  it("does not persist stale inherited main OAuth after main refreshes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-stale-inherited-"));
    const stateDir = path.join(root, ".nova-ai");
    const childAgentDir = path.join(stateDir, "agents", "worker", "agent");
    const childAuthPath = resolveAuthStorePath(childAgentDir);
    vi.stubEnv("NOVA_AI_STATE_DIR", stateDir);
  ...
  it("preserves inherited main OAuth in active secondary runtime snapshots", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-auth-save-snapshot-"));
    const stateDir = path.join(root, ".nova-ai");
    const childAgentDir = path.join(stateDir, "agents", "worker", "agent");
    const childAuthPath = resolveAuthStorePath(childAgentDir);
    vi.stubEnv("NOVA_AI_STATE_DIR", stateDir);
