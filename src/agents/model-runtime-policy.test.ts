...
const ORIGINAL_BUILD_PRIVATE_QA = process.env.NOVA_AI_BUILD_PRIVATE_QA;
const ORIGINAL_QA_FORCE_RUNTIME = process.env.NOVA_AI_QA_FORCE_RUNTIME;
...
function restoreEnv(
  name: "NOVA_AI_BUILD_PRIVATE_QA" | "NOVA_AI_QA_FORCE_RUNTIME",
...
function makeProviderRuntimeConfig(runtime: string): Nova AIConfig {
  return {
    models: {
      providers: {
        openai: {
...
afterEach(() => {
  restoreEnv("NOVA_AI_BUILD_PRIVATE_QA", ORIGINAL_BUILD_PRIVATE_QA);
  restoreEnv("NOVA_AI_QA_FORCE_RUNTIME", ORIGINAL_QA_FORCE_RUNTIME);
});

describe("resolveModelRuntimePolicy", () => {
  it("ignores the QA force-runtime override when the private QA gate is unset", () => {
    delete process.env.NOVA_AI_BUILD_PRIVATE_QA;
    process.env.NOVA_AI_QA_FORCE_RUNTIME = "pi";
  ...
  it("respects the QA force-runtime override when the private QA gate is set", () => {
    process.env.NOVA_AI_BUILD_PRIVATE_QA = "1";
    process.env.NOVA_AI_QA_FORCE_RUNTIME = "pi";
  ...
  it("ignores invalid QA force-runtime values even when the private QA gate is set", () => {
    process.env.NOVA_AI_BUILD_PRIVATE_QA = "1";
    process.env.NOVA_AI_QA_FORCE_RUNTIME = "bogus";
