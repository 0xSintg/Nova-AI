...
function stripNova AIPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "nova-ai") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "nova-ai"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "nova-ai"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "nova-ai") {
      return argv.slice(idx);
    }
  }
  return argv;
...
function parseNova AIChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const nova-aiArgv = stripNova AIPackageRunner(argv);
  return (
    normalizeCommandBaseName(nova-aiArgv[0]) === "nova-ai" &&
    (nova-aiArgv[1] === "channels" || nova-aiArgv[1] === "channel") &&
    nova-aiArgv[2] === "login"
  );
}
...
function rejectUnsafeControlShellCommand(command: string): void {
  ...
  for (const candidate of candidates) {
    ...
    if (parseNova AIChannelsLoginShellCommand(candidate)) {
      throw new Error(
        [
          "exec cannot run interactive Nova AI channel login commands.",
          "Run `nova-ai channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
        ].join(" "),
      );
    }
  }
