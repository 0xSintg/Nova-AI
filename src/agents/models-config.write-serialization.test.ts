...
const planNova AIModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  const call = planNova AIModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planNova AIModelsJson: (...args: unknown[]) => planNova AIModelsJsonMock(...args),
  }));
  ...
  ({ ensureNova AIModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureNova AIModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureNova AIModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureNova AIModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".nova-ai", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".nova-ai", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      planNova AIModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
      ...
      expect(planNova AIModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureNova AIModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureNova AIModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planNova AIModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planNova AIModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureNova AIModelsJson(first);
      await ensureNova AIModelsJson(second);
      await ensureNova AIModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureNova AIModelsJson(first),
        ensureNova AIModelsJson(second),
      ]);
