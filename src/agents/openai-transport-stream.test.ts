...
import {
  ...
  createNova AITransportStreamFnForModel,
...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    const thinkingBlock = output.content[0] as {
      ...
      nova-aiReasoningReplay?: unknown;
    };
    ...
    expect(replayItem).not.toHaveProperty("__nova-ai_replay");
    expect(thinkingBlock.nova-aiReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.NOVA_AI_DEBUG_MODEL_PAYLOAD;
    process.env.NOVA_AI_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.NOVA_AI_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.NOVA_AI_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.NOVA_AI_DEBUG_MODEL_PAYLOAD;
    process.env.NOVA_AI_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave Nova AI", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds Nova AI attribution to native OpenAI transport headers and protects it from pi", () => {
    vi.stubEnv("NOVA_AI_VERSION", "2026.3.22");
    ...
    expectRecordFields(headers, {
      originator: "nova-ai",
      version: "2026.3.22",
      "User-Agent": "nova-ai/2026.3.22",
      "X-Provider": "model",
      "X-Caller": "request",
    });
  });

  it("adds Nova AI attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("NOVA_AI_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("nova-ai-openai-responses-transport");
    expectRecordFields(prepared, {
      api: "nova-ai-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("nova-ai-anthropic-messages-transport");
    expectRecordFields(prepared, {
      api: "nova-ai-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "nova-ai-google-generative-ai-transport",
    );
  });
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        nova-ai_session_id: "session-123",
        nova-ai_turn_id: "turn-123",
      },
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { nova-ai_session_id: "session-123" },
  ...
  it("preserves prior Responses replay item ids for custom Codex-compatible responses", () => {
            ...
            content: [
              {
                ...
                nova-aiReasoningReplay: testing.buildOpenAIResponsesReasoningReplayMetadata(
                  model,
                  {
                    authProfileId: "openai-codex:oauth",
                    sessionId: "session-123",
    ...
    expect(reasoningItem).not.toHaveProperty("__nova-ai_replay");
    const assistantMessage = params.input?.find(
      (item) => item.type === "message" && item.role === "assistant",
    );
    expectRecordFields(assistantMessage, {
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        nova-ai_session_id: "session-123",
        nova-ai_turn_id: "turn-123",
        nova-ai_turn_attempt: "1",
        nova-ai_transport: "stream",
      },
