...
export function buildNova AIToolFallbackText(params: {
  surface: AgentPromptSurfaceKind;
  execToolName: string;
  ...
  if (params.surface === "pi_main") {
    return [
      ...
      "- browser: control Nova AI's dedicated browser",
  ...
  return "No Nova AI tool list is injected for this runtime prompt surface. Use only tools exposed directly by the active backend.";
}

export function shouldRenderNova AIToolWorkflowHints(params: {
  surface: AgentPromptSurfaceKind;
  hasToolList: boolean;
}): boolean {
