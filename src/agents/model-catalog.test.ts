...
let ensureNova AIModelsJsonMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    ensureNova AIModelsJsonMock = vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false });
    vi.doMock("./models-config.js", () => ({
      ensureNova AIModelsJson: ensureNova AIModelsJsonMock,
    }));
    vi.doMock("./agent-scope.js", () => ({
      resolveDefaultAgentDir: () => "/tmp/nova-ai",
    }));
  ...
  beforeEach(() => {
    ...
    ensureNova AIModelsJsonMock.mockClear();
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as Nova AIConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as Nova AIConfig });
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as Nova AIConfig });
  ...
  it("does not prepare models.json or import provider discovery when loading fallback catalog in read-only mode", async () => {
    ...
    expect(ensureNova AIModelsJsonMock).not.toHaveBeenCalled();
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as Nova AIConfig, readOnly: true });
  ...
  it("loads manifest model id policies once for discovered catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as Nova AIConfig, useCache: false });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as Nova AIConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/nova-ai-model-catalog-env" } as NodeJS.ProcessEnv;
