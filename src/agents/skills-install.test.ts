...
import { resolveNova AIMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"nova-ai":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("nova-ai-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["NOVA_AI_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("NOVA_AI_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Nova AI-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["NOVA_AI_STATE_DIR", "NOVA_AI_CONFIG_PATH"]);
    try {
      process.env.NOVA_AI_STATE_DIR = "/tmp/untrusted-state";
      process.env.NOVA_AI_CONFIG_PATH = "/tmp/untrusted-config/nova-ai.json";
  ...
  it("surfaces plugin scanner findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("nova-ai-workspace");
