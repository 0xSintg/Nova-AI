...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: Nova AIConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/nova-ai" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: Nova AIConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: Nova AIConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: Nova AIConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: Nova AIConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: Nova AIConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: Nova AIConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: Nova AIConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: Nova AIConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: Nova AIConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: Nova AIConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/nova-ai");
  });

  it("uses NOVA_AI_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "nova-ai-home");
    vi.stubEnv("NOVA_AI_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as Nova AIConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".nova-ai", "workspace"));
  });

  it("uses NOVA_AI_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses NOVA_AI_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "nova-ai-home");
    vi.stubEnv("NOVA_AI_HOME", home);
    // Clear state dir so it falls back to NOVA_AI_HOME
    vi.stubEnv("NOVA_AI_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as Nova AIConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".nova-ai", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/nova-ai-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/nova-ai-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-agent-scope-"));
