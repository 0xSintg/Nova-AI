...
function redactTranscriptOptions(cfg?: Nova AIConfig) {
...
function redactTranscriptText(value: string, cfg?: Nova AIConfig): string {
...
export function redactTranscriptMessage(message: AgentMessage, cfg?: Nova AIConfig): AgentMessage {
  if (cfg?.logging?.redactSensitive === "off") {
    return message;
  }
