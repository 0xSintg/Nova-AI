...
function createDefaultSpawnConfig(): Nova AIConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: Nova AIConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects Nova AI config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
    ...
    expect(result).toHaveProperty(
      "error",
      'agentId "pleres" is an Nova AI config agent, not an ACP harness. Use runtime="subagent" or omit runtime for Nova AI config agents. Use runtime="acp" only with external ACP harness ids such as codex, claude, droid, gemini, or opencode, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
    expectGatewayMethodNotCalled("agent");
  });

  it("maps Nova AI ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
  ...
  it("includes cwd in ACP thread intro banner when provided at spawn time", async () => {
    ...
    expect(metadata.introText).toContain("cwd: /home/bob/nova");
  });
