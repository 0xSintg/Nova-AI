...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__nova-aiCatalog) ? globalThis.__nova-aiCatalog : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__nova-aiHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __nova-aiSettleBridge: { value: settle },
    __nova-aiTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__nova-aiResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__nova-aiCatalog", catalogHandle);
  } finally {
    catalogHandle.dispose();
  }
  const hostRequest = vm.newFunction(
    "__nova-aiHostRequest",
  ...
  try {
    vm.setProp(vm.global, "__nova-aiHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "nova-ai-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__nova-aiTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__nova-aiResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  try {
    const settle = vm.global.getProp("__nova-aiSettleBridge");
    try {
