...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NOVA_AI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NOVA_AI_HOME: "/srv/nova-ai-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/nova-ai-home"), ".nova-ai", "workspace"));
  });

  it("prefers NOVA_AI_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NOVA_AI_WORKSPACE_DIR: "/srv/nova-ai-workspace",
      NOVA_AI_HOME: "/srv/nova-ai-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/nova-ai-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".nova-ai", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("nova-ai-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-workspace-hardlink-"));
    try {
