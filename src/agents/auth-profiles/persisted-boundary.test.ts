...
describe("persisted auth profile boundary", () => {
  ...
  it("rehydrates legacy oauthRef sidecars read-only for upgraded Codex OAuth users", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-auth-oauthref-runtime-"));
    const agentDir = path.join(stateDir, "agents", "main", "agent");
    const restoreStateDir = withEnvValue("NOVA_AI_STATE_DIR", stateDir);
    const restoreOAuthDir = withEnvValue("NOVA_AI_OAUTH_DIR", undefined);
    const restoreSecretKey = withEnvValue("NOVA_AI_AUTH_PROFILE_SECRET_KEY", "legacy-seed");
    try {
