...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: Nova AIConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<Nova AIConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<Nova AIConfig["agents"]>["list"]>[number]["tools"];
  }): Nova AIConfig {
    return {
      tools: params.tools,
      agents: {
