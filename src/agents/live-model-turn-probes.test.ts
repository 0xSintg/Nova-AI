...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "NOVA_AI_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { NOVA_AI_LIVE_MODEL_IMAGE_PROBE: "false" },
        "NOVA_AI_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { NOVA_AI_LIVE_MODEL_IMAGE_PROBE: "1" },
        "NOVA_AI_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
