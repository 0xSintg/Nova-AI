...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-tts-status-"));
});
...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  process.env.NOVA_AI_STATE_DIR = path.join(home, ".nova-ai");
  try {
    await run(home);
  } finally {
    restoreEnv("HOME", previousHome);
    restoreEnv("USERPROFILE", previousUserProfile);
    restoreEnv("NOVA_AI_HOME", previousNova AIHome);
    restoreEnv("NOVA_AI_STATE_DIR", previousStateDir);
  }
}
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".nova-ai", "settings", "tts.json");
  ...
  it("derives the default prefs path from NOVA_AI_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".nova-ai-dev");
      ...
      vi.stubEnv("NOVA_AI_CONFIG_PATH", path.join(stateDir, "nova-ai.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
