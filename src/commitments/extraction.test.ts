...
describe("commitment extraction", () => {
  ...
  async function createConfig(): Promise<Nova AIConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-commitments-"));
    tmpDirs.push(tmpDir);
    vi.stubEnv("NOVA_AI_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
  ...
  it("rejects disabled, low-confidence, and non-future candidates", () => {
    const cfg: Nova AIConfig = { commitments: { enabled: true } };
