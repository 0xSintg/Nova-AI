...
describe("image-ops temp dir", () => {
  ...
  beforeEach(() => {
    process.env.NOVA_AI_IMAGE_BACKEND = "sips";
  ...
  afterEach(() => {
    delete process.env.NOVA_AI_IMAGE_BACKEND;
    vi.restoreAllMocks();
  });

  it.skipIf(process.platform !== "darwin")(
    "creates sips temp dirs under the secured Nova AI tmp root",
    async () => {
      const secureRoot = await fs.realpath(resolvePreferredNova AITmpDir());
      ...
      const uuidPrefix = path.join(secureRoot, "nova-ai-img-");
