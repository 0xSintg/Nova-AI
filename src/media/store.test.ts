...
describe("media store", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("nova-ai-test-home-");
    home = tempHome.home;
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (store, home) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.nova-ai${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (store, home) => {
          ...
          const targetDir = path.join(home, ".nova-ai", "media", "oversized-stream");
