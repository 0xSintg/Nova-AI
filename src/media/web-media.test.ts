...
const CANVAS_HOST_PATH = "/__nova-ai__/canvas";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredNova AITmpDir(), "web-media-core-"));
...
describe("loadWebMedia", () => {
  ...
  it("resolves home-relative local media paths through allowed local roots", async () => {
    vi.stubEnv("NOVA_AI_HOME", fixtureRoot);
    try {
