...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace nova-ai devDependencies and plugin host floors", () => {
    const rootDir = makeTempDir(tempDirs, "nova-ai-sync-plugin-versions-");
    ...
    expect(summary.updated).toContain("@nova-ai/imessage");
    expect(updatedPackage.version).toBe("2026.4.1");
    expect(updatedPackage.devDependencies?.nova-ai).toBe("workspace:*");
    expect(updatedPackage.peerDependencies?.nova-ai).toBe(">=2026.4.1");
    expect(updatedPackage.nova-ai?.install?.minHostVersion).toBe(">=2026.3.30");
    expect(updatedPackage.nova-ai?.compat?.pluginApi).toBe(">=2026.4.1");
    expect(updatedPackage.nova-ai?.build?.nova-aiVersion).toBe("2026.4.1");
  });

  it("reports pending version sync without writing in check mode", () => {
    const rootDir = makeTempDir(tempDirs, "nova-ai-sync-plugin-versions-check-");
    ...
    expect(summary.updated).toEqual(["@nova-ai/discord"]);
    expect(unchangedPackage.version).toBe("2026.4.1");
    expect(unchangedPackage.peerDependencies?.nova-ai).toBe(">=2026.4.1");
    expect(unchangedPackage.nova-ai?.compat?.pluginApi).toBe(">=2026.4.1");
  });

  it("uses the base release version for beta changelog entries", () => {
    const rootDir = makeTempDir(tempDirs, "nova-ai-sync-plugin-versions-beta-changelog-");
    ...
    expect(summary.changelogged).toEqual(["@nova-ai/matrix"]);
