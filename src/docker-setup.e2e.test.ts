...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "nova-ai-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    NOVA_AI_GATEWAY_TOKEN: "test-token",
    NOVA_AI_CONFIG_DIR: join(sandbox.rootDir, "config"),
    NOVA_AI_WORKSPACE_DIR: join(sandbox.rootDir, "nova-ai"),
    NOVA_AI_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("nova-ai-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    NOVA_AI_GATEWAY_TOKEN: undefined,
    NOVA_AI_CONFIG_DIR: configDir,
    NOVA_AI_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_DOCKER_APT_PACKAGES: "curl wget",
      NOVA_AI_EXTRA_MOUNTS: undefined,
      NOVA_AI_HOME_VOLUME: "nova-ai-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).toContain("NOVA_AI_EXTRA_MOUNTS=");
    expect(envFile).toContain("NOVA_AI_HOME_VOLUME=nova-ai-home"); // pragma: allowlist secret
    expect(envFile).toContain("NOVA_AI_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `NOVA_AI_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("nova-ai-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/nova-ai`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("nova-ai-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg NOVA_AI_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node nova-ai-gateway dist/index.js onboard --mode local --no-install-daemon`,
    );
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node nova-ai-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]`,
    );
    expect(log).not.toContain("run --rm nova-ai-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy NOVA_AI_DOCKER_APT_PACKAGES into NOVA_AI_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("NOVA_AI_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg NOVA_AI_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg NOVA_AI_DOCKER_APT_PACKAGES");
  });

  it("prefers NOVA_AI_IMAGE_APT_PACKAGES over legacy NOVA_AI_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_IMAGE_APT_PACKAGES: "curl wget httpie",
      NOVA_AI_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("NOVA_AI_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg NOVA_AI_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg NOVA_AI_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty NOVA_AI_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg NOVA_AI_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network nova-ai-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const prestartCliRunLines = collectMatchingLines(prestartLines, (line) =>
      /\bcompose\b.*\brun\b.*\bnova-ai-cli\b/.test(line),
    );
  ...
  it("pins setup-time CLI state paths inside the container", async () => {
    ...
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_HOME: "/mnt/c/Users/Trevor",
      NOVA_AI_STATE_DIR: "/mnt/c/Users/Trevor/.nova-ai",
      NOVA_AI_CONFIG_PATH: "/mnt/c/Users/Trevor/.nova-ai/nova-ai.json",
      NOVA_AI_SKIP_ONBOARDING: "1",
    });
    expect(result.status).toBe(0);
  ...
  it("writes NOVA_AI_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh nova-ai-gateway -c");
  });
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_CONFIG_DIR: configDir,
      NOVA_AI_WORKSPACE_DIR: workspaceDir,
      NOVA_AI_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/nova-ai -xdev");
  });

  it("reuses existing config token when NOVA_AI_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("NOVA_AI_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when NOVA_AI_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "NOVA_AI_GATEWAY_TOKEN=dotenv-token-123\nNOVA_AI_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("NOVA_AI_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "NOVA_AI_GATEWAY_TOKEN=first-token",
        "NOVA_AI_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("NOVA_AI_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("NOVA_AI_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats NOVA_AI_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg NOVA_AI_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg NOVA_AI_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker nova-ai-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        NOVA_AI_SANDBOX: "1",
        NOVA_AI_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps nova-ai-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate nova-ai-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --force-recreate nova-ai-gateway`,
      );
  ...
  it("rejects injected multiline NOVA_AI_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("NOVA_AI_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid NOVA_AI_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid NOVA_AI_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("NOVA_AI_HOME_VOLUME must match");
  });

  it("rejects NOVA_AI_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("NOVA_AI_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when NOVA_AI_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("NOVA_AI_SKIP_ONBOARDING=1");
  });

  it("treats NOVA_AI_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      NOVA_AI_SKIP_ONBOARDING: "0",
    });
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/NOVA_AI_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/NOVA_AI_DISABLE_BONJOUR: \$\{NOVA_AI_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:nova-ai-gateway"');
    expect(compose).toContain("depends_on:\n      - nova-ai-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/NOVA_AI_GATEWAY_TOKEN: \$\{NOVA_AI_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        "${NOVA_AI_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.nova-ai-auth-profile-secrets}:/home/node/.config/nova-ai",
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{NOVA_AI_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/NOVA_AI_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/NOVA_AI_STATE_DIR: \/home\/node\/\.nova-ai$/gm)).toHaveLength(2);
    expect(
      compose.match(/NOVA_AI_CONFIG_PATH: \/home\/node\/\.nova-ai\/nova-ai\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/NOVA_AI_CONFIG_DIR: \/home\/node\/\.nova-ai$/gm)).toHaveLength(2);
    expect(
      compose.match(/NOVA_AI_WORKSPACE_DIR: \/home\/node\/\.nova-ai\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG NOVA_AI_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG NOVA_AI_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${NOVA_AI_IMAGE_APT_PACKAGES-$NOVA_AI_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg NOVA_AI_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG NOVA_AI_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG NOVA_AI_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG NOVA_AI_IMAGE_APT_PACKAGES");
  });
});
