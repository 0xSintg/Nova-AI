...
const MEMO_RELEVANT_ENV_KEYS = [
  ...
  "NOVA_AI_COMPATIBILITY_HOST_VERSION",
  "NOVA_AI_CONFIG_PATH",
  "NOVA_AI_DISABLE_BUNDLED_PLUGINS",
  "NOVA_AI_DISABLE_BUNDLED_SOURCE_OVERLAYS",
  "NOVA_AI_DISABLE_PERSISTED_PLUGIN_REGISTRY",
  "NOVA_AI_HOME",
  "NOVA_AI_NIX_MODE",
  "NOVA_AI_STATE_DIR",
  "USERPROFILE",
  ...
  return Object.entries(records)
    ...
    .map(([pluginId, rawRecord]) => {
      ...
      return [
        ...
        watchedFileFingerprint(
          resolvedInstallPath ? path.join(resolvedInstallPath, "nova-ai.plugin.json") : undefined,
          watchedFiles,
        ),
        ...
        watchedFileFingerprint(
          resolvedSourcePath ? path.join(resolvedSourcePath, "nova-ai.plugin.json") : undefined,
          watchedFiles,
        ),
      ];
    });
  ...
  return Object.entries(dependencies)
    ...
    .map(([packageName, rawSpec]) => {
      ...
      return [
        ...
        watchedFileFingerprint(path.join(packageDir, "nova-ai.plugin.json"), watchedFiles),
      ];
    });
}
...
function resolvePersistedRegistryFastMemoFingerprint(params: {
  ...
  const disabledByEnv = params.env.NOVA_AI_DISABLE_PERSISTED_PLUGIN_REGISTRY?.trim().toLowerCase();
  const disabled =
