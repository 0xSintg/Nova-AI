...
import type {
  ...
  Nova AIPluginApi,
  Nova AIPluginChannelRegistration,
  Nova AIPluginCliCommandDescriptor,
  Nova AIPluginCliRegistrar,
  Nova AIPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  Nova AIPluginGatewayRuntimeScopeSurface,
  Nova AIGatewayDiscoveryService,
  Nova AIPluginHostedMediaResolver,
  Nova AIPluginHttpRouteParams,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@nova-ai/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@nova-ai/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("nova-ai.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<Nova AIPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<Nova AIPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<Nova AIPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: Nova AIPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: Nova AIPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: Nova AIPluginHookOptions | undefined,
    config: Nova AIPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: Nova AIPluginHttpRouteParams) => {
  ...
  const registerHostedMediaResolver = (
    record: PluginRecord,
    resolver: Nova AIPluginHostedMediaResolver,
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: Nova AIPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const registrationCapabilities = resolvePluginRegistrationCapabilities(mode);
    const normalized =
      typeof (registration as Nova AIPluginChannelRegistration).plugin === "object"
        ? (registration as Nova AIPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is Nova AIPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: Nova AIPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: Nova AIPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: Nova AIPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: Nova AIPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: Nova AIPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: Nova AIPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: Nova AIGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: Nova AIPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<Nova AIPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<Nova AIPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as Nova AIConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as Nova AIConfig | undefined) ??
                    params.config;
