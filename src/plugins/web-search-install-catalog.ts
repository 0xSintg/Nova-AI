...
function getConfigPath(config: Nova AIConfig | undefined, path: string): unknown {
...
function setConfigPath(target: Nova AIConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: Nova AIConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: Nova AIConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: Nova AIConfig) =>
