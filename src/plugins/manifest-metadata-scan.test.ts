...
function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listNova AIPluginManifestMetadata", () => {
  afterEach(() => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "nova-ai.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "nova-ai.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    writeJson(path.join(home, ".nova-ai", "plugins", "installs.json"), {
      plugins: [{ rootDir: path.join(staleBundledRoot, "openai"), origin: "bundled" }],
    });

    const records = listNova AIPluginManifestMetadata({
      NOVA_AI_HOME: home,
