...
import {
  ...
  readNova AIManagedNpmRootOverrides,
  repairManagedNpmRootNova AIPeer,
...
import { installedPackageNeedsNova AIPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
import {
  linkNova AIPeerDependencies,
  relinkNova AIPeerDependenciesInManagedNpmRoot,
...
function formatUnresolvedNova AIPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares nova-ai as a peer dependency, but Nova AI could not create a plugin-local node_modules/nova-ai link. Run from a packaged Nova AI install or reinstall Nova AI, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing nova-ai.extensions; update the plugin package to include nova-ai.extensions (for example ["./dist/index.js"]). See https://docs.nova-ai.com/help/troubleshooting#plugin-install-fails-with-missing-nova-ai-extensions';
...
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_nova-ai-pack-archives";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_NOVA_AI_EXTENSIONS: "missing_nova-ai_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_NOVA_AI_EXTENSIONS: "empty_nova-ai_extensions",
  INVALID_NOVA_AI_EXTENSIONS: "invalid_nova-ai_extensions",
...
function ensureNova AIExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_NOVA_AI_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json nova-ai.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_NOVA_AI_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_NOVA_AI_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareNova AIReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@nova-ai/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official Nova AI package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official Nova AI package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official Nova AI install.`,
  );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.packageName !== "nova-ai") {
    try {
      await repairManagedNpmRootNova AIPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
      ...
      params.logger.warn?.(
        `Failed to repair managed npm nova-ai peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
    await relinkNova AIPeerDependenciesInManagedNpmRoot({
      npmRoot: params.npmRoot,
      logger: params.logger,
    });
  } catch (error) {
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "nova-ai") {
      continue;
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  if (params.packageName !== "nova-ai") {
    const repairedNova AIPeer = await repairManagedNpmRootNova AIPeer({
      npmRoot,
      timeoutMs,
      logger,
    });
    if (repairedNova AIPeer) {
      logger.info?.(`Repaired stale nova-ai peer dependency in ${npmRoot}`);
    }
  }
  const preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
  const managedOverrides = await readNova AIManagedNpmRootOverrides();
  ...
  if (params.packageName !== "nova-ai") {
    ...
    if (repairedNova AIPeer) {
      logger.info?.(`Repaired stale nova-ai peer dependency in ${npmRoot} after npm install`);
    }
  }
  try {
    await relinkNova AIPeerDependenciesInManagedNpmRoot({
    ...
    return {
      ok: false,
      error: `Failed to repair nova-ai peer links after npm install: ${String(error)}`,
    };
  }
  if (installedPackageNeedsNova AIPeerLinkRepair(installRoot)) {
    await rollbackManagedNpmPluginInstall({
      npmRoot,
      packageName: params.packageName,
      targetDir: installRoot,
    ...
    return {
      ok: false,
      error: formatUnresolvedNova AIPeerLinkError(params.packageName),
    };
  }
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "nova-ai security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureNova AIExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  const extensionsResult = ensureNova AIExtensions({
    manifest,
  });
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid nova-ai.plugin.json: ${ocManifestResult.error}`,
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json nova-ai.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Nova AI >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set NOVA_AI_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Nova AI >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Nova AI and retry.`,
...
async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkNova AIPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedNova AIPeerLinkError(params.pluginId),
    };
  }
  return null;
}
