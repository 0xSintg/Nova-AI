...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/nova-ai-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "nova-ai.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): Nova AIConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    NOVA_AI_HOME: path.join(homeDir, "home"),
    NOVA_AI_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
