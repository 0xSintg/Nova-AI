...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadNova AIPlugins(loadOptions);

      loadNova AIPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadNova AIPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadNova AIPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadNova AIPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadNova AIPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadNova AIPlugins(loadOptions)).not.toBe(registry);
  });
});
