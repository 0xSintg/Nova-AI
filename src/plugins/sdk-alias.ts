...
function hasTrustedNova AIRootIndicator(params: {
  packageRoot: string;
  ...
  const hasNova AIBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("nova-ai")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.nova-ai === "string");
  const hasNova AIEntrypoint = fs.existsSync(path.join(params.packageRoot, "nova-ai.mjs"));
  return hasCliEntryExport || hasNova AIBin || hasNova AIEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedNova AIRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedNova AIRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveNova AIPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedNova AIRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveNova AIPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedNova AIRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["nova-ai/plugin-sdk", "@nova-ai/plugin-sdk"] as const;
const OFFICIAL_CODEX_PLUGIN_PACKAGE_NAME = "@nova-ai/codex";
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@nova-ai/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function isOfficialInstalledCodexPluginPackageRoot(packageRoot: string) {
  ...
  return last === "codex" && scope === "@nova-ai" && nodeModules === "node_modules";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `nova-ai/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "nova-ai/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
