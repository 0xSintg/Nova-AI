...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".nova-ai", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "NOVA_AI_HOME", "NOVA_AI_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("nova-ai-bundle-commands-home-");
      const workspaceDir = await createTempDir("nova-ai-bundle-commands-workspace-");
