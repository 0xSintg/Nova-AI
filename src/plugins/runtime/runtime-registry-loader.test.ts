...
const mocks = vi.hoisted(() => ({
  loadNova AIPlugins: vi.fn<typeof import("../loader.js").loadNova AIPlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadNova AIPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadNova AIPlugins: (...args: Parameters<typeof mocks.loadNova AIPlugins>) =>
    mocks.loadNova AIPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadNova AIPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadNova AIPlugins>) => mocks.loadNova AIPlugins(...args),
    );
