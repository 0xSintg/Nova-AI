...
vi.mock("../loader.js", () => ({
  loadNova AIPlugins: (...args: unknown[]) => loadNova AIPluginsMock(...args),
}));
...
function getOnlyLoadNova AIPluginsOptions(): PluginLoadOptions {
  expect(loadNova AIPluginsMock).toHaveBeenCalledTimes(1);
  const options = loadNova AIPluginsMock.mock.calls[0]?.[0];
  if (!options || typeof options !== "object") {
    throw new Error("expected loadNova AIPlugins to receive plugin load options");
  }
...
describe("loadPluginMetadataRegistrySnapshot", () => {
  ...
  beforeEach(() => {
    ...
    loadNova AIPluginsMock.mockReturnValue({ plugins: [], diagnostics: [] });
  });
  ...
  it("defaults to a non-activating validate snapshot", () => {
    loadPluginMetadataRegistrySnapshot({
      ...
      env: { HOME: "/tmp/nova-ai-home" } as NodeJS.ProcessEnv,
      workspaceDir: "/workspace",
      onlyPluginIds: ["demo"],
    });

    const loadOptions = getOnlyLoadNova AIPluginsOptions();
    expect(loadOptions).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: { allow: ["demo"] } },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
      env: { HOME: "/tmp/nova-ai-home" },
  ...
  it("forwards an explicit logger through metadata snapshots", () => {
    ...
    expect(getOnlyLoadNova AIPluginsOptions()).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: {} },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
  ...
  it("honors explicit load options when reusing a resolved runtime context", () => {
    ...
    expect(getOnlyLoadNova AIPluginsOptions()).toEqual({
      config: { plugins: { allow: ["compat-provider"] } },
      activationSourceConfig: { plugins: { allow: ["raw-plugin"] } },
      autoEnabledReasons: {},
      workspaceDir: "/compat-workspace",
