...
import { normalizeNova AIVersionBase } from "../config/version.js";
...
import { loadNova AIPlugins } from "./loader.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeNova AIVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeNova AIVersionBase(plugin.version) ??
    plugin.version
  );
}
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "nova-ai";
