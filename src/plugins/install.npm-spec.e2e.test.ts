...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `nova-ai-${label}-`));
  tempDirs.push(dir);
  return dir;
}
...
async function packPlugin(params: {
  ...
  await fs.writeFile(
    path.join(packageDir, "nova-ai.plugin.json"),
    `${JSON.stringify(
      {
        id: params.pluginId,
...
describe("installPluginFromNpmSpec e2e", () => {
  it("scrubs root nova-ai materialized by required npm peers", async () => {
    ...
    const openClawVersions = [
      await packPlugin({
        packageName: "nova-ai",
        pluginId: "registry-nova-ai-copy",
        version: "2026.0.0",
        rootDir,
      }),
    ];
    const registry = await startStaticRegistry([
      ...
      { packageName: "nova-ai", latest: "2026.0.0", versions: openClawVersions },
    ]);
    ...
    const rawNova AILockEntry = rawLock.packages?.["node_modules/nova-ai"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawNova AILockEntry?.peer).toBe(true);
    expect(rawNova AILockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/nova-ai"]).toBeUndefined();
    await expect(fs.lstat(path.join(npmRoot, "node_modules", "nova-ai"))).rejects.toHaveProperty(
      "code",
      "ENOENT",
    );
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "nova-ai"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    expect(rootManifest.nova-ai?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("rolls back managed peer dependencies added before a failed install scan", async () => {
    ...
    expect(rootManifest.nova-ai?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
    await expect(
  ...
  it("falls back to the legacy install path when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.nova-ai?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(npmRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.nova-ai?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers when installing beside an existing host-peer plugin", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { nova-ai: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { nova-ai: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { nova-ai: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(npmRoot, "node_modules", codexName, "node_modules", "nova-ai"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(npmRoot, "node_modules", opikName, "node_modules", "nova-ai"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("relinks managed npm sibling nova-ai peers after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "nova-ai");
    ...
    expect(manifest.dependencies?.nova-ai).toBeUndefined();
    const lock = JSON.parse(await fs.readFile(path.join(npmRoot, "package-lock.json"), "utf8")) as {
      packages?: Record<string, unknown>;
    };
    expect(lock.packages?.["node_modules/nova-ai"]).toBeUndefined();
