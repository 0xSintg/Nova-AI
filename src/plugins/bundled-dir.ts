...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "nova-ai-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.NOVA_AI_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "nova-ai.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveNova AIPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveNova AIPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveNova AIPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.NOVA_AI_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.NOVA_AI_BUNDLED_PLUGINS_DIR ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.NOVA_AI_BUNDLED_PLUGINS_DIR?.trim();
