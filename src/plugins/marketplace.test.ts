...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("nova-ai-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("nova-ai-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("nova-ai-marketplace-test-", async (homeDir) => {
      const openClawHome = path.join(homeDir, "nova-ai-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, NOVA_AI_HOME: openClawHome },
