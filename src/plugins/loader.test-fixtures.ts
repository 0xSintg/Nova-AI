...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadNova AIPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadNova AIPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "nova-ai-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.NOVA_AI_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".nova-ai", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ NOVA_AI_STATE_DIR: stateDir, ...params.env }, () =>
    loadNova AIPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
