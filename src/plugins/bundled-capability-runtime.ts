...
import { discoverNova AIPlugins, type PluginDiscoveryResult } from "./discovery.js";
...
import type { Nova AIPluginDefinition, Nova AIPluginModule } from "./types.js";
...
export function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`nova-ai/plugin-sdk/${subpath}`, targetPath],
        [`@nova-ai/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    "nova-ai/plugin-sdk": _ignoredLegacyRootAlias,
    "@nova-ai/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as Nova AIPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  const discovery = params.discovery ?? discoverNova AIPlugins({ env });
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getModuleLoader(safeSource)(safeSource) as Nova AIPluginModule;
    } catch (error) {
