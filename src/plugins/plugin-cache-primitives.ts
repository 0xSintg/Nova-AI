...
export type ConfigScopedRuntimeCache<T> = WeakMap<Nova AIConfig, Map<string, T>>;

export type ConfigScopedPromiseLoader<T> = {
  load(config?: Nova AIConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: Nova AIConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<Nova AIConfig, Promise<T>>();

  const createPromise = (config?: Nova AIConfig): Promise<T> => {
  ...
  return {
    async load(config?: Nova AIConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<Nova AIConfig, Promise<T>>();
    },
  };
}
