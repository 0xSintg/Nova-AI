...
const APPROVALS_PATH = "~/.nova-ai/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "nova-ai-app-server:thread:",
  "nova-ai-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("nova-ai.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("nova-ai.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Nova AI instead. If this started after an update, run "nova-ai doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
