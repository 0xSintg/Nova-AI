...
vi.mock("./loader.js", () => ({
  ...
  loadNova AIPlugins: mocks.loadNova AIPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadNova AIPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadNova AIPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadNova AIPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadNova AIPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadNova AIPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadNova AIPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
