...
const fixtureRoot = makeTrackedTempDir("nova-ai-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "nova-ai.plugin.json");
...
async function loadPlugins(pluginPaths: string[], warnings?: string[]) {
  const { loadNova AIPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("nova-ai plugins list");
  });
});
