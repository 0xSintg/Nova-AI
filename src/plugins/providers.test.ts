...
type LoadNova AIPlugins = typeof import("./loader.js").loadNova AIPlugins;
...
const loadNova AIPluginsMock = vi.fn<LoadNova AIPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/nova-ai.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadNova AIPluginsMock, "Nova AI plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadNova AIPluginsMock, "Nova AI plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: Nova AIConfig = {
    plugins: {},
  };
  const autoEnabledConfig: Nova AIConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadNova AIPlugins: (...args: Parameters<LoadNova AIPlugins>) =>
        loadNova AIPluginsMock(...args),
