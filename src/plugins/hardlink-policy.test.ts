...
const nixEnv: NodeJS.ProcessEnv = { NOVA_AI_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat NOVA_AI_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-nova-ai-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-nova-ai-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-nova-ai-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
