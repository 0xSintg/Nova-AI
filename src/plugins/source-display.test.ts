...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "nova-ai", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".nova-ai", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".nova-ai", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "nova-ai-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".nova-ai", "extensions"),
      },
    });
  });
});
