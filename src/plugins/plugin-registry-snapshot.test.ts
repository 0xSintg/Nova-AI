...
function makeTempDir() {
  return makeTrackedTempDir("nova-ai-plugin-registry-snapshot", tempDirs);
}

function createHermeticEnv(rootDir: string): NodeJS.ProcessEnv {
  return {
    NOVA_AI_BUNDLED_PLUGINS_DIR: path.join(rootDir, "bundled"),
    NOVA_AI_STATE_DIR: path.join(rootDir, "state"),
    NOVA_AI_VERSION: "2026.4.26",
    VITEST: "true",
  };
}
...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  ...
  it("recovers managed npm plugins missing from a stale persisted registry", () => {
    ...
    expect(result.snapshot.installRecords.whatsapp).toEqual({
      source: "npm",
      spec: "@nova-ai/whatsapp@2026.5.2",
      installPath: whatsappDir,
      version: "2026.5.2",
      resolvedName: "@nova-ai/whatsapp",
      resolvedVersion: "2026.5.2",
      resolvedSpec: "@nova-ai/whatsapp@2026.5.2",
    });
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), NOVA_AI_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
  ...
  it("keeps persisted package plugins when file hashes match", () => {
    ...
    expect(record.manifestFile.size).toBe(
      fs.statSync(path.join(rootDir, "nova-ai.plugin.json")).size,
    );
