...
const mocks = vi.hoisted(() => ({
  ...
  loadNova AIPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadNova AIPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadNova AIPluginCliRegistry(...args),
  loadNova AIPlugins: (...args: unknown[]) => mocks.loadNova AIPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: Nova AIConfig;
  autoEnabledConfig: Nova AIConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadNova AIPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadNova AIPluginCliRegistry.mockReset();
    mocks.loadNova AIPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadNova AIPlugins.mockReset();
    mocks.loadNova AIPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as Nova AIConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as Nova AIConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as Nova AIConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as Nova AIConfig);

    const loadOptions = getMockCallObject(mocks.loadNova AIPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadNova AIPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as Nova AIConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as Nova AIConfig);

    expect(mocks.loadNova AIPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as Nova AIConfig;
    const configB = { plugins: {} } as Nova AIConfig;
    const envA = { NOVA_AI_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { NOVA_AI_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadNova AIPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadNova AIPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadNova AIPluginCliRegistry);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadNova AIPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as Nova AIConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadNova AIPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadNova AIPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadNova AIPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadNova AIPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadNova AIPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadNova AIPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as Nova AIConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
