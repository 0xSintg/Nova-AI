...
import { bundledPluginRootAt } from "nova-ai/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-nova-ai",
    targetDir: params?.targetDir ?? "/tmp/opik-nova-ai",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/nova-ai-plugins/legacy-chat",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "nova-ai-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/nova-ai-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createNova AIPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { nova-ai: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "nova-ai");
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@nova-ai/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["nova-ai-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/nova-ai-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-nova-ai",
        spec: "@opik/opik-nova-ai",
        integrity: "sha512-old",
        installPath: "/tmp/opik-nova-ai",
      }),
      pluginIds: ["opik-nova-ai"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-nova-ai",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "nova-ai-codex-app-server",
        spec: "nova-ai-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/nova-ai-codex-app-server",
      }),
      pluginIds: ["nova-ai-codex-app-server"],
      specOverrides: {
        "nova-ai-codex-app-server": "nova-ai-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "nova-ai-codex-app-server",
        targetDir: "/tmp/nova-ai-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "nova-ai-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        spec: "@nova-ai/acpx",
        installPath,
        resolvedName: "@nova-ai/acpx",
        resolvedSpec: "@nova-ai/acpx@2026.5.2-beta.1",
        resolvedVersion: "2026.5.2-beta.1",
      }),
      pluginIds: ["acpx"],
    });

    expect(npmInstallCall()?.spec).toBe("@nova-ai/acpx");
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nova-ai/acpx",
          version: "2026.5.2",
          resolvedSpec: "@nova-ai/acpx@2026.5.2",
        },
      }),
    );
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nova-ai/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@nova-ai/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@nova-ai/acpx",
        resolvedSpec: "@nova-ai/acpx@2026.5.3",
  ...
  it("repairs missing nova-ai peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@nova-ai/codex",
      version: "2026.5.3",
      peerDependencies: { nova-ai: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@nova-ai/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nova-ai/codex",
          version: "2026.5.3",
          resolvedSpec: "@nova-ai/codex@2026.5.3",
        },
      }),
    );
    const config: Nova AIConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@nova-ai/codex");
  ...
  it("skips unchanged npm plugins when the nova-ai peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@nova-ai/codex",
      version: "2026.5.3",
      peerDependencies: { nova-ai: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "nova-ai"), { recursive: true });
    mockNpmViewMetadata({
      name: "@nova-ai/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs nova-ai peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@nova-ai/brave-plugin" },
      { pluginId: "codex", packageName: "@nova-ai/codex" },
      { pluginId: "discord", packageName: "@nova-ai/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createNova AIPeerLinkFixtures(plugins);
  ...
  it("repairs sibling nova-ai peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@nova-ai/brave-plugin" },
      { pluginId: "codex", packageName: "@nova-ai/codex" },
      { pluginId: "discord", packageName: "@nova-ai/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@nova-ai/codex",
            version: "2026.5.4",
            resolvedSpec: "@nova-ai/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling nova-ai peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@nova-ai/brave-plugin" },
      { pluginId: "codex", packageName: "@nova-ai/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@nova-ai/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@nova-ai/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@nova-ai/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair nova-ai peer link for "broken" at ${brokenInstallPath}: Skipping nova-ai peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".nova-ai", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.nova-ai/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@nova-ai/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@nova-ai/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@nova-ai/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@nova-ai/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@nova-ai/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@nova-ai/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@nova-ai/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@nova-ai/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@nova-ai/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@nova-ai/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("clears stale plugin policy and slot references when disabling failed updates", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; Nova AI will continue without it. Failed to update demo: security scan blocked install';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-nova-ai@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-nova-ai@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-nova-ai" (@opik/opik-nova-ai@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-nova-ai: aborted: npm package integrity drift detected for @opik/opik-nova-ai@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @nova-ai/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@nova-ai/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @nova-ai/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "nova-ai-codex-app-server@beta",
        resolvedName: "nova-ai-codex-app-server",
        resolvedSpec: "nova-ai-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "nova-ai-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "nova-ai-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "nova-ai-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "nova-ai-codex-app-server",
      }),
      specOverrides: {
        "nova-ai-codex-app-server": "nova-ai-codex-app-server@beta",
      },
      expectedSpec: "nova-ai-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "nova-ai-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "nova-ai-codex-app-server",
      });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for nova-ai-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "nova-ai-codex-app-server",
          ...
          npmResolution: {
            name: "nova-ai-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "nova-ai-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("nova-ai-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("nova-ai-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "nova-ai-codex-app-server" has no beta npm release for nova-ai-codex-app-server@beta; using nova-ai-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "nova-ai-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated nova-ai-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used nova-ai-codex-app-server because nova-ai-codex-app-server@beta could not be used).",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("nova-ai-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "nova-ai-codex-app-server" failed beta npm update for nova-ai-codex-app-server@beta; using nova-ai-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "nova-ai-codex-app-server",
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update nova-ai-codex-app-server: npm package not found for nova-ai-codex-app-server.",
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "nova-ai-codex-app-server@rc",
      }),
      pluginIds: ["nova-ai-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("falls back to npm for trusted official ClawHub artifact blocks", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      error:
        'ClawHub artifact download for "@nova-ai/discord@2026.5.16-beta.5" is not available yet (ClawHub /api/v1/packages/%40nova-ai%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.). Use "npm:@nova-ai/discord@2026.5.16-beta.5" for launch installs while ClawHub artifact routing is being rolled out.',
    });
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nova-ai/discord",
          version: "2026.5.16-beta.5",
          resolvedSpec: "@nova-ai/discord@2026.5.16-beta.5",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createClawHubInstallConfig({
        ...
        installPath,
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@nova-ai/discord",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        spec: "clawhub:@nova-ai/discord",
      }),
      pluginIds: ["discord"],
      updateChannel: "beta",
      disableOnFailure: true,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@nova-ai/discord@beta");
    expect(npmInstallCall()?.spec).toBe("@nova-ai/discord@beta");
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          "Updated discord: 2026.5.12 -> 2026.5.16-beta.5. (warning: official ClawHub artifact fallback used @nova-ai/discord@beta).",
      },
    ]);
    expect(warnMessages).toEqual([
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@nova-ai/discord@beta; using npm @nova-ai/discord@beta instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nova-ai/discord",
          version: "2026.5.16",
          resolvedSpec: "@nova-ai/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@nova-ai/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@nova-ai/discord");
    expect(npmInstallCall()?.spec).toBe("@nova-ai/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@nova-ai/discord",
      installPath: "/tmp/nova-ai-plugins/discord",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @nova-ai/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@nova-ai/discord@beta; using clawhub:@nova-ai/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@nova-ai/discord; using npm @nova-ai/discord instead. Core update can still complete.',
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        nextVersion: "2026.5.16-beta.5",
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @nova-ai/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@nova-ai/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@nova-ai/voice-call",
      targetDir: "/tmp/nova-ai-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@nova-ai/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@nova-ai/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@nova-ai/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@nova-ai/voice-call");
    expect(result.config.plugins?.entries?.["@nova-ai/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@nova-ai/voice-call"], {
      source: "npm",
      spec: "@nova-ai/voice-call",
      installPath: "/tmp/nova-ai-voice-call",
      version: "0.0.2",
    });
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@nova-ai/context-engine",
      targetDir: "/tmp/nova-ai-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@nova-ai/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@nova-ai/context-engine"], {
      source: "npm",
      spec: "@nova-ai/context-engine",
      installPath: "/tmp/nova-ai-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("nova-ai-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("nova-ai-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/nova-ai/extensions/demo";
    const extensionsDir = "/var/nova-ai/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    const env = { NOVA_AI_HOME: "/srv/nova-ai-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/nova-ai-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/nova-ai-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@nova-ai/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@nova-ai/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@nova-ai/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@nova-ai/legacy-chat",
      installPath: "/tmp/nova-ai-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@nova-ai/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/nova-ai-plugins/voice-call",
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @nova-ai/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@nova-ai/voice-call",
          npmSpec: "@nova-ai/voice-call",
          channelIds: ["voice-call"],
        },
      ],
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/nova-ai-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@nova-ai/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@nova-ai/default-chat",
      installPath: "/tmp/nova-ai-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@nova-ai/legacy-chat@1.2.3",
              resolvedSpec: "@nova-ai/legacy-chat@1.2.3",
              installPath: "/tmp/nova-ai-plugins/legacy-chat",
            },
          },
        },
