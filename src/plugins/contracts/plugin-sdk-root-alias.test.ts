...
describe("plugin-sdk root alias", () => {
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(aliasMap["nova-ai/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["@nova-ai/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["nova-ai/plugin-sdk/group-access"]).toContain(
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "nova-ai/plugin-sdk/alpha",
      "@nova-ai/plugin-sdk/alpha",
      "nova-ai/plugin-sdk/group-access",
      "@nova-ai/plugin-sdk/group-access",
      "nova-ai/plugin-sdk/zeta",
      "@nova-ai/plugin-sdk/zeta",
      "nova-ai/plugin-sdk",
      "@nova-ai/plugin-sdk",
    ]);
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { NOVA_AI_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["nova-ai/plugin-sdk/qa-lab"]).toBe(
      path.join(packageRoot, "src", "plugin-sdk", "qa-lab.ts"),
    );
    expect(aliasMap["@nova-ai/plugin-sdk/qa-lab"]).toBe(
      path.join(packageRoot, "src", "plugin-sdk", "qa-lab.ts"),
    );
    expect(aliasMap).not.toHaveProperty("nova-ai/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("nova-ai/plugin-sdk/nested/path");
  });
  ...
  it("keeps non-QA private local-only plugin-sdk subpaths out of the CJS root alias", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap).not.toHaveProperty("nova-ai/plugin-sdk/codex-native-task-runtime");
    expect(aliasMap).not.toHaveProperty("@nova-ai/plugin-sdk/codex-native-task-runtime");
    expect(aliasMap).not.toHaveProperty("nova-ai/plugin-sdk/qa-runtime");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["nova-ai/plugin-sdk/channel-runtime"]).toBe(
