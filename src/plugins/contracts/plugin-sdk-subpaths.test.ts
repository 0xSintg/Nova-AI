...
import type {
  ...
  Nova AIPluginApi as CoreNova AIPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "nova-ai/plugin-sdk/core";
import * as providerEntrySdk from "nova-ai/plugin-sdk/provider-entry";
...
import type {
  ...
  Nova AIPluginApi as SharedNova AIPluginApi,
...
import type { Nova AIPluginApi } from "../types.js";
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_NOVA_AI_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps the deprecated channel-runtime shim unused in repo imports", () => {
    const matches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']nova-ai\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreNova AIPluginApi>().toMatchTypeOf<Nova AIPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreNova AIPluginApi>().toMatchTypeOf<SharedNova AIPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("nova-ai/plugin-sdk/core");
    const channelActionsSdk = await importResolvedPluginSdkSubpath(
      "nova-ai/plugin-sdk/channel-actions",
    );
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "nova-ai/plugin-sdk/global-singleton",
    );
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("nova-ai/plugin-sdk/plugin-entry");
    const channelLifecycleSdk = await importResolvedPluginSdkSubpath(
      "nova-ai/plugin-sdk/channel-lifecycle",
    );
    const channelPairingSdk = await importResolvedPluginSdkSubpath(
      "nova-ai/plugin-sdk/channel-pairing",
    );
    const channelReplyPipelineSdk = await importResolvedPluginSdkSubpath(
      "nova-ai/plugin-sdk/channel-reply-pipeline",
    );
    const representativeModules = [];
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`nova-ai/plugin-sdk/${id}`));
    }
  ...
  it("keeps the Zalouser command-auth compatibility facade importable", async () => {
    const commandAuthSdk = await importResolvedPluginSdkSubpath("nova-ai/plugin-sdk/command-auth");
    const zalouserSdk = await importResolvedPluginSdkSubpath("nova-ai/plugin-sdk/zalouser");
