...
} from "nova-ai/plugin-sdk/plugin-test-runtime";
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "nova-ai/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "nova-ai/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolveWorkspacePackagePublicModuleUrl } from "../../plugin-sdk/test-helpers/public-surface-loader.js";

type TtsRuntimeModule = typeof import("nova-ai/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("nova-ai/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@nova-ai/speech-core",
...
function asLegacyTtsConfig(value: unknown): Nova AIConfig {
  return value as Nova AIConfig;
}

function asLegacyNova AIConfig(value: Record<string, unknown>): Nova AIConfig {
  return value as unknown as Nova AIConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): Nova AIConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("nova-ai/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: Nova AIConfig): ResolvedTtsConfig {
  const rawConfig =
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyNova AIConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: Nova AIConfig = asLegacyNova AIConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.NOVA_AI_TTS_PREFS;
      process.env.NOVA_AI_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.NOVA_AI_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: Nova AIConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
