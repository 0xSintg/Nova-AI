...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: Nova AIPluginApi | undefined;
    let workspaceApi: Nova AIPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: Nova AIPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: Nova AIPluginApi | undefined;
      registerTestPlugin({
