...
const resolveNova AIPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/nova-ai-root.js", () => ({
  resolveNova AIPackageRootSync: (...args: unknown[]) =>
    resolveNova AIPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("nova-ai-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      nova-ai: { extensions: ["./dist/index.js"] },
      ...(params.dependency
...
type MockNpmPackage = {
  ...
  materializesRootNova AI?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootNova AI) {
      lockPackages["node_modules/nova-ai"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalNova AIPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      fs.rmSync(path.join(packageDir, "node_modules", "nova-ai"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalNova AIPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "nova-ai") {
            const nova-aiRoot = path.join(npmRoot, "node_modules", "nova-ai");
            fs.mkdirSync(nova-aiRoot, { recursive: true });
            fs.writeFileSync(
              path.join(nova-aiRoot, "package.json"),
              JSON.stringify({ name: "nova-ai", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootNova AI) {
            ...
            fs.writeFileSync(
              path.join(nova-aiRoot, "package.json"),
              JSON.stringify({ name: "nova-ai", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "nova-ai") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "nova-ai"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveNova AIPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "nova-ai", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveNova AIPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
});
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    const archivePath = path.join(stateDir, "nova-ai-pack-demo-1.2.3.tgz");
    fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

    mockNpmViewAndInstallMany([
      {
        packageName: "@nova-ai/pack-demo",
        version: "1.2.3",
        pluginId: "pack-demo",
        npmRoot,
      ...
      {
        spec: "@nova-ai/voice-call@0.0.1",
        packageName: "@nova-ai/voice-call",
        version: "0.0.1",
        pluginId: "voice-call",
        npmRoot,
      },
    ...
    expect(result.targetDir).toBe(path.join(npmRoot, "node_modules", "@nova-ai/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@nova-ai/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("nova-ai-pack-demo-1.2.3.tgz");
    ...
    const dependencySpec = managedManifest.dependencies?.["@nova-ai/pack-demo"];
    expect(dependencySpec).toMatch(/^file:\.\/_nova-ai-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_nova-ai-pack-archives"))).toBe(false);
    expect(runCommandWithTimeoutMock.mock.calls).toHaveLength(1);
  });

  it("installs npm plugins into .nova-ai/npm", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    ...
    expect(result.targetDir).toBe(path.join(npmRoot, "node_modules", "@nova-ai/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@nova-ai/voice-call@0.0.1");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed nova-ai peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { nova-ai: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs
          .lstatSync(path.join(npmRoot, "node_modules", "peer-plugin", "node_modules", "nova-ai"))
          .isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/nova-ai");
      }
      expect(
        fs
          .lstatSync(path.join(npmRoot, "node_modules", "peer-plugin", "node_modules", "nova-ai"))
    ...
    async () => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes(`Skipping nova-ai peerDependency link because ${staleNodeModulesPath}`),
        ),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their nova-ai peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveNova AIPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@nova-ai/codex@2026.5.7",
      packageName: "@nova-ai/codex",
      version: "2026.5.7",
      pluginId: "@nova-ai/codex",
      npmRoot,
      peerDependencies: { nova-ai: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@nova-ai/codex");
    expect(result.error).toContain("plugin-local node_modules/nova-ai link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate nova-ai package root")),
    ).toBe(true);
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@nova-ai", "codex"))).toBe(false);
    const managedManifest = JSON.parse(
      fs.readFileSync(path.join(npmRoot, "package.json"), "utf8"),
    ) as { dependencies?: Record<string, string> };
    expect(managedManifest.dependencies?.["@nova-ai/codex"]).toBeUndefined();
  });

  it.runIf(process.platform !== "win32")(
    "repairs root nova-ai materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(result.ok).toBe(true);
      expect(fs.existsSync(path.join(npmRoot, "node_modules", "nova-ai"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/nova-ai"]).toBeUndefined();
      expect(
        fs
          .lstatSync(
            path.join(npmRoot, "node_modules", "required-peer-plugin", "node_modules", "nova-ai"),
          )
          .isSymbolicLink(),
      ).toBe(true);
    },
  ...
  it("repairs stale managed nova-ai root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    fs.mkdirSync(path.join(npmRoot, "node_modules", "nova-ai"), { recursive: true });
    fs.writeFileSync(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/nova-ai": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/nova-ai/-/nova-ai-2026.5.4.tgz",
            },
          },
          dependencies: {
            nova-ai: {
    ...
    fs.writeFileSync(
      path.join(npmRoot, "node_modules", "nova-ai", "package.json"),
      JSON.stringify({
        name: "nova-ai",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@nova-ai/discord@beta",
      packageName: "@nova-ai/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { nova-ai: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("nova-ai");
    expect(manifest.dependencies?.["@nova-ai/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/nova-ai"]).toBeUndefined();
    expect(lockfile.dependencies?.nova-ai).toBeUndefined();
  });

  it("preserves the active host nova-ai runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "nova-ai");
    ...
    resolveNova AIPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/nova-ai-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/nova-ai-memory-libravdb",
      version: "1.4.69",
    ...
    expect(manifest.dependencies?.nova-ai).toBe("2026.5.12-beta.6");
    expect(manifest.dependencies?.["@xdarkicex/nova-ai-memory-libravdb"]).toBe("1.4.69");
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "nova-ai");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@nova-ai/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@nova-ai/voice-call",
              version: "0.0.1",
              dist: {
    ...
    await expect(
      fs.promises.access(path.join(npmRoot, "node_modules", "nova-ai")),
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; nova-ai?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.nova-ai?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.nova-ai?.managedOverrides).toEqual(["axios", "nested"]);
        }
  ...
  it.each(officialLaunchPluginCases)(
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Nova AI package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = path.join(npmRoot, "node_modules", "@nova-ai", "voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@nova-ai/whatsapp@0.0.1",
        packageName: "@nova-ai/whatsapp",
        version: "0.0.1",
        pluginId: "whatsapp",
        npmRoot,
      },
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@nova-ai", "voice-call"))).toBe(true);
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@nova-ai", "whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@nova-ai/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@nova-ai/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @nova-ai/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@nova-ai/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@nova-ai/voice-call@0.0.2-beta.1",
        packageName: "@nova-ai/voice-call",
        version: "0.0.2-beta.1",
        pluginId: "voice-call",
        npmRoot: prereleaseOnlyNpmRoot,
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@nova-ai/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @nova-ai/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@nova-ai/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoRoot({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
    });
