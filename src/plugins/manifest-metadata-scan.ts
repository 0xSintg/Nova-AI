...
const NOVA_AI_PACKAGE_ROOT = fileURLToPath(new URL("../..", import.meta.url));
const PLUGIN_MANIFEST_FILENAME = "nova-ai.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.NOVA_AI_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.NOVA_AI_STATE_DIR);
  if (override) {
...
function areBundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const value = normalizeTrimmedString(env.NOVA_AI_DISABLE_BUNDLED_PLUGINS)?.toLowerCase();
...
function resolveBundledPluginRoot(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = normalizeTrimmedString(env.NOVA_AI_BUNDLED_PLUGINS_DIR);
  if (override) {
    return resolveUserPath(override, env);
  }

  const sourceRoot = path.join(NOVA_AI_PACKAGE_ROOT, "extensions");
  const runtimeRoot = path.join(NOVA_AI_PACKAGE_ROOT, "dist-runtime", "extensions");
  const distRoot = path.join(NOVA_AI_PACKAGE_ROOT, "dist", "extensions");
...
export function listNova AIPluginManifestMetadata(
