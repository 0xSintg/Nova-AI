...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@nova-ai/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@nova-ai/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@nova-ai/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@nova-ai/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@nova-ai/google-meet:dangerous-exec:src/node-host.ts",
  "@nova-ai/google-meet:dangerous-exec:src/realtime.ts",
  "@nova-ai/matrix:dangerous-exec:src/matrix/deps.ts",
  "@nova-ai/voice-call:dangerous-exec:src/tunnel.ts",
  "@nova-ai/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@nova-ai/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@nova-ai/acpx:dangerous-exec:dist/service-<hash>.js",
  "@nova-ai/codex:dangerous-exec:dist/client-<hash>.js",
  "@nova-ai/google-meet:dangerous-exec:dist/index.js",
  "@nova-ai/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@nova-ai/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "nova-ai-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        nova-ai?: { release?: { publishToNpm?: unknown } };
      };
      try {
