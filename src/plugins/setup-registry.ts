...
function collectConfiguredPluginEntryIds(config: Nova AIConfig): string[] {
...
function resolveRegister(mod: Nova AIPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: Nova AIPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as Nova AIPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: Nova AIPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Nova AI ignored the plugin setup runtime entry. Remove setup-api/nova-ai.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
