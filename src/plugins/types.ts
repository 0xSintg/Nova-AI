...
import type {
  Nova AIPluginHookOptions,
  Nova AIPluginToolFactory,
  Nova AIPluginToolOptions,
} from "./tool-types.js";
...
export type {
  Nova AIPluginActiveModelContext,
  Nova AIPluginHookOptions,
 ...
 */
export type Nova AIPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<Nova AIConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Nova AI uses this to expose
 ...
 *
 * Runs after Nova AI resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Nova AI auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Nova AI wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Nova AI builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Nova AI no
...
/**
 * Provider-owned stream wrapper hook after Nova AI applies its generic
 ...
 *
 * Return `true` or `false` to override Nova AI's built-in provider cache TTL
 ...
 *
 * Runs only after Nova AI exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Nova AI should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Nova AI loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Nova AI falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Nova AI calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Nova AI performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Nova AI registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Nova AI wrappers.
   *
   ...
   *
   * Called after Nova AI resolves the raw configured credential but before the
   ...
   * hint than Nova AI's generic auth-store guidance.
   */
   ...
   * Nova AI's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Nova AI should surface a provider-specific hint for
   ...
   * merging. Nova AI deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Nova AI stores the
   ...
   * without replacing the full Nova AI-owned system prompt.
   */
   ...
   *
   * Runs after Nova AI's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Nova AI's default fallback policy.
   */
   ...
   * rewrites after Nova AI has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Nova AI handles
   ...
   *
   * Nova AI uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `nova-ai doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `nova-ai doctor` should offer.
   *
   ...
   *
   * Nova AI calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Nova AI's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Nova AI falls back to that stored profile.
   */
...
export type Nova AIPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral Nova AI session id when available. */
  sessionId?: string;
  /** Transcript file for this Nova AI session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Nova AI session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Nova AI configuration */
  config: Nova AIConfig;
...
export type Nova AIPluginCommandDefinition = {
...
export type Nova AIPluginHttpRouteAuth = "gateway" | "plugin";
export type Nova AIPluginHttpRouteMatch = "exact" | "prefix";
export type Nova AIPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type Nova AIPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type Nova AIPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type Nova AIPluginHttpRouteParams = {
  path: string;
  handler: Nova AIPluginHttpRouteHandler;
  handleUpgrade?: Nova AIPluginHttpRouteUpgradeHandler;
  auth: Nova AIPluginHttpRouteAuth;
  match?: Nova AIPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: Nova AIPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type Nova AIPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type Nova AIPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `nova-ai` program. For nested
...
export type Nova AIPluginCliRegistrar = (ctx: Nova AIPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Nova AI to keep a plugin command lazy-loaded while still
 ...
 */
export type Nova AIPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type Nova AIPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `nova-ai nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `nova-ai nodes`, so a descriptor named
   * `"camera"` exposes `nova-ai nodes camera`.
   */
  descriptors?: Nova AIPluginCliCommandDescriptor[];
};

export type Nova AIPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type Nova AIPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type Nova AIPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type Nova AIPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type Nova AIPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: Nova AIPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type Nova AIPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: Nova AIPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<Nova AIPluginNodeInvokeTransportResult>;
};

export type Nova AIPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type Nova AIPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: Nova AIPluginNodeInvokePolicyContext,
  ) => Promise<Nova AIPluginNodeInvokePolicyResult> | Nova AIPluginNodeInvokePolicyResult;
};

export type Nova AIPluginSecurityAuditContext = {
  config: Nova AIConfig;
  sourceConfig: Nova AIConfig;
  env: NodeJS.ProcessEnv;
  stateDir: string;
  configPath: string;
};

export type Nova AIPluginSecurityAuditCollector = (
  ctx: Nova AIPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type Nova AIGatewayDiscoveryAdvertiseContext = {
...
export type Nova AIGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: Nova AIGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type Nova AIPluginServiceContext = {
  config: Nova AIConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type Nova AIPluginService = {
  id: string;
  start: (ctx: Nova AIPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: Nova AIPluginServiceContext) => void | Promise<void>;
};

export type Nova AIPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type Nova AIPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: Nova AIPluginReloadRegistration;
  nodeHostCommands?: Nova AIPluginNodeHostCommand[];
  securityAuditCollectors?: Nova AIPluginSecurityAuditCollector[];
  register?: (api: Nova AIPluginApi) => void;
  activate?: (api: Nova AIPluginApi) => void;
};

export type Nova AIPluginModule = Nova AIPluginDefinition | ((api: Nova AIPluginApi) => void);
...
export type PluginConfigMigration = (config: Nova AIConfig) =>
  | {
      config: Nova AIConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `nova-ai migrate`. */
...
export type Nova AIPluginSessionStateApi = {
...
export type Nova AIPluginSessionWorkflowApi = {
...
export type Nova AIPluginSessionControlsApi = {
...
export type Nova AIPluginSessionApi = {
  state: Nova AIPluginSessionStateApi;
  workflow: Nova AIPluginSessionWorkflowApi;
  controls: Nova AIPluginSessionControlsApi;
};

export type Nova AIPluginAgentEventsApi = {
...
export type Nova AIPluginAgentApi = {
  events: Nova AIPluginAgentEventsApi;
};

export type Nova AIPluginRunContextApi = {
...
export type Nova AIPluginLifecycleApi = {
...
export type Nova AIPluginApi = {
   ...
   */
  session: Nova AIPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: Nova AIPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: Nova AIPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: Nova AIPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | Nova AIPluginToolFactory,
    opts?: Nova AIPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: Nova AIPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: Nova AIPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: Nova AIPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: Nova AIPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: Nova AIPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, Nova AI
  ...
  /**
   * Register a plugin-owned node feature command group under `nova-ai nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: Nova AIPluginCliRegistrar,
    opts?: Nova AIPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: Nova AIPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: Nova AIPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: Nova AIPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: Nova AIPluginSecurityAuditCollector) => void;
  registerService: (service: Nova AIPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: Nova AIGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `nova-ai migrate` (migration capability). */
   ...
   */
  registerCommand: (command: Nova AIPluginCommandDefinition) => void;
