...
export const PLUGIN_MANIFEST_FILENAME = "nova-ai.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `nova-ai qa`, for example `matrix`. */
  commandName: string;
...
// package.json "nova-ai" metadata (used for setup/catalog)
...
export type Nova AIPackageStartup = {
  /**
...
export type Nova AIPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type Nova AIPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: Nova AIPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
...
} & Partial<Record<ManifestKey, Nova AIPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): Nova AIPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawNova AI = manifest?.[MANIFEST_KEY] as unknown;
  if (rawNova AI === undefined || rawNova AI === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawNova AI)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json nova-ai must be an object",
    };
  }
  const raw = rawNova AI.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json nova-ai.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json nova-ai.extensions[${index}] must be a non-empty string`,
      };
    }
