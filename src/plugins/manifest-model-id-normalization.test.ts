...
import { listNova AIPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  NOVA_AI_DISABLE_BUNDLED_PLUGINS: process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS,
  NOVA_AI_BUNDLED_PLUGINS_DIR: process.env.NOVA_AI_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/nova-ai.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reflects manifest edits and state-dir changes on the next lookup", () => {
    ...
    process.env.NOVA_AI_STATE_DIR = stateDirA;
    process.env.NOVA_AI_HOME = undefined;
    process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.NOVA_AI_STATE_DIR = stateDirB;
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listNova AIPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listNova AIPluginManifestMetadata(process.env)).toHaveLength(1);
