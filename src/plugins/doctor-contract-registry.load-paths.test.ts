...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "nova-ai-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): Nova AIConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: Nova AIConfig, pluginId: string): Record<string, unknown> {
