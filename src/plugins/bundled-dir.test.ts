...
const originalBundledDir = process.env.NOVA_AI_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
...
function createNova AIRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "nova-ai" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@nova-ai/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "nova-ai.mjs"),
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
  if (originalDisableBundledPlugins === undefined) {
    delete process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "nova-ai-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "nova-ai-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "nova-ai-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "nova-ai-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "nova-ai-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "nova-ai-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasGitCheckout: true,
        hasPnpmWorkspace: true,
    ...
    const repoRoot = createNova AIRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "nova-ai.mjs");

    expect(resolveSourceCheckoutDependencyDiagnostic()).toEqual({
      source: repoRoot,
      message:
        "Nova AI source checkout detected without pnpm workspace dependencies; run `pnpm install` from the repo root so bundled plugins can load package-local dependencies.",
    });

    process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS = "1";
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by NOVA_AI_HOME", () => {
    const homeA = makeRepoRoot("nova-ai-bundled-dir-home-a-");
    const homeB = makeRepoRoot("nova-ai-bundled-dir-home-b-");
    seedBundledPluginTree(homeA, "bundled", "memory-core");
    seedBundledPluginTree(homeB, "bundled", "discord");
    const envBase = {
      NOVA_AI_BUNDLED_PLUGINS_DIR: "~/bundled",
      NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
      VITEST: "true",
    } satisfies NodeJS.ProcessEnv;

    const bundledA = resolveBundledPluginsDir({ ...envBase, NOVA_AI_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, NOVA_AI_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "nova-ai.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("nova-ai-bundled-dir-vitest-override-reject-");
    ...
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
    delete process.env.NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    delete process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("nova-ai-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("nova-ai-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("nova-ai-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("nova-ai-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "nova-ai.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createNova AIRoot({
      prefix: "nova-ai-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createNova AIRoot({
          prefix: "nova-ai-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createNova AIRoot({
          prefix: "nova-ai-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "nova-ai.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createNova AIRoot({
          prefix: "nova-ai-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
