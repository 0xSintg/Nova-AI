...
function buildScanFailureBlockReason(params: { error: string; targetLabel: string }) {
  return `${params.targetLabel} blocked: code safety scan failed (${params.error}). Run "nova-ai security audit --deep" for details.`;
}
...
function isPackageRootNova AIPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "nova-ai") ||
    (segments.length === 3 &&
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootNova AIPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedNova AIPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootNova AIPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostNova AIRootRealPath(): Promise<string | null> {
...
function isTrustedHostNova AIPath(params: {
  resolvedTargetPath: string;
  trustedHostNova AIRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostNova AIRootRealPath !== null &&
    isPathInside(params.trustedHostNova AIRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    ...
    if (
      ...
      isTrustedHostNova AIPath({
        resolvedTargetPath,
        trustedHostNova AIRootRealPath: params.trustedHostNova AIRootRealPath,
      })
    ) {
      return {};
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "nova-ai" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostNova AIRootRealPath = await resolveTrustedHostNova AIRootRealPath();
      ...
      if (entry.isSymbolicLink()) {
        ...
        if (pathContainsNodeModulesSegment(relativeNextPath)) {
          const symlinkTargetInspection = await inspectNodeModulesSymlinkTarget({
            ...
            trustedHostNova AIRootRealPath,
          });
...
export async function scanBundleInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Bundle "{target}" has {count} suspicious code pattern(s). Run "nova-ai security audit --deep" for details.`,
...
export async function scanPackageInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Plugin "{target}" has {count} suspicious code pattern(s). Run "nova-ai security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanFileInstallSourceRuntime(
  ...
  const builtinScan = await scanFileTarget({
    ...
    suspiciousMessage: `Plugin file "{target}" has {count} suspicious code pattern(s). Run "nova-ai security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanSkillInstallSourceRuntime(params: {
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    path: params.sourceDir,
    suspiciousMessage:
      'Skill "{target}" has {count} suspicious code pattern(s). Run "nova-ai security audit --deep" for details.',
