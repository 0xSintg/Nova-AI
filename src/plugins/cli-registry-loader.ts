...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly Nova AIPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: Nova AIPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadNova AIPluginCliRegistry(
...
): Promise<Nova AIPluginCliCommandDescriptor[]> {
  try {
