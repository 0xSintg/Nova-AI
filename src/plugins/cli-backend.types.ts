...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Nova AI consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Nova AI should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Nova AI's bundle
   ...
   *
   * Use this when the backend should consume a canonical Nova AI auth profile
   ...
   * `profile-only` treats the selected Nova AI auth profile as the sole auth
   ...
   * as static config, such as mapping Nova AI thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Nova AI's tool
