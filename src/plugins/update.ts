...
import {
  ...
  installedPackageNeedsNova AIPeerLinkRepair,
...
import { linkNova AIPeerDependencies } from "./plugin-peer-link.js";
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareNova AIReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function shouldFallbackClawHubBridgeToNpm(params: {
  ...
  if (!isNova AIOrgNpmSpec(params.npmSpec)) {
    return false;
  }
  return (
...
function migratePluginConfigId(cfg: Nova AIConfig, fromId: string, toId: string): Nova AIConfig {
  if (fromId === toId) {
    return cfg;
  }
...
function withoutPluginInstallRecord(cfg: Nova AIConfig, pluginId: string): Nova AIConfig {
  const installs = cfg.plugins?.installs;
...
function resetDisabledPluginSlots(
  slots: NonNullable<Nova AIConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<Nova AIConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: Nova AIConfig, pluginId: string): Nova AIConfig {
...
async function repairNova AIPeerLinksForNpmInstalls(params: {
  config: Nova AIConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair nova-ai peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsNova AIPeerLinkRepair(installPath)) {
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair nova-ai peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsNova AIPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair nova-ai peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (pluginId: string, message: string) => {
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; Nova AI will continue without it. ` +
        message;
    ...
    if (!params.dryRun && record.source === "npm" && currentVersion) {
      ...
      if (metadataResult.ok) {
        if (
          ...
          !installedPackageNeedsNova AIPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairNova AIPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
