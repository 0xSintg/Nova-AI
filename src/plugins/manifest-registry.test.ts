...
import type { Nova AIPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("nova-ai-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "nova-ai.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "nova-ai" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageName?: string;
  packageVersion?: string;
  packageManifest?: Nova AIPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "nova-ai.plugin.json");
  const linkedManifest = path.join(rootDir, "nova-ai.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-prometheus": {
          source: "npm",
          installPath: dir,
          resolvedName: "@nova-ai/diagnostics-prometheus",
  ...
  it("reports non-bundled providerAuthEnvVars as deprecated compat metadata", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(dir, "nova-ai.plugin.json"),
  ...
  it("hydrates supplemental official external catalog contracts for lagging npm manifests", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        idHint: "wecom-nova-ai-plugin",
        rootDir: dir,
        origin: "global",
        packageName: "@wecom/wecom-nova-ai-plugin",
      }),
    ]);
  ...
  it("ignores legacy provider discovery entries outside the plugin root", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(pluginDir, "nova-ai.plugin.json"),
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "nova-ai.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { NOVA_AI_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Nova AI >=2026.3.22, but this host is 2026.3.21",
      expectWarn: true,
    },
    {
      name: "skips plugins whose beta minHostVersion is newer than the current host",
      minHostVersion: ">=2026.5.1-beta.1",
      env: { NOVA_AI_VERSION: "2026.4.30" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Nova AI >=2026.5.1-beta.1, but this host is 2026.4.30",
      expectWarn: true,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | nova-ai.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { NOVA_AI_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("accepts legacy bare minHostVersion metadata for recorded installed globals", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "nova-ai.install.minHostVersion must use");
  });
  ...
  it("does not runtime-gate bundled source plugins by install minHostVersion", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "requires Nova AI");
  });
  ...
  it("suppresses duplicate warning when global candidates come from the same package artifact", () => {
    ...
    const manifest = { id: "opik-nova-ai", configSchema: { type: "object" } };
  ...
  it("still rejects config manifest hardlinks outside the Nix store in Nix mode", () => {
    ...
    const registry = loadPluginManifestRegistry({
      env: hermeticEnv({ NOVA_AI_NIX_MODE: "1" }),
      candidates: [
        createPluginCandidate({
  ...
  it("resolves load-path manifests from the current env home", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        NOVA_AI_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeB,
        NOVA_AI_HOME: undefined,
        NOVA_AI_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
  ...
  it("resolves manifests against the current host version", () => {
    ...
    const olderHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        NOVA_AI_VERSION: "2026.3.21",
      }),
    });
    const newerHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        NOVA_AI_VERSION: "2026.3.22",
      }),
    });
