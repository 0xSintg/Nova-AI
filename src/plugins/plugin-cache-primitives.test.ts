...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as Nova AIConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as Nova AIConfig;
    const load = vi.fn(async (config?: Nova AIConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: Nova AIConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
