...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: Nova AIConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when NOVA_AI_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: Nova AIConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from NOVA_AI_GATEWAY_TOKEN", () => {
    const cfg: Nova AIConfig = {
