...
describe("security audit exec surface findings", () => {
  // Redirect the Nova AI home (NOVA_AI_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.nova-ai/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".nova-ai"), { recursive: true });
    // NOVA_AI_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
    // so all three must point at the tempdir to neutralize whichever the host
    // happens to have set.
    process.env.NOVA_AI_HOME = tempDir;
