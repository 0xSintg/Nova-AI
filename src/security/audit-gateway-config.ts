...
type CollectDangerousConfigFlags = (cfg: Nova AIConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.NOVA_AI_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.NOVA_AI_GATEWAY_PASSWORD);
