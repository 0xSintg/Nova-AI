...
function summarizeGroupPolicy(cfg: Nova AIConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: Nova AIConfig): boolean {
...
function isBrowserEnabled(cfg: Nova AIConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: Nova AIConfig): SecurityAuditFinding[] {
