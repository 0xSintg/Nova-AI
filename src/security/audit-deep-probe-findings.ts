...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("nova-ai status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("nova-ai security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set NOVA_AI_GATEWAY_TOKEN/NOVA_AI_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("nova-ai security audit --deep")}".`,
    });
  }
  return findings;
}
