...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: Nova AIConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: Nova AIConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: Nova AIConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): Nova AIConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: Nova AIConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
