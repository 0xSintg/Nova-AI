...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.NOVA_AI_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.NOVA_AI_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.NOVA_AI_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.NOVA_AI_GATEWAY_PASSWORD)),
  });
