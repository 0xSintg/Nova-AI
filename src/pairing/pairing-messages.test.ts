...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NOVA_AI_CONTAINER_HINT", "NOVA_AI_PROFILE"]);
    delete process.env.NOVA_AI_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:nova-ai|nova-ai) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
