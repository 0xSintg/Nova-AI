...
export function shouldEnableNova AICompileCache(params: {
...
export function resolveNova AICompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
export type Nova AICompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
};

type Nova AICompileCacheRespawnRuntime = {
  spawn: typeof spawn;
...
export function buildNova AICompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
...
}): Nova AICompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
  if (!isSourceCheckoutInstallRoot(params.installRoot)) {
    return undefined;
  }
  if (env.NOVA_AI_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return undefined;
  }
...
export function respawnWithoutNova AICompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runNova AICompileCacheRespawnPlan(plan);
  return true;
}

export function runNova AICompileCacheRespawnPlan(
  plan: Nova AICompileCacheRespawnPlan,
  runtime: Nova AICompileCacheRespawnRuntime = {
    spawn,
  ...
  child.once("error", (error) => {
    clearSignalExitTimer();
    runtime.writeError(
      `[nova-ai] Failed to respawn CLI without compile cache: ${
...
export function enableNova AICompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableNova AICompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveNova AICompileCacheDirectory(params));
  } catch {
