...
function setConfiguredFetchProviderKey(configTarget: Nova AIConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: Nova AIConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: Nova AIConfig, provider: ProviderUnderTest): unknown {
...
describe("runtime web tools resolution", () => {
  ...
  describe("when brave is installed as an external plugin and explicitly configured", () => {
    ...
    beforeEach(() => {
      loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
        brave: { source: "npm", spec: "@nova-ai/brave-search" },
      });
