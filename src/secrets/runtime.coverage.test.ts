...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: Nova AIConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "nova-ai.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.NOVA_AI_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_NOVA_AI_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.NOVA_AI_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.NOVA_AI_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function collectNova AICoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "nova-ai.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_NOVA_AI_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForNova AITarget(
...
async function expectNova AICoverageBatchResolved(
  label: string,
  batch: readonly SecretRegistryEntry[],
): Promise<void> {
  logCoverageBatch(label, batch);
  const config = {} as Nova AIConfig;
  const env: Record<string, string> = {};
  for (const [index, entry] of batch.entries()) {
    const envId = `NOVA_AI_SECRET_TARGET_${entry.id}`;
    ...
    applyConfigForNova AITarget(config, entry, envId, wildcardToken);
  }
...
const NOVA_AI_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectNova AICoverageEntries({ includePluginEntries: false }),
);
const NOVA_AI_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectNova AICoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  ...
  describe("nova-ai.json core and channel registry targets", () => {
    test.each(NOVA_AI_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectNova AICoverageBatchResolved("nova-ai.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("nova-ai.json plugin registry targets", () => {
    test.each(NOVA_AI_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectNova AICoverageBatchResolved("nova-ai.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = `NOVA_AI_AUTH_SECRET_TARGET_${entry.id}`;
        ...
        const snapshot = await prepareAuthCoverageSnapshot({
          ...
          env,
          agentDirs: ["/tmp/nova-ai-agent-main"],
