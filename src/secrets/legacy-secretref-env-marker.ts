...
function toCandidate(
  ...
  defaults: NonNullable<Nova AIConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: Nova AIConfig): {
  config: Nova AIConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as Nova AIConfig & Record<string, unknown>;
