...
let compiledCoreNova AITargetState: {
  knownTargetIds: Set<string>;
  openClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledChannelNova AITargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledSecretTargetRegistryState() {
  ...
  const openClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "nova-ai.json",
  );
...
function getCompiledCoreNova AITargetState() {
  if (compiledCoreNova AITargetState) {
    return compiledCoreNova AITargetState;
  }
  const openClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "nova-ai.json")
    .map(compileTargetRegistryEntry);
  compiledCoreNova AITargetState = {
...
function getCompiledChannelNova AITargets(
  channelId: string,
  ...
  if (compiledChannelNova AITargets.has(normalizedChannelId)) {
    return compiledChannelNova AITargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "nova-ai.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelNova AITargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreNova AITargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreNova AITargetState().openClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelNova AITargets(explicitChannelId)
    : null;
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreNova AITargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreNova AITargetState()
