...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("nova-ai-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".nova-ai", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".nova-ai", "agents", "ops", "agent");
