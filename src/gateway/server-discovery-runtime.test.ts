...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn(() => "nova-ai.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/nova-ai.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (Nova AI)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/nova-ai"),
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    ...
    process.env.NOVA_AI_SSH_PORT = "2222";
  ...
  it("continues startup when a local discovery service never settles", async () => {
    ...
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.NOVA_AI_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "10";
  ...
  it("skips local discovery services for truthy NOVA_AI_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.NOVA_AI_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "nova-ai.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("nova-ai.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (Nova AI)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (nova-ai.internal. → /tmp/nova-ai.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
