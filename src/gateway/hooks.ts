...
export function resolveHooksConfig(cfg: Nova AIConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: Nova AIConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-nova-ai-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-nova-ai-idempotency-key"]) ||
