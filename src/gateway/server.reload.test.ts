...
const hoisted = vi.hoisted(() => {
  ...
  const resolveNova AIPackageRootSync = vi.fn((_params: unknown) => "/package");
...
vi.mock("../infra/nova-ai-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/nova-ai-root.js")>();
  return {
    ...actual,
    resolveNova AIPackageRootSync: hoisted.resolveNova AIPackageRootSync,
  };
});
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    prevSkipChannels = process.env.NOVA_AI_SKIP_CHANNELS;
    prevSkipGmail = process.env.NOVA_AI_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.NOVA_AI_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.NOVA_AI_SKIP_CHANNELS = "0";
    ...
    hoisted.resolveNova AIPackageRootSync.mockClear();
    hoisted.resolveNova AIPackageRootSync.mockReturnValue("/package");
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.NOVA_AI_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.NOVA_AI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NOVA_AI_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.NOVA_AI_SKIP_PROVIDERS;
    } else {
      process.env.NOVA_AI_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    const configPath = process.env.NOVA_AI_CONFIG_PATH;
    if (!configPath) {
      throw new Error("NOVA_AI_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ NOVA_AI_TEST_MINIMAL_GATEWAY: undefined }, async () =>
...
`,
    ...
    const previousGatewayTokenEnv = process.env.NOVA_AI_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.NOVA_AI_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
