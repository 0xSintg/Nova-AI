...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "nova-ai-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.NOVA_AI_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as Nova AIConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "nova-ai-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.NOVA_AI_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
