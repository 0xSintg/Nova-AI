...
import { NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("nova-aiReasoningReplay" in entry) {
    delete entry.nova-aiReasoningReplay;
    changed = true;
  }
...
function readTtsSupplementMarker(
  ...
  const marker = message.nova-aiTtsSupplement;
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === NOVA_AI_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
