...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([NOVA_AI_MODEL_ID, NOVA_AI_DEFAULT_MODEL_ID]);
  ids.add(`nova-ai/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`nova-ai/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== NOVA_AI_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
