...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Nova AI MCP tool `nova-ai-tools/cron` (server `nova-ai-tools`, tool `cron`). " +
      "If the harness shows Claude-style MCP names, use `mcp__nova-ai-tools__cron` or `mcp__nova-ai_tools__cron`. " +
      ...
      "Do the actual tool call; I will verify externally with the Nova AI cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Nova AI MCP tool `nova-ai-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__nova-ai-tools__cron` or `mcp__nova-ai_tools__cron`. " +
      ...
      "I will verify externally with the Nova AI cron CLI."
    );
  }
  return (
    "Your previous Nova AI cron MCP tool call was cancelled before the job was created. " +
    "Retry the Nova AI MCP tool `nova-ai-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__nova-ai-tools__cron` or `mcp__nova-ai_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runNova AICliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  delete childEnv.VITEST;
  delete childEnv.VITEST_MODE;
  delete childEnv.VITEST_POOL_ID;
  delete childEnv.VITEST_WORKER_ID;
  const { stdout, stderr } = await execFileAsync(process.execPath, ["nova-ai.mjs", ...args], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: 30_000,
    maxBuffer: 1024 * 1024,
  ...
  if (!trimmed) {
    throw new Error(
      [
        `nova-ai ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `nova-ai ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runNova AICliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
