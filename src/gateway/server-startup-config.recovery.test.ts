...
import type { ConfigFileSnapshot, ModelDefinitionConfig, Nova AIConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: Nova AIConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: Nova AIConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: Nova AIConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/nova-ai-startup-recovery.json";
const validConfig = {
  gateway: {
    mode: "local",
  },
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires Nova AI >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
      legacyIssues: [],
    });
