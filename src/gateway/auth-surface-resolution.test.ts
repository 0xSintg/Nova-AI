...
function remoteGatewayConfig(remote?: GatewayRemoteConfig): Nova AIConfig {
  return {
    gateway: {
      mode: "remote",
      remote: {
...
describe("resolveGatewayInteractiveSurfaceAuth", () => {
  it("uses NOVA_AI_GATEWAY_TOKEN as remote interactive fallback", async () => {
    await expect(
  ...
  it("keeps configured remote token ahead of NOVA_AI_GATEWAY_TOKEN", async () => {
    await expect(
      resolveGatewayInteractiveSurfaceAuth({
  ...
  it("falls back to NOVA_AI_GATEWAY_TOKEN when the remote token ref is unresolved", async () => {
    await expect(
      resolveGatewayInteractiveSurfaceAuth({
        config: {
          ...remoteGatewayConfig({
