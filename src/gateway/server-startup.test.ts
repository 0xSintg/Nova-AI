...
const ensureNova AIModelsJsonMock = vi.fn<
  (
    config: unknown,
    agentDir: unknown,
    options?: unknown,
...
vi.mock("../agents/models-config.js", () => ({
  ensureNova AIModelsJson: (config: unknown, agentDir: unknown, options?: unknown) =>
    ensureNova AIModelsJsonMock(config, agentDir, options),
}));
...
function expectModelsJsonPrewarmCall(cfg: Nova AIConfig) {
  expect(ensureNova AIModelsJsonMock).toHaveBeenCalledTimes(1);
  const [calledConfig, agentDir, options] = ensureNova AIModelsJsonMock.mock.calls.at(0) ?? [];
...
describe("gateway startup primary model warmup", () => {
  ...
  it("honors the startup model prewarm skip env", () => {
    ...
    expect(
      shouldSkipStartupModelPrewarm({
        NOVA_AI_SKIP_STARTUP_MODEL_PREWARM: "true",
      }),
    ).toBe(true);
  });
  ...
  it("warns when scoped models.json preparation fails", async () => {
    ensureNova AIModelsJsonMock.mockRejectedValueOnce(new Error("models write failed"));
    const warn = vi.fn();
