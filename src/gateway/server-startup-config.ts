...
import type { ConfigFileSnapshot, Nova AIConfig } from "../config/types.nova-ai.js";
...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: Nova AIConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: Nova AIConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: Nova AIConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: Nova AIConfig): Nova AIConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: Nova AIConfig): void {
