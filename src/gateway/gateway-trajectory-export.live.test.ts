...
const CODEX_HARNESS_LIVE = process.env.NOVA_AI_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.NOVA_AI_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["NOVA_AI_TRAJECTORY", "NOVA_AI_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-nova-ai-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.NOVA_AI_TRAJECTORY = "1";
      process.env.NOVA_AI_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".nova-ai", "trajectory-exports", "bundle");
