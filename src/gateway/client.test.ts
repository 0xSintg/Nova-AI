...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("nova-ai doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.NOVA_AI_PROXY_ACTIVE;
    delete process.env.NOVA_AI_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.NOVA_AI_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://nova-ai-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { NOVA_AI_HOME: "/tmp/custom-nova-ai-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      NOVA_AI_STATE_DIR: "/tmp/nova-ai-client-service-state",
