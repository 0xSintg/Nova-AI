...
const READ_SCOPE_HEADER = { "x-nova-ai-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-session-history-"));
...
function expectNova AIMetadata(
...
async function expectMessageEventMatch(
  ...
  if (params.id !== undefined) {
    expectNova AIMetadata(
      (event.data as { message?: { __nova-ai?: { id?: string; seq?: number } } }).message?.[
        "__nova-ai"
      ],
      {
        id: params.id,
...
describe("session history HTTP endpoints", () => {
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectNova AIMetadata(
        (
          body.messages?.[0] as {
            __nova-ai?: { id?: string; seq?: number };
          }
        )?.["__nova-ai"],
        {
          seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __nova-ai?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __nova-ai?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message["__nova-ai"]?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __nova-ai?: { seq?: number } }>;
        messages?: Array<{ __nova-ai?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message["__nova-ai"]?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectNova AIMetadata(nextData.messages?.[0]?.["__nova-ai"], {
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(refreshData.messages?.[0]?.["__nova-ai"]?.seq).toBe(2);
  ...
  test("sanitizes phased assistant history entries before returning them", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectNova AIMetadata(body.messages?.[0]?.["__nova-ai"], {
        id: visibleMessageId,
        seq: 2,
      });
    });
