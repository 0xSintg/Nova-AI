...
export function attachNova AITranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__nova-ai"] &&
    typeof record["__nova-ai"] === "object" &&
    !Array.isArray(record["__nova-ai"])
      ? (record["__nova-ai"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __nova-ai: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __nova-ai: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachNova AITranscriptMeta(message, { seq: firstSeq + index }),
  );
...
function parsedSessionEntryToMessage(parsed: unknown, seq: number): unknown {
  ...
  if (entry.message) {
    return attachNova AITranscriptMeta(entry.message, {
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "nova-ai" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "nova-ai" && model === "delivery-mirror";
