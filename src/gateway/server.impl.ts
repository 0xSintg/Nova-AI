...
import { ensureNova AICliOnPath } from "../infra/path-env.js";
...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.NOVA_AI_GATEWAY_STARTUP_TRACE);
  let timelineConfig: Nova AIConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.NOVA_AI_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: Nova AIConfig) {
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`nova-ai config set gateway.auth.mode token\` and \`nova-ai config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed Nova AI config and rebuild.",
    "For the first-party Nix flow, see https://github.com/nova-ai/nix-nova-ai#quick-start and https://docs.nova-ai.com/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing nova-ai.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.NOVA_AI_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.NOVA_AI_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "NOVA_AI_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "NOVA_AI_RAW_STREAM_PATH",
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: Nova AIConfig) =>
  ...
  const getReadiness = createReadinessChecker({
    ...
    shouldSkipChannelReadiness: () =>
      isTruthyEnvValue(process.env.NOVA_AI_SKIP_CHANNELS) ||
      isTruthyEnvValue(process.env.NOVA_AI_SKIP_PROVIDERS),
  });
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: Nova AIConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
