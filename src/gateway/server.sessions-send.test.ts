...
const { createNova AITools } = await import("../agents/nova-ai-tools.js");
...
type SessionSendTool = ReturnType<typeof createNova AITools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createNova AITools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["NOVA_AI_GATEWAY_PORT", "NOVA_AI_GATEWAY_TOKEN"]);
  ...
  process.env.NOVA_AI_GATEWAY_PORT = String(gatewayPort);
  process.env.NOVA_AI_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("NOVA_AI_CONFIG_PATH missing in gateway test environment");
      }
