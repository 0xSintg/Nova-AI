...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-gateway-ui-hardlink-"));
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "nova-ai");
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Nova AI package control-ui", async () => {
