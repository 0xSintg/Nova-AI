...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listNova AIPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listNova AIPluginManifestMetadata: (
      params?: Parameters<typeof actual.listNova AIPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listNova AIPluginManifestMetadata(params);
      return actual.listNova AIPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listNova AIPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as Nova AIConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listNova AIPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/nova-ai.plugin.json",
    ...overrides,
  };
}
