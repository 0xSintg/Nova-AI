...
function asPluginInstallConfig(records: PluginInstallRecords): Nova AIConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: Nova AIConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: Nova AIConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: Nova AIConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: Nova AIConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
