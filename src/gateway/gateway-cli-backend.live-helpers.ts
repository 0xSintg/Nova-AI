...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `NOVA_AI_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }
  ...
  if (migrated?.legacyProvider === "codex-cli") {
    throw new Error(
      "NOVA_AI_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
  if (migrated?.cli) {
    return {
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("NOVA_AI_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.NOVA_AI_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.NOVA_AI_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "NOVA_AI_LIVE_CLI_BACKEND_ARGS",
      process.env.NOVA_AI_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `NOVA_AI_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "NOVA_AI_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.NOVA_AI_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.NOVA_AI_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.NOVA_AI_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.NOVA_AI_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.NOVA_AI_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.NOVA_AI_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.NOVA_AI_SKIP_PROVIDERS = "1";
  process.env.NOVA_AI_SKIP_GMAIL_WATCHER = "1";
  process.env.NOVA_AI_SKIP_CRON = "1";
  process.env.NOVA_AI_SKIP_CANVAS_HOST = "1";
  process.env.NOVA_AI_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.NOVA_AI_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("NOVA_AI_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("NOVA_AI_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("NOVA_AI_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("NOVA_AI_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("NOVA_AI_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("NOVA_AI_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("NOVA_AI_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("NOVA_AI_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("NOVA_AI_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("NOVA_AI_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("NOVA_AI_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
