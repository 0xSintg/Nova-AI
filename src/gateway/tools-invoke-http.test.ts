...
let lastCreateNova AIToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/nova-ai-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateNova AIToolsContext?.agentTo,
          agentThreadId: lastCreateNova AIToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createNova AITools: (ctx: Record<string, unknown>) => {
      lastCreateNova AIToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateNova AIToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-nova-ai-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-nova-ai-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateNova AIToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateNova AIToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNova AIToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNova AIToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateNova AIToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateNova AIToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateNova AIToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateNova AIToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateNova AIToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-nova-ai-message-to": "channel:24514",
        "x-nova-ai-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
