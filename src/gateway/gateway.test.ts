...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "nova-ai-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".nova-ai", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["NOVA_AI_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "nova-ai");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".nova-ai");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "nova-ai-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-wizard-home-"));
      const configPath = path.join(tempHome, ".nova-ai", "nova-ai.json");
      process.env.HOME = tempHome;
      process.env.NOVA_AI_STATE_DIR = path.join(tempHome, ".nova-ai");
      process.env.NOVA_AI_CONFIG_PATH = configPath;
      process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".nova-ai", "nova-ai.json");
      const bundledPluginsDir = path.join(tempHome, "nova-ai-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.NOVA_AI_STATE_DIR = path.join(tempHome, ".nova-ai");
      process.env.NOVA_AI_CONFIG_PATH = configPath;
      process.env.NOVA_AI_SKIP_CHANNELS = "1";
      ...
      process.env.NOVA_AI_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.NOVA_AI_SKIP_PROVIDERS = "1";
      process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.NOVA_AI_TEST_MINIMAL_GATEWAY = "1";
