...
import { clearActivatedPluginRuntimeState, loadNova AIPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "nova-ai.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "nova-ai.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: Nova AIConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.nova-ai.com/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadNova AIPlugins({
    config: resolvedConfig,
