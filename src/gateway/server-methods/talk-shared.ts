...
function getVoiceCallRealtimeConfig(config: Nova AIConfig): {
  provider?: string;
...
export function getVoiceCallStreamingConfig(config: Nova AIConfig): {
  provider?: string;
...
export function buildTalkRealtimeConfig(config: Nova AIConfig, requestedProvider?: string) {
...
export function buildTalkTranscriptionConfig(config: Nova AIConfig, requestedProvider?: string) {
...
const DEFAULT_REALTIME_INSTRUCTIONS = `You are Nova AI's realtime voice interface. Keep spoken replies concise. If the user asks for code, repository state, tools, files, current Nova AI context, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`;
