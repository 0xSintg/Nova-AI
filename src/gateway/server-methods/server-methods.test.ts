  ...
  it("ignores user messages that merely contain canvas-shaped text", () => {
    const previewJson = JSON.stringify({
      ...
      view: {
        backend: "canvas",
        id: "cv_user_text",
        url: "/__nova-ai__/canvas/documents/cv_user_text/index.html",
  ...
  it("merges delayed TTS supplements into their original assistant message", () => {
    ...
    const result = projectRecentChatDisplayMessages([
      ...
      },
      {
        ...
        nova-aiTtsSupplement: { textSha256, spokenText },
        timestamp: 4,
      },
    ]);
  ...
  it("merges delayed TTS supplements when directive tags are stripped for display", () => {
    ...
    const result = projectRecentChatDisplayMessages([
      ...
      },
      {
        ...
        nova-aiTtsSupplement: { textSha256 },
        timestamp: 2,
      },
    ]);
  ...
  it("does not merge visible TTS finals into an older identical assistant message", () => {
    ...
    const result = projectRecentChatDisplayMessages([
      ...
      },
      {
        ...
        nova-aiTtsSupplement: ttsSupplement,
        timestamp: 3,
      },
    ]);
...
describe("exec approval handlers", () => {
  ...
  function createExecApprovalFixture(opts?: { config?: Nova AIConfig }) {
...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "nova-ai-logs-"));
    const older = path.join(tempDir, "nova-ai-2026-01-20.log");
    const newer = path.join(tempDir, "nova-ai-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "nova-ai-2026-01-22.log") });
  ...
  it("redacts sensitive CLI tokens from returned lines", async () => {
    const tempDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "nova-ai-logs-"));
    const file = path.join(tempDir, "nova-ai-2026-01-22.log");
