...
function expectPersistedAbortMessage(
  ...
  const abort = expectRecord(actual.nova-aiAbort, "persisted abort metadata");
...
describe("chat abort transcript persistence", () => {
  it("persists run-scoped abort partial with rpc metadata and idempotency", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("nova-ai-chat-abort-run-");
  ...
  it("persists /stop partials with stop-command metadata", async () => {
    const { transcriptPath, sessionId } = await createTranscriptFixture("nova-ai-chat-stop-");
    const respond = vi.fn();
    const context = createChatAbortContext({
  ...
  it("plain stop aborts runs tracked under the canonical session key", async () => {
    const { sessionId } = await createTranscriptFixture("nova-ai-chat-stop-canonical-");
    const respond = vi.fn();
  ...
  it("plain stop aborts raw-alias runs for the same backing session", async () => {
    const { sessionId } = await createTranscriptFixture("nova-ai-chat-stop-raw-alias-");
    const respond = vi.fn();
  ...
  it("does not match stop targets by client-supplied session id without a stored entry", async () => {
    const { sessionId } = await createMissingEntryFixture("nova-ai-chat-stop-client-session-");
    const respond = vi.fn();
