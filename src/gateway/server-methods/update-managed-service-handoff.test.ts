...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      NOVA_AI_LAUNCHD_LABEL: "com.example.nova-ai.test",
      NOVA_AI_SYSTEMD_UNIT: "nova-ai-test.service",
      NOVA_AI_WINDOWS_TASK_NAME: "Nova AI Test Gateway",
    ...
    expect(result.command).toBe("nova-ai update --yes --timeout 1800");
    ...
    expect(options.cwd).toBe("/tmp/nova-ai");
    ...
    expect(options.env.NOVA_AI_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("does not overwrite a restart sentinel owned by another startup task", async () => {
    ...
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-handoff-helper-test-"));
    tempDirs.add(tmpDir);
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "nova-ai-other-temp");
