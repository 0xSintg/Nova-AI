...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "nova-ai update --yes --timeout 1800",
  logPath: "/tmp/nova-ai-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/nova-ai-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/nova-ai-root.js")>(
    "../../infra/nova-ai-root.js",
  );
  return {
    ...actual,
    resolveNova AIPackageRoot: async () => "/tmp/nova-ai",
  };
});
...
vi.mock("./update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (timeoutMs?: number) =>
    timeoutMs
      ? `nova-ai update --yes --timeout ${Math.ceil(timeoutMs / 1000)}`
      : "nova-ai update --yes",
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
      kind: "global",
      mode: "npm",
      root: "/tmp/nova-ai-global",
      packageRoot: "/tmp/nova-ai-global",
    });
  ...
  it("keeps git/dev updates on the in-process gateway update path", async () => {
    ...
    resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
      kind: "git",
      mode: "git",
      root: "/tmp/nova-ai-git",
      packageRoot: "/tmp/nova-ai-git",
    });
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      command: "nova-ai update --yes --timeout 1800",
      message:
        "Run `nova-ai update --yes --timeout 1800` from a shell outside the gateway service.",
    });
  });
