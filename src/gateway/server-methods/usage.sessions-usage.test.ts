...
async function runSessionsUsage(
  ...
  config: Nova AIConfig = TEST_RUNTIME_CONFIG,
) {
  const respond = vi.fn();
...
describe("sessions.usage", () => {
  ...
  it("uses the requested agent for legacy specific session keys", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-usage-test-"));

    try {
      await withEnvAsync({ NOVA_AI_STATE_DIR: stateDir }, async () => {
