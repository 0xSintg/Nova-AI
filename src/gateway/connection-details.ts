...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: Nova AIConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.NOVA_AI_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.NOVA_AI_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: nova-ai doctor --fix",
