...
describe("gateway network runtime", () => {
  ...
  it("bootstraps env proxy dispatching when the gateway starts directly", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-gw-proxy-home-"));
    ...
    try {
      ...
      process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = path.join(tempHome, "empty-bundled-plugins");
      await fs.mkdir(process.env.NOVA_AI_BUNDLED_PLUGINS_DIR, { recursive: true });
