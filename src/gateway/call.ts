...
function loadGatewayConfig(): Nova AIConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: Nova AIConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.NOVA_AI_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.NOVA_AI_TEST_HANDSHAKE_TIMEOUT_MS);
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.NOVA_AI_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as Nova AIConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `nova-ai doctor` for diagnostics.";
  }
  return message;
}
