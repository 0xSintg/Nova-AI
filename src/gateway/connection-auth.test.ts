...
function cfg(input: Partial<Nova AIConfig>): Nova AIConfig {
  return input as Nova AIConfig;
}
...
const DEFAULT_ENV = {
  ...
  NOVA_AI_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when NOVA_AI env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when NOVA_AI env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when NOVA_AI env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
