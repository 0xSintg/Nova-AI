...
import type { loadNova AIPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "NOVA_AI_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: Nova AIConfig): GatewayMemoryStartupPolicy {
...
async function resolveRestartSentinelPathFast(
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.NOVA_AI_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.NOVA_AI_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".nova-ai");
  if (env.NOVA_AI_TEST_FAST === "1" || (await pathExists(newStateDir))) {
    return path.join(newStateDir, RESTART_SENTINEL_FILENAME);
  }
  const legacyStateDir = path.join(home, ".novabot");
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadNova AIPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { ensureNova AIModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureNova AIModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: Nova AIConfig;
  pluginRegistry: ReturnType<typeof loadNova AIPlugins>;
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: Nova AIConfig;
    pluginRegistry: ReturnType<typeof loadNova AIPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
