...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.NOVA_AI_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.NOVA_AI_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: Nova AIConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-nova-ai-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-nova-ai-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-nova-ai-inbound-event-kind")),
