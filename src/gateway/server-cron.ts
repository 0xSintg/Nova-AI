...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.NOVA_AI_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: Nova AIConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: Nova AIConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: Nova AIConfig, requestedAgentId: string) => {
