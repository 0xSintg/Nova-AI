export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Nova AI session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsNova AIStatus =
    normalized.includes("nova-ai is running on") ||
    /nova-ai\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("nova-ai status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsNova AIStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from nova-ai status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live nova-ai config shows") ||
