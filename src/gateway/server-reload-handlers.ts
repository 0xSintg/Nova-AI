...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: Nova AIConfig) => ChannelHealthMonitor | null;
...
> & {
  ...
  initialConfig: Nova AIConfig;
  initialCompareConfig?: Nova AIConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: Nova AIConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const markActiveMainSessionsForRestart = async (nextConfig: Nova AIConfig, reason: string) => {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: Nova AIConfig) => {
            ...
            await startGmailWatcherWithLogs({
              ...
              onSkipped: () =>
                params.logHooks.info(
                  "skipping gmail watcher restart (NOVA_AI_SKIP_GMAIL_WATCHER=1)",
                ),
            });
          }
        }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (NOVA_AI_SKIP_CHANNELS=1 or NOVA_AI_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: Nova AIConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
