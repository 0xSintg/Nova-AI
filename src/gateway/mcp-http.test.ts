  ...
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-nova-ai-account-id": "work",
        "x-nova-ai-message-channel": "telegram",
        "x-nova-ai-inbound-event-kind": "room_event",
      },
    ...
    const sendToolsList = async (inboundEventKind: string) =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-nova-ai-message-channel": "telegram",
          "x-nova-ai-inbound-event-kind": inboundEventKind,
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-nova-ai-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
  ...
  it("ignores spoofed owner headers when the bearer token is non-owner scoped", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-session-key": "agent:main:matrix:dm:test",
        "x-nova-ai-message-channel": "matrix",
        "x-nova-ai-sender-is-owner": "true",
      },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
    });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.nova-ai?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.nova-ai?.headers?.Authorization).toBe(
      "Bearer ${NOVA_AI_MCP_TOKEN}",
    );
    expect(config.mcpServers?.nova-ai?.headers?.["x-nova-ai-message-channel"]).toBe(
      "${NOVA_AI_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.nova-ai?.headers?.["x-nova-ai-sender-is-owner"]).toBeUndefined();
  });
});
