...
function gatewayTokenConfig(config: Nova AIConfig): Nova AIConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): Nova AIConfig {
  return value as Nova AIConfig;
}

function buildSnapshot(config: Nova AIConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/nova-ai-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: Nova AIConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.NOVA_AI_SKIP_CHANNELS;
  const previousSkipProviders = process.env.NOVA_AI_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.NOVA_AI_SKIP_CHANNELS;
    } else {
      process.env.NOVA_AI_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.NOVA_AI_SKIP_PROVIDERS;
    } else {
      process.env.NOVA_AI_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
  ...
  it("activates no-SecretRef startup config without importing the full secrets runtime", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "nova-ai-startup-fast-path-"));
  ...
  it("keeps the full secrets runtime path when startup config has a SecretRef", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "nova-ai-startup-secret-ref-"));
    const runtimeImport = vi.fn();
    const prepareRuntimeSecretsSnapshot = vi.fn(async ({ config }) => preparedSnapshot(config));
    const activateRuntimeSecretsSnapshot = vi.fn();
    (
  ...
  it("keeps the full secrets runtime path when auth profile files are present", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "nova-ai-startup-auth-store-"));
    writeFileSync(
