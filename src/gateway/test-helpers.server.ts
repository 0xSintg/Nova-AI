...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.NOVA_AI_CONFIG_PATH) {
    configPaths.add(process.env.NOVA_AI_CONFIG_PATH);
  }
  if (process.env.NOVA_AI_STATE_DIR) {
    configPaths.add(path.join(process.env.NOVA_AI_STATE_DIR, "nova-ai.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "nova-ai-test-no-bundled-extensions")
    : "nova-ai-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".nova-ai-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".nova-ai-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".nova-ai-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>nova-ai-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__nova-aiTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__nova-aiTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["NOVA_AI_GATEWAY_TOKEN"]);
  const prev = process.env.NOVA_AI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NOVA_AI_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.NOVA_AI_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
