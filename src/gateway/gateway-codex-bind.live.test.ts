...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.NOVA_AI_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const nova-aiDir = path.join(params.homeDir, ".nova-ai");
  await fs.mkdir(nova-aiDir, { recursive: true });
  await fs.writeFile(
    path.join(nova-aiDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.NOVA_AI_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.NOVA_AI_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.NOVA_AI_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-live-codex-bind-"));
      ...
      const bindModel =
        process.env.NOVA_AI_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("NOVA_AI_CONFIG_PATH", previous.configPath);
        restoreEnvVar("NOVA_AI_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("NOVA_AI_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("NOVA_AI_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("NOVA_AI_SKIP_CRON", previous.skipCron);
        restoreEnvVar("NOVA_AI_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("NOVA_AI_STATE_DIR", previous.stateDir);
      }
    },
