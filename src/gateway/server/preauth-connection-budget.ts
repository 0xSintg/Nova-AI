...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__nova-ai_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.NOVA_AI_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.NOVA_AI_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
