...
describe("native Gateway protocol levels", () => {
  it("match the TypeScript source of truth", async () => {
    ...
    const androidPath = "apps/android/app/src/main/java/ai/nova-ai/app/gateway/GatewayProtocol.kt";
  ...
  it("uses the min constant for native connect compatibility ranges", async () => {
    const swiftConnectFiles = [
      "apps/shared/Nova AIKit/Sources/Nova AIKit/GatewayChannel.swift",
      "apps/macos/Sources/Nova AIMacCLI/WizardCommand.swift",
    ];
    ...
    const androidPath = "apps/android/app/src/main/java/ai/nova-ai/app/gateway/GatewaySession.kt";
    const android = await readRepoFile(androidPath);
    assertPattern(
      android,
      androidPath,
