...
const hoisted = vi.hoisted(() => {
  ...
  const resolveDefaultAgentDir = vi.fn(() => "/tmp/nova-ai-state/agents/default/agent");
  ...
  const ensureNova AIModelsJson = vi.fn(async () => {});
  return {
    startPluginServices,
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/nova-ai-state",
    resolveConfigPath: vi.fn(() => "/tmp/nova-ai-state/nova-ai.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/nova-ai-state"),
  };
});
...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/nova-ai-workspace"),
...
vi.mock("../agents/models-config.js", () => ({
  ensureNova AIModelsJson: hoisted.ensureNova AIModelsJson,
}));
...
function firstEnsureModelsJsonCall(): [
  ...
  const call = hoisted.ensureNova AIModelsJson.mock.calls[0];
  if (!call || call.length < 3) {
    throw new Error("expected ensureNova AIModelsJson call");
  }
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("NOVA_AI_SKIP_CHANNELS", "0");
    vi.stubEnv("NOVA_AI_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureNova AIModelsJson.mockReset();
    hoisted.ensureNova AIModelsJson.mockResolvedValue(undefined);
  });
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-home-"));
    try {
      const nova-aiHome = path.join(osHome, "nova-ai-home");
      const stateDirFromHome = path.join(nova-aiHome, ".nova-ai");
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          NOVA_AI_HOME: "~/nova-ai-home",
        } as NodeJS.ProcessEnv),
      ).toBe(true);

      const backslashStateDir = path.resolve(`${osHome}\\nova-ai-state`);
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/nova-ai-workspace",
        deps: {} as never,
  ...
  it("prewarms models.json in the configured default agent dir", async () => {
    ...
    hoisted.resolveDefaultAgentDir.mockReturnValue("/tmp/nova-ai-state/agents/ops/agent");
    ...
    expect(hoisted.ensureNova AIModelsJson).toHaveBeenCalledTimes(1);
    const ensureCall = firstEnsureModelsJsonCall();
    expect(ensureCall[0]).toBe(cfg);
    expect(ensureCall[1]).toBe("/tmp/nova-ai-state/agents/ops/agent");
    const options = ensureCall[2];
    expect(options?.workspaceDir).toBe("/tmp/nova-ai-workspace");
  ...
  it("starts channels without waiting for primary model prewarm completion", async () => {
    await withEnvAsync(
      { NOVA_AI_SKIP_CHANNELS: undefined, NOVA_AI_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { NOVA_AI_SKIP_CHANNELS: "1", NOVA_AI_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (NOVA_AI_SKIP_CHANNELS=1 or NOVA_AI_SKIP_PROVIDERS=1)",
    );
  });
