...
type GatewayInstallAuthMode = NonNullable<NonNullable<Nova AIConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: Nova AIConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.NOVA_AI_GATEWAY_PASSWORD) ||
    normalizeOptionalString(durableServiceEnv.NOVABOT_GATEWAY_PASSWORD),
  );
}
