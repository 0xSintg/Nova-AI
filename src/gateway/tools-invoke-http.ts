...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-nova-ai-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-nova-ai-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-nova-ai-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-nova-ai-thread-id"));
