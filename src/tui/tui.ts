...
import { getRuntimeConfig, type Nova AIConfig } from "../config/config.js";
...
const NOVA_AI_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../nova-ai.mjs", import.meta.url));
const NOVA_AI_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const NOVA_AI_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const NOVA_AI_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(NOVA_AI_DIST_ENTRY_JS_PATH) || existsSync(NOVA_AI_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(NOVA_AI_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? NOVA_AI_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? NOVA_AI_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run nova-ai devices list",
      pairingHint:
        "Pairing required. Run `nova-ai devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("nova-ai tui forcing process exit after return\n");
    } catch {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__nova-ai_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "nova-ai tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("nova-ai tui forcing exit\n");
    } catch {
    ...
    void drainAndStopTuiSafely(tui)
      .catch((err) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`nova-ai tui shutdown failed: ${String(err)}\n`);
          } catch {
