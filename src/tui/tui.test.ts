...
describe("resolveInitialTuiAgentId", () => {
  const cfg: Nova AIConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/nova-ai" },
        { id: "ops", workspace: "/tmp/nova-ai/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/nova-ai/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run nova-ai devices list");
    expect(state.pairingHint).toContain("nova-ai devices list");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("nova-ai tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/nova-ai.mjs", "models", "auth", "login"],
    });
  });
});
