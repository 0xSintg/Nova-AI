...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      NOVA_AI_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when NOVA_AI_THEME=light", async () => {
    const mod = await importThemeWithEnv({ NOVA_AI_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when NOVA_AI_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ NOVA_AI_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats NOVA_AI_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ NOVA_AI_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("NOVA_AI_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      NOVA_AI_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ NOVA_AI_THEME: "light" });
    const darkMod = await importThemeWithEnv({ NOVA_AI_THEME: "dark" });
