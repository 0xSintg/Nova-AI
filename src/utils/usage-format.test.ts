...
describe("usage-format", () => {
  const originalAgentDir = process.env.NOVA_AI_AGENT_DIR;
  const originalStateDir = process.env.NOVA_AI_STATE_DIR;
  let agentDir: string;
  let stateDir: string;

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-usage-format-"));
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.NOVA_AI_AGENT_DIR = originalAgentDir;
    }
  ...
  it("prefers models.json pricing over nova-ai config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to nova-ai config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
