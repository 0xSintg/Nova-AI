...
function resolveAgentEntry(cfg: Nova AIConfig, agentId?: string) {
  if (!agentId) {
    return undefined;
  }
...
function resolveScopedExecConfig(cfg: Nova AIConfig, agentId?: string) {
...
function isYoloHostPosture(cfg: Nova AIConfig, agentId?: string): boolean {
...
export function resolveCrestodianRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Crestodian rescue is blocked because Nova AI sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  ...
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Crestodian rescue only accepts commands from an Nova AI owner.",
    };
  }
