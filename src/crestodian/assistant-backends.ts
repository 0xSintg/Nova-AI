...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => Nova AIConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): Nova AIConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): Nova AIConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
