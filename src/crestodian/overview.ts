...
type Nova AIReferencePaths = Awaited<ReturnType<typeof resolveNova AIReferencePaths>>;
...
type CrestodianOverviewDependencies = {
  ...
  resolveNova AIReferencePaths?: typeof resolveNova AIReferencePaths;
};
...
function buildAgentSummaries(cfg: Nova AIConfig): CrestodianAgentSummary[] {
...
function resolveFastTestReferences(env: NodeJS.ProcessEnv): Nova AIReferencePaths | undefined {
...
export async function loadCrestodianOverview(
  ...
  const resolveReferences = deps.resolveNova AIReferencePaths ?? resolveNova AIReferencePaths;
  ...
  return {
    ...
    references: {
      ...
      docsUrl: NOVA_AI_DOCS_URL,
      sourcePath: references.sourcePath ?? undefined,
      sourceUrl: NOVA_AI_SOURCE_URL,
    },
  };
}
