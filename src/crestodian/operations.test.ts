...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/nova-ai.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.nova-ai.com",
      sourceUrl: "https://github.com/nova-ai/nova-ai",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @nova-ai/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@nova-ai/demo",
    });
    expect(parseCrestodianOperation("plugin install clawhub:nova-ai-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:nova-ai-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall nova-ai-demo")).toEqual({
      kind: "plugin-uninstall",
      pluginId: "nova-ai-demo",
    });
  });
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/nova-ai.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("applies config set through typed deps and writes an audit entry", async () => {
    ...
    vi.stubEnv("NOVA_AI_STATE_DIR", tempDir);
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "clawhub:nova-ai-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin clawhub:nova-ai-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("clawhub:nova-ai-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin clawhub:nova-ai-demo",
      },
      { rescue: true, spec: "clawhub:nova-ai-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "nova-ai-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin nova-ai-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("nova-ai-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin nova-ai-demo",
      },
      { rescue: true, pluginId: "nova-ai-demo" },
    );
  });
