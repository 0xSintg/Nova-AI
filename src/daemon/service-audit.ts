...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds NOVA_AI_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `nova-ai gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service NOVA_AI_GATEWAY_TOKEN does not match gateway.auth.token in nova-ai.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.NOVA_AI_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.NOVA_AI_GATEWAY_TOKEN);
}
