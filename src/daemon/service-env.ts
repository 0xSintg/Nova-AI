...
function readServiceProxyEnvironment(
  ...
  const proxyUrl = normalizeOptionalString(env.NOVA_AI_PROXY_URL);
  return proxyUrl ? { NOVA_AI_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.NOVA_AI_PROFILE;
  const wrapperPath = normalizeOptionalString(env.NOVA_AI_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    NOVA_AI_PROFILE: profile,
    NOVA_AI_WRAPPER: wrapperPath,
    NOVA_AI_GATEWAY_PORT: String(port),
    NOVA_AI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    NOVA_AI_SYSTEMD_UNIT: systemdUnit,
    NOVA_AI_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    NOVA_AI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    NOVA_AI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    NOVA_AI_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.NOVA_AI_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.NOVA_AI_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    NOVA_AI_GATEWAY_TOKEN: gatewayToken,
    NOVA_AI_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
    NOVA_AI_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    NOVA_AI_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    NOVA_AI_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    NOVA_AI_WINDOWS_TASK_HIDDEN_LAUNCHER: "1",
    NOVA_AI_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    NOVA_AI_LOG_PREFIX: "node",
    NOVA_AI_SERVICE_MARKER: NODE_SERVICE_MARKER,
    NOVA_AI_SERVICE_KIND: NODE_SERVICE_KIND,
    NOVA_AI_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    NOVA_AI_STATE_DIR: sharedEnv.stateDir,
    NOVA_AI_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.NOVA_AI_STATE_DIR;
