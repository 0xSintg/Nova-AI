...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.NOVA_AI_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.NOVA_AI_PROFILE);
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** Nova AI-managed keys that must not be preserved from an old env file; stale file values
  ...
  // Nova AI-managed keys (identified by inlineManagedKeys) are excluded: a stale
