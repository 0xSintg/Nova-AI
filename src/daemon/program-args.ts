...
export const NOVA_AI_WRAPPER_ENV_KEY = "NOVA_AI_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/nova-ai -> .pnpm/nova-ai@X.Y.Z/...
...
export async function resolveNova AIWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${NOVA_AI_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveNova AIWrapperPath(params.wrapperPath);
  if (wrapperPath) {
