...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.NOVA_AI_GATEWAY_PORT).toBe("18789");
    expect(env.NOVA_AI_GATEWAY_TOKEN).toBeUndefined();
    expect(env.NOVA_AI_SERVICE_MARKER).toBe("nova-ai");
    expect(env.NOVA_AI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.NOVA_AI_SERVICE_VERSION).toBe("string");
    expect(env.NOVA_AI_SYSTEMD_UNIT).toBe("nova-ai-gateway.service");
    expect(env.NOVA_AI_WINDOWS_TASK_NAME).toBe("Nova AI Gateway");
    if (process.platform === "darwin") {
      expect(env.NOVA_AI_LAUNCHD_LABEL).toBe("ai.nova-ai.gateway");
    }
  });

  it("passes through NOVA_AI_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        NOVA_AI_WRAPPER: " /usr/local/bin/nova-ai-doppler ",
      },
      port: 18789,
    });

    expect(env.NOVA_AI_WRAPPER).toBe("/usr/local/bin/nova-ai-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".nova-ai", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", NOVA_AI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.NOVA_AI_SYSTEMD_UNIT).toBe("nova-ai-gateway-work.service");
    expect(env.NOVA_AI_WINDOWS_TASK_NAME).toBe("Nova AI Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.NOVA_AI_LAUNCHD_LABEL).toBe("ai.nova-ai.work");
    }
  });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        NOVA_AI_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.NOVA_AI_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through NOVA_AI_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NOVA_AI_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.NOVA_AI_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through NOVA_AI_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NOVA_AI_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.NOVA_AI_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits NOVA_AI_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.NOVA_AI_WINDOWS_TASK_NAME).toBe("Nova AI Node");
    expect(env.NOVA_AI_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.NOVA_AI_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".nova-ai"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", NOVA_AI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".nova-ai-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", NOVA_AI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".nova-ai"));
  });

  it("uses NOVA_AI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", NOVA_AI_STATE_DIR: "/var/lib/nova-ai" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/nova-ai"));
  });

  it("expands ~ in NOVA_AI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", NOVA_AI_STATE_DIR: "~/nova-ai-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/nova-ai-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { NOVA_AI_STATE_DIR: "C:\\State\\nova-ai" };
