...
import {
  ...
  disableNova AIUpdateLaunchdJob,
  findStaleNova AIUpdateLaunchdJobs,
  isLaunchAgentListed,
  isNova AIUpdateLaunchdLabel,
  parseLaunchctlPrint,
  parseLaunchctlListNova AIUpdateJobs,
  readLaunchAgentProgramArguments,
  readLaunchAgentRuntime,
  repairLaunchAgentBootstrap,
  removeNova AIUpdateLaunchdJob,
  restartLaunchAgent,
...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.nova-ai.gateway";
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.nova-ai.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", NOVA_AI_PROFILE: "default" },
    });
  ...
  it("parses stale Nova AI updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListNova AIUpdateJobs(
      [
        "123 0 ai.nova-ai.gateway",
        "- 127 ai.nova-ai.update.2026.5.12",
        "8142 0 ai.nova-ai.update.2026.5.13-beta.1",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      ...
      {
        label: "ai.nova-ai.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale Nova AI updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.nova-ai.update.2026.5.12\n";

      const jobs = await findStaleNova AIUpdateLaunchdJobs();
  ...
  it("recognizes only legacy Nova AI update launchd labels", () => {
    expect(isNova AIUpdateLaunchdLabel("ai.nova-ai.update.2026.5.12")).toBe(true);
    expect(isNova AIUpdateLaunchdLabel("ai.nova-ai.gateway")).toBe(false);
    expect(isNova AIUpdateLaunchdLabel("com.example.update")).toBe(false);
  });

  it.runIf(process.platform === "darwin")("removes legacy updater launchd jobs", async () => {
    await expect(removeNova AIUpdateLaunchdJob(" ai.nova-ai.update.2026.5.12 ")).resolves.toBe(
      true,
    );

    expect(state.launchctlCalls).toContainEqual(["remove", "ai.nova-ai.update.2026.5.12"]);
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentNova AIUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.nova-ai.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.nova-ai.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from Nova AI label env",
    async () => {
      await expect(
        disableCurrentNova AIUpdateLaunchdJob({
          NOVA_AI_LAUNCHD_LABEL: "ai.nova-ai.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the Nova AI update label",
    async () => {
      await expect(
        disableCurrentNova AIUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentNova AIUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.nova-ai.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentNova AIUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.nova-ai.update.2026.5.12",
          NOVA_AI_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableNova AIUpdateLaunchdJob("ai.nova-ai.update.2026.5.12")).resolves.toBe(
      true,
    );
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.nova-ai/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.nova-ai/service-env/ai.nova-ai.gateway.env";
    const wrapperPath = "/Users/test/.nova-ai/service-env/ai.nova-ai.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.nova-ai/service-env")).toBe(0o700);
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/nova-ai/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.nova-ai.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.nova-ai.gateway`;
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { NOVA_AI_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.nova-ai-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { NOVA_AI_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/nova-ai/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.nova-ai.com/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when NOVA_AI_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.nova-ai.gateway.plist",
    },
    {
      name: "uses profile-specific label when NOVA_AI_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", NOVA_AI_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.nova-ai.jbphoenix.plist",
    },
    {
      name: "prefers NOVA_AI_LAUNCHD_LABEL over NOVA_AI_PROFILE",
      env: {
        HOME: "/Users/test",
        NOVA_AI_PROFILE: "jbphoenix",
        NOVA_AI_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from NOVA_AI_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        NOVA_AI_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty NOVA_AI_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        NOVA_AI_PROFILE: "myprofile",
        NOVA_AI_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.nova-ai.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        NOVA_AI_LAUNCHD_LABEL: "../evil/label",
      }),
