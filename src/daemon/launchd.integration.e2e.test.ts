...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `nova-ai-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      NOVA_AI_LAUNCHD_LABEL: `ai.nova-ai.launchd-int-${testId}`,
      NOVA_AI_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
