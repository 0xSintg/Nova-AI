...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when NOVA_AI_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".nova-ai", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when NOVA_AI_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", NOVA_AI_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".nova-ai-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers NOVA_AI_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        NOVA_AI_PROFILE: "rescue",
        NOVA_AI_STATE_DIR: "C:\\State\\nova-ai",
      },
      expected: path.join("C:\\State\\nova-ai", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", NOVA_AI_PROFILE: "default" },
      expected: path.join("/home/test", ".nova-ai", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\nova-ai\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Nova AI Share\\node.exe" "\\\\fileserver\\Nova AI Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Nova AI Share\\node.exe",
            "\\\\fileserver\\Nova AI Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from NOVA_AI_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ NOVA_AI_STATE_DIR: path.join(tmpDir, "custom-state") }),
