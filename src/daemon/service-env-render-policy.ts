...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.NOVA_AI_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.NOVA_AI_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.NOVA_AI_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  if (!isLaunchAgentServiceEnvironment(params)) {
    return;
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    NOVA_AI_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
