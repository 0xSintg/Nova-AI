...
async function resolveNova AIPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveNova AIPackageRoot(entrypointReal) : undefined;
