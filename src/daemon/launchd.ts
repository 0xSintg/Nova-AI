...
const NOVA_AI_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.nova-ai.update.";

export type StaleNova AIUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeNova AIUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  return trimmed.startsWith(NOVA_AI_UPDATE_LAUNCHD_LABEL_PREFIX) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.NOVA_AI_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.NOVA_AI_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.NOVA_AI_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.NOVA_AI_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isNova AIUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeNova AIUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentNova AIUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.NOVA_AI_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeNova AIUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.NOVA_AI_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.NOVA_AI_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by Nova AI. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListNova AIUpdateJobs(
  output: string,
): StaleNova AIUpdateLaunchdJob[] {
  const jobs: StaleNova AIUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (!label.startsWith(NOVA_AI_UPDATE_LAUNCHD_LABEL_PREFIX)) {
      continue;
    }
...
export async function findStaleNova AIUpdateLaunchdJobs(): Promise<
  StaleNova AIUpdateLaunchdJob[]
> {
  ...
  return parseLaunchctlListNova AIUpdateJobs(result.stdout);
}

export async function removeNova AIUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeNova AIUpdateLaunchdLabel(label);
...
export async function disableNova AIUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeNova AIUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentNova AIUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentNova AIUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableNova AIUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseStrictPositiveInteger(
    command?.environment?.NOVA_AI_GATEWAY_PORT ?? "",
  );
  if (fromServiceEnv !== undefined) {
    return fromServiceEnv;
  }
  const fromEnv = parseStrictPositiveInteger(env.NOVA_AI_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.nova-ai.com/gateway",
    ].join("\n"),
  );
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `nova-ai gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.NOVA_AI_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "nova-ai gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "nova-ai gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `nova-ai gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "nova-ai gateway restart",
    });
