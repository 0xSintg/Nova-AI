...
describe("scheduleDetachedLaunchdRestartHandoff", () => {
  it("waits for the caller pid before kickstarting launchd", () => {
    ...
    expect(args[2]).toBe("nova-ai-launchd-restart-handoff");
    expect(args[6]).toBe("9876");
    expect(args[7]).toBe("ai.nova-ai.gateway");
    expect(args[1]).toContain('while kill -0 "$wait_pid" >/dev/null 2>&1; do');
    expect(args[1]).toContain("exec >>'/Users/test/.nova-ai/logs/gateway-restart.log' 2>&1");
    expect(args[1]).toContain("nova-ai restart attempt source=launchd-handoff mode=kickstart");
  ...
  it("bootouts and bootstraps for reload mode", () => {
    ...
    const [, args] = requireSpawnCall();
    expect(args[1]).toContain("nova-ai restart attempt source=launchd-handoff mode=reload");
    expect(args[1]).toContain('launchctl enable "$service_target"');
  ...
  it("sanitizes restart helper environment overrides before spawning", () => {
    ...
    expect(options.env.NOVA_AI_PROFILE).toBe("default");
