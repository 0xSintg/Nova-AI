...
declare const __NOVA_AI_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "nova-ai";
...
function readInjectedVersion(): string | undefined {
  return typeof __NOVA_AI_VERSION__ === "string" ? __NOVA_AI_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["NOVA_AI_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["NOVA_AI_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["NOVA_AI_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.NOVA_AI_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Nova AI version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.NOVA_AI_BUNDLED_VERSION,
});
