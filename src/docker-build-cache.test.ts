...
describe("docker build cache layout", () => {
  ...
  it("uses pnpm cache mounts in Dockerfiles that install repo dependencies", async () => {
    for (const path of [
      ...
      expect(
        ...
        /--mount=type=cache,id=nova-ai-pnpm-store,target=\/(?:root|home\/appuser)\/\.local\/share\/pnpm\/store,sharing=locked/,
      );
    }
  });
  ...
  it("keeps the shared e2e image on the packaged tarball install path", async () => {
    ...
    expect(dockerfile).toMatch(
      /^COPY --from=nova-ai_package --chown=appuser:appuser nova-ai-current\.tgz \/tmp\/nova-ai-current\.tgz$/m,
    );
    expect(dockerfile).toContain(
      "npm install -g --prefix /tmp/nova-ai-prefix /tmp/nova-ai-current.tgz --no-fund --no-audit",
    );
    expect(dockerfile).not.toContain(
      "cp -a /tmp/nova-ai-prefix/lib/node_modules/. /app/node_modules/",
    );
    expect(dockerfile).toContain("cp -a /tmp/nova-ai-prefix/lib/node_modules/nova-ai/. /app/");
    expect(dockerfile).toContain("rm -rf /app/node_modules/nova-ai");
    expect(dockerfile).toContain("ln -sf /app /app/node_modules/nova-ai");
  });
