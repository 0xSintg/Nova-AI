...
export function resolveNova AIManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type Nova AIManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveNova AIManifestRequires(
  metadataObj: Record<string, unknown>,
): Nova AIManifestRequires | undefined {
  const requiresRaw =
...
export function resolveNova AIManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveNova AIManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedNova AIManifestInstallBase = {
...
export function parseNova AIManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedNova AIManifestInstallBase | undefined {
  ...
  const spec: ParsedNova AIManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyNova AIManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedNova AIManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
