...
describe("shared/node-match", () => {
  ...
  it("prefers a unique current Nova AI client over a legacy novabot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=nova-ai-macos.*node=third-mac.*client=nova-ai-macos/,
    );
  });
