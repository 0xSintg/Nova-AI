...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<Nova AIConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<Nova AIConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.nova-ai.com/tools/web";
...
function rawKeyValue(config: Nova AIConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: Nova AIConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: Nova AIConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: Nova AIConfig, result: Nova AIConfig): Nova AIConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: Nova AIConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as Nova AIConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Nova AI will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.nova-ai.com/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Nova AI will store a reference instead of the API key.",
