...
import {
  ...
  type LegacyNovaBrowserProfileResidue,
...
import type { ConfigValidationIssue, Nova AIConfig } from "../config/types.nova-ai.js";
...
const BROWSER_NOVA_PROFILE_RESIDUE_CHECK_ID = "core/doctor/browser-nova-profile-residue";
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "nova-ai.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `nova-ai configure` and set Gateway mode (local/remote), or `nova-ai config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `nova-ai config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `nova-ai config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};

function resolveDoctorMode(cfg: Nova AIConfig): "local" | "remote" {
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `nova-ai doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
  ...
  async detect(ctx) {
    ...
    return detected.preview.map(
      (line): HealthFinding => ({
        ...
        fixHint: "Run `nova-ai doctor --fix` to migrate legacy state.",
      }),
    );
  },
};
    ...
    return [
      {
        ...
        path: workspaceDir,
        fixHint:
          "Inspect the legacy directories and migrate or remove them; see `nova-ai doctor` for the detailed migration prompt.",
      },
    ];
  },
};
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `nova-ai doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyNovaBrowserProfileResidue): HealthFinding {
  return {
    checkId: BROWSER_NOVA_PROFILE_RESIDUE_CHECK_ID,
    severity: "warning",
    ...
    fixHint:
      "Run `nova-ai doctor --fix` to archive the stale nova profile safely instead of deleting it in place.",
  };
}

function formatWouldArchiveBrowserResidue(residue: LegacyNovaBrowserProfileResidue): string {
  return [
    "Would archive legacy nova managed browser profile residue.",
    `- legacy profile: ${residue.legacyProfileDir}`,
    `- canonical profile: ${residue.canonicalUserDataDir}`,
  ].join("\n");
}

const browserNovaProfileResidueCheck: HealthCheck = {
  id: BROWSER_NOVA_PROFILE_RESIDUE_CHECK_ID,
  kind: "core",
  description:
    "Legacy nova managed browser profile residue has been archived after the Nova AI rename.",
  source: "doctor",
  async detect(ctx, scope) {
    const residue = await detectLegacyNovaBrowserProfileResidue(ctx.cfg, browserResidueDeps(ctx));
    if (!residue) {
      return [];
    }
  ...
  async repair(ctx) {
    ...
    if (!residue) {
      return {
        status: "skipped",
        reason: "legacy nova browser profile residue no longer exists",
        changes: [],
      };
    }
    const effect = {
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active nova-ai.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
...
export function registerCoreHealthChecks(): void {
  ...
  registerHealthCheck(browserNovaProfileResidueCheck);
...
export const CORE_HEALTH_CHECKS: readonly HealthCheck[] = [
  ...
  browserNovaProfileResidueCheck,
  finalConfigValidationCheck,
];

function detectUnavailableSkills(cfg: Nova AIConfig): SkillStatusEntry[] {
