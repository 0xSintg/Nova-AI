...
export type PromptDefaultModelResult = { model?: string; config?: Nova AIConfig };
...
function resolveConfiguredModelRaw(cfg: Nova AIConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: Nova AIConfig): string[] {
