...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("skips mutating config loads for config schema", () => {
    process.argv = ["node", "nova-ai", "config", "schema"];
  ...
  it("supports JSON5 comments and trailing commas", () => {
    writeConfig(`{
      // users commonly keep comments in nova-ai.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("returns undefined for missing or malformed config files", () => {
    process.env.NOVA_AI_CONFIG_PATH = path.join(os.tmpdir(), "nova-ai-missing-config.json");
    expect(readLoggingConfig()).toBeUndefined();
