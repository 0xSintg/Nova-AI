...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "nova-ai.log");
    const configFile = path.join(dir, "nova-ai.json");
    ...
    await fs.writeFile(
      ...
      [
        ...
        `X-Nova AI-Token: ${openClawToken}`,
        `x-pomerium-jwt-assertion: ${pomeriumJwt}`,
        "normal diagnostic line",
      ].join("\n"),
      "utf8",
    );
    process.env.NOVA_AI_CONFIG_PATH = configFile;
    ...
    expect(text).toContain("X-Nova AI-Token: supers…7890");
