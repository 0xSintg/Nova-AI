...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "nova-ai.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["nova-ai", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              NOVA_AI_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$NOVA_AI_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/nova-ai-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.NOVA_AI_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$NOVA_AI_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    expect(serialized).toContain("$NOVA_AI_STATE_DIR\\\\nova-ai.json");
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/nova-ai.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/nova-ai-sanitized.jsonl");
