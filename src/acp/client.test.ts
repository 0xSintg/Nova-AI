...
const createTempDir = () => tempDirs.make("nova-ai-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets NOVA_AI_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "nova-ai",
    });

    expect(env.NOVA_AI_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("nova-ai");
  });

  it("overrides pre-existing NOVA_AI_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      NOVA_AI_SHELL: "wrong",
    });
    expect(env.NOVA_AI_SHELL).toBe("acp-client");
  ...
  it("preserves NOVA_AI_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        NOVA_AI_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["NOVA_AI_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Nova AI bridge", () => {
    ...
    expect(env.NOVA_AI_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.NOVA_AI_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "nova-ai",
        serverArgs: ["acp"],
        defaultServerCommand: "nova-ai",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("NOVA_AI_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "nova-ai", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "nova-ai", "dist", "entry.js");
    const shimPath = path.join(dir, "nova-ai.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\nova-ai\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/nova-ai-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/nova-ai-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/nova-ai-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
