...
type ConfiguredBinding = NonNullable<Nova AIConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<Nova AIConfig>,
): Nova AIConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: Nova AIConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/nova-ai" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
