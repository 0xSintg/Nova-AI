...
function logMigrationHint(runtime: RuntimeEnv, candidate: ResolvedProviderCandidate): void {
  const command = formatCliCommand(`nova-ai migrate ${candidate.provider.id} --dry-run`);
 ...
 * prompt) is owned by `nova-ai migrate <provider>`; this helper only owns
 * the gate prompt.
 */
    ...
    try {
      ...
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. ` +
          `Re-run with ${formatCliCommand(`nova-ai migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    } finally {
