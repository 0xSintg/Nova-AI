...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: Nova AIConfig =
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.startGatewayNow", {
            command: formatCliCommand("nova-ai gateway run"),
          }),
          t("wizard.finalize.rerunInstallDaemon", {
            command: formatCliCommand("nova-ai onboard --install-daemon"),
          }),
          t("wizard.finalize.skipHealthNextTime", {
            command: formatCliCommand("nova-ai onboard --skip-health"),
          }),
        ].join("\n"),
        "Gateway",
      );
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.gatewayTokenView", {
            command: formatCliCommand("nova-ai config get gateway.auth.token"),
          }),
          t("wizard.finalize.gatewayTokenGenerate", {
            command: formatCliCommand("nova-ai doctor --generate-gateway-token"),
          }),
          t("wizard.finalize.dashboardTokenMemory"),
          t("wizard.finalize.dashboardOpenAnytime", {
            command: formatCliCommand("nova-ai dashboard --no-open"),
          }),
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("nova-ai configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchGetKey", {
            url: entry?.signupUrl ?? "https://docs.nova-ai.com/tools/web",
          }),
          t("wizard.finalize.webDocs"),
        ].join("\n"),
        t("wizard.finalize.webSearchTitle"),
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchReenable", {
            command: formatCliCommand("nova-ai configure --section web"),
          }),
          "",
          t("wizard.finalize.webDocs"),
        ].join("\n"),
