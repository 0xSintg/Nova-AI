...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: Nova AIConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: Nova AIConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: Nova AIConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): Nova AIConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe but omits auth from TUI hatch", async () => {
    const previous = process.env.NOVA_AI_GATEWAY_PASSWORD;
    process.env.NOVA_AI_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.NOVA_AI_GATEWAY_PASSWORD = previous;
      }
    }
  ...
  it("uses the resolved setup password for health checks", async () => {
    vi.stubEnv("NOVA_AI_GATEWAY_PASSWORD", "env-password");
