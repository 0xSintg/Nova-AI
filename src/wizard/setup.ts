...
async function writeWizardConfigFile(config: Nova AIConfig): Promise<Nova AIConfig> {
...
export async function runSetupWizard(
  ...
  let baseConfig: Nova AIConfig = snapshot.valid
    ? snapshot.exists
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("nova-ai doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("nova-ai doctor")}`,
        `Inspect: ${formatCliCommand("nova-ai plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("nova-ai configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: nova-ai onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.NOVA_AI_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.NOVA_AI_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: Nova AIConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
