...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: Nova AIConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => Nova AIConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: Nova AIConfig, sourceConfig: Nova AIConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: Nova AIConfig;
      effectiveConfig: Nova AIConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "nova-ai-agent-" });
}
...
function mockConfig(home: string, storePath: string): Nova AIConfig {
  const cfg = {
    agents: {
      defaults: {
