...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "nova-ai-tui") {
    return true;
  }
  return executable === "nova-ai" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
function hasWhatsappEnabled(cfg: Nova AIConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("nova-ai doctor --fix")}.`,
      );
    }
  }
