...
export const resolveNova AIPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadNova AIPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("nova-ai/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("nova-ai/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("nova-ai/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/nova-ai-root.js", () => ({
  ...
  resolveNova AIPackageRootSync: vi.fn(() => "/tmp/nova-ai"),
}));
...
beforeEach(() => {
  ...
  resolveNova AIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.NOVA_AI_STATE_DIR;
  originalUpdateInProgress = process.env.NOVA_AI_UPDATE_IN_PROGRESS;
  process.env.NOVA_AI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-doctor-state-"));
