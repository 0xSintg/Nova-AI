...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 nova-ai-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/nova-ai/dist/index.js gateway --port 18789",
        " 103 nova-ai channels",
        " 104 nova-ai tui --local",
        " 105 /usr/bin/nova-ai chat",
        " 106 helper --note 'nova-ai tui'",
        " 107 nova-ai-helper nova-ai terminal",
        " 108 nova-ai --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "nova-ai-tui" },
      { pid: 104, command: "nova-ai tui --local" },
      { pid: 105, command: "/usr/bin/nova-ai chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "nova-ai-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as Nova AIConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "nova-ai-tui" }],
