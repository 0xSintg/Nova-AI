...
function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("nova-ai cron list")} and ${formatCliCommand("nova-ai cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.nova-ai/bin/ensure-whatsapp.sh` is not maintained by current Nova AI and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("nova-ai channels status --probe")}, ${formatCliCommand("nova-ai doctor")}, and ${formatCliCommand("nova-ai gateway status")} for current health checks.`,
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  note(
    [
      ...
      ...previewLines,
      `Repair with ${formatCliCommand("nova-ai doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
