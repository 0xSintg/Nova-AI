...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("nova-ai doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("nova-ai skills check --agent <id>")} or ${formatCliCommand("nova-ai skills info <name> --agent <id>")}`,
  );
  return lines;
}
