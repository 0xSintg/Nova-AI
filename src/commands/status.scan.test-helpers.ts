...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/nova-ai-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = Nova AIConfig>(
  overrides: T = {} as T,
): Nova AIConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): Nova AIConfig {
