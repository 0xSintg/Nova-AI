...
import {
  auditNova AIPeerDependenciesInManagedNpmRoot,
...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "nova-ai.plugin.json"));
...
function listStaleManagedNpmBundledPlugins(
  ...
  for (const packageName of Object.keys(dependencies).toSorted()) {
    if (!packageName.startsWith("@nova-ai/")) {
      continue;
    }
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("nova-ai doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
...
export async function maybeRepairManagedNpmNova AIPeerLinks(
  params: PluginRegistryDoctorRepairParams,
): Promise<boolean> {
  const npmRoot = resolveManagedPluginNpmRoot(params);
  if (!params.prompter.shouldRepair) {
    const audit = await auditNova AIPeerDependenciesInManagedNpmRoot({ npmRoot });
    if (audit.broken > 0) {
      note(
        [
          "Managed npm Nova AI host peer links need repair:",
          ...audit.issues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("nova-ai doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  const result = await relinkNova AIPeerDependenciesInManagedNpmRoot({
    npmRoot,
    logger,
  });
  ...
  if (result.repaired > 0) {
    note(
      `Repaired Nova AI host peer link(s) for ${result.repaired} managed npm plugin package(s).`,
      "Plugin registry",
    );
  }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm Nova AI host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedManagedNpmNova AIPeerLinks = await maybeRepairManagedNpmNova AIPeerLinks(params);
  if (!params.prompter.shouldRepair) {
    if (preflight.action === "migrate") {
      note(
        [
          "Persisted plugin registry is missing or stale.",
          `Repair with ${formatCliCommand("nova-ai doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (
    ...
    repairedManagedNpmNova AIPeerLinks
  ) {
