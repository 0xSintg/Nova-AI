  ...
  return {
    ...
    status: {
      ...
      probeAccount: async () => ({
        ok: false,
        error:
          "imsg cannot access /Users/alice/Library/Messages/chat.db. Grant Full Disk Access to the Gateway/launcher process and restart Gateway. privateApi=/tmp/nova-ai/private.sock",
        privateApi: {
...
describe("getHealthSnapshot", () => {
  ...
  it("probes telegram getMe + webhook info when configured", async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-health-"));
