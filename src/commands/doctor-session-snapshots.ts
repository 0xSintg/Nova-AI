...
function isTempBackedNova AIRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const nova-aiIndex = lower.lastIndexOf("nova-ai");
  if (nova-aiIndex < 1) {
    return false;
  }
  return lower[nova-aiIndex - 1] === "tmp" || lower[nova-aiIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("nova-ai@"),
    ) || isTempBackedNova AIRoot(beforeSkillRoot)
  );
}
