...
import type { Nova AIConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: Nova AIConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: Nova AIConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Nova AI default (full)";
  const defaultRequestedAskSource = "Nova AI default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("nova-ai approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: Nova AIConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: Nova AIConfig): string[] {
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.nova-ai/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("nova-ai approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.nova-ai.com/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nova-ai configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nova-ai config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nova-ai doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("nova-ai config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('nova-ai config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: Nova AIConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("nova-ai security audit --deep")}`;
