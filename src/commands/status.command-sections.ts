...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("nova-ai security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("nova-ai security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.nova-ai.com/faq",
    "Troubleshooting: https://docs.nova-ai.com/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("nova-ai status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("nova-ai logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("nova-ai node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("nova-ai status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("nova-ai gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`nova-ai devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("nova-ai devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("nova-ai devices list")}`),
  ];
}
