...
export function summarizeExistingConfig(config: Nova AIConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: Nova AIConfig): string | null {
...
export function printWizardHeader(runtime: RuntimeEnv) {
  ...
  const title = supportsDecorativeEmoji() && icon ? `${icon} NOVA_AI ${icon}` : "NOVA_AI";
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    authedUrl,
    "Docs:",
    "https://docs.nova-ai.com/gateway/remote",
    "https://docs.nova-ai.com/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
