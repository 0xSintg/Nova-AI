...
function makeTempDir() {
  return makeTrackedTempDir("nova-ai-doctor-plugin-registry", tempDirs);
}
...
function createCandidate(rootDir: string, id = "demo"): PluginCandidate {
  ...
  fs.writeFileSync(
    path.join(rootDir, "nova-ai.plugin.json"),
    JSON.stringify({
      id,
      name: id,
...
function createBundledCandidate(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "nova-ai.plugin.json"),
    JSON.stringify({
      id: params.id,
      name: params.id,
      configSchema: { type: "object" },
...
function expectedPluginIndexRecord(params: {
  ...
  return {
    ...
    manifestPath: path.join(params.rootDir, "nova-ai.plugin.json"),
...
describe("maybeRepairPluginRegistryState", () => {
  ...
  it("warns about stale managed npm packages that shadow bundled plugins", async () => {
    ...
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("@nova-ai/google-meet@2026.5.2");
    expect(
      fs.existsSync(path.join(stateDir, "npm", "node_modules", "@nova-ai", "google-meet")),
    ).toBe(true);
  });
  ...
  it("removes stale managed npm packages from the package lock during repair", async () => {
    ...
    expect(packageLock.packages).not.toHaveProperty("node_modules/@nova-ai/google-meet");
    expect(packageLock.dependencies).not.toHaveProperty("@nova-ai/google-meet");
    expect(packageLock.dependencies).toHaveProperty("other-plugin");
  });

  it("repairs managed npm nova-ai peer links during registry repair", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    const linkPath = path.join(managed.packageDir, "node_modules", "nova-ai");
    expect(fs.lstatSync(linkPath).isSymbolicLink()).toBe(true);
    expect(fs.realpathSync(linkPath)).toBe(fs.realpathSync(process.cwd()));
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("Repaired Nova AI host peer link");
  });

  it("warns about broken managed npm nova-ai peer links without repairing them", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    expect(notes).toContain("Managed npm Nova AI host peer links need repair");
    expect(notes).toContain("codex-plugin");
    expect(notes).toContain("nova-ai doctor --fix");
