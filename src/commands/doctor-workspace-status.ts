...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("nova-ai tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("nova-ai tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: Nova AIConfig) {
