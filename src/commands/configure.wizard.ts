...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.NOVA_AI_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.NOVA_AI_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.nova-ai.com/gateway/health",
        "https://docs.nova-ai.com/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from nova-ai.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<Nova AIConfig["tools"]>["web"]>["search"];
  ...
  note(
    [
      ...
      "Docs: https://docs.nova-ai.com/tools/web",
    ].join("\n"),
    "Web search",
  );
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Nova AI update wizard" : "Nova AI configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: Nova AIConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.nova-ai.com/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("nova-ai doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.NOVA_AI_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.NOVA_AI_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.nova-ai.com/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.nova-ai.com/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
