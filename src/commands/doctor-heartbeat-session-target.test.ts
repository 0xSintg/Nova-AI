...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): Nova AIConfig {
  ...
  function writeStore(cfg: Nova AIConfig, entries: Record<string, unknown>) {
