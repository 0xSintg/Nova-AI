...
export async function requireValidConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Nova AI config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(`Fix: ${formatCliCommand("nova-ai doctor --fix")}`);
    runtime.error(`Inspect: ${formatCliCommand("nova-ai config validate")}`);
    runtime.exit(1);
    return null;
  }
