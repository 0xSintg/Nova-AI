...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
      return "Fix: run `nova-ai doctor --fix`.";
    case "module-missing":
      return "Fix: run `nova-ai doctor --fix`.";
    case "service-missing":
      return "Fix: run `nova-ai gateway install --force`.";
    case "service-stopped":
      return "Fix: run `nova-ai gateway restart`.";
    case "startup-blocked":
      return "Fix: run `nova-ai gateway status --deep`.";
    case "not-listening":
      return "Fix: start `nova-ai gateway run`, or run `nova-ai gateway restart` for a managed gateway.";
    default:
      return undefined;
  }
}
