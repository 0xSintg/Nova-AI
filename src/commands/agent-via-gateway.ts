...
function parseTimeoutSeconds(opts: { cfg: Nova AIConfig; timeout?: string }) {
  const raw =
...
async function agentViaGatewayCommand(opts: AgentCliOpts, runtime: RuntimeEnv) {
  ...
  if (!body) {
    throw new Error(
      `Missing message. Use ${formatCliCommand('nova-ai agent --message "..." --agent <id>')} or pass --to/--session-id for an existing conversation.`,
    );
  }
  if (!opts.to && !opts.sessionId && !opts.agent) {
    throw new Error(
      `No target session selected. Use --agent <id>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("nova-ai agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("nova-ai agents list")}" to see configured agents.`,
      );
    }
  }
