...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:nova-ai security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:nova-ai security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:nova-ai status --all",
      "  Need to debug live? cmd:nova-ai logs --follow",
      "  Fix reachability first: cmd:nova-ai gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:nova-ai devices approve req-123)",
      "muted(Fallback: cmd:nova-ai devices approve --latest)",
      "muted(Inspect: cmd:nova-ai devices list)",
    ]);
  });
