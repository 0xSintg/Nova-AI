...
function renderEstablishedGatewayConnections(connections: PortConnection[]): string {
  return [
    ...
    "If logs show protocol mismatch after rollback, stop stale Nova AI client processes listed here and rerun doctor.",
  ].join("\n");
}
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.NOVA_AI_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("nova-ai gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
