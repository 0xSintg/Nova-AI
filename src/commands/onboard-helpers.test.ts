...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".nova-ai-work");
    const defaultStateDir = path.join(homeDir, ".nova-ai");
    const profileConfigPath = path.join(profileStateDir, "nova-ai.json");
    ...
    vi.stubEnv("NOVA_AI_HOME", homeDir);
    vi.stubEnv("NOVA_AI_PROFILE", "work");
    vi.stubEnv("NOVA_AI_STATE_DIR", profileStateDir);
    vi.stubEnv("NOVA_AI_CONFIG_PATH", profileConfigPath);
