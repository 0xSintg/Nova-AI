...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.NOVA_AI_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: Nova AIConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: Nova AIConfig) => {
...
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.NOVA_AI_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<Nova AIConfig> {
