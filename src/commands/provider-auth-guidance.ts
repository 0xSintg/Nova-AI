...
function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`nova-ai models auth login --provider ${choice.providerId}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("nova-ai configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("nova-ai configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
