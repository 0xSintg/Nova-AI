...
import {
  NOVA_AI_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveNova AIWrapperPath,
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperPath = await resolveNova AIWrapperPath(
    params.wrapperPath ?? params.env[NOVA_AI_WRAPPER_ENV_KEY],
  );
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [NOVA_AI_WRAPPER_ENV_KEY]: wrapperPath }
    : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.NOVA_AI_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("nova-ai gateway install")}\` after fixing the error.`;
}
