...
type EnvSnapshot = {
  HOME?: string;
  NOVA_AI_HOME?: string;
  NOVA_AI_STATE_DIR?: string;
  NOVA_AI_OAUTH_DIR?: string;
  NOVA_AI_AGENT_DIR?: string;
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    NOVA_AI_HOME: process.env.NOVA_AI_HOME,
    NOVA_AI_STATE_DIR: process.env.NOVA_AI_STATE_DIR,
    NOVA_AI_OAUTH_DIR: process.env.NOVA_AI_OAUTH_DIR,
    NOVA_AI_AGENT_DIR: process.env.NOVA_AI_AGENT_DIR,
...
function setupSessionState(cfg: Nova AIConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("NOVA_AI_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: Nova AIConfig) {
...
async function runStateIntegrityText(cfg: Nova AIConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.NOVA_AI_HOME = tempHome;
    ...
    fs.mkdirSync(process.env.NOVA_AI_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when NOVA_AI_OAUTH_DIR is explicitly configured", async () => {
    process.env.NOVA_AI_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: Nova AIConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when NOVA_AI_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    process.env.NOVA_AI_AGENT_DIR = legacyAgentDir;
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("nova-ai tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.NOVA_AI_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.NOVA_AI_HOME = symlinkHome;
        process.env.NOVA_AI_STATE_DIR = path.join(symlinkHome, ".nova-ai");
  ...
  it("prints nova-ai-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: Nova AIConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/nova-ai sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/nova-ai sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /nova-ai sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-tui-pointer-clear-"));
    try {
