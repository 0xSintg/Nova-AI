...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Nova AI", ".nova-ai");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/nova-ai-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".nova-ai");
  ...
  it("anchors cloud detection to OS homedir when NOVA_AI_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".nova-ai");
    const originalNova AIHome = process.env.NOVA_AI_HOME;
    process.env.NOVA_AI_HOME = "/tmp/nova-ai-home-override";
