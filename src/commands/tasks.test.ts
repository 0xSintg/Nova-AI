...
import type { Nova AITestState } from "../test-utils/nova-ai-test-state.js";
...
async function withTaskCommandStateDir(
  run: (state: Nova AITestState) => Promise<void>,
): Promise<void> {
  await withNova AITestState(
    { layout: "state-only", prefix: "nova-ai-tasks-command-" },
    async (state) => {
      resetTaskRegistryDeliveryRuntimeForTests();
