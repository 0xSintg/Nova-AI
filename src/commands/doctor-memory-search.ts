...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("nova-ai doctor --fix")} or ${formatCliCommand("nova-ai memory status --fix")}`
    : `Verify: ${formatCliCommand("nova-ai memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: Nova AIConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: Nova AIConfig): boolean {
 ...
 * Runs as part of `nova-ai doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("nova-ai config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("nova-ai config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("nova-ai memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`nova-ai config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    if (isKeyOptionalMemoryProvider(resolved.provider)) {
      ...
      // not warn. A skipped probe means the user ran `nova-ai doctor` without
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("nova-ai configure --section model")}`,
        `- To disable: ${formatCliCommand("nova-ai config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("nova-ai memory status --deep")}`,
      ].join("\n"),
      "Memory search",
