...
const REASON_CODE_MESSAGES: Record<string, string> = {
  ...
  already_active: "Plugin is already active in Nova AI",
  installed: "Plugin is already installed in Nova AI",
...
function formatItemMessage(item: MigrationItem, mode: FormatMode): string | undefined {
  if (mode === "preview") {
    ...
    if (item.kind === "skill" && item.action === "copy") {
      return "Copy Codex skill into Nova AI";
    }
    if (item.kind === "plugin" && item.action === "install") {
      return "Install Codex plugin into Nova AI";
    }
...
export function assertConflictFreePlan(plan: MigrationPlan, providerId: string): void {
  if (plan.summary.conflicts > 0) {
    throw new Error(
      `Migration has ${formatCount(plan.summary.conflicts, "conflict")}. Re-run with --overwrite after reviewing nova-ai migrate plan ${providerId}.`,
    );
  }
}
...
export function writeApplyResult(
  ...
  if (result.backupPath) {
    ...
    runtime.log("Backup: skipped (no existing Nova AI state found)");
  }
  if (result.reportDir) {
