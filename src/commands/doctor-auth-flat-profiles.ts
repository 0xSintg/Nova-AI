...
function listAuthProfileRepairCandidates(cfg: Nova AIConfig): AuthProfileRepairCandidate[] {
...
function ensureConfigAuthProfiles(config: Nova AIConfig): Record<string, AuthProfileConfig> {
...
export async function maybeRepairLegacyFlatAuthProfileStores(params: {
  ...
  const noteLines = [
    ...
    ...awsSdkMarkerStores.map(
      (entry) =>
        `- ${shortenHomePath(entry.authPath)} contains aws-sdk profile markers that belong in nova-ai.json auth.profiles.`,
    ),
  ];
  if (legacyStores.length > 0) {
    noteLines.push(
      `- The gateway expects the canonical version/profiles store; ${formatCliCommand("nova-ai doctor --fix")} rewrites this legacy shape with a backup.`,
    );
  }
  if (awsSdkMarkerStores.length > 0) {
    noteLines.push(
      `- AWS SDK profile markers are routing metadata, not stored credentials; ${formatCliCommand("nova-ai doctor --fix")} moves them to config with a backup.`,
    );
  }
  note(noteLines.join("\n"), "Auth profiles");
