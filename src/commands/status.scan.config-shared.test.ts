...
describe("status.scan.config-shared", () => {
  beforeEach(() => {
    ...
    mocks.resolveConfigPath.mockReturnValue(
      `/tmp/nova-ai-status-scan-config-shared-missing-${process.pid}.json`,
    );
  });
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readBestEffortConfig,
      resolveConfig,
      env: { VITEST: "true", NOVA_AI_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      "NOVA_AI_GATEWAY_TOKEN conflicts with gateway.auth.token: Remove NOVA_AI_GATEWAY_TOKEN from the shell, ~/.nova-ai/.env, or launchctl env if gateway.auth.token is intended, or point gateway.auth.token at ${NOVA_AI_GATEWAY_TOKEN} if the env var should be canonical.",
    ]);
  });
  ...
  it("does not add a token conflict diagnostic inside the managed gateway service context", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      env: {
        VITEST: "true",
        NOVA_AI_GATEWAY_TOKEN: "env-token",
  ...
  it("does not add a status diagnostic when config uses NOVA_AI_GATEWAY_TOKEN", async () => {
    const sourceConfig = {
