...
describe("noteSessionLockHealth", () => {
  let state: Nova AITestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createNova AITestState({
      layout: "state-only",
      prefix: "nova-ai-doctor-locks-",
    });
  });
  ...
  it("removes fresh live locks when the owner is not an Nova AI process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-nova-ai-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
