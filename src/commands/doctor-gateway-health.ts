...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is Nova AI ${VERSION}; the running Gateway is Nova AI ${gatewayVersion}.`,
      "Check `nova-ai --version`, `which nova-ai`, and `nova-ai gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `nova-ai` points to the version you want, or reinstall the Gateway service from that same Nova AI install.",
    ].join("\n"),
    "Nova AI version mismatch",
  );
}
