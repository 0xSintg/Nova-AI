...
export async function agentsAddCommand(
  ...
  if (nonInteractive) {
    if (!nameInput) {
      runtime.error(
        `Agent name is required in non-interactive mode. Run ${formatCliCommand("nova-ai agents add <id> --workspace <path>")}.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (agentId === DEFAULT_AGENT_ID) {
      runtime.error(
        `"${DEFAULT_AGENT_ID}" is reserved. Choose another name, or run ${formatCliCommand("nova-ai agents list")} to inspect the default agent.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (findAgentEntryIndex(listAgentEntries(cfg), agentId) >= 0) {
      runtime.error(
        `Agent "${agentId}" already exists. Run ${formatCliCommand("nova-ai agents list")} to inspect configured agents.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  try {
    await prompter.intro("Add Nova AI agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
      ...
      if (wantsBindings) {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.nova-ai.com/concepts/multi-agent",
          ].join("\n"),
          "Routing",
        );
      }
