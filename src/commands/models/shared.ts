...
export async function loadValidConfigOrThrow(): Promise<Nova AIConfig> {
...
export async function updateConfig(
  mutator: (cfg: Nova AIConfig) => Nova AIConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: Nova AIConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: Nova AIConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("nova-ai agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
