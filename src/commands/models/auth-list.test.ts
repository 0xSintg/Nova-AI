...
vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: Nova AIConfig, agentId: string) => `/tmp/nova-ai/agents/${agentId}`,
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as Nova AIConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/nova-ai/agents/coder/auth-state.json",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/nova-ai/agents/main/auth-state.json",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state file: /tmp/nova-ai/agents/main/auth-state.json",
      "Profiles: (none)",
    ]);
  });
});
