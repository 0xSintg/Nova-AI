...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/nova-ai-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/nova-ai-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/nova-ai-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/nova-ai-dev/nova-ai.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/nova-ai-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/nova-ai-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/nova-ai-dev/nova-ai.json");
    expect(payload.auth.storePath).toBe("/tmp/nova-ai-agent/auth-profiles.json");
  ...
  it("honors NOVA_AI_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    const previous = process.env.NOVA_AI_AGENT_DIR;
    process.env.NOVA_AI_AGENT_DIR = "/tmp/nova-ai-isolated-agent";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.NOVA_AI_AGENT_DIR = previous;
      }
    }

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/nova-ai-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/nova-ai-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/nova-ai-isolated-agent/auth-profiles.json");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/nova-ai-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/nova-ai-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/nova-ai-agent-custom/auth-profiles.json",
        });
      },
    );
  });
      ...
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          ...
          effective: {
            kind: "profiles",
            detail: "/tmp/nova-ai-agent/auth-profiles.json",
          },
        },
      ]);
      expect(localRuntime.exit).not.toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/nova-ai-agent/workspace"
          ? {
