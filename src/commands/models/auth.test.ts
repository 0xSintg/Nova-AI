...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: Nova AIConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: Nova AIConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: Nova AIConfig;
  let lastUpdatedConfig: Nova AIConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/nova-ai/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/nova-ai/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/nova-ai/workspace");
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: Nova AIConfig) => Nova AIConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/nova-ai/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: Nova AIConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/nova-ai/agents/coder" : "/tmp/nova-ai/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: Nova AIConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/nova-ai/workspaces/coder" : "/tmp/nova-ai/workspace",
    );
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/nova-ai/agents/main", {
      externalCli: {
        mode: "scoped",
    ...
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai-codex:user@example.com",
      agentDir: "/tmp/nova-ai/agents/main",
    });
    ...
    expect(upsertCall.agentDir).toBe("/tmp/nova-ai/agents/main");
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with nova-ai configure --section web (recommended mode: cached). Docs: https://docs.nova-ai.com/tools/web",
    );
  });
  ...
  it("defaults OpenAI login to ChatGPT OAuth when API key is also available", async () => {
    ...
    expect(mocks.resolvePluginSetupProvider).toHaveBeenCalledWith({
      ...
      workspaceDir: "/tmp/nova-ai/workspace",
    });
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(authRunCall.agentDir).toBe("/tmp/nova-ai/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/nova-ai/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/nova-ai/agents/coder");
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/nova-ai/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/nova-ai/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/nova-ai/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `nova-ai plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic setup-token auth is supported in Nova AI.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Nova AI prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic staff told us this Nova AI path is allowed again.",
    );
  });
  ...
  it("writes pasted tokens to the requested agent store", async () => {
    ...
    expect(mocks.upsertAuthProfileWithLock).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/nova-ai/agents/coder",
    });
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "nova-ai agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/nova-ai/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/nova-ai/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/nova-ai/agents/coder");
  });
