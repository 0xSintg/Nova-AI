...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<Nova AITestState> {
  const state = await createNova AITestState({
    layout: "state-only",
    prefix: "nova-ai-doctor-oauth-sidecar-",
    env: {
      NOVA_AI_AGENT_DIR: undefined,
      NOVA_AI_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "nova-ai-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans NOVA_AI_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
