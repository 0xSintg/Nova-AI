...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/nova-ai.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@nova-ai/feishu",
          installCommand: "nova-ai plugins install @nova-ai/feishu",
          doctorFixCommand: "nova-ai doctor --fix",
          repairHint:
            "Install the official external plugin with: nova-ai plugins install @nova-ai/feishu, or run: nova-ai doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: nova-ai plugins install @nova-ai/feishu, or run: nova-ai doctor --fix.",
    );
  });
    ...
    mocks.callGateway.mockRejectedValue(
      new Error(
        [
          ...
          "Source: env NOVA_AI_GATEWAY_URL",
        ].join("\n"),
      ),
    );
    mocks.requireValidConfigSnapshot.mockResolvedValue({ secretResolved: false, channels: {} });
