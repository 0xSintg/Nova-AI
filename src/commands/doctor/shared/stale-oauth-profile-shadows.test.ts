...
describe("stale OAuth profile shadow doctor repair", () => {
  const envSnapshot = captureEnv(["NOVA_AI_STATE_DIR", "NOVA_AI_HOME"]);
  let tempRoot = "";
  let stateDir = "";

  beforeEach(async () => {
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-stale-oauth-shadow-"));
    stateDir = path.join(tempRoot, "state");
    process.env.NOVA_AI_STATE_DIR = stateDir;
    process.env.NOVA_AI_HOME = stateDir;
  });
  ...
  it("warns about stale local OAuth shadows without modifying the child store", async () => {
    ...
    expect(warnings[0]).toContain("nova-ai doctor --fix");
  ...
  it("uses the injected env for the main auth store", async () => {
    ...
    const injectedEnv = {
      ...process.env,
      NOVA_AI_STATE_DIR: injectedStateDir,
      NOVA_AI_HOME: injectedStateDir,
    };
    saveAuthProfileStore(
      storeWith(
        profileId,
  ...
  it("removes stale child OAuth shadows and local cooldown state", async () => {
    ...
    const result = await repairStaleOAuthProfileShadows({
      cfg: { agents: { list: [{ id: "telegram" }] } } satisfies Nova AIConfig,
      now,
    });
