...
vi.mock("./stale-plugin-config.js", () => ({
  scanStalePluginConfig: (cfg: {
    ...
    if (cfg.channels?.["nova-ai-weixin"]) {
      hits.push({ id: "nova-ai-weixin", surface: "channel" });
    }
    return hits.filter(
...
describe("doctor preview warnings", () => {
  ...
  it("includes stale plugin config warnings", async () => {
    ...
    expect(warning).toContain('Run "nova-ai doctor --fix"');
  ...
  it("includes stale channel config warnings without plugin config", async () => {
    ...
    expectSingleWarningContaining(warnings, "channels.nova-ai-weixin: dangling channel config");
  });
  ...
  it("includes stale OAuth profile shadow warnings", async () => {
    staleOAuthShadowState.warnings = [
      '- ~/.nova-ai/agents/telegram/agent/auth-profiles.json has stale OAuth auth profile openai-codex:default. Run "nova-ai doctor --fix".',
    ];
  ...
  it("warns but skips auto-removal when plugin discovery has errors", async () => {
    ...
    expect(warning).toContain('rerun "nova-ai doctor --fix"');
  });
