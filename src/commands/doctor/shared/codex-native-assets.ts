...
function isCodexRuntimeConfigured(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg, env).includes("codex");
}

function isCodexPluginConfigured(cfg: Nova AIConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetWarnings(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode Nova AI agents use isolated per-agent Codex homes.",
      ...
      "- Run `nova-ai migrate codex --dry-run` to inventory them. Applying that migration copies skills into the current Nova AI agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
