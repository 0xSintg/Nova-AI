...
import { isNova AIOrgNpmSpec, parseRegistryNpmSpec } from "../../../infra/npm-registry-spec.js";
import {
...
function collectConfiguredPluginIds(cfg: Nova AIConfig, env?: NodeJS.ProcessEnv): Set<string> {
...
function collectBlockedPluginIds(cfg: Nova AIConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: Nova AIConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      changes.push(
        `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "nova-ai doctor --fix" after the update completes.`,
      );
    }
  }
