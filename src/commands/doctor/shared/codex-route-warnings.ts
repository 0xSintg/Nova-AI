...
function resolveRuntime(params: {
  ...
  return (
    normalizeRuntimeString(params.env?.NOVA_AI_AGENT_RUNTIME) ??
...
function resolveImplicitDefaultAgentModelRef(cfg: Nova AIConfig): string {
...
function collectConfigModelRefs(cfg: Nova AIConfig, env?: NodeJS.ProcessEnv): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: Nova AIConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: Nova AIConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: Nova AIConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: Nova AIConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: Nova AIConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: Nova AIConfig): string[] {
  const changes: string[] = [];
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores Nova AI compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `nova-ai doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ...
    : "- Run `nova-ai doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to a PI runtime policy.";
  return [
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `nova-ai doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `nova-ai doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
}): { cfg: Nova AIConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg, params.env);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `nova-ai doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
