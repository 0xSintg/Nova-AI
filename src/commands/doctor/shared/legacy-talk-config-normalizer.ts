...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<Nova AIConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as Nova AIConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: Nova AIConfig, changes: string[]): Nova AIConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as Nova AIConfig["talk"]) ?? {};
