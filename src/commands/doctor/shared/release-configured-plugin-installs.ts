...
import { compareNova AIVersions } from "../../../config/version.js";
...
function isPluginsGloballyDisabled(cfg: Nova AIConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: Nova AIConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: Nova AIConfig): string[] {
...
function isPluginEntryDisabled(cfg: Nova AIConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: Nova AIConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: Nova AIConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: Nova AIConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: Nova AIConfig): string[] {
...
function collectConfiguredProviderIds(cfg: Nova AIConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: Nova AIConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: Nova AIConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: Nova AIConfig): string[] {
...
function addEligiblePluginId(cfg: Nova AIConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareNova AIVersions(
  ...
  const touchedComparedToRelease = compareNova AIVersions(params.touchedVersion, releaseVersion);
