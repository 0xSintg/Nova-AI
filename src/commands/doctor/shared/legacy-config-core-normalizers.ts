...
export function normalizeLegacyCommandsConfig(
  ...
  return {
    ...cfg,
    commands: commands as Nova AIConfig["commands"],
  };
}
...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as Nova AIConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as Nova AIConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<Nova AIConfig["models"]>>;
...
function normalizeLegacyCodexCliProviderRuntimePins(
  cfg: Nova AIConfig,
  changes: string[],
): { config: Nova AIConfig; changed: boolean } {
  ...
  return changed
    ? {
        config: {
          ...
          models: {
            ...rawModels,
            providers: nextProviders as NonNullable<Nova AIConfig["models"]>["providers"],
          },
        },
        changed: true,
      }
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  const nextCfg = changed
    ? {
        ...cfgWithProviders,
        agents: nextAgents as Nova AIConfig["agents"],
      }
    : cfgWithProviders;
  return nextCfg;
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<Nova AIConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as Nova AIConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<Nova AIConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyOllamaNativeNumCtxParams(
  ...
  type ProviderConfigMap = NonNullable<NonNullable<Nova AIConfig["models"]>["providers"]>;
  for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
    if (!isRecord(rawProvider)) {
      continue;
    }
