...
function listAgentRecords(cfg: Nova AIConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: Nova AIConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: Nova AIConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: Nova AIConfig): boolean {
...
function hasExplicitChannelPluginBlockerConfig(cfg: Nova AIConfig): boolean {
...
function hasConfiguredSafeBins(cfg: Nova AIConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: Nova AIConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: Nova AIConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: Nova AIConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Nova AI falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Nova AI falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: Nova AIConfig): string[] {
