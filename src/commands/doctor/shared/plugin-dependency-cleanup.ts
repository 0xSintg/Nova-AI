...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".nova-ai-runtime-deps.json" ||
    name === ".nova-ai-runtime-deps-stamp.json" ||
    name.startsWith(".nova-ai-runtime-deps-")
  );
}
...
function isLegacyDependencyDebrisName(name: string): boolean {
  return (
    ...
    name === ".nova-ai-pnpm-store" ||
    name === ".nova-ai-install-backups" ||
    name.startsWith(".nova-ai-install-stage-")
  );
}
...
async function collectLegacyPluginDependencyTargets(
  ...
  const explicitStageRoots = splitPathList(env.NOVA_AI_PLUGIN_STAGE_DIR).map((entry) =>
