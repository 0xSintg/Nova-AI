...
async function resolveCatalogChannelEntry(raw: string, cfg: Nova AIConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as Nova AIConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run nova-ai doctor --fix, then retry nova-ai channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("nova-ai channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
