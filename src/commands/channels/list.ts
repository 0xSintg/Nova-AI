...
export async function channelsListCommand(
  ...
  // there are no configured channels even though nova-ai.json has one.
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `nova-ai channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `nova-ai status` or `nova-ai models list`.",
    ),
  );
