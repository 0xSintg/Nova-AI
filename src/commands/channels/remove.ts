...
export async function channelsRemoveCommand(
  ...
  let cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as Nova AIConfig;
  ...
  if (useWizard && prompter) {
    ...
    if (!rawChannel) {
      runtime.error(
        `Missing channel. Use ${formatCliCommand("nova-ai channels remove --channel <name>")} or run ${formatCliCommand("nova-ai channels status")} to inspect configured channels.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  if (!plugin) {
    if (resolvedPluginState?.catalogEntry) {
      runtime.error(
        `Channel plugin "${resolvedPluginState.catalogEntry.id}" is not installed. Run ${formatCliCommand(`nova-ai channels add --channel ${resolvedPluginState.catalogEntry.id}`)} first.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  if (deleteConfig) {
    if (!plugin.config.deleteAccount) {
      runtime.error(
        `${formatUnsupportedChannelActionMessage({ channel, action: "delete" })} Use ${formatCliCommand("nova-ai channels remove --channel " + channel)} to disable it without deleting config.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (!plugin.config.setAccountEnabled) {
      runtime.error(
        `${formatUnsupportedChannelActionMessage({ channel, action: "disable" })} Use ${formatCliCommand("nova-ai channels remove --channel " + channel + " --delete")} only if you want to remove config.`,
      );
      runtime.exit(1);
      return;
    }
