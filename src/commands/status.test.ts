...
beforeAll(() => {
  envSnapshot = captureEnv(["NOVA_AI_PROFILE"]);
  process.env.NOVA_AI_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByNova AI: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/nova-ai/pnpm-lock.yaml",
        markerPath: "/tmp/nova-ai/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.NOVA_AI_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.nova-ai.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.nova-ai.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.NOVA_AI_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/nova-ai-root.js", () => ({
  resolveNova AIPackageRoot: vi.fn().mockResolvedValue("/tmp/nova-ai"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /nova-ai (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("nova-ai --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("NOVA_AI_GATEWAY_TOKEN", "abcd1234", async () => {
