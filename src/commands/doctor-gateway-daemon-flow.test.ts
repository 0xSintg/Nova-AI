...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.nova-ai.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.nova-ai.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        NOVA_AI_STATE_DIR: "/tmp/nova-ai-service",
        NOVA_AI_CONFIG_PATH: "/tmp/nova-ai-service/nova-ai.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { NOVA_AI_STATE_DIR?: string; NOVA_AI_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.NOVA_AI_STATE_DIR).toBe("/tmp/nova-ai-service");
    expect(handoffEnv?.NOVA_AI_CONFIG_PATH).toBe("/tmp/nova-ai-service/nova-ai.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-nova-ai/bin/nova-ai logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "nova-ai-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "nova-ai-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "nova-ai-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("nova-ai gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system Nova AI gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "nova-ai-gateway.service",
        detail: "unit: /etc/systemd/system/nova-ai-gateway.service",
        scope: "system",
        marker: "nova-ai",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level Nova AI gateway service detected while the user gateway service is not installed.",
        "- nova-ai-gateway.service (unit: /etc/systemd/system/nova-ai-gateway.service)",
        "Nova AI will not install a second user-level gateway service automatically.",
        "Run `nova-ai gateway status --deep` or `nova-ai doctor --deep` to inspect duplicate services.",
