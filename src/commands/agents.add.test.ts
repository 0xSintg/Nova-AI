...
const transformConfigWithPendingPluginInstallsMock = vi.hoisted(() =>
  vi.fn(
    async (params: {
      ...
      return {
        path: snapshot.path ?? "/tmp/nova-ai.json",
        previousHash: snapshot.hash ?? null,
...
describe("agents add command", () => {
  ...
  it("requires --workspace when flags are present", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      `Non-interactive agent creation requires --workspace. Re-run ${formatCliCommand("nova-ai agents add <id> --workspace <path>")} or omit flags to use the wizard.`,
    );
  ...
  it("copies only portable auth profiles when seeding a new agent store", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-agents-add-auth-copy-"));
    try {
  ...
  it("copies portable Codex OAuth profiles inline", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-agents-add-oauth-copy-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.NOVA_AI_STATE_DIR = previousStateDir;
      }
      await fs.rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("skips legacy sidecar-backed Codex OAuth profiles when seeding a new agent store", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-agents-add-oauth-ref-skip-"));
    const previousOAuthDir = process.env.NOVA_AI_OAUTH_DIR;
    const previousSecretKey = process.env.NOVA_AI_AUTH_PROFILE_SECRET_KEY;
    process.env.NOVA_AI_OAUTH_DIR = path.join(root, "credentials");
