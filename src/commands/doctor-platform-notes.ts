...
import { findStaleNova AIUpdateLaunchdJobs } from "../daemon/launchd.js";
...
export function collectMacLaunchAgentOverrideWarning(deps?: {
  ...
  const markerCandidates = [path.join(home, ".nova-ai", "disable-launchagent")];
...
export async function collectMacStaleNova AIUpdateLaunchdJobsWarning(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleNova AIUpdateLaunchdJobs;
}): Promise<string | null> {
  const platform = deps?.platform ?? process.platform;
  if (platform !== "darwin") {
    return null;
  }
  const jobs = await (deps?.findJobs ?? findStaleNova AIUpdateLaunchdJobs)().catch(() => []);
  if (jobs.length === 0) {
    return null;
  }

  return [
    "- Stale Nova AI updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("nova-ai gateway restart")}`,
  ].join("\n");
}

export async function noteMacStaleNova AIUpdateLaunchdJobs(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleNova AIUpdateLaunchdJobs;
  noteFn?: typeof note;
}) {
  const warning = await collectMacStaleNova AIUpdateLaunchdJobsWarning(deps);
  if (warning) {
...
function hasConfigGatewayCreds(cfg: Nova AIConfig): boolean {
...
export async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const tokenEntries = [
    ["NOVA_AI_GATEWAY_TOKEN", await getenv("NOVA_AI_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["NOVA_AI_GATEWAY_PASSWORD", await getenv("NOVA_AI_GATEWAY_PASSWORD")],
  ] as const;
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "NOVA_AI_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const staleUpdateWarning = await collectMacStaleNova AIUpdateLaunchdJobsWarning();
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.NOVA_AI_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- NOVA_AI_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
