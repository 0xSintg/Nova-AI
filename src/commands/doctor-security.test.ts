...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.NOVA_AI_GATEWAY_TOKEN;
    prevPassword = process.env.NOVA_AI_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.NOVA_AI_SERVICE_KIND;
    delete process.env.NOVA_AI_GATEWAY_TOKEN;
    delete process.env.NOVA_AI_GATEWAY_PASSWORD;
    delete process.env.NOVA_AI_SERVICE_KIND;
  });
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.NOVA_AI_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.NOVA_AI_GATEWAY_PASSWORD;
    } else {
      process.env.NOVA_AI_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.NOVA_AI_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".nova-ai"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".nova-ai", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as Nova AIConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.NOVA_AI_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as Nova AIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when NOVA_AI_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
    process.env.NOVA_AI_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("NOVA_AI_GATEWAY_TOKEN conflicts with gateway.auth.token");
    expect(message).toContain("Direct local Gateway clients commonly prefer the env token");
    expect(message).toContain("~/.nova-ai/.env");
  });

  it("does not warn when only env token is set without config token", async () => {
    process.env.NOVA_AI_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as Nova AIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("NOVA_AI_GATEWAY_TOKEN overrides");
  });
  ...
  it("does not warn inside the managed gateway service credential context", async () => {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("NOVA_AI_GATEWAY_TOKEN conflicts");
  });

  it("does not warn when config token uses NOVA_AI_GATEWAY_TOKEN SecretRef", async () => {
    process.env.NOVA_AI_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as Nova AIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as Nova AIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as Nova AIConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("nova-ai approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as Nova AIConfig);
    ...
    expect(message).toContain("Run: nova-ai security audit --deep");
  });
