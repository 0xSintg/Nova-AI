...
function hasExplicitMemorySearchConfig(cfg: Nova AIConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: Nova AIConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: Nova AIConfig, agentId: string) => { store: { path: string } } | null;
