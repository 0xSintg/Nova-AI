...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.NOVA_AI_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.NOVA_AI_GATEWAY_TOKEN);
}
