...
function isUpdateInProgress(): boolean {
  const value = process.env.NOVA_AI_UPDATE_IN_PROGRESS;
...
export function stripUnknownConfigKeys(config: Nova AIConfig): {
  ...
  const parsed = Nova AISchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: Nova AIConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: Nova AIConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: Nova AIConfig): void {
