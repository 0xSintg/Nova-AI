...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("nova-ai security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("nova-ai status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("nova-ai tasks maintenance --apply")}`,
