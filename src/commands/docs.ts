...
const SEARCH_TOOL = "https://docs.nova-ai.com/mcp.search_open_claw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.nova-ai.com");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('nova-ai docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.nova-ai.com/");
      runtime.log(`Search: ${formatCliCommand('nova-ai docs "your query"')}`);
    }
    return;
  }
