...
export const NOVA_AI_LIVE_TRANSCRIPT_MARKER_RE = /open(?:claw|cl|flaw|clar|core)/;

export function expectNova AILiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(NOVA_AI_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? NOVA_AI_LIVE_TRANSCRIPT_MARKER_RE;
