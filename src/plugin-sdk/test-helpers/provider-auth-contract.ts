...
type EnsureAuthProfileStore =
  typeof import("nova-ai/plugin-sdk/provider-auth").ensureAuthProfileStore;
type ListProfilesForProvider =
  typeof import("nova-ai/plugin-sdk/provider-auth").listProfilesForProvider;
...
function installSharedAuthProfileStoreHooks(state: { authStore: AuthProfileStore }) {
  beforeEach(() => {
    vi.doMock("nova-ai/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/provider-auth")>(
        "nova-ai/plugin-sdk/provider-auth",
      );
      return {
        ...actual,
