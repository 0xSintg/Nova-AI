...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked NOVA_AI_HOME.
  delete process.env.NOVA_AI_HOME;
  process.env.NOVA_AI_STATE_DIR = path.join(base, ".nova-ai");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "nova-ai-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".nova-ai", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
