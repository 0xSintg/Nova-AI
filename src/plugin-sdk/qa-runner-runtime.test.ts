...
const resolveNova AIPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.NOVA_AI_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveNova AIPackageRootSync.mockReset().mockReturnValue(null);
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "nova-ai-qa-test-api-root-");
    resolveNova AIPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.NOVA_AI_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.NOVA_AI_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "nova-ai-qa-runner-root-");
    resolveNova AIPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.NOVA_AI_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.NOVA_AI_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.NOVA_AI_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.NOVA_AI_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in nova-ai.plugin.json',
    );
  });
});
