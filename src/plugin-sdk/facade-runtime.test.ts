...
const originalBundledPluginsDir = process.env.NOVA_AI_BUNDLED_PLUGINS_DIR;
...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = dir;
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
...
describe("plugin-sdk facade runtime", () => {
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("nova-ai-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("nova-ai-facade-runtime-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir is partial", () => {
    const overrideDir = createTrustedBundledFixtureRoot("nova-ai-facade-runtime-empty-");
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("nova-ai-facade-identity-", "identity-check");
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("nova-ai-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("nova-ai-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("nova-ai-facade-throw-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("nova-ai-facade-runtime-enabled-");
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("nova-ai-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "nova-ai.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin public surface from package dist", () => {
    const lineDir = createTempDirSync("nova-ai-facade-global-line-dist-");
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("nova-ai-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "nova-ai.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("nova-ai-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "nova-ai.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
