...
const ORIGINAL_ENV = {
  ...
  NOVA_AI_CONFIG_PATH: process.env.NOVA_AI_CONFIG_PATH,
  NOVA_AI_STATE_DIR: process.env.NOVA_AI_STATE_DIR,
  NOVA_AI_TEST_FAST: process.env.NOVA_AI_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("nova-ai-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@nova-ai/qa-linked",
        type: "module",
        nova-ai: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@nova-ai/qa-linked",
          },
        },
      }),
      "utf8",
