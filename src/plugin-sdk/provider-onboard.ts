...
export type { Nova AIConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: Nova AIConfig, ...args: TArgs) => Nova AIConfig;
  applyConfig: (cfg: Nova AIConfig, ...args: TArgs) => Nova AIConfig;
};
...
>(params: {
  ...
  applyPreset: (cfg: Nova AIConfig, preset: TParams) => Nova AIConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: Nova AIConfig): {
  next: Nova AIConfig;
  changed: boolean;
} {
