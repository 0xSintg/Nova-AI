...
function listAgents(cfg: Nova AIConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: Nova AIConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<Nova AIConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: Nova AIConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: Nova AIConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: Nova AIConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<Nova AIConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  Nova AIConfig,
  {
    bindingsRef: Nova AIConfig["bindings"];
    agentsRef: Nova AIConfig["agents"];
    sessionRef: Nova AIConfig["session"];
...
function resolveRouteCacheForConfig(cfg: Nova AIConfig): Map<string, ResolvedAgentRoute> {
