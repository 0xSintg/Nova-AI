...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: Nova AIConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: Nova AIConfig) {
...
function resolveHeartbeatAgents(cfg: Nova AIConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: Nova AIConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: Nova AIConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: Nova AIConfig, heartbeat?: HeartbeatConfig) {
...
function usesCodexHarness(params: {
  ...
  const runtimeId =
    ...
    normalizeHeartbeatRuntimeId(process.env.NOVA_AI_AGENT_RUNTIME) ||
...
function resolveHeartbeatAckMaxChars(cfg: Nova AIConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: Nova AIConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: Nova AIConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: Nova AIConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
