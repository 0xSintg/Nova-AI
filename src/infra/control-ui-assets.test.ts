...
vi.mock("./nova-ai-root.js", () => ({
  resolveNova AIPackageRoot: vi.fn(async () => null),
  resolveNova AIPackageRootSync: vi.fn(() => null),
}));
...
let nova-aiRoot: typeof import("./nova-ai-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    nova-aiRoot = await import("./nova-ai-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/nova-ai");
    const wrapperArgv1 = abs("fixtures/bin/nova-ai");
  ...
  it("uses resolveNova AIPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/nova-ai");
    (
      nova-aiRoot.resolveNova AIPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/nova-ai"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "nova-ai" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "nova-ai.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-nova-ai");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/nova-ai-bundle");
    (
      nova-aiRoot.resolveNova AIPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Nova AI.app/Contents/MacOS/Nova AI");
    const bundledUiDir = abs("fixtures/Nova AI.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/nova-ai-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
