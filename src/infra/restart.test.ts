...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("nova-ai/plugin-sdk/test-node-mocks");
...
let triggerNova AIRestart: typeof import("./restart.js").triggerNova AIRestart;
...
beforeAll(async () => {
  ...
  ({ triggerNova AIRestart } = await import("./restart.js"));
});
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-nova-ai/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "cnova-ai", `p${stalePidB}`, "cnova-ai-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
  ...
  it("uses explicit port override when provided", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePid}`, "cnova-ai"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerNova AIRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    ...
    const result = triggerNova AIRestart();
    ...
    expect(result).toEqual({
      ...
      tried: [
        ...
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.nova-ai.gateway.plist`,
      ],
    });
  });
  ...
  it("continues when launchctl bootstrap reports the service is already loaded", () => {
    ...
    expect(result).toEqual({
      ...
      tried: [
        `launchctl kickstart -k gui/${uid}/ai.nova-ai.gateway`,
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.nova-ai.gateway.plist`,
        `launchctl kickstart gui/${uid}/ai.nova-ai.gateway`,
      ],
    });
  });
});
