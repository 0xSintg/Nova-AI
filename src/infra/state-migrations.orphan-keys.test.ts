...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".nova-ai");
...
function sharedMainOpsConfig(sharedStorePath: string): Nova AIConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: Nova AIConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { NOVA_AI_STATE_DIR: stateDir };
