...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "NOVA_AI_LIVE_ANTHROPIC_KEYS",
  "NOVA_AI_LIVE_GEMINI_KEY",
  "NOVA_AI_LIVE_OPENAI_KEY",
  "NOVA_AI_GATEWAY_TOKEN",
  "NOVA_AI_GATEWAY_PASSWORD",
  "NOVA_AI_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "NOVA_AI_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "NOVA_AI_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "NOVA_AI_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "NOVA_AI_BUNDLED_SKILLS_DIR=./attacker-skills",
  "NOVA_AI_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.nova-ai/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.nova-ai/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".nova-ai");
        process.env.NOVA_AI_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "nova-ai", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NOVA_AI_STATE_DIR=./evil-state",
            "NOVA_AI_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.NOVA_AI_STATE_DIR).toBe(stateDir);
        expect(process.env.NOVA_AI_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NOVA_AI_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "NOVA_AI_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "NOVA_AI_LIVE_GEMINI_KEY=sk-gemini-live",
            "NOVA_AI_LIVE_OPENAI_KEY=sk-openai-live",
            "NOVA_AI_GATEWAY_TOKEN=attacker-token",
            "NOVA_AI_GATEWAY_PASSWORD=attacker-password",
            "NOVA_AI_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.NOVA_AI_STATE_DIR).toBeUndefined();
  ...
  it("blocks path-override vars (NOVA_AI_AGENT_DIR, NOVA_AI_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, NOVA_AI_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "NOVA_AI_AGENT_DIR=./evil-agent",
            `NOVA_AI_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "NOVA_AI_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.NOVA_AI_AGENT_DIR;
        delete process.env.NOVA_AI_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.NOVA_AI_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NOVA_AI_AGENT_DIR).toBeUndefined();
        expect(process.env.NOVA_AI_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.NOVA_AI_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks NOVA_AI_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "NOVA_AI_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.NOVA_AI_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NOVA_AI_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "NOVA_AI_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'NOVA_AI_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.NOVA_AI_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.NOVA_AI_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NOVA_AI_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.NOVA_AI_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "NOVA_AI_PINNED_PYTHON=./attacker-python",
            "NOVA_AI_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.NOVA_AI_PINNED_PYTHON;
        delete process.env.NOVA_AI_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NOVA_AI_PINNED_PYTHON).toBeUndefined();
        expect(process.env.NOVA_AI_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NOVA_AI_PINNED_PYTHON=/trusted/python",
            "NOVA_AI_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.NOVA_AI_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.NOVA_AI_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NOVA_AI_LIVE_GEMINI_KEY=sk-gemini-live",
            "NOVA_AI_LIVE_OPENAI_KEY=sk-openai-live",
            "NOVA_AI_GATEWAY_TOKEN=trusted-token",
            "NOVA_AI_GATEWAY_PASSWORD=trusted-password",
            "NOVA_AI_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.NOVA_AI_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.NOVA_AI_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.NOVA_AI_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.NOVA_AI_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.NOVA_AI_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.NOVA_AI_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.NOVA_AI_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "NOVA_AI_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks NOVA_AI_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "NOVA_AI_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when NOVA_AI_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.NOVA_AI_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "nova-ai", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.NOVA_AI_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".novabot");
      process.env.HOME = base;
      delete process.env.NOVA_AI_STATE_DIR;
      delete process.env.NOVA_AI_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "NOVA_AI_MDNS_HOSTNAME",
          "NOVA_AI_SESSION_CACHE_TTL_MS",
          "NOVA_AI_UPDATE_PACKAGE_SPEC",
          ...
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          "NOVA_AI_DISABLE_BUNDLED_PLUGINS",
          "NOVA_AI_ALLOW_INSECURE_PRIVATE_WS",
          "NOVA_AI_BROWSER_EXECUTABLE_PATH",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "NOVA_AI_SKIP_PROVIDERS",
          "NOVA_AI_SKIP_CRON",
          "NOVA_AI_RAW_STREAM",
          "NOVA_AI_RAW_STREAM_PATH",
          "NOVA_AI_CACHE_TRACE",
          "NOVA_AI_CACHE_TRACE_FILE",
          "NOVA_AI_CACHE_TRACE_MESSAGES",
          "NOVA_AI_CACHE_TRACE_PROMPT",
          "NOVA_AI_CACHE_TRACE_SYSTEM",
          "NOVA_AI_SHOW_SECRETS",
          "NOVA_AI_PLUGIN_CATALOG_PATHS",
          "NOVA_AI_MPM_CATALOG_PATHS",
          "NOVA_AI_NODE_EXEC_HOST",
          "NOVA_AI_NODE_EXEC_FALLBACK",
          "NOVA_AI_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("nova-ai/nova-ai");
