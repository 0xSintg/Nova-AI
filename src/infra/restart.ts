...
export function emitGatewayRestart(reasonOverride?: string): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerNova AIRestart() uses schtasks to restart the gateway.
...
export function triggerNova AIRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.NOVA_AI_SYSTEMD_UNIT,
      process.env.NOVA_AI_PROFILE,
    );
  ...
  const label =
    process.env.NOVA_AI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NOVA_AI_PROFILE);
