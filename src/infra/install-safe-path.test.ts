...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@nova-ai/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@nova-ai/matrix", id: "matrix", expected: true },
    { packageName: "@nova-ai/matrix", id: "@nova-ai/matrix", expected: true },
    { packageName: "@nova-ai/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@nova-ai/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@nova-ai__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "nova-ai-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "nova-ai-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "nova-ai-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "nova-ai-install-safe-" }, async (parentDir) => {
