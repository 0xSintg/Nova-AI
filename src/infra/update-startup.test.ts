...
vi.mock("./nova-ai-root.js", async () => {
  const actual = await vi.importActual<typeof import("./nova-ai-root.js")>("./nova-ai-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "nova-ai-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv([
      "NOVA_AI_NO_AUTO_UPDATE",
      "NOVA_AI_STATE_DIR",
      "NODE_ENV",
      "VITEST",
    ]);
    process.env.NOVA_AI_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveNova AIPackageRoot } = await import("./nova-ai-root.js"));
    ...
    vi.mocked(resolveNova AIPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveNova AIPackageRoot).mockResolvedValue("/opt/nova-ai");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("nova-ai update")}`,
    );
  ...
  it("honors NOVA_AI_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.NOVA_AI_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by NOVA_AI_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by NOVA_AI_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/nova-ai/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
