...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevNova AIHome = process.env.NOVA_AI_HOME;
    const prevHome = process.env.HOME;
    process.env.NOVA_AI_HOME = "/srv/nova-ai-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/nova-ai-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevNova AIHome === undefined) {
        delete process.env.NOVA_AI_HOME;
      } else {
        process.env.NOVA_AI_HOME = prevNova AIHome;
      }
