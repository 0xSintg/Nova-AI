...
import { triggerNova AIRestart } from "./restart.js";
 ...
 * - NOVA_AI_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.NOVA_AI_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerNova AIRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.NOVA_AI_NO_RESPAWN)) {
    return { mode: "disabled", detail: "NOVA_AI_NO_RESPAWN" };
  }
  const supervisor = detectRespawnSupervisor(process.env);
  if (supervisor) {
    if (supervisor === "schtasks") {
