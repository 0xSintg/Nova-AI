...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "NOVA_AI_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "nova-ai", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("nova-ai channels login --channel whatsapp");
  });
