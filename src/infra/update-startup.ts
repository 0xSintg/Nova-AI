...
import { resolveNova AIPackageRoot } from "./nova-ai-root.js";
...
function resolveAutoUpdatePolicy(cfg: Nova AIConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: Nova AIConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.NOVA_AI_NO_AUTO_UPDATE);
  ...
  const root = await resolveNova AIPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("nova-ai update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by NOVA_AI_NO_AUTO_UPDATE", {
