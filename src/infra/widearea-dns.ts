...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NOVA_AI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nova-ai-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nova-ai";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nova-ai");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nova-ai-gw");
  ...
  records.push(`_nova-ai-gw._tcp IN PTR ${instanceLabel}._nova-ai-gw._tcp`);
  records.push(`${instanceLabel}._nova-ai-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nova-ai-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nova-ai-content-hash: ${contentHash}\n${contentBody}`;
}
