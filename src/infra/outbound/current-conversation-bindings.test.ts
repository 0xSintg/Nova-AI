...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.NOVA_AI_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-current-bindings-"));
    process.env.NOVA_AI_STATE_DIR = testStateDir;
