  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          installSpec: `@nova-ai/${channelId}`,
          installCommand: `nova-ai plugins install @nova-ai/${channelId}`,
          doctorFixCommand: "nova-ai doctor --fix",
          repairHint: `Install the official external plugin with: nova-ai plugins install @nova-ai/${channelId}, or run: nova-ai doctor --fix.`,
        }
      : null,
}));
...
describe("resolveMessageChannelSelection", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedMessage:
        "Channel is unavailable: feishu. Install the official external plugin with: nova-ai plugins install @nova-ai/feishu, or run: nova-ai doctor --fix.",
    },
    {
      params: { cfg: {} as never },
      expectedMessage:
        "Channel is required (no configured channels detected). Run nova-ai channels add to configure one",
    },
    {
      setup: () => {
        mocks.resolveOutboundChannelPlugin.mockReturnValue(undefined);
      ...
      expectedMessage:
        "Channel is required (no available channels detected). Configured official external channel WhatsApp is missing its plugin. Install the official external plugin with: nova-ai plugins install @nova-ai/whatsapp, or run: nova-ai doctor --fix.",
    },
    {
      setup: () => {
        mocks.listChannelPlugins.mockReturnValue([
