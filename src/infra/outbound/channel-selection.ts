...
function isConfiguredChannel(cfg: Nova AIConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: nova-ai doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run nova-ai channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use nova-ai channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: Nova AIConfig): Promise<boolean> {
