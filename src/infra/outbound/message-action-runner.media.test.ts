...
describe("runMessageAction media behavior", () => {
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before nova-ai#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Nova AI tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideNova AI = path.join(os.tmpdir(), "outside-nova-ai", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
