...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: Nova AIConfig | null = null;
  ...
  get(key: string, cfg: Nova AIConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: Nova AIConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: Nova AIConfig): void {
