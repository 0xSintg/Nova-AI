...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__nova-ai_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "nova-ai-root");
...
function setPackageRoot(root: string, name = "nova-ai") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./nova-ai-root.js").resolveNova AIPackageRootSync,
  asyncResolver: typeof import("./nova-ai-root.js").resolveNova AIPackageRoot,
  opts: Parameters<typeof import("./nova-ai-root.js").resolveNova AIPackageRootSync>[0],
...
vi.mock("./nova-ai-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveNova AIPackageRoot", () => {
  let resolveNova AIPackageRoot: typeof import("./nova-ai-root.js").resolveNova AIPackageRoot;
  let resolveNova AIPackageRootSync: typeof import("./nova-ai-root.js").resolveNova AIPackageRootSync;
  let clearNova AIPackageRootCaches: typeof import("./nova-ai-root.js").testing.clearNova AIPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveNova AIPackageRoot,
      resolveNova AIPackageRootSync,
      testing: { clearNova AIPackageRootCaches },
    } = await import("./nova-ai-root.js"));
  });

  beforeEach(() => {
    clearNova AIPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "nova-ai");
        const pkgRoot = path.join(project, "node_modules", "nova-ai");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "nova-ai");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "nova-ai.mjs")));
    ...
    {
      name: "falls through from a non-nova-ai moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-nova-ai");
    ...
    {
      name: "returns null for non-nova-ai package roots",
      setup: () => {
        const pkgRoot = fx("not-nova-ai");
        setPackageRoot(pkgRoot, "not-nova-ai");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "nova-ai.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "nova-ai");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
