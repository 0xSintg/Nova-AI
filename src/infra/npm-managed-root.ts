...
import { resolveNova AIPackageRootSync } from "./nova-ai-root.js";
...
type ManagedNpmRootNova AIMetadata = {
...
type ManagedNpmRootNova AIHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedNova AIMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootNova AIMetadata | undefined {
  const metadata: ManagedNpmRootNova AIMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
export async function readNova AIManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.nova-ai)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const nova-aiMetadata = buildManagedNova AIMetadata({
    current: manifest.nova-ai,
    managedOverrideKeys,
  });
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "nova-ai" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.nova-ai;
  ...
  if (isRecord(value.peerDependenciesMeta) && "nova-ai" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.nova-ai;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])nova-ai(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.nova-ai);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-managed-peer-plan-"));
  try {
    delete dependencies.nova-ai;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_nova-ai-pack-archives"),
      path.join(tempRoot, "_nova-ai-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.nova-ai).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.nova-ai)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const managedOverrideKeys = readManagedOverrideKeys(manifest.nova-ai).toSorted();
  const nova-aiMetadata = buildManagedNova AIMetadata({
    current: manifest.nova-ai,
    managedOverrideKeys,
...
export async function repairManagedNpmRootNova AIPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootNova AIHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "nova-ai" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasNova AIPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "nova-ai"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootNova AIPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall nova-ai" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall nova-ai" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootNova AIPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootNova AIHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootNova AIHostState> {
  const packageRoot =
  ...
  const managedNova AIPackageDir = path.join(params.npmRoot, "node_modules", "nova-ai");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedNova AIPackageDir),
    lstatIfExists(managedNova AIPackageDir),
  ]);
...
async function managedNpmRootLockfileHasNova AIPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "nova-ai" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/nova-ai" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "nova-ai" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootNova AIPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("nova-ai" in dependencies) {
    const { nova-ai: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if ("node_modules/nova-ai" in parsed.packages) {
        delete parsed.packages["node_modules/nova-ai"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "nova-ai" in parsed.dependencies) {
  ...
  const nova-aiPackageDir = path.join(params.npmRoot, "node_modules", "nova-ai");
  if (!params.preservePackageDir && (await pathExists(nova-aiPackageDir))) {
    await fs.rm(nova-aiPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["nova-ai", "nova-ai.cmd", "nova-ai.ps1"].map((binName) =>
