...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "nova-ai.plugin.json");
...
const DIST_RUNTIME_EXTENSION_MANIFEST = "dist-runtime/extensions/demo/nova-ai.plugin.json";
...
const DIST_NOVA_AI_ALIAS_PACKAGE = "dist/extensions/node_modules/nova-ai/package.json";
const DIST_NOVA_AI_ALIAS_PLUGIN_SDK_INDEX =
  "dist/extensions/node_modules/nova-ai/plugin-sdk/index.js";
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "nova-ai.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"nova-ai-test"}\n',
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_NOVA_AI_ALIAS_PACKAGE]:
      '{"name":"nova-ai","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "nova-ai.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "nova-ai.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "nova-ai.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "nova-ai.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "nova-ai.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          NOVA_AI_FORCE_BUILD: "1",
          NOVA_AI_RUNNER_LOG: "1",
          NOVA_AI_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[nova-ai]");
      expect(spawnCalls.at(-1)?.args).toEqual(["nova-ai.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.NOVA_AI_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "nova-ai.mjs" ? childStderr : "",
        });
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          NOVA_AI_RUNNER_LOG: "0",
          NOVA_AI_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          NOVA_AI_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched Nova AI child when requested", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          NOVA_AI_RUNNER_LOG: "0",
          NOVA_AI_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=nova-ai-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["nova-ai.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.NOVA_AI_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
      expect(fsSync.existsSync(profileDir)).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched Nova AI child when requested", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "nova-ai.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["nova-ai.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.NOVA_AI_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("passes the synthesized private QA env into runtime postbuild staging", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      expect(postBuildParams?.env?.NOVA_AI_BUILD_PRIVATE_QA).toBe("1");
      expect(postBuildParams?.env?.NOVA_AI_ENABLE_PRIVATE_QA_CLI).toBe("1");
      expect(postBuildParams?.env?.NOVA_AI_DISABLE_BUNDLED_PLUGINS).toBe("0");
    });
  });
  ...
  it("preserves an explicit bundled plugin disable flag for QA runs", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      const exitCode = await runQaCommand({
        ...
        spawnSync,
        runRuntimePostBuild,
        env: { NOVA_AI_DISABLE_BUNDLED_PLUGINS: "1" },
      });
      ...
      const postBuildParams = firstMockCall(runRuntimePostBuild)?.[0] as
        | { cwd?: string; env?: Record<string, string | undefined> }
        | undefined;
      expect(postBuildParams?.env?.NOVA_AI_DISABLE_BUNDLED_PLUGINS).toBe("1");
    });
  });
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { NOVA_AI_BUILD_PRIVATE_QA: "1" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
  ...
  it("reruns runtime postbuild in watch mode when required outputs are missing with no runtime stamp", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      await fs.rm(resolvePath(tmp, DIST_NOVA_AI_ALIAS_PACKAGE));
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./index.ts"]}}\n',
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      ...
      const runs = Promise.all([
        runStatusCommand({
          ...
          spawnSync,
          env: {
            NOVA_AI_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
          },
          runRuntimePostBuild,
        }),
        runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active nova-ai child and returns 143", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(spawnCall?.[1]).toEqual(["nova-ai.mjs", "status"]);
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_INDEX]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","nova-ai":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","nova-ai":{"extensions":["./stale.js"]}}\n',
        },
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./src/index.js"]}}\n',
          [DIST_EXTENSION_RUNTIME_SRC]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./src/index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"nova-ai":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
  ...
  it("does not require Nova AI SDK alias outputs when dist extensions are absent", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("reports missing Nova AI SDK alias outputs when runtime stamps match HEAD", async () => {
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_NOVA_AI_ALIAS_PACKAGE]:
            '{"name":"nova-ai","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
          [DIST_NOVA_AI_ALIAS_PLUGIN_SDK_INDEX]:
      ...
      await fs.rm(resolvePath(tmp, DIST_NOVA_AI_ALIAS_PLUGIN_SDK_INDEX));
    ...
    await withTempDir({ prefix: "nova-ai-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"nova-ai":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
