...
vi.mock("@nova-ai/fs-safe/config", () => ({
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.NOVA_AI_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in Nova AI", async () => {
  ...
  it("honors the Nova AI-specific env mode override", async () => {
    process.env.NOVA_AI_FS_SAFE_PYTHON_MODE = "auto";
