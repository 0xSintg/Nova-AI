...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("nova-ai-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("nova-ai-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("nova-ai-fs-safe-");
  ...
  it("writes external command output within an allowed root", async () => {
    const dir = await tempDirs.make("nova-ai-fs-safe-output-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("nova-ai-fs-safe-root-");
    const sourceDir = await tempDirs.make("nova-ai-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("nova-ai-fs-safe-root-");
    const outside = await tempDirs.make("nova-ai-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const originalNova AIHome = process.env.NOVA_AI_HOME;
    const fakeHome = path.resolve(path.sep, "tmp", "fake-home-test");
    process.env.HOME = fakeHome;
    process.env.NOVA_AI_HOME = fakeHome;
    try {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    } finally {
      process.env.HOME = originalHome;
      process.env.NOVA_AI_HOME = originalNova AIHome;
    }

    const root = await tempDirs.make("nova-ai-tilde-test-");
    process.env.HOME = root;
    process.env.NOVA_AI_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("nova-ai-tilde-outside-");
    await expectRejectCode(
