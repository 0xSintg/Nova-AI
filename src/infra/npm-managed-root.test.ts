...
async function makeTempRoot(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-npm-managed-root-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("managed npm root", () => {
  ...
  it("syncs Nova AI-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads package-level npm overrides for managed plugin installs", async () => {
    await expect(readNova AIManagedNpmRootOverrides()).resolves.toEqual({
      axios: "1.16.0",
      "fast-uri": "3.1.2",
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readNova AIManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "@aws-sdk/client-bedrock-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        ...
        resolution: {
          name: "@nova-ai/discord",
          version: "2026.5.2",
          resolvedSpec: "@nova-ai/discord@2026.5.2",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@nova-ai/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@nova-ai/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for nova-ai@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed nova-ai peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    await fs.mkdir(path.join(npmRoot, "node_modules", "nova-ai"), { recursive: true });
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
        {
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "nova-ai", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nova-ai"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nova-ai.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nova-ai.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootNova AIPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@nova-ai/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.nova-ai).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "nova-ai"));
    for (const binName of ["nova-ai", "nova-ai.cmd", "nova-ai.ps1"]) {
  ...
  it("does not repair the active Nova AI host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/nova-ai-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "nova-ai", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "nova-ai"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nova-ai"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nova-ai.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nova-ai.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/nova-ai-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "nova-ai"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/nova-ai"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/nova-ai-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.nova-ai).toBeUndefined();
    for (const binName of ["nova-ai", "nova-ai.cmd", "nova-ai.ps1"]) {
