...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.NOVA_AI_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Nova AI. You can keep running the web gateway without it: \`${formatCliCommand("nova-ai gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
