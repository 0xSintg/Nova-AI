...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/nova-ai-state", "devices")).toEqual({
      dir: path.join("/tmp/nova-ai-state", "devices"),
      pendingPath: path.join("/tmp/nova-ai-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/nova-ai-state", "devices", "paired.json"),
    });
  });
