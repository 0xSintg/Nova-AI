...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Nova AI to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "nova-ai.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.NOVA_AI_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.NOVA_AI_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set NOVA_AI_APNS_TEAM_ID and NOVA_AI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.NOVA_AI_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.NOVA_AI_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.NOVA_AI_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set NOVA_AI_APNS_PRIVATE_KEY_P8 or NOVA_AI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading NOVA_AI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
