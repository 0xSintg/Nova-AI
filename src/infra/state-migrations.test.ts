...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: Nova AIConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: Nova AIConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.NOVA_AI_STATE_DIR;
        if (!root) {
          return [];
        }
...
const createTempDir = () => tempDirs.make("nova-ai-state-migrations-test-");
