...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "nova-ai-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      NOVA_AI_DIAGNOSTICS: "timeline",
      NOVA_AI_DIAGNOSTICS_RUN_ID: "run-1",
      NOVA_AI_DIAGNOSTICS_ENV: "env-1",
      NOVA_AI_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, NOVA_AI_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NOVA_AI_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, NOVA_AI_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.NOVA_AI_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as Nova AIConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as Nova AIConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as Nova AIConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("nova-ai.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.NOVA_AI_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as Nova AIConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
