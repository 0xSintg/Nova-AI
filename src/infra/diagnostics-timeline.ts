...
const NOVA_AI_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "nova-ai.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.NOVA_AI_DIAGNOSTICS)) &&
    typeof env.NOVA_AI_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.NOVA_AI_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: NOVA_AI_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.NOVA_AI_DIAGNOSTICS_RUN_ID ? { runId: env.NOVA_AI_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.NOVA_AI_DIAGNOSTICS_ENV ? { envName: env.NOVA_AI_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.NOVA_AI_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
