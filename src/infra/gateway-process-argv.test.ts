...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Nova AI\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/nova-ai/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/nova-ai/nova-ai.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/nova-ai/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/nova-ai/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the nova-ai executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\nova-ai.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/nova-ai-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/nova-ai-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\nova-ai-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/nova-ai/custom.js", "gateway"])).toBe(false);
