...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "nova-ai-backup-2026-01-01",
    archivePath: "/tmp/nova-ai-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/nova-ai-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.nova-ai",
        "Skipped 1 path:",
        "- workspace: ~/Projects/nova-ai (covered by ~/.nova-ai)",
        "Created /tmp/nova-ai-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.nova-ai/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.nova-ai/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.nova-ai/config.json",
        "- credentials: ~/.nova-ai/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/nova-ai-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.nova-ai",
      "Created /tmp/nova-ai-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/nova-ai.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.nova-ai\\");

    expect(filter(String.raw`C:\Users\me\.nova-ai\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.nova-ai\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("skips current live volatile state files while preserving workspace locks", async () => {
    await withNova AITestState(
      {
        layout: "split",
        prefix: "nova-ai-backup-volatile-",
        scenario: "minimal",
      },
      async (state) => {
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withNova AITestState(
      {
        layout: "state-only",
        prefix: "nova-ai-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "nova-ai.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/nova-ai.plugin.json");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withNova AITestState(
      {
        layout: "state-only",
        prefix: "nova-ai-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withNova AITestState(
      {
        layout: "state-only",
        prefix: "nova-ai-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
