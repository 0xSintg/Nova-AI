...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "nova-ai-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".nova-ai");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".nova-ai", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      NOVA_AI_STATE_DIR: path.join(home, ".nova-ai"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "nova-ai.json"),
