...
export const POSIX_NOVA_AI_TMP_DIR = "/tmp/nova-ai";
...
export type ResolvePreferredNova AITmpDirOptions = {
...
export function resolvePreferredNova AITmpDir(
  options: ResolvePreferredNova AITmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "nova-ai" : `nova-ai-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[nova-ai] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Nova AI temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Nova AI temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_NOVA_AI_TMP_DIR;
