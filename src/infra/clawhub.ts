...
// Keep aligned with @nova-ai/plugin-package-contract ExternalPluginCompatibility.
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.NOVA_AI_CLAWHUB_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.NOVA_AI_CLAWHUB_CONFIG_PATH) ||
    normalizeOptionalString(process.env.CLAWHUB_CONFIG_PATH) ||
    normalizeOptionalString(process.env.NOVAHUB_CONFIG_PATH); // legacy misspelling from older clawhub CLI builds; keep for back-compat
  if (explicit) {
    return [explicit];
  }
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.NOVA_AI_CLAWHUB_TOKEN) ||
...
const NOVA_AI_CALVER_STABLE_CORRECTION_PATTERN = /^[vV]?(\d{4}\.\d{1,2}\.\d{1,2})-\d+$/;

function normalizeCalVerCorrectionForPluginApi(pluginApiVersion: string): string {
  const match = NOVA_AI_CALVER_STABLE_CORRECTION_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubPackageArchive(params: {
  ...
  if (params.artifact === "clawpack") {
    ...
    const target = await createTempDownloadTarget({
      prefix: "nova-ai-clawhub-clawpack",
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "nova-ai-clawhub-package",
