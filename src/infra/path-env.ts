...
function candidateBinDirs(opts: EnsureNova AIPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Nova AI process is on.
  try {
  ...
  // Bundled macOS app: `nova-ai` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "nova-ai");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NOVA_AI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "nova-ai"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `nova-ai`
...
/**
 * Best-effort PATH bootstrap so skills that require the `nova-ai` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNova AICliOnPath(opts: EnsureNova AIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NOVA_AI_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.NOVA_AI_PATH_BOOTSTRAPPED = "1";
