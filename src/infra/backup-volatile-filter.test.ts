...
const stateDir = "/opt/nova-ai/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/nova-ai/state/sessions/../config.jsonl` resolves to
    // `/opt/nova-ai/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/nova-ai/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
