...
import { bundledDistPluginFile } from "nova-ai/plugin-sdk/test-fixtures";
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "nova-ai", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["NOVA_AI_UPDATE_PACKAGE_SPEC"]);
    process.env.NOVA_AI_UPDATE_PACKAGE_SPEC = "file:/tmp/nova-ai.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "nova-ai", tag: "latest" })).toBe(
      "file:/tmp/nova-ai.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "nova-ai",
        tag: "beta",
        env: { NOVA_AI_UPDATE_PACKAGE_SPEC: "nova-ai@next" },
      }),
    ).toBe("nova-ai@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "nova-ai"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "nova-ai", tag: "main" })).toBe(
      NOVA_AI_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "nova-ai",
        tag: "github:nova-ai/nova-ai#feature/my-branch",
      }),
    ).toBe("github:nova-ai/nova-ai#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "nova-ai",
        tag: "https://example.com/nova-ai-main.tgz",
      }),
    ).toBe("https://example.com/nova-ai-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "nova-ai-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:nova-ai/nova-ai#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/nova-ai-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/nova-ai-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:nova-ai/nova-ai#main")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "nova-ai-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "nova-ai");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "nova-ai"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "nova-ai"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "nova-ai"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "nova-ai"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nova-ai-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "nova-ai@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "nova-ai@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "nova-ai@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "nova-ai@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "nova-ai-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "nova-ai-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".nova-ai", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "nova-ai");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "nova-ai"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "nova-ai-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "nova-ai");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "nova-ai-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "nova-ai");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "nova-ai-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "nova-ai");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "nova-ai-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "nova-ai@file+..+pack+nova-ai-2026.5.6.tgz",
        "node_modules",
        "nova-ai",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "nova-ai"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "nova-ai-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "nova-ai");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "nova-ai"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "nova-ai@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "nova-ai@latest",
    ...
    expect(globalInstallArgs("pnpm", "nova-ai@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "nova-ai@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:nova-ai/nova-ai#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=nova-ai",
      "github:nova-ai/nova-ai#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "nova-ai@git+https://github.com/nova-ai/nova-ai.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=nova-ai",
      "nova-ai@git+https://github.com/nova-ai/nova-ai.git",
    ]);
    expect(globalInstallArgs("bun", "nova-ai@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "nova-ai@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "nova-ai@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "nova-ai@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "nova-ai@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "nova-ai@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "nova-ai@latest"]);
    expect(globalInstallArgs("pnpm", "nova-ai@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "nova-ai@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "nova-ai@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/nova-ai/lib/node_modules")).toEqual({
      prefix: "/opt/nova-ai",
      globalRoot: "/opt/nova-ai/lib/node_modules",
      binDir: "/opt/nova-ai/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "nova-ai-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".nova-ai-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".nova-ai-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".nova-ai-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "nova-ai"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".nova-ai-123", ".nova-ai-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "nova-ai"));
      const markerFileStat = await fs.stat(path.join(root, ".nova-ai-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "nova-ai-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "nova-ai-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".nova-ai-install-stage", ".nova-ai-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.nova-ai-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.nova-ai-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "nova-ai-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "nova-ai");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "nova-ai-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "nova-ai-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "nova-ai-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "nova-ai-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@nova-ai/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "nova-ai-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@nova-ai/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "nova-ai-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@nova-ai/qa-lab");
        await writePackageDistInventory(packageRoot);
