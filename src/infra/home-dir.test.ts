...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers NOVA_AI_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for NOVA_AI_HOME",
      env: { NOVA_AI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when NOVA_AI_HOME is tilde-only and no fallback home exists",
      env: { NOVA_AI_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores NOVA_AI_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          NOVA_AI_HOME: "/srv/nova-ai-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/nova-ai-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/nova-ai-home " },
      expected: "/srv/nova-ai-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/nova-ai-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of NOVA_AI_HOME", () => {
    expect(
