...
import {
  ...
  isNova AIOrgNpmSpec,
  isNova AIStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@nova-ai/voice-call",
    "@nova-ai/voice-call@1.2.3",
    "@nova-ai/voice-call@1.2.3-beta.4",
    "@nova-ai/voice-call@latest",
    "@nova-ai/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    ...
    {
      spec: "git+ssh://github.com/nova-ai/nova-ai",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@nova-ai/voice-call@2026.5.3-1",
      expected: {
        name: "@nova-ai/voice-call",
        raw: "@nova-ai/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@nova-ai/voice-call@1.2.3-beta.1",
      expected: {
        name: "@nova-ai/voice-call",
        raw: "@nova-ai/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@nova-ai/voice-call", expected: true },
    { spec: "@nova-ai/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@nova-ai/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects Nova AI-org npm specs for %s", ({ spec, expected }) => {
    expect(isNova AIOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects Nova AI stable correction versions for %s", ({ value, expected }) => {
    expect(isNova AIStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares Nova AI release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareNova AIReleaseVersions(left, right)).toBe(expected);
  });
});
...
describe("npm prerelease resolution policy", () => {
  it.each([
    ...
    {
      spec: "@nova-ai/voice-call@latest",
  ...
  it.each([
    {
      spec: "@nova-ai/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@nova-ai/voice-call@beta"`,
    },
    {
      spec: "@nova-ai/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
