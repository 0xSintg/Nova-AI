...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.nova-ai.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const nova-aiPayload = requireRecord(payload.nova-ai, "nova-ai payload");
    expectRecordFields(nova-aiPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof nova-aiPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.nova-ai.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open Nova AI to review this request.",
      },
      sound: "default",
      category: "nova-ai.exec-approval",
      "content-available": 1,
    });
    const nova-aiPayload = requireRecord(payload.nova-ai, "nova-ai payload");
    expectRecordFields(nova-aiPayload, {
    ...
    expect(typeof nova-aiPayload.ts).toBe("number");
    expectNoProperties(nova-aiPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.nova-ai, "nova-ai payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
