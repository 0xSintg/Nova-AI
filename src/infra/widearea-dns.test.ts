...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 18789,
  displayName: "Mac Studio (Nova AI)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "nova-ai.internal", expected: "nova-ai.internal." },
    { value: "nova-ai.internal.", expected: "nova-ai.internal." },
    { value: "  nova-ai.internal  ", expected: "nova-ai.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { NOVA_AI_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { NOVA_AI_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("nova-ai.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "nova-ai.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN nova-ai.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_nova-ai-gw._tcp IN PTR studio-london._nova-ai-gw._tcp`,
      `studio-london._nova-ai-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Nova AI)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/nova-ai`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN nova-ai.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._nova-ai-gw._tcp IN TXT`,
        `displayName=Mac Studio (Nova AI)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("nova-ai.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("nova-ai.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
