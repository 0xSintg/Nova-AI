 ...
 * returning control to the caller (typically `triggerNova AIRestart` →
...
function parsePidsFromLsofOutput(stdout: string, spawnTimeoutMs: number): number[] {
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("nova-ai")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an nova-ai
 ...
 * Returns only PIDs that belong to nova-ai gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only nova-ai gateway processes.
