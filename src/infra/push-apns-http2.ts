  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.nova-ai.proxy.validation",
        "apns-topic": "ai.nova-ai.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "Nova AI APNs proxy validation" } }));
    });
  } finally {
