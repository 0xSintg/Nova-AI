...
const mockedModuleIds = [
  "@nova-ai/proxyline/dispatcher-brand",
  "node:net",
  "./proxy-env.js",
  "./undici-runtime.js",
  "../wsl.js",
...
vi.mock("@nova-ai/proxyline/dispatcher-brand", () => ({
  isProxylineDispatcher,
}));
...
describe("forceResetGlobalDispatcher", () => {
  ...
  it("restores a direct Agent when clearing a proxy dispatcher installed by Nova AI", () => {
    vi.mocked(hasEnvHttpProxyAgentConfigured).mockReturnValue(true);
    ensureGlobalUndiciEnvProxyDispatcher();
    expect(getCurrentDispatcher()).toBeInstanceOf(EnvHttpProxyAgent);
