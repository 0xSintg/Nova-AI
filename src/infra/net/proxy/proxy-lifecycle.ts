/**
 * High-level lifecycle management for Nova AI's operator-managed network
 * proxy routing.
 *
 * Nova AI does not spawn or configure the filtering proxy. When enabled, it
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    NOVA_AI_PROXY_ACTIVE: process.env["NOVA_AI_PROXY_ACTIVE"],
    NOVA_AI_PROXY_LOOPBACK_MODE: process.env["NOVA_AI_PROXY_LOOPBACK_MODE"],
    NOVA_AI_PROXY_CA_FILE: process.env["NOVA_AI_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["NOVA_AI_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["NOVA_AI_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["NOVA_AI_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["NOVA_AI_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or NOVA_AI_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["NOVA_AI_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["NOVA_AI_PROXY_CA_FILE"],
  });
