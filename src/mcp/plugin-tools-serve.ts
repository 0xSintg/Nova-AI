/**
 * Standalone MCP server that exposes Nova AI plugin-registered tools
...
function resolvePluginToolPolicy(config: Nova AIConfig): {
...
function resolveTools(config: Nova AIConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "nova-ai-plugin-tools", tools });
}
