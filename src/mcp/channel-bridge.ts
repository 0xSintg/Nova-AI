...
export class Nova AIChannelBridge {
  ...
  constructor(
    private readonly cfg: Nova AIConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`nova-ai mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `nova-ai mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
