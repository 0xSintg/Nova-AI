...
import { createNova AIChannelMcpServer, Nova AIChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createNova AIChannelMcpServer({
...
describe("nova-ai channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new Nova AIChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__nova-ai"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
