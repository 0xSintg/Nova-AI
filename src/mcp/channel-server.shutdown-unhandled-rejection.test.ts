...
vi.mock("./channel-bridge.js", () => ({
  Nova AIChannelBridge: class MockNova AIChannelBridge {
...
describe("serveNova AIChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveNova AIChannelMcp } = await import("./channel-server.js");

    const servePromise = serveNova AIChannelMcp({ verbose: false });
