...
val androidStoreFile = providers.gradleProperty("NOVA_AI_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("NOVA_AI_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("NOVA_AI_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("NOVA_AI_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set NOVA_AI_ANDROID_STORE_FILE, " +
      "NOVA_AI_ANDROID_STORE_PASSWORD, NOVA_AI_ANDROID_KEY_ALIAS, and " +
      "NOVA_AI_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/NovaAIKit/Sources/NovaAIKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.novaai.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "nova-ai-$versionName-$buildType.apk"
          } else {
            "nova-ai-$versionName-$flavorName-$buildType.apk"
          }
