...
private const val NOTIFICATION_CHANNEL_BASE_ID = "novaai.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Nova AI Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Nova AI Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Nova AI Active")
      }
