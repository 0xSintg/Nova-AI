# Nova AI iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Nova AI Gateway as a `role: node`.
...
- Beta release uses canonical `ai.nova-ai.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `Nova AIPushTransport=relay`, `Nova AIPushDistribution=official`, and `Nova AIPushAPNsEnvironment=production`.
...
- `NOVA_AI_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export NOVA_AI_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Nova AI-<version>.ipa`
   - `apps/ios/build/beta/Nova AI-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Nova AI.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `Nova AIPushTransport=direct` and `Nova AIPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `NOVA_AI_APNS_TEAM_ID`, `NOVA_AI_APNS_KEY_ID`, and either `NOVA_AI_APNS_PRIVATE_KEY_P8` or `NOVA_AI_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.nova-ai/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `NOVA_AI_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `Nova AIPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `nova-ai.json`. `NOVA_AI_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Nova AI official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Nova AI node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
