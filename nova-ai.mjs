...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `nova-ai: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[nova-ai] Failed to respawn launcher: ${
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.NOVA_AI_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.NOVA_AI_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    NOVA_AI_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["nova-ai: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:nova-ai/nova-ai#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g nova-ai@latest`.");
  return lines.join("\n");
};
...
const isHelpFastPathDisabled = () =>
  process.env.NOVA_AI_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.NOVA_AI_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.NOVA_AI_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.NOVA_AI_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "nova-ai.json"), path.join(stateDir, "novabot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".nova-ai", "nova-ai.json"),
    path.join(homeDir, ".nova-ai", "novabot.json"),
    path.join(homeDir, ".novabot", "nova-ai.json"),
    path.join(homeDir, ".novabot", "novabot.json"),
  ];
};

const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.NOVA_AI_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.NOVA_AI_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
