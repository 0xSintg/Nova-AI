...
function usesClaudeCliModelSelection(config: Nova AIConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: Nova AIConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: Nova AIConfig): string[] {
  const refs = new Set<string>(
