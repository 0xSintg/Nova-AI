import { formatCliCommand, parseDurationMs } from "nova-ai/plugin-sdk/cli-runtime";
import type {
  Nova AIPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type Nova AIConfig as ProviderAuthConfig,
...
} from "nova-ai/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "nova-ai/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("nova-ai models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("nova-ai models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("nova-ai doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: Nova AIPluginApi): void {
