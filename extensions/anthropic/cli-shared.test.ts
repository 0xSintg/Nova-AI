...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it("maps Nova AI thinking levels to Claude effort args", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from Nova AI YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent Nova AI exec policy", () => {
    expect(
      resolveClaudePermissionMode({
        backendId: "claude-cli",
        agentId: "safe-agent",
