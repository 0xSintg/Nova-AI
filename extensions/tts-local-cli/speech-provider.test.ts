...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "nova-ai/plugin-sdk/speech-core";
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "nova-ai-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.NOVA_AI_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("nova-ai/plugin-sdk/media-runtime")>(
