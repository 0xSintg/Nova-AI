...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.NOVA_AI_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-nostr-"));
  process.env.NOVA_AI_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.NOVA_AI_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".nova-ai");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.NOVA_AI_STATE_DIR = previous;
    }
