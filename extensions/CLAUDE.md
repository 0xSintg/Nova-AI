...
- Extension production code should import from `nova-ai/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `nova-ai.plugin.json` and the package
  `nova-ai` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `nova-ai/plugin-sdk/*`.
