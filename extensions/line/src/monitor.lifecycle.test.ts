...
import { createMockIncomingRequest } from "nova-ai/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "nova-ai/plugin-sdk/webhook-request-guards";
...
vi.mock("nova-ai/plugin-sdk/channel-message", () => ({
  createChannelMessageReplyPipeline: vi.fn(() => ({})),
  hasFinalChannelTurnDispatch: vi.fn(() => false),
}));

vi.mock("nova-ai/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/webhook-ingress")>(
    "nova-ai/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("nova-ai/plugin-sdk/reply-runtime");
    vi.doUnmock("nova-ai/plugin-sdk/runtime-env");
    vi.doUnmock("nova-ai/plugin-sdk/channel-message");
    vi.doUnmock("nova-ai/plugin-sdk/webhook-ingress");
