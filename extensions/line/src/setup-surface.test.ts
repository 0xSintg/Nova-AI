...
import type { WizardPrompter } from "nova-ai/plugin-sdk/plugin-test-runtime";
import { bundledPluginRoot } from "nova-ai/plugin-sdk/test-fixtures";
import ts from "typescript";
import { afterAll, afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { Nova AIConfig, PluginRuntime, ResolvedLineAccount } from "../api.js";
...
function collectRuntimeApiPreExports(runtimeApiPath: string): string[] {
  ...
  const removedLineRuntimeSpecifier = ["nova-ai", "plugin-sdk", "line-runtime"].join("/");
...
describe("line setup wizard", () => {
  ...
  it("reports account-scoped config keys for named accounts", () => {
    expect(lineSetupWizard.dmPolicy?.resolveConfigKeys?.({} as Nova AIConfig, "work")).toEqual({
