...
vi.mock("nova-ai/plugin-sdk/media-store", () => ({
...
describe("downloadLineMedia", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("nova-ai/plugin-sdk/media-store");
    vi.resetModules();
  });
  ...
  beforeEach(() => {
    ...
    saveMediaStreamMock.mockImplementation(
      async (stream: AsyncIterable<Buffer>, contentType?: string, subdir?: string) => {
        ...
        return {
          path: `/home/user/.nova-ai/media/${subdir ?? "unknown"}/saved-media`,
  ...
  it("persists inbound media with the shared media store", async () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.nova-ai/media/inbound/saved-media",
