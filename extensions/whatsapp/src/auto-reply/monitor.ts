import { resolveAccountEntry } from "nova-ai/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "nova-ai/plugin-sdk/channel-inbound-debounce";
import { formatCliCommand } from "nova-ai/plugin-sdk/cli-runtime";
import { hasControlCommand } from "nova-ai/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "nova-ai/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "nova-ai/plugin-sdk/reply-history";
import { resolveAgentRoute } from "nova-ai/plugin-sdk/routing";
import { logVerbose } from "nova-ai/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "nova-ai/plugin-sdk/runtime-env";
import { getChildLogger } from "nova-ai/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  warn,
  type RuntimeEnv,
} from "nova-ai/plugin-sdk/runtime-env";
import { enqueueSystemEvent } from "nova-ai/plugin-sdk/system-event-runtime";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("nova-ai channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("nova-ai channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
      ...
      try {
        ...
        if (getStatusCode(error) === 428) {
          ...
          if (retryDecision.action === "stop") {
            ...
            runtime.error(
              `WhatsApp Web connection closed during setup (status 428) after ${retryDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("nova-ai channels login --channel whatsapp")}\` if the issue persists.`,
            );
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("nova-ai channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("nova-ai channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
