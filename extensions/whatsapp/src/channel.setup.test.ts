import { createQueuedWizardPrompter } from "nova-ai/plugin-sdk/plugin-test-runtime";
import { DEFAULT_ACCOUNT_ID } from "nova-ai/plugin-sdk/routing";
...
vi.mock("nova-ai/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: Nova AIConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<Nova AIConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
...
async function runConfigureWithHarness(params: {
  ...
  const result = await finalizeWhatsAppSetup({
    cfg: params.cfg ?? ({} as Nova AIConfig),
