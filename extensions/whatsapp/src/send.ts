...
import { generateSecureUuid } from "nova-ai/plugin-sdk/core";
import { redactIdentifier } from "nova-ai/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "nova-ai/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "nova-ai/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "nova-ai/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "nova-ai/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: Nova AIConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`nova-ai channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: Nova AIConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
