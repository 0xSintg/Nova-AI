...
import { resolveStateDir } from "nova-ai/plugin-sdk/state-paths";
import { resolvePreferredNova AITmpDir } from "nova-ai/plugin-sdk/temp-path";
import { captureEnv } from "nova-ai/plugin-sdk/test-env";
import { mockPinnedHostnameResolution } from "nova-ai/plugin-sdk/test-env";
import { withMockedWindowsPlatform, withRestoredMocks } from "nova-ai/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "nova-ai/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredNova AITmpDir(), "nova-ai-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub NOVA_AI_STATE_DIR.
    // Also keep it outside the Nova AI temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["NOVA_AI_STATE_DIR"]);
    process.env.NOVA_AI_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "nova-ai-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredNova AITmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/nova-ai` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredNova AITmpDir();
  ...
  it("allows default Nova AI state workspace and sandbox roots", async () => {
  ...
  it("rejects default Nova AI state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expectLocalMediaAccessCode(
      loadWebMedia(path.join(stateDir, "workspace-novay", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
      "path-not-allowed",
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-novay");
