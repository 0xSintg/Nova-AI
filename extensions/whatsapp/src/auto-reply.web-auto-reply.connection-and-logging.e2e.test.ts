...
import { escapeRegExp, formatEnvelopeTimestamp } from "nova-ai/plugin-sdk/channel-test-helpers";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
import { setLoggerOverride } from "nova-ai/plugin-sdk/runtime-env";
...
vi.mock("nova-ai/plugin-sdk/delivery-queue-runtime", () => ({
...
describe("web auto-reply connection", () => {
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\[nova-ai\\] first`),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(`\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\[nova-ai\\] second`),
        );
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/nova-ai-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/nova-ai-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
  ...
  it("marks dispatch idle after replies flush", async () => {
    ...
    const mockConfig: Nova AIConfig = {
