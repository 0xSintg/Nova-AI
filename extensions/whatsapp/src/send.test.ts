...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "nova-ai/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("nova-ai/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("nova-ai/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: Nova AIConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("nova-ai/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `nova-ai-outbound-${crypto.randomUUID()}.log`);
