...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "nova-ai-test-web-logout-"));
    previousOAuthDir = process.env.NOVA_AI_OAUTH_DIR;
    process.env.NOVA_AI_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the Nova AI auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Nova AI auth root", async () => {
