...
vi.mock("nova-ai/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/logging-core")>(
    "nova-ai/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("Nova AI WhatsApp listener close");
