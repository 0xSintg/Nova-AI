...
describe("collectWhatsAppStatusIssues", () => {
  beforeEach(() => {
    vi.stubEnv("NOVA_AI_CONTAINER_HINT", "");
    vi.stubEnv("NOVA_AI_PROFILE", "");
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: nova-ai channels login (scan QR on the gateway host).",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: nova-ai doctor (or restart the gateway). If it persists, relink via channels login and check logs.",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Watch: nova-ai logs --follow and run nova-ai channels status --probe if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.",
      },
    ]);
  });
