...
import { VERSION } from "nova-ai/plugin-sdk/cli-runtime";
import {
...
import { danger, success } from "nova-ai/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "nova-ai/plugin-sdk/runtime-env";
    ...
    try {
      ...
      if (connection === "close") {
        ...
        if (status === LOGGED_OUT_STATUS) {
          console.error(
            danger(
              `WhatsApp session logged out. Run: ${formatCliCommand("nova-ai channels login")}`,
            ),
          );
        }
      }
