...
import { formatEnvelopeTimestamp } from "nova-ai/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("nova-ai:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("nova-ai:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[nova-ai]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/nova-ai-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("nova-ai:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("nova-ai:lastSocket")];
  if (typeof getter === "function") {
