...
import type { ChannelSetupWizard } from "nova-ai/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "nova-ai/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<Nova AIConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: Nova AIConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: Nova AIConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: Nova AIConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: Nova AIConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("nova-ai channels login"),
      }),
      "WhatsApp",
    );
  }
