...
import { info, success } from "nova-ai/plugin-sdk/runtime-env";
import { getChildLogger } from "nova-ai/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "nova-ai/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("nova-ai channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
