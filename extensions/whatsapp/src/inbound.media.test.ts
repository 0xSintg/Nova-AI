...
vi.mock("nova-ai/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/channel-pairing")>(
...
vi.mock("nova-ai/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/media-store")>(
    "nova-ai/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `nova-ai-inbound-media-${crypto.randomUUID()}`);
