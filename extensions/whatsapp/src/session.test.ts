...
import { resetLogger, setLoggerOverride } from "nova-ai/plugin-sdk/runtime-env";
...
function createTempCaFile(contents: string): string {
  const dir = createTempAuthDir("nova-ai-wa-proxy-ca");
...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join("/tmp", "nova-ai-oauth", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("nova-ai-wa-creds-test");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("nova-ai-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("nova-ai-wa-a");
    const authDirB = createTempAuthDir("nova-ai-wa-b");
    const onError = vi.fn();
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    try {
      await writeCredsJsonAtomically("/tmp/nova-ai-oauth/whatsapp/default", {
        me: { id: "123@s.whatsapp.net" },
      });

      const write = firstWriteFileCall(openMock.writeFileSpy);
      expect(write.path).toContain(
        path.join("/tmp", "nova-ai-oauth", "whatsapp", "default", ".creds."),
      );
      expect(typeof write.data).toBe("string");
      expect(write.options.mode).toBe(0o600);
      expect(write.options.flag).toBe("wx");
      ...
      expect(chmodSpy).toHaveBeenCalledWith(
        path.join("/tmp", "nova-ai-oauth", "whatsapp", "default", "creds.json"),
        0o600,
      );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("nova-ai-wa-creds-atomic");
