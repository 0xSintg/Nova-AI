...
import { resetInboundDedupe } from "nova-ai/plugin-sdk/reply-dedupe";
...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("nova-ai:webAutoReplySessionSockets");
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-session-"));
