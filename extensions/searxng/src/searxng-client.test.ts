import type { LookupFn } from "nova-ai/plugin-sdk/ssrf-runtime";
...
vi.mock("nova-ai/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/provider-web-search")>();
...
describe("searxng client", () => {
  ...
  it("preserves a configured base-path prefix when building the search URL", () => {
    expect(
      ...
      "https://search.example.com/searxng/search?q=nova-ai&format=json&categories=general%2Cnews&language=en",
    );
  });
