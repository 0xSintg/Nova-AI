...
import type { MemoryEmbeddingProvider } from "nova-ai/plugin-sdk/memory-core-host-engine-embeddings";
import { resolveLivePluginConfigObject } from "nova-ai/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "nova-ai/plugin-sdk/runtime-env";
...
import { definePluginEntry, type Nova AIPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("nova-ai/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("nova-ai/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("nova-ai/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("nova-ai/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("nova-ai/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("nova-ai/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as Nova AIConfig;
...
function createEmbeddings(api: Nova AIPluginApi, cfg: MemoryConfig): Embeddings {
