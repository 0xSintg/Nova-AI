...
import type { Nova AIPluginApi, Nova AIPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<Nova AIPluginApi> = {}): Nova AIPluginApi {
...
function fakeCtx(overrides: Partial<Nova AIPluginToolContext> = {}): Nova AIPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: Nova AIPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
