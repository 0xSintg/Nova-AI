...
function findLobsterPackageRoot(resolvedEntryPath: string): string {
  ...
  while (true) {
    ...
    try {
      ...
      if (parsed.name === "@novabot/lobster") {
        return dir;
      }
    } catch {
    ...
    if (parent === dir) {
      throw new Error(`Could not locate @novabot/lobster package root from ${resolvedEntryPath}`);
    }
    dir = parent;
  }
}
    ...
    if (envelope.status === "needs_input") {
      return {
        ...
        error: {
          ...
          message: "Lobster input requests are not supported by the Nova AI Lobster tool yet",
        },
      };
    }
    return {
...
export async function loadEmbeddedToolRuntimeFromPackage(
  ...
  try {
    const coreSpecifier = ["@novabot", "lobster", "core"].join("/");
    return toEmbeddedToolRuntime(await importModule(coreSpecifier), "@novabot/lobster/core");
  } catch (error) {
    coreLoadError = error;
  }
  ...
  try {
    const packageEntryPath = resolvePackageEntry("@novabot/lobster");
