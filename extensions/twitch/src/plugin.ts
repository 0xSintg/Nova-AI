/**
 * Twitch channel plugin for Nova AI.
 *
...
} from "nova-ai/plugin-sdk/channel-pairing";
...
} from "nova-ai/plugin-sdk/extension-shared";
import {
 ...
 * for Nova AI. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: Nova AIConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: Nova AIConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: Nova AIConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: Nova AIConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("nova-ai/plugin-sdk/runtime-env").RuntimeEnv;
