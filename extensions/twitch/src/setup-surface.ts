...
import { normalizeOptionalAccountId } from "nova-ai/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "nova-ai/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: Nova AIConfig, requestedAccountId?: string): string {
...
export async function configureWithEnvToken(
  ...
  accountId: string = resolveSetupAccountId(cfg),
): Promise<{ cfg: Nova AIConfig } | null> {
  const resolvedAccountId = accountId.trim()
    ? normalizeRequestedSetupAccountId(accountId)
    : resolveSetupAccountId(cfg);
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.NOVA_AI_TWITCH_ACCESS_TOKEN?.trim();
