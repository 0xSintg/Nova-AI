...
import type { DoctorSessionRouteStateOwner } from "nova-ai/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.codex.config.codexDynamicToolsProfile is retired; Codex app-server always keeps Codex-native workspace tools native. Run "nova-ai doctor --fix".',
    match: hasRetiredDynamicToolsProfile,
  },
];

export function normalizeCompatibilityConfig({ cfg }: { cfg: Nova AIConfig }): {
  ...
  const nextConfig = structuredClone(cfg) as Nova AIConfig & {
