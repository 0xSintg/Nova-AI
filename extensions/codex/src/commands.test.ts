...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`Nova AI session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`Nova AI session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-command-"));
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "nova-ai.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in nova-ai.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in nova-ai.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this Nova AI session to Codex thread thread-123.",
    });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this Nova AI session yet.",
    });
  });
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("Nova AI session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("Nova AI session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("Nova AI session key: `agent:main:discord:two`");
    expect(request.text).toContain("Nova AI session id: `session-two`");
