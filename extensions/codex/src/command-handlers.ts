...
import { resolveAgentDir, resolveSessionAgentIds } from "nova-ai/plugin-sdk/agent-runtime";
import type { PluginCommandContext, PluginCommandResult } from "nova-ai/plugin-sdk/plugin-entry";
...
const CODEX_DIAGNOSTICS_SOURCE = "nova-ai-diagnostics";
    ...
    if (!deps.codexPluginsManagementIo) {
      return {
        text:
          ...
          "Edit ~/.nova-ai/nova-ai.json or use `nova-ai config patch` until the runtime exposes the IO.",
      };
    }
...
async function bindConversation(
  ...
  if (!ctx.sessionFile) {
    return {
      text: "Cannot bind Codex because this command did not include an Nova AI session file.",
    };
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Nova AI session file.";
  }
  ...
  return `Attached this Nova AI session to Codex thread ${formatCodexDisplayText(
    effectiveThreadId,
  )}.`;
}
...
async function stopConversationTurn(
  ...
  if (!target) {
    return "Cannot stop Codex because this command did not include an Nova AI session file.";
  }
  return (
...
async function steerConversationTurn(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot steer Codex because this command did not include an Nova AI session file.";
  }
  return (
...
async function setConversationModel(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex model because this command did not include an Nova AI session file.";
  }
  const [model = ""] = args;
...
async function setConversationFastMode(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex fast mode because this command did not include an Nova AI session file.";
  }
  const value = args[0];
...
async function setConversationPermissions(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex permissions because this command did not include an Nova AI session file.";
  }
  const value = args[0];
...
async function requestCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return {
      text: "Cannot send Codex diagnostics because this command did not include an Nova AI session file.",
    };
  }
  ...
  if (targets.length === 0) {
    return {
      text: [
        "No Codex thread is attached to this Nova AI session yet.",
...
async function previewCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return "Cannot send Codex diagnostics because this command did not include an Nova AI session file.";
  }
  const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
  if (targets.length === 0) {
    return [
  ...
  return [
    ...
    "The completed diagnostics reply will list the Nova AI session ids and Codex thread ids that were sent.",
    ...(displayReason ? [`Note: ${displayReason}`] : []),
    "Included: Codex logs and spawned Codex subthreads when available.",
  ].join("\n");
}
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`Nova AI session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`Nova AI session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`Nova AI session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
...
async function startThreadAction(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return `Cannot start Codex ${label} because this command did not include an Nova AI session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(target.sessionFile, {
    agentDir: target.agentDir,
    config: ctx.config,
  ...
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Nova AI session yet.`;
  }
