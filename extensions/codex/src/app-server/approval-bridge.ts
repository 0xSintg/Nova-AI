...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runNova AIToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
...
async function runNova AIToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  ...
  const policyRequest = buildNova AIToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `Nova AI native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
    };
  }
...
function readNativeRelayPreToolUseDecision(
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "Nova AI native hook relay failed for Codex app-server approval.",
    };
  }
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "Nova AI native hook policy denied Codex app-server approval.",
    };
  }
  ...
  return {
    blocked: true,
    reason: output
      ? "Nova AI native hook relay returned a non-deny Codex app-server approval decision."
...
function buildNova AIToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
