...
describe("Codex app-server dynamic tool schema boundary contract", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-schema-contract-"));
  });
  ...
  it("passes prepared executable dynamic tool schemas through thread start unchanged", async () => {
    ...
    expect(startPayload?.serviceName).toBe("Nova AI");
    ...
    expect(startPayload?.developerInstructions).toContain("Nova AI");
  });
