...
import { upsertAuthProfile } from "nova-ai/plugin-sdk/provider-auth";
...
vi.mock("nova-ai/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if (params.forceRefresh || (oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.NOVA_AI_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("sets agent-owned CODEX_HOME without overriding HOME for local app-server launches", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-app-server-"));
  ...
  it("applies native Codex CLI OAuth when no Nova AI auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-app-server-"));
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an Nova AI profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
