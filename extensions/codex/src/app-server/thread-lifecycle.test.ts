import type { EmbeddedRunAttemptParams } from "nova-ai/plugin-sdk/agent-harness-runtime";
...
describe("Codex app-server native code mode config", () => {
  it("keeps Codex-native subagents primary while limiting Nova AI spawn to Nova AI delegation", () => {
    const instructions = buildDeveloperInstructions(createAttemptParams({ provider: "openai" }));

    expect(instructions).toContain("Use Codex native `spawn_agent` for Codex subagents");
    expect(instructions).toContain(
      "Use Nova AI `sessions_spawn` only for Nova AI or ACP delegation.",
    );
  });
  ...
  it("summarizes deferred dynamic tool names in developer instructions", () => {
    ...
    expect(instructions).toContain(
      "Deferred searchable Nova AI dynamic tools available: image_generate, music_generate.",
    );
  ...
  it("keeps developer instructions compact when no dynamic tools are deferred", () => {
    ...
    expect(instructions).not.toContain("Deferred searchable Nova AI dynamic tools available");
  });

  it("keeps Nova AI skill catalogs out of developer instructions", () => {
