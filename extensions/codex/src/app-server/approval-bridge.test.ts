...
vi.mock("nova-ai/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("nova-ai/plugin-sdk/agent-harness-runtime")>()),
...
describe("Codex app-server approval bridge", () => {
  ...
  it("routes command approvals through plugin approvals and accepts allowed commands", async () => {
    ...
    expect(requestPayload.pluginId).toBe("nova-ai-codex-app-server");
  ...
  it("normalizes prefixed channel targets for Nova AI tool policy context", async () => {
    const params = createParams();
  ...
  it("denies command approvals before prompting when Nova AI tool policy blocks", async () => {
    const params = createParams();
  ...
  it("fails closed when the native hook relay returns unreadable approval output", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "Nova AI native hook relay returned an unreadable Codex app-server approval result.",
    });
  });
  ...
  it("fails closed when the native hook relay returns a non-deny decision", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message: "Nova AI native hook relay returned a non-deny Codex app-server approval decision.",
    });
  });
  ...
  it("fails closed when the expected native hook relay cannot be invoked", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "Nova AI native hook relay unavailable for Codex app-server approval: native hook relay not found",
    });
  });
  ...
  it("denies command approvals when Nova AI tool policy rewrites params", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
  ...
  it("denies command approvals when Nova AI tool policy requires approval", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: true,
      kind: "failure",
  ...
  it("fails closed for unsupported native approval methods without requesting plugin approval", async () => {
    ...
    expect(result).toEqual({
      decision: "decline",
      reason: "Nova AI codex app-server bridge does not grant native approvals yet.",
    });
    expect(mockCallGatewayTool).not.toHaveBeenCalled();
