...
import { buildCodexUserMcpServersThreadConfigPatch } from "nova-ai/plugin-sdk/codex-mcp-projection";
import { listRegisteredPluginAgentPromptGuidance } from "nova-ai/plugin-sdk/plugin-runtime";
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is an Nova AI cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
...
function buildHeartbeatCollaborationInstructions(): string {
  return [
    "This is an Nova AI heartbeat turn. Apply these instructions only to this heartbeat wake; ordinary chat turns should stay in Codex Default mode.",
    "When you are ready to end the heartbeat, prefer the structured `heartbeat_respond` tool so Nova AI can record the wake outcome and notification decision. If `heartbeat_respond` is not already available and `tool_search` is available, search for `heartbeat_respond`, load it, then call it. Use `notify=false` when nothing should visibly interrupt the user.",
...
export function buildDeveloperInstructions(
  ...
  const sections = [
    "You are a personal agent running inside Nova AI. Nova AI has dynamic tools for Nova AI-owned messaging, cron, sessions, media, gateway, and nodes.",
    buildDeferredDynamicToolManifest(options.dynamicTools),
    "Use Codex native `spawn_agent` for Codex subagents. Use Nova AI `sessions_spawn` only for Nova AI or ACP delegation.",
...
function buildDeferredDynamicToolManifest(
  ...
  return `Deferred searchable Nova AI dynamic tools available: ${deferredToolNames.join(", ")}. Use \`tool_search\` to load exact callable specs before use.`;
}
...
export function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is Nova AI's virtual provider; let Codex app-server keep its
  ...
  if (
    ...
    // When Nova AI is forwarding ChatGPT/Codex OAuth, `openai` is Codex's
