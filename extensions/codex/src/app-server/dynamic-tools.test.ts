...
import type { AnyAgentTool } from "nova-ai/plugin-sdk/agent-harness";
import {
...
describe("createCodexDynamicToolBridge", () => {
  it("keeps turn-yield direct while deferring Nova AI session spawn", () => {
  ...
  it("truncates configured text tool results before returning them to Codex", async () => {
    ...
    expect(text).toContain("Nova AI truncated dynamic tool result");
  ...
  it("honors normalized per-agent dynamic tool result caps", async () => {
    ...
    expect(firstItem.text).toContain("Nova AI truncated dynamic tool result");
  });
  ...
  it("keeps truncation notices within tiny configured caps", async () => {
    ...
    expect(firstItem.text).toBe("...(Nova AI truncated dynamic tool".slice(0, 32));
  });
  ...
  it("keeps config out of Codex tool-result contexts", async () => {
    const config = { session: { store: "/tmp/nova-ai-session-store.json" } };
    const registry = createEmptyPluginRegistry();
