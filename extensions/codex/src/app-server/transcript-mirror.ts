...
export function attachCodexMirrorIdentity<T extends AgentMessage>(message: T, identity: string): T {
  ...
  const existing = record["__nova-ai"];
  const baseMeta =
  ...
  return {
    ...record,
    __nova-ai: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}

function readMirrorIdentity(message: MirroredAgentMessage): string | undefined {
  const record = message as unknown as { __nova-ai?: unknown };
  const meta = record["__nova-ai"];
