...
} from "nova-ai/plugin-sdk/hook-runtime";
import { clearPluginCommands, registerPluginCommand } from "nova-ai/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "nova-ai/plugin-sdk/plugin-test-runtime";
...
import {
  CODEX_NOVA_AI_DYNAMIC_TOOL_NAMESPACE,
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/nova-ai-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt", () => {
  beforeEach(async () => {
    resetAgentEventsForTest();
    resetDiagnosticEventsForTest();
    vi.stubEnv("NOVA_AI_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-run-"));
  });

  afterEach(async () => {
    resetCodexAppServerClientFactoryForTest();
    testing.resetNova AICodingToolsFactoryForTests();
    resetCodexRateLimitCacheForTests();
  ...
  it("exposes Nova AI sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
    testing.setNova AICodingToolsFactoryForTests(() => [
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await testing.buildDynamicTools({
      ...
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/nova-ai-data:/data:rw"] },
      } as never,
  ...
  it("starts Codex threads without duplicate Nova AI workspace tools by default", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("passes auth profiles into Codex dynamic tool construction", async () => {
    ...
    testing.setNova AICodingToolsFactoryForTests((options) => {
  ...
  it("enables gateway subagent binding for forced private QA Codex runs", async () => {
    vi.stubEnv("NOVA_AI_BUILD_PRIVATE_QA", "1");
    vi.stubEnv("NOVA_AI_QA_FORCE_RUNTIME", "codex");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("disables Codex native tool surfaces when Docker bind targets need container paths", () => {
    ...
    expect(
      testing.shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/nova-ai-data:/tmp/nova-ai-data:rw"] },
      } as never),
    ).toBe(true);
    ...
    expect(
      testing.shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/nova-ai-data:/tmp/nova-ai-data:rw",
            "/tmp/nova-ai-data/secrets:/tmp/nova-ai-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
  ...
  it("keeps Nova AI skills out of Codex developer instructions", async () => {
    const llmInput = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "llm_input", handler: llmInput }]),
    );
    vi.stubEnv("NOVA_AI_TRAJECTORY", "1");
    vi.stubEnv("NOVA_AI_TRAJECTORY_DIR", path.join(tempDir, "trajectory"));
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    ...
    expect(inputText).toContain("## Nova AI Skills");
  ...
  it("keeps searchable Nova AI dynamic tools when code-mode-only is enabled", async () => {
    testing.setNova AICodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("message"),
    ...
    expect(webSearch?.namespace).toBe(CODEX_NOVA_AI_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_NOVA_AI_DYNAMIC_TOOL_NAMESPACE);
    expect(heartbeat?.deferLoading).toBe(true);
    expect(sessionsSpawn?.namespace).toBe(CODEX_NOVA_AI_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("fails closed for Codex app defaults when restricted native tools have no plugin config", async () => {
    testing.setNova AICodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
    const harness = createStartedThreadHarness(async (method) => {
      if (method === "app/list") {
        throw new Error("app/list should not run when runtime toolsAllow is empty.");
      }
  ...
  it("keeps searchable Codex dynamic tools canonical in mirrored transcript snapshots", async () => {
    testing.setNova AICodingToolsFactoryForTests(() => [createRuntimeDynamicTool("wiki_status")]);
    const harness = createStartedThreadHarness();
    const params = createParams(
      path.join(tempDir, "session.jsonl"),
      path.join(tempDir, "workspace"),
    ...
    const toolResult = (await harness.handleServerRequest({
      ...
      params: {
        ...
        namespace: CODEX_NOVA_AI_DYNAMIC_TOOL_NAMESPACE,
        tool: "wiki_status",
  ...
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      testing.resolveNova AICodingToolsSessionKeys(
        params,
    ...
    expect(testing.resolveNova AICodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("returns a failed dynamic tool response when an app-server tool call exceeds the deadline", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Nova AI dynamic tool call timed out after 1ms while running tool message.",
        },
      ],
    });
  ...
  it("logs process poll timeout context separately from session idle", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Nova AI dynamic tool call timed out after 1ms while waiting for process action=poll sessionId=rapid-crustacean. This is a tool RPC timeout, not a session idle timeout.",
        },
      ],
    });
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown Nova AI tool: lookup$/u);
  ...
  it("clears dynamic tool diagnostics after successful app-server tool responses", async () => {
    ...
    testing.setNova AICodingToolsFactoryForTests(() => [createRuntimeDynamicTool("echo")]);
  ...
  it("emits request-boundary terminal diagnostics when a wrapped dynamic tool does not", async () => {
    ...
    testing.setNova AICodingToolsFactoryForTests(() => [markedWrappedTool as never]);
  ...
  it("does not duplicate terminal diagnostics for wrapped dynamic tool timeout fallbacks", async () => {
    ...
    expect(toolResult).toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Nova AI dynamic tool call timed out after 1ms while running tool echo.",
        },
      ],
    });
  ...
  it("suppresses normalized tool progress for activity-log dynamic bash requests", async () => {
    ...
    await harness.handleServerRequest({
      ...
      params: {
        ...
        arguments: {
          command:
            '/bin/bash -lc \'/home/nova-ai/.nova-ai/workspace/bin/log_activity.sh "web_search" "Grilled salmon research"\'',
        },
      },
    });
  ...
  it("releases the session when Codex never completes after a dynamic tool response", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(
      /^(Unknown Nova AI tool: message|Action send requires a target\.)$/u,
    );
  ...
  it("projects mirrored history when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage nova-ai";
    ...
    expect(inputText).toContain("Nova AI assembled context for this turn:");
    expect(inputText).toContain("we are fixing the Opik default project");
    expect(inputText).toContain("Opik default project context");
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage nova-ai");
  });
  ...
  it("passes stable workspace files as Codex developer instructions and keeps MEMORY.md as turn context", async () => {
    ...
    expect(threadStartParams.developerInstructions).toContain("Nova AI Agent Soul");
    ...
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).toContain("Nova AI runtime context for this turn:");
  ...
  it("omits heartbeat Codex workspace pointers for empty HEARTBEAT.md files", async () => {
    ...
    expect(collaborationInstructions).toContain("This is an Nova AI heartbeat turn");
  ...
  it("keeps lightweight cron Codex turns out of Nova AI bootstrap context", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/nova-ai && /Users/phaedrus/nova/scripts/clawsweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("fires llm_input, llm_output, and agent_end hooks for codex turns", async () => {
    ...
    expect(llmInputPayload.systemPrompt).toContain(
      "You are a personal agent running inside Nova AI.",
    );
  ...
  it("promotes implicit Codex yolo approval policy when Nova AI tool policy exists", async () => {
    initializeGlobalHookRunner(
  ...
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = testing.resolveCodexAppServerForNova AIToolPolicy({
      appServer,
  ...
  it("keeps explicit Codex yolo mode unpromoted when Nova AI tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("ignores invalid Codex app-server env overrides when promoting tool policy approval", async () => {
    ...
    vi.stubEnv("NOVA_AI_CODEX_APP_SERVER_MODE", " ");
    vi.stubEnv("NOVA_AI_CODEX_APP_SERVER_APPROVAL_POLICY", "always");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for nova-ai/nova-ai#67996: a throw inside the projector's
  ...
  it("keeps native hook relay config as the final thread config patch", async () => {
    ...
    const finalConfigPatch = {
      ...
      "hooks.PreToolUse": [
        {
          hooks: [{ type: "command", command: "nova-ai-native-hook-relay", timeout: 5 }],
        },
      ],
    };
    const buildPluginThreadConfig = vi.fn(async () => ({
  ...
  it("maps active Nova AI sandbox egress into Codex workspace-write turns", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
    ...
    expect(
      testing.resolveCodexAppServerSandboxPolicyForNova AISandbox(
        appServer,
        {
          enabled: true,
          backendId: "docker",
    ...
    expect(
      testing.resolveCodexAppServerSandboxPolicyForNova AISandbox(
        appServer,
        {
          ...
          docker: {
            network: "bridge",
            binds: [
              "/tmp/nova-ai-writable-data:/data:rw",
              "/tmp/nova-ai-readonly-data:/readonly:ro",
            ],
          },
        } as never,
        "/tmp/workspace",
      ...
      type: "workspaceWrite",
      writableRoots: ["/tmp/workspace", path.resolve("/tmp/nova-ai-writable-data")],
      networkAccess: true,
      excludeTmpdirEnvVar: false,
      excludeSlashTmp: false,
    });
  ...
  it("builds resume and turn params from the currently selected Nova AI model", () => {
  ...
  it("uses turn-scoped collaboration instructions for heartbeat Codex turns", () => {
    ...
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
      "This is an Nova AI heartbeat turn. Apply these instructions only to this heartbeat wake",
    );
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
  ...
  it("uses turn-scoped collaboration instructions for cron Codex turns", () => {
    ...
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
      "This is an Nova AI cron automation turn",
    );
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
