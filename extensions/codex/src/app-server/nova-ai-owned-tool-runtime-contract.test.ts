...
import { wrapToolWithBeforeToolCallHook } from "nova-ai/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installNova AIOwnedToolHooks,
  mediaToolResult,
  resetNova AIOwnedToolHooks,
  textToolResult,
...
describe("Nova AI-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetNova AIOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installNova AIOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installNova AIOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installNova AIOwnedToolHooks();
