...
import { resetAgentEventsForTest } from "nova-ai/plugin-sdk/agent-harness-runtime";
import {
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-projector-"));
...
describe("CodexAppServerEventProjector", () => {
  ...
  it("records dynamic Nova AI tool calls in mirrored transcript snapshots", async () => {
    const projector = await createProjector();
