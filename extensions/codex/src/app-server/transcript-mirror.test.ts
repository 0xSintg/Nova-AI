...
import type { AgentMessage } from "nova-ai/plugin-sdk/agent-harness-runtime";
import {
...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-codex-transcript-"));
  tempDirs.push(dir);
...
describe("mirrorCodexAppServerTranscript", () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("nova-ai-codex-transcript-missing-dir-");
