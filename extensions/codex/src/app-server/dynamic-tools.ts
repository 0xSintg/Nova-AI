...
import { normalizeAgentId } from "nova-ai/plugin-sdk/routing";
...
export const CODEX_NOVA_AI_DYNAMIC_TOOL_NAMESPACE = "nova-ai";

// Keep Nova AI session spawning searchable in Codex mode so Codex's native
...
export function createCodexDynamicToolBridge(params: {
  ...
  return {
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!tool) {
        return {
          contentItems: [{ type: "inputText", text: `Unknown Nova AI tool: ${call.tool}` }],
          success: false,
        };
      }
...
function convertToolContents(
  ...
  const noticeText = `...(Nova AI truncated dynamic tool result: original ${totalTextChars} chars, showing ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;
