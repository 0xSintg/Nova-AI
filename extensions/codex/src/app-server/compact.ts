...
export async function maybeCompactCodexAppServerSession(
  ...
  warnIfIgnoringNova AICompactionOverrides(params);
...
function warnIfIgnoringNova AICompactionOverrides(params: CompactEmbeddedPiSessionParams): void {
  const activeContextEngine = isActiveHarnessContextEngine(params.contextEngine)
    ? params.contextEngine
    : undefined;
  ...
  embeddedAgentLog.warn(
    "ignoring Nova AI compaction overrides for Codex app-server compaction; Codex uses native server-side compaction",
    {
      sessionId: params.sessionId,
      sessionKey: params.sessionKey,
      ignoredConfig,
...
function resolveCompactionWaitTimeoutMs(): number {
  const raw = process.env.NOVA_AI_CODEX_COMPACTION_WAIT_TIMEOUT_MS?.trim();
