...
import { markAuthProfileBlockedUntil, resolveAgentDir } from "nova-ai/plugin-sdk/agent-runtime";
import {
...
import { pathExists } from "nova-ai/plugin-sdk/security-runtime";
...
type Nova AICodingToolsOptions = NonNullable<
  Parameters<(typeof import("nova-ai/plugin-sdk/agent-harness"))["createNova AICodingTools"]>[0]
>;
type Nova AICodingToolsFactory =
  (typeof import("nova-ai/plugin-sdk/agent-harness"))["createNova AICodingTools"];
type Nova AIDynamicTool = ReturnType<Nova AICodingToolsFactory>[number];
...
let openClawCodingToolsFactoryForTests: Nova AICodingToolsFactory | undefined;
...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Nova AI dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Nova AI dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
type Nova AISandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;

function resolveCodexAppServerSandboxPolicyForNova AISandbox(
  appServer: CodexAppServerRuntimeOptions,
  sandbox: Nova AISandboxContext,
  cwd: string,
): CodexSandboxPolicy | undefined {
  if (!sandbox?.enabled || appServer.sandbox === "read-only") {
    return undefined;
  }
  const networkAccess = codexNetworkAccessForNova AISandbox(sandbox);
  ...
  // filesystem sandbox while mirroring the Nova AI sandbox egress policy.
  return {
    type: "workspaceWrite",
...
function codexNetworkAccessForNova AISandbox(sandbox: Nova AISandboxContext): boolean {
...
function resolveCodexAppServerForNova AIToolPolicy(params: {
  appServer: CodexAppServerRuntimeOptions;
  ...
  const explicitMode =
    ...
    isCodexAppServerPolicyMode(params.env.NOVA_AI_CODEX_APP_SERVER_MODE);
  const explicitApprovalPolicy =
    params.pluginConfig.appServer?.approvalPolicy !== undefined ||
    isCodexAppServerApprovalPolicy(params.env.NOVA_AI_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export async function runCodexAppServerAttempt(
  ...
  const codexSandboxPolicy = resolveCodexAppServerSandboxPolicyForNova AISandbox(
    configuredAppServer,
    sandbox,
    effectiveWorkspace,
  );
  const appServer = resolveCodexAppServerForNova AIToolPolicy({
  ...
  const openClawPromptContext = buildCodexNova AIPromptContext({
    params,
  ...
  const decorateCodexTurnPromptText = (prompt: string) =>
    prependCodexNova AIPromptContext(prompt, openClawPromptContext);
  ...
  const pendingNova AIDynamicToolCompletionIds = new Set<string>();
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    const trackedDynamicToolCompletion = isPendingNova AIDynamicToolCompletionNotification(
      notification,
      pendingNova AIDynamicToolCompletionIds,
    );
    ...
    if (
      ...
      // accepts a turn or after Nova AI hands a turn-scoped request result
      // back to Codex. Bookkeeping that closes the just-served Nova AI
    ...
    if (trackedDynamicToolCompletion) {
      ...
      if (itemId) {
        pendingNova AIDynamicToolCompletionIds.delete(itemId);
      }
    }
  ...
  const requestCleanup = client.addRequestHandler(async (request) => {
    ...
    try {
      ...
      markCurrentTurnRequestProgress();
      turnCrossedToolHandoff = true;
      pendingNova AIDynamicToolCompletionIds.add(call.callId);
...
async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedDynamicToolResponse("Nova AI dynamic tool call aborted before execution.");
  }
  ...
  const abortFromRun = () => {
    const message = "Nova AI dynamic tool call aborted.";
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Nova AI dynamic tool call finished."));
    }
  }
}
...
function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Nova AI plugin approval prompt.
...
function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("nova-ai:codex:native-hook-relay:v1");
  hash.update("\0");
...
function buildCodexPluginThreadConfigEligibilityLogData(params: {
  ...
  return {
    ...
    appCacheKeyFingerprint: fingerprintCodexLogValue(
      "nova-ai:codex:plugin-app-cache-key:v1",
...
function resolveNova AICodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<Nova AICodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
    sessionKey: sandboxSessionKey,
    runSessionKey:
...
async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const createNova AICodingTools =
    openClawCodingToolsFactoryForTests ??
    (await import("nova-ai/plugin-sdk/agent-harness")).createNova AICodingTools;
  const sessionKeys = resolveNova AICodingToolsSessionKeys(params, input.sandboxSessionKey);
  const allTools = createNova AICodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as Nova AICodingToolsOptions["modelCompat"])
        : undefined,
...
function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow Nova AI allowlists must fail closed rather than
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: Nova AISandboxContext | undefined,
): boolean {
  if (!sandbox?.enabled || sandbox.backendId !== "docker") {
    return true;
  }
...
function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: Nova AIDynamicTool[],
  allTools: Nova AIDynamicTool[],
  ...
  const sandboxExecTool: Nova AIDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through Nova AI's configured sandbox backend for this session. Use only when the command must execute in the Nova AI sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout that Codex's native shell cannot represent. Use Codex's native shell for normal local workspace commands.",
  ...
  const sandboxProcessTool: Nova AIDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage sandbox_exec sessions that were started through Nova AI's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox_exec follow-up; use Codex's native shell session handling for normal native shell commands.",
  };
...
function isPendingNova AIDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingNova AIDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingNova AIDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
...
async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexNova AIPromptContext(params.params)
...
function buildCodexNova AIPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexNova AIPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.skillsPrompt?.trim()
      ? ["## Nova AI Skills", "", params.skillsPrompt.trim()].join("\n")
      : undefined,
    params.workspacePromptContext?.trim()
      ? ["## Nova AI Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ].filter(isNonEmptyString);
  ...
  return [
    "Nova AI runtime context for this turn:",
    "Treat this Nova AI-provided context as supporting project/user reference for the current request.",
    "",
    ...sections,
  ].join("\n");
}

function shouldInjectCodexNova AIPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
function prependCodexNova AIPromptContext(prompt: string, context: string | undefined): string {
  if (!context?.trim()) {
    return prompt;
  }
  const promptSection = prompt.startsWith("Nova AI assembled context for this turn:")
    ? prompt
...
function renderCodexWorkspaceBootstrapPromptContext(
  ...
  const lines = [
    "Nova AI loaded these user-editable workspace files for the current turn. Codex loads AGENTS.md natively. SOUL.md, IDENTITY.md, TOOLS.md, and USER.md are provided separately as Codex developer instructions. HEARTBEAT.md is handled by heartbeat collaboration-mode guidance. Those files are not repeated here.",
    "",
    "# Project Context",
    "",
...
function renderCodexWorkspaceDeveloperInstructions(
  ...
  const lines = [
    "## Nova AI Agent Soul",
    "",
    "Nova AI loaded these workspace instruction files from the active agent workspace. They define who you are, how you work, what tools are available, and the human you work alongside. Internalize and follow them accordingly.",
    "",
  ];
  for (const file of files) {
    lines.push(`### ${file.path}`, "", file.content, "");
...
export const testing = {
  ...
  resolveCodexAppServerSandboxPolicyForNova AISandbox,
  resolveCodexAppServerForNova AIToolPolicy,
  resolveNova AICodingToolsSessionKeys,
  shouldEnableCodexAppServerNativeToolSurface,
  shouldForceMessageTool,
  buildCodexPluginThreadConfigEligibilityLogData,
  setNova AICodingToolsFactoryForTests(factory: Nova AICodingToolsFactory): void {
    openClawCodingToolsFactoryForTests = factory;
  },
  resetNova AICodingToolsFactoryForTests(): void {
