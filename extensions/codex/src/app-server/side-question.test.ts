import { nativeHookRelayTesting } from "nova-ai/plugin-sdk/agent-harness-runtime";
import {
...
vi.mock("nova-ai/plugin-sdk/agent-harness", () => ({
  createNova AICodingTools: (...args: unknown[]) => createNova AICodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createNova AICodingToolsMock.mockReset();
    ...
    createNova AICodingToolsMock.mockReturnValue([
      {
        name: "wiki_status",
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createNova AICodingToolsMock);
  ...
  it("bridges side-thread dynamic tool requests to Nova AI tools", async () => {
    const client = createFakeClient();
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createNova AICodingToolsMock).toHaveBeenCalledWith(
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createNova AICodingToolsMock.mockImplementation(() => {
