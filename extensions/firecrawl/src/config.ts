...
import { canResolveEnvSecretRefInReadOnlyPath } from "nova-ai/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "nova-ai/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<Nova AIConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: Nova AIConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: Nova AIConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: Nova AIConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: Nova AIConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: Nova AIConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: Nova AIConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: Nova AIConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: Nova AIConfig, override?: number): number {
