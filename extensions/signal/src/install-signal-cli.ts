...
import { runPluginCommandWithTimeout } from "nova-ai/plugin-sdk/run-command";
import type { RuntimeEnv } from "nova-ai/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "nova-ai/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredNova AITmpDir(), "nova-ai-signal-"));
