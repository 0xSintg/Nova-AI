import type { MockFn } from "nova-ai/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/nova-ai-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("nova-ai/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("nova-ai/plugin-sdk/runtime-config-snapshot")
  >("nova-ai/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/reply-runtime")>(
...
vi.mock("nova-ai/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/security-runtime")>(
    "nova-ai/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/system-event-runtime")>(
    "nova-ai/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/transport-ready-runtime", () => ({
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("nova-ai/plugin-sdk/reply-runtime"),
      import("nova-ai/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
