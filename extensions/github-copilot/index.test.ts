...
type RegisteredMemoryEmbeddingProvider = Parameters<
  Nova AIPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type GithubCopilotTestProvider = {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<Nova AIConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<Nova AIPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<Nova AIPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<Nova AIPluginApi["registerMemoryEmbeddingProvider"]>();
