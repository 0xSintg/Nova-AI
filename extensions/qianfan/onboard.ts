...
function resolveQianfanPreset(cfg: Nova AIConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: Nova AIConfig) => {
...
export function applyQianfanProviderConfig(cfg: Nova AIConfig): Nova AIConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: Nova AIConfig): Nova AIConfig {
