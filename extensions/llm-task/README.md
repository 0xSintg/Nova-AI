...
`nova-ai.invoke --each`) without adding new Nova AI code per workflow.
...
This extension depends on Nova AI internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Nova AI extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.nova-ai/extensions` as a standalone plugin directory.
