...
import { buildModelAliasIndex, resolveModelRefFromString } from "nova-ai/plugin-sdk/agent-runtime";
import {
  type JsonSchemaObject,
  validateJsonSchemaValue,
} from "nova-ai/plugin-sdk/json-schema-runtime";
import { normalizeOptionalString } from "nova-ai/plugin-sdk/string-coerce-runtime";
import { Type } from "typebox";
import { resolvePreferredNova AITmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<Nova AIPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via nova-ai.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<Nova AIPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return !!level && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: Nova AIPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<Nova AIPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredNova AITmpDir(), prefix: "nova-ai-llm-task-" },
        async ({ dir: tmpDir }) => {
