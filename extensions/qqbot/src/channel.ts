import { getExecApprovalReplyMetadata } from "nova-ai/plugin-sdk/approval-runtime";
import {
...
import type { ChannelPlugin } from "nova-ai/plugin-sdk/core";
...
import { toGatewayAccount, writeNova AIConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped nova-ai.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // nova-ai.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeNova AIConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe nova-ai.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeNova AIConfigThroughRuntime(getQQBotRuntime(), nextCfg as Nova AIConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as Nova AIConfig, accountId);
