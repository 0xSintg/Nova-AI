import type { Nova AIConfig } from "nova-ai/plugin-sdk/config-types";
import { describe, expect, it, vi } from "vitest";
import { createActiveCfgProvider, resolveActiveCfg, type GatewayCfgLoader } from "./active-cfg.js";

const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => Nova AIConfig>());

vi.mock("nova-ai/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): Nova AIConfig {
  return shape as unknown as Nova AIConfig;
}
