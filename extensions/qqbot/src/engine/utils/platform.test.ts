...
describe("qqbot local media path remapping", () => {
  ...
  function createNova AITestRoot() {
    const actualHome = getHomeDir();
    const nova-aiDir = path.join(actualHome, ".nova-ai");
    fs.mkdirSync(nova-aiDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(nova-aiDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createNova AITestRoot();
  ...
  it("allows structured payload files inside sibling Nova AI media subdirectories", () => {
    ...
    // trust the shared `~/.nova-ai/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".nova-ai", "media", "outbound");
