 ...
 * silk-wasm availability checks — all without importing `nova-ai/plugin-sdk`.
...
/** Return a path under `~/.nova-ai/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".nova-ai", "qqbot", ...subPaths);
}

/** Return a path under `~/.nova-ai/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.nova-ai/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Nova AI's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".nova-ai", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.nova-ai/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.nova-ai/media`, Nova AI's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `nova-ai/src/media/local-roots.ts`). Using it as a
 ...
 */
function getNova AIMediaDir(): string {
  return path.join(getHomeDir(), ".nova-ai", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".nova-ai", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Nova AI's shared `~/.nova-ai/media`
  ...
  const allowedRoots = [getNova AIMediaDir(), getQQBotMediaPath()];
