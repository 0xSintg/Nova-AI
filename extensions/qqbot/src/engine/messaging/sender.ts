...
let nova-aiVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; Nova AI/${nova-aiVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; nova-aiVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.nova-aiVersion) {
    nova-aiVersion = options.nova-aiVersion;
  }
}

/** Update the Nova AI framework version in the User-Agent (called after runtime injection). */
export function setNova AIVersion(version: string): void {
  if (version) {
