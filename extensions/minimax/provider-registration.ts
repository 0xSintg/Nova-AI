...
import { buildOauthProviderAuthResult } from "nova-ai/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "nova-ai/plugin-sdk/provider-auth-api-key";
import { buildProviderReplayFamilyHooks } from "nova-ai/plugin-sdk/provider-model-shared";
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "nova-ai/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "nova-ai/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: Nova AIConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: Nova AIConfig): string | undefined {
...
export function registerMinimaxProviders(api: Nova AIPluginApi) {
