...
vi.mock("nova-ai/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/media-understanding")>(
    "nova-ai/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("nova-ai/plugin-sdk/media-understanding");
  vi.resetModules();
});
