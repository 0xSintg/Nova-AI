import { type ChannelDoctorAdapter } from "nova-ai/plugin-sdk/channel-contract";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
import {
  detectPluginInstallPathIssue,
  formatPluginInstallPathIssue,
  removePluginFromConfig,
} from "nova-ai/plugin-sdk/runtime-doctor";
import {
...
function hasConfiguredMatrixChannel(cfg: Nova AIConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: Nova AIConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: Nova AIConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "nova-ai doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "nova-ai doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: Nova AIConfig): Promise<string[]> {
  ...
  return formatPluginInstallPathIssue({
    issue,
    pluginLabel: "Matrix",
    defaultInstallCommand: "nova-ai plugins install @nova-ai/matrix",
  }).map((entry) => `- ${entry}`);
}

export async function cleanStaleMatrixPluginConfig(cfg: Nova AIConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
