...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  try {
    ...
    const homeDir = env.NOVA_AI_HOME?.trim() || env.HOME?.trim() || os.homedir();
    return path.join(homeDir, ".nova-ai");
  }
}
...
function shouldReadLegacyCredentialsForAccount(accountId?: string | null): boolean {
  ...
  const cfg = getMatrixRuntime().config.current() as Nova AIConfig;
