...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleNova AIDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Nova AI devices detected for ${params.auth.userId}: ${deviceHealth.staleNova AIDevices.map((device) => device.deviceId).join(", ")}. Run 'nova-ai matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
