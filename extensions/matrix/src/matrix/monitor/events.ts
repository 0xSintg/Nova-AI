...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'nova-ai matrix verify status --verbose --account ${accountId}' and 'nova-ai matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Nova AI runtime is using the same account. " +
    `Check 'nova-ai matrix verify status --verbose --account ${accountId}' and 'nova-ai matrix devices list --account ${accountId}'.`
  );
}
