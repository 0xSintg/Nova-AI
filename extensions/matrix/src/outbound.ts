...
import { createReplyToFanout } from "nova-ai/plugin-sdk/outbound-runtime";
import { resolvePayloadMediaUrls } from "nova-ai/plugin-sdk/reply-payload";
import type { ReplyPayload } from "nova-ai/plugin-sdk/reply-runtime";
...
const MATRIX_NOVA_AI_PRESENTATION_KEY = "com.nova-ai.presentation" as const;
const MATRIX_NOVA_AI_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_NOVA_AI_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_NOVA_AI_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_NOVA_AI_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_NOVA_AI_PRESENTATION_KEY]: buildMatrixPresentationContent(params.presentation),
        },
      },
    },
  };
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_NOVA_AI_PRESENTATION_KEY]: presentation } : undefined;
}
