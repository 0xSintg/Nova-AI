...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<Nova AIConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<Nova AIConfig["channels"]>["matrix"]>>,
): Nova AIConfig {
  return {
    channels: {
      matrix: {
