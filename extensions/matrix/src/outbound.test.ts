...
describe("matrixOutbound cfg threading", () => {
  ...
  it("passes resolved cfg to sendMessageMatrix for media sends", async () => {
    ...
    expect(options.mediaLocalRoots).toEqual(["/tmp/nova-ai"]);
  ...
  it("renders MessagePresentation into Matrix custom content metadata", async () => {
    ...
    expect(matrixData.extraContent?.["com.nova-ai.presentation"]).toEqual({
      ...presentation,
      version: 1,
    ...
    await matrixOutbound.sendPayload!({
      ...
      payload: {
        ...
        channelData: {
          matrix: {
            extraContent: {
              "com.nova-ai.presentation": presentationContent,
            },
          },
        },
      },
