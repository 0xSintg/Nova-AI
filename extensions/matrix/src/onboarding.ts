...
import type { DmPolicy } from "nova-ai/plugin-sdk/config-contracts";
import {
...
import { isPrivateNetworkOptInEnabled } from "nova-ai/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "Nova AI agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "Nova AI Gateway",
      }),
    ) ?? "";
