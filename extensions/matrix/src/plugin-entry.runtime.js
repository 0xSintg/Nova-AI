...
function hasTrustedNova AIRootIndicator(packageRoot, packageJson) {
  ...
  const hasNova AIBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("nova-ai")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.nova-ai === "string");
  const hasNova AIEntrypoint = fs.existsSync(path.join(packageRoot, "nova-ai.mjs"));
  return hasCliEntryExport || hasNova AIBin || hasNova AIEntrypoint;
}

function findNova AIPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "nova-ai" && hasTrustedNova AIRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findNova AIPackageRoot(moduleDir);
  if (location) {
