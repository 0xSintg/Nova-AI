import { CUSTOM_LOCAL_AUTH_MARKER } from "nova-ai/plugin-sdk/provider-auth";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "nova-ai/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "nova-ai/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "nova-ai/plugin-sdk/provider-setup";
import type { WizardPrompter } from "nova-ai/plugin-sdk/setup";
...
vi.mock("nova-ai/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("nova-ai/plugin-sdk/provider-auth");
  vi.doUnmock("nova-ai/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): Nova AIConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as Nova AIConfig),
    env: params?.env ?? {},
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.NOVA_AI_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("NOVA_AI_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
