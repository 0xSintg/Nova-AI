...
vi.mock("nova-ai/plugin-sdk/provider-auth-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/provider-auth-runtime")>();
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("nova-ai/plugin-sdk/provider-auth-runtime");
  vi.resetModules();
});
