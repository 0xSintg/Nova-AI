...
import type { ResolvedAgentRoute } from "nova-ai/plugin-sdk/routing";
...
type ResolveConfiguredBindingRouteFn =
  typeof import("nova-ai/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("nova-ai/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("nova-ai/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("nova-ai/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("nova-ai/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/conversation-runtime")>(
    "nova-ai/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/session-store-runtime")>(
    "nova-ai/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/command-auth-native")>(
    "nova-ai/plugin-sdk/command-auth-native",
  );
...
vi.mock("nova-ai/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/agent-runtime")>(
    "nova-ai/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/plugin-runtime")>(
    "nova-ai/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/nova-ai-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/nova-ai-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/nova-ai-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/nova-ai-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as Nova AIConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/nova-ai-sessions/sessions.json",
        sessionsDir: "/tmp/nova-ai-sessions",
        fallbackSessionFile: path.resolve("/tmp/nova-ai-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/nova-ai-sessions", "sess-topic-topic-42.jsonl"),
        messageThreadId: 42,
      },
