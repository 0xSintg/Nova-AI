...
import * as runtimeEnvModule from "nova-ai/plugin-sdk/runtime-env";
import { withEnv } from "nova-ai/plugin-sdk/test-env";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", NOVA_AI_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): Nova AIConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): Nova AIConfig => ({
    channels: {
      telegram: {
