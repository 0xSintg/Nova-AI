...
function formatTelegramAccountConfigPath(cfg: Nova AIConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: Nova AIConfig): {
...
async function repairTelegramConfig(params: { cfg: Nova AIConfig }): Promise<{
  config: Nova AIConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: Nova AIConfig): Promise<{
  config: Nova AIConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("nova-ai/plugin-sdk/runtime");
