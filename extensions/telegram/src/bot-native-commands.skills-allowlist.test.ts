...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "nova-ai/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("nova-ai-telegram-skills-");
    await writeSkill({
    ...
    listSkillCommandsForAgents.mockImplementation(
      ({ cfg, agentIds }: { cfg: Nova AIConfig; agentIds?: string[] }) =>
