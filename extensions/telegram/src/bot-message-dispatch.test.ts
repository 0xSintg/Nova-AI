...
const getAgentScopedMediaLocalRoots = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.nova-ai/workspace-${agentId}`]),
);
...
vi.mock("nova-ai/plugin-sdk/agent-harness-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/agent-harness-runtime")>();
  return {
    ...actual,
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    ...
    base.turn = {
      storePath: "/tmp/nova-ai/telegram-sessions.json",
  ...
  it("streams drafts in private threads and forwards thread id", async () => {
    ...
    expect(mediaLocalRoots?.some((root) => /[\\/]\.nova-ai[\\/]workspace-work$/u.test(root))).toBe(
      true,
    );
