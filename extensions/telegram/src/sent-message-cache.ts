...
import { replaceFileAtomicSync } from "nova-ai/plugin-sdk/security-runtime";
import { resolveStorePath } from "nova-ai/plugin-sdk/session-store-runtime";

const TTL_MS = 24 * 60 * 60 * 1000;
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("nova-ai.telegramSentMessagesState");
...
function resolveSentMessageStorePath(cfg?: Pick<Nova AIConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<Nova AIConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<Nova AIConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<Nova AIConfig, "session">,
): void {
