...
import { resolveChannelStreamingBlockEnabled } from "nova-ai/plugin-sdk/channel-streaming";
import { resolveNativeCommandSessionTargets } from "nova-ai/plugin-sdk/command-auth-native";
import {
...
import type { ChannelGroupPolicy } from "nova-ai/plugin-sdk/config-contracts";
import type {
  ReplyToMode,
...
import { resolveMarkdownTableMode } from "nova-ai/plugin-sdk/markdown-table-runtime";
import { resolveSendableOutboundReplyParts } from "nova-ai/plugin-sdk/reply-payload";
import { resolveAgentRoute } from "nova-ai/plugin-sdk/routing";
import { getRuntimeConfigSnapshot } from "nova-ai/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "nova-ai/plugin-sdk/runtime-env";
...
type TelegramChunkMode = ReturnType<
  typeof import("nova-ai/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("nova-ai/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): Nova AIConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: Nova AIConfig): TelegramAccountConfig => {
    try {
