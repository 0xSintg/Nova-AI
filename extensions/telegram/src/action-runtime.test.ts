...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): Nova AIConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as Nova AIConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): Nova AIConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as Nova AIConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: Nova AIConfig, accountId = "media") {
    await handleTelegramAction(
