...
import { resolveFetch } from "nova-ai/plugin-sdk/fetch-runtime";
...
const TEST_UNDICI_RUNTIME_DEPS_KEY = "__NOVA_AI_TEST_UNDICI_RUNTIME_DEPS__";
...
vi.mock("nova-ai/plugin-sdk/runtime-env", () => ({
...
beforeEach(() => {
  ...
  for (const key of [
    ...
    "no_proxy",
    "NOVA_AI_PROXY_URL",
    "NOVA_AI_PROXY_ACTIVE",
    "NOVA_AI_PROXY_CA_FILE",
  ]) {
    vi.stubEnv(key, "");
  }
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "nova-ai-telegram-proxy-ca-"));
  tempDirs.push(dir);
...
describe("resolveTelegramFetch", () => {
  ...
  it("adds managed proxy CA trust to Telegram env proxy dispatchers", async () => {
    ...
    vi.stubEnv("NOVA_AI_PROXY_ACTIVE", "1");
    vi.stubEnv("NOVA_AI_PROXY_CA_FILE", caFile);
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses the Nova AI debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("NOVA_AI_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("NOVA_AI_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses NOVA_AI_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("NOVA_AI_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
    ...
    expect(ProxyAgentCtor).toHaveBeenCalledTimes(1);
    const proxyOptions = constructorOptions(ProxyAgentCtor, "Nova AI proxy") as {
      allowH2?: boolean;
      uri?: string;
  ...
  it("preserves caller-provided custom fetch when NOVA_AI_PROXY_URL is present", async () => {
    vi.stubEnv("NOVA_AI_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over NOVA_AI_PROXY_URL for Telegram", async () => {
    vi.stubEnv("NOVA_AI_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("keeps resolver-scoped transport policy for Nova AI proxy fetches", async () => {
