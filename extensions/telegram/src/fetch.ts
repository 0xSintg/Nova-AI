...
} from "nova-ai/plugin-sdk/fetch-runtime";
import {
  captureHttpExchange,
  resolveEffectiveDebugProxyUrl,
} from "nova-ai/plugin-sdk/proxy-capture";
import { resolveRequestUrl } from "nova-ai/plugin-sdk/request-url";
...
// fix for the leak observed in nova-ai#68128 is the transport lifecycle that
...
function resolveNova AIProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.NOVA_AI_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveNova AIProxyUrlForTelegram() : undefined;
