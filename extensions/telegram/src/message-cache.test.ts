...
describe("telegram message cache", () => {
  it("hydrates reply chains from persisted cached messages", async () => {
    const storePath = `/tmp/nova-ai-telegram-message-cache-${process.pid}-${Date.now()}.json`;
  ...
  it("records embedded reply targets as normal cached messages", async () => {
    const storePath = `/tmp/nova-ai-telegram-message-cache-reply-target-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
  ...
  it("shares one persisted bucket across live cache instances", async () => {
    const storePath = `/tmp/nova-ai-telegram-message-cache-shared-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
      const firstCache = createTelegramMessageCache({ persistedPath });
  ...
  it("appends cached records between compactions and reloads the bounded cache window", async () => {
    const storePath = `/tmp/nova-ai-telegram-message-cache-append-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
  ...
  it("keeps the persisted log bounded by compacting cached records", async () => {
    const storePath = `/tmp/nova-ai-telegram-message-cache-compact-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
  ...
  it("loads mixed legacy array caches and rewrites them as line-delimited entries", async () => {
    const storePath = `/tmp/nova-ai-telegram-message-cache-legacy-${process.pid}-${Date.now()}.json`;
    const persistedPath = resolveTelegramMessageCachePath(storePath);
    await rm(persistedPath, { force: true });
    try {
      const legacyEntries = [
  ...
  it("does not select messages before the latest session reset command", () => {
    ...
    const staleInstruction = "okay so we just flip in nova-ai? if yes do it up";
    const record = (params: {
      id: number;
      text: string;
      timestampMs: number;
