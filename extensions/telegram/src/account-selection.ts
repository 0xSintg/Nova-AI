...
} from "nova-ai/plugin-sdk/account-core";
import {
  DEFAULT_ACCOUNT_ID,
  normalizeAccountId,
...
function resolveDefaultAgentId(cfg: Nova AIConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: Nova AIConfig): string[] {
...
function listBoundAccountIds(cfg: Nova AIConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: Nova AIConfig, channelId: string): string | null {
...
function hasImplicitDefaultTelegramAccount(cfg: Nova AIConfig): boolean {
...
export function listTelegramAccountIds(cfg: Nova AIConfig): string[] {
...
export function resolveDefaultTelegramAccountSelection(cfg: Nova AIConfig): {
  accountId: string;
  accountIds: string[];
...
export function resolveDefaultTelegramAccountId(cfg: Nova AIConfig): string {
