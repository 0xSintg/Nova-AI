...
vi.mock("nova-ai/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/config-mutation")>(
    "nova-ai/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/cron-store-runtime")>(
    "nova-ai/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/nova-ai.json");
