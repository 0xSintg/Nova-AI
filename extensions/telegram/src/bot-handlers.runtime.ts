...
import { parseExecApprovalCommandText } from "nova-ai/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "nova-ai/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "nova-ai/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "nova-ai/plugin-sdk/command-auth-native";
import { hasControlCommand } from "nova-ai/plugin-sdk/command-detection";
import { isAbortRequestText } from "nova-ai/plugin-sdk/command-primitives-runtime";
import { buildCommandsMessagePaginated } from "nova-ai/plugin-sdk/command-status";
import type { DmPolicy, Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
import type {
  TelegramDirectConfig,
  TelegramGroupConfig,
  TelegramTopicConfig,
} from "nova-ai/plugin-sdk/config-contracts";
import { mutateConfigFile } from "nova-ai/plugin-sdk/config-mutation";
import {
...
import { applyModelOverrideToSessionEntry } from "nova-ai/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "nova-ai/plugin-sdk/models-provider-runtime";
import { resolveAgentRoute } from "nova-ai/plugin-sdk/routing";
import { resolveThreadSessionKeys } from "nova-ai/plugin-sdk/routing";
import { danger, logVerbose, warn } from "nova-ai/plugin-sdk/runtime-env";
import {
  loadSessionStore,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in nova-ai.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
