...
import type { Nova AIPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredNova AITmpDir: vi.fn(() => path.join(os.tmpdir(), "nova-ai-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredNova AITmpDir: pluginApiMocks.resolvePreferredNova AITmpDir,
...
function createApi(params?: {
  ...
  runtime?: Nova AIPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as Nova AIPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: Nova AIPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as Nova AIPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredNova AITmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredNova AITmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Nova AI iOS app:");
    ...
    expect(text).not.toContain("![Nova AI pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Nova AI iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://nova-ai.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://nova-ai.local:18789");
  });
