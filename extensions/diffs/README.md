...
Read-only diff viewer plugin for **Nova AI** agents.
...
Set plugin-wide defaults in `~/.nova-ai/nova-ai.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/nova-ai",
        },
      },
    },
  },
...
Before:
Nova AI supports plugins.

After:
Nova AI supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/nova-ai-diffs`).
