...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-nova-ai-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-nova-ai-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.nova-aiDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.nova-aiDiffsError = "true";
