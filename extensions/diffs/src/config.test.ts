...
import type { JsonSchemaObject } from "nova-ai/plugin-sdk/config-schema";
...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../nova-ai.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/nova-ai/",
      }),
    ).toBe("https://example.com/nova-ai");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/nova-ai?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/nova-ai#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/nova-ai/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/nova-ai");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/nova-ai",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/nova-ai/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("nova-aiDiffsReady");
