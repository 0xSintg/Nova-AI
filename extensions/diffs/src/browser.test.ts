...
import { createMockServerResponse } from "nova-ai/plugin-sdk/test-env";
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import type { Nova AIConfig } from "../api.js";
import type { Nova AIPluginApi, Nova AIPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("nova-ai-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<Nova AIPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: Nova AIPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    let registeredHttpRouteHandler: HttpRouteHandler | undefined;
    let configFile: Nova AIConfig = {
      gateway: {
        port: 18789,
        bind: "loopback",
      },
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/nova-ai",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    const api = createTestPluginApi({
      ...
      registerTool(tool: Parameters<Nova AIPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as Nova AIPluginApi);
