...
import { isRecord, normalizeOptionalString } from "nova-ai/plugin-sdk/string-coerce-runtime";
...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["nova-aiPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["nova-ai", "pollId"]) ??
    readNestedString(value, ["nova-ai", "poll", "id"]) ??
    readNestedString(value, ["data", "nova-aiPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "nova-ai", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          nova-aiPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "nova-ai poll vote",
          displayText: "Vote recorded",
          value: { nova-aiPollId: pollId, pollId },
        },
      },
    ],
  };
