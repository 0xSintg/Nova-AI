...
import type { Nova AIConfig, RuntimeEnv } from "../runtime-api.js";
...
function createConfig(port: number): Nova AIConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: Nova AIConfig,
  patch: NonNullable<NonNullable<Nova AIConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): Nova AIConfig {
  const registered = readMockCallArg(registerMSTeamsHandlers, 0, 1) as { cfg?: Nova AIConfig };
  if (!registered?.cfg) {
