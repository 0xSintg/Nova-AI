...
vi.mock("nova-ai/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: nova-ai", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "nova-ai" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: nova-ai", async () => {
