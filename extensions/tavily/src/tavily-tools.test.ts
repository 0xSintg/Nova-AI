...
import type { Nova AIPluginApi } from "nova-ai/plugin-sdk/plugin-runtime";
...
function fakeApi(): Nova AIPluginApi {
  return {
    config: {},
  } as Nova AIPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.nova-ai.com", "", "nova-ai.com"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.nova-ai.com", "nova-ai.com"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<Nova AIPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<Nova AIPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "nova-ai" });
    ...
    expect(searchParams.query).toBe("nova-ai");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as Nova AIConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
