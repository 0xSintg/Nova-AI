...
describe("canvas host state dir defaults", () => {
  ...
  it("uses NOVA_AI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("nova-ai-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Nova AI Canvas");
      } finally {
        await handler.close();
      }
    });
