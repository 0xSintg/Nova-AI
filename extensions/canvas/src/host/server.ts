...
import { detectMime } from "nova-ai/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "nova-ai/plugin-sdk/runtime-env";
import { resolveStateDir } from "nova-ai/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "nova-ai/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "nova-ai/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.nova-aiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.nova-aiCanvasA2UIAction &&
        typeof window.nova-aiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.nova-aiSendUserAction === "function";
  ...
  window.addEventListener("nova-ai:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Nova AI node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.nova-aiSendUserAction === "function"
        ? window.nova-aiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.NOVA_AI_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Nova AI Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
