...
vi.mock("nova-ai/plugin-sdk/channel-actions", async (importOriginal) => ({
  ...(await importOriginal<typeof import("nova-ai/plugin-sdk/channel-actions")>()),
...
describe("Canvas tool", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      tempRoot = await mkdtemp(path.join(os.tmpdir(), "nova-ai-canvas-tool-"));
  ...
  it("applies configured image limits to canvas snapshots", async () => {
    ...
    expect(imageResultParams?.path).toMatch(/nova-ai-canvas-snapshot-.*\.png$/);
