...
export function isCanvasPluginEnabled(config?: Nova AIConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: Nova AIConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the Nova AI state canvas directory.",
      advanced: true,
    },
    "host.port": {
