import type { AnyAgentTool, Nova AIPluginApi } from "nova-ai/plugin-sdk/plugin-entry";
...
function registerCanvas() {
  const routes: Array<Parameters<Nova AIPluginApi["registerHttpRoute"]>[0]> = [];
  const services: Array<Parameters<Nova AIPluginApi["registerService"]>[0]> = [];
  const resolvers: Array<Parameters<Nova AIPluginApi["registerHostedMediaResolver"]>[0]> = [];
  const tools: Array<Parameters<Nova AIPluginApi["registerTool"]>[0]> = [];
  const cliFeatures: Array<{
    registrar: Parameters<Nova AIPluginApi["registerNodeCliFeature"]>[0];
    opts: Parameters<Nova AIPluginApi["registerNodeCliFeature"]>[1];
  }> = [];
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__nova-ai__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__nova-ai__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );
