...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    NOVA_AI_HOME: params.homeDir,
    NOVA_AI_CONFIG_PATH: params.configPath,
    NOVA_AI_STATE_DIR: params.stateDir,
    NOVA_AI_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    NOVA_AI_SKIP_STARTUP_MODEL_PREWARM: "1",
    NOVA_AI_NO_RESPAWN: "1",
    NOVA_AI_TEST_FAST: "1",
    NOVA_AI_QA_PARENT_PID: String(process.pid),
    NOVA_AI_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { NOVA_AI_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    NOVA_AI_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    NOVA_AI_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { NOVA_AI_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { NOVA_AI_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: Nova AIConfig) => Nova AIConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredNova AITmpDir(), "nova-ai-qa-suite-"),
  );
  ...
  const keepTemp = process.env.NOVA_AI_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
  let child: ReturnType<typeof spawn> | null = null;
  let cfg!: Nova AIConfig;
