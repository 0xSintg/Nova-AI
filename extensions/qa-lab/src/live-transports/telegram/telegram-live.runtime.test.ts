...
vi.mock("nova-ai/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/ssrf-runtime")>(
    "nova-ai/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      testing.resolveTelegramQaRuntimeEnv({
        ...
        NOVA_AI_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      testing.resolveTelegramQaRuntimeEnv({
        NOVA_AI_QA_TELEGRAM_GROUP_ID: "qa-group",
        NOVA_AI_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("NOVA_AI_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
  ...
  it("applies NOVA_AI_QA_SUITE_PROGRESS override to Telegram live logging", () => {
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "false",
        NOVA_AI_QA_SUITE_PROGRESS: "true",
      }),
    ).toBe(true);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        NOVA_AI_QA_SUITE_PROGRESS: "false",
      }),
    ).toBe(false);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        NOVA_AI_QA_SUITE_PROGRESS: "definitely",
      }),
    ).toBe(true);
  });
  ...
  it("normalizes the Telegram QA canary timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        NOVA_AI_QA_TELEGRAM_CANARY_TIMEOUT_MS: "90000",
      }),
    ).toBe(90_000);
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        NOVA_AI_QA_TELEGRAM_CANARY_TIMEOUT_MS: "nope",
      }),
    ).toBe(30_000);
  });
  ...
  it("normalizes the Telegram QA scenario timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "nope",
      }),
    ).toBe(45_000);
  });
  ...
  it("includes mention gating in the Telegram live scenario catalog", () => {
    ...
    ).toEqual(["Nova AI", "Model:", "Session:", "Activation:"]);
    expect(
      scenarios
    ...
    expect(otherBotStep?.input).toBe("/status@Nova AIQaOtherBot");
  ...
  it("lists default status and regression refs in the Telegram scenario catalog", () => {
    ...
    expect(status.regressionRefs).toEqual(["nova-ai/nova-ai#74698"]);
    ...
    expect(streamSingle.regressionRefs).toEqual(["nova-ai/nova-ai#39905"]);
