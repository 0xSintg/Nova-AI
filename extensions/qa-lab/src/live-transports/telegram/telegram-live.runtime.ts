...
import { formatErrorMessage } from "nova-ai/plugin-sdk/error-runtime";
import { fetchWithSsrFGuard } from "nova-ai/plugin-sdk/ssrf-runtime";
...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    regressionRefs: ["nova-ai/nova-ai#74698"],
    timeoutMs: 45_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        expectReply: true,
        input: `/status@${sutUsername}`,
        expectedTextIncludes: ["Nova AI", "Model:", "Session:", "Activation:"],
      }),
  },
  {
    ...
    buildRun: (sutUsername) => {
      const steps = [
        ...
        },
        {
          ...
          expectReply: true,
          input: `/status@${sutUsername}`,
          expectedTextIncludes: ["Nova AI", "Session:"],
        },
        {
          expectReply: true,
          input: `/help@${sutUsername}`,
  ...
  },
  {
    ...
    buildRun: () =>
      telegramQaStepRun({
        expectReply: false,
        input: "/status@Nova AIQaOtherBot",
      }),
  },
  {
  ...
  },
  {
    ...
    regressionRefs: ["nova-ai/nova-ai#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "NOVA_AI_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "NOVA_AI_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "NOVA_AI_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "NOVA_AI_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "NOVA_AI_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("NOVA_AI_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "NOVA_AI_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledNova AITelegramOnboardingPreflight(params: {
  openClawCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredNova AITmpDir(), "nova-ai-npm-telegram-"),
  );
  const homeDir = path.join(tempRoot, "home");
  const stateDir = path.join(homeDir, ".nova-ai");
  ...
  const env = {
    ...process.env,
    HOME: homeDir,
    NOVA_AI_HOME: stateDir,
    NOVA_AI_CONFIG_PATH: path.join(stateDir, "nova-ai.json"),
    NOVA_AI_STATE_DIR: stateDir,
    NOVA_AI_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-nova-ai-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutNova AICommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledNova AITelegramOnboardingPreflight({
        openClawCommand: params.sutNova AICommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutNova AICommand
        ? {
            executablePath: params.sutNova AICommand,
  ...
  const publishedCleanupIssues = redactPublicMetadata
    ? cleanupIssues.map(() => "details redacted (NOVA_AI_QA_REDACT_PUBLIC_METADATA=1)")
    : cleanupIssues;
