...
import { startWhatsAppQaDriverSession } from "@nova-ai/whatsapp/api.js";
import { normalizeE164 } from "nova-ai/plugin-sdk/account-resolution";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "NOVA_AI_QA_WHATSAPP_CAPTURE_CONTENT";
...
const WHATSAPP_QA_ENV_KEYS = [
  "NOVA_AI_QA_WHATSAPP_DRIVER_PHONE_E164",
  "NOVA_AI_QA_WHATSAPP_SUT_PHONE_E164",
  "NOVA_AI_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
  "NOVA_AI_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64",
] as const;
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /Nova AI: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `nova-aiqa reply with only this exact marker: ${replyToken}`,
...
function inferWhatsAppCredentialSource(
  ...
  const normalized =
    value?.trim().toLowerCase() || env.NOVA_AI_QA_CREDENTIAL_SOURCE?.trim().toLowerCase();
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "NOVA_AI_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "NOVA_AI_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "NOVA_AI_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "NOVA_AI_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.NOVA_AI_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "NOVA_AI_QA_WHATSAPP",
  );
}
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredNova AITmpDir(), "nova-ai-whatsapp-qa-"),
    );
