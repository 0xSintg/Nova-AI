...
describe("WhatsApp QA live runtime", () => {
  ...
  it("unpacks auth archives into a caller-provided temp directory", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-wa-qa-test-"));
    try {
  ...
  it("uses automatic visible replies for WhatsApp group mention gating", () => {
    ...
    expect(scenarioRun.input).toContain("nova-aiqa reply with only this exact marker");
    ...
    const cfg = testing.buildWhatsAppQaConfig(
      {},
      {
        ...
        authDir: "/tmp/nova-ai-whatsapp-qa-auth",
        dmPolicy: "allowlist",
    ...
    expect(cfg.messages?.groupChat?.mentionPatterns).toContain("\\bnova-aiqa\\b");
  });
