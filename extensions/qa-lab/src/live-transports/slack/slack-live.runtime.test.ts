...
describe("Slack live QA runtime helpers", () => {
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        NOVA_AI_QA_SLACK_CHANNEL_ID: "C123456789",
        NOVA_AI_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        NOVA_AI_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        NOVA_AI_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        NOVA_AI_QA_SLACK_CHANNEL_ID: "qa-channel",
        NOVA_AI_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        NOVA_AI_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        NOVA_AI_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("NOVA_AI_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "nova-ai-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing NOVA_AI_QA_CONVEX_SITE_URL");
