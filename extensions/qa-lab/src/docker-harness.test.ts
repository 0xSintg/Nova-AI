...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "nova-ai.json"),
    ...
    expect(compose).toContain("image: nova-ai:qa-local-prebaked");
    ...
    expect(compose).toContain(":/opt/nova-ai-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://nova-ai-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/nova-ai-qa-lab-ui");
    expect(compose).toContain(":/opt/nova-ai-repo:ro");
    expect(compose).toContain("./state:/opt/nova-ai-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/nova-ai-scaffold/seed-workspace/. /tmp/nova-ai/workspace/",
    );
    expect(compose).toContain("NOVA_AI_CONFIG_PATH: /tmp/nova-ai/nova-ai.json");
    expect(compose).toContain("NOVA_AI_STATE_DIR: /tmp/nova-ai/state");
    expect(compose).toContain('NOVA_AI_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("NOVA_AI_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "nova-ai.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/nova-ai/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/nova-ai",
        imageName: "nova-ai:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("nova-ai:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t nova-ai:qa-local-prebaked --build-arg NOVA_AI_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/nova-ai",
    ]);
  });
});
