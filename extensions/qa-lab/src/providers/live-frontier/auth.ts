...
} from "nova-ai/plugin-sdk/provider-auth";
import { resolveQaAgentAuthDir, writeQaAuthProfiles } from "../shared/auth-store.js";

export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "NOVA_AI_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "NOVA_AI_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "NOVA_AI_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["NOVA_AI_LIVE_ANTHROPIC_KEY"],
  gemini: ["NOVA_AI_LIVE_GEMINI_KEY"],
  openai: ["NOVA_AI_LIVE_OPENAI_KEY", "OPENAI_API_KEY"],
  "openai-codex": [
    "CODEX_API_KEY",
    "NOVA_AI_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: Nova AIConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: Nova AIConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: Nova AIConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.NOVA_AI_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "pi") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or NOVA_AI_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Nova AI OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
