...
describe("runtime tool fixture", () => {
  ...
  it("does not fail Codex-native fixtures solely because Nova AI dynamic exposure is absent", async () => {
    const env = await makeEnv({
      ...
      gateway: {
        ...
        tempRoot: "",
        workspaceDir: "",
        runtimeEnv: { NOVA_AI_QA_FORCE_RUNTIME: "codex" },
        call: vi.fn(),
      },
    });
    ...
    expect(details).toContain("Nova AI dynamic exposure is intentionally omitted");
    expect(details).toContain("mock provider happy planned args (diagnostic only)");
  });

  it("still fails required Nova AI dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
