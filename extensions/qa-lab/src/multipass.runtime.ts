...
import { sleep } from "nova-ai/plugin-sdk/runtime-env";
import { appendRegularFile } from "nova-ai/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/nova-ai-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/nova-ai";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/nova-ai-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `nova-ai-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "NOVA_AI_CONFIG_PATH" &&
          key !== "NOVA_AI_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `NOVA_AI_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `NOVA_AI_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm nova-ai qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredNova AITmpDir(), `${plan.vmName}-qa-suite-`),
  );
