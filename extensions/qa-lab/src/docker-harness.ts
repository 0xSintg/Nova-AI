...
const QA_LAB_UI_OVERLAY_DIR = "/opt/nova-ai-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        NOVA_AI_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${params.bindUiDist ? `      - ${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - NOVA_AI_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/nova-ai-scaffold/nova-ai.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://nova-ai-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      NOVA_AI_CONFIG_PATH: /tmp/nova-ai/nova-ai.json
      NOVA_AI_STATE_DIR: /tmp/nova-ai/state
      NOVA_AI_NO_RESPAWN: "1"
      NOVA_AI_SKIP_GMAIL_WATCHER: "1"
      NOVA_AI_SKIP_BROWSER_CONTROL_SERVER: "1"
      NOVA_AI_SKIP_CANVAS_HOST: "1"
      NOVA_AI_PROFILE: ""
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/nova-ai/workspace /tmp/nova-ai/state && cp /opt/nova-ai-scaffold/nova-ai.json /tmp/nova-ai/nova-ai.json && cp -R /opt/nova-ai-scaffold/seed-workspace/. /tmp/nova-ai/workspace/ && ln -snf /opt/nova-ai-repo /tmp/nova-ai/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t nova-ai:qa-local-prebaked --build-arg NOVA_AI_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "nova-ai:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "nova-ai.json"),
  ];
