...
vi.mock("nova-ai/plugin-sdk/gateway-runtime", () => ({
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.NOVA_AI_HOME = originalHome;
      }
    }

    expect(process.env.NOVA_AI_HOME).toBe(originalHome);
  });
