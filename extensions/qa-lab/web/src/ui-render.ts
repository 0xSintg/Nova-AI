...
function renderCaptureStartupInstructions(status: CaptureStartupStatus | null): string {
  ...
  const gatewayStart = `NOVA_AI_DEBUG_PROXY_ENABLED=1 \\
NOVA_AI_DEBUG_PROXY_REQUIRE=1 \\
NOVA_AI_DEBUG_PROXY_URL=http://127.0.0.1:7799 \\
pnpm nova-ai gateway --port 18789 --bind loopback`;
  const qaStart = "pnpm qa:lab:ui --port 43124 --control-ui-url http://127.0.0.1:18789/";
  const caInstall = "pnpm proxy:install-ca";
  const caTrust =
    "sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain /Users/thoffman/.nova-ai/debug-proxy/certs/root-ca.pem";
