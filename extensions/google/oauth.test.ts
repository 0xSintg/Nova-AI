...
describe("loginGeminiCliOAuth", () => {
  ...
  function userInfoResponse(): Response {
    return responseJson({ email: "lobster@nova-ai.com" });
  }
  ...
  beforeEach(() => {
    envSnapshot = Object.fromEntries(ENV_KEYS.map((key) => [key, process.env[key]]));
    process.env.NOVA_AI_GEMINI_OAUTH_CLIENT_ID = "test-client-id.apps.googleusercontent.com";
    process.env.NOVA_AI_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret"; // pragma: allowlist secret
