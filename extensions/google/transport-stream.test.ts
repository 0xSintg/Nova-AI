...
vi.mock("nova-ai/plugin-sdk/provider-transport-runtime", async (importOriginal) => ({
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for(
  "nova-ai.modelProviderRequestTransport",
);
...
describe("google transport stream", () => {
  ...
  afterAll(() => {
    vi.doUnmock("nova-ai/plugin-sdk/provider-transport-runtime");
    vi.resetModules();
  });
  ...
  it("retries Gemini 3 requests with lean thinking when the first attempt has no first response", async () => {
    vi.stubEnv("NOVA_AI_GOOGLE_GEMINI_FIRST_RESPONSE_RETRY_MS", "10");
    guardedFetchMock
  ...
  it("refreshes authorized_user ADC before Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "nova-ai-google-vertex-adc-"));
  ...
  it("refreshes authorized_user ADC from the Windows APPDATA fallback for Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "nova-ai-google-vertex-appdata-adc-"));
  ...
  it("treats the Google transport alias as the same route for signature replay", () => {
    const model = {
      ...
      api: "nova-ai-google-generative-ai-transport",
    } as Model<"nova-ai-google-generative-ai-transport">;
