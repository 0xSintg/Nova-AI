import { transcodeAudioBufferToOpus } from "nova-ai/plugin-sdk/media-runtime";
import {
  assertOkOrThrowProviderError,
  postJsonRequest,
  sanitizeConfiguredModelProviderRequest,
} from "nova-ai/plugin-sdk/provider-http";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/provider-onboard";
...
import { asObject, trimToUndefined } from "nova-ai/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: Nova AIConfig): string | undefined {
...
function isNova AIGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isNova AIGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
