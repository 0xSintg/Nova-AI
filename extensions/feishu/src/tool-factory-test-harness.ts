...
export function createToolFactoryHarness(cfg: Nova AIPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<Nova AIPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
