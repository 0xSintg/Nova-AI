...
const emptyConfig: NovabotConfig = {};
...
function expectPathIsolatedToTmpRoot(pathValue: string, key: string): void {
  ...
  const tmpRoot = realpathSync(resolvePreferredNova AITmpDir());
...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("nova-ai/plugin-sdk/media-runtime");
    vi.resetModules();
  });
  ...
  it("passes mediaLocalRoots as localRoots to loadWebMedia for local paths (#27884)", async () => {
    ...
    const roots = ["/allowed/workspace", "/tmp/nova-ai"];
    await sendMediaFeishu({
      cfg: emptyConfig,
      to: "user:ou_target",
...
describe("downloadMessageResourceFeishu", () => {
  ...
  it("saves message resource streams directly to the media store", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-feishu-media-"));
    try {
      ...
      expect(result.saved.path).toContain(`${path.sep}.nova-ai${path.sep}media${path.sep}inbound`);
