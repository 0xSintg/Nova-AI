import { describeAccountSnapshot } from "nova-ai/plugin-sdk/account-helpers";
import { formatAllowFromLowercase } from "nova-ai/plugin-sdk/allow-from";
import {
...
import { createChatChannelPlugin } from "nova-ai/plugin-sdk/channel-core";
import {
  defineChannelMessageAdapter,
  type ChannelMessageSendResult,
  type MessageReceiptPartKind,
} from "nova-ai/plugin-sdk/channel-message";
import { createPairingPrefixStripper } from "nova-ai/plugin-sdk/channel-pairing";
import {
  createAllowlistProviderGroupPolicyWarningCollector,
  projectConfigAccountIdWarningCollector,
} from "nova-ai/plugin-sdk/channel-policy";
import { getSessionBindingService } from "nova-ai/plugin-sdk/conversation-runtime";
import {
  createChannelDirectoryAdapter,
  createRuntimeDirectoryLiveAdapter,
} from "nova-ai/plugin-sdk/directory-runtime";
import {
  normalizeMessagePresentation,
  renderMessagePresentationFallbackText,
} from "nova-ai/plugin-sdk/interactive-runtime";
import { createLazyRuntimeNamedExport } from "nova-ai/plugin-sdk/lazy-runtime";
import { createRuntimeOutboundDelegates } from "nova-ai/plugin-sdk/outbound-runtime";
import { createComputedAccountStatusAdapter } from "nova-ai/plugin-sdk/status-helpers";
...
function areAnyFeishuReactionActionsEnabled(cfg: NovabotConfig): boolean {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      config: {
        ...
        deleteAccount: ({ cfg, accountId }) => {
          ...
          if (isDefault) {
            ...
            const next = { ...cfg } as NovabotConfig;
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("nova-ai/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("nova-ai/plugin-sdk/config-mutation");
