...
import { createNonExitingRuntimeEnv } from "nova-ai/plugin-sdk/plugin-test-runtime";
...
async function resolveNonBotReaction(params?: { cfg?: NovabotConfig; uuid?: () => string }) {
  return await resolveReactionSyntheticEvent({
    cfg: params?.cfg ?? cfg,
    accountId: "default",
...
function buildDebounceConfig(): NovabotConfig {
  return {
    messages: {
      inbound: {
        debounceMs: 0,
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("passes prefetched botName through to handleFeishuMessage", async () => {
    ...
    vi.spyOn(dedup, "recordProcessedFeishuMessage").mockResolvedValue(true);
    vi.spyOn(dedup, "hasProcessedFeishuMessage").mockResolvedValue(false);
    const onMessage = await setupDebounceMonitor({ botName: "Nova AI Bot" });
    ...
    const firstParams = mockCallAt(handleFeishuMessageMock, 0, "Feishu message dispatch")[0] as
      | { botName?: string }
      | undefined;
    expect(firstParams?.botName).toBe("Nova AI Bot");
  });
