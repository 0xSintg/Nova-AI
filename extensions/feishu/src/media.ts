...
import type { MessageReceipt } from "nova-ai/plugin-sdk/channel-message";
import { mediaKindFromMime } from "nova-ai/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "nova-ai/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "nova-ai/plugin-sdk/media-store";
import { readByteStreamWithLimit } from "nova-ai/plugin-sdk/response-limit-runtime";
import { readRegularFile, writeExternalFileWithinRoot } from "nova-ai/plugin-sdk/security-runtime";
...
import type { NovabotConfig } from "../runtime-api.js";
...
function createConfiguredFeishuMediaClient(params: { cfg: NovabotConfig; accountId?: string }): {
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "nova-ai-feishu-img-",
...
async function downloadMessageResourceWithType(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "nova-ai-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredNova AITmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
