...
import type { MessagePresentation } from "nova-ai/plugin-sdk/interactive-runtime";
...
const cardRenderConfig: NovabotConfig = {
  channels: {
    feishu: {
      renderMode: "card",
    },
...
describe("feishuOutbound.sendText local-image auto-convert", () => {
  ...
  async function createTmpImage(ext = ".png"): Promise<{ dir: string; file: string }> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-feishu-outbound-"));
...
describe("feishuOutbound.sendPayload native cards", () => {
  ...
  async function createTmpImage(ext = ".png"): Promise<{ dir: string; file: string }> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-feishu-payload-"));
    const file = path.join(dir, `sample${ext}`);
    await fs.writeFile(file, "image-data");
    return { dir, file };
  }
