 ...
 * `__NOVA_AI_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__NOVA_AI_REDACTED__")) {
      guardSentinel("__NOVA_AI_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
