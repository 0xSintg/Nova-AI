/**
 * `nova-ai path` — shell access to the OcPath substrate verbs.
 *
...
export function registerPathCli(program: Command): void {
  const path = program
    ...
    .addHelpText("after", "\nDocs: https://docs.nova-ai.com/cli/path\n");
  ...
  // Bare `nova-ai path` prints help and exits 0 (matches the core
  // applyParentDefaultHelpAction contract — see nova-ai#73077).
  path.action(() => {
    path.outputHelp();
    process.exitCode = 0;
  });
