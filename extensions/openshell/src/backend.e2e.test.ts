...
import { createSandboxTestContext } from "nova-ai/plugin-sdk/test-fixtures";
import {
...
const NOVA_AI_OPENSHELL_E2E = process.env.NOVA_AI_E2E_OPENSHELL === "1";
const NOVA_AI_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const NOVA_AI_OPENSHELL_COMMAND =
  process.env.NOVA_AI_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "nova-ai-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && NOVA_AI_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: NOVA_AI_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(NOVA_AI_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(NOVA_AI_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-openshell-e2e-"));
      ...
      const gatewayName = `nova-ai-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `nova-ai-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/nova-ai-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: NOVA_AI_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("nova-ai-openshell-e2e");
