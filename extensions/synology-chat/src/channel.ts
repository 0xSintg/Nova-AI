/**
 * Synology Chat Channel Plugin for Nova AI.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import { DEFAULT_ACCOUNT_ID } from "nova-ai/plugin-sdk/account-id";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/account-resolution";
import {
...
import { waitUntilAbort } from "nova-ai/plugin-sdk/channel-lifecycle";
import {
...
import { createEmptyChannelDirectoryAdapter } from "nova-ai/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "nova-ai pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: Nova AIConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: Nova AIConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Nova AI",
        order: 90,
      },
      capabilities: {
