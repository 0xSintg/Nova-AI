...
vi.mock("nova-ai/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("nova-ai/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("nova-ai/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("nova-ai/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/nova-ai/synology-chat-sessions.json"),
