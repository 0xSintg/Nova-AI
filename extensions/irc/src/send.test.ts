import { verifyChannelMessageAdapterCapabilityProofs } from "nova-ai/plugin-sdk/channel-message";
import { createSendCfgThreadingRuntime } from "nova-ai/plugin-sdk/channel-test-helpers";
...
vi.mock("nova-ai/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("nova-ai/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("nova-ai/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("nova-ai/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("nova-ai/plugin-sdk/plugin-config-runtime");
  vi.doUnmock("nova-ai/plugin-sdk/text-chunking");
  vi.resetModules();
});
