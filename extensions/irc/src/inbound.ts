...
import { resolveInboundRouteEnvelopeBuilderWithRuntime } from "nova-ai/plugin-sdk/inbound-envelope";
import {
...
export async function handleIrcInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as Nova AIConfig,
    surface: CHANNEL_ID,
  });
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as Nova AIConfig);
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as Nova AIConfig);
