...
import type { Nova AIPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/nova-ai-diagnostics-otel-test";
...
const ORIGINAL_NOVA_AI_OTEL_PRELOADED = process.env.NOVA_AI_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<Nova AIPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): Nova AIPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.NOVA_AI_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_NOVA_AI_OTEL_PRELOADED === undefined) {
      delete process.env.NOVA_AI_OTEL_PRELOADED;
    } else {
      process.env.NOVA_AI_OTEL_PRELOADED = ORIGINAL_NOVA_AI_OTEL_PRELOADED;
    }
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("nova-ai.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.channel": "telegram",
      "nova-ai.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("nova-ai.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "nova-ai.channel": "telegram",
      "nova-ai.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("nova-ai.message.queued")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.channel": "telegram",
      "nova-ai.source": "telegram",
    });
    expect(telemetryState.histograms.get("nova-ai.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("nova-ai.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "nova-ai.channel": "telegram",
      "nova-ai.source": "telegram",
    });
    expect(telemetryState.histograms.get("nova-ai.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "nova-ai.lane": "main",
    });
    expect(telemetryState.counters.get("nova-ai.message.processed")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.channel": "telegram",
      "nova-ai.outcome": "completed",
    });
    expect(
      telemetryState.histograms.get("nova-ai.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "nova-ai.channel": "telegram",
      "nova-ai.outcome": "completed",
    });
    expect(telemetryState.histograms.get("nova-ai.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "nova-ai.lane": "main",
      },
    );
    expect(telemetryState.counters.get("nova-ai.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("nova-ai.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.state": "processing",
    });
    expect(
      telemetryState.histograms.get("nova-ai.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "nova-ai.state": "processing",
    });
    expect(telemetryState.counters.get("nova-ai.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.attempt": 2,
    });

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("nova-ai.webhook.processed");
    expect(spanNames).toContain("nova-ai.message.processed");
    expect(spanNames).toContain("nova-ai.session.stuck");
    const webhookSpanOptions = startedSpanOptions("nova-ai.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("nova-ai.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("nova-ai.message.processed");
    expect(messageSpanOptions?.attributes?.["nova-ai.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["nova-ai.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["nova-ai.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("nova-ai.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("nova-ai.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.NOVA_AI_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("nova-ai.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["nova-ai.provider"]).toBe("openai");
    expect(runDurationAttributes?.["nova-ai.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("nova-ai.run");
    expect(runSpanOptions?.attributes?.["nova-ai.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("nova-ai.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "nova-ai.exporter": "diagnostics-otel",
      "nova-ai.signal": "logs",
      "nova-ai.status": "started",
      "nova-ai.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("nova-ai.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("nova-ai.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "nova-ai.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("nova-ai.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "nova-ai.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("nova-ai.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["nova-ai.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["nova-ai.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["nova-ai.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "nova-ai.liveness.warning");
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "nova-ai.exporter": "diagnostics-otel",
      "nova-ai.signal": "logs",
      "nova-ai.status": "failure",
      "nova-ai.reason": "emit_failed",
      "nova-ai.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("nova-ai.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("nova-ai.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["nova-ai.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["nova-ai.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("nova-ai.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["nova-ai.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["nova-ai.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "nova-ai.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "nova-ai.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "nova-ai.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/nova-ai/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/nova-ai/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["nova-ai.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["nova-ai.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `nova-ai.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "nova-ai.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "nova-ai.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "nova-ai.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "nova-ai.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "nova-ai.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "nova-ai.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("nova-ai.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "nova-ai.channel": "webchat",
      "nova-ai.agent": "ops",
      "nova-ai.provider": "openai",
      "nova-ai.model": "gpt-5.4",
      "nova-ai.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("nova-ai.tokens")?.add).toHaveBeenCalledWith(2, {
      "nova-ai.channel": "unknown",
      "nova-ai.agent": "unknown",
      "nova-ai.provider": "openai",
      "nova-ai.model": "gpt-5.4",
      "nova-ai.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("nova-ai.tokens")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("nova-ai.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "nova-ai.sessionId")).toBe(false);
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("nova-ai.run");
    expect(spanNames).toContain("nova-ai.model.call");
    expect(spanNames).toContain("nova-ai.harness.run");
    expect(spanNames).toContain("nova-ai.tool.execution");

    const runOptions = startedSpanOptions("nova-ai.run");
    expect(runOptions?.attributes?.["nova-ai.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["nova-ai.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["nova-ai.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["nova-ai.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "nova-ai.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "nova-ai.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("nova-ai.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "nova-ai.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "nova-ai.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("nova-ai.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["nova-ai.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["nova-ai.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["nova-ai.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["nova-ai.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["nova-ai.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["nova-ai.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["nova-ai.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["nova-ai.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["nova-ai.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["nova-ai.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["nova-ai.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "nova-ai.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "nova-ai.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "nova-ai.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("nova-ai.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["nova-ai.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["nova-ai.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["nova-ai.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["nova-ai.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "nova-ai.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "nova-ai.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("nova-ai.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["nova-ai.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["nova-ai.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("nova-ai.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["nova-ai.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["nova-ai.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("nova-ai.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["nova-ai.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["nova-ai.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("nova-ai.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["nova-ai.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["nova-ai.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("nova-ai.model.call");
    expect(modelSpanAttributes["nova-ai.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["nova-ai.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["nova-ai.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("nova-ai.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "nova-ai.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("nova-ai.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["nova-ai.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["nova-ai.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["nova-ai.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "nova-ai.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "nova-ai.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("nova-ai.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "nova-ai.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "nova-ai.runId")).toBe(false);

    const toolSpan = spanByName("nova-ai.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("nova-ai.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("nova-ai.model.failover");
    expect(failoverOptions?.attributes?.["nova-ai.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["nova-ai.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["nova-ai.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["nova-ai.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["nova-ai.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["nova-ai.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["nova-ai.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["nova-ai.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "nova-ai.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("nova-ai.model.failover")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "nova-ai.model.call")
  ...
  test("uses latest GenAI provider attribute only when semconv opt-in is set", async () => {
    ...
    const modelCallOptions = startedSpanOptions("nova-ai.model.call");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("nova-ai.model.call");
    expect(modelCallOptions?.attributes?.["nova-ai.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "nova-ai.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "nova-ai.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("nova-ai.provider.request", {
      "nova-ai.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("nova-ai.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["nova-ai.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "nova-ai.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("nova-ai.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "nova-ai.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["nova-ai.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["nova-ai.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["nova-ai.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["nova-ai.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["nova-ai.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["nova-ai.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["nova-ai.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["nova-ai.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["nova-ai.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["nova-ai.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["nova-ai.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["nova-ai.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["nova-ai.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("nova-ai.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.toolName": "process",
      "nova-ai.loop.level": "critical",
      "nova-ai.loop.action": "block",
      "nova-ai.loop.detector": "known_poll_no_progress",
      "nova-ai.loop.count": 20,
      "nova-ai.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("nova-ai.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["nova-ai.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["nova-ai.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["nova-ai.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["nova-ai.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["nova-ai.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["nova-ai.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "nova-ai.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("nova-ai.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("nova-ai.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "nova-ai.memory.level": "critical",
        "nova-ai.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("nova-ai.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.memory.level": "critical",
      "nova-ai.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("nova-ai.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["nova-ai.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["nova-ai.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["nova-ai.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["nova-ai.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["nova-ai.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["nova-ai.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["nova-ai.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["nova-ai.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["nova-ai.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["nova-ai.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "nova-ai.memory.pressure",
    );
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "nova-ai.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "nova-ai.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "nova-ai.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["nova-ai.run"]).toBeUndefined();
    expect(parentBySpanName["nova-ai.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["nova-ai.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("keeps trusted run spans alive long enough for post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("nova-ai.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["nova-ai.run"]).toBeUndefined();
    expect(parentBySpanName["nova-ai.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["nova-ai.run"]).toBeUndefined();
    expect(parentBySpanName["nova-ai.model.call"]).toBeUndefined();
    expect(parentBySpanName["nova-ai.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "nova-ai.run"),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "nova-ai.model.call",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "nova-ai.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "nova-ai.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("nova-ai.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["nova-ai.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["nova-ai.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["nova-ai.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["nova-ai.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("nova-ai.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["nova-ai.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["nova-ai.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["nova-ai.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["nova-ai.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["nova-ai.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["nova-ai.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["nova-ai.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "nova-ai.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "nova-ai.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("nova-ai.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("nova-ai.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("nova-ai.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "nova-ai.channel": "matrix",
      "nova-ai.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "nova-ai.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["nova-ai.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["nova-ai.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["nova-ai.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["nova-ai.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["nova-ai.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["nova-ai.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["nova-ai.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "nova-ai.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["nova-ai.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["nova-ai.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["nova-ai.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["nova-ai.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["nova-ai.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["nova-ai.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["nova-ai.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["nova-ai.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "nova-ai.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "nova-ai.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "nova-ai.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "nova-ai.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "nova-ai.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "nova-ai.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "nova-ai.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("nova-ai.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["nova-ai.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["nova-ai.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["nova-ai.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("nova-ai.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["nova-ai.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["nova-ai.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["nova-ai.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["nova-ai.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("nova-ai.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["nova-ai.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["nova-ai.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["nova-ai.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("nova-ai.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["nova-ai.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["nova-ai.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["nova-ai.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("nova-ai.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["nova-ai.status"]).toBe("released");
    expect(telemetryState.counters.get("nova-ai.talk.event")?.add).toHaveBeenCalledWith(1, {
      "nova-ai.talk.brain": "agent-consult",
      "nova-ai.talk.event_type": "input.audio.delta",
      "nova-ai.talk.mode": "realtime",
      "nova-ai.talk.provider": "openai",
      "nova-ai.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("nova-ai.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "nova-ai.talk.brain": "agent-consult",
        "nova-ai.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("nova-ai.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "nova-ai.talk.brain": "agent-consult",
      "nova-ai.talk.event_type": "latency.metrics",
      "nova-ai.talk.mode": "realtime",
      "nova-ai.talk.provider": "openai",
      "nova-ai.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("nova-ai.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("nova-ai.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "nova-ai.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "nova-ai.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "nova-ai.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("nova-ai.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "nova-ai.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "nova-ai.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["nova-ai.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["nova-ai.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["nova-ai.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["nova-ai.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["nova-ai.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["nova-ai.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("nova-ai.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["nova-ai.reason"])).toContain("…");
    expect(typeof attrs?.["nova-ai.reason"]).toBe("string");
    expect(String(attrs?.["nova-ai.reason"])).not.toContain(
