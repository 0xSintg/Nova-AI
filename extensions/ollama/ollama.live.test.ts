...
const LIVE = process.env.NOVA_AI_LIVE_TEST === "1" && process.env.NOVA_AI_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.NOVA_AI_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.NOVA_AI_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.NOVA_AI_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.NOVA_AI_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.NOVA_AI_LIVE_OLLAMA_WEB_SEARCH !== "0";
...
async function withTempNova AIState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "nova-ai.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runNova AICli(args: string[], env: NodeJS.ProcessEnv) {
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "nova-ai-ollama-cli-output-"));
  ...
  try {
    const result = spawnSync(process.execPath, ["nova-ai.mjs", ...args], {
      cwd: process.cwd(),
      env,
      timeout: 90_000,
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  return {
    ...
    NOVA_AI_LIVE_TEST: "1",
    NOVA_AI_LIVE_OLLAMA: "1",
    NOVA_AI_LIVE_OLLAMA_WEB_SEARCH: "0",
    NOVA_AI_STATE_DIR: path.join(root, "state"),
    NOVA_AI_CONFIG_PATH: path.join(root, "nova-ai.json"),
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without PI model discovery", async () => {
    await withTempNova AIState(async ({ root }) => {
      const result = await runNova AICli(
        [
          "infer",
          "model",
          "run",
