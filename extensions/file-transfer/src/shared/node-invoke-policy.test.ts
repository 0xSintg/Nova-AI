...
import type { Nova AIPluginNodeInvokePolicyContext } from "nova-ai/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: Nova AIPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<Nova AIPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<Nova AIPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<Nova AIPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
