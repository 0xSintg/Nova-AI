...
}): Promise<Nova AIPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: Nova AIPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: Nova AIPluginNodeInvokePolicyContext,
): Promise<Nova AIPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): Nova AIPluginNodeInvokePolicy {
  return {
