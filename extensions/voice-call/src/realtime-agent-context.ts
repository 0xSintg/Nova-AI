...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "nova-ai/plugin-sdk/realtime-voice";
import { root } from "nova-ai/plugin-sdk/security-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "Nova AI agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the Nova AI agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call nova-ai_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as Nova AIConfig,
      agentId,
