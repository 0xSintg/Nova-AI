...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Nova AI agent receiving delegated requests from a live phone voice bridge.",
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as Nova AIConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as Nova AIConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
