...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Nova AI's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Nova AI's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated Nova AI-managed browser (`nova-ai`).",
...
export const browserPluginNodeHostCommands: Nova AIPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: Nova AIPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): Nova AIPluginService {
  let service: Nova AIPluginService | null = null;
...
export function registerBrowserPlugin(api: Nova AIPluginApi) {
  api.registerTool(((ctx: Nova AIPluginToolContext) =>
    createLazyBrowserTool({
      sandboxBridgeUrl: ctx.browser?.sandboxBridgeUrl,
      allowHostControl: ctx.browser?.allowHostControl,
      agentSessionKey: ctx.sessionKey,
    })) as Nova AIPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
