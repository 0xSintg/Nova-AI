...
import { normalizeOptionalString } from "nova-ai/plugin-sdk/string-coerce-runtime";
import {
...
import { DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
const LEGACY_NOVA_BROWSER_PROFILE_NAME = "nova";
...
function collectChromeMcpProfiles(cfg: Nova AIConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: Nova AIConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "nova-ai";
    if (driver !== "existing-session") {
...
function isLegacyNovaProfileConfigured(cfg: Nova AIConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
  if (normalizeOptionalString(browser.defaultProfile) === LEGACY_NOVA_BROWSER_PROFILE_NAME) {
    return true;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(configuredProfiles, LEGACY_NOVA_BROWSER_PROFILE_NAME)) {
    return true;
  }
...
export function detectLegacyNovaBrowserProfileResidue(
  cfg: Nova AIConfig,
  ...
  const legacyProfileDir = resolveManagedBrowserProfileDir(
    configDir,
    LEGACY_NOVA_BROWSER_PROFILE_NAME,
  );
  ...
  if (isLegacyNovaProfileConfigured(cfg, legacyProfileDir)) {
    return null;
  }
  ...
  if (
    resolved.defaultProfile !== DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME ||
  ...
  return {
    ...
    canonicalUserDataDir: resolveManagedBrowserUserDataDir(
      configDir,
      DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME,
    ),
  };
}

function formatLegacyNovaBrowserProfileResidueNote(
  residue: LegacyNovaBrowserProfileResidue,
): string {
  return [
    `- Legacy managed browser profile residue was found at ${residue.legacyProfileDir}.`,
    `- The canonical Nova AI-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("nova-ai doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  const resolved = resolveBrowserConfig(cfg.browser, cfg);
  const legacyNovaResidue = detectLegacyNovaBrowserProfileResidue(cfg, {
    configDir: deps?.configDir,
    pathExists: deps?.pathExists,
  });
  if (legacyNovaResidue) {
    noteFn(formatLegacyNovaBrowserProfileResidueNote(legacyNovaResidue), "Browser");
  }
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Nova AI-managed browser profile(s) are configured: ${managedProfileLabel}.`,
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Nova AI-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Nova AI connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Nova AI does not bundle Chrome.`,
...
export async function maybeArchiveLegacyNovaBrowserProfileResidue(
  ...
  const residue = detectLegacyNovaBrowserProfileResidue(cfg, deps);
  if (!residue) {
  ...
  try {
    ...
    return {
      changes: [],
      warnings: [`Legacy nova browser profile residue could not be archived: ${message}`],
    };
  }
}
