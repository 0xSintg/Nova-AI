import type { Nova AIPluginSecurityAuditContext } from "nova-ai/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "nova-ai/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: Nova AIPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("nova-ai security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.NOVA_AI_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.NOVA_AI_GATEWAY_PASSWORD) ||
