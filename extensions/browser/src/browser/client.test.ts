...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("nova-ai");
    ...
    expect(deepDoctorResult.profile).toBe("nova-ai");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=nova-ai&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=nova-ai&deep=true"));
