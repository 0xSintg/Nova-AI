...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredNova AITmpDir: vi.fn(() => "/tmp/nova-ai"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-nova-ai-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-nova-ai-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredNova AITmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredNova AITmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredNova AITmpDir.mockReturnValue("/tmp/nova-ai");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredNova AITmpDir.mockReturnValue("/tmp/nova-ai-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "nova-ai-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "nova-ai-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredNova AITmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "nova-ai-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "nova-ai-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredNova AITmpDir.mockReturnValue(tempDir);
