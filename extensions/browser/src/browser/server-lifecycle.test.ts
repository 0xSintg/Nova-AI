...
const { stopNova AIChromeMock } = vi.hoisted(() => ({
  stopNova AIChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopNova AIChrome: stopNova AIChromeMock,
}));
...
beforeEach(() => {
  ...
  stopNova AIChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["nova-ai", "user"]);
    ...
    expect(stopMap.nova-ai).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopNova AIChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("nova-ai browser stop failed: Error: oops");
  });
});
