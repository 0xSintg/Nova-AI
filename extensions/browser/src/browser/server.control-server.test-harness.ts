...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "nova-ai" | "existing-session";
      attachOnly?: boolean;
    }
  >;
  tabUrl: string;
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.NOVA_AI_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "nova-ai",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/nova-ai" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    nova-ai: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/nova-ai.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/nova-ai.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/nova-ai.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchNova AIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveNova AIUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopNova AIChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "nova-ai";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.NOVA_AI_GATEWAY_PORT;
  process.env.NOVA_AI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.NOVA_AI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.NOVA_AI_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.NOVA_AI_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.NOVA_AI_GATEWAY_PASSWORD;
  } else {
    process.env.NOVA_AI_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
