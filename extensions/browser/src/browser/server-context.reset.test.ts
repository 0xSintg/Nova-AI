...
function localNova AIProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "nova-ai",
...
function createLocalNova AIResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localNova AIProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveNova AIUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-reset-"));
    const profileDir = path.join(tempRoot, "nova-ai");
    ...
    const ops = createLocalNova AIResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveNova AIUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nova-ai-reset-no-own-"));
    const profileDir = path.join(tempRoot, "nova-ai");
    fs.mkdirSync(profileDir, { recursive: true });
