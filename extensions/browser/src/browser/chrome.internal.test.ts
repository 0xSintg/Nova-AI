...
vi.mock("../infra/tmp-nova-ai-dir.js", () => ({
  resolvePreferredNova AITmpDir: () => "/tmp/nova-ai-browser-test",
}));
...
import {
  buildNova AIChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchNova AIChrome,
  resolveNova AIUserDataDir,
  stopNova AIChrome,
} from "./chrome.js";
 ...
 * not exercise: launchNova AIChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopNova AIChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildNova AIChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveNova AIUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveNova AIUserDataDir();
      expect(dir.endsWith(path.join("nova-ai", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveNova AIUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildNova AIChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildNova AIChromeLaunchArgs({
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildNova AIChromeLaunchArgs({
        ...
        env: { NOVA_AI_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchNova AIChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchNova AIChrome(resolved, profile);
  ...
  describe("launchNova AIChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "nova-ai-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchNova AIChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchNova AIChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchNova AIChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "nova-ai.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      vi.stubEnv("NOVA_AI_CONFIG_PATH", configPath);
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveNova AIUserDataDir(profile.name);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchNova AIChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopNova AIChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopNova AIChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopNova AIChrome>[0],
        1,
      );
  ...
  describe("launchNova AIChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "nova-ai-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveNova AIUserDataDir(profileName));
        ...
        const staged = resolveNova AIUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_NOVA_AI_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "nova-ai-redact-off-"));
      ...
      let message = "";
      try {
        await launchNova AIChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateNova AIProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateNova AIProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateNova AIProfile }, "decorateNova AIProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".nova-ai-profile-decorated") || s.endsWith("Preferences")) {
