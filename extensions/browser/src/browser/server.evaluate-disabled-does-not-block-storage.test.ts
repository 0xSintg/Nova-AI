...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        nova-ai: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.NOVA_AI_GATEWAY_PORT;
    process.env.NOVA_AI_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.NOVA_AI_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.NOVA_AI_GATEWAY_PASSWORD;
