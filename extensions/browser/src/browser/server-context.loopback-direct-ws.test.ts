...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const nova-ai = ctx.forProfile("nova-ai");

    const opened = await nova-ai.openTab("about:blank");
  ...
  it("uses an HTTP /json base for focus and close under strict SSRF", async () => {
    ...
    await nova-ai.focusTab("T1");
    await nova-ai.closeTab("T1");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await nova-ai.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await nova-ai.focusTab("T2");
    await nova-ai.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(nova-ai.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(nova-ai.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(nova-ai.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
