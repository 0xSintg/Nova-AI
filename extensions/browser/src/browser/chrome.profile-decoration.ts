...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".nova-ai-profile-decorated");
}
 ...
 */
export function decorateNova AIProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_NOVA_AI_BROWSER_COLOR).toUpperCase();
