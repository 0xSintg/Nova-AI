...
export const NOVA_AI_BROWSER_HEADLESS_ENV = "NOVA_AI_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_NOVA_AI_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  cfg: BrowserConfig | undefined,
  rootConfig?: Nova AIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NOVA_AI_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME]
        ? DEFAULT_NOVA_AI_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "nova-ai";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[NOVA_AI_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  const sourceHint =
    mode.source === "request"
      ? "request override"
      : mode.source === "env"
        ? `${NOVA_AI_BROWSER_HEADLESS_ENV}=0`
        : mode.source === "profile"
  ...
  return (
    ...
    `Set ${NOVA_AI_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
