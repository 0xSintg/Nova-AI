...
function setupEnsureBrowserAvailableHarness() {
  ...
  const launchNova AIChrome = vi.mocked(chromeModule.launchNova AIChrome);
  const stopNova AIChrome = vi.mocked(chromeModule.stopNova AIChrome);
  ...
  const profile = ctx.forProfile("nova-ai");

  return { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, profile, state };
}
...
function requireFirstLaunchOptions(launchNova AIChrome: {
  mock: { calls: unknown[][] };
}): unknown {
  const [call] = launchNova AIChrome.mock.calls;
  if (!call) {
...
describe("browser server-context ensureBrowserAvailable", () => {
  it("waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValueOnce(false).mockResolvedValue(true);
    mockLaunchedChrome(launchNova AIChrome, 123);

    const promise = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(promise).resolves.toBeUndefined();

    expect(launchNova AIChrome).toHaveBeenCalledTimes(1);
    expect(isChromeCdpReady).toHaveBeenCalled();
    expect(stopNova AIChrome).not.toHaveBeenCalled();
  });
  ...
  it("stops launched chrome when CDP readiness never arrives", async () => {
    ...
    mockLaunchedChrome(launchNova AIChrome, 321);
    ...
    expect(launchNova AIChrome).toHaveBeenCalledTimes(1);
    expect(stopNova AIChrome).toHaveBeenCalledTimes(1);
  });

  it("uses configured local CDP readiness timeout after launching", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, profile, state } =
      setupEnsureBrowserAvailableHarness();
    state.resolved.localCdpReadyTimeoutMs = 250;
    isChromeCdpReady.mockResolvedValue(false);
    mockLaunchedChrome(launchNova AIChrome, 322);
  ...
  it("deduplicates concurrent lazy-start calls to prevent PortInUseError", async () => {
    ...
    mockLaunchedChrome(launchNova AIChrome, 456);
  ...
  it("deduplicates concurrent lazy-start calls across fresh profile contexts", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchNova AIChrome, 457);

    const firstCtx = createBrowserRouteContext({ getState: () => state });
    const secondCtx = createBrowserRouteContext({ getState: () => state });
    const first = firstCtx.forProfile("nova-ai").ensureBrowserAvailable();
    const second = secondCtx.forProfile("nova-ai").ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(Promise.all([first, second])).resolves.toEqual([undefined, undefined]);
  ...
  it("passes request-local headless override to initial launch", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchNova AIChrome, 654);
    ...
    expect(launchNova AIChrome).toHaveBeenCalledTimes(1);
    expect(requireFirstLaunchOptions(launchNova AIChrome)).toEqual({ headlessOverride: true });
    expect(stopNova AIChrome).not.toHaveBeenCalled();
  });
  ...
  it("passes request-local headless override to the owned restart path", async () => {
    ...
    state.profiles.set("nova-ai", {
      profile: profile.profile,
      running: {
        pid: 111,
        exe: { kind: "chromium", path: "/usr/bin/chromium" },
        userDataDir: "/tmp/nova-ai-test",
        cdpPort: 18800,
        startedAt: Date.now(),
        proc: existingProc,
      },
    ...
    mockLaunchedChrome(launchNova AIChrome, 987);
  ...
  it("does not share inflight lazy-start promises across different headless overrides", async () => {
    const { launchNova AIChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("clears the concurrent lazy-start guard after launch failure", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchNova AIChrome.mockRejectedValueOnce(
    ...
    mockLaunchedChrome(launchNova AIChrome, 789);
    const retry = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(retry).resolves.toBeUndefined();

    expect(launchNova AIChrome).toHaveBeenCalledTimes(2);
    expect(stopNova AIChrome).not.toHaveBeenCalled();
  });
  ...
  it("cools down repeated managed Chrome launch failures across route contexts", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchNova AIChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));

    for (let attempt = 0; attempt < 3; attempt += 1) {
      const ctx = createBrowserRouteContext({ getState: () => state });
      await expect(ctx.forProfile("nova-ai").ensureBrowserAvailable()).rejects.toThrow(
        "Failed to start Chrome CDP",
      );
    }

    const cooledDownCtx = createBrowserRouteContext({ getState: () => state });
    await expect(cooledDownCtx.forProfile("nova-ai").ensureBrowserAvailable()).rejects.toThrow(
      'Browser launch for profile "nova-ai" is cooling down after 3 consecutive managed Chrome launch failures.',
    );
    await expect(cooledDownCtx.forProfile("nova-ai").ensureBrowserAvailable()).rejects.toThrow(
      "set browser.enabled=false if the browser tool is not needed",
    );

    expect(launchNova AIChrome).toHaveBeenCalledTimes(3);
    expect(stopNova AIChrome).not.toHaveBeenCalled();
  });

  it("allows one managed Chrome launch attempt after the cooldown expires", async () => {
    const { launchNova AIChrome, isChromeCdpReady, state } = setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchNova AIChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(retryCtx.forProfile("nova-ai").ensureBrowserAvailable()).rejects.toThrow(
      "Failed to start Chrome CDP",
    );

    expect(launchNova AIChrome).toHaveBeenCalledTimes(4);
  });
  ...
  it("reuses a pre-existing loopback browser after an initial short probe miss", async () => {
    ...
    expect(launchNova AIChrome).not.toHaveBeenCalled();
    expect(stopNova AIChrome).not.toHaveBeenCalled();
  });
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      'Port 18800 is in use for profile "nova-ai" but not by nova-ai.',
    );
    await expect(promise).rejects.toThrow(
      "set browser.profiles.nova-ai.attachOnly=true so Nova AI attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint Nova AI can reach via browser.profiles.<name>.cdpUrl.",
    );
  ...
  it("retries remote CDP websocket reachability once before failing", async () => {
    const { launchNova AIChrome, stopNova AIChrome, isChromeCdpReady } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);

    const state = makeBrowserServerState();
    state.resolved.profiles.nova-ai = {
  ...
  it("treats attachOnly loopback CDP as local control with remote-class probe timeouts", async () => {
    const { launchNova AIChrome, stopNova AIChrome } = setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
    const isChromeCdpReady = vi.mocked(chromeModule.isChromeCdpReady);
