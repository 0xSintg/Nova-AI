...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "nova-ai" }),
    });
    ...
    const createNova = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "legacynova", driver: "nova" }),
    });
    expect(createNova.status).toBe(200);
    const createNovaBody = (await createNova.json()) as {
      profile?: string;
      transport?: string;
      cdpPort?: number | null;
      userDataDir?: string | null;
    };
    expect(createNovaBody.profile).toBe("legacynova");
    expect(createNovaBody.transport).toBe("cdp");
    expect(createNovaBody.cdpPort).toBeTypeOf("number");
    expect(createNovaBody.userDataDir).toBeNull();

    const explicitUserDataDir = "/tmp/nova-ai-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/nova-ai`, {
      method: "DELETE",
    });
