...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-nova-ai-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-nova-ai-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-nova-ai-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveDispatcherBrowserControlOwnership(url: string): BrowserControlOwnership {
  ...
  try {
    ...
    return profile.driver === "nova-ai" && profile.cdpIsLoopback && !profile.attachOnly
      ? "local-managed"
      : "external-browser";
  } catch {
    return "unknown";
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to Nova AI; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the Nova AI gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the Nova AI gateway (Nova AI.app menubar, or \`${formatCliCommand("nova-ai gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the Nova AI browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
  ...
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Nova AI browser control service. ${operatorHint} (${msg})`,
    ),
    err instanceof Error ? { cause: err } : undefined,
  );
}
