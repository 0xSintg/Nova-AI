...
import { resolveNova AIUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => Nova AIConfig>(),
  writeConfigFile: vi.fn<(cfg: Nova AIConfig) => Promise<void>>(async (_cfg) => {}),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: Nova AIConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: Nova AIConfig }) => {
...
vi.mock("./chrome.js", () => ({
  resolveNova AIUserDataDir: vi.fn(() => "/tmp/nova-ai-test/nova-ai/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "nova-ai-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveNova AIUserDataDir).mockReturnValue(userDataDir);
