...
import type { ResolvedTtsConfig } from "nova-ai/plugin-sdk/agent-runtime";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
import { encodePngRgba, fillPixel } from "nova-ai/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "nova-ai/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "nova-ai/plugin-sdk/provider-test-contracts";
import { getRuntimeConfig } from "nova-ai/plugin-sdk/runtime-config-snapshot";
import {
  isOverloadedErrorMessage,
  isServerErrorMessage,
  isTimeoutErrorMessage,
} from "nova-ai/plugin-sdk/test-env";
import { describe, expect, it } from "vitest";
import plugin from "./index.js";

const OPENAI_API_KEY = process.env.OPENAI_API_KEY ?? "";
const LIVE_MODEL_ID = process.env.NOVA_AI_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.NOVA_AI_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.NOVA_AI_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.NOVA_AI_LIVE_TEST === "1";
...
function createLiveConfig(): Nova AIConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "Nova AI integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing Nova AI OpenAI realtime transcription integration test OK.";
