...
import type { ProviderAuthContext } from "nova-ai/plugin-sdk/plugin-entry";
import { ensureGlobalUndiciEnvProxyDispatcher } from "nova-ai/plugin-sdk/runtime-env";
import { formatCliCommand } from "nova-ai/plugin-sdk/setup-tools";

const manualInputPromptMessage = "Paste the authorization code (or full redirect URL):";
const openAICodexOAuthOriginator = "nova-ai";
const localManualFallbackDelayMs = 15_000;
const localManualFallbackGraceMs = 1_000;
const openAIAuthProbeUrl =
  "https://auth.openai.com/oauth/authorize?response_type=code&client_id=nova-ai-preflight&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback&scope=openid+profile+email";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Nova AI process and then retry `nova-ai models auth login --provider openai-codex`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this Nova AI process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://docs.nova-ai.com/start/faq", "OAuth help");
