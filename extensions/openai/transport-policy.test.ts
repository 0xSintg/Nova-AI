import type { ProviderRuntimeModel } from "nova-ai/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-nova-ai-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-nova-ai-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-nova-ai-turn-attempt"]).toBe("2");
    expect(state?.metadata?.nova-ai_session_id).toBe("session-123");
    expect(state?.metadata?.nova-ai_turn_id).toBe("turn-123");
    expect(state?.metadata?.nova-ai_turn_attempt).toBe("2");
    expect(state?.metadata?.nova-ai_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-nova-ai-session-id"]).toBe("session-123");
