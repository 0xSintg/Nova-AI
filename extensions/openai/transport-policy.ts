...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-nova-ai-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-nova-ai-turn-id": turnId,
      "x-nova-ai-turn-attempt": attempt,
    },
    metadata: {
      nova-ai_session_id: sessionHeaders["x-nova-ai-session-id"] ?? "",
      nova-ai_turn_id: turnId,
      nova-ai_turn_attempt: attempt,
      nova-ai_transport: ctx.transport,
    },
  };
}
