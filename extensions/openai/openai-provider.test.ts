...
vi.mock("nova-ai/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("nova-ai/plugin-sdk/provider-stream-family")>();
...
describe("buildOpenAIProvider", () => {
  ...
  it("owns Codex wrapper composition for responses payloads", () => {
    ...
    expect(String(headers?.["User-Agent"]).startsWith("nova-ai/")).toBe(true);
