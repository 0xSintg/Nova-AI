...
import { fetchCodexUsage } from "nova-ai/plugin-sdk/provider-usage";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.nova-ai.com/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `nova-ai models auth login --provider openai-codex` or `nova-ai configure`.";
}
