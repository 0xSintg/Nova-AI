...
vi.mock("nova-ai/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/routing")>(
    "nova-ai/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/realtime-voice")>(
    "nova-ai/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same Nova AI agent");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("nova-ai_agent_consult");
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("Call nova-ai_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("nova-ai_agent_consult");
