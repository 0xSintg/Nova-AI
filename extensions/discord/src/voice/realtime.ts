...
import type { DiscordAccountConfig, Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
import {
...
import { createSubsystemLogger } from "nova-ai/plugin-sdk/runtime-env";
import { formatErrorMessage } from "nova-ai/plugin-sdk/ssrf-runtime";
import {
...
export function buildDiscordSpeakExactUserMessage(text: string): string {
  return [
    "Internal Nova AI voice playback result.",
    "Do not call nova-ai_agent_consult or any other tool for this message.",
    "Speak this exact Nova AI answer to the Discord voice channel, without adding, removing, or rephrasing words.",
...
function extractDiscordExactSpeechConsultText(args: unknown): string | undefined {
  ...
  if (
    !message.includes("Speak this exact Nova AI answer") &&
  ...
  private async runForcedAgentProxyConsult(
    ...
    try {
      const promise = this.runAgentTurn({
        ...
        message: [
          question,
          "Context: The realtime model produced a final user transcript without calling nova-ai_agent_consult. Nova AI is forcing the consult because consultPolicy is always.",
        ].join("\n\n"),
      });
    ...
    const submitAlreadyDelivered = () => {
      session.submitToolResult(
        callId,
        {
          ...
          message: "Nova AI already delivered this answer to Discord voice.",
        },
...
function buildDiscordRealtimeInstructions(params: {
  ...
  const base =
    params.instructions ??
    [
      "You are Nova AI's Discord voice interface.",
  ...
  if (isDiscordAgentProxyVoiceMode(params.mode)) {
    return [
      base,
      "Mode: Nova AI agent proxy.",
      "You are the realtime voice surface for the same Nova AI agent the user can message directly.",
      "Do not mention a backend, supervisor, helper, or separate system. Present the result as your own work.",
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with nova-ai_agent_consult.",
      "Do not block, refuse, or downscope at the voice layer. Delegate to Nova AI and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      "When Nova AI sends an internal exact answer to speak, do not call tools. Say only that answer.",
      buildRealtimeVoiceAgentConsultPolicyInstructions({
