...
import type { RuntimeEnv } from "nova-ai/plugin-sdk/runtime-env";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: Nova AIConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): Nova AIConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("nova-ai/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/acp-runtime")>(
    "nova-ai/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/command-auth")>(
    "nova-ai/plugin-sdk/command-auth",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("nova-ai/plugin-sdk/native-command-config-runtime")
  >("nova-ai/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/runtime-env")>(
    "nova-ai/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/error-runtime")>(
    "nova-ai/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
