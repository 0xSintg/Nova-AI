...
vi.mock("nova-ai/plugin-sdk/web-media", async () => {
...
afterAll(() => {
  vi.doUnmock("nova-ai/plugin-sdk/web-media");
});
...
describe("uploadStickerDiscord", () => {
  ...
  it("uploads sticker assets", async () => {
    ...
    expect(stickerBody.name).toBe("nova-ai_wave");
    expect(stickerBody.description).toBe("Nova AI waving");
