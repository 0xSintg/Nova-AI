import { createUnionActionGate } from "nova-ai/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "nova-ai/plugin-sdk/channel-contract";
import type { DiscordActionConfig, Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: Nova AIConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: Nova AIConfig) {
