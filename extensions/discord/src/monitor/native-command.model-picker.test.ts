...
import * as commandRegistryModule from "nova-ai/plugin-sdk/command-auth";
import type { ChatCommandDefinition, CommandArgsParsing } from "nova-ai/plugin-sdk/command-auth";
import type { ModelsProviderData } from "nova-ai/plugin-sdk/command-auth";
import type { Nova AIConfig } from "nova-ai/plugin-sdk/config-contracts";
import * as globalsModule from "nova-ai/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "nova-ai/plugin-sdk/session-store-runtime";
import * as commandTextModule from "nova-ai/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "nova-ai-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "pi", label: "Nova AI Pi Default", description: "Use Pi." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
