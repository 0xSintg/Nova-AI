...
import { peekSystemEvents, resetSystemEventsForTest } from "nova-ai/plugin-sdk/test-fixtures";
...
describe("agent components", () => {
  ...
  const createCfg = (): Nova AIConfig => ({}) as Nova AIConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`nova-ai pairing approve discord ${code}`);
