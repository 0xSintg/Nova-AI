...
import type { NativeCommandSpec } from "nova-ai/plugin-sdk/command-auth";
import { resolveDirectStatusReplyForSession } from "nova-ai/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "nova-ai/plugin-sdk/reply-dispatch-runtime";
...
function createConfig(): Nova AIConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: Nova AIConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: Nova AIConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("nova-ai/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("nova-ai/plugin-sdk/plugin-runtime").executePluginCommand,
    );
