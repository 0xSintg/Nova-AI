...
vi.mock("nova-ai/plugin-sdk/channel-message", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/channel-message")>(
    "nova-ai/plugin-sdk/channel-message",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    await deliverDiscordReply({
      ...
      textLimit: 2000,
      replyToMode: "off",
      mediaLocalRoots: ["/tmp/nova-ai-media"],
      kind: "final",
    });
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/nova-ai-media"] });
  });
