...
describe("maybeCreateDiscordAutoThread autoThreadName", () => {
  it("renames created thread when generated mode is enabled", async () => {
    ...
    const cfg = { agents: { defaults: { model: "anthropic/claude-opus-4-6" } } } as Nova AIConfig;
    const result = await maybeCreateDiscordAutoThread(
      createBaseParams({
        baseText: "Need help with deploy rollout",
        combinedBody: "Need help with deploy rollout",
        channelName: "nova-ai",
        channelDescription: "Nova AI development coordination and release planning",
    ...
    expectGeneratedTitleField("channelName", "nova-ai");
    expectGeneratedTitleField(
      "channelDescription",
      "Nova AI development coordination and release planning",
    );
