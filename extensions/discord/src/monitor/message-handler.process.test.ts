...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "nova-ai/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "nova-ai/plugin-sdk/inbound-reply-dispatch";
import type { ReplyPayload } from "nova-ai/plugin-sdk/reply-dispatch-runtime";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/nova-ai-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/nova-ai-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("nova-ai/plugin-sdk/reply-runtime", () => ({
...
vi.mock("nova-ai/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("nova-ai/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/nova-ai-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/nova-ai-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/nova-ai-discord-process-test-sessions.json" },
      },
    });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/nova-ai-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
...
describe("processDiscordMessage draft streaming", () => {
  ...
  it("keeps finalized previews when later tool warning finals are delivered", async () => {
    ...
    dispatchInboundMessage.mockImplementationOnce(async (params?: DispatchInboundParams) => {
      ...
      await params?.dispatcher.sendFinalReply({
        text: "⚠️ 🛠️ `run nova-ai definitely-not-a-real-subcommand (agent)` failed",
        isError: true,
      } as never);
