...
let runtimeEnvModule: typeof import("nova-ai/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): Nova AIConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("nova-ai/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("nova-ai/plugin-sdk/runtime-env");
  ...
  it("passes Nova AI event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
