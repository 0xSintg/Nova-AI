...
import { clearSessionStoreCacheForTest } from "nova-ai/plugin-sdk/session-store-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { ChannelType, type AutocompleteInteraction } from "../internal/discord.js";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("nova-ai/plugin-sdk/conversation-binding-runtime");
...
vi.mock("nova-ai/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("nova-ai/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("nova-ai/plugin-sdk/conversation-binding-runtime")>(
        "nova-ai/plugin-sdk/conversation-binding-runtime",
      ),
  );
});
...
vi.mock("nova-ai/plugin-sdk/agent-runtime", () => ({
  ...
  resolveDefaultModelForAgent: (params: { cfg: Nova AIConfig }) => {
...
vi.mock("nova-ai/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `nova-ai-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("nova-ai/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("nova-ai/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("nova-ai/plugin-sdk/command-auth");
