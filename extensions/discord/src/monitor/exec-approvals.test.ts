...
vi.mock("nova-ai/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("nova-ai/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<Nova AIConfig["channels"]>["discord"]>["execApprovals"],
): Nova AIConfig {
  return {
    channels: {
      discord: {
