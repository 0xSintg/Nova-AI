...
import type { MemorySearchResult } from "nova-ai/plugin-sdk/memory-core-host-runtime-files";
import { formatMemoryDreamingDay } from "nova-ai/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "nova-ai/plugin-sdk/memory-host-events";
import { privateFileStore } from "nova-ai/plugin-sdk/security-runtime";
...
const PROMOTION_MARKER_PREFIX = "nova-ai-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*nova-ai-memory-promotion:/i.test(snippet) ||
...
const DREAMING_FENCE_START_RE = /<!--\s*nova-ai:dreaming:[a-z][a-z0-9-]*:start\s*-->/i;
const DREAMING_FENCE_END_RE = /<!--\s*nova-ai:dreaming:[a-z][a-z0-9-]*:end\s*-->/i;
...
async function rehydratePromotionCandidate(
  ...
  for (const sourcePath of sourcePaths) {
    ...
    // content. If rehydration lands inside an nova-ai:dreaming fence (for example
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*nova-ai-memory-promotion:([^\n]+?)\s*-->/gi);
