...
import { peekSystemEventEntries } from "nova-ai/plugin-sdk/system-event-runtime";
...
export function registerShortTermPromotionDreaming(api: Nova AIPluginApi): void {
  ...
  const resolveCurrentConfig = (): Nova AIConfig =>
    (api.runtime.config?.current?.() ?? api.config) as Nova AIConfig;
