...
import { withMockedWindowsPlatform } from "nova-ai/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("nova-ai.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("nova-ai.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("nova-ai.qmdEmbedQueueTail");
...
vi.mock("nova-ai/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("nova-ai/plugin-sdk/memory-core-host-engine-foundation")
  >("nova-ai/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("nova-ai/plugin-sdk/file-lock")>(
    "nova-ai/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { Nova AIConfig } from "nova-ai/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
