...
import type {
  MemorySearchConfig,
  Nova AIConfig,
} from "nova-ai/plugin-sdk/memory-core-host-engine-foundation";
...
const { createdWatchers, memoryLoggerWarn, watchMock } = vi.hoisted(() => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("nova-ai.test.memoryWatchFactory")] =
    result.watchMock;
  return result;
});

vi.mock("nova-ai/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("nova-ai/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
} from "nova-ai/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  afterAll(() => {
    Reflect.deleteProperty(globalThis, Symbol.for("nova-ai.test.memoryWatchFactory"));
  });
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): Nova AIConfig {
    const defaults: NonNullable<NonNullable<Nova AIConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: Nova AIConfig) {
