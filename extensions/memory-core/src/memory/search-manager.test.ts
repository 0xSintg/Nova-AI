...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "nova-ai/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("nova-ai/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): Nova AIConfig {
  return {
    agents: {
      defaults: {
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: Nova AIConfig;
  secondCfg: Nova AIConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("nova-ai.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-qmd-workspace-"));
