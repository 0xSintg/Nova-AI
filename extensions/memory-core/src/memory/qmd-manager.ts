...
import { withFileLock } from "nova-ai/plugin-sdk/file-lock";
import {
...
const QMD_UPDATE_QUEUE_KEY = Symbol.for("nova-ai.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Nova AI search semantics match direct qmd search.
  return trimmed;
}
