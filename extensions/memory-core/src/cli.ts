    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nova-ai memory status", "Show index and provider status."],
          [
            "nova-ai memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["nova-ai memory status --deep", "Probe embedding provider readiness."],
          ["nova-ai memory index --force", "Force a full reindex."],
          ['nova-ai memory search "meeting notes"', "Quick search using positional query."],
          [
            'nova-ai memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `nova-ai memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "nova-ai memory rem-backfill --path ./memory",
          ...
          ["nova-ai memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.nova-ai.com/cli/memory")}\n`,
    );
