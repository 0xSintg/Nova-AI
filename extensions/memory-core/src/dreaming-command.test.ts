...
function resolveStoredDreaming(config: Nova AIConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: Nova AIConfig = {}) {
  const registered: { command?: Nova AIPluginCommandDefinition } = {};
  let runtimeConfig: Nova AIConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: Nova AIConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: Nova AIConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: Nova AIPluginCommandDefinition) => {
