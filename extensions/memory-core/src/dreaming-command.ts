...
import { resolveMemoryDreamingConfig } from "nova-ai/plugin-sdk/memory-core-host-status";
import type { Nova AIPluginApi, PluginCommandContext } from "nova-ai/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "nova-ai/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: Nova AIConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: Nova AIConfig, enabled: boolean): Nova AIConfig {
...
function formatStatus(cfg: Nova AIConfig): string {
...
export async function handleDreamingCommand(api: Nova AIPluginApi, ctx: PluginCommandContext) {
...
export function registerDreamingCommand(api: Nova AIPluginApi): void {
  api.registerCommand({
    name: "dreaming",
