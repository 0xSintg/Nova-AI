...
import type { MemoryEmbeddingProbeResult } from "nova-ai/plugin-sdk/memory-core-host-engine-storage";
import { resolveMemoryRemDreamingConfig } from "nova-ai/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "nova-ai/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: Nova AIConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredNova AITmpDir(), "nova-ai-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: Nova AIConfig): string {
...
function resolveAgent(cfg: Nova AIConfig, agent?: string) {
...
function resolveAgentIds(cfg: Nova AIConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: nova-ai memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredNova AITmpDir(), "nova-ai-rem-backfill-"),
      );
      try {
