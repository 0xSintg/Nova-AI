...
import { createCapturedPluginRegistration } from "nova-ai/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when NOVA_AI_HOME is set", () => {
    const previous = process.env.NOVA_AI_HOME;
    process.env.NOVA_AI_HOME = path.join(path.sep, "tmp", "nova-ai-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.NOVA_AI_HOME = previous;
      }
    }
  });
