...
import { isBlockedHostnameOrIp } from "nova-ai/plugin-sdk/ssrf-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the Nova AI browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the Nova AI browser profile; configure browser.defaultProfile to choose another profile",
  });
