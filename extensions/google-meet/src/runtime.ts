...
import type { PluginRuntime, RuntimeLogger } from "nova-ai/plugin-sdk/plugin-runtime";
  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking Nova AI to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the Nova AI Google Meet microphone before asking Nova AI to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
