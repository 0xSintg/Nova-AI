import { createMigrationItem, summarizeMigrationItems } from "nova-ai/plugin-sdk/migration";
import type {
  MigrationItem,
  MigrationPlan,
...
export async function buildClaudePlan(ctx: MigrationProviderContext): Promise<MigrationPlan> {
  ...
  const warnings = [
    ...
    ...(items.some((item) => item.kind === "archive")
      ? [
          "Some Claude files are archive-only. They will be copied into the migration report for manual review, not loaded into Nova AI.",
        ]
      : []),
  ...
  return {
    ...
    items,
    warnings,
    nextSteps: ["Run nova-ai doctor after applying the migration."],
