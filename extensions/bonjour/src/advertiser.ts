...
import type { PluginLogger } from "nova-ai/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "nova-ai/plugin-sdk/runtime-env";
...
// See https://github.com/nova-ai/nova-ai/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.NOVA_AI_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Nova AI"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Nova AI";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Nova AI\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.NOVA_AI_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "nova-ai";
    ...
    const recreateAdvertiser = async (reason: string, opts?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or NOVA_AI_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
