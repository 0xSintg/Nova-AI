...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.NOVA_AI_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const started = await startAdvertiser({
      ...
      cliPath: "/opt/homebrew/bin/nova-ai",
      minimal: false,
    });

    expect(createService).toHaveBeenCalledTimes(1);
    const [gatewayCall] = createService.mock.calls as Array<[Record<string, unknown>]>;
    expect(gatewayCall?.[0]?.type).toBe("nova-ai-gw");
  ...
  it("honors truthy NOVA_AI_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.NOVA_AI_DISABLE_BONJOUR = "true";
  ...
  it("auto-disables Bonjour on Fly Machines without Docker sentinel files", async () => {
    ...
    process.env.FLY_APP_NAME = "nova-ai-clawcks-test";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.NOVA_AI_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("logs ciao handler classifications at the bonjour caller", async () => {
    ...
    expect(
      handler?.(
        new Error(
          "Can't probe for a service which is announced already. Received announcing for service Nova AI Gateway._nova-ai._tcp.local.",
        ),
      ),
    ).toBe(true);
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._nova-ai-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._nova-ai-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("defers probing recovery while a name conflict is still settling", async () => {
    ...
    await vi.advanceTimersByTimeAsync(10_000);
    listenerMap.get("name-change")?.("test-host (Nova AI) (2)");

    await vi.advanceTimersByTimeAsync(15_000);

    expect(createService).toHaveBeenCalledTimes(1);
    expectWarnContaining('name conflict resolved; newName="test-host (Nova AI) (2)"');
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("Mac (Nova AI)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
  ...
  it("falls back to nova-ai when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.NOVA_AI_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("nova-ai");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("nova-ai.local");
  ...
  it("truncates reported Kubernetes service name at the DNS label byte limit", async () => {
    ...
    expectDnsLabelByteLength(`${reportedHostname} (Nova AI)`, 64);
  ...
  it("truncates multi-byte hostname within DNS label byte limit", async () => {
    // 21 CJK characters = 63 bytes in UTF-8, adding " (Nova AI)" pushes over
  ...
  it("uses system hostname when NOVA_AI_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.NOVA_AI_MDNS_HOSTNAME;
