...
function normalizeDenyList(cfg: Nova AIPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: Nova AIPluginApi["config"]): string[] {
...
async function disarmNow(params: {
  ...
  const cfg = api.runtime.config.current() as Nova AIConfig;
...
export default definePluginEntry({
  ...
  register(api: Nova AIPluginApi) {
    ...
    const timerService: Nova AIPluginService = {
