...
The ACPX extension is a thin Nova AI wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `nova-ai/acpx`, not in this extension.
...
Use this flow when Nova AI needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `nova-ai/acpx` repo first.
2. In Nova AI, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Nova AI and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Nova AI ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Nova AI-specific glue, move it to `nova-ai/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
