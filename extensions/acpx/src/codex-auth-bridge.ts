...
import { readJsonFileWithFallback } from "nova-ai/plugin-sdk/json-store";
import {
...
import {
  NOVA_AI_ACPX_LEASE_ID_ARG,
  NOVA_AI_ACPX_LEASE_ID_ENV,
  NOVA_AI_GATEWAY_INSTANCE_ID_ARG,
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--nova-ai-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @nova-ai/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Nova AI's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const openClawWrapperArgs = new Set([
  ${quoteCommandPart(NOVA_AI_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(NOVA_AI_GATEWAY_INSTANCE_ID_ARG)},
]);

function readNova AIWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(NOVA_AI_ACPX_LEASE_ID_ENV)}] ||
    readNova AIWrapperArg(args, ${quoteCommandPart(NOVA_AI_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripNova AIWrapperArgs(args) {
  const stripped = [];
...
const configuredArgs = stripNova AIWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[nova-ai] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[nova-ai] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Nova AI; fallback must not float to an unpatched newer release.
