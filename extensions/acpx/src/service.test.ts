...
const { cleanupNova AIOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupNova AIOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleNova AIOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleNova AIOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupNova AIOwnedAcpxProcessTree: cleanupNova AIOwnedAcpxProcessTreeMock,
  reapStaleNova AIOwnedAcpxOrphans: reapStaleNova AIOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { Nova AIPluginServiceContext } from "../runtime-api.js";
import { createAcpxRuntimeService } from "./service.js";

const tempDirs: string[] = [];
const previousEnv = {
  NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE: process.env.NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE,
  NOVA_AI_SKIP_ACPX_RUNTIME: process.env.NOVA_AI_SKIP_ACPX_RUNTIME,
  NOVA_AI_SKIP_ACPX_RUNTIME_PROBE: process.env.NOVA_AI_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupNova AIOwnedAcpxProcessTreeMock.mockClear();
  reapStaleNova AIOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("NOVA_AI_SKIP_ACPX_RUNTIME");
  restoreEnv("NOVA_AI_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): Nova AIPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".nova-ai-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.NOVA_AI_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupNova AIOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupNova AIOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Nova AI-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    reapStaleNova AIOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupNova AIOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleNova AIOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Nova AI-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.NOVA_AI_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.NOVA_AI_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (NOVA_AI_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
