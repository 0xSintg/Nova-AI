...
import { redactSensitiveText } from "nova-ai/plugin-sdk/security-runtime";
...
import {
  cleanupNova AIOwnedAcpxProcessTree,
  isNova AILeaseAwareAcpxProcessCommand,
...
type Nova AIAcpxRuntimeOptions = AcpRuntimeOptions & {
  nova-aiWrapperRoot?: string;
  nova-aiGatewayInstanceId?: string;
  nova-aiProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  nova-aiProcessCleanup?: AcpxProcessCleanupDeps;
};
type Nova AIRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];

type ResetAwareSessionStore = AcpSessionStore & {
  markFresh: (sessionKey: string) => void;
};

function withNova AIManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // Nova AI owns ACP turn deadlines. acpx treats timeout after partial agent
...
function readNova AILeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { nova-aiLeaseId } = record as { nova-aiLeaseId?: unknown };
  return typeof nova-aiLeaseId === "string" ? nova-aiLeaseId.trim() || undefined : undefined;
}
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return {
        ...
        nova-aiLeaseId: lease.leaseId,
        nova-aiGatewayInstanceId: lease.gatewayInstanceId,
    ...
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = {
          ...
          nova-aiLeaseId: launch.leaseId,
          nova-aiGatewayInstanceId: launch.gatewayInstanceId,
        } as AcpSessionRecord;
      }
...
const NOVA_AI_BRIDGE_EXECUTABLE = "nova-ai";
const NOVA_AI_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_NOVA_AI_PREFIX = "openai-codex/";
...
function isNova AIBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === NOVA_AI_BRIDGE_EXECUTABLE) {
    return parts[1] === NOVA_AI_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^nova-ai(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === NOVA_AI_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_NOVA_AI_PREFIX)) {
    value = value.slice(CODEX_ACP_NOVA_AI_PREFIX.length);
  }
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isNova AIBridgeCommand(command);
}
...
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: Nova AIAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { nova-aiProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = nova-aiProcessCleanup;
    this.wrapperRoot = options.nova-aiWrapperRoot;
    this.gatewayInstanceId = options.nova-aiGatewayInstanceId;
    this.processLeaseStore = options.nova-aiProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readNova AILeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readNova AILeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupNova AIOwnedAcpxProcessTree({
    ...
    await cleanupNova AIOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
      wrapperRoot: this.wrapperRoot,
      deps: this.processCleanupDeps,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withNova AIManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: Nova AIRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withNova AIManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
