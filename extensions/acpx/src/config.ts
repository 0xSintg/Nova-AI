...
import { formatPluginConfigIssue } from "nova-ai/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "nova-ai-plugin-tools";
const ACPX_NOVA_AI_TOOLS_MCP_SERVER_NAME = "nova-ai-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "nova-ai.plugin.json")) &&
...
function resolveAcpxPluginRootFromNova AILayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Nova AI layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromNova AILayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveNova AIRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const openClawRoot = resolveNova AIRoot(pluginRoot);
...
function resolveNova AIToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const openClawRoot = resolveNova AIRoot(pluginRoot);
  const distEntry = path.join(openClawRoot, "dist", "mcp", "nova-ai-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(openClawRoot, "src", "mcp", "nova-ai-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.openClawToolsMcpBridge && resolved[ACPX_NOVA_AI_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_NOVA_AI_TOOLS_MCP_SERVER_NAME} is reserved when openClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.openClawToolsMcpBridge) {
    resolved[ACPX_NOVA_AI_TOOLS_MCP_SERVER_NAME] = resolveNova AIToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
