...
import { NOVA_AI_ACPX_LEASE_ID_ARG, NOVA_AI_GATEWAY_INSTANCE_ID_ARG } from "./process-lease.js";
...
const DOCUMENTED_NOVA_AI_BRIDGE_COMMAND =
  "env NOVA_AI_HIDE_BANNER=1 NOVA_AI_SUPPRESS_NOTES=1 nova-ai acp --url ws://127.0.0.1:18789 --token-file ~/.nova-ai/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/nova-ai/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${NOVA_AI_ACPX_LEASE_ID_ARG} lease-close ${NOVA_AI_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "nova-ai" ? "nova-ai acp" : agentName),
        list: () => ["codex", "nova-ai"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("normalizes Nova AI Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("adds Codex wrapper stderr tail to generic session initialization failures", async () => {
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      nova-aiGatewayInstanceId: "gateway-test",
      nova-aiProcessLeaseStore: leaseStore.store,
      nova-aiWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        nova-aiLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic startTurn failure results", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        nova-aiLeaseId: "lease-start-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        nova-aiLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for Nova AI-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("nova-ai acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/nova-ai/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/nova-ai/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("nova-ai acp")).toBe(false);
  ...
  it("cleans up Nova AI-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/nova-ai/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        nova-aiWrapperRoot: "/tmp/nova-ai/acpx",
      },
      {
        nova-aiProcessCleanup: {
          listProcesses: vi.fn(async () => [
            {
              pid: 900,
              ppid: 1,
              command: 'node "/tmp/nova-ai/acpx/codex-acp-wrapper.mjs"',
            },
            {
              pid: 901,
              ppid: 900,
              command:
                "node /tmp/nova-ai/plugin-runtime-deps/node_modules/@zed-industries/codex-acp/bin/codex-acp.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/nova-ai/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("NOVA_AI_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("NOVA_AI_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.nova-aiGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.nova-aiLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    leaseStore.leases.set("lease-loaded", {
      ...
      sessionKey: "agent:codex:acp:binding:test",
      wrapperRoot: "/tmp/nova-ai/acpx",
      wrapperPath: "/tmp/nova-ai/acpx/codex-acp-wrapper.mjs",
      rootPid: 777,
      commandHash: "hash",
      startedAt: 1,
      state: "open",
    ...
    expect(loadedRecord?.nova-aiGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.nova-aiLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.nova-aiGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.nova-aiLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${NOVA_AI_ACPX_LEASE_ID_ARG} lease-old ${NOVA_AI_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${NOVA_AI_ACPX_LEASE_ID_ARG} lease-current ${NOVA_AI_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("routes nova-ai ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:nova-ai:acp:test",
      backend: "acpx",
  ...
  it("routes non-nova-ai sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for nova-ai sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:nova-ai:acp:test",
      backend: "acpx",
      runtimeSessionName: "nova-ai-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the nova-ai agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "nova-ai" ? "codex" : agentName),
        list: () => ["codex", "nova-ai"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the nova-ai bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "nova-ai acp" : agentName),
        list: () => ["codex", "nova-ai"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped nova-ai bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "nova-ai" ? DOCUMENTED_NOVA_AI_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "nova-ai"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node nova-ai entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "nova-ai" ? "env NOVA_AI_HIDE_BANNER=1 node nova-ai.mjs acp" : agentName,
        list: () => ["codex", "nova-ai"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:nova-ai:acp:test",
        agentCommand: DOCUMENTED_NOVA_AI_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:nova-ai:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:nova-ai:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to nova-ai bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
