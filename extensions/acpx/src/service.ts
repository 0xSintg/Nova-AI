...
const ENABLE_STARTUP_PROBE_ENV = "NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "NOVA_AI_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        nova-aiGatewayInstanceId: params.gatewayInstanceId,
        nova-aiProcessLeaseStore: params.processLeaseStore,
        nova-aiWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: Nova AIPluginServiceContext): string | undefined {
...
async function measureAcpxStartup<T>(
  ctx: Nova AIPluginServiceContext,
  name: string,
  run: () => T | Promise<T>,
): Promise<T> {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleNova AIOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: Nova AIPluginServiceContext): Promise<void> {
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale Nova AI-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: Nova AIPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
