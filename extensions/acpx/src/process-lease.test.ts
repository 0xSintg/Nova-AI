...
import {
  ...
  NOVA_AI_GATEWAY_INSTANCE_ID_ENV,
...
describe("createAcpxProcessLeaseStore", () => {
  it("serializes concurrent lease saves without dropping records", async () => {
    const stateDir = await mkdtemp(path.join(tmpdir(), "nova-ai-acpx-leases-"));
    try {
...
describe("withAcpxLeaseEnvironment", () => {
  it("adds lease environment and wrapper args on POSIX", () => {
    ...
    expect(command).toBe(
      [
        "env",
        `${NOVA_AI_ACPX_LEASE_ID_ENV}=lease-test`,
        `${NOVA_AI_GATEWAY_INSTANCE_ID_ENV}=gateway-test`,
        "node /tmp/nova-ai/acpx/codex-acp-wrapper.mjs",
        NOVA_AI_ACPX_LEASE_ID_ARG,
        "lease-test",
        NOVA_AI_GATEWAY_INSTANCE_ID_ARG,
        "gateway-test",
  ...
  it("keeps Windows logs keyed by lease id with wrapper args", () => {
    const command = withAcpxLeaseEnvironment({
      command: "node C:/nova-ai/acpx/codex-acp-wrapper.mjs",
      leaseId: "lease-test",
      gatewayInstanceId: "gateway-test",
      platform: "win32",
    });
    ...
    expect(command).toBe(
      [
        "node C:/nova-ai/acpx/codex-acp-wrapper.mjs",
        NOVA_AI_ACPX_LEASE_ID_ARG,
        "lease-test",
        NOVA_AI_GATEWAY_INSTANCE_ID_ARG,
        "gateway-test",
    ...
    expect(command).not.toContain(`${NOVA_AI_ACPX_LEASE_ID_ENV}=`);
    expect(command).not.toContain(`${NOVA_AI_GATEWAY_INSTANCE_ID_ENV}=`);
  });
});
