...
const NOVA_AI_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-nova-ai-owned" | "unverified-root";
};
...
export function isNova AILeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, NOVA_AI_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, NOVA_AI_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

export function isNova AIOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(NOVA_AI_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
export async function cleanupNova AIOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an Nova AI-owned wrapper, fail closed to
...
export async function reapStaleNova AIOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
