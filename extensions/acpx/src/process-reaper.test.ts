...
import {
  ...
  isNova AIOwnedAcpxProcessCommand,
  reapStaleNova AIOwnedAcpxOrphans,
  type AcpxProcessInfo,
} from "./process-reaper.js";

const WRAPPER_ROOT = "/tmp/nova-ai-state/acpx";
const CODEX_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/codex-acp-wrapper.mjs`;
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} ${NOVA_AI_ACPX_LEASE_ID_ARG} lease-1 ${NOVA_AI_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
...
describe("process reaper", () => {
  ...
  it("only treats generated wrappers as launch-lease aware", () => {
    ...
    expect(
      isNova AILeaseAwareAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND }),
    ).toBe(false);
    expect(isNova AILeaseAwareAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(
      false,
    );
  });

  it("recognizes Nova AI plugin-runtime-deps ACP adapter children", () => {
    expect(isNova AIOwnedAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(true);
    expect(isNova AIOwnedAcpxProcessCommand({ command: "npx @zed-industries/codex-acp" })).toBe(
      false,
    );
  });

  it("recognizes plugin-local ACP adapter package paths without trusting arbitrary installs", () => {
    expect(isNova AIOwnedAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND })).toBe(
      true,
    );
    expect(
      isNova AIOwnedAcpxProcessCommand({
  ...
  it("does not kill a reused same-root wrapper pid with a different lease identity", async () => {
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${NOVA_AI_ACPX_LEASE_ID_ARG} other-lease ${NOVA_AI_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
    ...
    expect(result).toEqual({
      inspectedPids: [113],
      terminatedPids: [],
      skippedReason: "not-nova-ai-owned",
    });
  ...
  it("does not kill a reused pid when the live command is not Nova AI-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-nova-ai-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale Nova AI-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleNova AIOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
