...
vi.mock("nova-ai/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.NOVA_AI_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.NOVA_AI_SKIP_ACPX_RUNTIME;
  } else {
    process.env.NOVA_AI_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/nova-ai-acpx-register-test",
    stateDir: "/tmp/nova-ai-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
