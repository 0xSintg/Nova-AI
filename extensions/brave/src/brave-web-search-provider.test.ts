...
import { validateJsonSchemaValue } from "nova-ai/plugin-sdk/config-schema";
...
const braveManifest = JSON.parse(
  fs.readFileSync(new URL("../nova-ai.plugin.json", import.meta.url), "utf-8"),
) as {
...
describe("brave web search provider", () => {
  ...
  it("points provider metadata at the canonical Brave docs page", () => {
    expect(createBraveWebSearchProvider().docsUrl).toBe(
      "https://docs.nova-ai.com/tools/brave-search",
    );
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    ...
    const result = await tool.execute({ query: "Nova AI docs" });

    expect(result).toEqual({
      error: "missing_brave_api_key",
      message:
        "web_search (brave) needs a Brave Search API key. Run `nova-ai configure --section web` to store it, or set BRAVE_API_KEY in the Gateway environment. If you do not want to configure a search API key, use web_fetch for a specific URL or the browser tool for interactive pages.",
