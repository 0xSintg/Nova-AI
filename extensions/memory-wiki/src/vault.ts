...
import { FsSafeError, pathExists, root as fsRoot } from "nova-ai/plugin-sdk/security-runtime";
...
- Use \`.nova-ai-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Nova AI memory-wiki plugin.
...
- \`.nova-ai-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".nova-ai-wiki/log.jsonl", "", createdFiles);
