...
import {
  ...
  withTrailingNewline,
} from "nova-ai/plugin-sdk/memory-host-markdown";
import { root as fsRoot } from "nova-ai/plugin-sdk/security-runtime";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- nova-ai:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- nova-ai:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- nova-ai:wiki:index:start -->",
      endMarker: "<!-- nova-ai:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- nova-ai:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- nova-ai:wiki:${group.dir}:index:end -->`,
