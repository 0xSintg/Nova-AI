...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Nova AI**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.nova-ai-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.nova-ai-wiki/cache/agent-digest.json` and `.nova-ai-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
