...
description: Maintain the Nova AI memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `nova-ai wiki ingest`, `nova-ai wiki compile`, and `nova-ai wiki lint` as the default maintenance loop.
- In `bridge` mode, run `nova-ai wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `nova-ai wiki unsafe-local import` only when the user explicitly opted into private local path access.
