...
import { isLoopbackHost } from "nova-ai/plugin-sdk/gateway-runtime";
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  const cfg = (opts.config ?? core.config.current()) as Nova AIConfig;
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `Nova AI ignored this action for ${decision.roomLabel}.`,
          },
        };
      },
