...
function createRuntimeCore(
  ...
  return {
    ...
    channel: {
      ...
      session: {
        resolveStorePath: () => "/tmp/nova-ai-test-sessions.json",
...
describe("mattermost inbound user posts", () => {
  ...
  it("drops posts when neither REST nor websocket channel type can be resolved", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const channelTypeConfig: Nova AIConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
  ...
  it("pins direct-message main route updates to the configured owner", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const directConfig: Nova AIConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
    ...
    expect(recordCall?.storePath).toBe("/tmp/nova-ai-test-sessions.json");
