...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__nova-ai_channel_id";
...
type InteractionCallbackConfig = Pick<Nova AIConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "nova-ai-mattermost-interactions").update(botToken).digest("hex");
}
