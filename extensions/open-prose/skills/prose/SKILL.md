...
metadata: { "nova-ai": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Nova AI `sessions_spawn`
- **File I/O** == Nova AI `read`/`write`
- **Remote fetch** == Nova AI `web_fetch` (or `exec` with curl when POST is required)
