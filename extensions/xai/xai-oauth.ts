...
import type { ProviderAuthContext, ProviderAuthMethod } from "nova-ai/plugin-sdk/plugin-entry";
import {
...
import { waitForLocalOAuthCallback } from "nova-ai/plugin-sdk/provider-auth-runtime";
...
export function buildXaiOAuthAuthorizeUrl(params: {
  ...
  url.searchParams.set("referrer", "nova-ai");
  return url.toString();
}
  ...
  try {
    ...
    return buildOauthProviderAuthResult({
      ...
      notes: [
        ...
        "xAI may label the consent app as Grok Build because Nova AI uses xAI's shared OAuth client.",
      ],
    });
  } catch (err) {
