...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "nova-ai/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "nova-ai/plugin-sdk/provider-stream-shared";
import { jsonResult } from "nova-ai/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          message:
            "code_execution needs xAI credentials. Run `nova-ai onboard --auth-choice xai-oauth` to sign in with Grok, run `nova-ai onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`.",
