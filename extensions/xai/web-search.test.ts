import { createTestWizardPrompter } from "nova-ai/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "nova-ai/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "nova-ai/plugin-sdk/runtime-env";
...
vi.mock("nova-ai/plugin-sdk/provider-web-search", async (importOriginal) => {
  const original = await importOriginal<typeof import("nova-ai/plugin-sdk/provider-web-search")>();
  return {
    ...original,
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      const result = await maybeTool.execute({ query: "Nova AI" });
  ...
  it("routes Grok web search through plugin webSearch.baseUrl", async () => {
    ...
    await tool.execute({ query: "Nova AI Grok proxy test" });
  ...
  it("reports malformed xAI web search JSON as a provider error", async () => {
    ...
    await expect(tool.execute({ query: "Nova AI" })).rejects.toThrow(
