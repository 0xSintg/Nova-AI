...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.NOVA_AI_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `nova-ai/${version}`);
      headers.set("originator", "nova-ai");
...
vi.mock("nova-ai/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("nova-ai/plugin-sdk/provider-http", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/nova-ai-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/nova-ai-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("nova-ai/2026.3.22");
    expect(request.headers?.get("originator")).toBe("nova-ai");
