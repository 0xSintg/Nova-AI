// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatNova AIUserAgent`.

import { NOVA_AI_VERSION as PACKAGE_VERSION } from "nova-ai/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "nova-ai";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.NOVA_AI_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.NOVA_AI_SERVICE_VERSION) ??
...
// an empty record so the nova-ai identity is not forwarded to the proxy.
