...
function resolveLegacyXSearchConfig(config?: Nova AIConfig): JsonRecord | undefined {
...
function resolvePluginXSearchConfig(config?: Nova AIConfig): JsonRecord | undefined {
...
function resolveLegacyGrokWebSearchConfig(config?: Nova AIConfig): JsonRecord | undefined {
  const web = config?.tools?.web as Record<string, unknown> | undefined;
...
function resolvePluginWebSearchConfig(config?: Nova AIConfig): JsonRecord | undefined {
  const pluginConfig = config?.plugins?.entries?.xai?.config;
...
export function resolveEffectiveXSearchConfig(config?: Nova AIConfig): JsonRecord | undefined {
