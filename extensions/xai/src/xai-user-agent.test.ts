...
describe("xaiUserAgent", () => {
  ...
  it("prefers NOVA_AI_VERSION env over the bundled package version", () => {
    vi.stubEnv("NOVA_AI_VERSION", "2026.3.22");
    expect(xaiUserAgent()).toBe("nova-ai/2026.3.22");
  });

  it("falls back to NOVA_AI_SERVICE_VERSION when NOVA_AI_VERSION is unset", () => {
    vi.stubEnv("NOVA_AI_VERSION", "");
    vi.stubEnv("NOVA_AI_SERVICE_VERSION", "2026.3.99");
    // NOVA_AI_VERSION from the SDK is the bundled VERSION constant. In a dev
    ...
    expect(result.startsWith("nova-ai/")).toBe(true);
    expect(result).not.toBe("nova-ai/");
  });

  it("returns the nova-ai/<version> shape", () => {
    vi.stubEnv("NOVA_AI_VERSION", "2026.5.16");
    expect(xaiUserAgent()).toMatch(/^nova-ai\/\d+\.\d+\.\d+$/u);
  });
});
