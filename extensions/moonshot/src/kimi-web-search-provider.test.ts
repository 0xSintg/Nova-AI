...
import { withEnvAsync } from "nova-ai/plugin-sdk/test-env";
...
describe("kimi web search provider", () => {
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
    const toolArguments = JSON.stringify({
      query: "Nova AI GitHub repository",
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "Nova AI is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "Nova AI is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "Nova AI", url: "https://github.com/nova-ai/nova-ai" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "Nova AI is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "Nova AI is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/nova-ai/nova-ai"]);
      expect(result).not.toHaveProperty("error");
    });
  });
