...
import { resolvePluginConfigObject } from "nova-ai/plugin-sdk/plugin-config-runtime";
import type { Nova AIPluginApi } from "nova-ai/plugin-sdk/plugin-entry";
import {
  ANTHROPIC_BY_MODEL_REPLAY_HOOKS,
  normalizeProviderId,
} from "nova-ai/plugin-sdk/provider-model-shared";
import { streamWithPayloadPatch } from "nova-ai/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Nova AI's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Nova AI identifies this as an Anthropic model via the ARN heuristic.
 ...
 * the Nova AI config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: Nova AIPluginApi): void {
  ...
  function resolveCurrentPluginConfig(
    config: Nova AIConfig | undefined,
