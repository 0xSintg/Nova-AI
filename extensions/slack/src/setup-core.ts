import { hasConfiguredSecretInput } from "nova-ai/plugin-sdk/secret-input";
import {
  ...
  type Nova AIConfig,
} from "nova-ai/plugin-sdk/setup-runtime";
import { formatDocsLink } from "nova-ai/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: Nova AIConfig, accountId: string): Nova AIConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: Nova AIConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: Nova AIConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: Nova AIConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: Nova AIConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: Nova AIConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: Nova AIConfig; accountId: string }) =>
    ...
    disable: (cfg: Nova AIConfig) => setSetupChannelEnabled(cfg, channel, false),
