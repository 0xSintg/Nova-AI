...
const NOVA_AI_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const NOVA_AI_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const NOVA_AI_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__nova-aiNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, NOVA_AI_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, NOVA_AI_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : NOVA_AI_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, NOVA_AI_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
            reject(error);
...
export function shouldSkipNova AISlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? NOVA_AI_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Nova AI's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
  });
  app.use(async (args) => {
    if (shouldSkipNova AISlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
