...
vi.mock("nova-ai/plugin-sdk/channel-feedback", () => ({
  DEFAULT_TIMING: {
    doneHoldMs: 0,
    errorHoldMs: 0,
  },
...
vi.mock("nova-ai/plugin-sdk/channel-message", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nova-ai/plugin-sdk/channel-message")>();
  return {
    ...actual,
...
vi.mock("nova-ai/plugin-sdk/channel-streaming", () => ({
...
vi.mock("nova-ai/plugin-sdk/outbound-runtime", () => ({
...
vi.mock("nova-ai/plugin-sdk/reply-payload", () => ({
...
vi.mock("nova-ai/plugin-sdk/security-runtime", () => ({
  resolvePinnedMainDmOwnerFromAllowlist: () => mockedPinnedMainDmOwner,
}));

vi.mock("nova-ai/plugin-sdk/string-coerce-runtime", () => ({
...
vi.mock("../config.runtime.js", () => ({
  resolveStorePath: () => "/tmp/nova-ai-store.json",
...
describe("dispatchPreparedSlackMessage preview fallback", () => {
  ...
  it("updates non-main DM last-route metadata on the prepared direct session", async () => {
    ...
    expect(updateLastRouteMock).toHaveBeenCalledWith({
      storePath: "/tmp/nova-ai-store.json",
      sessionKey: "agent:main:slack:direct:u1",
      deliveryContext: {
        channel: "slack",
        to: "user:U1",
