...
type SlackRouteBinding = NonNullable<Nova AIConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  Nova AIConfig,
  { bindingsRef: Nova AIConfig["bindings"]; normalizedCfg: Nova AIConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: Nova AIConfig): Nova AIConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as Nova AIConfig)
    : cfg;
