import { CURRENT_MESSAGE_MARKER } from "nova-ai/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "nova-ai/plugin-sdk/channel-test-helpers";
import { resetInboundDedupe } from "nova-ai/plugin-sdk/reply-dedupe";
import { HISTORY_CONTEXT_MARKER } from "nova-ai/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bnova-ai\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("nova-ai: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("nova-ai: hello <@U2>");
  });
