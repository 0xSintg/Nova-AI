...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): Nova AIConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as Nova AIConfig;
  ...
  function expectLastSlackSend(content: string, cfg: Nova AIConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: Nova AIConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: Nova AIConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/nova-ai-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/nova-ai-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/nova-ai-media/report.pdf",
      contentType: "application/pdf",
    });
  });
