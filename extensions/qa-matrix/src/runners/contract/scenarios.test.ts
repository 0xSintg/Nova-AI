...
const {
  ...
  resolveMatrixQaNova AICliEntryPath,
  runMatrixQaNova AICli,
  startMatrixQaNova AICli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `nova-ai ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaNova AICliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaNova AICli: vi.fn(),
  startMatrixQaNova AICli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaNova AICli.mockReset();
    startMatrixQaNova AICli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Nova AI Matrix QA run",
        scenarios: [
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          NOVA_AI_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaNova AICli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaNova AICli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.NOVA_AI_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.NOVA_AI_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          NOVA_AI_CONFIG_PATH: "/tmp/gateway-config.json",
          NOVA_AI_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaNova AICli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaNova AICli, "startMatrixQaNova AICli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaNova AICli, "startMatrixQaNova AICli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe(
        "Nova AI Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "Nova AI Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = mockObjectArg(
      ...
      expect(runMatrixQaNova AICli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaNova AICli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaNova AICli, "runMatrixQaNova AICli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaNova AICli, "startMatrixQaNova AICli")
        .env as Record<string, unknown>;
      expect(cliEnv?.NOVA_AI_STATE_DIR).toContain("nova-ai-matrix-cli-qa-");
      expect(cliEnv?.NOVA_AI_CONFIG_PATH).toContain("nova-ai-matrix-cli-qa-");
      const configPath = String(cliEnv?.NOVA_AI_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.NOVA_AI_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaNova AICli.mockImplementation(async ({ args, env }) => {
        if (env.NOVA_AI_CONFIG_PATH) {
      ...
      expect(runMatrixQaNova AICli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "Nova AI Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaNova AICli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.NOVA_AI_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.NOVA_AI_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("nova-ai matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaNova AICli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaNova AICli, "startMatrixQaNova AICli").env as Record<
            string,
            unknown
          >
        ).NOVA_AI_CONFIG_PATH,
      ).toContain("nova-ai-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaNova AICli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.NOVA_AI_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Nova AI Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Nova AI Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Nova AI Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
