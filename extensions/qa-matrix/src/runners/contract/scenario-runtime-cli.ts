import { spawn as startNova AICliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `nova-ai ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaNova AICliEntryPath(cwd: string): string {
...
export function startMatrixQaNova AICli(params: {
  ...
  const distEntryPath = resolveMatrixQaNova AICliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startNova AICliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
    env: params.env,
...
export async function runMatrixQaNova AICli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaNova AICli(params).wait();
}
...
export async function createMatrixQaNova AICliRuntime(params: {
  accountId: string;
  accessToken: string;
