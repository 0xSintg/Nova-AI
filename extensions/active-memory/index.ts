...
import { closeActiveMemorySearchManager } from "nova-ai/plugin-sdk/memory-host-search";
import {
  resolveLivePluginConfigObject,
  resolvePluginConfigObject,
} from "nova-ai/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type Nova AIPluginApi } from "nova-ai/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "nova-ai/plugin-sdk/routing";
import { isPathInside, replaceFileAtomic } from "nova-ai/plugin-sdk/security-runtime";
import {
  resolveSessionStoreEntry,
  updateSessionStore,
} from "nova-ai/plugin-sdk/session-store-runtime";
import { tempWorkspace, resolvePreferredNova AITmpDir } from "nova-ai/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: Nova AIConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: Nova AIConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: Nova AIPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: Nova AIPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: Nova AIConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: Nova AIPluginApi): Nova AIConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as Nova AIConfig | undefined) ??
      (api.config as Nova AIConfig | undefined)
    );
  } catch {
    return api.config as Nova AIConfig | undefined;
  }
}
...
export default definePluginEntry({
  ...
  register(api: Nova AIPluginApi) {
    const readCurrentConfig = (): Nova AIConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as Nova AIConfig | undefined) ??
          (api.config as Nova AIConfig | undefined)
    ...
    const refreshLiveConfigFromRuntime = () => {
      const livePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as Nova AIConfig
          : undefined,
        "active-memory",
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as Nova AIConfig;
          if (action === "status") {
            return {
