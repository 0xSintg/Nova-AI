import { createAccountListHelpers } from "nova-ai/plugin-sdk/account-helpers";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "nova-ai/plugin-sdk/account-id";
import { resolveMergedAccountConfig } from "nova-ai/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "nova-ai";
  const botDisplayName = merged.botDisplayName?.trim() || "Nova AI QA";
  return {
    accountId,
    enabled,
