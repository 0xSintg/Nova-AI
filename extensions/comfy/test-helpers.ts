...
export function buildComfyConfig(config: Record<string, unknown>): Nova AIConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): Nova AIConfig {
  return {
    models: {
      providers: {
        comfy: config,
