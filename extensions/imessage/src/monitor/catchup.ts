...
import { readJsonFileWithFallback, writeJsonFileAtomically } from "nova-ai/plugin-sdk/json-store";
...
//
// See https://github.com/nova-ai/nova-ai/issues/78649 for design discussion.
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    const name = "nova-ai-vitest-" + process.pid;
    return path.join(resolvePreferredNova AITmpDir(), name);
  }
  return resolveStateDir(env);
}
...
/**
 * Cursor file path: `<nova-aiStateDir>/imessage/catchup/<safePrefix>__<sha256[:12]>.json`.
 * `nova-aiStateDir` resolves through `NOVA_AI_STATE_DIR` (or the plugin-sdk default,
 * `~/.nova-ai`). On a default install the cursor lands at
 * `~/.nova-ai/imessage/catchup/<safePrefix>__<sha256[:12]>.json`.
 */
