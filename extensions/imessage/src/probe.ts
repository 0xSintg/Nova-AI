...
import type { BaseProbeResult } from "nova-ai/plugin-sdk/channel-contract";
import { runCommandWithTimeout } from "nova-ai/plugin-sdk/process-runtime";
...
import { detectBinary } from "nova-ai/plugin-sdk/setup";
...
export function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run Nova AI on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by nova-ai/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // nova-ai/imsg#114. We do this even when the bridge is unavailable
