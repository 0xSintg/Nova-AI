...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: Nova AIConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: Nova AIConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: Nova AIConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run Nova AI on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `nova-ai channels status --probe` to verify private API actions.",
    "Ensure Nova AI has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: Nova AIConfig; accountId?: string }) =>
