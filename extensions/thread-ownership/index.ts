...
type AgentEntry = NonNullable<NonNullable<Nova AIConfig["agents"]>["list"]>[number];
...
function resolveOwnershipAgent(config: Nova AIConfig): { id: string; name: string } {
...
export default definePluginEntry({
  ...
  register(api: Nova AIPluginApi) {
    const resolveCurrentState = () => {
      const currentConfig = (api.runtime.config?.current?.() ?? api.config) as Nova AIConfig;
