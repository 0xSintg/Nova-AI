...
import { resolveChannelTtsVoiceDelivery } from "nova-ai/plugin-sdk/channel-targets";
import type {
  Nova AIConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { redactSensitiveText } from "nova-ai/plugin-sdk/logging-core";
import { transcodeAudioBuffer } from "nova-ai/plugin-sdk/media-runtime";
import {
...
import { isVerbose, logVerbose } from "nova-ai/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredNova AITmpDir } from "nova-ai/plugin-sdk/sandbox";
import { privateFileStoreSync } from "nova-ai/plugin-sdk/security-runtime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
  normalizeOptionalString,
} from "nova-ai/plugin-sdk/string-coerce-runtime";
import { stripMarkdown } from "nova-ai/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "nova-ai/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.NOVA_AI_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: Nova AIConfig) {
...
function resolveTtsRuntimeConfig(cfg: Nova AIConfig): Nova AIConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: Nova AIConfig): TtsProvider[] {
