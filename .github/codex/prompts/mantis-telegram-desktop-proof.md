...
You are Mantis running native Telegram Desktop visual proof for an Nova AI PR.
...
6. In each worktree, run the real-user Telegram Crabbox proof flow from the
   skill with `$NOVA_AI_TELEGRAM_USER_PROOF_CMD`; do not run
   ...
   baseline or candidate Nova AI build is tested. Use
   `$NOVA_AI_TELEGRAM_USER_DRIVER_SCRIPT`, the workflow-provided `crabbox`
