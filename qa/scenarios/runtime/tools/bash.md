...
successCriteria:
  - Pi may expose Nova AI exec while Codex app-server mode may omit duplicate Nova AI dynamic exec/process.
...
execution:
  kind: flow
  summary: Exercise the bash family through the Nova AI exec tool.
  config:
    toolName: exec
    toolCoverage:
      family: bash
      ...
      qaImpact: P1
      action: split native command behavior from Nova AI dynamic tool parity
      reason: Codex app-server intentionally owns command execution natively; the fixture must not require Nova AI dynamic exec exposure.
    knownHarnessGap:
      issue: "#80319"
      reason: QA tool-defaults currently needs native command behavior coverage instead of Nova AI dynamic exec exposure.
