...
successCriteria:
  - Pi may expose Nova AI write while Codex app-server mode may omit duplicate Nova AI dynamic write.
...
execution:
  ...
  config:
    ...
    toolCoverage:
      ...
      qaImpact: P1
      action: split native write behavior from Nova AI dynamic tool parity
      reason: Codex app-server intentionally owns write natively; the fixture must not require Nova AI dynamic write exposure.
    knownHarnessGap:
      issue: "#80319"
      reason: QA tool-defaults currently needs native write behavior coverage instead of Nova AI dynamic write exposure.
