...
function resolveControlUiBuildId(): string {
  const explicit =
    process.env.NOVA_AI_CONTROL_UI_BUILD_ID?.trim() || process.env.NOVA_AI_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__NOVA_AI_CONTROL_UI_BUILD_ID__"';
...
export default defineConfig(() => {
  const envBase = process.env.NOVA_AI_CONTROL_UI_BASE_PATH?.trim();
  const base = envBase ? normalizeBase(envBase) : "./";
  const controlUiBuildId = resolveControlUiBuildId();
  return {
    base,
    define: {
      NOVA_AI_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId),
    },
    ...
    plugins: [
      controlUiServiceWorkerBuildIdPlugin(controlUiBuildId),
      {
        ...
        configureServer(server) {
          server.middlewares.use("/__nova-ai/control-ui-config.json", (_req, res) => {
