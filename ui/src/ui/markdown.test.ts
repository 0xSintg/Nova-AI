...
describe("toSanitizedMarkdownHtml", () => {
  ...
  describe("explicit protocol links", () => {
    ...
    it("links http:// URLs", () => {
      const html = toSanitizedMarkdownHtml("Visit http://github.com/nova-ai");
      expect(html).toBe(
        '<p>Visit <a href="http://github.com/nova-ai" rel="noreferrer noopener" target="_blank">http://github.com/nova-ai</a></p>\n',
      );
    });
    ...
    it("does NOT rewrite explicit markdown links with CJK display text", () => {
      const html = toSanitizedMarkdownHtml("[Nova AI中文](https://docs.nova-ai.com)");
      expect(html).toBe(
        '<p><a href="https://docs.nova-ai.com" rel="noreferrer noopener" target="_blank">Nova AI中文</a></p>\n',
      );
    });
