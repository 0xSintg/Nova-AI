...
describe("Nova AIApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    const { Nova AIApp } = await import("./app.ts");
    const app = Object.create(Nova AIApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await Nova AIApp.prototype.toggleRealtimeTalk.call(app as never);
