...
vi.mock("../views/agents-utils.ts", () => {
  ...
  return {
    assistantAvatarFallbackUrl: () => "/nova-ai-molty.png",
    agentLogoUrl: () => "/nova-ai-logo.svg",
...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__nova-ai__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  localStorageValues.delete("nova-ai:skipDeleteConfirm");
}
...
describe("grouped chat rendering", () => {
  ...
  it("renders allowed transcript and content image variants", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      expect(mediaUrl.pathname).toBe("/nova-ai/__nova-ai__/assistant-media");
      expect([...mediaUrl.searchParams.keys()].toSorted()).toEqual(["meta", "source"]);
      expect(mediaUrl.searchParams.get("meta")).toBe("1");
      expect(mediaUrl.searchParams.get("source")).toMatch(/^\/tmp\/nova-ai\/.+\.(png|jpg)$/u);
    ...
    const renderUserMedia = (message: unknown) => {
      ...
      const renderMessage = () =>
        renderGroupedMessage(container, message, "user", {
          ...
          localMediaPreviewRoots: ["/tmp/nova-ai"],
    ...
    let container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/nova-ai/user-upload.png",
      timestamp: Date.now(),
    });
    ...
    expect(
      ...
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Fuser-upload.png&mediaTicket=ticket-user",
    );
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPaths: ["/tmp/nova-ai/first.png", "/tmp/nova-ai/second.jpg"],
    ...
    expect(
      ...
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ffirst.png&mediaTicket=ticket-user",
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Fsecond.jpg&mediaTicket=ticket-user",
    ]);
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/__nova-ai__/media/user-upload.pdf",
    ...
    expect(documentLink?.getAttribute("href")).toBe("/__nova-ai__/media/user-upload.pdf");
    vi.unstubAllGlobals();
  });
  ...
  it("fetches managed chat images with auth and renders blob previews", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      const headers = init?.headers as Headers;
      expect(headers.get("Authorization")).toBe("Bearer session-token");
      expect(headers.get("x-nova-ai-requester-session-key")).toBe("agent:main:main");
      return {
        ok: true,
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/nova-ai/test image.png\nMEDIA:/tmp/nova-ai/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    const [, fetchInit] = requireFetchCallForUrl(
      fetchMock,
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest+image.png&meta=1",
    );
    expectSameOriginGet(fetchInit);
    ...
    expect(image?.getAttribute("src")).toBe(
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest+image.png&mediaTicket=ticket-local",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest-doc.pdf&mediaTicket=ticket-local",
    );
  ...
  it("refreshes cached local assistant media tickets before they expire without another render", async () => {
    ...
    expect(
      ...
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest+image.png&mediaTicket=ticket-old",
    );
    ...
    expect(
      ...
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest+image.png&mediaTicket=ticket-new",
    );
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(image.getAttribute("src")).toBe(
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest+image.png&mediaTicket=ticket-fresh",
    );
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__nova-ai__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__nova-ai__/media/test-doc.pdf");
    expect(container.querySelector(".chat-assistant-attachment-badge")).toBeNull();
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/nova-ai/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/nova-ai/__nova-ai__/assistant-media?source=%2FC%3A%2Ftmp%2Fnova-ai%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/nova-ai/__nova-ai__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/nova-ai/__nova-ai__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("revalidates cached unavailable local assistant attachments after retry window", async () => {
    ...
    expect(
      ...
      "/nova-ai/__nova-ai__/assistant-media?source=%2Ftmp%2Fnova-ai%2Ftest+image.png&mediaTicket=ticket-retry",
    );
    expect(container.querySelector(".chat-assistant-attachment-badge")).toBeNull();
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__nova-ai__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasPluginSurfaceUrl: "http://127.0.0.1:19003/__nova-ai__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__nova-ai__/cap/cap_123/__nova-ai__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
  ...
  it("renders server-history canvas blocks for the live toolResult sequence after history reload", () => {
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__nova-ai__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__nova-ai__/canvas/documents/cv_canvas_live_history/index.html",
    );
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__nova-ai__/canvas/documents/cv_inline_default/index.html",
    );
  ...
  it("renders assistant_message canvas results in the assistant bubble even when tool rows are visible", () => {
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__nova-ai__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
    ...
    const iframe = expectElement(bubble, ".chat-tool-card__preview-frame", HTMLIFrameElement);
    expect(iframe.getAttribute("src")).toBe(
      "/__nova-ai__/canvas/documents/cv_inline_visible/index.html",
    );
