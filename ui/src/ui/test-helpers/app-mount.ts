...
import "../app.ts";
import type { Nova AIApp } from "../app.ts";
...
const mountedApps = new Set<Nova AIApp>();

function collectMountedApps() {
  return new Set<Nova AIApp>([
    ...mountedApps,
    ...document.querySelectorAll<Nova AIApp>("nova-ai-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<Nova AIApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<Nova AIApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("nova-ai-app") as Nova AIApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__NOVA_AI_CONTROL_UI_BASE_PATH__"] = undefined;
