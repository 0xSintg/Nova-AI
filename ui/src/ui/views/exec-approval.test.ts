...
import { type Nova AIModalDialog } from "../components/modal-dialog.ts";
...
describe("approval and confirmation modals", () => {
  ...
  it("renders exec approval as a labelled modal", async () => {
    ...
    expect(dialog.getAttribute("aria-labelledby")).toBe("nova-ai-modal-dialog-label");
    expect(dialog.getAttribute("aria-describedby")).toBe("nova-ai-modal-dialog-description");
    expect(modal.shadowRoot?.querySelector("#nova-ai-modal-dialog-label")?.textContent).toBe(
      "Exec approval needed",
    );
    expect(
      modal.shadowRoot?.querySelector("#nova-ai-modal-dialog-description")?.textContent?.trim(),
    ).toBe("expires in 1m");
  ...
  it("uses the shared modal primitive for gateway URL confirmation and cancels on Escape", async () => {
    ...
    render(
      renderGatewayUrlConfirmation({
        pendingGatewayUrl: "wss://gateway.example/nova-ai",
