...
import { type Nova AIModalDialog } from "./modal-dialog.ts";
...
async function renderModal() {
  ...
  const modal = container.querySelector<Nova AIModalDialog>("nova-ai-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected nova-ai-modal-dialog");
  }
...
function expectShadowElement(modal: Nova AIModalDialog, id: string): HTMLElement {
...
describe("nova-ai-modal-dialog", () => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("nova-ai-modal-dialog-label");
    expect(descriptionId).toBe("nova-ai-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "nova-ai-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "nova-ai-modal-dialog-description").textContent).toBe(
