...
declare global {
  interface Window {
    __NOVA_AI_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class Nova AIApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "Nova AI"}: ${entry.text}`;
        },
      },
...
if (!customElements.get("nova-ai-app")) {
  customElements.define("nova-ai-app", Nova AIApp);
}
