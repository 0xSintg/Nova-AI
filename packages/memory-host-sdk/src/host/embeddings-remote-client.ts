...
import { requireApiKey, resolveApiKeyForProvider } from "./nova-ai-runtime-auth.js";
...
function resolveNova AIAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.NOVA_AI_VERSION?.trim() : undefined;
  return {
    originator: "nova-ai",
    ...(version ? { version } : {}),
    "User-Agent": version ? `nova-ai/${version}` : "nova-ai",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveNova AIAttributionHeaders());
  }
