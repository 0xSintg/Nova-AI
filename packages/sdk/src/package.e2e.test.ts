...
describe("Nova AI SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nova-ai-sdk-consumer-"));
    tempDirs.push(tempDir);

    await runCommand("pnpm", ["--filter", "@nova-ai/sdk", "build"], {
      cwd: repoRoot,
      timeoutMs: 180_000,
    });
    ...
    const importScript = `
      import { GatewayClientTransport, Nova AI, normalizeGatewayEvent } from "@nova-ai/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof Nova AI !== "function") throw new Error("missing client export");
