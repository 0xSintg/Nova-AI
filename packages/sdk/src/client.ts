...
export type Nova AIOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: Nova AITransport;
};

function resolveGatewayUrl(options: Nova AIOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Nova AI Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Nova AI Gateway yet`);
}
...
function readChatProjection(event: Nova AIEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: Nova AIEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: Nova AIEvent): boolean {
  return (
...
export class Nova AI {
  ...
  private readonly transport: Nova AITransport;
  private readonly normalizedEvents = new EventHub<Nova AIEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, Nova AIEvent[]>();
  private connected = false;
  private eventPumpPromise: Promise<void> | null = null;
  private eventPumpReady: Promise<void> | null = null;

  constructor(options: Nova AIOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: Nova AIEvent) => boolean): AsyncIterable<Nova AIEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: Nova AIEvent) => boolean,
  ): AsyncIterable<Nova AIEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: Nova AIEvent): Nova AIEvent | undefined => {
    ...
    const matches = (event: Nova AIEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: Nova AIEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): Nova AIEvent[] {
...
export class Agent {
  constructor(
    private readonly client: Nova AI,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: Nova AI) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<Nova AIEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Nova AI,
