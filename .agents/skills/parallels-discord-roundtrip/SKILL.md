...
```bash
export NOVA_AI_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.nova-ai/nova-ai.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env NOVA_AI_PARALLELS_DISCORD_TOKEN \
...
- Use the `nova-ai` wrapper for guest `message send/read`; `node nova-ai.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/nova-ai-parallels-smoke.*`
...
- host inbound nonce appears in `nova-ai message read` output
