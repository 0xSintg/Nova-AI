...
description: Use gitcrawl to search duplicate Nova AI PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local Nova AI history and clustering source.
...
```bash
gitcrawl threads nova-ai/nova-ai --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors nova-ai/nova-ai --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search nova-ai/nova-ai --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search nova-ai/nova-ai --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail nova-ai/nova-ai --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo nova-ai/nova-ai --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo nova-ai/nova-ai --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo nova-ai/nova-ai --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R nova-ai/nova-ai "<problem phrase>" --types group --limit 10
prtags search similar -R nova-ai/nova-ai "<problem summary>" --types group --limit 10
prtags group list -R nova-ai/nova-ai
```
...
```bash
prtags group create -R nova-ai/nova-ai \
  --kind mixed \
...
```bash
prtags field ensure -R nova-ai/nova-ai --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R nova-ai/nova-ai --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R nova-ai/nova-ai --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R nova-ai/nova-ai --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R nova-ai/nova-ai --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R nova-ai/nova-ai --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R nova-ai/nova-ai --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R nova-ai/nova-ai --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R nova-ai/nova-ai --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R nova-ai/nova-ai <pr-number> \
...
```bash
prtags annotation issue set -R nova-ai/nova-ai <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R nova-ai/nova-ai
```
