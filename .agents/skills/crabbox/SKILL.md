---
name: crabbox
description: Use the Crabbox wrapper for Nova AI remote validation across Linux, macOS, Windows, and WSL2, including delegated Blacksmith Testbox proof. Report the actual provider and id.
---

# Crabbox

Use the Crabbox wrapper when Nova AI needs remote Linux proof for broad tests,
...
For Nova AI maintainer broad `pnpm` gates, Blacksmith Testbox through the
...
comparison. Use `--provider blacksmith-testbox` when the task needs Nova AI
...
- Nova AI scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
- Check `.crabbox.yaml` for direct-provider defaults. Omitting `--provider`
  means brokered AWS today.
- For broad Nova AI maintainer `pnpm` gates, prefer the repo wrapper with
...
- Do not treat inherited shell env as operator intent. In particular,
  `NOVA_AI_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `NOVA_AI_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. Nova AI broad
...
Do not silently substitute AWS macOS for normal Nova AI Linux proof. Report
...
pnpm crabbox:run -- \
  ...
  --timing-json \
  --shell -- \
  "env CI=1 NODE_OPTIONS=--max-old-space-size=4096 NOVA_AI_TEST_PROJECTS_PARALLEL=6 NOVA_AI_VITEST_MAX_WORKERS=1 NOVA_AI_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
```
...
Use this for Nova AI maintainer broad/heavy `pnpm` gates when the prepared CI
...
node scripts/crabbox-wrapper.mjs run \
  ...
  --timing-json \
  -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 NOVA_AI_TEST_PROJECTS_PARALLEL=6 NOVA_AI_VITEST_MAX_WORKERS=1 NOVA_AI_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 NOVA_AI_TESTBOX=1 NOVA_AI_TESTBOX_REMOTE_RUN=1 pnpm check:changed
```
...
install Nova AI as the user would, run the same setup/update/CLI/Gateway/API
...
- Prefer `NOVA_AI_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `NOVA_AI_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `NOVA_AI_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `NOVA_AI_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `NOVA_AI_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `NOVA_AI_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@nova-ai/codex`, and the
...
- Bad local config: inspect `.crabbox.yaml`, `crabbox config show`, and
  `crabbox whoami`; normal Nova AI proof should use brokered AWS without
...
pnpm crabbox:run -- --debug --timing-json -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 NOVA_AI_TEST_PROJECTS_PARALLEL=6 NOVA_AI_VITEST_MAX_WORKERS=1 NOVA_AI_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed
```
...
```sh
pnpm crabbox:run -- --debug --timing-json -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 NOVA_AI_TEST_PROJECTS_PARALLEL=6 NOVA_AI_VITEST_MAX_WORKERS=1 NOVA_AI_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test
```

Auth fallback, only when `blacksmith` says auth is missing:

```sh
blacksmith auth login --non-interactive --organization nova-ai
```
...
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- \
  CI=1 NODE_OPTIONS=--max-old-space-size=4096 NOVA_AI_TEST_PROJECTS_PARALLEL=6 NOVA_AI_VITEST_MAX_WORKERS=1 pnpm test:changed
```
...
Use AWS for normal Nova AI remote proof. The repo `.crabbox.yaml` already
...
pnpm crabbox:run -- --id <cbx_id-or-slug> --timing-json --shell -- "env NODE_OPTIONS=--max-old-space-size=4096 NOVA_AI_TEST_PROJECTS_PARALLEL=6 NOVA_AI_VITEST_MAX_WORKERS=1 NOVA_AI_VITEST_NO_OUTPUT_TIMEOUT_MS=900000 pnpm test:changed"
...
```sh
brew install nova-ai/tap/crabbox
crabbox login --url https://crabbox.nova-ai.com --provider aws
```
...
- If broker auth is missing, run `crabbox login --url https://crabbox.nova-ai.com --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal Nova AI validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered Nova AI proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.nova-ai.com --provider aws --token-stdin`.
...
for Nova AI lanes. Let that config drive normal validation.
...
Do not add Nova AI-specific setup to Crabbox itself. Put repo setup in the
