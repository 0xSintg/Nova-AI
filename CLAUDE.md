...
- Repo: `https://github.com/nova-ai/nova-ai`
...
- Installers: sibling `../nova-ai.com`.
...
- Plugins cross into core only via `nova-ai/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- Legacy config repair belongs in `nova-ai doctor --fix`, not startup/load-time core migrations. Runtime paths use canonical contracts.
...
- CLI: `pnpm nova-ai ...` or `pnpm dev`; build: `pnpm build`.
...
- Use `$nova-ai-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Use `$nova-ai-pr-maintainer` immediately for maintainer-side Nova AI issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. Contributor PR creation/refresh follows the requested contributor workflow; linked refs alone do not require maintainer archive tooling.
...
- Naming: **Nova AI** product/docs; `nova-ai` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `nova-ai/plugin-sdk/*`.
- Do not edit baseline/inventory/ignore/snapshot/expected-failure files to silence checks without explicit approval.
- Do not run independent `pnpm test`/Vitest commands concurrently in one worktree; Vitest cache races with `ENOTEMPTY`. Group one command or use distinct `NOVA_AI_VITEST_FS_MODULE_CACHE_PATH`.
- Test workers max 16. Memory pressure: `NOVA_AI_VITEST_MAX_WORKERS=1 pnpm test`.
- Live: `NOVA_AI_LIVE_TEST=1 pnpm test:live`; verbose `NOVA_AI_LIVE_TEST_QUIET=0`.
- Guide: `docs/reference/test.md`.

## Docs / Changelog

- Use `$nova-ai-docs` for docs writing/review. Docs change with behavior/API.
...
- Docs final answers: include relevant full `https://docs.nova-ai.com/...` URL(s). If issue/PR work too, GitHub URL last.
...
- Contributor-facing changelog entries thank credited human `@author`. Never thank bots, `@nova-ai`, `@clawsweeper`, or `@steipete`; if unknown, omit thanks.
...
- Secrets: channel/provider creds in `~/.nova-ai/credentials/`; model auth profiles in `~/.nova-ai/agents/<agentId>/agent/auth-profiles.json`.
- Dependency patches/overrides/vendor changes need explicit approval. `pnpm-workspace.yaml` patched dependencies use exact versions only.
- Carbon pins owner-only: do not change `@buape/carbon` unless Shadow (`@thewilloftheshadow`, verified by `gh`) asks.
- Releases/publish/version bumps need explicit approval. Use `$nova-ai-release-maintainer`.
- GHSA/advisories: `$nova-ai-ghsa-maintainer` / `$security-triage`. Secret scanning: `$nova-ai-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `nova-ai gateway restart/status --deep`; logs = `./scripts/clawlog.sh`. No launchd/ad-hoc tmux.
- Version bump surfaces live in `$nova-ai-release-maintainer`.
- Parallels: `$nova-ai-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `nova-ai doctor`.
