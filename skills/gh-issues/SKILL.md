...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${NOVA_AI_CONFIG_PATH:-${NOVA_AI_STATE_DIR:-$HOME/.nova-ai}/nova-ai.json}"
...
```text
${NOVA_AI_STATE_DIR:-$HOME/.nova-ai}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with nova-ai message send if route provided.
```
