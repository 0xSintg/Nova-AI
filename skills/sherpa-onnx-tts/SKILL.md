...
1. Download the runtime for your OS (extracts into `$NOVA_AI_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.nova-ai/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$NOVA_AI_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.nova-ai/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Nova AI config file (`$NOVA_AI_CONFIG_PATH`, default `~/.nova-ai/nova-ai.json`):
