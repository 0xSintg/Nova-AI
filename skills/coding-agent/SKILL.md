...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.nova-ai`, `$NOVA_AI_STATE_DIR`, or active Nova AI state dirs.
...
- Worker must send completion/failure via `nova-ai message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/nova-ai`; use an isolated checkout.
...
nova-ai message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use nova-ai system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t nova-ai-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
