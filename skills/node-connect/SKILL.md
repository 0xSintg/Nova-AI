---
name: node-connect
description: "Diagnose Nova AI Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Nova AI is advertising that route, then fix pairing/auth.
...
- whether `nova-ai devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `nova-ai qr --json`. It uses the same setup-code payload Android scans.

```bash
nova-ai config get gateway.mode
nova-ai config get gateway.bind
nova-ai config get gateway.tailscale.mode
nova-ai config get gateway.remote.url
nova-ai config get gateway.auth.mode
nova-ai config get gateway.auth.allowTailscale
nova-ai config get plugins.entries.device-pair.config.publicUrl
nova-ai qr --json
nova-ai devices list
nova-ai nodes status
```

If this Nova AI instance is pointed at a remote gateway, also run:

```bash
nova-ai qr --remote --json
```
...
If `nova-ai qr --json` says `Gateway is only bound to loopback`:
...
- `nova-ai devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run nova-ai qr again, rescan, then approve the pending device pairing.`
