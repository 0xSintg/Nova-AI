---
name: canvas
description: "Present HTML on connected Nova AI node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "nova-ai": { "emoji": "🖼️" } }
---
...
Active config: `$NOVA_AI_CONFIG_PATH` or `~/.nova-ai/nova-ai.json`.
...
4. Present the hosted URL: `/__nova-ai__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__nova-ai__/canvas/index.html
http://<gateway-host>:<gateway.port>/__nova-ai__/canvas/games/snake.html
```

Path mapping:

- `/__nova-ai__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__nova-ai__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
