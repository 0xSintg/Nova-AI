---
summary: "Use xAI Grok models in Nova AI"
read_when:
  - You want to use Grok models in Nova AI
  - You are configuring xAI auth or model ids
title: "xAI"
---

Nova AI ships a bundled `xai` provider plugin for Grok models. For most
users, the recommended path is Grok OAuth with an eligible SuperGrok or X Premium
subscription. Nova AI stays local-first: the Gateway, config, routing, and
...
app. xAI may still show Grok Build on the consent screen because Nova AI uses
xAI's shared OAuth client.

## Choose your setup path

Use the path that matches your Nova AI install state:

<Steps>
  <Step title="New Nova AI install">
    ...
    ```bash
    nova-ai onboard --install-daemon --auth-choice xai-device-code
    ```

    OAuth does not require an xAI API key. Nova AI does not require the Grok
    Build app. xAI may still label the consent app as Grok Build because
    Nova AI uses xAI's shared OAuth client.

  </Step>
  <Step title="Existing Nova AI install">
    If Nova AI is already configured, sign in to xAI only. Do not rerun full
    onboarding or reinstall the daemon just to connect Grok:

    ```bash
    nova-ai models auth login --provider xai --method oauth
    ```
    ...
    ```bash
    nova-ai models auth login --provider xai --device-code
    ```

    To make Grok the default model after signing in, apply it separately:

    ```bash
    nova-ai models set xai/grok-4.3
    ```
  ...
  <Step title="API-key path">
    ...
    ```bash
    nova-ai models auth login --provider xai --method api-key
...
<Note>
Nova AI uses the xAI Responses API as the bundled xAI transport. The same
credential from `nova-ai models auth login --provider xai --method oauth`,
`nova-ai models auth login --provider xai --device-code`, or
`nova-ai models auth login --provider xai --method api-key` can also power first-class
...
- To inspect saved xAI auth profiles, run:
  ...
  ```bash
  nova-ai models auth list --provider xai
...
<Tip>
Use `xai-device-code` when signing in from SSH, Docker, or a VPS. Nova AI
...
Nova AI includes the current xAI chat models out of the box, ordered newest
...
Fast, and Grok Code slugs for existing configs, but Nova AI no longer shows
...
The bundled plugin maps xAI's current public API surface onto Nova AI's shared
...
| xAI capability             | Nova AI surface                          | Status                                                              |
| -------------------------- | ----------------------------------------- | ------------------------------------------------------------------- |
...
| Streaming TTS              | -                                         | Not exposed; Nova AI's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                 |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                 |
| Realtime voice             | -                                         | Not exposed yet; different session/WebSocket contract               |
| Files / batches            | Generic model API compatibility only      | Not a first-class Nova AI tool                                     |

<Note>
Nova AI uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different Nova AI contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    nova-ai config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    Nova AI can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    Nova AI asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. Nova AI forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    Nova AI uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the Nova AI speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through Nova AI's
    ...
    transcription request. Prompt hints are accepted by the shared Nova AI
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an Nova AI tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an Nova AI tool for
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config fallback,
      ...
      the consent page may show Grok Build even though Nova AI does not require
      the Grok Build app.
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      normal xAI provider path because it requires a different upstream API
      surface than the standard Nova AI xAI transport.
    - xAI Realtime voice is not registered as an Nova AI provider yet. It
  ...
  <Accordion title="Advanced notes">
    - Nova AI applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as Nova AI
      tools. Nova AI enables the specific xAI built-in it needs inside each tool
...
NOVA_AI_LIVE_TEST=1 NOVA_AI_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
NOVA_AI_LIVE_TEST=1 NOVA_AI_LIVE_TEST_QUIET=1 NOVA_AI_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
shared image live file verifies the same xAI provider through Nova AI's
