...
Deepgram is a speech-to-text API. In Nova AI it is used for inbound
...
For batch transcription, Nova AI uploads the complete audio file to Deepgram
and injects the transcript into the reply pipeline (`{{Transcript}}` +
`[Audio]` block). For Voice Call streaming, Nova AI forwards live G.711
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Nova AI transcribes it
