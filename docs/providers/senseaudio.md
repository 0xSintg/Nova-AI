...
SenseAudio can transcribe inbound audio and voice-note attachments through Nova AI's shared `tools.media.audio` pipeline. Nova AI posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Nova AI uploads the
...
<Note>
SenseAudio is batch STT only in Nova AI. Voice Call realtime transcription
