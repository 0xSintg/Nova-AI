---
summary: "Use Venice AI privacy-focused models in Nova AI"
read_when:
  - You want privacy-focused inference in Nova AI
...
<Steps>
  ...
  <Step title="Configure Nova AI">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        nova-ai onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    nova-ai agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Nova AI shows all available Venice models. Pick based on your needs:
...
```bash
nova-ai models set venice/kimi-k2-5
nova-ai models set venice/claude-opus-4-6
```

List all available models:

```bash
nova-ai models list --all --provider venice
```

You can also run `nova-ai configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Nova AI fills the required DeepSeek V4
...
Nova AI keeps that provider-specific replay fix separate from the native
...
Nova AI ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
nova-ai agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
nova-ai agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
nova-ai agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
nova-ai agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
nova-ai agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    nova-ai models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `nova-ai models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
