---
summary: "Use Qwen Cloud via Nova AI's bundled qwen provider"
read_when:
  - You want to use Qwen with Nova AI
...
See [Issue #49557](https://github.com/nova-ai/nova-ai/issues/49557) for
background.

</Warning>

Nova AI now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        nova-ai onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        nova-ai onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        nova-ai models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        nova-ai onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        nova-ai onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Nova AI currently ships this bundled Qwen catalog. The configured catalog is
...
For reasoning-enabled Qwen Cloud models, the bundled provider maps Nova AI
...
<AccordionGroup>
  ...
  <Accordion title="Qwen 3.6 Plus availability">
    ...
    Nova AI's bundled Qwen catalog does not advertise `qwen3.6-plus` on Coding
  ...
  <Accordion title="Video generation details">
    For video generation, Nova AI maps the configured Qwen region to the matching
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. Nova AI keys that off endpoint
  ...
  <Accordion title="Environment and daemon setup">
    ...
    available to that process (for example, in `~/.nova-ai/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
