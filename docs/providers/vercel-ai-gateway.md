...
read_when:
  - You want to use Vercel AI Gateway with Nova AI
...
<Tip>
Nova AI auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    nova-ai onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    nova-ai models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Nova AI accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Nova AI resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Nova AI Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.nova-ai/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow trusted upstream model prefixes when Nova AI knows
