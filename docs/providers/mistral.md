---
summary: "Use Mistral models and Voxtral transcription with Nova AI"
read_when:
  - You want to use Mistral models in Nova AI
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Nova AI includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nova-ai onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    nova-ai onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    nova-ai models list --provider mistral
    ```
  </Step>
</Steps>
...
Nova AI currently ships this bundled Mistral catalog:
...
```bash
nova-ai models list --all --provider mistral --plain
```
...
<Note>
Nova AI defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    Nova AI maps the session **thinking** level to Mistral's API:

    | Nova AI thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so Nova AI sends
