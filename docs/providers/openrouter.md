---
summary: "Use OpenRouter's unified API to access many models in Nova AI"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Nova AI
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nova-ai onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    nova-ai models set openrouter/<provider>/<model>
    ```
...
Nova AI submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. Nova AI sends `modalities: ["text",
...
Nova AI sends OpenRouter STT requests as JSON with base64 audio under
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), Nova AI also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------ |
| `HTTP-Referer`            | `https://nova-ai.com`                                                                                  |
| `X-OpenRouter-Title`      | `Nova AI`                                                                                             |
| `X-OpenRouter-Categories` | `cli-agent,cloud-agent,programming-app,creative-writing,writing-assistant,general-chat,personal-agent` |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Nova AI
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Nova AI sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Nova AI uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Nova AI maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. Nova AI sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Nova AI keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, Nova AI forwards
