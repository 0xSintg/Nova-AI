---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in Nova AI"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in Nova AI
title: "DeepInfra"
---
...
```bash
nova-ai onboard --deepinfra-api-key <key>
```
...
## Supported Nova AI surfaces

The bundled plugin registers all DeepInfra surfaces that match current
Nova AI provider contracts:

| Surface                  | Default model                      | Nova AI config/tool                                     |
| ------------------------ | ---------------------------------- | -------------------------------------------------------- |
...
native model types. Nova AI does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

Nova AI dynamically discovers available DeepInfra models at startup. Use
