...
read_when:
  - You want to use Volcano Engine or Doubao models with Nova AI
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    nova-ai onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    nova-ai models list --provider volcengine
    nova-ai models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `nova-ai.json`:
...
For voice-note targets, Nova AI asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `nova-ai onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Nova AI falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.nova-ai/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Nova AI as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Nova AI setup.
  </Card>
</CardGroup>
