---
summary: "Use Synthetic's Anthropic-compatible API in Nova AI"
read_when:
...
Nova AI registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nova-ai onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Nova AI's Anthropic client appends `/v1` to the base URL automatically, so use
...
`nova-ai models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Nova AI appends `/v1` automatically.
