---
summary: "Use OpenAI via API keys or Codex subscription in Nova AI"
read_when:
  - You want to use OpenAI models in Nova AI
...
ChatGPT-plan coding agent through OpenAI's Codex clients. Nova AI keeps those
surfaces separate so config stays predictable.

Nova AI uses `openai/*` as the canonical OpenAI model route. Embedded agent
...
- **Legacy config** - `openai-codex/*` model refs are repaired by
  `nova-ai doctor --fix` to `openai/*` plus the Codex runtime.

OpenAI explicitly supports subscription OAuth usage in external tools and workflows like Nova AI.
...
| `codex` plugin                          | Plugin                     | Bundled Nova AI plugin that provides native Codex app-server runtime and `/codex` chat controls.                    |
...
remain supported. `nova-ai doctor --fix` rewrites legacy `openai-codex/*` model
...
explicitly selected with an `openai-codex` auth profile, Nova AI keeps the
public model ref as `openai/*` and routes PI internally through the legacy
Codex-auth transport. Run `nova-ai doctor --fix` to repair stale
`openai-codex/*`, `codex-cli/*`, or old PI session pins that do not come from
explicit runtime config.
</Note>

## Nova AI feature coverage

| OpenAI capability         | Nova AI surface                                                                 | Status                                                 |
| ------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------ |
...
Nova AI can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. Nova AI forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        nova-ai onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        nova-ai onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        nova-ai models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    Nova AI normalizes incompatible OpenAI text-verbosity overrides for this
    model.

    <Warning>
    Nova AI does **not** expose `openai/gpt-5.3-codex-spark`. Live OpenAI API requests reject that model, and the current Codex catalog does not expose it either.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex app-server execution instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        nova-ai onboard --auth-choice openai-codex
        ```
        ...
        ```bash
        nova-ai models auth login --provider openai-codex --device-code
        ```
      </Step>
      <Step title="Use the canonical OpenAI model route">
        ```bash
        nova-ai config set agents.defaults.model.primary openai/gpt-5.5
        ```

        No runtime config is required for the default path. OpenAI agent turns
        select the native Codex app-server runtime automatically, and Nova AI
        installs or repairs the bundled Codex plugin when this route is chosen.
      </Step>
      <Step title="Verify Codex auth is available">
        ```bash
        nova-ai models list --provider openai-codex
        ```
    ...
    path; run `nova-ai doctor --fix` when you want to migrate them to canonical
    `openai/*` refs.
    </Warning>
    ...
    auth order under `openai`. Nova AI will try the subscription first, then
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — Nova AI manages the resulting credentials in its own agent auth store.
    </Note>
    ...
    nova-ai models auth list --provider openai-codex
    nova-ai config get agents.defaults.model --json
    nova-ai config get models.providers.openai.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    nova-ai models status --agent <id>
    nova-ai models auth list --agent <id> --provider openai-codex
    ```
    ...
    ```bash
    nova-ai models auth login --provider openai-codex
    nova-ai models status --probe --probe-provider openai-codex
    ```
    ...
    session state, `nova-ai doctor --fix` rewrites them to `openai/*` with the
    Codex runtime unless PI is explicitly configured.

    ### Context window cap

    Nova AI treats model metadata and the runtime context cap as separate values.
    ...
    Nova AI uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while
    the account is authenticated, Nova AI synthesizes that OAuth model row so
...
still account-based. Nova AI selects auth in this order:
...
profile is selected, Nova AI also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
Codex usage limit, Nova AI can rotate to the next ordered `openai:*` API-key
...
still accepted. Nova AI also protects the public OpenAI and
...
`nova-ai infer image edit` when starting from an input file.
...
`openai-codex` OAuth profile is configured, Nova AI resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; Nova AI keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for Nova AI on macOS" size=3840x2160 count=1
```
...
Nova AI adds a shared GPT-5 prompt contribution for GPT-5-family runs on Nova AI-assembled prompt surfaces. It applies by model id, so PI/provider routes such as legacy pre-repair refs (`openai-codex/gpt-5.5`), `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.

The bundled native Codex harness does not receive this Nova AI GPT-5 overlay through Codex app-server developer instructions. Native Codex keeps Codex-owned base, model, personality, and project-doc behavior; Nova AI contributes only runtime context such as channel delivery, Nova AI dynamic tools, ACP delegation, workspace context, and Nova AI skills.

The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification on matching Nova AI-assembled prompts. Channel-specific reply and silent-message behavior stays in the shared Nova AI system prompt and outbound delivery policy. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    nova-ai config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after Nova AI's generated fields, so use it for OpenAI-compatible endpoints that require additional keys such as `lang`. Prototype keys are ignored.
  ...
  <Accordion title="Speech-to-text">
    ...
    Nova AI's media-understanding transcription surface.

    - Default model: `gpt-4o-transcribe`
    - Endpoint: OpenAI REST `/v1/audio/transcriptions`
    - Input path: multipart audio file upload
    - Supported by Nova AI wherever inbound audio transcription uses
  ...
  <Accordion title="Realtime voice">
    ...
    model has emitted audio in that session. Nova AI currently exposes the
...
generation by overriding the base URL. On the image-generation path, Nova AI
...
Nova AI recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, Nova AI:
...
Nova AI 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in Nova AI
...
Nova AI. If an Azure request fails with a validation error, check the
...
Nova AI's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    Nova AI uses WebSocket-first with SSE fallback (`"auto"`) for `openai/*`.

    In `"auto"` mode, Nova AI:
  ...
  <Accordion title="Fast mode">
    Nova AI exposes a shared fast-mode toggle for `openai/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, Nova AI maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in Nova AI:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, Nova AI leaves `service_tier` untouched.
    </Warning>
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, Nova AI can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    Nova AI treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
