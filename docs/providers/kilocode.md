---
summary: "Use Kilo Gateway's unified API to access many models in Nova AI"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Nova AI
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nova-ai onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    nova-ai models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Nova AI treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Nova AI.
</Note>

## Built-in catalog

Nova AI dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Nova AI queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Nova AI keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Nova AI falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.nova-ai/.env` or via `env.shellEnv`).
