...
read_when:
  - You need a beginner-friendly overview of Nova AI logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Nova AI has two main log surfaces:
...
Nova AI keeps up to five numbered archives beside the active file, such as
`nova-ai-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.nova-ai/nova-ai.json`:

```json
{
  "logging": {
    "file": "/path/to/nova-ai.log"
  }
}
```
...
or times out before `logs.tail` answers, `nova-ai logs` falls back to the
...
```bash
nova-ai channels logs --channel whatsapp
```
...
Nova AI preserves the original structured log arguments alongside these fields
...
`nova-ai gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
nova-ai gateway
nova-ai gateway --verbose --ws-log compact
nova-ai gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.nova-ai/nova-ai.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/nova-ai/nova-ai-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`NOVA_AI_LOG_LEVEL`** environment variable (e.g. `NOVA_AI_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `nova-ai.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `nova-ai --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
NOVA_AI_DEBUG_MODEL_TRANSPORT=1 nova-ai gateway
NOVA_AI_DEBUG_MODEL_PAYLOAD=tools NOVA_AI_DEBUG_SSE=events nova-ai gateway
```

Available flags:

- `NOVA_AI_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `NOVA_AI_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `NOVA_AI_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `NOVA_AI_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `NOVA_AI_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `NOVA_AI_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `NOVA_AI_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal Nova AI logging, so `nova-ai logs --follow`
...
Nova AI writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
Nova AI can redact sensitive tokens before they hit console output, file logs,
...
policy. Nova AI still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `NOVA_AI_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `nova-ai doctor` first.
