---
summary: "CLI reference for `nova-ai secrets` (reload, audit, configure, apply)"
read_when:
...
Use `nova-ai secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
nova-ai secrets reload
nova-ai secrets reload --json
nova-ai secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Nova AI state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `nova-ai.json` refs)
...
```bash
nova-ai secrets audit
nova-ai secrets audit --check
nova-ai secrets audit --json
nova-ai secrets audit --allow-exec
```
...
```bash
nova-ai secrets configure
nova-ai secrets configure --plan-out /tmp/nova-ai-secrets-plan.json
nova-ai secrets configure --apply --yes
nova-ai secrets configure --providers-only
nova-ai secrets configure --skip-provider-setup
nova-ai secrets configure --agent ops
nova-ai secrets configure --json
```
...
- `configure` targets secret-bearing fields in `nova-ai.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Nova AI fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
nova-ai secrets apply --from /tmp/nova-ai-secrets-plan.json --json
```
...
- `nova-ai.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.nova-ai/.env` known secret keys whose values were migrated
