---
summary: "CLI reference for `nova-ai webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into Nova AI
...
```bash
nova-ai webhooks gmail setup --account <email> [...]
nova-ai webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the Nova AI webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and Nova AI webhook delivery.

```bash
nova-ai webhooks gmail setup --account you@example.com
nova-ai webhooks gmail setup --account you@example.com --project my-gcp-project --json
nova-ai webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### Nova AI delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | Nova AI webhook URL.                      |
| `--hook-token <token>` | (none)  | Nova AI webhook token.                    |
...
```bash
nova-ai webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, Nova AI delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| Nova AI delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
