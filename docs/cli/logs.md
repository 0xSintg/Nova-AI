---
summary: "CLI reference for `nova-ai logs` (tail gateway logs via RPC)"
read_when:
...
`nova-ai logs` also accepts the standard Gateway client flags:
...
nova-ai logs
nova-ai logs --follow
nova-ai logs --follow --interval 2000
nova-ai logs --limit 500 --max-bytes 500000
nova-ai logs --json
nova-ai logs --plain
nova-ai logs --no-color
nova-ai logs --limit 500
nova-ai logs --local-time
nova-ai logs --follow --local-time
nova-ai logs --url ws://127.0.0.1:18789 --token "$NOVA_AI_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `nova-ai logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
