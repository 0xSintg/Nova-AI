---
summary: "CLI reference for `nova-ai configure` (interactive configuration prompts)"
read_when:
...
Use `nova-ai onboard` for the full guided first-run journey, `nova-ai setup` for the baseline config/workspace only, and `nova-ai channels add` when you only need channel account setup.
...
<Tip>
`nova-ai config` without a subcommand opens the same wizard. Use `nova-ai config get|set|unset` for non-interactive edits.
</Tip>

For web search, `nova-ai configure --section web` lets you choose a provider
...
nova-ai configure --section model --section channels
nova-ai configure --section gateway --section daemon
```
