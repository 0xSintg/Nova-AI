---
summary: "CLI reference for `nova-ai memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
nova-ai memory status
nova-ai memory status --deep
nova-ai memory status --fix
nova-ai memory index --force
nova-ai memory search "meeting notes"
nova-ai memory search --query "deployment" --max-results 20
nova-ai memory promote --limit 10 --min-score 0.75
nova-ai memory promote --apply
nova-ai memory promote --json --min-recall-count 0 --min-unique-queries 0
nova-ai memory promote-explain "router vlan"
nova-ai memory promote-explain "router vlan" --json
nova-ai memory rem-harness
nova-ai memory rem-harness --json
nova-ai memory status --json
nova-ai memory status --deep --index
nova-ai memory status --deep --index --verbose
nova-ai memory status --agent main
nova-ai memory index --agent main --verbose
```
...
```bash
nova-ai memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `nova-ai cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
nova-ai memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
nova-ai memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
