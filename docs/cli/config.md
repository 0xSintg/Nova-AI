---
summary: "CLI reference for `nova-ai config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `nova-ai.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `nova-ai configure`).

<Note>
When `NOVA_AI_NIX_MODE=1`, Nova AI treats `nova-ai.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-nova-ai distribution, use [nix-nova-ai Quick Start](https://github.com/nova-ai/nix-nova-ai#quick-start) and set values under `programs.nova-ai.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `nova-ai config` without a subcommand.
</ParamField>
...
```bash
nova-ai config file
nova-ai config --section model
nova-ai config --section gateway --section daemon
nova-ai config schema
nova-ai config get browser.executablePath
...
nova-ai config set agents.defaults.heartbeat.every "2h"
nova-ai config set agents.list[0].tools.exec.node "node-id-or-name"
nova-ai config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
nova-ai config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
nova-ai config set secrets.providers.vaultfile --provider-source file --provider-path /etc/nova-ai/secrets.json --provider-mode json
nova-ai config patch --file ./nova-ai.patch.json5 --dry-run
nova-ai config unset plugins.entries.brave.config.webSearch.apiKey
nova-ai config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
nova-ai config validate
nova-ai config validate --json
```

### `config schema`

Print the generated JSON schema for `nova-ai.json` to stdout as JSON.
...
```bash
nova-ai config schema > nova-ai.schema.json
```
...
```bash
nova-ai config get agents.defaults.workspace
nova-ai config get agents.list[0].id
```

Use the agent list index to target a specific agent:

```bash
nova-ai config get agents.list
nova-ai config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
nova-ai config set agents.defaults.heartbeat.every "0m"
nova-ai config set gateway.port 19001 --strict-json
nova-ai config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
nova-ai config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
nova-ai config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`nova-ai config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    nova-ai config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    nova-ai config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    nova-ai config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/nova-ai-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ...
    ```bash
    nova-ai config set --batch-file ./config-set.batch.json --dry-run
    ```
...
```bash
nova-ai config patch --file ./nova-ai.patch.json5 --dry-run
nova-ai config patch --file ./nova-ai.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh nova-ai-host 'nova-ai config patch --stdin --dry-run' < ./nova-ai.patch.json5
ssh nova-ai-host 'nova-ai config patch --stdin' < ./nova-ai.patch.json5
```
...
```bash
nova-ai config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
nova-ai config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/nova-ai/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `nova-ai.json`.
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "~/.nova-ai/nova-ai.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
`nova-ai config set` and other Nova AI-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `nova-ai.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `nova-ai.json` layouts are unsupported for writes; use `NOVA_AI_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
nova-ai config set gateway.reload.mode hybrid --dry-run
nova-ai config set gateway.reload.mode hybrid
nova-ai config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `nova-ai.json`. Run `nova-ai doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `NOVA_AI_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `nova-ai config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `nova-ai configure` or `nova-ai doctor --fix`. `nova-ai chat` does not bypass the invalid-config guard.
</Note>
...
```text
!nova-ai config file
!nova-ai docs gateway auth token secretref
!nova-ai config validate
!nova-ai doctor
```
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `nova-ai config set` or `nova-ai configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `nova-ai config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `nova-ai doctor` or `nova-ai doctor --fix` for migration and repair help.
  </Step>
</Steps>
