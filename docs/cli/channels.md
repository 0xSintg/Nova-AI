---
summary: "CLI reference for `nova-ai channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `nova-ai sessions`, Gateway `sessions.list`, or the agent
...
```bash
nova-ai channels add --channel telegram --token <bot-token>
nova-ai channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
nova-ai channels remove --channel telegram --delete
```

<Tip>
`nova-ai channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Nova AI uses the channel's default install source without opening the interactive plugin install prompt.

When you run `nova-ai channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `nova-ai agents bindings`, `nova-ai agents bind`, and `nova-ai agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Nova AI promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `nova-ai status --deep` for a broad probe.
- Use `nova-ai doctor` for guided fixes.
- `nova-ai channels list` no longer prints model provider usage/quota snapshots. For those, use `nova-ai status` (overview) or `nova-ai models list` (per-provider).
- `nova-ai channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
nova-ai channels resolve --channel slack "#general" "@jane"
nova-ai channels resolve --channel discord "My Server/#support" "@someone"
nova-ai channels resolve --channel matrix "Project Room"
```
