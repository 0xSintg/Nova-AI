---
summary: "Nova AI Gateway CLI (`nova-ai gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Nova AI's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `nova-ai gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How Nova AI advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.nova-ai/nova-ai.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `nova-ai onboard --mode local` and `nova-ai setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `NOVA_AI_GATEWAY_TOKEN` for the process).
</ParamField>
...
nova-ai gateway restart --safe
nova-ai gateway restart --safe --skip-deferral
nova-ai gateway restart --force
```

`nova-ai gateway restart --safe` asks the running Gateway to preflight active Nova AI work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`nova-ai gateway restart --safe --skip-deferral` runs the same Nova AI-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `NOVA_AI_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `NOVA_AI_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `NOVA_AI_DIAGNOSTICS=timeline` with `NOVA_AI_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `NOVA_AI_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
nova-ai gateway health --url ws://127.0.0.1:18789
```
...
```bash
nova-ai gateway usage-cost
nova-ai gateway usage-cost --days 7
nova-ai gateway usage-cost --json
```
...
```bash
nova-ai gateway stability
nova-ai gateway stability --type payload.large
nova-ai gateway stability --bundle latest
nova-ai gateway stability --bundle latest --export
nova-ai gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, Nova AI writes the same diagnostic snapshot to `~/.nova-ai/logs/stability/nova-ai-stability-*.json` when the recorder has events. Inspect the newest bundle with `nova-ai gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
nova-ai gateway diagnostics export --output nova-ai-diagnostics.zip
nova-ai gateway diagnostics export --json
```
...
nova-ai gateway status --require-rpc
```
...
```bash
nova-ai gateway call status
nova-ai gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `nova-ai` or Node with those args.

```bash
cat > ~/.local/bin/nova-ai-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- nova-ai "$@"
EOF
chmod +x ~/.local/bin/nova-ai-doppler

nova-ai gateway install --wrapper ~/.local/bin/nova-ai-doppler --force
nova-ai gateway restart
```
...
`NOVA_AI_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
NOVA_AI_WRAPPER="$HOME/.local/bin/nova-ai-doppler" nova-ai gateway install --force
nova-ai doctor
```

To remove a persisted wrapper, clear `NOVA_AI_WRAPPER` while reinstalling:

```bash
NOVA_AI_WRAPPER= nova-ai gateway install --force
nova-ai gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active Nova AI work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the Nova AI-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `NOVA_AI_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `NOVA_AI_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_nova-ai-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `nova-ai.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
nova-ai gateway discover --timeout 4000
nova-ai gateway discover --json | jq '.beacons[].wsUrl'
```
