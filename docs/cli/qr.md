---
summary: "CLI reference for `nova-ai qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
nova-ai qr
nova-ai qr --setup-code-only
nova-ai qr --json
nova-ai qr --remote
nova-ai qr --url wss://gateway.example/ws
```
...
- With `--remote`, Nova AI requires either `gateway.remote.url` or
