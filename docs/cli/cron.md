---
summary: "CLI reference for `nova-ai cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `nova-ai cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`nova-ai cron list` and `nova-ai cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
If an isolated run times out before the first model request, `nova-ai cron show`
and `nova-ai cron runs` include a phase-specific error such as
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `nova-ai cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
`nova-ai cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
nova-ai cron run <job-id>
nova-ai cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
nova-ai cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, Nova AI passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.

`nova-ai doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
nova-ai cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
nova-ai cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
nova-ai cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
nova-ai cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
nova-ai cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
nova-ai cron list
nova-ai cron list --agent ops
nova-ai cron get <job-id>
nova-ai cron show <job-id>
nova-ai cron run <job-id>
nova-ai cron run <job-id> --due
nova-ai cron run <job-id> --wait --wait-timeout 10m
nova-ai cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
nova-ai cron runs --id <job-id> --limit 50
nova-ai cron runs --id <job-id> --run-id <run-id>
```

`nova-ai cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`nova-ai cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
nova-ai cron edit <job-id> --agent ops
nova-ai cron edit <job-id> --clear-agent
nova-ai cron edit <job-id> --session current
nova-ai cron edit <job-id> --session "session:daily-brief"
```

`nova-ai cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
nova-ai cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
nova-ai cron edit <job-id> --best-effort-deliver
nova-ai cron edit <job-id> --no-best-effort-deliver
nova-ai cron edit <job-id> --no-deliver
```
