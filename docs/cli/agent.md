---
summary: "CLI reference for `nova-ai agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
nova-ai agent --to +15555550123 --message "status update" --deliver
nova-ai agent --agent ops --message "Summarize logs"
nova-ai agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
nova-ai agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Nova AI persists markers from the active source config snapshot, not from resolved runtime secret values.
