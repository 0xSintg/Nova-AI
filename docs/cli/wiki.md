---
summary: "CLI reference for `nova-ai wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `nova-ai wiki`
title: "Wiki"
---
...
Use `nova-ai wiki` when you want a compiled knowledge vault with:
...
nova-ai wiki doctor
nova-ai wiki init
nova-ai wiki ingest ./notes/alpha.md
nova-ai wiki compile
nova-ai wiki lint
nova-ai wiki search "alpha"
nova-ai wiki search "who should I ask about Teams?" --mode route-question
nova-ai wiki get entity.alpha --from 1 --lines 80

nova-ai wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

nova-ai wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

nova-ai wiki bridge import
nova-ai wiki unsafe-local import

nova-ai wiki obsidian status
nova-ai wiki obsidian search "alpha"
nova-ai wiki obsidian open syntheses/alpha-summary.md
nova-ai wiki obsidian command workspace:quick-switcher
nova-ai wiki obsidian daily
```
...
- `.nova-ai-wiki/cache/agent-digest.json`
- `.nova-ai-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `nova-ai memory search` when the
...
```bash
nova-ai wiki search "bgroux" --mode find-person
nova-ai wiki search "who knows Teams rollout?" --mode route-question
nova-ai wiki search "maintainer-whois" --mode source-evidence
nova-ai wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
nova-ai wiki get entity.alpha
nova-ai wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`nova-ai wiki` behavior is shaped by:
