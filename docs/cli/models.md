---
summary: "CLI reference for `nova-ai models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
nova-ai models list
nova-ai models set <model-or-alias>
nova-ai models scan
```

`nova-ai models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Nova AI falls back to matching OAuth/API-key
...
the command uses `NOVA_AI_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For Codex OAuth troubleshooting, `nova-ai models status`,
`nova-ai models auth list --provider openai-codex`, and
`nova-ai config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, Nova AI resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Nova AI
...
By default Nova AI tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `NOVA_AI_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Nova AI could not resolve a probeable
...
```bash
nova-ai models auth add
nova-ai models auth list [--provider <id>] [--json]
nova-ai models auth login --provider <id>
nova-ai models auth setup-token --provider <id>
nova-ai models auth paste-token
```
...
`nova-ai plugins list` to see which providers are installed.
Use `nova-ai models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
nova-ai models auth login --provider openai --set-default
nova-ai models auth login --provider openai --method api-key
nova-ai models auth list --provider openai
```
...
- Anthropic note: Anthropic staff told us Nova AI-style Claude CLI usage is allowed again, so Nova AI treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Nova AI token path, but Nova AI now prefers Claude CLI reuse and `claude -p` when available.
