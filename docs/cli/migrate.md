---
summary: "CLI reference for `nova-ai migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Nova AI
...
```bash
nova-ai migrate list
nova-ai migrate claude --dry-run
nova-ai migrate codex --dry-run
nova-ai migrate codex --skill gog-vault77-google-workspace
nova-ai migrate codex --plugin google-calendar --dry-run
nova-ai migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
nova-ai migrate hermes --dry-run
nova-ai migrate hermes
nova-ai migrate apply codex --yes --skill gog-vault77-google-workspace
nova-ai migrate apply codex --yes --plugin google-calendar
nova-ai migrate apply codex --yes
nova-ai migrate apply claude --yes
nova-ai migrate apply hermes --yes
nova-ai migrate apply hermes --include-secrets --yes
nova-ai onboard --flow import
nova-ai onboard --import-from claude --import-source ~/.claude
nova-ai onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `nova-ai migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Nova AI state exists.
</ParamField>
...
`nova-ai migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `nova-ai migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Nova AI backup before applying the migration. If no local Nova AI state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Nova AI agent workspace.
...
- Claude command Markdown files converted into Nova AI skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Nova AI does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Nova AI Codex harness and you want to
...
Running `nova-ai migrate codex` in an interactive terminal previews the full
...
start checked unless the target Nova AI Codex plugin config already has that
...
```bash
nova-ai migrate codex --dry-run --skill gog-vault77-google-workspace
nova-ai migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
nova-ai migrate codex --dry-run --plugin google-calendar
nova-ai migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Nova AI agent workspace when you want per-agent ownership.
...
- `SOUL.md` and `AGENTS.md` into the Nova AI agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Nova AI file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that Nova AI cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Nova AI config or credentials. This preserves opaque or unsafe state without pretending Nova AI can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `nova-ai.plugin.json`:
...
Provider plugins can use `nova-ai/plugin-sdk/migration` for item construction and summary counts, plus `nova-ai/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `nova-ai onboard --flow import` and `nova-ai setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Nova AI setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Nova AI to a new machine.
