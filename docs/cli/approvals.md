---
summary: "CLI reference for `nova-ai approvals` and `nova-ai exec-policy`"
read_when:
...
`nova-ai exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.nova-ai/exec-approvals.json`

Examples:

```bash
nova-ai exec-policy show
nova-ai exec-policy show --json

nova-ai exec-policy preset yolo
nova-ai exec-policy preset cautious --json

nova-ai exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `nova-ai exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `nova-ai approvals set --gateway`
or `nova-ai approvals set --node <id|name|ip>`.

## Common commands

```bash
nova-ai approvals get
nova-ai approvals get --node <id|name|ip>
nova-ai approvals get --gateway
```

`nova-ai approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
nova-ai approvals set --file ./exec-approvals.json
nova-ai approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
nova-ai approvals set --node <id|name|ip> --file ./exec-approvals.json
nova-ai approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Nova AI policy aligned, also set:
...
```bash
nova-ai approvals allowlist add "~/Projects/**/bin/rg"
nova-ai approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
nova-ai approvals allowlist add --agent "*" "/usr/bin/uname"

nova-ai approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `nova-ai nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.nova-ai/exec-approvals.json`.
