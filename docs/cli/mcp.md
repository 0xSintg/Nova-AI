---
summary: "Expose Nova AI channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Nova AI-backed channels
  - Running `nova-ai mcp serve`
  - Managing Nova AI-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`nova-ai mcp` has two jobs:

- run Nova AI as an MCP server with `nova-ai mcp serve`
- manage Nova AI-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is Nova AI acting as an MCP server
- `list` / `show` / `set` / `unset` is Nova AI acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`nova-ai acp`](/cli/acp) when Nova AI should host a coding harness session itself and route that runtime through ACP.

## Nova AI as an MCP server

This is the `nova-ai mcp serve` path.

### When to use `serve`

Use `nova-ai mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Nova AI-backed channel conversations
- you already have a local or remote Nova AI Gateway with routed sessions
- you want one MCP server that works across Nova AI's channel backends instead of running separate per-channel bridges

Use [`nova-ai acp`](/cli/acp) instead when Nova AI should host the coding runtime itself and keep the agent session inside Nova AI.

### How it works

`nova-ai mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Nova AI Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `nova-ai mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Nova AI Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `nova-ai agent` and `nova-ai infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Nova AI (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Nova AI already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    nova-ai mcp serve --url wss://gateway-host:18789 --token-file ~/.nova-ai/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    nova-ai mcp serve --url wss://gateway-host:18789 --password-file ~/.nova-ai/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    nova-ai mcp serve --verbose
    nova-ai mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Nova AI equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Nova AI channel configuration
...
Nova AI ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `nova-ai mcp serve`
...
## Nova AI as an MCP client registry

This is the `nova-ai mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Nova AI over MCP. They manage Nova AI-owned MCP server definitions under `mcp.servers` in Nova AI config.

Those saved definitions are for runtimes that Nova AI launches or configures later, such as embedded Pi and other runtime adapters. Nova AI stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Nova AI config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes Nova AI `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
Nova AI projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific Nova AI
...
Nova AI strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

Nova AI also stores a lightweight MCP server registry in config for surfaces that want Nova AI-managed MCP definitions.

Commands:

- `nova-ai mcp list`
- `nova-ai mcp show [name]`
- `nova-ai mcp set <name> <json>`
- `nova-ai mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `nova-ai mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
nova-ai mcp list
nova-ai mcp show context7 --json
nova-ai mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
nova-ai mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
nova-ai mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Nova AI uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |

Nova AI config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `nova-ai mcp set` and repaired by `nova-ai doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
