---
summary: "Nova AI CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `nova-ai` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`nova-ai` is the main CLI entry point. Each core command has either a
...
- `nova-ai setup` creates the baseline config and workspace without walking the full guided onboarding flow.
- `nova-ai onboard` is the full guided first-run path for gateway, model auth, workspace, channels, skills, and health.
- `nova-ai configure` changes targeted parts of an existing setup, such as model auth, gateway, channels, plugins, or skills.
- `nova-ai channels add` configures channel accounts after the baseline exists; run it without flags for guided channel setup or with channel-specific flags for scripts.
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.nova-ai-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.nova-ai-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`nova-ai update`](/cli/update) (source installs only) |
...
```
nova-ai [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
Plugins can add additional top-level commands (for example `nova-ai voicecall`).
...
`nova-ai status --usage` and the Control UI surface provider usage/quota when
