---
summary: "CLI reference for `nova-ai tui` (Gateway-backed or local embedded terminal UI)"
read_when:
  - You want a terminal UI for the Gateway (remote-friendly)
  - You want to pass url/token/session from scripts
  - You want to run the TUI in local embedded mode without a Gateway
  - You want to use nova-ai chat or nova-ai tui --local
title: "TUI"
---
...
Aliases: `nova-ai chat` and `nova-ai terminal` invoke the same command with `--local` implied.

Notes:

- `chat` and `terminal` are aliases for `nova-ai tui --local`.
...
nova-ai tui --local
nova-ai tui
nova-ai tui --url ws://127.0.0.1:18789 --token <token>
nova-ai tui --session main --deliver
nova-ai chat --message "Compare my config to the docs and tell me what to fix"
# when run inside an agent workspace, infers that agent automatically
nova-ai tui --session bugfix
```
...
If `nova-ai config validate` is already failing, use `nova-ai configure` or
`nova-ai doctor --fix` first. `nova-ai chat` does not bypass the invalid-
config guard.
...
Apply targeted fixes with `nova-ai config set` or `nova-ai configure`, then
rerun `nova-ai config validate`. See [TUI](/web/tui) and [Config](/cli/config).
