---
summary: "CLI reference for `nova-ai doctor` (health checks + guided repairs)"
read_when:
...
`nova-ai doctor` is the Nova AI health surface. Use it when the gateway,
...
| ------- | ------------------------ | ------------------------------------------------------------------------------- |
| Inspect | `nova-ai doctor`        | Human-oriented checks and guided prompts.                                       |
| Repair  | `nova-ai doctor --fix`  | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint    | `nova-ai doctor --lint` | Read-only structured findings for CI, preflight, and review gates.              |
...
nova-ai doctor --deep
nova-ai doctor --fix
nova-ai doctor --fix --non-interactive
nova-ai doctor --generate-gateway-token
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
nova-ai channels capabilities --channel discord --target channel:<channel-id>
...
`nova-ai doctor --lint` is the read-only automation posture for doctor checks.
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `nova-ai configure` and set Gateway mode (local/remote), or `nova-ai config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `nova-ai configure` and set Gateway mode (local/remote), or `nova-ai config set gateway.mode local`."
    }
  ]
}
```
...
example, `nova-ai doctor --lint --severity-min error` can print no findings and
...
health path. The `nova-ai/plugin-sdk/health` subpath exposes the same
...
```bash
nova-ai doctor --lint --only core/doctor/gateway-config --json
nova-ai doctor --lint --skip core/doctor/skills-readiness
```
...
- In Nix mode (`NOVA_AI_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `nova-ai.json` is immutable. Edit the Nix source for this install instead; for nix-nova-ai, use the agent-first [Quick Start](https://github.com/nova-ai/nix-nova-ai#quick-start).
...
- `--fix` (alias for `--repair`) writes a backup to `~/.nova-ai/nova-ai.json.bak` and drops unknown config keys, listing each removal.
...
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `nova-ai gateway install` for a missing service, or `nova-ai gateway install --force` when you intentionally want to replace the launcher.
...
- Doctor also scans `~/.nova-ai/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and can rewrite them in place before the scheduler has to auto-normalize them at runtime.
...
- On Linux, doctor warns when the user's crontab still runs legacy `~/.nova-ai/bin/ensure-whatsapp.sh`; that script is no longer maintained and can log false WhatsApp gateway outages when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `nova-ai-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state created by older Nova AI versions and relinks the host `nova-ai` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins that are referenced by config, such as `plugins.entries`, configured channels, configured provider/search settings, or configured agent runtimes. During package updates, doctor skips package-manager plugin repair until the package swap is complete; rerun `nova-ai doctor --fix` afterward if a configured plugin still needs recovery. If the download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Set `NOVA_AI_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first or use `nova-ai gateway install --force` when you intentionally want to replace the active launcher.
...
- Doctor includes a memory-search readiness check and can recommend `nova-ai configure --section model` when embedding credentials are missing.
...
- Doctor warns when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes, so use `nova-ai migrate codex --dry-run` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `nova-ai config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files (`~/.nova-ai/sandbox/containers.json` or `~/.nova-ai/sandbox/browsers.json`) are present, doctor reports them; `nova-ai doctor --fix` migrates valid entries into sharded registry directories and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv NOVA_AI_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv NOVA_AI_GATEWAY_TOKEN
launchctl getenv NOVA_AI_GATEWAY_PASSWORD

launchctl unsetenv NOVA_AI_GATEWAY_TOKEN
launchctl unsetenv NOVA_AI_GATEWAY_PASSWORD
```
