---
summary: "CLI reference for `nova-ai onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want Nova AI to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Nova AI onboarding fits together.
  </Card>
...
nova-ai onboard --flow quickstart
nova-ai onboard --flow manual
nova-ai onboard --flow import
nova-ai onboard --import-from hermes --import-source ~/.hermes
nova-ai onboard --skip-bootstrap
nova-ai onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh Nova AI setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `nova-ai onboard` keeps the classic onboarding flow.
...
`NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
NOVA_AI_LOCALE=zh-CN nova-ai onboard
```
...
Nova AI marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export NOVA_AI_GATEWAY_TOKEN="your-token"
nova-ai onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env NOVA_AI_GATEWAY_TOKEN \
  --accept-risk
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `nova-ai gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `nova-ai dashboard` (Control UI, no channel setup).
...
nova-ai configure
nova-ai agents add <name>
```

Use `nova-ai setup` instead when you only need the baseline config/workspace. Use `nova-ai configure` later for targeted changes and `nova-ai channels add` for channel-only setup.
