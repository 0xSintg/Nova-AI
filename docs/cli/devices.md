---
summary: "CLI reference for `nova-ai devices` (device pairing + token rotation/revocation)"
read_when:
...
### `nova-ai devices remove <deviceId>`
...
```
nova-ai devices remove <deviceId>
nova-ai devices remove <deviceId> --json
```

### `nova-ai devices clear --yes [--pending]`

Clear paired devices in bulk.

```
nova-ai devices clear --yes
nova-ai devices clear --yes --pending
nova-ai devices clear --yes --pending --json
```

### `nova-ai devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Nova AI only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Nova AI supersedes the previous pending entry and issues a new `requestId`. Run `nova-ai devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
Nova AI keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `nova-ai devices list`
or use `nova-ai devices approve --latest` to preview the exact upgrade before
approving it.
...
```
nova-ai devices approve
nova-ai devices approve <requestId>
nova-ai devices approve --latest
```
...
### `nova-ai devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
nova-ai devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `nova-ai devices revoke --device <id> --role <role>`
...
```
nova-ai devices revoke --device <deviceId> --role node
```
...
```bash
nova-ai devices rotate --device <deviceId> --role operator
```
