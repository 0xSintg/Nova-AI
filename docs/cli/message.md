---
summary: "CLI reference for `nova-ai message` (send + channel actions)"
read_when:
...
```
nova-ai message <subcommand> [flags]
```
...
- `nova-ai message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, Nova AI will attempt a live directory lookup when the provider supports it.
...
- `nova-ai message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Nova AI does not force a `default` account SecretRef scope
...
```
nova-ai message send --channel discord \
...
```bash
nova-ai message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
nova-ai message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
nova-ai message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
nova-ai message poll --channel msteams \
...
```
nova-ai message react --channel slack \
...
```
nova-ai message react --channel signal \
...
```
nova-ai message send --channel telegram --target @mychat --message "Choose:" \
...
```
nova-ai message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
nova-ai message send --channel telegram --target @mychat \
