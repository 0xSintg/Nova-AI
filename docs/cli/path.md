---
summary: "CLI reference for `nova-ai path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
nova-ai plugins enable oc-path
```

## Why use it

Nova AI state is spread across human-edited markdown, commented JSONC config,
...
`nova-ai path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `nova-ai path` for ordinary whole-file edits, rich
...
```bash
nova-ai path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
nova-ai path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
nova-ai path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
nova-ai path resolve 'oc://AGENTS.md/runtime-safety/nova-ai-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
nova-ai path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
nova-ai path resolve 'oc://gateway.jsonc/version'

# Wildcard search
nova-ai path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
nova-ai path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
nova-ai path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
nova-ai path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
nova-ai path emit ./AGENTS.md
```
...
nova-ai path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Predicate search over JSONC children
nova-ai path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
nova-ai path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
nova-ai path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
nova-ai path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
nova-ai path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
nova-ai path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
nova-ai path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
nova-ai path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
nova-ai path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
nova-ai path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
nova-ai path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ nova-ai path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ nova-ai path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ nova-ai path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ nova-ai path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ nova-ai path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ nova-ai path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ nova-ai path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: nova-ai.invoke
```

```bash
$ nova-ai path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ nova-ai path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
nova-ai path resolve 'oc://AGENTS.md/tools/gh/risk' --human
nova-ai path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
nova-ai path find 'oc://AGENTS.md/tools/**/risk'
nova-ai path find 'oc://session.jsonl/[event=action]/userId'
nova-ai path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
nova-ai path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
nova-ai path set 'oc://gateway.jsonc/version' '2.0'
nova-ai path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ nova-ai path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
nova-ai path emit ./AGENTS.md
nova-ai path emit ./gateway.jsonc --json
```
...
`nova-ai path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__NOVA_AI_REDACTED__` (verbatim or as a substring) is refused at write
  time.
