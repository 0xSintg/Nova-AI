...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Nova AI Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`nova-ai acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Nova AI channel
conversations instead of hosting an ACP harness session, use
[`nova-ai mcp serve`](/cli/mcp) instead.
...
- Nova AI acts as an ACP server
- an IDE or ACP client connects to Nova AI
- Nova AI forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Nova AI runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Nova AI: use `nova-ai acp`
- Nova AI should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Nova AI parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Nova AI gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
nova-ai acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
nova-ai acp --url wss://gateway-host:18789 --token-file ~/.nova-ai/gateway.token

# Attach to an existing session key
nova-ai acp --session agent:main:main

# Attach by label (must already exist)
nova-ai acp --session-label "support inbox"

# Reset the session key before the first prompt
nova-ai acp --session agent:main:main --reset-session
```
...
nova-ai acp client --server-args --url wss://gateway-host:18789 --token-file ~/.nova-ai/gateway.token

# Override the server command (default: nova-ai)
nova-ai acp client --server "node" --server-args nova-ai.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Nova AI through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`nova-ai acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `nova-ai gateway call sessions.list` as the only ACP proof. That
...
it to drive an Nova AI Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `nova-ai acp` over stdio.

Example config (persisted):

```bash
nova-ai config set gateway.remote.url wss://gateway-host:18789
nova-ai config set gateway.remote.token <token>
```
...
```bash
nova-ai acp --session agent:main:main
nova-ai acp --session agent:design:main
nova-ai acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Nova AI plugin tools or selected
...
[Nova AI tools MCP bridge](/tools/acp-agents-setup#nova-ai-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Nova AI bot over ACP, use `acpx` with its built-in `nova-ai` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx nova-ai` at `nova-ai acp`.
3. Target the Nova AI session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Nova AI ACP session
acpx nova-ai exec "Summarize the active Nova AI session state."

# Persistent named session for follow-up turns
acpx nova-ai sessions ensure --name codex-bridge
acpx nova-ai -s codex-bridge --cwd /path/to/repo \
  "Ask my Nova AI work agent for recent context relevant to this repo."
```

If you want `acpx nova-ai` to target a specific Gateway and session key every
time, override the `nova-ai` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "nova-ai": {
      "command": "env NOVA_AI_HIDE_BANNER=1 NOVA_AI_SUPPRESS_NOTES=1 nova-ai acp --url ws://127.0.0.1:18789 --token-file ~/.nova-ai/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Nova AI checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env NOVA_AI_HIDE_BANNER=1 NOVA_AI_SUPPRESS_NOTES=1 node nova-ai.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Nova AI agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Nova AI ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`NOVA_AI_GATEWAY_TOKEN`, `NOVA_AI_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`NOVA_AI_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `NOVA_AI_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `nova-ai acp client` sets `NOVA_AI_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `nova-ai`).
