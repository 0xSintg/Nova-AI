...
read_when:
  - You run nova-ai with no command and want to understand Crestodian
  - You need a configless-safe way to inspect or repair Nova AI
...
Crestodian is Nova AI's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `nova-ai` with no command starts Crestodian in an interactive terminal.
Running `nova-ai crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`nova-ai tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same Nova AI reference discovery as regular agents. In a Git checkout,
...
[https://github.com/nova-ai/nova-ai](https://github.com/nova-ai/nova-ai), with explicit
...
nova-ai
nova-ai crestodian
nova-ai crestodian --json
nova-ai crestodian --message "models"
nova-ai crestodian --message "validate config"
nova-ai crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
nova-ai crestodian --message "set default model openai/gpt-5.5" --yes
nova-ai onboard --modern
```
...
config set-ref gateway.auth.token env NOVA_AI_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:nova-ai-codex-app-server
plugin uninstall nova-ai-codex-app-server
talk to work agent
...
- `nova-ai.json` is missing
- `nova-ai.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`nova-ai --help` and `nova-ai --version` still use the normal fast paths.
Noninteractive `nova-ai` exits with a short message instead of printing root
...
```text
~/.nova-ai/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`nova-ai onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `nova-ai onboard` still runs classic onboarding.
...
planner turn through Nova AI's normal runtime paths. It first uses the
configured Nova AI model. If no configured model is usable yet, it can fall
...
`nova-ai tui`, `nova-ai chat`, and `nova-ai terminal` still open the normal
...
Nova AI: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
Nova AI: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
Nova AI: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `nova-ai` for agent handoff.
...
That lane starts with an empty state dir, routes bare `nova-ai` to Crestodian,
