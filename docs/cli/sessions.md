---
summary: "CLI reference for `nova-ai sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `nova-ai channels status --probe`,
`nova-ai status --deep`, or `nova-ai health --verbose` when you need live
channel connectivity.

`nova-ai sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
nova-ai sessions
nova-ai sessions --agent work
nova-ai sessions --all-agents
nova-ai sessions --active 120
nova-ai sessions --limit 25
nova-ai sessions --verbose
nova-ai sessions --json
```
...
nova-ai sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.nova-ai/trajectory-exports/` under the selected workspace.

`nova-ai sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`nova-ai sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.nova-ai/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.nova-ai/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
nova-ai sessions cleanup --agent work --dry-run
nova-ai sessions cleanup --all-agents --dry-run
nova-ai sessions cleanup --enforce
nova-ai sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
nova-ai sessions cleanup --dry-run --fix-dm-scope
nova-ai sessions cleanup --json
```

`nova-ai sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `nova-ai sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run logs (`cron/runs/<jobId>.jsonl`), which are managed by `cron.runLog.maxBytes` and `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`nova-ai sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.nova-ai/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.nova-ai/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
