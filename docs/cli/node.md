---
summary: "CLI reference for `nova-ai node` (headless node host)"
read_when:
...
`nova-ai node run` and `nova-ai node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `NOVA_AI_GATEWAY_TOKEN` / `NOVA_AI_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `nova-ai.json` config
key.
`nova-ai node install` persists it into the supervised node service when it is
...
```bash
nova-ai node install --host <gateway-host> --port 18789
```
...
nova-ai node uninstall
```

Use `nova-ai node run` for a foreground node host (no service).
...
- `nova-ai approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Nova AI prepares a canonical `systemRunPlan`
