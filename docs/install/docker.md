---
summary: "Optional Docker-based setup and onboarding for Nova AI"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Nova AI on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export NOVA_AI_IMAGE="ghcr.io/nova-ai/nova-ai:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/nova-ai/nova-ai/pkgs/container/nova-ai).
  ...
  <Step title="Complete onboarding">
    ...
    `nova-ai-gateway` directly. `nova-ai-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm nova-ai-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm nova-ai-cli channels login

    # Telegram
    docker compose run --rm nova-ai-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm nova-ai-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t nova-ai:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node nova-ai-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node nova-ai-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d nova-ai-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `NOVA_AI_EXTRA_MOUNTS`
or `NOVA_AI_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `nova-ai-cli` shares `nova-ai-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d nova-ai-gateway`, run onboarding
and setup-time config writes through `nova-ai-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `NOVA_AI_IMAGE`                           | Use a remote image instead of building locally                        |
| `NOVA_AI_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `NOVA_AI_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `NOVA_AI_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `NOVA_AI_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `NOVA_AI_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `NOVA_AI_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `NOVA_AI_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `NOVA_AI_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `NOVA_AI_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `NOVA_AI_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `NOVA_AI_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, Nova AI
...
`NOVA_AI_IMAGE_APT_PACKAGES` during image build. The legacy
`NOVA_AI_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `NOVA_AI_IMAGE_PIP_PACKAGES`. This runs
...
`NOVA_AI_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export NOVA_AI_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="nova-ai-gateway"
./scripts/docker/setup.sh
```

Install the official `@nova-ai/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`NOVA_AI_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@nova-ai/diagnostics-prometheus`, enable the
...
```bash
docker compose exec nova-ai-gateway node dist/index.js health --token "$NOVA_AI_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `NOVA_AI_GATEWAY_BIND=lan` so host access to
...
When Nova AI runs in Docker, `127.0.0.1` inside the container is the container
...
`NOVA_AI_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `NOVA_AI_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `NOVA_AI_CONFIG_DIR` to `/home/node/.nova-ai`,
`NOVA_AI_WORKSPACE_DIR` to `/home/node/.nova-ai/workspace`, and
`NOVA_AI_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/nova-ai`, so those
...
That mounted config directory is where Nova AI keeps:

- `nova-ai.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `NOVA_AI_GATEWAY_TOKEN`
...
but separate from `NOVA_AI_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
Nova AI home, so plugin install records and package roots survive container
...
If you installed ClawDock from the older `scripts/shell-helpers/novaock-helpers.sh` raw path, rerun the install command above so your local helper file tracks the new location.

Then use `novaock-start`, `novaock-stop`, `novaock-dashboard`, etc. Run
`novaock-help` for all commands.
See [ClawDock](/install/novaock) for the full helper guide.

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export NOVA_AI_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export NOVA_AI_SANDBOX=1
    export NOVA_AI_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the Nova AI sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm nova-ai-cli gateway probe
    docker compose run -T --rm nova-ai-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `nova-ai-cli` uses `network_mode: "service:nova-ai-gateway"` so CLI
    ...
    `no-new-privileges` on both `nova-ai-gateway` and `nova-ai-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in nova-ai-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `nova-ai-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `nova-ai plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm nova-ai-cli plugins install <package>
    ```

    If you already created a long-running `nova-ai-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.nova-ai`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/nova-ai-config /path/to/nova-ai-workspace
    ```
    ...
    `/path/to/nova-ai-config/npm` to `root:root` if you intentionally run
    Nova AI as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export NOVA_AI_HOME_VOLUME="nova-ai_home"`
    2. **Bake system deps**: `export NOVA_AI_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export NOVA_AI_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export NOVA_AI_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm nova-ai-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `NOVA_AI_HOME_VOLUME` or
       `NOVA_AI_EXTRA_MOUNTS`. Nova AI auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/nova-ai/nova-ai/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Nova AI runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm nova-ai-cli dashboard --no-open
    docker compose run --rm nova-ai-cli devices list
    docker compose run --rm nova-ai-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm nova-ai-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm nova-ai-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [ClawDock](/install/novaock) — Docker Compose community setup
- [Updating](/install/updating) — keeping Nova AI up to date
