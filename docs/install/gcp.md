---
summary: "Run Nova AI Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Nova AI running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Nova AI Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Nova AI 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Nova AI Gateway in Docker
- Persist `~/.nova-ai` + `~/.nova-ai/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.nova-ai` state includes `nova-ai.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-nova-ai-project --name="Nova AI Gateway"
    gcloud config set project my-nova-ai-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create nova-ai-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh nova-ai-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Nova AI repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    NOVA_AI_IMAGE=nova-ai:latest
    NOVA_AI_GATEWAY_TOKEN=
    NOVA_AI_GATEWAY_BIND=lan
    NOVA_AI_GATEWAY_PORT=18789

    NOVA_AI_CONFIG_DIR=/home/$USER/.nova-ai
    NOVA_AI_WORKSPACE_DIR=/home/$USER/.nova-ai/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.nova-ai
    ```

    Set `NOVA_AI_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Nova AI uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `NOVA_AI_GATEWAY_TOKEN`.
    ...
    services:
      nova-ai-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - NOVA_AI_GATEWAY_BIND=${NOVA_AI_GATEWAY_BIND}
          - NOVA_AI_GATEWAY_PORT=${NOVA_AI_GATEWAY_PORT}
          - NOVA_AI_GATEWAY_TOKEN=${NOVA_AI_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${NOVA_AI_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`NOVA_AI_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm nova-ai-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh nova-ai-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop nova-ai-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type nova-ai-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start nova-ai-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create nova-ai-deploy \
     --display-name="Nova AI Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-nova-ai-project \
     --member="serviceAccount:nova-ai-deploy@my-nova-ai-project.iam.gserviceaccount.com" \
