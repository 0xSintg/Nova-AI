---
summary: "Host Nova AI on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Nova AI on Oracle Cloud
  - Looking for free VPS hosting for Nova AI
  - Want 24/7 Nova AI on a small server
title: "Oracle Cloud"
---

Run a persistent Nova AI Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname nova-ai
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=nova-ai
    ```

    From now on, connect via Tailscale: `ssh ubuntu@nova-ai`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    nova-ai config set gateway.bind loopback
    nova-ai config set gateway.auth.mode token
    nova-ai doctor --generate-gateway-token
    nova-ai config set gateway.tailscale.mode serve
    nova-ai config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://nova-ai.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.nova-ai` to restrict credential file permissions.
- `nova-ai security audit` for an Nova AI-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Nova AI features work fine; a small number of native binaries need ARM builds:
...
- `~/.nova-ai/` — `nova-ai.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.nova-ai/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@nova-ai
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=nova-ai --reset` to re-authenticate.

**Gateway will not start** -- Run `nova-ai doctor --non-interactive` and check logs with `journalctl --user -u nova-ai-gateway.service -n 50`.
