...
Nova AI ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/nova-ai`, override with
  `NOVA_AI_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
nova-ai update --tag 2026.4.1-beta.1
...
nova-ai update --tag nova-ai@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `nova-ai update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Nova AI prompts for confirmation (skip with `--yes`).
...
Preview what `nova-ai update` would do without making changes:

```bash
nova-ai update --dry-run
nova-ai update --channel beta --dry-run
nova-ai update --tag 2026.4.1-beta.1 --dry-run
nova-ai update --dry-run --json
```
...
When you switch channels with `nova-ai update`, Nova AI also syncs plugin
sources:
