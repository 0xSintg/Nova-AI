---
summary: "Install Nova AI - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Nova AI, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Nova AI and Node kept under a local prefix such as
`~/.nova-ai`, without depending on a system-wide Node install:

```bash
curl -fsSL https://nova-ai.com/install-cli.sh | bash
```
...
`nova-ai update --channel dev` and `nova-ai update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the Nova AI package install. If you install manually with npm, your own
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g nova-ai@latest
```
...
Or skip the link and use `pnpm nova-ai ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:nova-ai/nova-ai#main
```
...
```bash
nova-ai --version      # confirm the CLI is available
nova-ai doctor         # check for config issues
nova-ai gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `nova-ai onboard --install-daemon` or `nova-ai gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Nova AI on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Nova AI completely.
  </Card>
</CardGroup>

## Troubleshooting: `nova-ai` not found

If the install succeeded but `nova-ai` is not found in your terminal:
