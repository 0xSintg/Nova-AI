---
summary: "Host Nova AI on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Nova AI on a Raspberry Pi
  - Running Nova AI on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Nova AI Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    nova-ai status
    systemctl --user status nova-ai-gateway.service
    journalctl --user -u nova-ai-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'nova-ai dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/nova-ai-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/nova-ai-compile-cache
mkdir -p /var/tmp/nova-ai-compile-cache
export NOVA_AI_NO_RESPAWN=1
EOF
source ~/.bashrc
```

`NOVA_AI_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running Nova AI, add a service drop-in:

```bash
systemctl --user edit nova-ai-gateway.service
```

```ini
[Service]
Environment=NOVA_AI_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/nova-ai-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart nova-ai-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
Most Nova AI features work on ARM64 without changes (Node.js, Telegram, WhatsApp/Baileys, Chromium). The binaries that occasionally lack ARM builds are typically optional Go/Rust CLI tools shipped by skills. Verify a missing binary's release page for `linux-arm64` / `aarch64` artifacts before falling back to building from source.

## Persistence and backups

Nova AI state lives under:

- `~/.nova-ai/` — `nova-ai.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.nova-ai/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u nova-ai-gateway.service --no-pager -n 100` and run `nova-ai doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep Nova AI up to date
