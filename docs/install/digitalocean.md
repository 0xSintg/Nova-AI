---
summary: "Host Nova AI on a DigitalOcean Droplet"
read_when:
  - Setting up Nova AI on DigitalOcean
  - Looking for a simple paid VPS for Nova AI
title: "DigitalOcean"
---

Run a persistent Nova AI Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own Nova AI state and services.
    adduser nova-ai
    usermod -aG sudo nova-ai
    loginctl enable-linger nova-ai

    su - nova-ai
    nova-ai --version
    ```

    Use the root shell only for system bootstrap. Run Nova AI commands as the non-root `nova-ai` user so state lives under `/home/nova-ai/.nova-ai/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    nova-ai config set gateway.bind tailnet
    nova-ai gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `nova-ai backup create` is portable across hosts.
