---
summary: "Schema-accurate configuration examples for common Nova AI setups"
read_when:
  - Learning how to configure Nova AI
...
Save to `~/.nova-ai/nova-ai.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.nova-ai/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/nova-ai" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.nova-ai/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
