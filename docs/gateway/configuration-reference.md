---
summary: "Gateway config reference for core Nova AI keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.nova-ai/nova-ai.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main Nova AI config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `nova-ai config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional - Nova AI uses safe defaults when omitted.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full Nova AI agent run behind Control UI Talk realtime consults
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. Nova AI probes the configured health endpoint, starts
...
Nova AI-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Pi and other runtime adapters. The `nova-ai mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `nova-ai mcp set` and
  `nova-ai doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is Nova AI metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed Nova AI agent ids.
  ...
  `default_tools_approval_mode` for that server. Nova AI strips the `codex`
...
See [MCP](/cli/mcp#nova-ai-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.nova-ai.install`
...
- Loaded from `~/.nova-ai/extensions`, `<workspace>/.nova-ai/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Nova AI plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Nova AI plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Nova AI option registry.
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Nova AI applies those as sanitized agent settings, not as raw Nova AI config patches.
...
`commitments` controls inferred follow-up memory: Nova AI can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
```json5
{
  browser: {
    ...
    profiles: {
      nova-ai: { cdpPort: 18800, color: "#FF4500" },
      work: {
        cdpPort: 18801,
        color: "#0066CC",
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Nova AI to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Nova AI treats the loopback port as a
...
- Local managed `nova-ai` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or NOVA_AI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `NOVA_AI_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, Nova AI
...
- `NOVA_AI_APNS_RELAY_BASE_URL` / `NOVA_AI_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `NOVA_AI_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `NOVA_AI_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```bash
NOVA_AI_CONFIG_PATH=~/.nova-ai/a.json \
NOVA_AI_STATE_DIR=~/.nova-ai-a \
nova-ai gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.nova-ai-dev` + port `19001`), `--profile <name>` (uses `~/.nova-ai-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/nova-ai/tls/server.crt",
      keyPath: "/etc/nova-ai/tls/server.key",
      caPath: "/etc/nova-ai/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.nova-ai/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-nova-ai-token: <token>`.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.nova-ai/hooks/transforms`; workspace skill directories are rejected. If `nova-ai doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `NOVA_AI_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.nova-ai/workspace/canvas",
            liveReload: true,
            // enabled: false, // or NOVA_AI_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__nova-ai__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__nova-ai__/a2ui/`
...
- Also serves A2UI at `/__nova-ai__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `nova-ai`. Override with `NOVA_AI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.nova-ai/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `nova-ai dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.nova-ai/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${NOVA_AI_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `nova-ai.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.nova-ai/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/nova-ai-vault-resolver",
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `nova-ai doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.nova-ai/credentials/oauth.json`.
...
- Default log file: `/tmp/nova-ai/nova-ai-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). Nova AI keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.nova-ai/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, Nova AI uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `NOVA_AI_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. Nova AI then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$NOVA_AI_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Nova AI.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `NOVA_AI_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `nova-ai doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `nova-ai.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
- Nova AI-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `nova-ai.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for Nova AI-owned writes; those writes fail closed instead of flattening the config.
