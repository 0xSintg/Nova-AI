...
Nova AI can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Nova AI tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
nova-ai agent --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, Nova AI now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Nova AI will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw Nova AI transcript history before compaction.
...
2. **Builds a system prompt** using the same Nova AI prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   Nova AI session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us Nova AI-style Claude CLI usage is allowed again, so Nova AI treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled Anthropic `claude-cli` backend receives the Nova AI skills snapshot
two ways: the compact Nova AI skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Nova AI would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Nova AI to the
child process environment for the run.

Claude CLI also has its own noninteractive permission mode. Nova AI maps that
...
`tools.exec.ask: "off"`), Nova AI adds `--permission-mode bypassPermissions`.
...
The bundled Anthropic `claude-cli` backend also maps Nova AI `/think` levels
...
Before Nova AI can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
nova-ai models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, Nova AI resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; Nova AI clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from Nova AI's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. Nova AI still keeps raw
...
- Nova AI drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, Nova AI lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), Nova AI seeds
...
provider would start cold because Nova AI's own session transcript is empty
for `claude-cli` runs.
...
Nova AI will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Nova AI appends the
...
- For Gemini CLI JSON output, Nova AI reads reply text from `response` and
...
agent runs use the Codex app-server harness through `openai/*`; Nova AI no
...
- `stats.cached` is normalized into Nova AI `cacheRead`.
- If `stats.input` is missing, Nova AI derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Nova AI handles
...
CLI backends do **not** receive Nova AI tool calls directly, but a backend can
...
When bundle MCP is enabled, Nova AI:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`NOVA_AI_MCP_TOKEN`)
...
If no MCP servers are enabled, Nova AI still injects a strict config when a
...
- **No direct Nova AI tool calls.** Nova AI does not inject tool calls into
