---
summary: "Use OpenShell as a managed sandbox backend for Nova AI agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Nova AI. Instead of running Docker
containers locally, Nova AI delegates sandbox lifecycle to the `openshell` CLI,
...
```bash
nova-ai plugins install @nova-ai/openshell-sandbox
```
...
2. Restart the Gateway. On the next agent turn, Nova AI creates an OpenShell
...
- You edit files locally outside Nova AI and want those changes visible in the
...
- When the sandbox is first created, Nova AI seeds the remote workspace from
...
- Nova AI does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Nova AI after the initial seed, the remote sandbox does **not** see those changes. Use `nova-ai sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"nova-ai"`  | Sandbox source for first-time create                  |
...
1. Nova AI calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Nova AI calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `nova-ai sandbox` commands
