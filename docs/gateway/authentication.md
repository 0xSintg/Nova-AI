...
Nova AI supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Nova AI also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `nova-ai gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.nova-ai/.env` so the daemon can read it:
...
API keys for daemon use: `nova-ai onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.nova-ai/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Nova AI as a supported token
path. Anthropic staff has since told us that Nova AI-style Claude CLI usage is
allowed again, so Nova AI treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Nova AI to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Nova AI auth profile.
...
```bash
nova-ai models auth paste-token --provider openrouter
```
...
Nova AI expects the canonical `version` + `profiles` shape at runtime. If an older install still has a flat file such as `{ "openrouter": { "apiKey": "..." } }`, run `nova-ai doctor --fix` to rewrite it as an `openrouter:default` API-key profile; doctor keeps a `.legacy-flat.*.bak` copy beside the original. Endpoint details such as `baseUrl`, `api`, model ids, headers, and timeouts belong under `models.providers.<id>` in `nova-ai.json` or `models.json`, not in `auth-profiles.json`.

External auth routes such as Bedrock `auth: "aws-sdk"` are also not credentials. If you want a named Bedrock route, put `auth.profiles.<id>.mode: "aws-sdk"` in `nova-ai.json`; do not write `type: "aws-sdk"` into `auth-profiles.json`. `nova-ai doctor --fix` moves legacy AWS SDK markers from the credential store into config metadata.
...
- If auth exists but Nova AI cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Nova AI integration path is allowed again.
- Nova AI therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `NOVA_AI_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Nova AI retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, Nova AI deletes
...
When you debug order issues, `nova-ai models status --probe` shows omitted
...
Run `nova-ai models status` to confirm which profile is expiring. If an
