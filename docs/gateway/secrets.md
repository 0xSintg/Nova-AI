...
Nova AI supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `NOVA_AI_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Nova AI runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Nova AI validates the resolved target path.
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/Nova AI QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/nova-ai"],
...
- Nova AI resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__NOVA_AI_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `nova-ai.json` refs)
...
When reload-time activation fails after a healthy state, Nova AI enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `nova-ai memory` remote-memory paths and `nova-ai qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `nova-ai status`, `nova-ai status --all`, `nova-ai channels status`, `nova-ai channels resolve`, `nova-ai security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `nova-ai secrets reload`.
...
<Steps>
  <Step title="Audit current state">
    ```bash
    nova-ai secrets audit --check
    ```
  </Step>
  <Step title="Configure SecretRefs">
    ```bash
    nova-ai secrets configure
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
<AccordionGroup>
  <Accordion title="secrets audit">
    ...
    - plaintext values at rest (`nova-ai.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `nova-ai.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `nova-ai secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `nova-ai.json` plus `auth-profiles.json` for one agent scope
    ...
    - `nova-ai secrets configure --providers-only`
    - `nova-ai secrets configure --skip-provider-setup`
    - `nova-ai secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    nova-ai secrets apply --from /tmp/nova-ai-secrets-plan.json
    nova-ai secrets apply --from /tmp/nova-ai-secrets-plan.json --allow-exec
    nova-ai secrets apply --from /tmp/nova-ai-secrets-plan.json --dry-run
    nova-ai secrets apply --from /tmp/nova-ai-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
Nova AI intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
