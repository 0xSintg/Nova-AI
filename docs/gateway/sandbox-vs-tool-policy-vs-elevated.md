...
Nova AI has three related (but different) controls:
...
Use the inspector to see what Nova AI is _actually_ doing:
...
- Nova AI validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:nova-ai`: all built-in Nova AI tools (excludes provider plugins)
