...
read_when:
  - Setting up Nova AI for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

Nova AI reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.nova-ai/nova-ai.json`.
The active config path must be a regular file. Symlinked `nova-ai.json`
layouts are unsupported for Nova AI-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `NOVA_AI_CONFIG_PATH` directly at the real file.

If the file is missing, Nova AI uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `nova-ai onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    nova-ai onboard       # full onboarding flow
    nova-ai configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.nova-ai/nova-ai.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Nova AI only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`nova-ai config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`nova-ai doctor`, `nova-ai logs`, `nova-ai health`, `nova-ai status`)
- Run `nova-ai doctor` to see exact issues
- Run `nova-ai doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `nova-ai.json`
...
Run `nova-ai doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `nova-ai config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@nova-ai", "nova-ai"],
            },
          },
        ],
      },
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `NOVA_AI_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `nova-ai.json`.
    ...
    ```bash
    nova-ai config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `NOVA_AI_APNS_RELAY_BASE_URL` and `NOVA_AI_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `NOVA_AI_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-nova-ai-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Nova AI-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Nova AI updates that included file and leaves `nova-ai.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Nova AI-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `nova-ai.json`. To share a tree across machines or users, set
      `NOVA_AI_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.nova-ai/nova-ai.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `nova-ai.json`. Nova AI-owned config
...
config`, inspect the config, run `nova-ai config validate`, then run `nova-ai
...
When you edit a source file that is referenced through `$include`, Nova AI plans
...
```bash
nova-ai gateway call config.get --params '{}'  # capture payload.hash
nova-ai gateway call config.patch --params '{
...
Nova AI reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.nova-ai/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Nova AI runs your login shell and imports only the missing keys:
...
Env var equivalent: `NOVA_AI_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${NOVA_AI_GATEWAY_TOKEN}" } },
