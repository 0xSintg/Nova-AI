---
summary: "Run multiple Nova AI Gateways on one host (isolation, ports, and profiles)"
read_when:
...
nova-ai --profile rescue onboard
nova-ai --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `nova-ai --profile rescue onboard`:
...
`nova-ai --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.nova-ai/workspace-rescue`)
...
# extra gateway
nova-ai --profile ops setup
nova-ai --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
nova-ai --profile main setup
nova-ai --profile main gateway --port 18789
...
nova-ai --profile ops gateway install --port 19789
```
...
- `NOVA_AI_CONFIG_PATH` — per-instance config file
- `NOVA_AI_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `NOVA_AI_GATEWAY_PORT` / `--port`).
...
```bash
NOVA_AI_CONFIG_PATH=~/.nova-ai/main.json \
NOVA_AI_STATE_DIR=~/.nova-ai \
nova-ai gateway --port 18789

NOVA_AI_CONFIG_PATH=~/.nova-ai/rescue.json \
NOVA_AI_STATE_DIR=~/.nova-ai-rescue \
nova-ai gateway --port 19789
```
...
nova-ai --profile rescue gateway status --deep
nova-ai --profile rescue gateway probe
nova-ai status
nova-ai --profile rescue status
nova-ai --profile rescue browser status
```
