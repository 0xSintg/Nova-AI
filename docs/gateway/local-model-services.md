---
summary: "Start local model servers on demand before Nova AI model requests"
read_when:
  - You want Nova AI to start a local model server only when its model is selected
...
`models.providers.<id>.localService` lets Nova AI start a provider-owned local
model server on demand. It is provider-level config: when the selected model
belongs to that provider, Nova AI probes the service, starts the process if the
...
2. If that provider has `localService`, Nova AI probes `healthUrl`.
3. If the probe succeeds, Nova AI uses the existing server.
4. If the probe fails, Nova AI starts `command` with `args`.
5. Nova AI polls readiness until `readyTimeoutMs` expires.
6. The model request is sent through the normal provider transport.
7. If Nova AI started the process and `idleStopMs` is positive, the process is
   stopped after the last in-flight request has been idle for that long.

Nova AI does not install launchd, systemd, Docker, or a daemon for this. The
server is a child process of the Nova AI process that first needed it.
...
- `env`: optional environment variables merged over the Nova AI process
  environment.
- `healthUrl`: readiness URL. If omitted, Nova AI appends `/models` to
  `baseUrl`, so `http://127.0.0.1:8000/v1` becomes
  `http://127.0.0.1:8000/v1/models`.
- `readyTimeoutMs`: startup readiness deadline. Default: `120000`.
- `idleStopMs`: idle shutdown delay for Nova AI-started processes. `0` or
  omitted keeps the process alive until Nova AI exits.
...
- One Nova AI process manages the child it started. Another Nova AI process
...
<CardGroup cols={2}>
  ...
  <Card title="Inferrs" href="/providers/inferrs" icon="cpu">
    Run Nova AI through the inferrs OpenAI-compatible local server.
  </Card>
</CardGroup>
