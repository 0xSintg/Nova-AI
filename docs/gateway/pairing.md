...
nova-ai nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), Nova AI treats
...
Pairing state is stored under the Gateway state directory (default `~/.nova-ai`):

- `~/.nova-ai/nodes/paired.json`
- `~/.nova-ai/nodes/pending.json`

If you override `NOVA_AI_STATE_DIR`, the `nodes/` folder moves with it.
