...
Nova AI's Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `nova-ai agent`), so routing/permissions/config match your Gateway.
...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `NOVA_AI_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `NOVA_AI_GATEWAY_PASSWORD`).
...
- Internal same-host callers that bypass the proxy can use
  `gateway.auth.password` / `NOVA_AI_GATEWAY_PASSWORD` as a local direct
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, Nova AI treats that caller as a trusted operator for this gateway.
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-nova-ai-scopes` header.
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`) honor `x-nova-ai-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `gateway.auth.mode="token"` or `"password"` + `Authorization: Bearer ...`
  ...
  - ignores narrower `x-nova-ai-scopes`
...
- trusted identity-bearing HTTP modes (for example trusted proxy auth, or `gateway.auth.mode="none"` on private ingress)
  ...
  - honor `x-nova-ai-scopes` when the header is present
...
Nova AI treats the OpenAI `model` field as an **agent target**, not a raw provider model id.

- `model: "nova-ai"` routes to the configured default agent.
- `model: "nova-ai/default"` also routes to the configured default agent.
- `model: "nova-ai/<agentId>"` routes to a specific agent.

Optional request headers:

- `x-nova-ai-model: <provider/model-or-bare-id>` overrides the backend model for the selected agent.
- `x-nova-ai-agent-id: <agentId>` remains supported as a compatibility override.
- `x-nova-ai-session-key: <sessionKey>` fully controls session routing.
- `x-nova-ai-message-channel: <channel>` sets the synthetic ingress channel context for channel-aware prompts and policies.

Compatibility aliases still accepted:

- `model: "nova-ai:<agentId>"`
- `model: "agent:<agentId>"`
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An Nova AI agent-target list.

    The returned ids are `nova-ai`, `nova-ai/default`, and `nova-ai/<agentId>` entries.
  ...
  <Accordion title="Why is `nova-ai/default` included?">
    `nova-ai/default` is the stable alias for the configured default agent.
  ...
  <Accordion title="How do I override the backend model?">
    ...
    Examples:
    `x-nova-ai-model: openai/gpt-5.4`
    `x-nova-ai-model: gpt-5.5`
  ...
  <Accordion title="How do embeddings fit this contract?">
    ...
    Use `model: "nova-ai/default"` or `model: "nova-ai/<agentId>"`.
    When you need a specific embedding model, send it in `x-nova-ai-model`.
...
- Model: `nova-ai/default`

Expected behavior:

- `GET /v1/models` should list `nova-ai/default`
- Open WebUI should use `nova-ai/default` as the chat model id
- If you want a specific backend provider/model for that agent, set the agent's normal default model or send `x-nova-ai-model`
...
If that returns `nova-ai/default`, most Open WebUI setups can connect with the same base URL and token.
...
curl -N http://127.0.0.1:18789/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-nova-ai-model: openai/gpt-5.4' \
  -d '{
    "model": "nova-ai/research",
    "stream": true,
    "messages": [{"role":"user","content":"hi"}]
  }'
```
...
```bash
curl -sS http://127.0.0.1:18789/v1/models/nova-ai%2Fdefault \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
curl -sS http://127.0.0.1:18789/v1/embeddings \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-nova-ai-model: openai/text-embedding-3-small' \
  -d '{
    "model": "nova-ai/default",
    "input": ["alpha", "beta"]
  }'
...
- `/v1/models` returns Nova AI agent targets, not raw provider catalogs.
- `nova-ai/default` is always present so one stable id works across environments.
- Backend provider/model overrides belong in `x-nova-ai-model`, not the OpenAI `model` field.
