...
- Default rolling log file is under `/tmp/nova-ai/` (one file per day): `nova-ai-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.nova-ai/nova-ai.json`:
  - `logging.file`
  - `logging.level`
...
Nova AI can mask sensitive tokens before log or transcript output leaves the
...
`nova-ai gateway` supports a per-gateway style switch:
