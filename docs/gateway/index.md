...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `nova-ai gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `NOVA_AI_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `NOVA_AI_GATEWAY_TOKEN` / `NOVA_AI_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Nova AI's highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `nova-ai`, `nova-ai/default`, and `nova-ai/<agentId>`.
- `nova-ai/default` is the stable alias that always maps to the configured default agent.
- Use `x-nova-ai-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `NOVA_AI_GATEWAY_PORT` → `gateway.port` → `18789` |
| Bind mode    | CLI/override → `gateway.bind` → `loopback`                    |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `nova-ai doctor --fix` or `nova-ai gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
- Unique `gateway.port`
- Unique `NOVA_AI_CONFIG_PATH`
- Unique `NOVA_AI_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
NOVA_AI_CONFIG_PATH=~/.nova-ai/a.json NOVA_AI_STATE_DIR=~/.nova-ai-a nova-ai gateway --port 19001
NOVA_AI_CONFIG_PATH=~/.nova-ai/b.json NOVA_AI_STATE_DIR=~/.nova-ai-b nova-ai gateway --port 19002
```
...
Use `nova-ai gateway restart` for restarts. Do not chain `nova-ai gateway stop` and `nova-ai gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default — this removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `nova-ai gateway stop --disable`.

LaunchAgent labels are `ai.nova-ai.gateway` (default) or `ai.nova-ai.<profile>` (named profile). `nova-ai doctor` audits and repairs service config drift.
...
```bash
nova-ai gateway install
systemctl --user enable --now nova-ai-gateway[-<profile>].service
nova-ai gateway status
```
...
```ini
[Unit]
Description=Nova AI Gateway
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/nova-ai gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `Nova AI Gateway`
(or `Nova AI Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Nova AI falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now nova-ai-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/nova-ai-gateway[-<profile>].service` and adjust
`ExecStart=` if your `nova-ai` binary lives elsewhere.

Do not also let `nova-ai doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level Nova AI gateway service; use `NOVA_AI_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
nova-ai --dev setup
nova-ai --dev gateway --allow-unconfigured
nova-ai --dev status
```
