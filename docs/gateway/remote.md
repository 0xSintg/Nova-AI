...
- Use the macOS app's remote mode (Settings → General → Nova AI runs).
...
- `nova-ai health` and `nova-ai status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `nova-ai gateway status`, `nova-ai gateway health`, `nova-ai gateway probe`, and `nova-ai gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `NOVA_AI_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`NOVA_AI_GATEWAY_URL`) may use env credentials only (`NOVA_AI_GATEWAY_TOKEN` / `NOVA_AI_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `NOVA_AI_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `NOVA_AI_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `NOVA_AI_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `NOVA_AI_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `NOVA_AI_GATEWAY_*` only.
...
```bash
nova-ai config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.nova-ai.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.nova-ai.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.nova-ai.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.nova-ai.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.nova-ai.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.nova-ai.ssh-tunnel
```
