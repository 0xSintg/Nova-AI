...
Nova AI can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Nova AI requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this Nova AI Serve/Funnel
mode. `off` means Nova AI is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `NOVA_AI_GATEWAY_TOKEN` is set)
- `password` (shared secret via `NOVA_AI_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Nova AI verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Nova AI only treats a request as Serve when it arrives from loopback with
...
Prefer `NOVA_AI_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
nova-ai gateway --tailscale serve
nova-ai gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Nova AI to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, Nova AI checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The Nova AI-managed Funnel password-only policy is unchanged.
