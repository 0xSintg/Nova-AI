...
Nova AI can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_nova-ai-gw._tcp` under a dedicated zone
   (example: `nova-ai.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

Nova AI supports any discovery domain; `nova-ai.internal.` is just an example.
...
- serve your chosen domain (example: `nova-ai.internal.`) from `~/.nova-ai/dns/<domain>.db`

Validate from a tailnet-connected machine:

```bash
dns-sd -B _nova-ai-gw._tcp nova-ai.internal.
dig @<TAILNET_IPV4> -p 53 _nova-ai-gw._tcp.nova-ai.internal PTR +short
```
...
`_nova-ai-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.nova-ai/nova-ai.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_nova-ai-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin when the plugin is enabled; wide-area
DNS-SD publishing remains Gateway-owned.

## Service types

- `_nova-ai-gw._tcp` - gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _nova-ai-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _nova-ai-gw._tcp local.
  ```
...
in-progress states. If the service never reaches `announced`, Nova AI eventually
...
invalid DNS-label character, Nova AI falls back to `nova-ai.local`. Set
`NOVA_AI_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_nova-ai-gw._tcp`.
...
```bash
nova-ai plugins enable bonjour
```
...
discovery plugin for that Nova AI config:

```bash
nova-ai plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `NOVA_AI_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `NOVA_AI_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `NOVA_AI_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `NOVA_AI_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep NOVA_AI_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `NOVA_AI_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `NOVA_AI_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. Nova AI retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `NOVA_AI_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `nova-ai plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `nova-ai plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `NOVA_AI_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `NOVA_AI_DISABLE_BONJOUR`).
- `NOVA_AI_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `NOVA_AI_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.nova-ai/nova-ai.json` controls the Gateway bind mode.
- `NOVA_AI_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `NOVA_AI_SSH_PORT`).
- `NOVA_AI_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `NOVA_AI_TAILNET_DNS`).
- `NOVA_AI_CLI_PATH` overrides the advertised CLI path (legacy: `NOVA_AI_CLI_PATH`).
