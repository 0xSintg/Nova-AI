...
- `nova-ai gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `nova-ai doctor` reports no blocking config/service issues.
- `nova-ai channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.
...
- `Update restart` in `nova-ai status` / `nova-ai status --all`. Pending or
  failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run nova-ai doctor --fix`
  under Channels. That means the channel config still exists, but plugin
  registration failed before the channel could load.
- provider 401s after re-auth. `nova-ai doctor --fix` checks for stale
...
Use this when a gateway service unexpectedly stops after an update, or logs show that one `nova-ai` binary is older than the version that last wrote `nova-ai.json`.

Nova AI stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer Nova AI, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which nova-ai
nova-ai --version
nova-ai gateway status --deep
nova-ai config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `nova-ai` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `nova-ai` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `NOVA_AI_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>

## Protocol mismatch after rollback

Use this when logs keep printing `protocol mismatch` after you downgrade or roll back Nova AI. This means an older Gateway is running, but a newer local client process is still trying to reconnect with a protocol range that the older Gateway cannot speak.
...
- `Established clients:` in `nova-ai gateway status --deep` or `Gateway clients` in `nova-ai doctor --deep`. This lists active TCP clients connected to the Gateway port, including PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer Nova AI install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale Nova AI client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed Nova AI, such as local dashboards, editors, app-server helpers, or long-running `nova-ai logs --follow` shells.
3. Re-run `nova-ai gateway status --deep` or `nova-ai doctor --deep` and confirm the stale client PID is gone.
...
Nova AI treats every skill root as a containment boundary. A symlink under
`~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or
`~/.nova-ai/skills` is skipped when its real target resolves outside that root
...
nova-ai config get skills.load
```
...
nova-ai config get agents.defaults.models
```
...
- Nova AI model runs fail only on normal agent turns
...
nova-ai infer model run --model <provider/model> --prompt "hi" --json
nova-ai logs --follow
```

Look for:

- direct tiny calls succeed, but Nova AI runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Nova AI retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but Nova AI agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → Nova AI transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Nova AI's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Nova AI agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
nova-ai status
nova-ai channels status --probe
nova-ai pairing list --channel <channel> [--account <id>]
nova-ai config get channels
nova-ai logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `nova-ai config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
If `nova-ai devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `nova-ai devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
nova-ai logs --follow
nova-ai doctor
nova-ai gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `nova-ai onboard --mode local` / `nova-ai setup` to restamp the expected local-mode config. If you are running Nova AI via Podman, the default config path is `~/.nova-ai/nova-ai.json`.
    ...
    - `System-level Nova AI gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `NOVA_AI_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `nova-ai doctor --fix` or `nova-ai gateway install --force`, then restart the gateway service.
...
nova-ai gateway status --deep
nova-ai logs --follow
nova-ai gateway stability --bundle latest
nova-ai gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → Nova AI captured a pre-OOM stability bundle. Inspect it with `nova-ai gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → Nova AI detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `nova-ai.json.rejected.*` file beside the active config
- A timestamped `nova-ai.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit
- Nova AI keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Nova AI-owned write.
    - Gateway startup fails closed instead of rewriting `nova-ai.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Nova AI-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `nova-ai doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, Nova AI rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(nova-ai config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Nova AI-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Nova AI-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `nova-ai doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `nova-ai config set` or `config.patch`.
    3. Run `nova-ai config validate` before restarting.
...
Use this when `nova-ai gateway probe` reaches something, but still prints a warning block.

```bash
nova-ai gateway probe
nova-ai gateway probe --json
nova-ai gateway probe --ssh user@gateway-host
```
...
nova-ai cron status
nova-ai cron list
nova-ai cron runs --id <jobId> --limit 20
nova-ai system heartbeat last
nova-ai logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Nova AI skips the model call.
...
nova-ai nodes describe --node <idOrNameOrIp>
nova-ai approvals get --node <idOrNameOrIp>
nova-ai logs --follow
nova-ai status
```
...
nova-ai browser start --browser-profile nova-ai
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `nova-ai browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Nova AI-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    nova-ai gateway status
    nova-ai config get gateway.mode
    nova-ai config get gateway.remote.url
    nova-ai config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    nova-ai config get gateway.bind
    nova-ai config get gateway.auth.mode
    nova-ai config get gateway.auth.token
    nova-ai gateway status
    nova-ai logs --follow
    ```
