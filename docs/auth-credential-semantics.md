...
Explicit copy flows, such as `nova-ai agents add`, use this portability policy:
...
has such a marker, `nova-ai doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but Nova AI cannot resolve a probeable model
