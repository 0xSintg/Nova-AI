...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.nova-ai/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Nova AI picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    nova-ai agent --message "give me a greeting"
    ```
...
| `metadata.nova-ai.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.)                    |
| `metadata.nova-ai.requires.bins`   | No       | Required binaries on PATH                                      |
| `metadata.nova-ai.requires.config` | No       | Required config keys                                           |
...
- **Test locally** — use `nova-ai agent --message "..."` to test before sharing
...
| `~/.nova-ai/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Nova AI) | Low        | Global                |
