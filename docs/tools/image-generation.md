...
Nova AI records a background task, returns the task id immediately, and wakes
the agent when the provider finishes. The completion agent must send generated
images through the `message` tool; Nova AI does not auto-post a private final
reply as a fallback.
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai-codex` OAuth profile is configured, Nova AI routes image
...
nearby geometry option instead of the exact requested one, Nova AI remaps to
...
Nova AI tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when Nova AI can
  ...
  <Accordion title="Timeouts">
    ...
    timeout budget when configured, bounded by Nova AI's 600000 ms dynamic-tool
    bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai-codex` OAuth profile is configured, Nova AI reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Nova AI
    ...
    Nova AI forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible Nova AI maps
    ...
    transparency-capable OpenAI image model. Nova AI routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    Nova AI forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Nova AI-managed image attachments

    Nova AI intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Nova AI image generation" size=3840x2160 count=1
```
  </Tab>
...
`nova-ai infer image edit`; `--openai-background` remains as an
