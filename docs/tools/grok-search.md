...
Nova AI supports Grok as a `web_search` provider, using xAI web-grounded
...
key under `plugins.entries.xai.config.webSearch.apiKey`, Nova AI now reuses it
...
Nova AI can show a separate follow-up step to enable `x_search` with the same
...
route through an operator proxy or xAI-compatible Responses endpoint. Nova AI
