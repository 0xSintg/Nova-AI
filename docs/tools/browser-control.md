---
summary: "Nova AI browser control API, CLI reference, and scripting actions"
read_when:
  - Scripting or debugging the agent browser via the local control API
  - Looking for the `nova-ai browser` CLI reference
...
This page is the reference for the local control HTTP API, the `nova-ai browser`
...
that override because Nova AI does not launch those browser processes.

If shared-secret gateway auth is configured, browser HTTP routes require auth too:

- `Authorization: Bearer <gateway token>`
- `x-nova-ai-password: <gateway password>` or HTTP Basic auth with that password
...
- Role-style accessibility snapshots (`--interactive`, `--compact`,
  ...
  action engine.
- Page screenshots for the managed `nova-ai` browser when a per-tab CDP
...
Nova AI, then restart the gateway. For Docker, also install the Chromium
...
```bash
NOVA_AI_INSTALL_BROWSER=1 ./scripts/docker/setup.sh
```
...
`NOVA_AI_HOME_VOLUME` or a bind mount. Nova AI auto-detects the persisted
...
nova-ai browser start --headless # one-shot local managed headless launch
nova-ai browser stop            # also clears emulation on attach-only/remote CDP
nova-ai browser tabs
nova-ai browser tab             # shortcut for current tab
nova-ai browser tab new
...
nova-ai browser screenshot --ref 12        # or --ref e12
nova-ai browser screenshot --labels
nova-ai browser snapshot
nova-ai browser snapshot --format aria --limit 200
nova-ai browser snapshot --interactive --compact --depth 6
nova-ai browser snapshot --efficient
nova-ai browser snapshot --labels
nova-ai browser snapshot --urls
nova-ai browser snapshot --selector "#main" --interactive
nova-ai browser snapshot --frame "iframe#main" --interactive
nova-ai browser console --level error
nova-ai browser errors --clear
nova-ai browser requests --filter api --clear
nova-ai browser pdf
nova-ai browser responsebody "**/api" --max-chars 5000
```
...
nova-ai browser click 12 --double           # or e12 for role refs
nova-ai browser click-coords 120 340        # viewport coordinates
nova-ai browser type 23 "hello" --submit
nova-ai browser press Enter
nova-ai browser hover 44
nova-ai browser scrollintoview e12
nova-ai browser drag 10 11
nova-ai browser select 9 OptionA OptionB
nova-ai browser download e12 report.pdf
nova-ai browser waitfordownload report.pdf
nova-ai browser upload /tmp/nova-ai/uploads/file.pdf
nova-ai browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'
nova-ai browser dialog --accept
nova-ai browser dialog --dismiss --dialog-id d1
nova-ai browser wait --text "Done"
nova-ai browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"
...
nova-ai browser highlight e12
nova-ai browser trace start
nova-ai browser trace stop
```
...
nova-ai browser storage local set theme dark
nova-ai browser storage session clear
nova-ai browser set offline on
nova-ai browser set headers --headers-json '{"X-Debug":"1"}'
nova-ai browser set credentials user pass            # --clear to remove
nova-ai browser set geo 37.7749 -122.4194 --origin "https://example.com"
nova-ai browser set media dark
nova-ai browser set timezone America/New_York
nova-ai browser set locale en-US
...
- `upload` and `dialog` are **arming** calls; run them before the click/press that triggers the chooser/dialog. If an action opens a modal, the action response includes `blockedByDialog` and `browserState.dialogs.pending`; pass that `dialogId` to respond directly. Dialogs handled outside Nova AI appear under `browserState.dialogs.recent`.
...
- Download, trace, and upload paths are constrained to Nova AI temp roots: `/tmp/nova-ai{,/downloads,/uploads}` (fallback: `${os.tmpdir()}/nova-ai/...`).
- `upload` can also set file inputs directly via `--input-ref` or `--element`.

Stable tab ids and labels survive Chromium raw-target replacement when Nova AI
...
- `--format aria`: accessibility tree with `axN` refs. When Playwright is available, Nova AI binds refs with backend DOM ids to the live page so follow-up actions can use them; otherwise treat the output as inspection-only.
...
Nova AI supports two "snapshot" styles:

- **AI snapshot (numeric refs)**: `nova-ai browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `nova-ai browser click 12`, `nova-ai browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright's `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `nova-ai browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `nova-ai browser click e12`, `nova-ai browser highlight e12`.
  - Internally, the ref is resolved via `getByRole(...)` (plus `nth()` for duplicates).
  - Add `--labels` to include a viewport screenshot with overlayed `e12` labels.
  - Add `--urls` when link text is ambiguous and the agent needs concrete
    navigation targets.

- **ARIA snapshot (ARIA refs like `ax12`)**: `nova-ai browser snapshot --format aria`
  - Output: the accessibility tree as structured nodes.
  - Actions: `nova-ai browser click ax12` works when the snapshot path can bind
...
- Wait for URL (globs supported by Playwright):
  - `nova-ai browser wait --url "**/dash"`
- Wait for load state:
  - `nova-ai browser wait --load networkidle`
- Wait for a JS predicate:
  - `nova-ai browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `nova-ai browser wait "#main"`
...
1. `nova-ai browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `nova-ai browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `nova-ai browser trace start`
   - reproduce the issue
   - `nova-ai browser trace stop` (prints `TRACE:<path>`)
...
```bash
nova-ai browser status --json
nova-ai browser snapshot --interactive --json
nova-ai browser requests --filter api --json
nova-ai browser cookies --json
```
...
- The nova-ai browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `nova-ai browser evaluate` and `wait --fn`
  execute arbitrary JavaScript in the page context. Prompt injection can steer
  this. Disable it with `browser.evaluateEnabled=false` if you do not need it.
- Use `nova-ai browser evaluate --timeout-ms <ms>` when the page-side function
