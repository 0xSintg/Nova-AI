...
Nova AI agents can generate videos from text prompts, reference images, or
...
Nova AI treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    nova-ai config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. Nova AI submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Nova AI wakes the same session with an internal completion event.
4. The agent tells the user and attaches the finished video through the
   message tool. Nova AI does not auto-post the video as a fallback if the
...
generation. Use `nova-ai tasks list` or `nova-ai tasks show <taskId>` to
...
Generated video files are saved under Nova AI-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, Nova AI
...
```bash
nova-ai tasks list
nova-ai tasks show <taskId>
nova-ai tasks cancel <taskId>
```
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Nova AI normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `480P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Nova AI normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, Nova AI uses `agents.defaults.videoGenerationModel.timeoutMs` if configured.</ParamField>
...
<Note>
Not all providers support all parameters. Nova AI normalizes duration to
...
Nova AI resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@nova-ai/byteplus-modelark`](https://www.npmjs.com/package/@nova-ai/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Nova AI waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Nova AI submits the
...
```bash
NOVA_AI_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `NOVA_AI_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `NOVA_AI_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your Nova AI config:
...
```bash
nova-ai config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
