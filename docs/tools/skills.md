...
Nova AI uses **[AgentSkills](https://agentskills.io)-compatible** skill
folders to teach the agent how to use tools. Each skill is a directory
containing a `SKILL.md` with YAML frontmatter and instructions. Nova AI
...
Nova AI loads skills from these sources, **highest precedence first**:
...
| 4   | Managed/local skills  | `~/.nova-ai/skills`             |
...
Codex CLI's native `$CODEX_HOME/skills` directory is not one of these Nova AI
...
`HOME` separately; Nova AI's own skill roots above already include
`~/.agents/skills`. Use `nova-ai migrate codex --dry-run` to inventory skills
from the Codex home, then `nova-ai migrate codex` to choose skill directories
with an interactive
checkbox prompt before copying them into the current Nova AI agent workspace.
...
| Shared managed/local | `~/.nova-ai/skills`                        | All agents on that machine  |
...
`nova-ai.plugin.json` (paths relative to the plugin root). Plugin skills
...
`metadata.nova-ai.requires.config` on the plugin's config entry.
...
[ClawHub](https://clawhub.ai) is the public skills registry for Nova AI.
Use native `nova-ai skills` commands for discover/install/update, or the
separate `clawhub` CLI for publish/sync workflows. Full guide:
[ClawHub](/clawhub).

| Action                                 | Command                                         |
| -------------------------------------- | ----------------------------------------------- |
| Install a skill into the workspace     | `nova-ai skills install <skill-slug>`          |
| Install a skill for all local agents   | `nova-ai skills install <skill-slug> --global` |
| Update all workspace-installed skills  | `nova-ai skills update --all`                  |
| Update a single shared managed skill   | `nova-ai skills update <skill-slug> --global`  |
| Update all shared managed/local skills | `nova-ai skills update --all --global`         |
| Sync (scan + publish updates)          | `clawhub sync --all`                            |

Native `nova-ai skills install` installs into the active workspace
`skills/` directory by default. Add `--global` to install into the shared
managed/local directory (`~/.nova-ai/skills` by default), which is visible to
...
directory (or falls back to the configured Nova AI workspace). Nova AI picks
...
`nova-ai skills install <slug>` or ClawHub install flow. It is off by default
and only works when `skills.install.allowUploadedArchives: true` is set in
`nova-ai.json`. Upload mode still installs into the default agent workspace
...
`nova-ai skills install <slug>` remains only the install path; publishers
...
- Workspace, project-agent, and extra-dir skill discovery only accepts skill roots whose resolved realpath stays inside the configured root unless `skills.load.allowSymlinkTargets` explicitly trusts a target root. Bundled skills always stay contained. Managed `~/.nova-ai/skills` and personal `~/.agents/skills` roots may contain symlinked skill folders installed by ClawHub or another local skill manager, but every `SKILL.md` realpath must still stay inside its resolved skill directory.
...
- `nova-ai skills install <slug>` is different — it downloads a ClawHub skill folder into the workspace, or into shared managed/local skills with `--global`, and does not use the installer-metadata path above.
...
Nova AI follows the AgentSkills spec for layout/intent. The parser used
...
<ParamField path="homepage" type="string">
  URL surfaced as "Website" in the macOS Skills UI. Also supported via `metadata.nova-ai.homepage`.
</ParamField>
<ParamField path="user-invocable" type="boolean" default="true">
  When `true`, the skill is exposed as a user slash command.
</ParamField>
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Nova AI keeps the skill's instructions out of the agent's normal
...
Nova AI filters skills at load time using `metadata` (single-line JSON):
...
<ParamField path="requires.config" type="string[]">
  List of `nova-ai.json` paths that must be truthy.
</ParamField>
...
If no `metadata.nova-ai` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

<Note>
Legacy `metadata.novabot` blocks are still accepted when
`metadata.nova-ai` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Nova AI lists each entry so you can see the available artifacts.
    - Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
    - Node installs honor `skills.install.nodeManager` in `nova-ai.json` (default: npm; options: npm/pnpm/yarn/bun). This only affects skill installs; the Gateway runtime should still be Node - Bun is not recommended for WhatsApp/Telegram.
    - Gateway-backed installer selection is preference-driven: when install specs mix kinds, Nova AI prefers Homebrew when `skills.install.preferBrew` is enabled and `brew` exists, then `uv`, then the configured node manager, then other fallbacks like `go` or `download`.
    - If every install spec is `download`, Nova AI surfaces all download options instead of collapsing to one preferred installer.

  </Accordion>
  <Accordion title="Per-installer details">
    - **Homebrew installs:** Nova AI does not auto-install Homebrew or translate
    ...
    - **Download installs:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.nova-ai/tools/<skillKey>`).
...
under `skills.entries` in `~/.nova-ai/nova-ai.json`:
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience for skills that declare `metadata.nova-ai.primaryEnv`. Supports plaintext or SecretRef.
</ParamField>
...
defines `metadata.nova-ai.skillKey`, use that key under `skills.entries`.

<Note>
For stock image generation/editing inside Nova AI, use the core
...
When an agent run starts, Nova AI:
...
For the bundled `claude-cli` backend, Nova AI also materializes the same
...
Nova AI still owns precedence, per-agent allowlists, gating, and
`skills.entries.*` env/API key injection. Other CLI backends use the
prompt catalog only.

## Snapshots and refresh

Nova AI snapshots the eligible skills **when a session starts** and
...
session, Nova AI refreshes the snapshot so visible skills stay aligned
with the current agent.

### Skills watcher

By default, Nova AI watches skill folders and bumps the skills snapshot
...
`~/.nova-ai/skills` and personal `~/.agents/skills` can follow skill-directory
...
Nova AI can treat macOS-only skills as eligible when the required
...
probes, Nova AI clears its cached bin matches so agents no longer see
skills that cannot currently run there.

## Token impact

When skills are eligible, Nova AI injects a compact XML list of available
...
Nova AI ships a baseline set of skills as **bundled skills** with the
install (npm package or Nova AI.app). `~/.nova-ai/skills` exists for
