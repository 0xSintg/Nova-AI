...
Nova AI can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `nova-ai configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `nova-ai doctor --fix`.
...
when you run Firecrawl yourself. Nova AI accepts `http://` only for loopback,
...
Nova AI always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Nova AI
