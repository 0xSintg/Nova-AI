---
summary: "Install, configure, and manage Nova AI plugins"
read_when:
...
Plugins extend Nova AI with channels, model providers, agent harnesses, tools,
...
- an Nova AI checkout or installation with the `nova-ai` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    ```bash
    nova-ai plugins search "calendar"
    ```
  ...
  <Step title="Install the plugin">
    ```bash
    # From ClawHub.
    nova-ai plugins install clawhub:<package>

    # From npm.
    nova-ai plugins install npm:<package>

    # From git.
    nova-ai plugins install git:github.com/<owner>/<repo>@<ref>

    # From a local development checkout.
    nova-ai plugins install ./my-plugin
    nova-ai plugins install --link ./my-plugin
    ```
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    nova-ai plugins enable <plugin-id>
    ```
    ...
    `nova-ai plugins install` adds the installed id to an existing
  ...
  <Step title="Let the Gateway reload">
    ...
    enabled, Nova AI detects the changed plugin install record and restarts the
  ...
  <Step title="Verify runtime registration">
    ```bash
    nova-ai plugins inspect <plugin-id> --runtime --json
    ```
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Nova AI-native discovery, scans, version metadata, and install hints | `nova-ai plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `nova-ai plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `nova-ai plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                     | `nova-ai plugins install --link ./my-plugin`                  |
| marketplace | You are installing a Claude-compatible marketplace plugin                      | `nova-ai plugins install <plugin> --marketplace <source>`     |

Bare package specs have special compatibility behavior. If the bare name matches
a bundled plugin id, Nova AI uses that bundled source. If it matches an
official external plugin id, Nova AI uses the official package catalog. Other
...
selection. See [`nova-ai plugins`](/cli/plugins#install) for the full command
contract.
...
Run `nova-ai doctor` or `nova-ai doctor --fix` when config validation reports
stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin paths.

## Understand plugin formats

Nova AI recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Nova AI plugin | `nova-ai.plugin.json` plus a runtime module loaded in process               | You are installing or building Nova AI-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into Nova AI plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `nova-ai plugins list`, `nova-ai plugins inspect`,
`nova-ai plugins enable`, and `nova-ai plugins disable`. See
...
`nova-ai plugins list` and plain `nova-ai plugins inspect` read cold config,
...
```bash
nova-ai gateway status --deep --require-rpc
nova-ai plugins inspect <plugin-id> --runtime --json
nova-ai gateway restart
```
...
sure any manual restart targets the actual `nova-ai gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `nova-ai plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `nova-ai plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `nova-ai doctor --fix` when it points to stale plugin state                                           | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `nova-ai plugins registry --refresh`                    |
| `NOVA_AI_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `nova-ai plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |
...
other channel. Run `nova-ai doctor --fix` to remove stale plugin and channel
...
plugin id. If both plugins are explicitly enabled, Nova AI keeps that request
...
Nova AI needs at runtime. Update or reinstall after the publisher ships
...
and config validation follows with `plugin present but blocked`, Nova AI found
...
Nova AI as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Nova AI config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Nova AI as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/nova-ai-config/npm
```

After fixing ownership, rerun `nova-ai doctor --fix` or
`nova-ai plugins registry --refresh` so the persisted plugin registry matches
the repaired files.
...
```bash
nova-ai config set logging.level trace
nova-ai logs --follow
```
...
Nova AI caches successful plugin tool factory results for repeated resolutions
...
- [`nova-ai plugins`](/cli/plugins) - full CLI reference
