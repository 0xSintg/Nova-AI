...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Nova AI
...
Today this applies to PI embedded runs and Nova AI dynamic tools in the Codex
app-server harness. Tokenjuice hooks Nova AI's tool-result middleware and
...
```bash
nova-ai config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
nova-ai plugins enable tokenjuice
```

Nova AI already ships the plugin. There is no separate `plugins install`
or `tokenjuice install nova-ai` step.
...
```bash
nova-ai config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
nova-ai plugins disable tokenjuice
```
