...
Nova AI supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Nova AI reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.nova-ai/.env`.

If you omit `baseUrl`, Nova AI defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Nova AI defaults to `kimi-k2.6`.
...
Nova AI treats Kimi `web_search` as successful only after Moonshot returns
...
Nova AI returns a structured `kimi_web_search_ungrounded` error instead of
