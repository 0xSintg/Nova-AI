...
Nova AI supports MiniMax as a `web_search` provider through the MiniMax
...
Nova AI also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Nova AI resolves
...
| `count`   | integer | 1-10        | Number of results to return. Nova AI trims the returned list to this size. |
