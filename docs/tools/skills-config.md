...
`~/.nova-ai/nova-ai.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.nova-ai/skills`, `~/.agents/skills`,
...
- `load.allowSymlinkTargets`: trusted real target directories that symlinked
  ...
  `~/.nova-ai/skills` and personal `~/.agents/skills` roots may follow
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `nova-ai setup --node-manager` is narrower and currently accepts `npm`,
...
symlink that resolves outside `<workspace>/skills`, Nova AI skips it and logs
...
layouts. Managed `~/.nova-ai/skills` and personal `~/.agents/skills`
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.nova-ai.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.nova-ai/skills` → bundled skills →
