---
summary: "Export redacted trajectory bundles for debugging an Nova AI agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Nova AI session
...
Trajectory capture is Nova AI's per-session flight recorder. It records a
...
Nova AI writes the bundle under the workspace:

```text
.nova-ai/trajectory-exports/nova-ai-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.nova-ai/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Nova AI sends the
...
```bash
nova-ai sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
```
...
Trajectory capture is on by default for Nova AI agent runs.
...
```json
{
  "traceSchema": "nova-ai-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | Nova AI version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Nova AI also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `NOVA_AI_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export NOVA_AI_TRAJECTORY_DIR=/var/lib/nova-ai/trajectories
```

When this variable is set, Nova AI writes one JSONL file per session id in that
directory.
...
Set `NOVA_AI_TRAJECTORY=0` before starting Nova AI:

```bash
export NOVA_AI_TRAJECTORY=0
```
...
Nova AI flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`NOVA_AI_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Nova AI:

```bash
export NOVA_AI_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Nova AI logs a `pi-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `NOVA_AI_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
Nova AI redacts sensitive values before writing export files:
...
- confirm Nova AI was started without `NOVA_AI_TRAJECTORY=0`
- check whether `NOVA_AI_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.nova-ai/trajectory-exports/`
