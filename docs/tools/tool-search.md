...
read_when:
  ...
  - You want Nova AI tools, MCP tools, and client tools exposed through one compact PI surface
  - You are implementing or debugging tool discovery for PI runs
---

Tool Search is an experimental Nova AI PI-agent feature. It gives PI agents one
...
This page documents Nova AI PI Tool Search. It is not the Codex-native tool
...
`nova-ai.tools` bridge:

```js
const hits = await nova-ai.tools.search("create a GitHub issue");
const tool = await nova-ai.tools.describe(hits[0].id);
return await nova-ai.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include Nova AI tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through Nova AI.

Codex harness runs do not receive these experimental Nova AI Tool Search
controls. Nova AI passes product capabilities to Codex as dynamic tools, and
...
2. List eligible Nova AI and plugin tools.
...
At execution time every real tool call returns to Nova AI. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`nova-ai.tools.call(...)` crosses the bridge back into the Gateway, where the
...
catalog includes eligible Nova AI, MCP, and client tools after normal policy
filtering.
...
`nova-ai.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await nova-ai.tools.search("calendar event", { limit: 5 });
```

`nova-ai.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await nova-ai.tools.describe("mcp:calendar:create_event");
```

`nova-ai.tools.call(id, args)`

Calls a selected tool through Nova AI.

```js
await nova-ai.tools.call(calendarCreate.id, {
  summary: "Planning",
...
network grants, and no child-process or worker grants. Nova AI enforces a
...
- `nova-ai.tools.search`
- `nova-ai.tools.describe`
- `nova-ai.tools.call`

Normal Nova AI behavior still applies to final calls:
...
```bash
nova-ai config set tools.toolSearch true
```
...
- final tool calls executed through Nova AI
...
- whether the result came from Nova AI, MCP, or a client tool
