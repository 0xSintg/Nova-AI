...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, Nova AI best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Manual-spawn delivery resilience">
    - Nova AI hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Nova AI first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, Nova AI falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
    - If the requester-agent completion handoff fails or produces no visible output, Nova AI treats delivery as failed and falls back to queue routing/retry. It does not raw-send the child result directly to the external chat.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Nova AI hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Nova AI config agents from `agents_list`.
...
- **Run timeout:** if `sessions_spawn.runTimeoutSeconds` is omitted, Nova AI uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
When active children exist, Nova AI injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    Nova AI creates or binds a thread to that session target in the active channel.
  </Step>
...
- If a nested requester subagent session is gone, Nova AI falls back to that session's requester when available.
...
target agent first. After that, Nova AI applies the sub-agent restriction
layer.
...
Nova AI does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, Nova AI persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`nova-ai tasks maintenance --apply` to reconcile the task record, or
`nova-ai doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
