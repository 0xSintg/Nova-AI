...
read_when:
  - Running Nova AI Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Nova AI Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/nova-ai/nova-ai/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Nova AI and Chrome are on the same machine
...
- Nova AI points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Nova AI is not the problem yet.
...
Fix that before touching Nova AI config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Nova AI at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Nova AI to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Nova AI to succeed
...
```bash
nova-ai browser open https://example.com --browser-profile remote
nova-ai browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `nova-ai browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `nova-ai browser stop --browser-profile remote`
...
3. Nova AI config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Nova AI config or Control UI auth
