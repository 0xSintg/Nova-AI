...
read_when:
  ...
  - Enabling the plugin-tools or Nova AI-tools MCP bridge
...
When Nova AI uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Nova AI `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Nova AI before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Nova AI/acpx cannot force a model selection.
...
Packaged installs use the official `@nova-ai/acpx` runtime plugin for ACP.
...
```bash
nova-ai plugins install ./path/to/local/acpx-plugin
```
...
`ready` signal. Set `NOVA_AI_ACPX_RUNTIME_STARTUP_PROBE=0` or
`NOVA_AI_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `command` accepts an absolute path, relative path (resolved from the Nova AI workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before Nova AI passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Nova AI globally with `npm install -g nova-ai`, the acpx
...
normally and reports the missing dependency through `nova-ai acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Nova AI plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Nova AI plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
nova-ai config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `nova-ai-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Nova AI
  plugins.
...
### Nova AI tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Nova AI tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
nova-ai config set plugins.entries.acpx.config.openClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `nova-ai-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Nova AI tools. The initial server exposes `cron`.
...
```bash
nova-ai config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use Nova AI agent/run timeouts, including `/acp timeout` and
...
```bash
nova-ai config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Nova AI exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
nova-ai config set plugins.entries.acpx.config.permissionMode approve-all
nova-ai config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Nova AI defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
