...
host-local `ask: "always"` in `~/.nova-ai/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `nova-ai approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `nova-ai exec-policy show`                                      | Local-machine merged view.                                                             |
| `nova-ai exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `nova-ai` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Nova AI also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.nova-ai/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, Nova AI treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  Nova AI may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in Nova AI config
...
- In YOLO mode, Nova AI does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when Nova AI's requested exec
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    nova-ai approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
nova-ai exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.nova-ai/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `nova-ai approvals set --gateway` or
`nova-ai approvals set --node <id|name|ip>`.
...
```bash
nova-ai approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `nova-ai exec-policy` does not synchronize node approvals.
- `nova-ai exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `nova-ai approvals --node ...`.
...
specific argument shape. Nova AI evaluates the regular expression
...
entries instead of hand-editing the encoded value. If Nova AI cannot
...
edit its local `~/.nova-ai/exec-approvals.json` directly.

CLI: `nova-ai approvals` supports gateway or node editing - see
...
When an async exec approval is denied, Nova AI prevents the agent from
