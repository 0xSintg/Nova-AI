...
- allowlist entries live in host-local `~/.nova-ai/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `nova-ai approvals allowlist ...`).
- `nova-ai security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `nova-ai doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Nova AI still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, Nova AI sends a followup `agent` turn to the same session.
...
the originating channel, Nova AI keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, Nova AI sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.nova-ai.approval` custom event content on the first prompt
  event so Nova AI-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. Nova AI first tries a private route on the
...
topics, Nova AI preserves the topic for the approval prompt and the post-approval follow-up.
