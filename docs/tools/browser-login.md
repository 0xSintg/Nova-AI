...
When a site requires login, **sign in manually** in the **host** browser profile (the nova-ai browser).
...
Nova AI controls a **dedicated Chrome profile** (named `nova-ai`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `nova-ai` browser.
...
nova-ai browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `nova-ai`).
...
```bash
nova-ai browser open https://x.com --browser-profile nova-ai
```
