...
Nova AI supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.nova-ai/.env`.
...
`pd`, `pw`, `pm`, and `py`. Nova AI converts these values, or an explicit
