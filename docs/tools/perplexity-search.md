...
Nova AI supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Nova AI also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `nova-ai configure --section web`. It stores the key in
`~/.nova-ai/nova-ai.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.nova-ai/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
