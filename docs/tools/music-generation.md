...
For session-backed agent runs, Nova AI starts music generation as a
...
the message tool; Nova AI does not auto-post the file as a fallback if the
...
<Note>
Not all providers support all parameters. Nova AI still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Nova AI
...
Provider request timeouts are operator configuration only. Nova AI uses
...
- **Status lookup:** `nova-ai tasks list` or `nova-ai tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Nova AI injects an internal completion event back
...
```bash
NOVA_AI_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
