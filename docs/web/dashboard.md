...
- Re-open anytime: `nova-ai dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
- If clipboard and browser delivery fail, `nova-ai dashboard` still prints the
  clean URL and tells you to use the token from `NOVA_AI_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `NOVA_AI_GATEWAY_TOKEN`); `nova-ai dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `nova-ai dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `nova-ai dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `NOVA_AI_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `nova-ai status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `nova-ai config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `NOVA_AI_GATEWAY_TOKEN` in this shell, then rerun `nova-ai dashboard`
  - No shared secret configured: `nova-ai doctor --generate-gateway-token`
