...
read_when:
  - Deciding how to automate work with Nova AI
  - Choosing between heartbeat, cron, commitments, hooks, and standing orders
  - Looking for the right automation entry point
title: "Automation"
---

Nova AI runs work in the background through tasks, scheduled jobs, inferred
...
| Audit what ran and when                 | Background Tasks       | `nova-ai tasks list` and `nova-ai tasks audit` |
...
The background task ledger tracks all detached work: ACP runs, subagent spawns, isolated cron executions, and CLI operations. Tasks are records, not schedulers. Use `nova-ai tasks list` and `nova-ai tasks audit` to inspect them.

See [Background Tasks](/automation/tasks).

### Inferred commitments

Commitments are opt-in, short-lived follow-up memories. Nova AI infers them
...
Task Flow is the flow orchestration substrate above background tasks. It manages durable multi-step flows with managed and mirrored sync modes, revision tracking, and `nova-ai tasks flow list|show|cancel` for inspection.
...
with `nova-ai hooks`. For in-process tool-call interception, use
