...
```bash
NOVA_AI_DIAGNOSTICS=telegram.http,telegram.payload
```

Disable all flags:

```bash
NOVA_AI_DIAGNOSTICS=0
```
...
NOVA_AI_DIAGNOSTICS_TIMELINE_PATH=/tmp/nova-ai-timeline.jsonl \
nova-ai gateway run
```
...
`NOVA_AI_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because Nova AI has
not read config yet; subsequent startup spans use the config flag.

`NOVA_AI_DIAGNOSTICS=1`, `NOVA_AI_DIAGNOSTICS=all`, and
`NOVA_AI_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `nova-ai.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/nova-ai/nova-ai-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/nova-ai/nova-ai-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/nova-ai/nova-ai-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/nova-ai/nova-ai-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `nova-ai logs --follow` (see [/cli/logs](/cli/logs)).
