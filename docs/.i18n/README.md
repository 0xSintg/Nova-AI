...
- repo: `nova-ai/docs`
- local checkout: `~/Projects/nova-ai-docs`

## Source of truth

- English docs are authored in `nova-ai/nova-ai`.
...
1. Edit English docs in `nova-ai/nova-ai`.
2. Push to `main`.
3. `nova-ai/nova-ai/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `nova-ai/docs`.
...
5. `nova-ai/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `nova-ai/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `nova-ai/docs` instead.
...
- Sync metadata is written to `.nova-ai-sync/source.json` in the publish repo.
- Source repo secret: `NOVA_AI_DOCS_SYNC_TOKEN`
- Publish repo secret: `NOVA_AI_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `nova-ai/docs` first.
