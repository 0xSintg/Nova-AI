...
- `consultThinkingLevel`: optional thinking level override for the full Nova AI agent run behind realtime `nova-ai_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `nova-ai_agent_consult` calls.
...
- `realtime.instructions`: appends provider-facing system instructions to Nova AI's built-in realtime prompt. Use it for voice style and tone; Nova AI keeps the default `nova-ai_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `nova-ai_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `nova-ai-mlx-tts` helper when present, or an executable on `PATH`. Set `NOVA_AI_MLX_TTS_BIN` to point at a custom helper binary during development.
