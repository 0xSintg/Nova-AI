...
Nova AI supports **camera capture** for agent workflows:
...
```bash
nova-ai nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
nova-ai nodes camera snap --node <id> --facing front
nova-ai nodes camera clip --node <id> --duration 3000
nova-ai nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`nova-ai.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `nova-ai` CLI to invoke camera commands on the macOS node.

Examples:

```bash
nova-ai nodes camera list --node <id>            # list camera ids
nova-ai nodes camera snap --node <id>            # prints MEDIA:<path>
nova-ai nodes camera snap --node <id> --max-width 1280
nova-ai nodes camera snap --node <id> --delay-ms 2000
nova-ai nodes camera snap --node <id> --device-id <id>
nova-ai nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
nova-ai nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
nova-ai nodes camera clip --node <id> --device-id <id>
nova-ai nodes camera clip --node <id> --no-audio
```

Notes:

- `nova-ai nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
nova-ai nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
