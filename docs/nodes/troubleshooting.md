...
```bash
nova-ai nodes describe --node <idOrNameOrIp>
nova-ai nodes canvas snapshot --node <idOrNameOrIp>
nova-ai logs --follow
```
...
nova-ai nodes status
nova-ai approvals get --node <idOrNameOrIp>
nova-ai approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`nova-ai approvals get --node ...`), not in the gateway pairing record.
