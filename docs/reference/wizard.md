...
This is the full reference for `nova-ai onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.nova-ai/nova-ai.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `nova-ai doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Nova AI now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.nova-ai/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.nova-ai/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.nova-ai/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$NOVA_AI_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.nova-ai/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `nova-ai pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `nova-ai configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `nova-ai health`.
    - Tip: `nova-ai status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
- Stores it under `~/.nova-ai/tools/signal-cli/<version>/`.
...
Typical fields in `~/.nova-ai/nova-ai.json`:
...
`nova-ai agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.nova-ai/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.nova-ai/agents/<agentId>/sessions/`.
