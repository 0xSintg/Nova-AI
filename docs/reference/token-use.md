---
summary: "How Nova AI builds prompt context and reports token usage + costs"
read_when:
...
Nova AI tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Nova AI assembles its own system prompt on every run. It includes:
...
For images, Nova AI downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Nova AI session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `nova-ai status --usage` and `nova-ai channels list` show
...
available; otherwise Nova AI falls back to matching OAuth/API-key credentials
...
Nova AI keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Nova AI shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Nova AI starts an
...
Provider prompt caching only applies within the cache TTL window. Nova AI can
...
Anthropic's 1M context window is currently beta-gated. Nova AI can inject the
...
Nova AI skips the `context-1m-*` beta header because Anthropic currently
