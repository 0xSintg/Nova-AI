...
Nova AI has three public release lanes:
...
- Every stable Nova AI release ships the npm package and macOS app together;
...
6. Run `Nova AI NPM Release` with `preflight_only=true`. Before a tag exists,
...
vYYYY.M.D-beta.N` from the matching `release/YYYY.M.D` branch. The helper runs
   ...
   `Nova AI Release Publish` command only after the evidence bundle is green.
   `Nova AI Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to ClawHub in parallel, and then promotes the
   prepared Nova AI npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the Nova AI npm publish child succeeds, it creates or updates the
   ...
   `nova-ai-<version>-dependency-evidence.zip` for post-release incident
   ...
   evidence as soon as Nova AI npm publish succeeds, waits for ClawHub whenever
   Nova AI npm is being published, then runs `pnpm release:verify-beta` and
   ...
   `nova-ai@YYYY.M.D-beta.N` or
   `nova-ai@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `Nova AI Release Publish`, reusing the successful preflight artifact via
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `Nova AI Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `nova-ai@beta`, `nova-ai@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=nova-ai@beta -f suite_profile=product -f published_upgrade_survivor_baseline=nova-ai@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- `Nova AI NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `nova-ai-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `nova-ai-<version>-dependency-evidence.zip`.
- Run `Nova AI Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.D` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful Nova AI npm
  ...
  workflow serializes plugin npm publish, plugin ClawHub publish, and Nova AI
  npm publish so the core package is not published before its externalized
  plugins.
- Release checks now run in a separate manual workflow:
  `Nova AI Release Checks`
- `Nova AI Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `Nova AI Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/nova-ai-cross-os-release-checks-reusable.yml` directly
...
- `Nova AI Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an Nova AI branch or release tag
- `Nova AI NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `NOVA_AI_LIVE_TEST=1 NOVA_AI_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:nova-ai:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/nova-ai-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/nova-ai-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `NOVA_AI_NPM_TELEGRAM_PACKAGE_SPEC=nova-ai@YYYY.M.D-beta.N NOVA_AI_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex NOVA_AI_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `NOVA_AI_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `nova-ai/releases-private/.github/workflows/nova-ai-npm-dist-tags.yml`
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=nova-ai@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `Nova AI Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `Nova AI Release
...
`Nova AI Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=nova-ai@YYYY.M.D-beta.N`
...
The cross-OS OpenAI install smoke uses `NOVA_AI_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=nova-ai@YYYY.M.D-beta.N \
  -f evidence_package_spec=nova-ai@YYYY.M.D-beta.N \
...
required Nova AI dynamic tools drift or disappear from the standard tier
summary.
...
The Docker box lives in `Nova AI Release Checks` through
`nova-ai-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `Nova AI Release Checks`. It is the agentic
...
`scripts/resolve-nova-ai-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `nova-ai@beta`, `nova-ai@latest`, or an exact Nova AI release
  version
...
`Nova AI Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=nova-ai@2026.4.26
```
...
`Nova AI Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `Nova AI NPM Release` with the release tag, npm dist-tag, and
...
`plugin_publish_scope=selected` and `plugins=@nova-ai/name` to
`Nova AI Release Publish`, or dispatch the child workflow directly when the
Nova AI package must not be published.

## NPM workflow inputs

`Nova AI NPM Release` accepts these operator-controlled inputs:
...
`Nova AI Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Nova AI NPM Release` preflight run id;
  required when `publish_nova-ai_npm=true`
- `npm_dist_tag`: npm target tag for the Nova AI package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused repair work
- `plugins`: comma-separated `@nova-ai/*` package names when
  `plugin_publish_scope=selected`
- `publish_nova-ai_npm`: defaults to `true`; set `false` only when using the
...
`Nova AI Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an Nova AI branch or
  release tag.
...
- For `Nova AI NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `Nova AI Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `Nova AI NPM Release` with `preflight_only=true`
...
6. Run `Nova AI Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ClawHub before promoting the Nova AI npm package
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/nova-ai/nova-ai/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/nova-ai/nova-ai/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/nova-ai-npm-release.yml`](https://github.com/nova-ai/nova-ai/blob/main/.github/workflows/nova-ai-npm-release.yml)
- [`.github/workflows/nova-ai-release-checks.yml`](https://github.com/nova-ai/nova-ai/blob/main/.github/workflows/nova-ai-release-checks.yml)
- [`.github/workflows/nova-ai-cross-os-release-checks-reusable.yml`](https://github.com/nova-ai/nova-ai/blob/main/.github/workflows/nova-ai-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-nova-ai-package-candidate.mjs`](https://github.com/nova-ai/nova-ai/blob/main/scripts/resolve-nova-ai-package-candidate.mjs)
- [`scripts/nova-ai-npm-release-check.ts`](https://github.com/nova-ai/nova-ai/blob/main/scripts/nova-ai-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/nova-ai/nova-ai/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/nova-ai/nova-ai/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`nova-ai/maintainers/release/README.md`](https://github.com/nova-ai/maintainers/blob/main/release/README.md)
