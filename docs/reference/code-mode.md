---
summary: "Nova AI code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable Nova AI code mode for an agent run
...
Code mode is an experimental Nova AI agent-runtime feature. It is off by
default. When you enable it, Nova AI changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents Nova AI code mode. It is not Codex Code mode. Codex Code
...
dynamic tool search are stable Codex harness surfaces. Nova AI code mode is an
Nova AI-owned experimental tool-surface adapter for generic Nova AI runs. It
uses `quickjs-wasi`, a hidden Nova AI tool catalog, and the normal Nova AI
tool executor.

## What is this?

Nova AI code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal Nova AI tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same Nova AI execution path used by normal agent turns.
...
surface. It does not replace Nova AI tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for Nova AI, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through Nova AI
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, Nova AI fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
NOVA_AI_DEBUG_CODE_MODE=1 \
NOVA_AI_DEBUG_MODEL_TRANSPORT=1 \
NOVA_AI_DEBUG_MODEL_PAYLOAD=tools \
nova-ai gateway
```
...
`NOVA_AI_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental Nova AI surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic Nova AI agent runs.
...
**Code mode** is the Nova AI runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, Nova AI fails closed for
...
2. Build the effective Nova AI tool list.
...
- If `language` is `"typescript"`, Nova AI transpiles before evaluation.
...
final value is JSON-compatible after Nova AI's output adapter runs.
...
`wait` exists because nested Nova AI tools can be slow, interactive, approval
...
2. On suspension, Nova AI snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. Nova AI re-registers host callbacks by stable names.
5. Nova AI delivers nested tool results into the restored VM.
6. Nova AI drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "nova-ai" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "Nova AI code mode" });
```
...
- images and files travel through ordinary Nova AI tools, not through the
  code-mode bridge
...
- Nova AI does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- The guest runtime receives compact `ALL_TOOLS` metadata and search, describe,
  and call helpers.
- Nested calls dispatch through the same Nova AI executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the PI compact
catalog bridge. Code mode is the generic Nova AI alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal Nova AI
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("nova-ai:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, Nova AI omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters Nova AI.
...
Nova AI loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside Nova AI's main event loop in a worker. A guest
...
inputs beyond existing Nova AI trajectory policy.
...
NOVA_AI_DEBUG_MODEL_TRANSPORT=1 \
NOVA_AI_DEBUG_MODEL_PAYLOAD=tools \
NOVA_AI_DEBUG_SSE=events \
nova-ai gateway
```

For payload-shape debugging, use `NOVA_AI_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `NOVA_AI_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for Nova AI tools
...
5. Send an agent turn with Nova AI, plugin, MCP, and client test tools.
