---
summary: "Reference design for the public Nova AI App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "Nova AI App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public Nova AI app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level Nova AI App SDK wrapper
---

This page is the detailed API reference design for the public
[Nova AI App SDK](/concepts/nova-ai-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@nova-ai/sdk` is the external app/client package for talking to the
  Gateway. `nova-ai/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `Nova AI`, `Agent`, `Session`, `Run`,
...
- source: Nova AI, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@nova-ai/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@nova-ai/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@nova-ai/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `nova-ai/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `Nova AI`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [Nova AI App SDK](/concepts/nova-ai-sdk)
