...
Nova AI manages sessions end-to-end across these areas:
...
Nova AI is designed around a single **Gateway process** that owns session state.
...
Nova AI persists sessions in two layers:
...
- Transcripts: `~/.nova-ai/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Nova AI resolves these via `src/config/sessions.ts`.
...
Nova AI no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `nova-ai doctor --fix` removes it from older configs.
...
`NOVA_AI_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS`, `NOVA_AI_SESSION_WRITE_LOCK_STALE_MS`, and
`NOVA_AI_SESSION_WRITE_LOCK_MAX_HOLD_MS`.
...
In `mode: "warn"`, Nova AI reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
nova-ai sessions cleanup --dry-run
nova-ai sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.nova-ai/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- **Parent fork policy** uses PI's active branch when creating a thread or subagent fork. If that branch is too large, Nova AI starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `nova-ai doctor --fix`.
...
Nova AI intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Nova AI splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Nova AI
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Nova AI preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Nova AI consumes the events, but Pi decides when to compact).

Nova AI can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: Nova AI still runs normal semantic compaction,
...
next model call, Nova AI estimates the prompt pressure using the same preflight
...
Nova AI also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Nova AI bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, Nova AI leaves it alone.
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  Nova AI rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, Nova AI falls back to built-in LLM summarization automatically.
...
- `nova-ai status` (CLI)
- `nova-ai sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `nova-ai logs --follow`): `embedded run auto-compaction start` + `complete`
...
Nova AI supports "silent" turns for background tasks where the user should not see intermediate output.
...
- Nova AI strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Nova AI also suppresses **draft/typing streaming** when a
...
Nova AI uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Nova AI's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `nova-ai status`.
