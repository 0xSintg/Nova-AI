...
An owner is a ClawHub publisher handle, such as `@alice` or `@nova-ai`.
...
`@nova-ai/dronzer`, it can only be published as `@nova-ai`. If you publish as
...
```text
Package scope "@nova-ai" must match selected owner "@vintageayu".
Publish as "@nova-ai" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-nova-ai --to opik
```
...
This protects org namespaces. A package named `@nova-ai/dronzer` claims the
`@nova-ai` namespace, so only publishers with access to the `@nova-ai` owner
can publish it.
