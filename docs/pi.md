---
summary: "Architecture of Nova AI's embedded Pi agent integration and session lifecycle"
title: "Pi integration architecture"
read_when:
  - Understanding Pi SDK integration design in Nova AI
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

Nova AI integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Nova AI uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Nova AI directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Nova AI's local TUI mode)                                             |
...
├── pi-tools.ts                    # createNova AICodingTools()
...
├── nova-ai-tools.ts              # Nova AI-specific tools
...
Image injection is prompt-local: Nova AI loads image refs from the current prompt and
...
2. **Custom Replacements**: Nova AI replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Nova AI Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Nova AI wraps this with `guardSessionManager()` for tool result safety.
...
Nova AI maintains an auth profile store with multiple API keys per provider:
...
Nova AI loads custom pi extensions for specialized behavior:
...
Nova AI also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Nova AI Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Nova AI tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.nova-ai/agents/<agentId>/sessions/` (or `$NOVA_AI_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `NOVA_AI_LIVE_TEST=1`)
