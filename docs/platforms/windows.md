...
read_when:
  - Installing Nova AI on Windows
  - Choosing between native Windows and WSL2
  - Looking for Windows companion app status
title: "Windows"
---

Nova AI supports both **native Windows** and **WSL2**. WSL2 is the more
...
- local CLI use such as `nova-ai --version`, `nova-ai doctor`, and `nova-ai plugins list --json`
- embedded local-agent/provider smoke such as:

```powershell
nova-ai agent --local --agent main --thinking low -m "Reply with exactly WINDOWS-HATCH-OK."
```

Current caveats:

- `nova-ai onboard --non-interactive` still expects a reachable local gateway unless you pass `--skip-health`
- `nova-ai onboard --non-interactive --install-daemon` and `nova-ai gateway install` try Windows Scheduled Tasks first
- if Scheduled Task creation is denied, Nova AI falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, Nova AI now aborts that path quickly and falls back instead of hanging forever
- Scheduled Tasks are still preferred when available because they provide better supervisor status

If you want the native CLI only, without gateway service install, use one of these:

```powershell
nova-ai onboard --non-interactive --skip-health
nova-ai gateway run
```
...
### 2) Install the Nova AI gateway user service
...
```bash
systemctl --user is-enabled nova-ai-gateway.service
systemctl --user status nova-ai-gateway.service --no-pager
```
...
pnpm build
pnpm ui:build
pnpm nova-ai onboard --install-daemon
```
