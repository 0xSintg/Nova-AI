...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `nova-ai.internal.`), **or**
...
Run `nova-ai devices list` again before approval.
...
nova-ai gateway call node.list --params "{}"
```
...
- `NOVA_AI_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Nova AI iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Nova AI builds.
...
```bash
export NOVA_AI_APNS_TEAM_ID="TEAMID"
export NOVA_AI_APNS_KEY_ID="KEYID"
export NOVA_AI_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.nova-ai/credentials/apns
chmod 700 ~/.nova-ai/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.nova-ai/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.nova-ai/credentials/apns/AuthKey_KEYID.p8
export NOVA_AI_APNS_PRIVATE_KEY_PATH="$HOME/.nova-ai/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_nova-ai-gw._tcp` on `local.` and, when configured, the same
...
`nova-ai.internal.`) and Tailscale split DNS.
...
```bash
nova-ai nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__nova-ai__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__nova-ai__/canvas/` and `/__nova-ai__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Nova AI node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Nova AI by invoking node
...
```bash
nova-ai nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__nova-ai; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
nova-ai nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `nova-ai devices list` and approve manually.
