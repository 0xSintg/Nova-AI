---
summary: "macOS app flow for controlling a remote Nova AI gateway"
read_when:
  - Setting up or debugging remote mac control
title: "Remote control"
---

This flow lets the macOS app act as a full remote control for an Nova AI gateway running on another host (desktop/server). The app can connect directly to trusted LAN/Tailnet gateway URLs or manage an SSH tunnel when the remote gateway is loopback-only. Health checks, Voice Wake forwarding, and Web Chat reuse the same remote configuration from _Settings → General_.

## Modes

- **Local (this Mac)**: Everything runs on the laptop. No SSH involved.
- **Remote over SSH (default)**: Nova AI commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
`nova-ai node install ...` and `nova-ai node start` (or run
`nova-ai node run ...` in the foreground), then target that browser-capable
node.

## Prereqs on the remote host

1. Install Node + pnpm and build/install the Nova AI CLI (`pnpm install && pnpm build && pnpm link --global`).
2. Ensure `nova-ai` is on PATH for non-interactive shells (symlink into `/usr/local/bin` or `/opt/homebrew/bin` if needed).
...
```bash
nova-ai-mac configure-remote \
  --ssh-target user@gateway.local \
  --local-port 18789 \
  --remote-port 18789 \
  --token "$NOVA_AI_GATEWAY_TOKEN"
```
...
2. Under **Nova AI runs**, pick **Remote** and set:
   ...
   - **CLI path** (advanced): optional path to a runnable `nova-ai` entrypoint/binary (auto-filled when advertised).
3. Hit **Test remote**. Success indicates the remote `nova-ai status --json` runs correctly. Failures usually mean PATH/CLI issues; exit 127 means the CLI isn't found remotely.
...
- Run `nova-ai channels login --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
- Re-run login on that host if auth expires. Health check will surface link problems.

## Troubleshooting

- **exit 127 / not found**: `nova-ai` isn't on PATH for non-login shells. Add it to `/etc/paths`, your shell rc, or symlink into `/usr/local/bin`/`/opt/homebrew/bin`.
- **Health probe failed**: check SSH reachability, PATH, and that Baileys is logged in (`nova-ai status --json`).
...
Pick sounds per notification from scripts with `nova-ai` and `node.invoke`, e.g.:

```bash
nova-ai nodes notify --node <id> --title "Ping" --body "Remote gateway ready" --sound Glass
```
