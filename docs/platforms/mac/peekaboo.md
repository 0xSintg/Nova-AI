...
read_when:
  - Hosting PeekabooBridge in Nova AI.app
...
Nova AI can host **PeekabooBridge** as a local, permission-aware UI automation
broker. This lets the `peekaboo` CLI drive UI automation while reusing the
macOS app's TCC permissions.

## What this is (and is not)

- **Host**: Nova AI.app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate `nova-ai ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Nova AI is a thin broker host.

## Relationship to Computer Use

Nova AI has three desktop-control paths, and they intentionally stay separate:

- **PeekabooBridge host**: Nova AI.app can host the local PeekabooBridge socket.
  The `peekaboo` CLI remains the client and uses Nova AI.app's macOS
...
- **Codex Computer Use**: the bundled `codex` plugin prepares Codex app-server,
  ...
  Codex own native desktop-control tool calls during Codex-mode turns. Nova AI
  does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Nova AI can register TryCua's upstream
...
Use Peekaboo when you want the broad macOS automation surface and Nova AI.app's
...
when you want the CUA driver exposed to any Nova AI-managed runtime as a normal
MCP server.
...
When enabled, Nova AI starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Nova AI app TeamID).
...
- If `peekaboo` reports "bridge client is not authorized", ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or Nova AI.app)
