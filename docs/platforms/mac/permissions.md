...
- Same path: run the app from a fixed location (for Nova AI, `dist/Nova AI.app`).
...
```bash
sudo tccutil reset Accessibility ai.nova-ai.mac
sudo tccutil reset ScreenCapture ai.nova-ai.mac
...
Workaround: move files into the Nova AI workspace (`~/.nova-ai/workspace`) if you want to avoid per-folder grants.
