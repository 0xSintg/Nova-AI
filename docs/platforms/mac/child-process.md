...
service via the external `nova-ai` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.nova-ai.gateway`
  (or `ai.nova-ai.<profile>` when using `--profile`/`NOVA_AI_PROFILE`; legacy `com.nova-ai.*` is supported).
