...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.nova-ai.app&hl=en_IN). It is a companion node and requires a running Nova AI Gateway. The source code is also available in the [Nova AI repository](https://github.com/nova-ai/nova-ai) under `apps/android`; see [apps/android/README.md](https://github.com/nova-ai/nova-ai/blob/main/apps/android/README.md) for build instructions.
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.nova-ai.app&hl=en_IN) for the app, [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
- You can run the CLI (`nova-ai`) on the gateway machine (or via SSH).
...
1. Set up a DNS-SD zone (example `nova-ai.internal.`) on the gateway host and publish `_nova-ai-gw._tcp` records.
...
1. Create `~/.nova-ai/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
nova-ai nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__nova-ai__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__nova-ai__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__nova-ai__/a2ui/`.
...
Android supports launching Nova AI from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Nova AI..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Nova AI as the default assistant app.
</Note>
