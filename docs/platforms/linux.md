...
2. `npm i -g nova-ai@latest`
3. `nova-ai onboard --install-daemon`
...
Nova AI installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `nova-ai gateway install` and
`nova-ai onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/nova-ai-gateway[-<profile>].service`:

```
[Unit]
Description=Nova AI Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Nova AI therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Nova AI starts the child through a short
...
- Nova AI-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `NOVA_AI_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
