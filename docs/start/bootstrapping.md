...
On the first agent run, Nova AI bootstraps the workspace (default
`~/.nova-ai/workspace`):
...
For embedded/local model runs, Nova AI keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `nova-ai onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.nova-ai/workspace`).
</Note>
