...
CLI onboarding is the **recommended** way to set up Nova AI on macOS,
...
`NOVA_AI_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `Nova AI`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`nova-ai dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`nova-ai configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.nova-ai/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `nova-ai doctor` first.
</Note>
...
Use `nova-ai agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.nova-ai/workspace-<agentId>`.
...
- CLI command reference: [`nova-ai onboard`](/cli/onboard)
