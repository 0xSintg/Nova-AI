...
summary: "Specification for making the bundled Codex app-server harness honor Nova AI context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Nova AI context-engine
...
Nova AI-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Nova AI-controlled code. The external
boundary is the Codex app-server protocol itself: Nova AI can control what it
...
For Codex harness turns, Nova AI should preserve this lifecycle:

1. Read the mirrored Nova AI session transcript.
...
8. Mirror the Codex result back into the Nova AI transcript.
...
11. Preserve Codex native compaction signals and Nova AI compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Nova AI should
not try to mutate the app-server's internal history except through supported
protocol calls.

Nova AI's transcript mirror remains the source for Nova AI features:
...
The context-engine interface returns Nova AI `AgentMessage[]`, not a Codex
...
- Label it clearly as Nova AI assembled context.
...
```text
Nova AI assembled context for this turn:
...
inside Nova AI and preserves context-engine semantics.
...
1. Nova AI context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Nova AI compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Nova AI compaction should prefer the context engine's `compact()` result for
the Nova AI transcript mirror and plugin state.
...
the Nova AI session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Nova AI session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Nova AI compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Nova AI core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Nova AI/plugin state.
