...
read_when:
  ...
  - Explaining how Nova AI updates one channel message while work is in progress
...
When progress drafts are enabled, Nova AI creates one visible work-in-progress
...
That is usually enough. Nova AI will pick an automatic one-word label, wait
...
Nova AI sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from Nova AI's built-in
...
Nova AI uses the same formatter for progress drafts and `/verbose`:
...
Nova AI truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, Nova AI still suppresses the older standalone
...
When the final answer is ready, Nova AI tries to keep the chat clean:

- If the draft can safely become the final answer, Nova AI edits it in place.
- If the channel uses native progress streaming, Nova AI finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, Nova AI sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, Nova AI keeps the
