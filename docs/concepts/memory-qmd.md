...
alongside Nova AI. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Nova AI falls back to the
...
Nova AI creates a self-contained QMD home under
`~/.nova-ai/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Nova AI creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so Nova AI skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, Nova AI retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, Nova AI groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, Nova AI falls back to the builtin SQLite engine.
  ...
  `nova-ai memory status` and one-shot CLI probes still recheck QMD directly.
...
Nova AI keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Nova AI checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Nova AI searches all
...
Older QMD builds only accept one collection filter. When Nova AI detects one
...
process, so you can tune QMD globally without adding new Nova AI config:
...
collection under `~/.nova-ai/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Nova AI logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Nova AI
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Nova AI still reports
...
with `nova-ai memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Nova AI uses.

**Many QMD subprocesses during search?** Update QMD if possible. Nova AI uses
...
`memory.qmd.searchMode = "search"`. Nova AI treats that mode as lexical-only,
...
the next startup reconciliation. Nova AI recreates stale managed collections
...
Nova AI's builtin symlink rules. Keep temporary monorepo checkouts under
