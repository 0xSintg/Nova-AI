...
read_when:
  - You want Nova AI to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, Nova AI can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. Nova AI may check in afterward.
- You say you are exhausted. Nova AI may ask later whether you slept.
- The agent says it will follow up after something changes. Nova AI may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: Nova AI remembers a
...
```bash
nova-ai config set commitments.enabled true
nova-ai config set commitments.maxPerDay 3
```

Equivalent `nova-ai.json`:
...
After an agent reply, Nova AI may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, Nova AI stores a commitment with:
...
without Nova AI tools.

Nova AI never delivers an inferred commitment immediately after writing it.
...
nova-ai commitments dismiss cm_abc123
```

See [`nova-ai commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local Nova AI state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
nova-ai config set commitments.enabled false
```
...
- Check `nova-ai commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`nova-ai commitments`](/cli/commitments)
