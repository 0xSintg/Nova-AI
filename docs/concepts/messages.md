...
Nova AI handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. Nova AI keeps a
...
Nova AI keeps that boundary explicit:
...
Nova AI separates the **prompt body** from the **command body**:
...
Nova AI can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, Nova AI
strips the silent text but still delivers the media attachment.
Nova AI resolves that behavior by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct
  run accidentally returns a bare silent token, Nova AI suppresses it instead
...
Nova AI also uses silent replies for internal runner failures that happen
