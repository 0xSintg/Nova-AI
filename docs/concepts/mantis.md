---
summary: "Mantis is the visual end-to-end verification system for reproducing Nova AI bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for Nova AI bugs
...
Mantis is the Nova AI end-to-end verification system for bugs that need a real
...
Mantis lives in the Nova AI QA stack.

- Nova AI owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm nova-ai qa mantis`.
...
- Nova AI agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in Nova AI, machine scheduling in
...
```bash
pnpm nova-ai qa mantis discord-smoke \
...
thread, calls Nova AI's `message.thread-reply` action with a repo-local
...
```bash
pnpm nova-ai qa mantis desktop-browser-smoke \
...
`NOVA_AI_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `NOVA_AI_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `NOVA_AI_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `NOVA_AI_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `NOVA_AI_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `NOVA_AI_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm nova-ai qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT Nova AI gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable Nova AI
home at `$HOME/.nova-ai-mantis/slack-nova-ai`, patches Slack Socket Mode
configuration for the selected channel, starts `nova-ai gateway run` on port
...
- `NOVA_AI_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `NOVA_AI_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `NOVA_AI_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `NOVA_AI_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent Nova AI Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `NOVA_AI_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/nova-ai-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`nova-ai/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm nova-ai qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm nova-ai qa telegram --credential-source convex
...
`@nova-ai-mantis telegram desktop proof`, from the Actions UI with freeform
...
Nova AI with the leased Telegram SUT bot token, starts `nova-ai gateway run`
...
token never logs Telegram Desktop in; it only configures Nova AI. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring Nova AI.
...
comment or creates one. The workflows publish to `nova-ai-crabbox-artifacts`
with public URLs under `https://artifacts.nova-ai.com`. They provide bucket,
...
```text
@nova-ai-mantis discord status reactions
```
...
```text
@nova-ai-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child Nova AI Gateway with an isolated state directory.
...
- It exercises a real Nova AI Gateway, Discord bot auth, message dispatch,
...
- `pnpm nova-ai qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or Nova AI browser control captures screenshots.
...
- Node 22 and pnpm
- Nova AI checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one Nova AI Gateway, one browser, and one model run
...
- `NOVA_AI_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `NOVA_AI_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `NOVA_AI_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `NOVA_AI_QA_CONVEX_SITE_URL`
- `NOVA_AI_QA_CONVEX_SECRET_CI`
- `NOVA_AI_QA_MANTIS_CRABBOX_COORDINATOR`
- `NOVA_AI_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`NOVA_AI_QA_REDACT_PUBLIC_METADATA=1` for this reason.
