...
`node-llama-cpp` runtime package next to Nova AI, then point `local.modelPath`
at a GGUF file:
...
Nova AI indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.nova-ai/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `nova-ai memory index --force`
...
**Memory search disabled?** Check `nova-ai memory status`. If no provider is
...
**Stale results?** Run `nova-ai memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Nova AI falls back to in-process cosine similarity
automatically. `nova-ai memory status --deep` reports the local vector store
