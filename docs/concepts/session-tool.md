...
Nova AI gives agents tools to work across sessions, inspect status, and
...
After the target responds, Nova AI can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `nova-ai-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Nova AI
...
the completion origin only identifies a channel Nova AI can still reuse the
