---
summary: "What the Nova AI system prompt contains and how it is assembled"
read_when:
...
Nova AI builds a custom system prompt for every agent run. The prompt is **Nova AI-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Nova AI and injected into each agent run.
...
the full Nova AI-owned prompt. The provider runtime can:
...
- **Nova AI Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **Nova AI Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Nova AI docs/source and when to read them.
...
Nova AI keeps large stable content, including **Project Context**, above the
...
Nova AI can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **Nova AI
...
For channel auto-reply runs, Nova AI omits the generic **Silent Replies**
...
Nova AI keeps committed prompt snapshots for the Codex runtime happy path under
...
Nova AI developer instructions, turn-scoped collaboration-mode instructions
when Nova AI provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after Nova AI sends
...
On the native Codex harness, Nova AI avoids repeating stable workspace files
...
Nova AI prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, Nova AI can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, Nova AI injects a compact **available skills list**
...
points to the local Nova AI docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.nova-ai.com](https://docs.nova-ai.com).

The same section also includes the Nova AI source location. Git checkouts expose the local
...
consult docs first for Nova AI behavior, commands, configuration, or architecture, and to
run `nova-ai status` itself when possible (asking the user only when it lacks access).
