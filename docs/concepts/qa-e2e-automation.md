...
The private QA stack is meant to exercise Nova AI in a more realistic,
...
Every QA flow runs under `pnpm nova-ai qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm nova-ai qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
...
```bash
pnpm nova-ai qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`nova-ai.run`, `nova-ai.harness.run`, `nova-ai.model.call`,
`nova-ai.context.assembled`, and `nova-ai.message.delivery` must be present;
model calls must not export `StreamAbandoned` on successful turns; raw diagnostic IDs and
`nova-ai.content.*` attributes must stay out of the trace. It writes
...
```bash
pnpm nova-ai qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `NOVA_AI_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, and Slack smoke lanes:

```bash
pnpm nova-ai qa telegram
pnpm nova-ai qa discord
pnpm nova-ai qa slack
```
...
```bash
pnpm nova-ai qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent Nova AI Slack gateway
...
```bash
pnpm nova-ai qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `nova-ai infer image describe`
...
```bash
pnpm nova-ai qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Nova AI
...
- `NOVA_AI_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `NOVA_AI_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `NOVA_AI_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `NOVA_AI_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm nova-ai qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.

Output artifacts:

- `telegram-qa-report.md`
- `telegram-qa-summary.json` - includes per-reply RTT (driver send → observed SUT reply) starting with the canary.
- `telegram-qa-observed-messages.json` - bodies redacted unless `NOVA_AI_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
`NOVA_AI_NPM_TELEGRAM_WARM_SAMPLES` and
`NOVA_AI_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS`, so `result.json` remains comparable
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child Nova AI gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `NOVA_AI_QA_DISCORD_GUILD_ID`
- `NOVA_AI_QA_DISCORD_CHANNEL_ID`
- `NOVA_AI_QA_DISCORD_DRIVER_BOT_TOKEN`
- `NOVA_AI_QA_DISCORD_SUT_BOT_TOKEN`
- `NOVA_AI_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `NOVA_AI_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `NOVA_AI_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `NOVA_AI_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child Nova AI gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `NOVA_AI_QA_SLACK_CHANNEL_ID`
- `NOVA_AI_QA_SLACK_DRIVER_BOT_TOKEN`
- `NOVA_AI_QA_SLACK_SUT_BOT_TOKEN`
- `NOVA_AI_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `NOVA_AI_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
...
- `slack-qa-observed-messages.json` - bodies redacted unless `NOVA_AI_QA_SLACK_CAPTURE_CONTENT=1`.
...
{
  "display_information": {
    "name": "Nova AI QA Driver",
    "description": "Test driver bot for Nova AI QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Nova AI QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
```json
{
  "display_information": {
    "name": "Nova AI QA SUT",
    "description": "Nova AI QA SUT connector for Nova AI"
  },
  "features": {
    "bot_user": {
      "display_name": "Nova AI QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#nova-ai-qa`) and invite both bots from inside the channel:

```
/invite @Nova AI QA Driver
/invite @Nova AI QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `NOVA_AI_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `NOVA_AI_QA_CONVEX_SITE_URL` and `NOVA_AI_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm nova-ai qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm nova-ai qa credentials list --kind slack --status all --json
```
...
- `mock-openai` is the scenario-aware Nova AI mock. It remains the default
...
- how `nova-ai qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `nova-ai qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `nova-ai.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm nova-ai qa coverage` (add `--json` for machine-readable output).

For character and style checks, run the same scenario across multiple live model
refs and write a judged Markdown report:

```bash
pnpm nova-ai qa character-eval \
