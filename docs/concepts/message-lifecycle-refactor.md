...
That shape fixes local bugs, but it leaves Nova AI with too many public
...
That gives Nova AI at-least-once recovery. Exactly-once behavior exists only
...
- Structured Nova AI-origin metadata for operational/system output so visible
...
Nova AI should borrow the vocabulary, not copy the surface.

What Nova AI needs beyond that model:
...
`thread.post()` style promises are not enough for Nova AI. They hide the
...
Origin describes who produced a message and how Nova AI should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be Nova AI-originated operational output.
...
Core owns the meaning of Nova AI-originated output. Channels own how that
...
Nova AI operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that Nova AI accepted
...
- **Inbound record ack:** confirms Nova AI persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared Nova AI echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropNova AIEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
...
same visible gateway-failure text but without Nova AI origin metadata still
...
middleware chain, but Nova AI persists only the safe completed update id after
...
source if we need platform-level redelivery beyond Nova AI's restart
...
process dies there, Nova AI cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "nova-ai/plugin-sdk/channel-message";
```
...
Before preflight authorization, core must run the shared Nova AI echo predicate
whenever `origin.decode` returns Nova AI-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  Nova AI can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- Nova AI gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps Nova AI gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode Nova AI gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged Nova AI gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve Nova AI operational provenance.
...
- Add `nova-ai/plugin-sdk/channel-message`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropNova AIEcho` predicate in the channel-message SDK surface.
...
This is where the model stops being "agent replies" and becomes "Nova AI sends
messages".
...
- Gateway-failure origin factory and `shouldDropNova AIEcho` predicate.
...
- Slack tagged Nova AI gateway failure messages stay visible outbound, tagged
...
- Matrix tagged Nova AI gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only Nova AI's persisted restart watermark.
...
- Nova AI-originated gateway failure output is visible to humans but tagged
