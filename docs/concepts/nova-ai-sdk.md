---
summary: "Public Nova AI App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "Nova AI App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Nova AI
...
The **Nova AI App SDK** is the public client API for apps outside the
Nova AI process. Use `@nova-ai/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@nova-ai/sdk` talks to the Gateway from outside Nova AI.
  `nova-ai/plugin-sdk/*` is only for plugins that run inside Nova AI and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@nova-ai/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `Nova AI`                | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `Nova AIEvent`,
`Nova AIEventType`, `GatewayEvent`, `Nova AITransport`,
...
```typescript
import { Nova AI } from "@nova-ai/sdk";

const oc = new Nova AI({
...
`new Nova AI({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `Nova AITransport`:
...
The SDK normalizes raw Gateway events into a stable `Nova AIEvent` envelope:
...
Task helpers use the durable task ledger that also backs `nova-ai tasks`:
...
Use the App SDK when code lives outside Nova AI:
...
Use the Plugin SDK when code runs inside Nova AI:
...
App SDK code should import from `@nova-ai/sdk`. Plugin code should import from
documented `nova-ai/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [Nova AI App SDK API design](/reference/nova-ai-sdk-api-design)
