---
summary: "How Nova AI separates model providers, models, channels, and agent runtimes"
...
| ------------- | ------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How Nova AI authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                              |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave Nova AI.                            |
...
`nova-ai doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside Nova AI's prepared agent loop. Today this
...
| Surface                                          | Nova AI name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `nova-ai doctor --fix` owns legacy
...
That means Nova AI selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Nova AI session store
becomes Codex.
...
4. If legacy config still contains **`openai-codex/*` model refs**, repair it to
   `openai/<model>` with `nova-ai doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; Nova AI no longer keeps a bundled Codex CLI backend.
...
| Surface                     | Nova AI PI embedded                    | Codex app-server                                                            |
| --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Nova AI through the PI embedded runner | Codex app-server                                                            |
| Canonical thread state      | Nova AI transcript                     | Codex thread, plus Nova AI transcript mirror                               |
| Nova AI dynamic tools      | Native Nova AI tool loop               | Bridged through the Codex adapter                                           |
| Native shell and file tools | PI/Nova AI path                        | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Nova AI context assembly        | Nova AI projects assembled context into the Codex turn                     |
| Compaction                  | Nova AI or selected context engine     | Codex-native compaction, with Nova AI notifications and mirror maintenance |
| Channel delivery            | Nova AI                                | Nova AI                                                                    |

This ownership split is the main design rule:

- If Nova AI owns the surface, Nova AI can provide normal plugin hook behavior.
- If the native runtime owns the surface, Nova AI needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Nova AI should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Nova AI chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, Nova AI uses PI as the
...
`NOVA_AI_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`nova-ai doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where Nova AI can preserve the intent.
...
Nova AI routes PI internally through the legacy Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI PI session pins are
ignored by runtime selection and can be cleaned with `nova-ai doctor --fix`.

If `nova-ai doctor` warns that the `codex` plugin is enabled while
`openai-codex/*` remains in config, treat that as legacy route state. Run
`nova-ai doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## Compatibility contract

When a runtime is not PI, it should document what Nova AI surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Nova AI can edit history or only mirror it.                                   |
| Do Nova AI dynamic tools work?        | Messaging, sessions, cron, and Nova AI-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Nova AI-owned tools. |
