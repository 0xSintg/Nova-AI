...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.nova-ai/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.nova-ai/agents/<agentId>/sessions`.
...
a local profile, but Nova AI does not clone OAuth refresh tokens into the
...
- State dir: `~/.nova-ai` (or `NOVA_AI_STATE_DIR`)
- Workspace: `~/.nova-ai/workspace` (or `~/.nova-ai/workspace-<agentId>`)
- Agent dir: `~/.nova-ai/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.nova-ai/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Nova AI runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.nova-ai/workspace` (or `~/.nova-ai/workspace-<profile>` when `NOVA_AI_PROFILE` is set).
- State defaults to `~/.nova-ai/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    nova-ai agents add coding
    nova-ai agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.nova-ai/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.nova-ai/workspace-alex" },
      { id: "mia", workspace: "~/.nova-ai/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Nova AI upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Nova AI falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.nova-ai/workspace-main" },
          { id: "coding", workspace: "~/.nova-ai/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.nova-ai/workspace-main" },
          { id: "alerts", workspace: "~/.nova-ai/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    nova-ai channels login --channel whatsapp --account personal
    nova-ai channels login --channel whatsapp --account biz
    ```

    `~/.nova-ai/nova-ai.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.nova-ai/workspace-home",
            agentDir: "~/.nova-ai/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.nova-ai/workspace-work",
            agentDir: "~/.nova-ai/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.nova-ai/credentials/whatsapp/personal
              // authDir: "~/.nova-ai/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.nova-ai/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.nova-ai/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.nova-ai/workspace-opus",
