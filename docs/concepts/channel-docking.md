---
summary: "Move one Nova AI session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Nova AI session.
...
Alice can message Nova AI on Telegram and Discord:
...
Nova AI keeps the current session context and changes the reply route:
...
4. Keep the same Nova AI session, but receive future replies in Discord.
