...
Paste this into `nova-ai.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Nova AI can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Nova AI cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. Nova AI
...
5. Verify memory search itself works with `nova-ai memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Nova AI auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `nova-ai memory status --deep` to inspect the memory-search backend
      and index health.
