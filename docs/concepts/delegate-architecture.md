---
summary: "Delegate architecture: running Nova AI as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Nova AI as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Nova AI agent that:
...
Nova AI's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Nova AI's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.nova-ai/workspace-delegate",
  tools: {
...
- Cron run history: `~/.nova-ai/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.nova-ai/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Nova AI's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
nova-ai agents add delegate
```

This creates:

- Workspace: `~/.nova-ai/workspace-delegate`
- State: `~/.nova-ai/agents/delegate/agent`
- Sessions: `~/.nova-ai/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.nova-ai/workspace" },
      {
        id: "delegate",
        workspace: "~/.nova-ai/workspace-delegate",
        tools: {
...
~/.nova-ai/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.nova-ai/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.nova-ai/workspace-org",
        agentDir: "~/.nova-ai/agents/org-assistant/agent",
...
recall view. Nova AI redacts credential/token-like text, truncates long
