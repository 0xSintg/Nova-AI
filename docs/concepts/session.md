---
summary: "How Nova AI manages conversation sessions"
read_when:
...
Nova AI organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `nova-ai security audit`.
...
Nova AI automatically bounds session storage over time. By default, it runs
...
`nova-ai sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `nova-ai sessions cleanup --dry-run`.

## Inspecting sessions

- `nova-ai status` -- session store path and recent activity.
- `nova-ai sessions --json` -- all sessions (filter with `--active <minutes>`).
