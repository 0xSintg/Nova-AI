...
When a normal prompt arrives while a session run is already streaming, Nova AI
...
internal steering queue. Nova AI batches queued prompts for the configured
...
runtime cannot accept steering in `steer` mode, Nova AI waits for the active
...
- With `/queue collect`, Nova AI does not steer. It waits until the active run
  ends, then creates a followup turn with compatible queued messages after the
  debounce window.
- With `/queue interrupt`, Nova AI aborts the active run and starts the newest
