---
summary: "How Nova AI remembers things across sessions"
...
Nova AI remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.nova-ai/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, Nova AI keeps the file on
...
`nova-ai doctor` to see raw vs injected sizes and truncation status.
...
for that case. Nova AI infers them in a hidden background pass, scopes them to
...
<Info>
Nova AI auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Nova AI
...
```bash
nova-ai memory rem-backfill --rollback
nova-ai memory rem-backfill --rollback-short-term
```

## CLI

```bash
nova-ai memory status          # Check index status and provider
nova-ai memory search "query"  # Search from the command line
nova-ai memory index --force   # Rebuild the index
```
