...
- `/usage cost` in chats: local cost summary aggregated from Nova AI session logs.
- CLI: `nova-ai status --usage` prints a full per-provider breakdown.
- CLI: `nova-ai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Nova AI treats
  ...
  quota, so Nova AI inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Nova AI prefers the
...
can supply plugin-specific usage auth logic; otherwise Nova AI falls back to
