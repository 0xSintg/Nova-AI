---
summary: "How Nova AI rotates auth profiles and falls back across models"
read_when:
...
Nova AI handles failures in two stages:
...
For a normal text run, Nova AI evaluates candidates in this order:
...
Nova AI separates the selected provider/model from why it was selected. That source controls whether the fallback chain is allowed:
...
- **Auto fallback override**: a runtime fallback writes `providerOverride`, `modelOverride`, `modelOverrideSource: "auto"`, and the selected origin model before retrying. That auto override can keep walking the configured fallback chain without probing the primary on every message, but Nova AI periodically probes the configured origin again and clears the auto override when it recovers. `/new`, `/reset`, and `sessions.reset` also clear auto-sourced overrides. Heartbeat runs without an explicit `heartbeat.model` clear direct auto overrides when their origin no longer matches the current configured default.
- **User session override**: `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` write `modelOverrideSource: "user"`. That is an exact session selection. If the selected provider/model fails before producing a reply, Nova AI reports the failure instead of answering from an unrelated configured fallback.
- **Legacy session override**: older session entries may have `modelOverride` without `modelOverrideSource`. Nova AI treats those as user overrides so an explicit old selection is not silently converted into fallback behavior.
...
The auto fallback primary-probe interval is five minutes and is not configurable. Nova AI remembers recent probes per session and primary model so a failing primary is not retried on every turn. Nova AI sends a visible notice when a session moves onto fallback and another notice when it returns to the selected primary; it does not repeat the notice on every sticky fallback turn.

## User-visible fallback notices

When a session moves onto an auto-selected fallback, Nova AI sends a status notice in the same reply surface:

```text
↪️ Model Fallback: <fallback> (selected <primary>; <reason>)
```

When a later probe succeeds and the session returns to the selected primary, Nova AI sends:
...
Nova AI uses **auth profiles** for both API keys and OAuth tokens.

- Secrets live in `~/.nova-ai/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.nova-ai/agent/auth-profiles.json`).
- Runtime auth-routing state lives in `~/.nova-ai/agents/<agentId>/agent/auth-state.json`.
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.nova-ai/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.nova-ai/agents/<agentId>/agent/auth-profiles.json` under `profiles`.

## Rotation order

When a provider has multiple profiles, Nova AI chooses an order like this:
...
If no explicit order is configured, Nova AI uses a round-robin order:
...
Nova AI **pins the chosen auth profile per session** to keep provider caches warm. It does **not** rotate on every request. The pinned profile is reused until:
...
<Note>
Auto-pinned profiles (selected by the session router) are treated as a **preference**: they are tried first, but Nova AI may rotate to another profile on rate limits/timeouts. When the original profile becomes available again, new runs can prefer it again without changing the selected model or runtime. User-pinned profiles stay locked to that profile; if it fails and model fallbacks are configured, Nova AI moves to the next model instead of switching profiles.
</Note>
...
Nova AI records the exact reset time when Codex provides one, tries the next
...
When a profile fails due to auth/rate-limit errors (or a timeout that looks like rate limiting), Nova AI marks it in cooldown and moves to the next profile.
...
<AccordionGroup>
  ...
  <Accordion title="SDK retry-after caps">
    Some provider SDKs may otherwise sleep for a long `Retry-After` window before returning control to Nova AI. For Stainless-based SDKs such as Anthropic and OpenAI, Nova AI caps SDK-internal `retry-after-ms` / `retry-after` waits at 60 seconds by default and surfaces longer retryable responses immediately so this failover path can run. Tune or disable the cap with `NOVA_AI_SDK_RETRY_MAX_WAIT_SECONDS`; see [Retry behavior](/concepts/retry).
  </Accordion>
  <Accordion title="Model-scoped cooldowns">
    Rate-limit cooldowns can also be model-scoped:

    - Nova AI records `cooldownModel` for rate-limit failures when the failing model id is known.
...
Billing/credit failures (for example "insufficient credits" / "credit balance too low") are treated as failover-worthy, but they're usually not transient. Instead of a short cooldown, Nova AI marks the profile as **disabled** (with a longer backoff) and rotates to the next profile/provider.

<Note>
Not every billing-shaped response is `402`, and not every HTTP `402` lands here. Nova AI keeps explicit billing text in the billing lane even when a provider returns `401` or `403` instead, but provider-specific matchers stay scoped to the provider that owns them (for example OpenRouter `403 Key limit exceeded`).
...
If all profiles for a provider fail, Nova AI moves to the next model in `agents.defaults.model.fallbacks`. This applies to auth failures, rate limits, and timeouts that exhausted profile rotation (other errors do not advance fallback). Provider errors that do not expose enough detail are still labeled precisely in fallback state: `empty_response` means the provider returned no usable message or status, `no_error_details` means the provider explicitly returned `Unknown error (no error details in response)`, and `unclassified` means Nova AI preserved the raw preview but no classifier matched it yet.

Overloaded and rate-limit errors are handled more aggressively than billing cooldowns. By default, Nova AI allows one same-provider auth-profile retry, then switches to the next configured model fallback without waiting. Provider-busy signals such as `ModelNotReadyException` land in that overloaded bucket. Tune this with `auth.cooldowns.overloadedProfileRotations`, `auth.cooldowns.overloadedBackoffMs`, and `auth.cooldowns.rateLimitedProfileRotations`.

When a run starts from the configured default primary, a cron job primary, an agent primary with explicit fallbacks, or an auto-selected fallback override, Nova AI can walk the matching configured fallback chain. Agent primaries without explicit fallbacks and explicit user selections (for example `/model ollama/qwen3.5:27b`, the model picker, `sessions.patch`, or one-off CLI provider/model overrides) are strict: if that provider/model is unreachable or fails before producing a reply, Nova AI reports the failure instead of answering from an unrelated fallback.

### Candidate chain rules

Nova AI builds the candidate list from the currently requested `provider/model` plus configured fallbacks.
...
<AccordionGroup>
  <Accordion title="Rules">
    ...
    - If the current run is already on a configured fallback in the same provider family, Nova AI keeps using the full configured chain.
...
When every auth profile for a provider is already in cooldown, Nova AI does not automatically skip that provider forever. It makes a per-candidate decision:
...
- Auto fallback overrides remain selected on subsequent turns so Nova AI does not probe a known-bad primary on every message. Nova AI periodically probes the configured origin again and clears the auto override when it recovers; `/new`, `/reset`, and `sessions.reset` clear auto-sourced overrides immediately.
...
- If a live-switch error points at a later candidate in the active fallback chain, Nova AI jumps directly to that selected model instead of walking unrelated candidates first.
...
When every candidate fails, Nova AI throws `FallbackSummaryError`. The outer reply runner can use that to build a more specific message such as "all models are temporarily rate-limited" and include the soonest cooldown expiry when one is known.
...
- if the remaining block is a matching model-scoped rate limit, Nova AI reports the last matching expiry that still blocks that model
