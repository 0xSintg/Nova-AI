...
[Honcho](https://honcho.dev) adds AI-native memory to Nova AI. It persists
...
```bash
nova-ai plugins install @honcho-ai/nova-ai-honcho
nova-ai honcho setup
...
Settings live under `plugins.entries["nova-ai-honcho"].config`:
    ...
    entries: {
      "nova-ai-honcho": {
        config: {
          ...
          workspaceId: "nova-ai", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `nova-ai honcho setup` detects and
...
```bash
nova-ai honcho setup                        # Configure API key and migrate files
nova-ai honcho status                       # Check connection status
nova-ai honcho ask <question>               # Query Honcho about the user
nova-ai honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/nova-ai-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Nova AI integration guide](https://docs.honcho.dev/v3/guides/integrations/nova-ai)
- [Memory](/concepts/memory) -- Nova AI memory overview
