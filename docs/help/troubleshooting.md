---
summary: "Symptom first troubleshooting hub for Nova AI"
read_when:
  - Nova AI is not working and you need the fastest path to a fix
...
- `nova-ai status` → shows configured channels and no obvious auth errors.
- `nova-ai status --all` → full report is present and shareable.
- `nova-ai gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `nova-ai gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `nova-ai doctor` → no blocking config/service errors.
- `nova-ai channels status --probe` → reachable gateway returns live per-account
...
- `nova-ai logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in Nova AI

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `nova-ai infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Nova AI agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Nova AI prompts crash the
...
## Plugin install fails with missing nova-ai extensions

If install fails with `package.json missing nova-ai.extensions`, the plugin package
is using an old shape that Nova AI no longer accepts.

Fix in the plugin package:

1. Add `nova-ai.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `nova-ai plugins install <package>` again.

Example:

```json
{
  "name": "@nova-ai/my-plugin",
  "version": "1.2.3",
  "nova-ai": {
...
If `nova-ai doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run Nova AI as
...
```mermaid
flowchart TD
  A[Nova AI is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `nova-ai gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    nova-ai config get tools.exec.host
    nova-ai config get tools.exec.security
    nova-ai config get tools.exec.ask
    ...
    ```bash
    nova-ai config set tools.exec.host gateway
    nova-ai config set tools.exec.security full
    nova-ai config set tools.exec.ask off
    nova-ai gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `nova-ai` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `nova-ai browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
