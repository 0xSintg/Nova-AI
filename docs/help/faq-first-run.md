...
read_when:
  ...
  - Cannot access docs.nova-ai.com, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --install-method git
    ```

    This installs Nova AI **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/nova-ai/nova-ai/issues](https://github.com/nova-ai/nova-ai/issues)
    [https://github.com/nova-ai/nova-ai/pulls](https://github.com/nova-ai/nova-ai/pulls)
    ...
    - `nova-ai status`: quick snapshot of gateway/agent health + basic config.
    - `nova-ai models status`: checks provider auth + model availability.
    - `nova-ai doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `nova-ai status --all`, `nova-ai logs --follow`,
    `nova-ai gateway status`, `nova-ai health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Nova AI">
    ...
    If you don't have a global install yet, run it via `pnpm nova-ai onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `NOVA_AI_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `NOVA_AI_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `nova-ai doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `nova-ai gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `nova-ai gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Nova AI now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Nova AI on the new machine.
    2. Copy `$NOVA_AI_STATE_DIR` (default: `~/.nova-ai`) from the old machine.
    3. Copy your workspace (default: `~/.nova-ai/workspace`).
    4. Run `nova-ai doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.nova-ai/` (for example `~/.nova-ai/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/nova-ai/nova-ai/blob/main/CHANGELOG.md](https://github.com/nova-ai/nova-ai/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.nova-ai.com (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.nova-ai.com` via Xfinity
    Advanced Security. Disable it or allowlist `docs.nova-ai.com`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/nova-ai/nova-ai/tree/main/docs](https://github.com/nova-ai/nova-ai/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://nova-ai.com/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://nova-ai.com/install.ps1](https://nova-ai.com/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://nova-ai.com/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or nova-ai not recognized">
    ...
    **2) nova-ai is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Nova AI, track/report it in:

    - [Issue #30640](https://github.com/nova-ai/nova-ai/issues/30640)
  ...
  <Accordion title="How do I install Nova AI on Linux?">
  ...
  <Accordion title="How do I install Nova AI on a VPS?">
  ...
  <Accordion title="Can I ask Nova AI to update itself?">
    ...
    ```bash
    nova-ai update
    nova-ai update status
    nova-ai update --channel stable|beta|dev
    nova-ai update --tag <dist-tag|version>
    nova-ai update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    nova-ai update --yes --no-restart
    nova-ai gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `nova-ai onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Nova AI with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Nova AI, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Nova AI**: Anthropic staff
      told us this usage is allowed again, and Nova AI is treating `claude -p`
    ...
    Nova AI also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Nova AI-style Claude CLI usage is allowed again, so
    Nova AI treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Nova AI treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Nova AI token path, but Nova AI now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Nova AI, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Nova AI Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Nova AI can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How does Codex auth work?">
    ...
    legacy config repaired by `nova-ai doctor --fix`. Direct OpenAI API-key
  ...
  <Accordion title="Why does Nova AI still mention openai-codex?">
    ...
    - `openai-codex/gpt-5.5` = legacy model route repaired by `nova-ai doctor --fix`
    ...
    `nova-ai models auth login --provider openai-codex`. Keep the model ref as
    `openai/gpt-5.5`; `openai-codex/*` model refs are legacy config that
    `nova-ai doctor --fix` rewrites.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Nova AI can show the currently visible provider usage/quota windows in
    `nova-ai models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Nova AI fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Nova AI. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `nova-ai.json`.
    ...
    2. Enable the plugin: `nova-ai plugins enable google`
    3. Login: `nova-ai models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Nova AI needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Nova AI runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Nova AI, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `nova-ai nodes status` / `nova-ai nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `nova-ai doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `nova-ai logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Nova AI instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Nova AI via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes. Use `nova-ai update --channel ...` when Nova AI is already installed.
    This **does not delete your data** - it only changes the Nova AI code install.
    Your state (`~/.nova-ai`) and workspace (`~/.nova-ai/workspace`) stay untouched.
    ...
    ```bash
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --install-method git
    curl -fsSL https://nova-ai.com/install.sh | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Nova AI-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Nova AI on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Nova AI is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Nova AI in a VM and what are the requirements?">
