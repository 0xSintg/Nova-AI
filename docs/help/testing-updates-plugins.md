---
summary: "How Nova AI validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing Nova AI update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `nova-ai doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` validates plugin install smoke, local folder installs,
  ...
  and Claude-bundle enable/inspect. Set `NOVA_AI_PLUGINS_E2E_CLAWHUB=0` to
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `nova-ai plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `nova-ai config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `nova-ai update --yes --json`, and requires the candidate update command to
...
```bash
NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPEC=nova-ai@2026.4.23 \
NOVA_AI_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

NOVA_AI_UPGRADE_SURVIVOR_BASELINE_SPEC=nova-ai@latest \
NOVA_AI_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```
...
`NOVA_AI_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=npm`: validate `nova-ai@beta`, `nova-ai@latest`, or an exact
  published version.
...
`package_acceptance_package_spec=nova-ai@YYYY.M.D` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published Nova AI
