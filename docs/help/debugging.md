...
This is handy when you need to toggle obscure settings without editing `nova-ai.json`.
...
Use `NOVA_AI_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
NOVA_AI_PLUGIN_LIFECYCLE_TRACE=1 nova-ai plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm nova-ai ...`
...
`NOVA_AI_RUN_NODE_CPU_PROF_DIR`:

```bash
NOVA_AI_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm nova-ai status
```
...
```bash
NOVA_AI_TRACE_SYNC_IO=1 pnpm nova-ai gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `NOVA_AI_TRACE_SYNC_IO=1` when you explicitly want Node
...
`nova-ai-gateway-watch-main` (or a profile/port-specific variant such as
`nova-ai-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t nova-ai-gateway-watch-main
```
...
# or
NOVA_AI_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
NOVA_AI_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`NOVA_AI_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`NOVA_AI_PROFILE`, `NOVA_AI_CONFIG_PATH`, `NOVA_AI_STATE_DIR`,
`NOVA_AI_GATEWAY_PORT`, and `NOVA_AI_SKIP_CHANNELS` into the pane. Put
...
`nova-ai doctor --fix --non-interactive` once and restarts the Gateway child.
Use `NOVA_AI_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `nova-ai.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.nova-ai-dev` and
...
```bash
pnpm gateway:dev
NOVA_AI_PROFILE=dev nova-ai tui
```

If you don't have a global install yet, run the CLI via `pnpm nova-ai ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `NOVA_AI_PROFILE=dev`
   - `NOVA_AI_STATE_DIR=~/.nova-ai-dev`
   - `NOVA_AI_CONFIG_PATH=~/.nova-ai-dev/nova-ai.json`
   - `NOVA_AI_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`NOVA_AI_SKIP_CHANNELS=1`).
...
```bash
NOVA_AI_PROFILE=dev nova-ai gateway --dev --reset
```
...
## Raw stream logging (Nova AI)

Nova AI can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.nova-ai/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
NOVA_AI_RAW_STREAM=1
NOVA_AI_RAW_STREAM_PATH=~/.nova-ai/logs/raw-stream.jsonl
```

Default file:

`~/.nova-ai/logs/raw-stream.jsonl`
...
- If you need to use the built Nova AI CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./nova-ai.mjs` or create a shell alias like `alias nova-ai-build="node $(pwd)/nova-ai.mjs"`
