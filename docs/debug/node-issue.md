...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Nova AI
...
Running Nova AI via Node with `tsx` fails at startup with:

```
[nova-ai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `nova-ai status` and `gateway:watch` worked.
