---
summary: "Build simple typed agent tools with defineToolPlugin and nova-ai plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple Nova AI plugin that only adds agent tools
...
Tool plugins add agent-callable tools to Nova AI without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want Nova AI to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `nova-ai plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `nova-ai.plugin.json` and `package.json` metadata with
   `nova-ai plugins build`.
...
- `nova-ai >=2026.5.17`, the first Nova AI version that exports
  `nova-ai/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `nova-ai.plugin.json`, and
  `package.json`.
...
- `nova-ai.plugin.json`: generated manifest metadata for the initial tool.
...
import { defineToolPlugin } from "nova-ai/plugin-sdk/tool-plugin";
...
`nova-ai plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so Nova AI can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the Nova AI tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Nova AI to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, Nova AI uses a strict empty object
...
Nova AI reads plugin config from the plugin entry in the Gateway config. Do not
...
Nova AI discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `nova-ai plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells Nova AI which
...
For the simple tool-plugin workflow, `nova-ai plugins build` aligns
`package.json` to the selected single runtime entry:

```json
{
  "type": "module",
  "files": ["dist", "nova-ai.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
...
- `nova-ai.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `nova-ai.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate Nova AI checkout or installed CLI, install the package path:

```bash
nova-ai plugins install ./stock-quotes
nova-ai plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
nova-ai plugins install npm-pack:./nova-ai-plugin-stock-quotes-0.1.0.tgz
nova-ai plugins inspect stock-quotes --runtime --json
```
...
```bash
nova-ai plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ClawHub
is the preferred discovery and distribution surface for Nova AI plugins.
...
`nova-ai plugins build --entry ./dist/index.js` or
`nova-ai plugins validate --entry ./dist/index.js`.
...
### `nova-ai.plugin.json generated metadata is stale`
...
Commit both `nova-ai.plugin.json` and `package.json` changes.

### `package.json nova-ai.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`nova-ai plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `nova-ai plugins inspect <plugin-id> --runtime`
2. `nova-ai plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `nova-ai.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `nova-ai.extensions: ["./dist/index.js"]`.
