...
read_when:
  - You see the NOVA_AI_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the NOVA_AI_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before Nova AI 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Nova AI plugin
---

Nova AI has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`nova-ai/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`nova-ai/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`nova-ai/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`nova-ai/extension-api`** - a bridge that gave plugins direct access to
...
Nova AI does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`nova-ai/plugin-sdk/\<subpath\>`)
...
`nova-ai/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `nova-ai doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `nova-ai/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `Nova AIConfig` | `nova-ai/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `nova-ai/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `nova-ai/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `nova-ai/plugin-sdk/config-mutation` |
    | Session store helpers | `nova-ai/plugin-sdk/session-store-runtime` |
    | Markdown table config | `nova-ai/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `nova-ai/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `nova-ai/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `nova-ai/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `nova-ai/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `nova-ai/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "nova-ai/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "nova-ai/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "nova-ai/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "nova-ai/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "nova-ai/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "nova-ai/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `nova-ai/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `nova-ai/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `nova-ai/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `nova-ai/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `nova-ai/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `nova-ai/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `nova-ai/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `nova-ai/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `nova-ai/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `nova-ai/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `nova-ai/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `nova-ai/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `nova-ai/plugin-sdk/error-runtime` |
    | Transport readiness waits | `nova-ai/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `nova-ai/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `nova-ai/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `nova-ai/plugin-sdk/number-runtime` |
    | Process-local async lock | `nova-ai/plugin-sdk/async-lock-runtime` |
    | File locks | `nova-ai/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `nova-ai/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `Nova AISchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | Nova AI-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@nova-ai/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`nova-ai/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`nova-ai/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "nova-ai/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "nova-ai/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `nova-ai/plugin-sdk/channel-inbound` or
    `nova-ai/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `nova-ai/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `nova-ai/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `nova-ai/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `nova-ai/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Nova AI no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Nova AI downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="Nova AISchemaType alias → Nova AIConfig">
    `Nova AISchemaType` re-exported from `nova-ai/plugin-sdk` is now a
    one-line alias for `Nova AIConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { Nova AISchemaType } from "nova-ai/plugin-sdk";
    // After
    import type { Nova AIConfig } from "nova-ai/plugin-sdk/config-schema";
    ```
...
```bash
NOVA_AI_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 nova-ai gateway run
NOVA_AI_SUPPRESS_EXTENSION_API_WARNING=1 nova-ai gateway run
```
