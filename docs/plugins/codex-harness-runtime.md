...
the native model loop, and Nova AI adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

Nova AI still owns channel routing, session files, visible message delivery,
Nova AI dynamic tools, approvals, media delivery, and a transcript mirror.
Codex owns the canonical native thread, native model loop, native tool
continuation, and native compaction unless the active Nova AI context engine
...
explicit PI compatibility route keeps the normal Nova AI/PI system prompt even
...
Nova AI runs still preserve their existing project-doc suppression. Nova AI
developer instructions cover Nova AI runtime concerns such as source-channel
delivery, Nova AI dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. Nova AI skill catalogs plus `MEMORY.md`
and active `BOOTSTRAP.md` content are projected as turn input reference context
for native Codex.

## Thread bindings and model changes

When an Nova AI session is attached to an existing Codex thread, the next turn
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable Nova AI
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Nova AI plugin hooks                 | Nova AI                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | Nova AI bundled plugins | Per-turn adapter behavior around Nova AI dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Nova AI does not use project or global Codex `hooks.json` files to route
Nova AI plugin behavior. For the supported native tool and permission bridge,
Nova AI injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and Nova AI's approval bridge handle real
...
Codex-level controls. They are not exposed as Nova AI plugin hooks in the v1
contract.

For Nova AI dynamic tools, Nova AI executes the tool after Codex asks for the
call, so Nova AI fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
Nova AI can mirror selected events, but it cannot rewrite the native Codex
...
notifications and Nova AI adapter state, not native Codex hook commands.
Nova AI's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke Nova AI plugin hooks.
...
| Nova AI channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                    |
| Nova AI dynamic tools                        | Supported                                                                        | Codex asks Nova AI to execute these tools, so Nova AI stays in the execution path.                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Nova AI projects Nova AI-specific prompt/context into the Codex turn while leaving Codex-owned base, model, personality, and configured project-doc prompts in the native Codex lane. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Nova AI-owned dynamic tools.                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Nova AI after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Nova AI records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Nova AI does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Nova AI owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Nova AI-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Nova AI observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current Nova AI compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Nova AI can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Nova AI only returns explicit allow or deny decisions
...
Nova AI remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through Nova AI's plugin
...
default `messages.queue.mode: "steer"`, Nova AI batches steer-mode chat
...
case, Nova AI waits for the active run to finish before starting the prompt.
...
harness, Nova AI also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, Nova AI session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, Nova AI does not print those Codex ids and
...
`ownsCompaction: true`. Owning context engines compact first and cause Nova AI
to abandon the old Codex backend thread so the next turn can rehydrate a fresh
thread from engine-managed context. Nova AI keeps a transcript mirror for
channel history, search, `/new`, `/reset`, and future model or harness
switching.

When a context engine requests Codex thread-bootstrap projection, Nova AI
...
reasoning or plan records when the app-server emits them. Today, Nova AI only
...
Nova AI is writing an Nova AI-owned session transcript tool result.

## Media and delivery

Nova AI continues to own media delivery and media provider selection. Image,
...
through the normal Nova AI delivery path. Media generation does not require PI.
When Codex emits a native image-generation item with a `savedPath`, Nova AI
