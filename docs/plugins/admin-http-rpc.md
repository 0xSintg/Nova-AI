...
The plugin is included with Nova AI, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for Nova AI clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    nova-ai plugins enable admin-http-rpc
...
```bash
nova-ai plugins disable admin-http-rpc
nova-ai gateway restart
```
...
- For `token` and `password` auth, narrower `x-nova-ai-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-nova-ai-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for Nova AI clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
