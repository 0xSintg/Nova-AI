...
read_when:
  - You maintain an Nova AI plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Nova AI keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core Nova AI repo as a separate
...
```sh
nova-ai-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. Nova AI
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `nova-ai` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against Nova AI plugin packages.
Run it from a clean Nova AI checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @nova-ai/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @nova-ai/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @nova-ai/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
Nova AI should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `nova-ai/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `nova-ai/extension-api`,
  `nova-ai/plugin-sdk/channel-runtime`, `nova-ai/plugin-sdk/command-auth`
  status builders, `nova-ai/plugin-sdk/test-utils` (replaced by focused
  `nova-ai/plugin-sdk/*` test subpaths), and the `NovabotConfig` /
  `Nova AISchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `nova-ai/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `nova-ai plugins registry --refresh` and
  `nova-ai doctor --fix`
