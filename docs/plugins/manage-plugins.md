---
summary: "Quick examples for listing, installing, updating, inspecting, and uninstalling Nova AI plugins"
read_when:
...
`plugins list` is a cold inventory check. It shows what Nova AI can discover
...
install hints such as `nova-ai plugins install clawhub:<package>`.
...
nova-ai plugins install clawhub:<package>@1.2.3
nova-ai plugins install clawhub:<package>@beta

# Install from npm.
nova-ai plugins install npm:<package>
nova-ai plugins install npm:@scope/nova-ai-plugin@1.2.3
nova-ai plugins install npm:@nova-ai/codex

# Install from a local npm pack artifact.
nova-ai plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
nova-ai plugins install git:github.com/acme/nova-ai-plugin@v1.0.0
...
If the bare name matches an official plugin id, Nova AI can install the
...
```bash
nova-ai plugins update <plugin-id>
nova-ai plugins update <npm-package-or-spec>
nova-ai plugins update --all
nova-ai plugins update <plugin-id> --dry-run
```

When you pass a plugin id, Nova AI reuses the tracked install spec. Stored
...
```bash
nova-ai plugins update @scope/nova-ai-plugin@beta
nova-ai plugins update @scope/nova-ai-plugin
```
...
When `nova-ai update` runs on the beta channel, plugin records can prefer
matching `@beta` releases. For the exact fallback and pinning rules, see
[`nova-ai plugins`](/cli/plugins#update).

## Uninstall plugins

```bash
nova-ai plugins uninstall <plugin-id> --dry-run
nova-ai plugins uninstall <plugin-id>
nova-ai plugins uninstall <plugin-id> --keep-files
```
...
In Nix mode (`NOVA_AI_NIX_MODE=1`), plugin install, update, uninstall, enable,
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want Nova AI-native discovery, scan summaries, versions, and hints     | `nova-ai plugins install clawhub:<package>`                   |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `nova-ai plugins install npm:@acme/nova-ai-plugin`           |
| git         | You want a branch, tag, or commit from a repository                         | `nova-ai plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                  | `nova-ai plugins install --link ./my-plugin`                  |
| npm pack    | You are proving a local package artifact through npm install semantics      | `nova-ai plugins install npm-pack:<path.tgz>`                 |
| marketplace | You are installing a Claude-compatible marketplace plugin                   | `nova-ai plugins install <plugin> --marketplace <source>`     |

## Publish plugins

ClawHub is the primary public discovery surface for Nova AI plugins. Publish
...
nova-ai plugins install npm:@acme/nova-ai-plugin
nova-ai plugins install npm:@acme/nova-ai-plugin@beta
nova-ai plugins install npm:@acme/nova-ai-plugin@1.0.0
```
