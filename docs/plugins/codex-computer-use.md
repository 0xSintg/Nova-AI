---
summary: "Set up Codex Computer Use for Codex-mode Nova AI agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Nova AI agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Nova AI
...
Use this page when Nova AI is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Nova AI.app and Peekaboo

Nova AI.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Nova AI.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Nova AI agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Nova AI node and exposes mobile
...
Nova AI-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Nova AI's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Nova AI command:

```bash
cua-driver mcp-config --client nova-ai
```

or register the stdio server yourself:

```bash
nova-ai mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal Nova AI MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Nova AI
...
With this config, Nova AI checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Nova AI asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Nova AI also tries to
...
plugin command surface is available. These are Nova AI chat/runtime commands,
not `nova-ai codex ...` CLI subcommands:
...
Nova AI uses the same app-server API that Codex itself exposes. The
...
During install, Nova AI polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Nova AI prefers
...
`computer-use` is registered, Nova AI tries to add the standard bundled
...
Nova AI reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Nova AI says Computer Use
...
Nova AI intentionally fails closed when `computerUse.enabled` is true. A
...
fresh Nova AI session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active Nova AI relay through the local bridge or
Gateway fallback. Start a fresh Nova AI session with `/new` or `/reset`. If it
