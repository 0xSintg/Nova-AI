---
summary: "Install and use Codex, Claude, and Cursor bundles as Nova AI plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Nova AI maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Nova AI can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Nova AI maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Nova AI plugins. Native plugins run
...
of requiring authors to rewrite them as native Nova AI plugins, Nova AI
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    nova-ai plugins install ./my-bundle

    # Archive
    nova-ai plugins install ./my-bundle.tgz
...
## What Nova AI maps from bundles

Not every bundle feature runs in Nova AI today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Nova AI skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Nova AI-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal Nova AI skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Nova AI skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Nova AI hook-pack
...
- Nova AI merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Nova AI exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Nova AI uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Nova AI config. `nova-ai mcp set` and `nova-ai doctor --fix` normalize the common alias.
...
Nova AI registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Nova AI sanitizes shell override keys before applying them
...
- Nova AI loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `nova-ai plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Nova AI does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Nova AI best when they use skill roots and Nova AI-style
...
Nova AI checks for native plugin format first:

1. `nova-ai.plugin.json` or valid `package.json` with `nova-ai.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Nova AI uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `nova-ai plugins install` and ship everything
  they need in the installed plugin directory.
- Nova AI-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `nova-ai doctor --fix` removes legacy staged dependency directories and can
...
- Nova AI does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `nova-ai plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Nova AI does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Nova AI hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
