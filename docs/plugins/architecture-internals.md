...
At startup, Nova AI does roughly this:
...
The manifest is the control-plane source of truth. Nova AI uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Nova AI
...
Nova AI does not cache plugin discovery results or direct manifest registry
...
Nova AI still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Nova AI calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Nova AI no longer calls, such as
...
| --  | _(built-in model lookup)_         | Nova AI tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on Nova AI's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Nova AI's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Nova AI adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while Nova AI owns the
...
- Nova AI only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Nova AI plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-nova-ai-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-nova-ai-scopes` only when the header is explicitly present
  - if `x-nova-ai-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-nova-ai-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `nova-ai/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `nova-ai/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `nova-ai/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `nova-ai/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `nova-ai/plugin-sdk/config-schema` | Root `nova-ai.json` Zod schema (`Nova AISchema`) |
...
<Info>
`nova-ai/plugin-sdk/channel-runtime`, `nova-ai/plugin-sdk/config-runtime`,
and `nova-ai/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `nova-ai/plugin-sdk/*` subpaths. Never
...
`nova-ai/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Nova AI writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Nova AI's
...
- if both `catalog` and `discovery` are registered, Nova AI uses `catalog`
...
- Read-only command paths such as `nova-ai status`, `nova-ai status --all`,
  `nova-ai channels status`, `nova-ai channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `nova-ai.extensions`:
...
Security guardrail: every `nova-ai.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `nova-ai plugins install` installs plugin dependencies with a
...
Optional: `nova-ai.setupEntry` can point at a lightweight setup-only module.
When Nova AI needs setup surfaces for a disabled channel plugin, or
...
Optional: `nova-ai.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Nova AI load the
...
Channel plugins can advertise setup/discovery metadata via `nova-ai.channel` and
install hints via `nova-ai.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@nova-ai/nextcloud-talk",
  "nova-ai": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@nova-ai/nextcloud-talk",
...
Useful `nova-ai.channel` fields beyond the minimal example:
...
Nova AI can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.nova-ai/mpm/plugins.json`
- `~/.nova-ai/mpm/catalog.json`
- `~/.nova-ai/plugins/catalog.json`

Or point `NOVA_AI_PLUGIN_CATALOG_PATHS` (or `NOVA_AI_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "nova-ai": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `nova-ai.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "nova-ai/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `Nova AIPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Nova AI stays opinionated without becoming hardcoded to one
