---
summary: "How Nova AI installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Nova AI installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Nova AI keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Nova AI
package directory.
...
- SDK/core imports are peer or supplied Nova AI imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Nova AI-owned package roots

Nova AI owns only the plugin lifecycle:
...
Nova AI uses stable per-source roots:

- npm packages install under `~/.nova-ai/npm`
- git packages clone under `~/.nova-ai/git`
...
`nova-ai plugins install npm-pack:<path.tgz>` uses that same managed npm root
for a local npm-pack tarball. Nova AI reads the tarball's npm metadata, adds it
...
npm may hoist transitive dependencies to `~/.nova-ai/npm/node_modules` beside
the plugin package. Nova AI scans the managed npm root before trusting the
...
Plugins that import `nova-ai/plugin-sdk/*` declare `nova-ai` as a peer
dependency. Nova AI does not let npm install a separate registry copy of the
...
resolution/materialization for the shared root and Nova AI reasserts
plugin-local `node_modules/nova-ai` links for installed packages that declare
...
Local plugins are treated as developer-controlled directories. Nova AI does not
...
nova-ai plugins install <source>
nova-ai doctor --fix
```

`doctor --fix` can clean legacy Nova AI-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of Nova AI.
...
In source checkouts, Nova AI treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g nova-ai`        | Built runtime tree inside the package | Nova AI package and explicit plugin install/update/doctor flows     |
...
| `nova-ai plugins install ...`   | Managed npm/git/ClawHub plugin root   | The plugin install/update flow                                       |

## Legacy cleanup

Older Nova AI versions generated bundled-plugin dependency roots at startup or
...
`.nova-ai-runtime-deps*` manifests, generated plugin `node_modules`, install
