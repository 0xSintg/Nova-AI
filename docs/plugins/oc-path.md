---
summary: "Bundled `oc-path` plugin: ships the `nova-ai path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`nova-ai path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Nova AI repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `nova-ai path … --json` instead of carrying separate markdown, JSONC,
...
nova-ai path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
nova-ai path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `nova-ai` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/nova-ai.plugin.json`:
...
`nova-ai path …`, so installs that never use the verb pay no cost.
...
state. Bare `nova-ai path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
nova-ai plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `nova-ai path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__NOVA_AI_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`nova-ai path` CLI reference](/cli/path)
