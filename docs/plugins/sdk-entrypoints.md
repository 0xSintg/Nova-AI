...
when Nova AI loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Nova AI will use a
...
**Import:** `nova-ai/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the Nova AI tool-result format, and
exposes static metadata that `nova-ai plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, Nova AI uses a strict empty object
...
- Tool names are static. `nova-ai plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `nova-ai.plugin.json` and `package.json` `nova-ai.extensions`; Nova AI does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `nova-ai/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `Nova AIPluginConfigSchema \| () => Nova AIPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: Nova AIPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `nova-ai.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Nova AI resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `nova-ai/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: Nova AIPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: Nova AIPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Nova AI may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Nova AI
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `nova-ai nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; Nova AI resolves it to the
...
Nova AI loads this instead of the full entry when a channel is disabled,
...
- `nova-ai/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `nova-ai/plugin-sdk/channel-setup` for optional-install setup surfaces
- `nova-ai/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`nova-ai/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so Nova AI can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Nova AI to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Nova AI rejects descriptor names outside
...
Nova AI classifies loaded plugins by their registration behavior:
...
Use `nova-ai plugins inspect <id>` to see a plugin's shape.
