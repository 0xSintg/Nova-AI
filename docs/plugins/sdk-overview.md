...
read_when:
  ...
  - You want a reference for all registration methods on Nova AIPluginApi
...
<Note>
  This page is for plugin authors using `nova-ai/plugin-sdk/*` inside
  Nova AI. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [Nova AI App SDK](/concepts/nova-ai-sdk) and the `@nova-ai/sdk` package
  instead.
</Note>
...
prefer `nova-ai/plugin-sdk/channel-core`; keep `nova-ai/plugin-sdk/core` for
...
`nova-ai.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Nova AI's
...
<Warning>
  ...
  `nova-ai/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`nova-ai/plugin-sdk/discord` and `nova-ai/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `Nova AIPluginApi` object with these
methods:
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `nova-ai nodes` |
...
cannot register this middleware; keep normal Nova AI plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. Nova AI calls the
...
`nova-ai nodes canvas` explicit plugin-owned node features.
...
- User config still wins. Nova AI merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Nova AI thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `nova-ai/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep Nova AI as the source of truth for due checks and execution.
...
| `api.config`             | `Nova AIConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `nova-ai/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Nova AI is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Nova AI's plugin
...
- **`@nova-ai/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@nova-ai/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `nova-ai/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `nova-ai/plugin-sdk/speech`, `.../provider-model-shared`, or another
