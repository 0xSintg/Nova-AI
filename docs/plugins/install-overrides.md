...
[`nova-ai plugins install`](/cli/plugins).
...
```bash
export NOVA_AI_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export NOVA_AI_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/nova-ai-codex-2026.5.8.tgz",
  "nova-ai-web-search": "npm:@nova-ai/web-search@2026.5.8"
}'
```
...
Nova AI uses the override source instead of the catalog, bundled, or default
...
entry normally represents an Nova AI-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches Nova AI.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Nova AI state:

```bash
npm pack extensions/codex --pack-destination /tmp

NOVA_AI_STATE_DIR="$(mktemp -d)" \
NOVA_AI_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
NOVA_AI_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/nova-ai-codex-2026.5.8.tgz"}' \
pnpm nova-ai onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$NOVA_AI_STATE_DIR/npm/node_modules" -maxdepth 3 -name package.json -print
grep -R '"@nova-ai/codex"' "$NOVA_AI_STATE_DIR/npm/package-lock.json"
```
