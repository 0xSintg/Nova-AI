...
By default, Nova AI starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Nova AI builds its inherited environment.                                                                       |
| `requestTimeoutMs`            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                               |
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while Nova AI waits for `turn/completed`.                                                                              |
...
| `sandbox`                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active Nova AI sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows Nova AI sandbox egress. |
...
unattended Nova AI turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Nova AI treats the implicit default as guardian
...
When an Nova AI sandbox is active, the local Codex app-server process still
runs on the Gateway host. Nova AI therefore keeps Codex's own filesystem
...
from the Nova AI sandbox egress setting: Docker `network: "none"` stays
...
1. An explicit Nova AI Codex auth profile for the agent.
...
Stdio app-server launches inherit Nova AI's process environment by default.
Nova AI owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Nova AI state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the Nova AI agent
instead of leaking in from the operator's personal `~/.codex` home.

Nova AI does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `nova-ai`, `gh`, `git`, cloud CLIs, and shell commands see
...
Nova AI plugins and Nova AI skill snapshots still flow through Nova AI's own
...
part of an Nova AI agent, inventory them explicitly:
...
Most remaining Nova AI integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `nova-ai` namespace. This keeps the initial
...
surface, while explicit Nova AI or ACP delegation is still available through
the `nova-ai` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, Nova AI aborts the
...
Codex then goes quiet without `turn/completed`, Nova AI best-effort interrupts
...
availability is owned by Codex app-server, so the list can change when Nova AI
...
If discovery fails or times out, Nova AI uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. Nova AI
...
For Nova AI workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
Nova AI Codex developer instructions because they define the active agent,
...
forwarded as Nova AI turn input reference context.
