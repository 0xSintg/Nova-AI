---
summary: "Configure migrated native Codex plugins for Codex-mode Nova AI agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Nova AI agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Nova AI agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Nova AI turn.

Nova AI does not translate Codex plugins into synthetic `codex_plugin_*`
Nova AI dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected Nova AI agent runtime must be the native Codex harness.
...
```bash
nova-ai migrate codex --dry-run --verify-plugin-apps
```
...
`enable` and `disable` write only to Nova AI config at
`~/.nova-ai/nova-ai.json`; they do not edit `~/.codex/config.toml` or install
...
- Enabled: Nova AI config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, Nova AI
...
Nova AI reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and Nova AI rebuilds it
...
A plugin app is exposed only when Nova AI can map it back to the migrated
...
Nova AI injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

Nova AI sets app-level `destructive_enabled` from the effective global or
...
are emitted with `open_world_enabled: true`; Nova AI does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, Nova AI returns a deterministic decline.
- When policy is `true`, Nova AI auto-accepts only safe schemas it can map to
...
empty or stale cache. Nova AI schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until Nova AI
