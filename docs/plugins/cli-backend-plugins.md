...
read_when:
  ...
  - You need to map a third-party CLI into Nova AI's text fallback runner
---

CLI backend plugins let Nova AI call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Nova AI at the plugin runtime module              |
| Manifest ownership   | `nova-ai.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `nova-ai.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json nova-ai.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Nova AI",
    ...
    `cliBackends` is the runtime ownership list. It lets Nova AI auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "nova-ai/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how Nova AI should launch and parse the CLI:
...
| `modelAliases`                            | Map Nova AI model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields Nova AI reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific Nova AI auth profile                |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into Nova AI's loopback MCP tool bridge           |
...
CLI backends do not receive Nova AI tools by default. If the CLI can consume an
...
"always-on"` so Nova AI can fail closed when a caller requires no native tools.
...
```bash
nova-ai plugins inspect acme-cli --runtime --json
nova-ai agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `nova-ai.extensions` and built runtime entries for published packages</Check>
<Check>`nova-ai.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
