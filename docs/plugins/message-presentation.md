...
Message presentation is Nova AI's shared contract for rich outbound chat UI.
...
```bash
nova-ai message send --channel slack \
  --target channel:C123 \
...
```bash
nova-ai message send --channel telegram \
...
Use helpers from `nova-ai/plugin-sdk/interactive-runtime` when bridging older
code:
