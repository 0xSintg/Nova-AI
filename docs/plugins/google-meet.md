...
read_when:
  - You want an Nova AI agent to join a Google Meet call
  - You want an Nova AI agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for Nova AI — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured Nova AI agent answers, and regular Nova AI TTS speaks into Meet.
...
provider plus regular Nova AI TTS. OpenAI is the default transcription
...
```bash
nova-ai googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `nova-ai googlemeet setup --json` for
...
```bash
nova-ai googlemeet setup --transport twilio
```
...
```bash
nova-ai googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
nova-ai googlemeet create --transport chrome-node --mode agent
```
...
```bash
nova-ai googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. Nova AI uses the
  ...
  the Nova AI Chrome profile on the node to already be signed in to Google.
...
Nova AI's microphone/camera permission grant and avoid the Meet **Use
...
Use `nova-ai googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in Nova AI browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by Nova AI. For
...
You do **not** need a full Nova AI Gateway or model API key inside a macOS VM
...
- Gateway host: Nova AI Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: Nova AI CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update Nova AI in the VM, then enable the bundled plugin there:

```bash
nova-ai plugins enable google-meet
```

Start the node host in the VM:

```bash
nova-ai node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1 \
  nova-ai node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1 \
  nova-ai node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
nova-ai node restart
```

`NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`nova-ai.json` setting. `nova-ai node install` stores it in the LaunchAgent
...
```bash
nova-ai googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, Nova AI browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, Nova AI auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `nova-ai node run` in the VM,
  approve pairing, and make sure `nova-ai plugins enable google-meet` and
  `nova-ai plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses Nova AI
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. Nova AI
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by Nova AI; use separate virtual devices or Loopback-style routing
...
Nova AI does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with Nova AI, review BlackHole's
...
Chrome transport opens the Meet URL through Nova AI browser control and joins
as the signed-in Nova AI browser profile. On macOS, the plugin checks for
...
```bash
nova-ai googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
nova-ai googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local Nova AI audio
...
meeting; Nova AI does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Nova AI agent. Be brief.",
...
```bash
nova-ai config validate
nova-ai plugins list | grep -E 'google-meet|voice-call'
nova-ai googlemeet setup
```
...
`NOVA_AI_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes Nova AI requests:
...
`meetings.space.readonly` lets Nova AI resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets Nova AI pass `SpaceConfig` settings such as
...
```bash
nova-ai googlemeet auth login --json
```
...
```bash
NOVA_AI_GOOGLE_MEET_CLIENT_ID="your-client-id" \
NOVA_AI_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
nova-ai googlemeet auth login --json
```
...
NOVA_AI_GOOGLE_MEET_CLIENT_ID="your-client-id" \
NOVA_AI_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
nova-ai googlemeet auth login --json --manual
```
...
support existed, rerun `nova-ai googlemeet auth login --json` so the refresh
...
```bash
nova-ai googlemeet doctor --oauth --json
```
...
```bash
nova-ai googlemeet doctor --oauth --create-space --json
nova-ai googlemeet create --no-join --json
```
...
```bash
nova-ai googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
nova-ai googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `nova-ai googlemeet auth login
...
- `NOVA_AI_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `NOVA_AI_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `NOVA_AI_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `NOVA_AI_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `NOVA_AI_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `NOVA_AI_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `NOVA_AI_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
nova-ai googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
nova-ai googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
nova-ai googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
nova-ai googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
nova-ai googlemeet latest --today
nova-ai googlemeet calendar-events --today --json
nova-ai googlemeet artifacts --event "Weekly sync"
nova-ai googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
nova-ai googlemeet latest --meeting https://meet.google.com/abc-defg-hij
nova-ai googlemeet artifacts --conference-record conferenceRecords/abc123 --json
nova-ai googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
nova-ai googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
Nova AI accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops Nova AI's local/session
...
```bash
nova-ai googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
nova-ai googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
nova-ai googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
nova-ai googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
NOVA_AI_LIVE_TEST=1 \
NOVA_AI_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
nova-ai googlemeet setup --transport chrome-node --mode transcribe
nova-ai googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `NOVA_AI_LIVE_TEST=1` enables guarded live tests.
- `NOVA_AI_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `NOVA_AI_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `NOVA_AI_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `NOVA_AI_GOOGLE_MEET_CLIENT_SECRET`,
  `NOVA_AI_GOOGLE_MEET_ACCESS_TOKEN`, and
  `NOVA_AI_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `NOVA_AI_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Nova AI browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the Nova AI browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, Nova AI reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured Nova AI TTS provider.
...
- `chrome.guestName: "Nova AI Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through Nova AI browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  Nova AI agent in a per-meeting sub-agent session, and spoken back through the
  normal Nova AI TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `nova-ai_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `nova-ai_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional Nova AI agent id for
  `nova-ai_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `nova-ai_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured Nova AI
agent produces the answer, and regular Nova AI TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means Nova AI did
...
transcripts are routed through the configured Nova AI agent, and the answer is
spoken through the normal Nova AI TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Nova AI agent | Normal Nova AI TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal Nova AI tools, it can call `nova-ai_agent_consult`.

The consult tool runs the regular Nova AI agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
Nova AI sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated Nova AI agent workspace, model defaults,
...
```bash
nova-ai googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
nova-ai googlemeet setup
nova-ai nodes status --connected
nova-ai nodes invoke \
  --node parallels-macos \
...
- `nova-ai logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
nova-ai plugins list | grep google-meet
nova-ai googlemeet setup
```
...
```bash
nova-ai plugins enable google-meet
nova-ai plugins enable browser
NOVA_AI_ALLOW_INSECURE_PRIVATE_WS=1 \
  nova-ai node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; Nova AI
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, Nova AI
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `NOVA_AI_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `nova-ai googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the Nova AI Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. Nova AI should click **Use microphone** or, for
...
```bash
nova-ai googlemeet setup
nova-ai googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> Nova AI agent -> TTS talk-back path,
...
run `nova-ai googlemeet status --json <session-id>` after participants speak
...
utterance but Nova AI did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  Nova AI. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
nova-ai googlemeet recover-tab
nova-ai googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
nova-ai googlemeet setup --transport twilio
nova-ai voicecall setup
nova-ai voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
nova-ai voicecall smoke --to "+15555550123"
```
...
```bash
nova-ai voicecall smoke --to "+15555550123" --yes
```
...
- Run `nova-ai googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `nova-ai voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `nova-ai voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `nova-ai logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `nova-ai googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `nova-ai logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: Nova AI owns the
