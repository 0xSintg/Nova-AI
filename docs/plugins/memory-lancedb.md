...
```bash
nova-ai plugins install @nova-ai/memory-lancedb
```

The plugin is published to npm and is not bundled into the Nova AI runtime image.
...
Set `dimensions` for non-standard embedding models. Nova AI knows the
...
`memory_forget` query path, and `nova-ai ltm search`. Auto-recall prefers the
...
```bash
nova-ai ltm list
nova-ai ltm search "project preferences"
nova-ai ltm stats
```
...
```bash
nova-ai ltm query --cols id,text,createdAt --limit 20
nova-ai ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.nova-ai/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/nova-ai",
          storageOptions: {
...
Nova AI treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Nova AI and restart the
Gateway.
...
```bash
nova-ai ltm stats
nova-ai ltm search "recent preference"
```
