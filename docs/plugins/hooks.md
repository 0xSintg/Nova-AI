...
Plugin hooks are in-process extension points for Nova AI plugins. Use them
...
current plugin options; Nova AI injects it per handler without mutating the
...
use `nova-ai sessions` or the Gateway session/status surfaces. When debugging
...
- Nova AI strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, Nova AI stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when Nova AI can
...
identifier when Nova AI can derive one from the session key or delivery
metadata.
...
Nova AI continues; it does not cancel plugin-owned network work unless the
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Nova AI can derive a
...
Codex native `Stop` hooks are relayed into this hook as Nova AI
...
reach the next model turn exactly once. Nova AI drains queued injections before
...
context when syncing external wake schedulers, and keep Nova AI as the
