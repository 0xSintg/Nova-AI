...
read_when:
  - You are building an Nova AI plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Nova AI plugin manifest** only.
...
Nova AI auto-detects those bundle layouts too, but they are not validated
against the `nova-ai.plugin.json` schema described here.

For compatible bundles, Nova AI currently reads bundle metadata plus declared
...
Nova AI runtime expectations.

Every native Nova AI plugin **must** ship a `nova-ai.plugin.json` file in the
**plugin root**. Nova AI uses this manifest to validate configuration
...
`nova-ai.plugin.json` is the metadata Nova AI reads **before it loads your
...
- QA runner metadata the shared `nova-ai qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Nova AI still reads this during the deprecation window.                                                 |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                          |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Nova AI can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                            |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `nova-ai qa` host before plugin runtime loads.                                                                                                                                      |
...
Nova AI reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Nova AI can
...
If a tool has no `toolMetadata`, Nova AI preserves the existing behavior and
...
Nova AI reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, Nova AI falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Nova AI
...
the shared `nova-ai qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `nova-ai qa`, for example `matrix`.    |
...
Nova AI also includes `setup.providers[].envVars` in generic provider auth and
...
Nova AI can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Nova AI treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `nova-ai.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Nova AI reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Nova AI can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `nova-ai migrate`.                                        |
...
Nova AI reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Nova AI can still load the plugin, but
...
the same defaults through `package.json#nova-ai.channel.commands` alongside
...
When `channels.chat` is configured, Nova AI considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Nova AI disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Nova AI
...
Use `modelSupport` when Nova AI should infer your provider plugin from
...
Nova AI applies this precedence:
...
Use `modelCatalog` when Nova AI should know provider model metadata before
...
provider-filtered list uses an alias, Nova AI can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Nova AI model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Nova AI
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Nova AI provider id, for example `z-ai` for a `zai` provider. |
...
### Nova AI Provider Index

The Nova AI Provider Index is Nova AI-owned preview metadata for providers
...
4. Nova AI Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `nova-ai doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `nova-ai.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `nova-ai` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Nova AI must know it before loading plugin code, put it in `nova-ai.plugin.json`
...
`nova-ai` block instead of `nova-ai.plugin.json`.
`nova-ai.bundle` and `nova-ai.bundle.json` are not Nova AI plugin contracts;
native plugins must use `nova-ai.plugin.json` plus the supported
`package.json#nova-ai` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `nova-ai.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `nova-ai.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `nova-ai.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `nova-ai.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `nova-ai.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `nova-ai.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `nova-ai.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `nova-ai.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `nova-ai.install.clawhubSpec` / `nova-ai.install.npmSpec` / `nova-ai.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `nova-ai.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `nova-ai.install.minHostVersion`                                                          | Minimum supported Nova AI host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `nova-ai.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `nova-ai.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `nova-ai.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#nova-ai.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `nova-ai.plugin.json`.

`nova-ai.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-nova-ai-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `nova-ai.setupEntry` when status, channel list,
...
entrypoint fields. For example, `nova-ai.runtimeExtensions` cannot make an
escaping `nova-ai.extensions` path loadable.

`nova-ai.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `nova-ai doctor --fix`.

`nova-ai.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`nova-ai.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Nova AI discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with Nova AI
3. **Global install** — plugins installed into the global Nova AI plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `nova-ai.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Nova AI plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `Nova AIPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
