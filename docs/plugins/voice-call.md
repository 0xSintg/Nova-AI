...
read_when:
  - You want to place an outbound voice call from Nova AI
...
Voice calls for Nova AI via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        nova-ai plugins install @nova-ai/voice-call
        ```
      </Tab>
      <Tab title="From a local folder (dev)">
        ```bash
        PLUGIN_SRC=./path/to/local/voice-call-plugin
        nova-ai plugins install "$PLUGIN_SRC"
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `nova-ai doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `nova-ai doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `nova-ai_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Nova AI tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `nova-ai_agent_consult`.
...
configured Nova AI agent without paying a full agent-consult round trip on
...
`nova-ai_agent_consult` still run the full Nova AI agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call nova-ai_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `nova-ai doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
nova-ai voicecall call --to "+15555550123" --message "Hello from Nova AI"
nova-ai voicecall start --to "+15555550123"   # alias for call
nova-ai voicecall continue --call-id <id> --message "Any questions?"
nova-ai voicecall speak --call-id <id> --message "One moment"
nova-ai voicecall dtmf --call-id <id> --digits "ww123456#"
nova-ai voicecall end --call-id <id>
nova-ai voicecall status --call-id <id>
nova-ai voicecall tail
nova-ai voicecall latency                      # summarize turn latency from logs
nova-ai voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL Nova AI reconstructs
...
Use `nova-ai logs --follow` for the live phase trace. A healthy Twilio Meet
...
`nova-ai voicecall tail` still shows persisted call records; it is useful for
...
- `nova-ai logs --follow` shows realtime TwiML served, the realtime bridge
