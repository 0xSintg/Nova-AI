---
summary: "Step-by-step guide to building a messaging channel plugin for Nova AI"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Nova AI to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Nova AI to a
...
<Info>
  If you have not built any Nova AI plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Nova AI keeps one
...
`defineChannelMessageAdapter` from `nova-ai/plugin-sdk/channel-message`. The
...
receive/send lifecycle helpers on `nova-ai/plugin-sdk/channel-message`.

Channels migrating inbound authorization can use the experimental
`nova-ai/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
Use `nova-ai/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `nova-ai/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `nova-ai/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `nova-ai/plugin-sdk/approval-auth-runtime`
- `nova-ai/plugin-sdk/approval-client-runtime`
- `nova-ai/plugin-sdk/approval-delivery-runtime`
- `nova-ai/plugin-sdk/approval-gateway-runtime`
- `nova-ai/plugin-sdk/approval-handler-adapter-runtime`
- `nova-ai/plugin-sdk/approval-handler-runtime`
- `nova-ai/plugin-sdk/approval-native-runtime`
- `nova-ai/plugin-sdk/approval-reply-runtime`
- `nova-ai/plugin-sdk/channel-runtime-context`

Likewise, prefer `nova-ai/plugin-sdk/setup-runtime`,
`nova-ai/plugin-sdk/setup-runtime`,
`nova-ai/plugin-sdk/reply-runtime`,
`nova-ai/plugin-sdk/reply-dispatch-runtime`,
`nova-ai/plugin-sdk/reply-reference`, and
`nova-ai/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `nova-ai/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `nova-ai/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `nova-ai/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `nova-ai.setupEntry` in
...
- use the broader `nova-ai/plugin-sdk/setup` seam only when you also need the
...
- `nova-ai/plugin-sdk/account-core`,
  `nova-ai/plugin-sdk/account-id`,
  `nova-ai/plugin-sdk/account-resolution`, and
  `nova-ai/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `nova-ai/plugin-sdk/inbound-envelope` and
  `nova-ai/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `nova-ai/plugin-sdk/messaging-targets` for target parsing/matching
- `nova-ai/plugin-sdk/outbound-media` and
  `nova-ai/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `nova-ai/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `nova-ai/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `nova-ai/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `nova-ai/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `nova-ai/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `nova-ai/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
} from "nova-ai/plugin-sdk/channel-inbound";
...
`nova-ai/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#nova-ai-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/nova-ai-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "nova-ai": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Nova AI to Acme Chat."
        }
      }
    }
    ```

    ```json nova-ai.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { Nova AIConfig } from "nova-ai/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "nova-ai/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Nova AI
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "nova-ai/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Nova AI loads this instead of the full entry when the channel is disabled
    ...
    `nova-ai/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Nova AI. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Nova AI.
...
├── package.json              # nova-ai.channel metadata
├── nova-ai.plugin.json      # Manifest with config schema
