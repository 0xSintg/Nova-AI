---
summary: "Create your first Nova AI plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Nova AI plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Nova AI without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Nova AI repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
nova-ai plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Nova AI loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Nova AI to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Nova AI model fallback.
  </Card>
...
```json nova-ai.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Nova AI",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so Nova AI can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    nova-ai plugins inspect my-plugin --runtime --json
    ```
    ...
    `nova-ai demo-plugin ping`.

    For a bundled plugin in this repository, Nova AI discovers source-checkout
  ...
  <Step title="Install">
    ...
    ```bash
    nova-ai plugins install clawhub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified Nova AI runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so Nova AI can avoid
...
```typescript
import { definePluginEntry } from "nova-ai/plugin-sdk";
```
...
`nova-ai/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `nova-ai` metadata</Check>
<Check>**nova-ai.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [nova-ai/nova-ai](https://github.com/nova-ai/nova-ai/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Nova AI X account [@nova-ai](https://x.com/nova-ai) for release announcements.
