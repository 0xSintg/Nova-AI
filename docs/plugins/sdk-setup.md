...
read_when:
  ...
  - You are defining plugin config schemas or package.json nova-ai metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`nova-ai.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `nova-ai` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/nova-ai-my-channel",
      "version": "1.0.0",
      "type": "module",
      "nova-ai": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ClawHub baseline">
    ```json nova-ai-clawhub-package.json
    {
      "name": "@myorg/nova-ai-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "nova-ai": {
        "extensions": ["./index.ts"],
...
`nova-ai.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`nova-ai.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`                            | Minimum supported Nova AI version in the form `>=x.y.z` or `>=x.y.z-prerelease`. |
...
<AccordionGroup>
  ...
  <Accordion title="Pinned npm installs">
    ...
    ```json
    {
      "nova-ai": {
        "install": {
          "npmSpec": "@wecom/wecom-nova-ai-plugin@1.2.3",
...
When enabled, Nova AI loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `nova-ai.plugin.json` in the package root. Nova AI uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Nova AI",
  "configSchema": {
    "type": "object",
...
The `setup-entry.ts` file is a lightweight alternative to `index.ts` that Nova AI loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).
...
Bundled workspace channels that keep setup-safe exports in sidecar modules can use `defineBundledChannelSetupEntry(...)` from `nova-ai/plugin-sdk/channel-entry-contract` instead of `defineSetupPluginEntry(...)`. That bundled contract also supports an optional `runtime` export so setup-time runtime wiring can stay lightweight and explicit.

<AccordionGroup>
  <Accordion title="When Nova AI uses setupEntry instead of the full entry">
...
CLI wizard locale (`NOVA_AI_LOCALE`, then system locale variables) and falls
...
```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "nova-ai/plugin-sdk/channel-config-schema";
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so Nova AI can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "nova-ai/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `nova-ai.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `nova-ai onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "nova-ai/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `nova-ai/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `nova-ai/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `nova-ai/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "nova-ai/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/nova-ai-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="npm">
    ```bash
    nova-ai plugins install @myorg/nova-ai-my-plugin
    ```
  ...
  <Tab title="ClawHub only">
    ```bash
    nova-ai plugins install clawhub:@myorg/nova-ai-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    ...
    ```bash
    nova-ai plugins install npm:@myorg/nova-ai-my-plugin
    ```
...
```bash
nova-ai plugins install <package-name>
```

<Info>
For npm-sourced installs, `nova-ai plugins install` installs the package under `~/.nova-ai/npm` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>
...
Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged Nova AI startup never repairs or mirrors plugin dependencies.
