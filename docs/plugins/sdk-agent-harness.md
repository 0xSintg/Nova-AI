...
An **agent harness** is the low level executor for one prepared Nova AI agent
...
runtime and the normal Nova AI provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Nova AI
  session transcript
...
Before a harness is selected, Nova AI has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Nova AI transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Nova AI-owned
...
**Import:** `nova-ai/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "nova-ai/plugin-sdk/agent-harness";
...
Nova AI chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, Nova AI uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, Nova AI does not
...
`agents.list[].agentRuntime`, and `NOVA_AI_AGENT_RUNTIME`. `/status` shows the
...
Nova AI. The harness then claims that provider in `supports(...)`.
...
- app-server request: Nova AI sends the bare model id to Codex and lets the
...
Nova AI requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Nova AI only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Nova AI to the newer tested stable line.
...
`nova-ai/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Nova AI's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded Nova AI
...
`nova-ai doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Nova AI still owns the chat channel,
...
By default, Nova AI uses `auto` provider/model runtime policy: registered
...
Keep that binding explicitly associated with the Nova AI session, and keep
mirroring user-visible assistant/tool output into the Nova AI transcript.

The Nova AI transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Nova AI can
clear it when the owning Nova AI session is reset.

## Tool and media results

Core constructs the Nova AI tool list and passes it into the prepared attempt.
