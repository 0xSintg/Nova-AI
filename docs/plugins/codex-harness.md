---
summary: "Run Nova AI embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
...
The bundled `codex` plugin lets Nova AI run embedded OpenAI agent turns
...
app-server execution. Nova AI still owns chat channels, session files, model
selection, Nova AI dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
Nova AI starts Codex app-server threads with Codex native code mode enabled
while leaving code-mode-only off by default. That keeps Codex native workspace
and code capabilities available while Nova AI dynamic tools continue through
...
- Nova AI with the bundled `codex` plugin available.
...
- Codex auth available through `nova-ai models auth login --provider openai-codex`,
...
Most users who want Codex in Nova AI want this path: sign in with a
...
```bash
nova-ai models auth login --provider openai-codex
```
...
harness options in Nova AI config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                              |
| -------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | Nova AI config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | Nova AI config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | Nova AI agent config              |
| Sign in with Codex OAuth               | `nova-ai models auth login --provider openai-codex`                             | CLI auth profile                   |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + Nova AI config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | Nova AI model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "pi"` with normal OpenAI auth                | Nova AI model/provider config     |
...
engine, Codex compacts through its native app-server thread state, so Nova AI
ignores those local summarizer overrides at runtime and `nova-ai doctor --fix`
...
`agents.defaults.compaction.provider`. `nova-ai doctor --fix` migrates the old
...
- Do not use `openai-codex/gpt-*` in config. Run `nova-ai doctor --fix` to
...
| Legacy config                                        | `openai-codex/gpt-*`                                             | `nova-ai doctor --fix` rewrites it     | Do not write new config this way   |
...
With Codex forced, Nova AI fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts Nova AI's managed Codex binary locally with stdio
...
implicit YOLO posture, Nova AI selects allowed guardian permissions instead.
When an Nova AI sandbox is active for the session, Nova AI narrows Codex
...
Nova AI sandbox egress policy: Docker `network: "none"` stays offline, while
...
supports Nova AI text commands.
...
- `/codex resume <thread-id>` attaches the current Nova AI session to an
...
When Nova AI sees a ChatGPT subscription-style Codex auth profile, it removes
...
If a subscription profile hits a Codex usage limit, Nova AI records the reset
...
For local stdio app-server launches, Nova AI sets `CODEX_HOME` to a per-agent
...
default. Nova AI preserves the normal process `HOME`; Codex-run subprocesses
...
Nova AI removes `CODEX_HOME` and `HOME` from this list during local launch
normalization: `CODEX_HOME` stays per-agent, and `HOME` stays inherited so
subprocesses can use normal user-home state.

Codex dynamic tools default to `searchable` loading. Nova AI does not expose
...
Nova AI integration tools such as messaging, media, cron, browser, nodes,
gateway, `heartbeat_respond`, and `web_search` are available through Codex tool
search under the `nova-ai` namespace, keeping the initial model context
smaller.
...
Nova AI or ACP delegation is still available through the `nova-ai` dynamic
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put Nova AI dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional Nova AI dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Nova AI builds its inherited environment. Nova AI keeps per-agent `CODEX_HOME` and inherited `HOME` for local launches.                                                                                          |
| `codeModeOnly`                | `false`                                                | Opt into Codex's code-mode-only tool surface. Nova AI dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                          |
| `requestTimeoutMs`            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                 |
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while Nova AI waits for `turn/completed`. Raise this for slow post-tool or status-only synthesis phases.                                                                                                                 |
...
| `sandbox`                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an Nova AI sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the Nova AI sandbox egress setting. |
| `approvalsReviewer`           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                          |
| `serviceTier`                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                             |

Nova AI-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 30 second
Nova AI watchdog by default. A positive per-call `timeoutMs` argument extends
...
budgets are capped at 600000 ms. On timeout, Nova AI aborts the tool signal
...
After Codex accepts a turn, and after Nova AI responds to a turn-scoped
...
quiet for `appServer.turnCompletionIdleTimeoutMs`, Nova AI best-effort
interrupts the Codex turn, records a diagnostic timeout, and releases the
Nova AI session lane so follow-up chat messages are not queued behind a stale
...
goes quiet without `turn/completed`, Nova AI best-effort interrupts the native
...
- `NOVA_AI_CODEX_APP_SERVER_BIN`
- `NOVA_AI_CODEX_APP_SERVER_ARGS`
- `NOVA_AI_CODEX_APP_SERVER_MODE=yolo|guardian`
- `NOVA_AI_CODEX_APP_SERVER_APPROVAL_POLICY`
- `NOVA_AI_CODEX_APP_SERVER_SANDBOX`

`NOVA_AI_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`NOVA_AI_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`NOVA_AI_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
capabilities in the same Codex thread as the Nova AI harness turn. Nova AI
does not translate Codex plugins into synthetic `codex_plugin_*` Nova AI
dynamic tools.
...
Thread app config is computed when Nova AI establishes a Codex harness session
...
The short version: Nova AI does not vendor the desktop-control app or execute
...
- Nova AI dynamic tools are supported. Codex asks Nova AI to execute those
  tools, so Nova AI remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  Nova AI can observe or block selected native events through the supported
  relay, but it does not rewrite native tool arguments.
- Codex owns native compaction unless the active Nova AI context engine
  declares `ownsCompaction: true`. Nova AI keeps a transcript mirror for
...
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching Nova AI provider/model settings.
- `tool_result_persist` applies to Nova AI-owned transcript tool results, not
...
**Nova AI uses PI instead of Codex:** make sure the model ref is
...
Ask affected collaborators to run this read-only command on their Nova AI host:
...
(
  ...
  if ls /tmp/nova-ai/nova-ai-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/nova-ai/nova-ai-*.log 2>/dev/null || true
  else
    journalctl --user -u nova-ai-gateway --since today --no-pager 2>/dev/null \
...
**Legacy `openai-codex/*` config remains:** run `nova-ai doctor --fix`.
...
`0.125.0-alpha.2` or `0.125.0+custom` are rejected because Nova AI tests the
