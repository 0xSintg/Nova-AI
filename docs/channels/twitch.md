...
read_when:
  - Setting up Twitch chat integration for Nova AI
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support via IRC connection. Nova AI connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

<Note>
Twitch ships as a bundled plugin in current Nova AI releases, so normal packaged builds do not need a separate install.
</Note>
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    nova-ai plugins install @nova-ai/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    nova-ai plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `NOVA_AI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "nova-ai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NOVA_AI_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    NOVA_AI_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
