...
title: IRC
read_when:
  - You want to connect Nova AI to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

Use IRC when you want Nova AI in classic channels (`#room`) and direct messages.
IRC ships as a bundled plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.nova-ai/nova-ai.json`.
2. Set at least:
...
- IRC uses raw TCP/TLS sockets outside Nova AI operator-managed forward proxy routing. In deployments that require all egress through that forward proxy, set `channels.irc.enabled=false` unless direct IRC egress is explicitly approved.
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Nova AI defaults to **mention-gating** in group contexts.
