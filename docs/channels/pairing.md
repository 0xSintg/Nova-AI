...
read_when:
  ...
  - Reviewing Nova AI security posture
title: "Pairing"
---

"Pairing" is Nova AI's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `nova-ai-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.nova-ai/credentials/`:
...
3. On your phone, open the Nova AI iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, Nova AI keeps the existing approval as-is and creates a fresh pending upgrade request. Use `nova-ai devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `nova-ai nodes pending|approve|reject|remove|rename`) is a
