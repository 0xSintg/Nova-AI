...
- bot user = the Nova AI Matrix account that sends the reply
...
If no pushers come back, fix normal Matrix push delivery for this account before continuing.
  ...
  <Step title="Install the override push rule">
    Nova AI marks finalized text-only preview edits with `content["com.nova-ai.finalized_preview"] = true`. Install a rule that matches that marker plus the bot MXID as sender:

```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/nova-ai-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.nova-ai\\.finalized_preview",
        "value": true
      },
...
```
    ...
    - `nova-ai-finalized-preview-botname`: a rule ID unique per bot per recipient (pattern: `nova-ai-finalized-preview-<botname>`)
    - `@bot:example.org`: your Nova AI bot MXID, not the recipient's
...
Push rules are keyed by `ruleId`: re-running `PUT` against the same ID updates a single rule. For multiple Nova AI bots notifying the same recipient, create one rule per bot with a distinct sender match.
