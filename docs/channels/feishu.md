...
<Note>
Requires Nova AI 2026.4.25 or above. Run `nova-ai --version` to check. Upgrade with `nova-ai update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  nova-ai channels login --channel feishu
  ```
...
```bash
nova-ai pairing list feishu
nova-ai pairing approve feishu <CODE>
```
...
4. Check logs: `nova-ai logs --follow`
...
5. Ensure the gateway is running: `nova-ai gateway status`
6. Check logs: `nova-ai logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `nova-ai channels login --channel feishu`
...
3. Restart the gateway: `nova-ai gateway restart`
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Nova AI
...
conversion fails, Nova AI falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, Nova AI
hydrates it from Feishu before routing the turn. Normal group replies that
Nova AI turns into threads keep using the reply root message ID (`om_*`) so the
