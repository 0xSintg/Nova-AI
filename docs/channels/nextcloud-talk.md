...
Nextcloud Talk ships as a bundled plugin in current Nova AI releases, so
...
```bash
nova-ai plugins install @nova-ai/nextcloud-talk
```
...
```bash
nova-ai plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Nova AI" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Nova AI:
   ...
   ```bash
   nova-ai channels add --channel nextcloud-talk \
...
- Approve via:
  - `nova-ai pairing list nextcloud-talk`
  - `nova-ai pairing approve nextcloud-talk <CODE>`
