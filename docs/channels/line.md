...
read_when:
  - You want to connect Nova AI to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Nova AI via the LINE Messaging API. The plugin runs as a webhook
...
```bash
nova-ai plugins install @nova-ai/line
```

Local checkout (when running from a git repo):

```bash
nova-ai plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Nova AI applies strict pre-auth body limits and timeout before verification.
- Nova AI processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
nova-ai pairing list line
nova-ai pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.nova-ai/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Nova AI validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
