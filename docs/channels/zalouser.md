...
read_when:
  - Setting up Zalo Personal for Nova AI
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Nova AI.
...
Zalo Personal ships as a bundled plugin in current Nova AI releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `nova-ai plugins install @nova-ai/zalouser`
- Pinned version: `nova-ai plugins install @nova-ai/zalouser@2026.5.2`
- Or from a source checkout: `nova-ai plugins install ./path/to/local/zalouser-plugin`
...
1. Ensure the Zalo Personal plugin is available.
   - Current packaged Nova AI releases already bundle it.
   - Older/custom installs can add it manually with the commands above.
2. Login (QR, on the Gateway machine):
   - `nova-ai channels login --channel zalouser`
...
```bash
nova-ai directory self --channel zalouser
nova-ai directory peers list --channel zalouser --query "name"
nova-ai directory groups list --channel zalouser --query "work"
```
...
- `nova-ai pairing list zalouser`
- `nova-ai pairing approve zalouser <code>`
...
- On startup, Nova AI resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, Nova AI stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Nova AI state. Example:
...
- Nova AI sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Nova AI sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `nova-ai channels logout --channel zalouser && nova-ai channels login --channel zalouser`
...
- The channel now runs fully in Nova AI without external CLI binaries.
