...
Tencent Yuanbao is Tencent's AI assistant platform. The Nova AI channel plugin
connects Yuanbao bots to Nova AI over WebSocket so they can interact with users
...
> **Requires Nova AI 2026.4.10 or above.** Run `nova-ai --version` to check. Upgrade with `nova-ai update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  nova-ai channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
nova-ai channels login --channel yuanbao
```
...
```bash
nova-ai pairing list yuanbao
nova-ai pairing approve yuanbao <CODE>
```
