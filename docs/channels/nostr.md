...
read_when:
  - You want Nova AI to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Nova AI to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Nova AI releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`nova-ai onboard`) and `nova-ai channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
nova-ai plugins install @nova-ai/nostr
```
...
```bash
nova-ai plugins install --link <path-to-local-nostr-plugin>
```
...
nova-ai channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "nova-ai@example.com",
        lud16: "nova-ai@example.com",
      },
    },
  },
}
