...
read_when:
  - Connecting Nova AI to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects Nova AI to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want an Nova AI agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.
...
When an account sets `agentId`, Nova AI requires the explicit
...
```bash
nova-ai message send --channel clickclack --target channel:general --message "hello"
nova-ai message send --channel clickclack --target dm:usr_123 --message "hello"
nova-ai message send --channel clickclack --target thread:msg_123 --message "following up"
```
...
Nova AI only needs `bot:write` for normal agent chat.
