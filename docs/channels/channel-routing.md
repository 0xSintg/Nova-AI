...
Nova AI routes replies **back to the channel where a message came from**. The
...
Nova AI infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Nova AI still records inbound session metadata, but it
...
when a guarded path must not create a new Nova AI session. In that mode,
Nova AI may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Nova AI would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.nova-ai/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.nova-ai`):

- `~/.nova-ai/agents/<agentId>/sessions/sessions.json`
