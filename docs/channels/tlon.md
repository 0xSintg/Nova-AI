...
Tlon is a decentralized messenger built on Urbit. Nova AI connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Nova AI releases, so normal packaged
...
```bash
nova-ai plugins install @nova-ai/tlon
```
...
```bash
nova-ai plugins install ./path/to/local/tlon-plugin
```
...
By default, Nova AI blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `nova-ai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Nova AI replies in-thread.
