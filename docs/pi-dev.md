...
A sane workflow for working on the Pi integration in Nova AI.
...
```bash
NOVA_AI_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm nova-ai agent --message "Hello" --thinking low`
...
State lives under the Nova AI state directory. Default is `~/.nova-ai`. If `NOVA_AI_STATE_DIR` is set, use that directory instead.

To reset everything:

- `nova-ai.json` for config
